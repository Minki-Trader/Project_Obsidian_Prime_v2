...
class ClassifyScopeTests(unittest.TestCase):
    ...
    def test_classify_scope_uses_helper_pack_specific_label(self) -> None:
        comparison = self.build_comparison(exact_parity=False, tolerance_parity=True, traceable=True)
        comparison["identity"]["expected"] = {
            "fixture_set_id": "fixture_fpmarkets_v2_runtime_helper_0001",
        }

        self.assertEqual(
            self.module.classify_scope(
                comparison,
                {"fixture_set_id": "fixture_fpmarkets_v2_runtime_helper_0001"},
            ),
            "first_helper_focused_pack_tolerance_closed_identity_trace_materialized_exact_open",
        )

    def test_classify_scope_uses_broader_reinforcement_label(self) -> None:
        comparison = self.build_comparison(exact_parity=False, tolerance_parity=True, traceable=True)
        comparison["identity"]["expected"] = {
        ...
        self.assertEqual(
            ...
            "additional_broader_reinforcement_pack_tolerance_closed_identity_trace_materialized_exact_open",
        )
    ...
    def test_render_report_uses_helper_specific_stage05_wording(self) -> None:
        comparison = {
            "aggregate_results": {
                "exact_parity": False,
                "tolerance_parity": True,
        ...
        python_snapshot = {
            ...
            "fixtures": {
                "fix_cash_close_boundary_1555_0002": {
                    "fixture_id": "fix_cash_close_boundary_1555_0002",
                    "timestamp_utc": "2026-03-31T19:55:00Z",
                    "timestamp_america_new_york": "2026-03-31T15:55:00-04:00",
                    "valid_row": True,
                    "selection_stratum": "cash_close_boundary",
                    "selection_bucket": "cash_close_boundary_1555",
                },
                "fix_negative_off_hours_post_close_0002": {
                    "fixture_id": "fix_negative_off_hours_post_close_0002",
                    "timestamp_utc": "2025-09-30T20:55:00Z",
                    "timestamp_america_new_york": "2025-09-30T16:55:00-04:00",
                    "valid_row": False,
                    "selection_stratum": "off_hours_external_alignment_missing",
                    "selection_bucket": "negative_off_hours_post_close",
                },
            },
        }
        mt5_request = {
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            python_snapshot_path.write_text(json.dumps(python_snapshot), encoding="utf-8")
            mt5_request_path.write_text(json.dumps(mt5_request), encoding="utf-8")
            mt5_snapshot_path.write_text('{"fixture_id":"fix_cash_close_boundary_1555_0002"}\n', encoding="utf-8")
        ...
        self.assertIn("first_helper_focused_pack_tolerance_closed_identity_trace_materialized_exact_open", rendered)
        self.assertIn("no runtime-helper parity closure yet", rendered)
        self.assertIn("keep broader_0002 as the active broader evidence", rendered)

    def test_render_report_uses_broader_reinforcement_wording(self) -> None:
        comparison = {
            "aggregate_results": {
                "exact_parity": False,
                "tolerance_parity": True,
        ...
        python_snapshot = {
            ...
            "fixtures": {
                "fix_regular_cash_session_0001": {
                    "fixture_id": "fix_regular_cash_session_0001",
                    "timestamp_utc": "2024-05-01T16:35:00Z",
                    "timestamp_america_new_york": "2024-05-01T12:35:00-04:00",
                    "valid_row": True,
                    "selection_stratum": "regular_cash_session",
                    "selection_bucket": "regular_cash_session",
                },
        ...
        self.assertIn("Stage 05 closure read", rendered)
        self.assertIn("broader_0002 plus helper_0001 as retained evidence", rendered)
