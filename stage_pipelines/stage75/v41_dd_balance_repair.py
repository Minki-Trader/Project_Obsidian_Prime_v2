...
from stage_pipelines.stage73 import v41_tp_risk_followup as s73  # noqa: E402
...
STAGE_ID = "75_adapter_research__v41_dd_balance_repair"
RUN_NUMBER = "run75A"
RUN_ID = "run75A_stage75_v41_dd_balance_repair_v1"
PACKET_ID = "stage75_v41_dd_balance_repair_v1"
PARENT_RUN_ID = "run74A_stage74_v41_tp_risk_followup_review_v1"
SOURCE_STAGE74_ID = "74_adapter_research__v41_tp_risk_followup_review"
SOURCE_STAGE74_COMMIT = "70735d9f461d916b4c7d340039a5bbf8bd23b4a2"
SOURCE_STAGE73_LATEST_COMMIT = "76db6f199ff917da2f8311544f68dc6f24612e0e"
NEXT_STAGE_ID = "76_adapter_research__v41_dd_balance_followup_review"
NEXT_RUN_ID = "run76A_stage76_v41_dd_balance_followup_review_v1"
NEXT_PACKET_ID = "stage76_v41_dd_balance_followup_review_v1"
SOURCE_ADAPTER_ID = "stage73_v41_tp_risk_followup_surface"
...
COMMON_ROOT = f"OPV2/s75a/{RUN_NUMBER}"

SUMMARY_JSON_PATH = REVIEWS_ROOT / "stage75_v41_dd_balance_summary.json"
SUMMARY_CSV_PATH = REVIEWS_ROOT / "stage75_v41_dd_balance_summary.csv"
REPORT_PATH = REVIEWS_ROOT / "stage75_v41_dd_balance_report.md"
SEGMENT_KPI_PATH = REVIEWS_ROOT / "stage75_segment_kpi_summary.csv"
RISK_ATR_TELEMETRY_PATH = REVIEWS_ROOT / "stage75_risk_atr_telemetry.csv"
GATE_FEATURE_SUMMARY_PATH = REVIEWS_ROOT / "stage75_gate_feature_summary.csv"
TIER_B_DIAGNOSTIC_PATH = REVIEWS_ROOT / "stage75_tier_b_diagnostic_summary.csv"
DECISION_PATH = REVIEWS_ROOT / "stage75_decision.md"
AUDIT_CSV_PATH = REVIEWS_ROOT / "stage75_trade_audit.csv"
...
SOURCE_RUN_ROOT = s73.SOURCE_RUN_ROOT
SOURCE_VARIANT_ROOT = s73.SOURCE_VARIANT_ROOT
SOURCE_MODEL = s73.SOURCE_MODEL
SOURCE_VAL_INI = s73.SOURCE_VAL_INI
SOURCE_OOS_INI = s73.SOURCE_OOS_INI

VARIANTS = (
    repair.RepairVariant(
        adapter_id="s75_v41_h3_risk45_gate08_tp35",
        label="run50BN_v41_h3_risk45_gate08_tp35",
        atr_enabled=True,
        ...
        max_hold_bars=3,
        notes="Stage75 DD/net balance: Stage73 best TP3.5 with risk cap lowered to 4.5%.",
    ),
    repair.RepairVariant(
        adapter_id="s75_v41_h3_risk475_gate08_tp35",
        label="run50BN_v41_h3_risk475_gate08_tp35",
        atr_enabled=True,
        model_risk_enabled=True,
        fixed_lot=0.25,
        atr_stop_multiplier=2.5,
        ...
        close_only_on_opposite_signal=True,
        max_hold_bars=3,
        notes="Stage75 DD/net balance: midpoint risk cap 4.75% with Stage73 best TP3.5.",
    ),
    repair.RepairVariant(
        adapter_id="s75_v41_h3_risk5_gate08_tp35_cd12",
        label="run50BN_v41_h3_risk5_gate08_tp35_cd12",
        atr_enabled=True,
        model_risk_enabled=True,
        fixed_lot=0.25,
        atr_stop_multiplier=2.5,
        ...
        close_only_on_opposite_signal=True,
        max_hold_bars=3,
        notes="Stage75 DD guard: keep Stage73 best risk/TP but extend same-direction cooldown to 12 bars.",
    ),
)
...
CONTEXT_GATE_SPECS = {
    variant.adapter_id: {
        "gate_column": "stage75_gate_margin_lt_008_short",
        "gate_type": "margin",
        "threshold": 0.08,
        "block_mode": "short",
...
def decide(summary_rows: Sequence[Mapping[str, Any]], segment_rows: Sequence[Mapping[str, Any]], external: str) -> str:
    if external != "completed":
        return "continue_stage75_due_to_incomplete_runtime"
    ...
    if val_net >= 850 and oos_net >= 470 and val_dd <= 22.0 and oos_dd <= 20.0 and val_pf >= 1.48 and oos_pf >= 1.38 and not reasons:
        return "proceed_to_stage76_candidate_review"
    if val_net >= 650 and oos_net >= 360 and val_dd <= 25.0 and oos_dd <= 20.0 and val_pf >= 1.42 and oos_pf >= 1.35:
        return "continue_v41_dd_balance_repair_in_stage76"
    if val_pf < 1.0 or val_net <= 0:
        return "open_new_model_source_branch_in_stage76"
    return "continue_v41_dd_balance_repair_in_stage76"
...
def report_markdown(
    ...
    return f"""# Stage75 V41 DD/Net Balance Repair Report(75단계 V41 손실률/순손익 균형 수리 보고서)

- run(실행): `{RUN_ID}`
- source_stage(원천 단계): `{SOURCE_STAGE74_ID}`
- source_stage74_commit(원천 74단계 커밋): `{SOURCE_STAGE74_COMMIT}`
...
def decision_markdown(decision: str, external: str) -> str:
    return f"""# Stage75 Decision(75단계 판정)

decision(판정): `{decision}`

Stage75(75단계)는 Stage73(73단계)에서 회복된 v41 TP/risk(V41 익절 폭/위험) 표면의 DD/net balance(손실률/순손익 균형)를 좁게 재측정했다.
            ...
            rows.append(
                {
                    ...
                    "split": split_name,
                    "tier_b_policy": "diagnostic_missing_required_but_disabled_for_this_v41_dd_balance_repair",
                    ...
                    "reason": "Stage75 isolates v41 DD/net balance repair first; Tier B fallback remains diagnostic and disabled.",
                }
            )
    return rows
            ...
            rows.append(
                {
                    ...
                    "artifact_type": "stage75_v41_dd_balance_repair_evidence",
                    "path": rel(path),
                    "sha256": sha256_file_lf_normalized(path),
                    "stage_id": STAGE_ID,
                    "run_id": RUN_ID,
                    "created_at_utc": created,
                    "notes": "Stage75 v2-native v41 DD/net balance repair artifact.",
                }
            )
            ...
            rows.append(
                {
                    ...
                    "run_id": RUN_ID,
                    "created_at_utc": created,
                    "notes": "Actual Stage75 MT5 Strategy Tester HTML report.",
                }
            )
    return rows
...
def write_run_identity(result: Mapping[str, Any]) -> None:
    s73.stage72.write_json(
        RUN_ROOT / "run_manifest.json",
        {
            ...
            "source_stage74_pushed_commit": SOURCE_STAGE74_COMMIT,
        ...
        [
            {
                "run_id": RUN_ID,
                "stage_id": STAGE_ID,
                "lane": "baseline_adapter_v2_native_v41_dd_balance_repair",
                "status": status,
                "judgment": decision,
                ...
                "notes": ledger_pairs(
                    (
                        ...
                        ("source_stage74_commit", SOURCE_STAGE74_COMMIT),
                        ("source_stage73_latest_commit", SOURCE_STAGE73_LATEST_COMMIT),
                        ("target_surface", TARGET_SURFACE),
        ...
        alpha_rows = [
            {
                ...
                "tier_scope": "Tier A+B",
                "kpi_scope": "stage75_v41_dd_balance_repair",
                ...
                "external_verification_status": external,
                "notes": "Stage75 run materialized or blocked before KPI records were available.",
            }
        ]
...
def write_packet_files(result: Mapping[str, Any], decision: str, ledger_payload: Mapping[str, Any]) -> None:
    ...
    s73.stage72.write_json(PACKET_ROOT / "routing_receipt.json", {"packet_id": PACKET_ID, "stage_id": STAGE_ID, "primary_family": "experiment_design", "primary_skill": "obsidian-experiment-design", "support_skills": ["obsidian-exploration-mandate", "obsidian-performance-attribution", "obsidian-result-judgment"], "required_gates": ["runtime_evidence_gate", "kpi_contract_audit", "result_judgment_gate"], "status": status})
    s73.stage72.write_json(PACKET_ROOT / "runtime_evidence_gate.json", {"packet_id": PACKET_ID, "run_id": RUN_ID, "external_verification_status": result.get("external_verification_status"), "completed_attempt_count": result.get("completed_attempt_count"), "expected_attempt_count": result.get("expected_attempt_count"), "gate_feature_summary_path": rel(GATE_FEATURE_SUMMARY_PATH), "claim_boundary": BOUNDARY})
    s73.stage72.write_json(PACKET_ROOT / "result_judgment_gate.json", {"packet_id": PACKET_ID, "run_id": RUN_ID, "decision": decision, "legacy_relation": "lesson_only_target_surface_no_code_copy", "forbidden_claims": ["deployment", "live_readiness", "production_baseline", "operating_promotion", "operating_reference", "runtime_authority", "legacy_inheritance"]})
    s73.stage72.write_json(PACKET_ROOT / "aggregate_summary.json", {"packet_id": PACKET_ID, "stage_id": STAGE_ID, "run_id": RUN_ID, "decision": decision, "source_stage74_commit": SOURCE_STAGE74_COMMIT, "gate_feature_summary_path": rel(GATE_FEATURE_SUMMARY_PATH), "ledger_payload": ledger_payload, "overall_goal_complete": False})


def create_next_stage(decision: str, external: str) -> None:
    s73.stage72.write_md(NEXT_STAGE_ROOT / "00_spec" / "stage_brief.md", f"""# {NEXT_STAGE_ID}

Stage76(76단계)는 Stage75(75단계)의 DD/net balance(손실률/순손익 균형) 결과를 받아 candidate review(후보 검토), continued repair(계속 수리), 또는 another model source(다른 모델 원천)를 고르는 follow-up(후속) 단계다.

## Bounded Question(경계 질문)

Stage75(75단계)의 DD/net balance(손실률/순손익 균형) 수리가 34D KPI(34D 핵심 성과 지표) 격차를 줄일 만큼 강한가?

Effect(효과): Stage76(76단계)는 다음 판단을 하나의 경계 질문으로 좁힌다.

## Boundary(경계)

`{BOUNDARY}`
""")
    s73.stage72.write_md(NEXT_STAGE_ROOT / "01_inputs" / "input_refs.md", f"""# Stage76 Input References(76단계 입력 참조)

- source_stage(원천 단계): `{STAGE_ID}`
- source_run(원천 실행): `{RUN_ID}`
- source_decision(원천 판정): `{decision}`
- source_external_verification_status(원천 외부 검증 상태): `{external}`
- stage75_report(75단계 보고서): `{rel(REPORT_PATH)}`
- stage75_decision(75단계 판정): `{rel(DECISION_PATH)}`
- summary(요약): `{rel(SUMMARY_CSV_PATH)}`
- target_surface(목표 표면): `{TARGET_SURFACE}`

Effect(효과): Stage76(76단계)는 v2-native evidence(v2 고유 근거)만 이어받아 34D KPI(34D 핵심 성과 지표) 격차 축소를 계속한다.
""")
    s73.stage72.write_md(NEXT_STAGE_ROOT / "03_reviews" / "review_index.md", f"""# Stage76 Review Index(76단계 검토 색인)
...
Effect(효과): Stage76(76단계)는 Stage75(75단계) closeout(종료 기록)을 이어받아 다음 bounded batch(경계 묶음 실행)만 검토한다.
""")
    s73.stage72.write_md(NEXT_STAGE_ROOT / "04_selected" / "selection_status.md", f"""# Stage76 Selection Status(76단계 선택 상태)
...
Effect(효과): Stage76(76단계)는 34D KPI(34D 핵심 성과 지표) 격차 축소를 계속하지만, 운영 의미 없이 연구개발로만 이어진다.
""")
...
- >-
  Stage75(75단계) closed(종료) as `{decision}` and Stage76(76단계) `{NEXT_STAGE_ID}` is open_planned(열림 계획). Effect(효과): v41 DD/net balance(V41 손실률/순손익 균형) 근거를 보존하고 다음 경계 연구 질문으로만 넘긴다.
- >-
  Stage75 result(75단계 결과): validation/OOS(검증/표본외), PF/net/DD(수익 팩터/순손익/손실률), risk/ATR telemetry(위험/ATR 텔레메트리)는 `{rel(SUMMARY_CSV_PATH)}`와 `{rel(RISK_ATR_TELEMETRY_PATH)}`에 기록된다. Effect(효과): 34D target surface(34D 목표 표면) 대비 KPI(핵심 성과 지표) 차이를 다음 단계 입력으로 보존한다.
- >-
...
"""
    ...
    s73.stage72.write_md(SELECTED_ROOT / "selection_status.md", f"""# Stage75 Selection Status(75단계 선택 상태)

- stage_status(단계 상태): `closed_{decision}`
- source_stage(원천 단계): `{SOURCE_STAGE74_ID}`
- source_decision(원천 판정): `continue_v41_dd_balance_repair_in_stage75`
- source_stage74_pushed_commit(원천 74단계 푸시 커밋): `{SOURCE_STAGE74_COMMIT}`
- current_run(현재 실행): `{RUN_ID}`
- adapter_under_review(검토 중 어댑터): `{SOURCE_ADAPTER_ID}`
- target_surface(목표 표면): `{TARGET_SURFACE}`
- stage75_decision(75단계 판정): `{decision}`
- next_stage_or_branch(다음 단계/분기): `{NEXT_STAGE_ID}`
- claim_boundary(주장 경계): `{BOUNDARY}`

Effect(효과): Stage75(75단계)는 v41 DD/net balance(V41 손실률/순손익 균형)를 측정하고, 운영 의미 없이 Stage76(76단계)로 넘긴다.
""")
    s73.stage72.write_md(CURRENT_WORKING_STATE_PATH, f"""# Current Working State(현재 작업 상태)
...
- target_surface(목표 표면): `{TARGET_SURFACE}`
- adapter_under_review(검토 중 어댑터): `{SOURCE_ADAPTER_ID}`
- status(상태): `stage75_closed_{decision}`
- claim_boundary(주장 경계): `{BOUNDARY}`

Stage75(75단계) closed(종료) as v2-native v41 DD/net balance repair batch(v2 고유 v41 손실률/순손익 균형 수리 묶음). Effect(효과): legacy 34D(레거시 34D)는 target surface(목표 표면)로만 쓰였고, 다음 연구는 Stage76(76단계)로 이어진다.

## Latest Stage75 Evidence(최신 75단계 근거)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    prepared = {
        ...
        "model_family": "baseline_adapter_stage75_v2_native_v41_dd_balance_repair_ebm_table",
        "feature_set_id": "stage75_v41_dd_balance_signal",
    ...
    s73.stage72.write_csv(AUDIT_CSV_PATH, audit_rows)
    s73.stage72.write_csv(SUMMARY_CSV_PATH, summary_rows)
    s73.stage72.write_csv(SEGMENT_KPI_PATH, segment_rows)
    s73.stage72.write_csv(RISK_ATR_TELEMETRY_PATH, risk_rows)
    s73.stage72.write_csv(GATE_FEATURE_SUMMARY_PATH, inputs["gate_rows"])
    s73.stage72.write_csv(TIER_B_DIAGNOSTIC_PATH, tier_b_rows())
    s73.stage72.write_md(REPORT_PATH, report_markdown(summary_rows, segment_rows, target_rows, decision, external))
    s73.stage72.write_md(DECISION_PATH, decision_markdown(decision, external))
    s73.stage72.write_json(
        SUMMARY_JSON_PATH,
        {
            "run_id": RUN_ID,
