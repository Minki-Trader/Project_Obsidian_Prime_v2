...
STAGE_ID = "208_adapter_research__stage206_risk_cap_interpolation_repair"
RUN_NUMBER = "run208A"
RUN_ID = "run208A_stage208_stage206_risk_cap_interpolation_repair_v1"
PACKET_ID = "stage208_stage206_risk_cap_interpolation_repair_v1"
PARENT_RUN_ID = "run207A_stage207_stage206_long_session_dd_micro_repair_followup_review_v1"
SOURCE_STAGE_ID = "207_adapter_research__stage206_long_session_dd_micro_repair_followup_review"
SOURCE_RUN_ID = "run207A_stage207_stage206_long_session_dd_micro_repair_followup_review_v1"
SOURCE_STAGE207_EVIDENCE_COMMIT = "f11eb494c9ea5ef87919ea0b16ff2988180c6cd1"
SOURCE_STAGE207_HASH_RECORD_COMMIT = "10dc204db25f2b772eb94c543eaad50155e37d19"
SOURCE_ADAPTER_ID = "s206_ls_ref_r0325"
NEXT_STAGE_ID = "209_adapter_research__stage208_risk_cap_interpolation_followup_review"
NEXT_RUN_ID = "run209A_stage209_stage208_risk_cap_interpolation_followup_review_v1"
NEXT_PACKET_ID = "stage209_stage208_risk_cap_interpolation_followup_review_v1"
TARGET_SURFACE = "legacy_34d_kpi_lesson_only_v2_native_risk_cap_interpolation_repair"
...
COMMON_ROOT = f"OPV2/s208a/{RUN_NUMBER}"

SUMMARY_JSON_PATH = REVIEWS_ROOT / "stage208_risk_cap_summary.json"
SUMMARY_CSV_PATH = REVIEWS_ROOT / "stage208_risk_cap_kpi_summary.csv"
REPORT_PATH = REVIEWS_ROOT / "stage208_risk_cap_report.md"
SEGMENT_KPI_PATH = REVIEWS_ROOT / "stage208_segment_kpi_summary.csv"
BALANCE_CURVE_AUDIT_PATH = REVIEWS_ROOT / "stage208_balance_curve_audit.csv"
MONTHLY_KPI_PATH = REVIEWS_ROOT / "stage208_monthly_kpi_summary.csv"
CONCENTRATION_PATH = REVIEWS_ROOT / "stage208_concentration_risk_summary.csv"
DRAWDOWN_PATH = REVIEWS_ROOT / "stage208_drawdown_recovery_summary.csv"
QUALITY_MATRIX_PATH = REVIEWS_ROOT / "stage208_quality_matrix.csv"
RISK_ATR_TELEMETRY_PATH = REVIEWS_ROOT / "stage208_risk_atr_telemetry.csv"
GATE_FEATURE_SUMMARY_PATH = REVIEWS_ROOT / "stage208_gate_feature_summary.csv"
PROBABILITY_BINDING_PATH = REVIEWS_ROOT / "stage208_probability_telemetry_summary.csv"
MODEL_SCORE_AUDIT_PATH = REVIEWS_ROOT / "stage208_model_score_audit.csv"
TIER_B_DIAGNOSTIC_PATH = REVIEWS_ROOT / "stage208_tier_b_diagnostic_summary.csv"
DECISION_PATH = REVIEWS_ROOT / "stage208_decision.md"
AUDIT_CSV_PATH = REVIEWS_ROOT / "stage208_trade_audit.csv"
...
PRODUCER_PATH = Path("stage_pipelines/stage208/risk_cap_interpolation_repair.py")
ARTIFACT_COLUMNS = s172.ARTIFACT_COLUMNS

VARIANTS = (
    repair.RepairVariant(
        adapter_id="s208_ls_r0275",
        label="stage208_ls_r0275",
        atr_enabled=True,
        ...
        max_hold_bars=3,
        notes="Stage208 interpolation: long-session structure with model risk cap 2.75%.",
    ),
    repair.RepairVariant(
        adapter_id="s208_ls_r0285",
        label="stage208_ls_r0285",
        atr_enabled=True,
        model_risk_enabled=True,
        fixed_lot=0.25,
        atr_stop_multiplier=2.075,
        ...
        close_only_on_opposite_signal=False,
        max_hold_bars=3,
        notes="Stage208 interpolation: long-session structure with model risk cap 2.85%.",
    ),
    repair.RepairVariant(
        adapter_id="s208_ls_r0295",
        label="stage208_ls_r0295",
        atr_enabled=True,
        model_risk_enabled=True,
        fixed_lot=0.25,
        atr_stop_multiplier=2.075,
        atr_take_profit_multiplier=4.75,
        model_risk_max_pct=0.0295,
        same_direction_reentry_cooldown_bars=8,
        short_threshold=0.54,
        long_threshold=0.52,
        close_on_flat_signal=False,
        ...
        close_only_on_opposite_signal=False,
        max_hold_bars=3,
        notes="Stage208 interpolation: long-session structure with model risk cap 2.95%.",
    ),
    repair.RepairVariant(
        adapter_id="s208_ls_r0305",
        label="stage208_ls_r0305",
        atr_enabled=True,
        model_risk_enabled=True,
        fixed_lot=0.25,
        atr_stop_multiplier=2.075,
        ...
        close_only_on_opposite_signal=False,
        max_hold_bars=3,
        notes="Stage208 interpolation: long-session structure with model risk cap 3.05%.",
    ),
)
            ...
            for role_index, (tier, attempt_role, prefix, attempt_token) in enumerate(
                ...
                attempts.append(
                    s161.base.attempt_payload(
                        ...
                        run_id=RUN_ID,
                        stage_number=208,
                        exploration_label="stage208_BaselineAdapter__RiskCapInterpolationRepair",
...
def configure_runner() -> None:
    ...
    for variant in VARIANTS:
        ...
        context_specs[variant.adapter_id] = {
            "gate_column": f"stage208_gate_{extra['axis']}",
            "gate_type": "encoded_stage208_risk_cap_interpolation_repair",
            ...
            "description": "Stage208 encoded gate: long-session structure with risk-cap interpolation.",
        }
...
def decide(rows: Sequence[Mapping[str, Any]], external: str) -> str:
    if external != "completed":
        return "continue_stage208_runtime_completion_due_to_incomplete_runtime_candidate_not_final"
    if any(row.get("hard_quality_pass") for row in rows):
        return "open_stage209_stage208_risk_cap_interpolation_followup_review_candidate_not_final"
    return "open_stage209_bounded_followup_due_to_risk_cap_interpolation_tradeoff_candidate_not_final"
...
def report_markdown(rows: Sequence[Mapping[str, Any]], decision: str, external: str) -> str:
    best = best_row(rows)
    ref = next((row for row in rows if row.get("adapter_id") == "s208_ls_r0305"), {})
    return f"""# Stage208 Risk Cap Interpolation Repair Report(208단계 위험 상한 보간 수리 보고서)
...
- source_stage207_evidence_commit(원천 207단계 근거 커밋): `{SOURCE_STAGE207_EVIDENCE_COMMIT}`
- source_stage207_hash_record_commit(원천 207단계 해시 기록 커밋): `{SOURCE_STAGE207_HASH_RECORD_COMMIT}`
...
def decision_markdown(decision: str, external: str) -> str:
    return f"""# Stage208 Decision(208단계 판정)
...
Stage208(208단계) closeout(종료)은 overall goal complete(전체 목표 완료)가 아니다. Effect(효과): Stage209(209단계) follow-up review(후속 검토)에서 risk cap interpolation(위험 상한 보간) 상충을 따로 판정한다.
"""
...
def artifact_rows(result: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    for row in rows:
        if row.get("artifact_type") != "mt5_strategy_tester_report":
            row["artifact_type"] = "stage208_evidence"
            row["notes"] = "Stage208 risk-cap interpolation repair evidence."
        else:
            row["notes"] = "Stage208 MT5 Strategy Tester report."
    return rows
        ...
        [
            {
                "run_id": RUN_ID,
                "stage_id": STAGE_ID,
                "lane": "baseline_adapter_stage208_risk_cap_interpolation_repair",
                "status": status,
                "judgment": decision,
                "path": rel(DECISION_PATH),
                "notes": s172.ledger_pairs(
                    (
                        ("source_stage207_evidence_commit", SOURCE_STAGE207_EVIDENCE_COMMIT),
                        ("source_stage207_hash_record_commit", SOURCE_STAGE207_HASH_RECORD_COMMIT),
        ...
        alpha_rows = [
            {
                ...
                "tier_scope": "Tier A+B",
                "kpi_scope": "stage208_risk_cap_interpolation_repair",
                ...
                "external_verification_status": external,
                "notes": "Stage208 materialized or blocked before KPI records were available.",
            }
        ]
...
def write_packet_files(result: Mapping[str, Any], decision: str, ledger_payload: Mapping[str, Any], quality: Sequence[Mapping[str, Any]]) -> None:
    ...
    s172.write_md(
        ...
        f"""# Stage208 Closeout Packet(208단계 종료 작업 묶음)
...
Stage209(209단계)는 Stage208(208단계) risk cap interpolation repair(위험 상한 보간 수리) 결과를 follow-up review(후속 검토)한다.
...
Effect(효과): Stage208(208단계) 안에서 추가 수리를 하지 않고, risk cap interpolation(위험 상한 보간) 측정 결과를 별도 판독 단계에서 과장 없이 판단한다.
...
- status(상태): `open_planned_from_stage208`
- source_stage(원천 단계): `{STAGE_ID}`
- source_decision(원천 판정): `{decision}`
...
- stage_status(단계 상태): `open_planned_from_stage208`
...
- >-
  Stage208(208단계) closed(종료) as `{decision}` and Stage209(209단계) `{NEXT_STAGE_ID}` is open_planned(개방 계획). Effect(효과): risk cap interpolation(위험 상한 보간) 결과를 별도 follow-up review(후속 검토)로 넘긴다.
- >-
  Stage208 evidence(208단계 근거)는 `{rel(REPORT_PATH)}`, `{rel(SUMMARY_CSV_PATH)}`, `{rel(QUALITY_MATRIX_PATH)}`, `{rel(BALANCE_CURVE_AUDIT_PATH)}`, `{rel(RISK_ATR_TELEMETRY_PATH)}`에 있다. Effect(효과): risk cap(위험 상한)이 DD/PF/OOS(낙폭/수익요인/표본외)에 준 영향을 같이 본다.
- >-
...
"""
    if re.search(r"(?ms)^current_focus:\r?\n.*?(?=\r?\nstage\d+_)", state):
        ...
        state = state.rstrip() + "\n" + focus
    state = re.sub(r"(?ms)^stage208_risk_cap_interpolation_repair:\r?\n.*?(?=^stage\d+_|\Z)", "", state)
    block = f"""
stage208_risk_cap_interpolation_repair:
  packet_id: {PACKET_ID}
  stage_id: {STAGE_ID}
...
- adapter_under_review(검토 중 어댑터): `stage208_risk_cap_interpolation_repair`
- status(상태): `stage208_{decision}`
- claim_boundary(주장 경계): `{BOUNDARY}`

Stage208(208단계)는 Stage207(207단계) 판정에 따라 risk cap interpolation repair(위험 상한 보간 수리)를 MT5 Strategy Tester(메타트레이더5 전략 테스터)로 측정했다. Effect(효과): Stage209(209단계)가 KPI(핵심 성과 지표) 상충을 별도 review(검토)로 판정한다.

## Latest Stage208 Evidence(최신 208단계 근거)
...
def append_changelog(decision: str) -> None:
    ...
    if RUN_ID in existing:
        return
    entry = (
        f"\n## {s172.utc_now()} Stage208 risk cap interpolation repair closeout(208단계 위험 상한 보간 수리 종료)\n\n"
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    prepared = {
        ...
        "model_family": "baseline_adapter_stage208_v2_native_risk_cap_interpolation_repair",
        "feature_set_id": "stage208_signal_plus_encoded_long_session_risk_cap_gate",
