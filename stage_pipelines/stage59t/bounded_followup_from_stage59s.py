...
STAGE59T_ID = "59T_adapter_repair__bounded_followup_from_stage59s"
NEXT_REPAIR_STAGE_ID = "59U_adapter_repair__bounded_followup_from_stage59t"
RUN_NUMBER = "run59O"
RUN_ID = "run59O_stage59t_bounded_followup_from_stage59s_v1"
PACKET_ID = "stage59t_bounded_followup_from_stage59s_v1"
PARENT_RUN_ID = "run59N_stage59s_bounded_followup_from_stage59r_v1"
...
STAGE_ROOT = Path("stages") / STAGE59T_ID
...
COMMON_ROOT = f"Project_Obsidian_Prime_v2/s59t/{RUN_NUMBER}"

REPORT_PATH = REVIEWS_ROOT / "bounded_followup_from_stage59s_report.md"
...
DECISION_PATH = REVIEWS_ROOT / "stage59t_decision.md"
AUDIT_CSV_PATH = REVIEWS_ROOT / "stage59t_trade_audit.csv"
...
STAGE59T_VARIANTS = (
    repair.RepairVariant(
        adapter_id="s59t_exit_flatclose_sl20_tp30_sd12_h5_rearm002",
        label="run50BQ_v61_transition_h2_exit_flatclose_sl20_tp30_sd12_h5_rearm002_risk3pct_atr",
        atr_enabled=True,
        ...
        max_hold_bars=5,
        notes="Stage59T bounded follow-up: keep Stage59S source/policy and close on flat signal without reversing.",
    ),
    repair.RepairVariant(
        adapter_id="s59t_exit_reverseopp_sl20_tp30_sd12_h5_rearm002",
        label="run50BQ_v61_transition_h2_exit_reverseopp_sl20_tp30_sd12_h5_rearm002_risk3pct_atr",
        atr_enabled=True,
        model_risk_enabled=True,
        fixed_lot=0.25,
        atr_stop_multiplier=2.0,
        ...
        close_only_on_opposite_signal=False,
        max_hold_bars=5,
        notes="Stage59T bounded follow-up: keep Stage59S source/policy and reverse on opposite signal without flat close.",
    ),
    repair.RepairVariant(
        adapter_id="s59t_exit_flat_reverse_sl20_tp30_sd12_h5_rearm002",
        label="run50BQ_v61_transition_h2_exit_flat_reverse_sl20_tp30_sd12_h5_rearm002_risk3pct_atr",
        atr_enabled=True,
        model_risk_enabled=True,
        fixed_lot=0.25,
        atr_stop_multiplier=2.0,
        ...
        close_only_on_opposite_signal=False,
        max_hold_bars=5,
        notes="Stage59T bounded follow-up: keep Stage59S source/policy and test flat close plus opposite reversal.",
    ),
)
...
def source_specs() -> dict[str, dict[str, Any]]:
    v61 = {
        ...
        "source_note": "Stage59S best source anchor: Stage56 run50BQ v61 context ExtraTrees firewall transition hold2 source",
    }
    return {variant.adapter_id: dict(v61) for variant in STAGE59T_VARIANTS}

def configure_reused_engine() -> None:
    engine.STAGE59_ID = STAGE59T_ID
    ...
    engine.STAGE59_VARIANTS = STAGE59T_VARIANTS
    engine.SOURCE_SPECS = source_specs()
    engine.MODEL_RISK_MIN_PCT = {variant.adapter_id: 0.005 for variant in STAGE59T_VARIANTS}

    repair.STAGE_ID = STAGE59T_ID
    repair.RUN_NUMBER = RUN_NUMBER
    repair.RUN_ID = RUN_ID
    repair.RUN_ROOT = RUN_ROOT
    repair.REPAIR_VARIANTS = STAGE59T_VARIANTS
    s58.STAGE58_ID = STAGE59T_ID
    ...
    s58.STAGE58_VARIANTS = STAGE59T_VARIANTS
...
def build_attempts(inputs: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    for variant_index, variant in enumerate(STAGE59T_VARIANTS, start=1):
            ...
            for role_index, (tier, attempt_role, prefix, attempt_token) in enumerate(
                ...
                attempts.append(
                    attempt_payload(
                        ...
                        run_id=RUN_ID,
                        stage_number=59,
                        exploration_label="stage59T_BaselineAdapter__LongThresholdPressureFromStage59S",
...
def execute_prepared_run_checkpointed(prepared: Mapping[str, Any], args: argparse.Namespace) -> dict[str, Any]:
    ...
    return {
        **dict(prepared),
        ...
        "judgment": "inconclusive_stage59t_runtime_probe_completed" if completed and report_completed else "blocked_stage59t_runtime_probe",
    }
...
def next_stage_for_decision(decision: str) -> str:
    ...
    if decision == "demote_current_adapter_and_select_backup":
        return "59U_adapter_repair__backup_or_demote_from_stage59t"
    if decision == "open_bounded_followup":
        return "59U_adapter_repair__bounded_followup_from_stage59t"
...
def next_packet_for_decision(decision: str) -> tuple[str, str]:
    ...
    return "stage59u_bounded_followup_from_stage59t_v1", "run59P_stage59u_bounded_followup_from_stage59t_v1"
            ...
            rows.append(
                {
                    ...
                    "stage_id": STAGE59T_ID,
                    "run_id": RUN_ID,
                    "artifact_type": "stage59t_bounded_followup_evidence",
                    "path": rel(path),
                    "sha256": sha256_file_lf_normalized(path),
                    "created_at_utc": created,
                    "notes": "Stage59T bounded follow-up from Stage59S artifact.",
                }
            )
            ...
            rows.append(
                {
                    ...
                    "created_at_utc": created,
                    "notes": "Actual Stage59T MT5 Strategy Tester HTML report.",
                }
            )
    return rows
...
def write_run_identity(result: Mapping[str, Any]) -> None:
    engine.write_json(
        RUN_ROOT / "run_manifest.json",
        {
            ...
            "bounded_question": "Can a bounded exit-policy sweep on the Stage59S best adapter improve validation early and OOS mid segment weakness without damaging validation/OOS PF, drawdown, cost stress, same-move concentration, ATR/risk telemetry, or starting ONNX?",
            ...
            "variants": [
                ...
                for variant in STAGE59T_VARIANTS
            ],
...
def write_ledgers(
        ...
        [
            {
                "run_id": RUN_ID,
                "stage_id": STAGE59T_ID,
                "lane": "baseline_adapter_bounded_followup_from_stage59s",
                "status": status,
                "judgment": decision,
    ...
    ledger_rows.append(
        {
            "ledger_row_id": f"{RUN_ID}__aggregate_bounded_followup_from_stage59s",
            "stage_id": STAGE59T_ID,
            "run_id": RUN_ID,
            "subrun_id": "aggregate_bounded_followup_from_stage59s",
            "parent_run_id": PARENT_RUN_ID,
            "record_view": "bounded_followup_from_stage59s",
            "tier_scope": "Tier A+B",
            ...
            "external_verification_status": external,
            "notes": "Stage59T bounded exit-policy follow-up from Stage59S evidence; not final package completion.",
        }
    )
...
def report_markdown(summary_rows: Sequence[Mapping[str, Any]], decision: str, reasons: Sequence[str], external: str) -> str:
    ...
    best = best_repaired_variant(summary_rows)
    return f"""# Stage59T Exit Policy Followup From Stage59S Report(59T단계 59S단계 기반 청산 정책 후속 보고서)

- stage(단계): `{STAGE59T_ID}`
- run(실행): `{RUN_ID}`
...
def equity_audit_markdown(summary_rows: Sequence[Mapping[str, Any]], segment_rows: Sequence[Mapping[str, Any]]) -> str:
    ...
    return f"""# Stage59T Bounded Followup Equity Curve Audit(59T단계 경계 후속 자금 곡선 감사)
...
def decision_markdown(decision: str, reasons: Sequence[str], best: Mapping[str, Any], external: str) -> str:
    return f"""# Stage59T Decision(59T단계 판정)

decision(판정): `{decision}`

Stage59T(59T단계)는 Stage59S evidence(59S단계 근거)를 사용한 bounded exit-policy follow-up(경계 청산 정책 후속)으로 기록한다. Effect(효과): exit policy(청산 정책) 압박의 성공/실패를 다음 bounded stage(경계 다음 단계)의 입력 근거로 넘긴다.
...
Stage59T closeout(59T단계 종료)는 overall goal completion(전체 목표 완료)이 아니다. Effect(효과): Stage60 ONNX hardening(60단계 ONNX 경화)은 adapter quality(어댑터 품질)가 강할 때만 열린다.
    ...
    engine.write_json(
        SUMMARY_JSON_PATH,
        {
            ...
            "required_outputs": {
                ...
                "stage59t_decision": rel(DECISION_PATH),
            },
...
def write_packet_files(
    ...
    files = {
        ...
        "experiment_design_receipt.json": {
            "hypothesis": "A bounded exit-policy sweep on the Stage59S best adapter may improve validation early and OOS mid segment weakness while preserving validation/OOS PF, cost stress, drawdown, same-move concentration, and ATR/risk telemetry.",
            ...
            "comparison_baseline": "Stage59S best candidate s59s_v61_long54_sl20_tp30_sd12_h5_rearm002.",
            "control_variables": ["US100", "M5", "split_v1", "Tier B disabled", "run50BQ v61 source anchor", "short threshold 0.52", "long threshold 0.52", "same-direction cooldown 12", "ATR SL 2.0", "ATR TP 3.0", "3% model-controlled risk cap", "max hold 5", "entry transition rearm 0.02", "ONNX deferred"],
            "changed_variables": ["exit_policy"],
            ...
            "stop_conditions": "three bounded Stage59T exit-policy variants only",
        ...
        "artifact_lineage_audit.json": {
            "source_inputs": [rel(SOURCE_STAGE59S_DECISION), rel(SOURCE_STAGE59S_REPORT), rel(SOURCE_STAGE59S_SUMMARY), rel(SOURCE_STAGE59S_JSON)],
...
- stage_id(단계 ID): `{STAGE59T_ID}`
- source_stage(원천 단계): `{SOURCE_STAGE59S_ID}`
- source_decision(원천 판정): `continue_repair_in_new_bounded_stage`
- bounded_question(경계 질문): `Can a bounded exit-policy sweep on the Stage59S best adapter improve validation early and OOS mid segment weakness without damaging validation/OOS PF, drawdown, cost stress, same-move concentration, ATR/risk telemetry, or starting ONNX?`
- boundary(경계): `{BOUNDARY}`

Stage59T(59T단계)는 Stage59S(59S단계) best adapter(최선 어댑터)의 source anchor(원천 기준점), thresholds(임계값), ATR bracket(ATR 브래킷), risk cap(위험 상한), cooldown(쿨다운), max hold(최대 보유)를 유지하고 exit policy(청산 정책) 세 변형만 측정한다. Effect(효과): Stage60 ONNX(60단계 ONNX)나 deployment(배포)로 이어가지 않고 청산 정책이 구간 약점을 줄이는지만 본다.
""",
    )
    engine.write_md(
...
- stage59t_changed_variable(59T단계 변경 변수): `exit_policy_only`

Effect(효과): Stage59T(59T단계)는 Stage59S(59S단계) 실패 기억과 run50BQ(run50BQ) v61 원천을 고정한 exit policy(청산 정책) 변형을 명시적으로 연결한다.
""",
    )
    engine.write_md(
...
- stage_status(단계 상태): `closed_bounded_followup_from_stage59s`
- source_stage(원천 단계): `{SOURCE_STAGE59S_ID}`
- source_decision(원천 판정): `continue_repair_in_new_bounded_stage`
- stage59t_decision(59T단계 판정): `{decision}`
- next_stage_or_branch(다음 단계/분기): `{next_stage}`
- selected_research_baseline(선택 연구 기준선): `none`
- claim_boundary(주장 경계): `{BOUNDARY}`

Effect(효과): Stage59T(59T단계)는 측정 결과를 보존하지만 final package(최종 패키지)나 operating claim(운영 주장)을 만들지 않는다.
""",
    )
    engine.write_md(
        REVIEWS_ROOT / "review_index.md",
        f"""# Stage59T Review Index(59T단계 검토 색인)
...
- bounded_followup_risk_atr_telemetry(경계 후속 위험/ATR 텔레메트리): `{rel(RISK_ATR_TELEMETRY_PATH)}`
- stage59t_decision(59T단계 판정): `{rel(DECISION_PATH)}`
""",
    )
...
- source_stage(원천 단계): `{STAGE59T_ID}`
- source_decision(원천 판정): `{decision}`
- bounded_question(경계 질문): `What bounded follow-up should repair, demote, or branch from the Stage59T evidence without starting ONNX prematurely?`
- boundary(경계): `{BOUNDARY}`

Stage59U(59U단계)는 Stage59T(59T단계) 결과가 Stage60 ONNX(60단계 ONNX)에 충분하지 않을 때 이어지는 bounded follow-up(경계 후속)이다. Effect(효과): repair(수리), demotion(강등), branch(분기)를 다음 작은 질문으로 제한한다.
""",
        )
        engine.write_md(
...
- stage59t_decision(59T단계 판정): `{rel(DECISION_PATH)}`
- stage59t_report(59T단계 보고서): `{rel(REPORT_PATH)}`
- stage59t_summary(59T단계 요약): `{rel(SUMMARY_CSV_PATH)}`
- stage59t_segment_kpi(59T단계 구간 KPI): `{rel(SEGMENT_KPI_PATH)}`
- stage59t_risk_atr_telemetry(59T단계 위험/ATR 텔레메트리): `{rel(RISK_ATR_TELEMETRY_PATH)}`
""",
        )
        engine.write_md(
            next_root / "03_reviews/review_index.md",
            """# Stage59U Review Index(59U단계 검토 색인)

Stage59U(59U단계)는 planned(계획) 상태다. Effect(효과): Stage59T(59T단계) 약점을 다음 bounded repair(경계 수리) 근거로 넘긴다.
""",
        )
        engine.write_md(
...
- stage_status(단계 상태): `active_planned_from_stage59t`
- source_stage(원천 단계): `{STAGE59T_ID}`
- source_decision(원천 판정): `{decision}`
- selected_research_baseline(선택 연구 기준선): `none`
- claim_boundary(주장 경계): `{BOUNDARY}`

Effect(효과): Stage59U(59U단계)는 Stage59T(59T단계) 이후 필요한 다음 작은 질문만 다룬다.
""",
        )
...
- status(상태): `stage59t_closed_{decision}`
- claim_boundary(주장 경계): research/development only(연구/개발 전용)

Stage59T(59T단계) closed(종료) as bounded exit-policy follow-up from Stage59S(59S단계 기반 경계 청산 정책 후속). Effect(효과): Stage59S(59S단계) segment/density weakness(구간/밀도 약점)는 보존하고, Stage60 ONNX(60단계 ONNX)는 품질 근거가 강할 때만 열린다.

## Latest Stage59T Evidence(최신 59T단계 근거)
...
""",
    ...
    focus = (
        "current_focus:\n"
        f"- >-\n"
        f"  Stage59T(59T단계) `{STAGE59T_ID}` closed(종료) as bounded exit-policy follow-up from Stage59S(59S단계 기반 경계 청산 정책 후속); decision(판정)=`{decision}`. "
        ...
        f"- >-\n"
        f"  Next stage_or_branch(다음 단계/분기) `{next_stage}` is active/planned(활성/계획). Effect(효과): Stage59T(59T단계) 결과를 다음 bounded step(경계 다음 단계)으로 넘긴다.\n"
    )
    text = re.sub(
...
stage59t_bounded_followup_from_stage59s:
  packet_id: {PACKET_ID}
  stage_id: {STAGE59T_ID}
  status: closed_bounded_followup_from_stage59s
  ...
  boundary: {BOUNDARY}
"""
    if "stage59t_bounded_followup_from_stage59s:" in text:
        text = re.sub(r"\nstage59t_bounded_followup_from_stage59s:\n(?:  .*\n)*", block, text, count=1)
    else:
        text += block
...
def parse_args(argv: Sequence[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run Stage59T bounded follow-up from Stage59S.")
...
def load_existing_result() -> dict[str, Any]:
    ...
    if not path_exists(manifest) or not path_exists(kpi):
        raise FileNotFoundError("Stage59T existing run_manifest.json or kpi_record.json is missing")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.finalize_existing:
        ...
        prepared = {
            ...
            "model_family": "baseline_adapter_stage59t_exit_policy_ebm_table",
            "feature_set_id": "stage59t_run50bq_v61_exit_policy_signal",
