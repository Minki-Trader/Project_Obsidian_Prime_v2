...
STAGE_ID = "stage_frontier_47__short_pf_edge_sequence_state_risk_budget_after_f46_sequence_context_memory"
PREV_STAGE_ID = "stage_frontier_46__short_pf_edge_event_sequence_context_pivot_after_f45_event_classifier_memory"
RUN_A = "frontier47A_stage_open_short_pf_edge_sequence_state_risk_budget_hypothesis_design_v1"
RUN_B = "frontier47B_train_only_sequence_state_risk_budget_proxy_v1"
RUN_C = "frontier47C_capped_state_risk_budget_repair_v1"
RUN_D = "frontier47D_stage_closeout_sequence_state_risk_budget_v1"
NEXT_STAGE_ID = "stage_frontier_48__short_pf_edge_event_rarity_risk_sizing_after_f47_state_budget_memory"
NEXT_RUN_ID = "frontier48A_stage_open_short_pf_edge_event_rarity_risk_sizing_hypothesis_design_v1"
RUNTIME_REVIEW_RUN_ID = "frontier47E_pre_expensive_runtime_validation_review_v1"
...
GROK_OPEN_ROOT = Path("docs") / "agent_control" / "grok_reviews" / "2026-06-15_frontier47_stage_open" / "small_review"
GROK_CLOSE_ROOT = Path("docs") / "agent_control" / "grok_reviews" / "2026-06-15_frontier47_stage_closeout" / "small_review"
...
def main() -> None:
    ...
    write_json(INPUT_ROOT / "sequence_state_risk_budget_manifest.json", manifest)
    ...
    write_json(
        RUN_D_ROOT / "run_manifest.json",
        {
            ...
            "profile": "short_sequence_state_risk_budget_proxy",
            "open_review": open_review,
            "closeout_review": closeout_review,
            "repair_decision": repair_decision,
            "closeout": closeout,
...
def load_open_grok_review() -> dict[str, Any]:
    ...
    return {
        ...
        "classification": "accepted_stage_open_train_split_only_state_risk_budget_lock" if accepted and guardrail_seen and claim_ok and not rejected else "needs_local_verification",
...
def load_closeout_grok_review() -> dict[str, Any]:
    ...
    return {
        ...
        "metadata": metadata,
        "clean_output_path": clean_path.as_posix(),
        "classification": "accepted_closeout_state_risk_budget_boundary" if accepted and boundary_ok and not rejected else "needs_local_verification",
...
def context_checks(
    ...
    checks = {
        "workspace_points_to_f47": f"next_stage_id: {STAGE_ID}" in workspace or f"current_stage_id: {STAGE_ID}" in workspace,
    ...
    if not all(checks.values()):
        raise RuntimeError(f"Frontier47 context check failed: {json.dumps(checks, ensure_ascii=False)}")
    return {
        "checks": checks,
...
def build_input_manifest(
    ...
    return {
        "stage_id": STAGE_ID,
        "hypothesis": "train_only_sequence_state_risk_budget_on_f46_loss_contained_clue",
        "idea_id": "IDEA-FR47-SHORT-SEQUENCE-STATE-RISK-BUDGET",
        "decision_use": "scout_seed_runtime_candidate_screening_only",
        "comparison_baseline": "F46 best and preserved nonwinner rows are reference-only clues, not inherited baseline",
        "control_variables": {
            "symbol": "US100",
            "timeframe": "M5",
            "side": SIDE_LABEL,
        ...
        "changed_variables": {
            "sequence_context": "lagged_frozen_event_score_rolls_and_horizon_embargoed_past_outcome_tape",
            "risk_budget": "train_only_loss_cluster_volatility_and_cooldown_state_caps",
            "novelty_line": "risk_budget_blocks_entries_it_does_not_rescore_or_repair_the_model",
            "model_family": "class_weighted_sequence_context_classifier_with_state_budget_overlay",
            "threshold_policy": "train_split_sequence_probability_and_risk_budget_quantiles_only",
        },
        "sample_scope": {
            "rows": int(len(frame)),
        ...
        "invalid_conditions": [
            ...
            "f46_sequence_context_score_quantile_only_repair_without_state_budget",
            "f44_continuous_regression_or_f42_f43_f38_f39_primary_lever_reopened",
        ],
        "evidence_plan": [
            "stage_open_grok_receipt",
            "sequence_state_risk_budget_model_audit",
            "candidate_ledger",
            "split_metrics",
            "candidate_summary",
            "stage_run_ledger",
...
def risk_budget_specs(profile: str) -> list[dict[str, Any]]:
    specs = [
        {
            "risk_budget_variant": "risk_budget_bad_slow_p78_high_fast_p85_vol_p88",
            "risk_budget_definition": (
                "train-only cap on slow past bad-event rate, fast high-score crowding, and ATR volatility state"
            ),
            "max_quantile_features": {
                "seq_past_bad_event_rate_slow": 0.78,
                "seq_high_count_fast": 0.85,
                "atr_14_over_atr_50": 0.88,
            },
            "min_fixed_features": {},
        },
        {
            "risk_budget_variant": "risk_budget_bad_fast_p72_realized_vol_p82",
            "risk_budget_definition": "train-only cap on fast bad-event rate and realized volatility expansion",
            "max_quantile_features": {
                "seq_past_bad_event_rate_fast": 0.72,
                "historical_vol_5_over_20": 0.82,
            },
            "min_fixed_features": {},
        },
        {
            "risk_budget_variant": "risk_budget_cooldown2_bad_slow_p82",
            "risk_budget_definition": "train-only two-bar cooldown after high score plus slow bad-event budget",
            "max_quantile_features": {
                "seq_past_bad_event_rate_slow": 0.82,
            },
            "min_fixed_features": {
    ...
    if profile == "repair":
        specs.extend(
            [
                {
                    "risk_budget_variant": "repair_risk_budget_bad_slow_p88_vol_p92",
                    "risk_budget_definition": "looser capped repair on slow bad-event rate and ATR volatility",
                    "max_quantile_features": {
                        "seq_past_bad_event_rate_slow": 0.88,
                        "atr_14_over_atr_50": 0.92,
                    },
                    "min_fixed_features": {},
                },
                {
                    "risk_budget_variant": "repair_risk_budget_squeeze_p80_bad_fast_p80",
                    "risk_budget_definition": "repair cap on squeeze/fast bad-event state before widening event rarity",
                    "max_quantile_features": {
                        "bb_squeeze": 0.80,
                        "seq_past_bad_event_rate_fast": 0.80,
                    },
                    "min_fixed_features": {},
                },
            ]
...
def apply_risk_budget_mask(
    *,
    ...
    return mask, {
        ...
        "risk_budget_threshold_source": "train_base_mask" if int(np.sum(train_base)) >= 50 else "train_context_universe",
...
def build_event_surface(
    ...
    *,
    ...
    summary = summarize_f47_candidates(split_metrics, selected)
    return {
...
def candidates_for_mask(
    ...
    condition = {
        ...
        "feature_family": "train_only_sequence_state_risk_budget_score",
    ...
    for row in threshold_rows(
        ...
        take_quantiles,
        rr_floors,
        f"f47_{profile}_train_only_sequence_state_risk_budget_sl_tp_caps",
    ):
        ...
        candidate["f47_train_selection_score"] = score_value
        candidate["selection_rank_basis"] = "train_only_event_sequence_state_risk_budget_no_validation_oos_feedback"
        variants.append(candidate)
    variants.sort(key=lambda item: float(item["f47_train_selection_score"]), reverse=True)
    return variants[:2]
...
def rank_candidates(candidates: list[dict[str, Any]], prefix: str, limit: int) -> list[dict[str, Any]]:
    ...
    for candidate in sorted(candidates, key=lambda item: float(item["f47_train_selection_score"]), reverse=True):
        key = (
...
def summarize_f47_candidates(split_metrics: pd.DataFrame, candidates: list[dict[str, Any]]) -> pd.DataFrame:
    ...
    metadata = {
        str(item["candidate_id"]): {
            ...
            "f47_train_selection_score": item.get("f47_train_selection_score", ""),
    ...
    summary["f47_scout_clue_flag"] = dual & density_scout & (f_min_pf >= SCOUT_MIN_PF) & (f_max_dd <= SCOUT_MAX_DD)
    summary["f47_seed_surface_flag"] = dual & density_seed & (f_min_pf >= SEED_MIN_PF) & (f_max_dd <= SEED_MAX_DD)
    summary["runtime_probe_candidate_flag"] = (
        summary["f47_seed_surface_flag"].astype(bool) & (f_min_pf >= RUNTIME_MIN_PF) & (f_max_dd <= RUNTIME_MAX_DD)
    )
    summary["f47_axis_gap_to_seed"] = (
...
def surface_counts(summary: pd.DataFrame) -> dict[str, Any]:
    ...
    return {
        "scout_clue_rows": int(summary["f47_scout_clue_flag"].sum()),
        "seed_surface_rows": int(summary["f47_seed_surface_flag"].sum()),
...
def closest_nonwinner_forward_observation(combined: pd.DataFrame, best: dict[str, Any]) -> dict[str, Any]:
    ...
    frame["__seed_gap"] = numeric_column("f47_axis_gap_to_seed", 999999.0)
...
def build_repair_decision(initial_summary: pd.DataFrame) -> dict[str, Any]:
    if initial_summary.empty:
        return {
            ...
            "repair_action": "run_capped_state_risk_budget_repair",
            "repair_reason": "Initial sequence state risk-budget surface produced no candidate summary.",
        }
    runtime = int(initial_summary["runtime_probe_candidate_flag"].sum())
    seed = int(initial_summary["f47_seed_surface_flag"].sum())
    scout_rows = int(initial_summary["f47_scout_clue_flag"].sum())
    if runtime:
        return {
    ...
    if seed:
        return {
            ...
            "repair_reason": "Seed surface exists; avoid state-budget overfit repair before runtime validation planning.",
        }
    return {
        "run_repair_grid": True,
        "repair_action": "run_capped_state_risk_budget_repair",
        "repair_reason": f"Initial sequence state risk-budget surface scout={scout_rows}, seed=0, runtime=0; run bounded risk-budget repair.",
...
def classify_closeout(initial_summary: pd.DataFrame, repair_summary: pd.DataFrame) -> dict[str, Any]:
    ...
    if combined.empty:
        ...
        scout_rows = int(combined["f47_scout_clue_flag"].sum())
        seed_rows = int(combined["f47_seed_surface_flag"].sum())
    ...
    if runtime_rows:
        ...
        runtime_status = "runtime_probe_out_of_scope_by_claim_seed_only_no_runtime_candidate_after_f47_state_risk_budget_proxy"
        next_stage = NEXT_STAGE_ID
        next_run = NEXT_RUN_ID
    elif scout_rows:
        closeout_class = "preserved_clue_negative_memory"
        runtime_status = "runtime_probe_ineligible_no_seed_or_runtime_candidate_after_f47_state_risk_budget_proxy"
        next_stage = NEXT_STAGE_ID
        next_run = NEXT_RUN_ID
    else:
        closeout_class = "negative_memory"
        runtime_status = "runtime_probe_ineligible_no_scout_seed_or_runtime_candidate_after_f47_state_risk_budget_proxy"
        next_stage = NEXT_STAGE_ID
        next_run = NEXT_RUN_ID
    return {
...
def build_closeout_prompt(closeout: dict[str, Any], best_rows: list[dict[str, Any]], repair_decision: dict[str, Any]) -> str:
    ...
    compact_rows = "\n".join(
        (
            ...
            f"fwd_dd={row.get('forward_max_dd')}; scout={row.get('f47_scout_clue_flag')}; "
            f"seed={row.get('f47_seed_surface_flag')}; runtime={row.get('runtime_probe_candidate_flag')}"
        )
        for row in best_rows[:6]
    )
    return f"""# Frontier47 closeout Grok review(그록 마감 검토)
...
- risk_budget_keep_rate(위험 예산 유지율): {best.get("risk_budget_train_keep_rate")}
- risk_budget_block_rate(위험 예산 차단율): {best.get("risk_budget_train_block_rate")}
...
- scout/seed/runtime(탐색/씨앗/런타임): {best.get("f47_scout_clue_flag")}/{best.get("f47_seed_surface_flag")}/{best.get("runtime_probe_candidate_flag")}
...
F47 opened train-only sequence state risk budget(학습 전용 순서 상태 위험 예산) hypothesis(가설). F46 is reference only(참조 전용), not baseline/winner(기준선/승자 아님).
"""
...
- scope_completion_gate(범위 완료 게이트): pass(통과), F47 hypothesis/proxy/repair/closeout(가설/프록시/수리/마감) materialized.
...
- data_integrity(데이터 무결성): pass(통과), closed-bar feature order(닫힌 봉 피처 순서), train-only frozen base scorer(학습 전용 고정 기본 채점기), lagged score context(지연 점수 문맥), and horizon+1 embargo(예측수평+1 유예) verified.
...
- final_codex_direction(최종 코덱스 방향): run F47 sequence state risk-budget proxy with validation/OOS read-only(검증/표본외 읽기 전용)
"""
...
- final_codex_direction(최종 코덱스 방향): close F47 as `{closeout.get("closeout_class")}` with no authority claim(권위 주장 없음)
"""
    return {
...
F47 negative memory(부정 기억)는 train-only sequence state risk budget(학습 전용 순서 상태 위험 예산)이 seed/runtime(씨앗/런타임) 후보를 만들었는지 여부와 반복 금지 경계를 기록한다.

- scout_clue_count(탐색 단서 수): {closeout.get("scout_clue_count")}
- seed_surface_count(씨앗 표면 수): {closeout.get("seed_surface_count")}
- runtime_probe_candidate_count(런타임 탐침 후보 수): {closeout.get("runtime_probe_candidate_count")}
- runtime_probe_status(런타임 탐침 상태): `{closeout.get("runtime_probe_status")}`
- do_not_repeat(반복 금지): F46 sequence-context score-only repair(순서 문맥 점수 전용 수리), F45 same-bar threshold-only repair(동일 봉 임계값 전용 수리), F44 continuous regression(연속 회귀), F42 timing gate(타이밍 게이트), F43 trade-shape source(거래 형태 원천), F38 shallow score quantile repair(얕은 점수 분위수 수리), F39 regime bucket overlay(체제 버킷 덧씌움)를 primary lever(주 레버)로 반복하지 않는다.
"""
    return {
    ...
    rows = [
        {
            ...
            "notes": "F47 opened with train-only sequence state risk-budget hypothesis and Grok guardrails.",
        },
        {
            "stage_id": STAGE_ID,
            "run_id": RUN_B,
            ...
            "notes": "Initial train-only sequence state risk-budget proxy surface.",
        },
        {
            "stage_id": STAGE_ID,
            "run_id": RUN_C,
            ...
            "runtime_probe_status": "evaluated_for_runtime_candidate",
            "notes": "Capped sequence state risk-budget repair diagnostic.",
        },
        {
            "stage_id": STAGE_ID,
            "run_id": RUN_D,
        ...
        },
        {
            ...
            "closeout_class": closeout.get("closeout_class"),
            "runtime_probe_status": "out_of_scope_by_claim_tier_a_state_risk_budget_proxy_only",
            "notes": "F47 used Tier A sequence state risk-budget proxy only; Tier B not claimed.",
        },
        {
            "stage_id": STAGE_ID,
            "run_id": RUN_D,
...
def project_ledger_row(row: dict[str, Any], fields: list[str]) -> dict[str, Any]:
    ...
    values = {
        ...
        "kpi_scope": "short_sequence_state_risk_budget_proxy",
        ...
        "run_family": "frontier_short_sequence_state_risk_budget_proxy",
...
current_judgment: {closeout.get("closeout_class")}(F47 train-only sequence state risk-budget proxy no operating authority)
...
Frontier47(F47, 전선 47단계)가 `{closeout.get("closeout_class")}`로 닫혔다.
...
F47 carry-forward(이월) 기록은 train-only sequence state risk budget(학습 전용 순서 상태 위험 예산)이 PF/DD/density(수익 팩터/손실폭/밀도)를 얼마나 바꾸었는지와 seed/runtime(씨앗/런타임) 후보가 생겼는지 여부다.
"""
...
def artifact_map() -> dict[str, str]:
    return {
        "input_manifest": (INPUT_ROOT / "sequence_state_risk_budget_manifest.json").as_posix(),
