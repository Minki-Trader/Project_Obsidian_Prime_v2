...
NEXT_STAGE_ID = "301_onnx_candidate_campaign__orthogonal_profit_source_rebuild"
RUN_ID = "run300C_review_split_forward_trade_shape_generalization_mt5_probe_v1"
RUN_NUMBER = "run300C"
SOURCE_RUN_ID = "run300B_split_forward_trade_shape_generalization_mt5_probe_v1"
PARENT_RUN_ID = "run300A_design_split_forward_trade_shape_generalization_rebuild_v1"
...
RUN300B = STAGE_ROOT / "02_runs" / "run300B"
...
SOURCE_EXECUTION = RUN300B / "execution_result.json"
SOURCE_KPI = RUN300B / "mt5_kpi_summary.csv"
SOURCE_SCOUT = RUN300A / "model_scout_scoreboard.csv"
PRODUCER = Path("stage_pipelines/stage300/review_split_forward_trade_shape_generalization_mt5_probe.py")

SCOREBOARD = RUN_ROOT / "split_forward_trade_shape_generalization_review_scoreboard.csv"
...
NEXT_STAGE_QUEUE = RUN_ROOT / "stage301_seed_queue.csv"
RESULT_JUDGMENT = RUN_ROOT / "result_judgment.csv"
GATE_AUDIT = RUN_ROOT / "required_gate_coverage_audit.csv"
RUN_MANIFEST = RUN_ROOT / "run_manifest.json"
LINEAGE = RUN_ROOT / "artifact_lineage_receipt.json"
REPORT = REVIEWS / "run300C_split_forward_trade_shape_generalization_review_stage301_open_report.md"
DECISION = ROOT / "docs" / "decisions" / "2026-05-24_stage300_split_forward_trade_shape_generalization_review_stage301_open.md"
...
def feature_trading_days(attempt: Mapping[str, Any]) -> int:
    ...
    local = RUN300B / "features" / Path(feature_path).name
...
def load_actual_rows() -> tuple[list[dict[str, Any]], list[dict[str, Any]], dict[str, dict[str, Any]]]:
    ...
    with ledger.io_path(SOURCE_KPI).open("r", encoding="utf-8-sig", newline="") as handle:
        for source_row in csv.DictReader(handle):
            ...
            materialized_id = str(attempt.get("stage300_branch_id") or attempt.get("materialized_branch_id") or "")
...
def build_scoreboard(rows: Sequence[Mapping[str, Any]], scout_rows: Mapping[str, Mapping[str, Any]]) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    ...
    for candidate_id, split_rows in sorted(by_candidate.items()):
        ...
        if not selected:
            ...
            failure_rows.append(
                {
                    "failure_id": f"{candidate_id}_stage300_negative",
                    "materialized_branch_id": candidate_id,
                    ...
                    "do_not_repeat": "Stage300 trade-shape score(거래 형태 점수)의 단순 quantile(분위) 조정이나 같은 loss-cluster veto(손실 군집 거부)만 반복하지 않는다.",
                    "claim_boundary": BOUNDARY,
                }
            )
...
def stage300_queue_rows() -> list[dict[str, Any]]:
    return [
        {
            "seed_id": "stage301_orthogonal_profit_source_primary",
            "source_stage_id": STAGE_ID,
            "source_run_id": RUN_ID,
            "seed_role": "fresh_thesis_primary(새 논제 주 입력)",
            "hypothesis": "Stage300(300단계)의 split-forward trade shape generalization(분할 전진 거래 형태 일반화)은 실제 MT5(메타트레이더5)에서 순수익 규모가 깨졌다. Stage301(301단계)은 같은 거래 형태 repair(수정 반복)가 아니라 새 profit source(수익 원천), 새 feature surface(피처 표면), 새 risk/reward asymmetry(위험/보상 비대칭)를 만든다.",
            "broad_sweep": "new feature construction(새 피처 구성), model variant(모델 변형), payoff source separation(보상 원천 분리), density-aware routing(밀도 인식 라우팅)",
            "aggressive_sweep": "profit scale(수익 규모)을 먼저 키우는 6-10 trades/day(일 6-10거래) 상방 분기",
            "defensive_sweep": "local hollow(국소 꺼짐), OOS-negative signature(표본외 음수 서명), zero-trade collapse(무거래 붕괴) 차단",
            "success_gate": "validation/OOS(검증/표본외) 모두 양수, 4-10 trades/day(일 4-10거래), combined net profit(합산 순수익) 충분, PF(수익 팩터)와 회복 계수 동시 통과, 확대 구간 곡선 꺼짐 없음",
            "discard_condition": "실제 MT5(메타트레이더5)에서 순수익 규모가 없거나 OOS(표본외)가 다시 음수면 폐기하고 같은 shape-threshold(형태 임계값) repair(수정)를 반복하지 않는다.",
            "prior_stage_refs": "stage298_profit_scale_failure;stage299_oos_generalization_failure;stage300_split_forward_profit_scale_failure",
            "claim_boundary": BOUNDARY,
        }
    ]
...
def stage300_scaffold() -> None:
    ...
    write_text(
        ...
        "\n".join(
            [
                "# Stage301 Brief(301단계 개요)",
                "",
                f"- stage_id(단계 ID): `{NEXT_STAGE_ID}`",
                f"- source_stage(원천 단계): `{STAGE_ID}`",
                f"- source_run(원천 실행): `{RUN_ID}`",
                "- question(질문): Can a new orthogonal profit source(직교 수익 원천) restore profit scale(수익 규모), 4-10 trades/day(일 4-10거래), and smooth equity curve(매끄러운 평가금 곡선) after split-forward trade shape(분할 전진 거래 형태) failed actual MT5(실제 메타트레이더5)?",
                f"- boundary(경계): `{BOUNDARY}`",
                "",
                "Effect(효과): Stage300(300단계)의 shape repair(형태 수정) 실패를 더 끌지 않고, 새 feature/model/risk surface(피처/모델/위험 표면)를 만든다.",
                "",
            ]
        ),
    )
    write_text(
        ...
        "\n".join(
            [
                "# Stage301 Selection Status(301단계 선택 상태)",
                "",
                "- stage_status(단계 상태): `opened_orthogonal_profit_source_rebuild`",
                "- current_packet(현재 작업 묶음): `301_onnx_candidate_campaign__orthogonal_profit_source_rebuild_v1`",
                ...
                f"- stage300_review(300단계 검토): `{rel(REPORT)}`",
                "",
            ]
        ),
    )
    ...
    write_csv(
        ...
        [
            {
                "row_id": "stage301_opened_from_run300C",
                "stage_id": NEXT_STAGE_ID,
                "run_id": RUN_ID,
                "view": "stage_open",
                "tier_scope": "not_applicable",
                "scoreboard": "stage300_review",
                "status": "opened_orthogonal_profit_source_rebuild",
                "judgment": "opened_from_stage300_actual_mt5_negative_profit_scale_failure_memory",
                "evidence_boundary": "planning_from_stage300_actual_mt5_evidence",
                "report_path": rel(REPORT),
                "notes": "next_action=run301A_design_orthogonal_profit_source_rebuild_packet",
            }
        ],
    )
...
def update_docs(status: str, judgment: str, next_action: str) -> None:
    ...
    selected += f"- run300C_report(300C 보고): `{rel(REPORT)}`\n"
    selected += f"- stage301_opened(301단계 열림): `{NEXT_STAGE_ID}`\n"
    write_text(SELECTED, selected)

    review_index = read_text(REVIEW_INDEX)
    review_index += f"- run300C_report(300C 보고): `{rel(REPORT)}`\n"
    review_index += f"- run300C_scoreboard(300C 점수판): `{rel(SCOREBOARD)}`\n"
    review_index += f"- stage301_seed_queue(301단계 씨앗 대기열): `{rel(NEXT_STAGE_QUEUE)}`\n"
    ...
    focus = (
        f"- >-\n"
        f"  Stage300(300단계) run300C(300C 실행) split-forward trade shape generalization MT5 review(런타임 실제 거래 형태 MT5 검토) `{RUN_ID}` closed Stage300 and opened Stage300(300단계). "
        f"Effect(효과): selected candidate(선택 후보), Adapter package(어댑터 패키지), ONNX readiness(온엑스 준비)는 없고 next_action(다음 행동)은 `run301A_design_orthogonal_profit_source_rebuild_packet`이다.\n"
    )
    ...
    changelog += (
        f"\n## {UPDATED_ON} run300C split-forward trade shape generalization review(300C 런타임 실제 거래 형태 검토)\n\n"
        f"- status(상태): `{status}`\n"
...
def update_registers(status: str, judgment: str, next_action: str) -> None:
    upsert(
        ...
        [{"run_id": RUN_ID, "stage_id": STAGE_ID, "lane": "split_forward_trade_shape_generalization_review", "status": status, "judgment": judgment, "path": rel(REPORT), "notes": f"selected_candidate=none;adapter_package=none;onnx_readiness=not_started;next_action={next_action}."}],
        "run_id",
    )
    upsert(
        ALPHA_LEDGER,
        ...
        [
            {
                ...
                "record_view": "split_forward_trade_shape_generalization_review",
    ...
    upsert(
        STAGE_LEDGER,
        STAGE_LEDGER_COLUMNS,
        [{"row_id": f"{RUN_ID}__review", "stage_id": STAGE_ID, "run_id": RUN_ID, "view": "split_forward_trade_shape_generalization_review", "tier_scope": "Tier A used/Tier B fallback/actual routed total", "scoreboard": "split_forward_trade_shape_generalization_review_scoreboard", "status": status, "judgment": judgment, "evidence_boundary": "runtime_probe_review_no_candidate_no_onnx", "report_path": rel(REPORT), "notes": "Stage301 opened; no Adapter; no ONNX(온엑스)."}],
        "row_id",
    )
...
def update_memory_registers(failures: Sequence[Mapping[str, Any]]) -> None:
    ...
    if RUN_ID not in negative:
        negative += (
            f"\n## {RUN_ID} Stage300 split-forward profit scale failure memory(300단계 검증 양수 표본외 음수 기억)\n\n"
        ...
        rows.append(
            {
                ...
                "artifact_type": "stage300_split_forward_trade_shape_generalization_review_artifact",
                "path": rel(path),
                "sha256": ledger.sha256_file_lf_normalized(path),
                "stage_id": STAGE_ID,
                "run_id": RUN_ID,
                "created_at_utc": "2026-05-24T20:30:00Z",
                "notes": "Stage300 review and Stage300 open handoff",
            }
        )
...
def write_result_files(scoreboard: Sequence[Mapping[str, Any]], failure_rows: Sequence[Mapping[str, Any]], quality_rows: Sequence[Mapping[str, Any]]) -> None:
    ...
    status = "completed_split_forward_trade_shape_generalization_review_no_candidate_stage301_opened"
    judgment = "split_forward_trade_shape_generalization_actual_mt5_negative_no_profit_scale_no_adapter_no_onnx"
    next_action = "run301A_design_orthogonal_profit_source_rebuild_packet"
    result_rows = [
        {
            "result_subject": "Stage300 split-forward trade shape generalization actual MT5 review(300단계 런타임 실제 거래 형태 실제 MT5 검토)",
    ...
    write_text(
        ...
        "\n".join(
            [
                "# Stage300 Review Decision(300단계 검토 결정)",
                "",
                f"- decision(결정): `{judgment}`",
                f"- next_stage(다음 단계): `{NEXT_STAGE_ID}`",
                "- effect(효과): Stage300(300단계)는 후보 없음으로 닫고, Stage300(300단계)에서 split-forward trade shape generalization(분할 전진 거래 형태 일반화)을 새 논제로 연다.",
                "",
            ]
        ),
    )
