...
RUN_ID = "run305B_runtime_realized_curve_attribution_mt5_probe_v1"
RUN_NUMBER = "run305B"
SOURCE_RUN_ID = "run305A_design_runtime_realized_curve_attribution_rebuild_v1"
PARENT_RUN_ID = "run304C_review_curve_pocket_aware_profit_source_mt5_probe_v1"
STATUS_PREPARED = "prepared_runtime_realized_curve_attribution_mt5_probe_no_runtime_kpi"
...
EXPLORATION_LABEL = "stage305_Model__RuntimeRealizedCurveAttributionReplay"
SIGNAL_COLUMN = "run305b_route_signal"
FEATURE_ORDER = (SIGNAL_COLUMN,)
COMMON_ROOT = "Project_Obsidian_Prime_v2/stage305/run305B_runtime_curve_attr"
...
MT5_QUEUE = RUN305A / "mt5_probe_queue.csv"
SOURCE_MANIFEST = RUN305A / "candidate_payload_manifest.csv"
SOURCE_RUN_MANIFEST = RUN305A / "run_manifest.json"
SOURCE_REPORT = REVIEWS / "run305A_runtime_realized_curve_attribution_materialization_report.md"
PRODUCER = Path("stage_pipelines/stage305/execute_runtime_realized_curve_attribution_mt5_probe.py")
...
REPORT = REVIEWS / "run305B_runtime_realized_curve_attribution_mt5_probe_report.md"
...
def safe_upsert_rows(path: Path, columns: Sequence[str], rows: Sequence[Mapping[str, Any]], key: str) -> None:
    try:
        runner.prev.e295.e294.base.base.upsert_rows(path, columns, rows, key=key)
        return
    except OSError:
        pass
...
def attach_identity(attempt: dict[str, Any], queue_row: Mapping[str, str], max_hold: int, close_on_flat: bool, same_reentry: int) -> None:
    ...
    attempt["stage305_branch_id"] = queue_row.get("materialized_branch_id", "")
    ...
    for queue_row in queue_rows:
        ...
        for runtime_split, split_token in (("validation_is", "val"), ("oos", "oos")):
            for tier_key, tier_label, tier_token in (
                ...
                attempt["signal_policy"] = "stage305 curve-pocket-aware profit source route_signal_value -1/0/+1 through single-feature table"
            ...
            if include_routed:
                ...
                attach_identity(attempt, queue_row, max_hold, close_on_flat, same_reentry)
                attempt["signal_policy"] = "Tier A primary + Tier B fallback stage305 curve-pocket-aware profit source route_signal_value"
                attempts.append(attempt)
    return attempts
...
def classify_status(result: Mapping[str, Any], materialize_only: bool) -> tuple[str, str, str, str]:
    ...
    if materialize_only:
        return (
            STATUS_PREPARED,
            "runtime_realized_curve_attribution_runtime_probe_prepared_no_external_execution",
            "out_of_scope_by_claim_materialize_only",
            "run305B_execute_runtime_realized_curve_attribution_mt5_probe_external_check",
        )
    ...
    if planned and completed_exec >= planned and len(kpis) >= planned:
        return (
            "completed_runtime_realized_curve_attribution_mt5_probe_no_selection",
            "runtime_probe_completed_requires_profit_scale_curve_review_no_selection",
            "completed",
            "run305C_review_runtime_realized_curve_attribution_mt5_probe",
        )
    if kpis:
        return (
            "partial_runtime_realized_curve_attribution_mt5_probe_no_selection",
            "runtime_probe_partial_requires_continuation_or_review_no_selection",
            "partial_or_blocked",
            "run305B_continue_runtime_realized_curve_attribution_mt5_probe" if limited else "run305C_review_with_runtime_gaps",
        )
    return (
        "blocked_runtime_realized_curve_attribution_mt5_probe_no_kpi",
        "runtime_probe_blocked_no_kpi_no_selection",
        "blocked_or_invalid",
        "run305B_repair_or_block_runtime_realized_curve_attribution_mt5_probe",
    )
...
def report_markdown(result: Mapping[str, Any], status: str, judgment: str, external_status: str, next_action: str) -> str:
    ...
    return "\n".join(
        [
            "# run305B Runtime-Realized Curve Attribution MT5 Probe(305B 怨≪꽑 ?ъ폆 ?몄떇 ?섏씡 ?먯쿇 MT5 ?먯묠)",
            "",
            ...
            "",
            "Effect(?④낵): run305A(305A ?ㅽ뻾)??route_signal_value(寃쎈줈 ?좏샇)瑜??ㅼ젣 MT5 tester(MT5 ?뚯뒪?????ｌ뼱 trade count(嫄곕옒 ??, net profit(?쒖닔??, PF(?섏씡 ?⑺꽣), DD(drawdown, ?먯떎??, recovery(?뚮났)瑜??뺤씤?쒕떎.",
            "",
            f"`{BOUNDARY}`",
        ]
    )
...
def upsert_ledgers(result: Mapping[str, Any], status: str, judgment: str, external_status: str, next_action: str) -> None:
    ...
    safe_upsert_rows(
        ...
        [{"run_id": RUN_ID, "stage_id": STAGE_ID, "lane": "runtime_realized_curve_attribution_mt5_probe", "status": status, "judgment": judgment, "path": runner.prev.e295.e294.base.base.rel(REPORT), "notes": f"attempts={attempt_count};mt5_kpi_records={kpi_count};selected_candidate=none;onnx_readiness=not_claimed;next_action={next_action}."}],
        key="run_id",
    )
    safe_upsert_rows(
        ALPHA_LEDGER,
        ...
        [
            {
                ...
                "record_view": "runtime_realized_curve_attribution_mt5_probe",
    ...
    rows = [
        {
            ...
            "artifact_type": "stage305_runtime_realized_curve_attribution_mt5_artifact",
            "path": runner.prev.e295.e294.base.base.rel(path),
            "sha256": runner.prev.e295.e294.base.base.sha256_file_lf_normalized(path),
            "stage_id": STAGE_ID,
            "run_id": RUN_ID,
            "created_at_utc": created_at,
            "notes": "run305B curve-pocket-aware profit source MT5 probe",
        }
        for path in paths
...
def update_docs(status: str, judgment: str, next_action: str, kpi_count: int, attempt_count: int) -> None:
    ...
    selected = replace_first_prefix(selected, "- stage_status(", f"- stage_status(?④퀎 ?곹깭): `{status}`")
    selected = replace_first_prefix(selected, "- current_run(", f"- current_run(?꾩옱 ?ㅽ뻾): `{RUN_ID}`")
    selected = replace_first_prefix(selected, "- next_action(", f"- next_action(?ㅼ쓬 ?됰룞): `{next_action}`")
    selected = runner.prev.e295.e294.base.base.append_once(selected, "run305B_report", f"- run305B_report(305B 蹂닿퀬): `{runner.prev.e295.e294.base.base.rel(REPORT)}`")
    selected = runner.prev.e295.e294.base.base.append_once(selected, "run305B_execution_result", f"- run305B_execution_result(305B ?ㅽ뻾 寃곌낵): `{runner.prev.e295.e294.base.base.rel(EXECUTION_RESULT)}`")
    runner.prev.e295.e294.base.base.write_md(SELECTED, selected)

    review_index = runner.prev.e295.e294.base.base.io_path(REVIEW_INDEX).read_text(encoding="utf-8-sig") if runner.prev.e295.e294.base.base.path_exists(REVIEW_INDEX) else "# Stage305 Review Index(304?④퀎 寃???됱씤)\n"
    review_index = runner.prev.e295.e294.base.base.append_once(
        review_index,
        "run305B_report",
        f"- run305B_report(305B 蹂닿퀬): `{runner.prev.e295.e294.base.base.rel(REPORT)}`\n- run305B_execution_result(305B ?ㅽ뻾 寃곌낵): `{runner.prev.e295.e294.base.base.rel(EXECUTION_RESULT)}`\n- run305B_mt5_kpi_summary(305B MT5 KPI ?붿빟): `{runner.prev.e295.e294.base.base.rel(MT5_KPI_SUMMARY)}`",
    )
    runner.prev.e295.e294.base.base.write_md(REVIEW_INDEX, review_index)
    ...
    current = replace_first_prefix(current, "- current_run(", f"- current_run(?꾩옱 ?ㅽ뻾): `{RUN_ID}`")
    current = replace_first_prefix(current, "- status(", f"- status(?곹깭): `{status}`")
    current = replace_first_prefix(current, "- next_action(", f"- next_action(?ㅼ쓬 ?됰룞): `{next_action}`")
    current = runner.prev.e295.e294.base.base.append_once(
        current,
        "run305B_summary",
        f"- run305B_summary(305B ?붿빟): curve-pocket-aware profit source MT5 probe(怨≪꽑 ?ъ폆 ?몄떇 ?섏씡 ?먯쿇 MT5 ?먯묠)瑜??ㅽ뻾?덈떎. Effect(?④낵): attempts(?쒕룄) `{attempt_count}`媛쒖? MT5 KPI records(MT5 KPI 湲곕줉) `{kpi_count}`媛쒕? ??뿀怨??꾨낫/?대뙌??ONNX(?⑥뿊?? 二쇱옣? ?섏? ?딅뒗??",
    )
    runner.prev.e295.e294.base.base.write_md(CURRENT_STATE, current)
    ...
    focus = (
        f"- >-\n"
        f"  Stage305(304?④퀎) run305B(305B ?ㅽ뻾) curve-pocket-aware profit source MT5 probe(怨≪꽑 ?ъ폆 ?몄떇 ?섏씡 ?먯쿇 MT5 ?먯묠) `{RUN_ID}`. "
    ...
    runner.prev.e295.e294.base.base.write_md(WORKSPACE_STATE, workspace)

    changelog = runner.prev.e295.e294.base.base.io_path(CHANGELOG).read_text(encoding="utf-8-sig") if runner.prev.e295.e294.base.base.path_exists(CHANGELOG) else "# Changelog(蹂寃?湲곕줉)\n"
    changelog = runner.prev.e295.e294.base.base.append_once(
        changelog,
        RUN_ID,
        f"## {UPDATED_ON} run305B Curve-pocket-aware profit source MT5 probe(305B 怨≪꽑 ?ъ폆 ?몄떇 ?섏씡 ?먯쿇 MT5 ?먯묠)\n\n"
        f"- status(?곹깭): `{status}`\n"
        f"- judgment(?먯젙): `{judgment}`\n"
    ...
    runner.prev.e295.e294.base.base.write_md(CHANGELOG, changelog)
