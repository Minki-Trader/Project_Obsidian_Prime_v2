...
from stage_pipelines.stage364 import review_h17_oos108_pf125_density3_regime_split_repair_without_db as fs
from stage_pipelines.stage364 import train_h17_oos108_pf125_density3_regime_split_repair_without_db as fr
...
TODAY = "2026-06-07"
STAGE_ID = fr.STAGE_ID
RUN_NUMBER = "run364FT"
RUN_ID = "run364FT_train_h17_oos108_pf125_regime_profit_density_reexpand_without_db_v1"
PARENT_RUN_ID = fs.RUN_ID
NEXT_RUN_ID = "run364FU_review_h17_oos108_pf125_regime_profit_density_reexpand_without_db_v1"

STATUS_NO_STRICT = "completed_stage364FT_regime_profit_density_reexpand_no_strict_pass_review_required_no_authority"
STATUS_STRICT = "completed_stage364FT_regime_profit_density_reexpand_strict_proxy_review_required_no_authority"
JUDGMENT_NO_STRICT = "inconclusive_regime_profit_density_reexpand_no_strict_pass_review_required_no_authority"
JUDGMENT_STRICT = "inconclusive_regime_profit_density_reexpand_strict_proxy_review_required_no_authority"
DECISION_NO_STRICT = "stage364FT_open_run364FU_regime_profit_density_reexpand_review"
DECISION_STRICT = "stage364FT_open_run364FU_regime_profit_density_reexpand_review"
CLAIM_BOUNDARY = (
    "research_development_regime_profit_density_reexpand_proxy_and_onnx_smoke_only_no_new_mt5_execution_"
    "no_runtime_package_no_forward_pass_no_live_readiness_no_operating_promotion_no_runtime_authority_no_goal_claim"
)

STAGE_DIR = fr.STAGE_DIR
...
FEATURE_AUDIT = RUN_DIR / "ft_feature_audit.csv"
LABEL_SUMMARY = RUN_DIR / "ft_label_summary.csv"
MODEL_SCORECARD = RUN_DIR / "ft_model_scorecard.csv"
TRADE_SURFACE = RUN_DIR / "ft_surface.csv"
SELECTED_CANDIDATE = RUN_DIR / "selected_ft_candidate.json"
SELECTED_TRADE_TAPE = RUN_DIR / "selected_ft_trade_tape.csv"
MONTH_STABILITY = RUN_DIR / "selected_ft_month_stability.csv"
COST_STRESS = RUN_DIR / "selected_ft_cost_stress.csv"
SIDE_SESSION_REVIEW = RUN_DIR / "selected_ft_side_session_review.csv"
...
RUN364FU_QUEUE = RUN_DIR / "ft_fu_queue.csv"
...
REPORT_PATH = REVIEW_DIR / "run364FT_regime_profit_density_reexpand.md"
DECISION_DOC = ROOT / "docs" / "decisions" / f"{TODAY}_stage364FT_regime_profit_density_reexpand.md"
...
INPUT_FILES = [
    ...
    fs.GATE_AUDIT,
    fs.REVIEW_SUMMARY,
    fs.SURFACE_DIAGNOSTIC,
    fs.FAILURE_ATTRIBUTION,
    fs.PACKAGE_DECISION,
    fs.FAILURE_MEMORY,
    fs.RUN364FT_QUEUE,
    fr.FINAL_DECISION,
    fr.TRADE_SURFACE,
    fr.SELECTED_CANDIDATE,
    fr.SELECTED_TRADE_TAPE,
    fr.COST_STRESS,
    fr.SIDE_SESSION_REVIEW,
    fr.MONTH_STABILITY,
...
LABEL_SPECS = [
    {"label_id": "ft_sym_h1_m0p75", "horizon_m5": 1, "threshold_points": 0.75, "mode": "symmetric"},
    {"label_id": "ft_sym_h1_m1p0", "horizon_m5": 1, "threshold_points": 1.00, "mode": "symmetric"},
    {"label_id": "ft_sym_h2_m1p0", "horizon_m5": 2, "threshold_points": 1.00, "mode": "symmetric"},
    {"label_id": "ft_asym_h2_l1p0_s1p75", "horizon_m5": 2, "threshold_points": 1.35, "long_threshold_points": 1.00, "short_threshold_points": 1.75, "mode": "asymmetric"},
]
TARGET_DENSITIES = [3.0, 3.25, 3.5, 4.0, 4.6]
MARGINS = [-0.20, -0.16, -0.12, -0.08, -0.04, 0.0]
HOUR_SETS = {
    "ft_wide_cash_14_to_22": [14, 15, 16, 17, 18, 19, 20, 21, 22],
    "ft_full_cash_15_to_22": [15, 16, 17, 18, 19, 20, 21, 22],
    "ft_dense_open_close": [14, 15, 16, 17, 18, 20, 21, 22],
    "ft_profit_core_plus_19": [15, 16, 17, 18, 19, 20, 22],
    "ft_late_density_16_to_22": [16, 17, 18, 19, 20, 21, 22],
}
EXTRA_FILTERS = [
    "none",
    "ft_broad_density",
...
def validate_inputs() -> dict[str, Any]:
    ...
    if missing:
        raise FileNotFoundError("missing FT inputs(FT 입력 누락): " + ", ".join(missing))
    with fn.io_path(fs.FINAL_DECISION).open("r", encoding="utf-8-sig") as handle:
        parent = json.load(handle)
    if parent.get("next_run_id") != RUN_ID:
        raise RuntimeError(f"FS next_run_id mismatch(FS 다음 실행 ID 불일치): {parent.get('next_run_id')} != {RUN_ID}")
    for key in ["runtime_authority", "operating_promotion", "live_readiness", "goal_achieve"]:
        if parent.get(key, "not_claimed") != "not_claimed":
            raise RuntimeError(f"forbidden FS claim(금지된 FS 주장): {key}={parent.get(key)}")
    gates = pd.read_csv(fn.io_path(fs.GATE_AUDIT), encoding="utf-8-sig").fillna("")
    if gates.empty or any(gates["status"].astype(str) != "passed"):
        raise RuntimeError("FS gate audit(FS 게이트 감사)가 모두 passed(통과)가 아닙니다.")
    return parent
    ...
    return [
        {
            ...
            "input_role": "FT regime profit density reexpand input(FT 국면 수익 밀도 재확장 입력)",
...
def ft_label_values(frame: pd.DataFrame, spec: Mapping[str, Any]) -> tuple[np.ndarray, np.ndarray]:
...
def ft_feature_sets(feature_order: Sequence[str]) -> dict[str, list[str]]:
    ...
    return {
        "ft_all72": list(dict.fromkeys(base + derived)),
        "ft_profit_density_broad": list(dict.fromkeys(price + macro + session + behavior + derived)),
        "ft_session_regime_broad": list(dict.fromkeys(price + macro + session + derived)),
    }


def ft_model_specs() -> list[tuple[str, str, Any]]:
    return [
        ("et7_l12_n176", "ExtraTrees(엑스트라트리)", et.dt.dp.ExtraTreesClassifier(n_estimators=176, max_depth=7, min_samples_leaf=12, class_weight="balanced", random_state=881, n_jobs=1)),
        ("et8_l18_n176", "ExtraTrees(엑스트라트리)", et.dt.dp.ExtraTreesClassifier(n_estimators=176, max_depth=8, min_samples_leaf=18, class_weight="balanced", random_state=882, n_jobs=1)),
        ("rf8_l24_n176", "RandomForest(랜덤포레스트)", et.dt.dp.RandomForestClassifier(n_estimators=176, max_depth=8, min_samples_leaf=24, class_weight="balanced_subsample", random_state=883, n_jobs=1)),
    ]
...
def ft_extra_mask(frame: pd.DataFrame, side: np.ndarray, extra_filter: str) -> np.ndarray:
    ...
    if extra_filter == "none":
        return mask
    if extra_filter == "ft_broad_density":
        long_ok = (side == "long") & np.isin(hour, [14, 15, 16, 17, 18, 20, 21, 22]) & (breadth >= 0.26) & (vix_stress <= 2.05)
    ...
    if extra_filter == "ft_profit_soft":
        long_ok = (side == "long") & np.isin(hour, [14, 15, 16, 18, 20, 21, 22]) & (breadth >= 0.34) & (range_ratio >= 0.60) & (vix_stress <= 1.82)
        short_ok = (side == "short") & np.isin(hour, [16, 17, 18, 20, 22]) & (vol_ratio >= 0.72) & ((breadth <= 0.59) | (log_return_3 < -0.00006))
        return mask & (long_ok | short_ok)
    if extra_filter == "ft_open_close_blend":
        long_ok = (side == "long") & np.isin(hour, [14, 15, 16, 17, 21, 22]) & (breadth >= 0.30) & (vix_stress <= 1.92)
        short_ok = (side == "short") & np.isin(hour, [15, 16, 17, 18, 20, 22]) & (vol_ratio >= 0.68) & (breadth <= 0.62)
        return mask & (long_ok | short_ok)
    if extra_filter == "ft_side_balance_soft":
        long_ok = (side == "long") & np.isin(hour, [14, 15, 16, 17, 18, 20, 21, 22]) & (breadth >= 0.28) & (vix_stress <= 2.00)
        short_ok = (side == "short") & np.isin(hour, [16, 17, 18, 20, 21, 22]) & (vol_ratio >= 0.66) & (breadth <= 0.63)
        return mask & (long_ok | short_ok)
    if extra_filter == "ft_light_bleed_veto":
        veto = (
            ((side == "short") & (hour == 20) & (vol_ratio < 0.70) & (breadth > 0.54))
            | ((side == "long") & (hour == 19) & (breadth < 0.48) & (vix_stress > 0.95))
            | ((side == "long") & (vix_stress > 2.35))
        )
        return mask & ~veto
    raise ValueError(f"unknown FT filter(알 수 없는 FT 필터): {extra_filter}")


def ft_cost_values(row: Mapping[str, Any]) -> dict[str, float]:
...
def ft_strict_success(row: Mapping[str, Any]) -> bool:
    values = ft_cost_values(row)
...
def ft_operational_stack(row: Mapping[str, Any]) -> bool:
    values = ft_cost_values(row)
    min_pf = min(as_float(row.get("validation_profit_factor")), as_float(row.get("oos_profit_factor")))
    return ft_strict_success(row) and values["combined_net"] >= RUNTIME_NET_REFERENCE and min_pf >= OPERATIONAL_MIN_PF_FLOOR
...
def ft_selection_score(row: Mapping[str, Any]) -> float:
    values = ft_cost_values(row)
    validation_net = as_float(row.get("validation_net"))
    oos_net = as_float(row.get("oos_net"))
    validation_pf = as_float(row.get("validation_profit_factor"))
    ...
    profit_salvage = min(
        readiness(validation_net, 0.0, 190.0),
        readiness(values["combined_net"], 0.0, 190.0),
        readiness(oos_net, -20.0, 180.0),
    )
    pf_bridge = min(readiness(oos_pf, 1.02, 0.24), readiness(min_pf, 0.98, 0.18))
    cost_scout = min(readiness(oos_pf, 1.18, 0.30), readiness(oos_cost09, -80.0, 240.0))
    broad_bonus = 1.0 if (filter_id in {"none", "ft_broad_density", "ft_light_bleed_veto"} and hour_id in {"ft_wide_cash_14_to_22", "ft_full_cash_15_to_22", "ft_dense_open_close"}) else 0.0
    density_positive = density3_ready >= 1.0 and validation_net > 0.0 and values["combined_net"] > 0.0
    all_positive_density = density_positive and oos_net > 0.0
    low_density_cost_only = oos_pf >= OOS_PF_TARGET and oos_cost09 >= 0.0 and min_density < 2.92
    narrow_density_loss = filter_id not in {"none", "ft_broad_density", "ft_light_bleed_veto"} and min_density < 2.95
    strict_shape_bonus = 1.0 if ft_strict_success(row) else 0.0
    return (
        8200.0 * density3_ready
        + 5200.0 * profit_salvage
        + 3600.0 * pf_bridge
        + 1900.0 * cost_scout
        + 1700.0 * broad_bonus
        + 5400.0 * (1.0 if density_positive else 0.0)
        + 7200.0 * (1.0 if all_positive_density else 0.0)
        + 1.55 * validation_net
        + 1.10 * oos_net
        + 0.95 * values["combined_net"]
        + 1650.0 * max(0.0, validation_pf - 1.0)
        + 2150.0 * max(0.0, oos_pf - 1.0)
        + 1000.0 * max(0.0, min_pf - 1.0)
        + 0.95 * validation_cost09
        + 1.70 * oos_cost09
        + 0.95 * values["combined_cost09_net"]
        + 2300.0 * min(min_density, 4.8)
        + 900.0 * min(validation_density, 5.8)
        + 880.0 * min(oos_density, 5.8)
        + 850.0 * max(0.0, PRESERVE_SHORT_SHARE_FLOOR - short_share)
        + 9500.0 * strict_shape_bonus
        - 9400.0 * (1.0 if low_density_cost_only else 0.0)
        - 5200.0 * (1.0 if narrow_density_loss else 0.0)
        - 2400.0 * max(0.0, OOS_PF_TARGET - oos_pf)
        - 2800.0 * (1.0 if validation_net <= 0.0 else 0.0)
        - 1200.0 * (1.0 if oos_net <= -20.0 else 0.0)
        - 5.0 * max(0.0, -validation_cost09)
        - 5.3 * max(0.0, -oos_cost09)
        - 2.2 * max(0.0, -values["combined_cost09_net"] - 160.0)
        - 6200.0 * max(0.0, DENSITY_FLOOR - validation_density)
        - 6200.0 * max(0.0, DENSITY_FLOOR - oos_density)
        - 6500.0 * max(0.0, DENSITY_FLOOR - density)
...
def write_work_packet(parent: Mapping[str, Any]) -> None:
    fn.write_json(
        WORK_PACKET,
        {
            ...
            "hypothesis": "FR profit salvage(FR 수익 회수) 단서를 보존하면서 broad hour/filter(넓은 시간/필터)와 lower label barrier(낮은 라벨 장벽)를 쓰면 density3(밀도3)를 재확장할 수 있습니다.",
            "decision_use": NEXT_RUN_ID,
            "comparison_baseline": PARENT_RUN_ID,
            "control_variables": ["US100 M5", "chronological train/validation/OOS(시간순 학습/검증/표본외)", "no MT5 execution(MT5 실행 없음)", "ONNX smoke only(ONNX 스모크만)"],
            "changed_variables": ["label thresholds(라벨 임계값)", "target densities(목표 밀도)", "hour/filter breadth(시간/필터 폭)", "selection score(선택 점수)"],
            "success_criteria": ["validation_density>=3", "oos_density>=3", "combined_density>=3", "validation_net>0", "combined_net>0", "OOS near breakeven or better(표본외 손익 근처 이상)"],
            "failure_criteria": ["density3_all_splits_count remains 0(전 분할 밀도3이 계속 0)", "profit salvage disappears(수익 회수 단서 소멸)", "low-density cost-only result(저밀도 비용 전용 결과)"],
            "invalid_conditions": ["parent gate failure(부모 게이트 실패)", "lookahead leakage(미래참조 누수)", "missing required input(필수 입력 누락)"],
            "evidence_plan": [rel(TRADE_SURFACE), rel(SELECTED_CANDIDATE), rel(ONNX_SMOKE_REPORT), rel(GATE_AUDIT), rel(RUN_REGISTRY), rel(PROJECT_LEDGER), rel(STAGE_LEDGER)],
            "parent_summary": {
...
def write_queue(summary: Mapping[str, Any]) -> None:
    RUN364FU_QUEUE.parent.mkdir(parents=True, exist_ok=True)
    et.write_csv(
        RUN364FU_QUEUE,
        [
            {
                ...
                "queue_rank": 1,
                "queue_id": "fu01_regime_profit_density_reexpand_review",
                ...
                "effect": "FU review(FU 검토)가 FT의 수익 보존과 밀도 재확장 성공 여부를 분리합니다.",
                "claim_boundary": CLAIM_BOUNDARY,
            }
        ],
    )
...
def write_receipts(final: Mapping[str, Any]) -> None:
    ...
    fn.write_json(EXPERIMENT_RECEIPT, {**base, "hypothesis": "Regime profit density reexpand(국면 수익 밀도 재확장)가 수익 단서와 density3(밀도3)를 같이 회복하는지 시험합니다.", "comparison_baseline": PARENT_RUN_ID, "decision_use": NEXT_RUN_ID})
    ...
    fn.write_json(MODEL_RECEIPT, {**base, "model_family": "RandomForest/ExtraTrees(랜덤포레스트/엑스트라트리)", "target_and_label": "3-class next-open horizon direction(3분류 다음 시가 horizon 방향)", "split_method": "chronological holdout(시간순 홀드아웃)", "selection_metric": "FT score rewards profit salvage plus density3 reexpand(FT 점수는 수익 회수와 밀도3 재확장을 보상)", "threshold_policy": "searched threshold and density target(탐색 임계값과 밀도 목표)", "selected_model_id": final["selected_model_id"], "onnx_smoke_pass_rows": final["onnx_smoke_pass_rows"], "validation_judgment": final["judgment"]})
    fn.write_json(ATTRIBUTION_RECEIPT, {**base, "observed_change": f"selected validation/OOS net/PF/density {final['selected_validation_net']}/{final['selected_validation_profit_factor']}/{final['selected_validation_trade_density']} and {final['selected_oos_net']}/{final['selected_oos_profit_factor']}/{final['selected_oos_trade_density']}", "likely_drivers": ["broader hour set(넓은 시간 집합)", "soft filters(완화 필터)", "lower label barrier(낮은 라벨 장벽)", "density reexpand score(밀도 재확장 점수)"], "next_probe": NEXT_RUN_ID})
    ...
    fn.write_json(CLAIM_RECEIPT, {**base, "runtime_package": "not_opened", "runtime_authority": "not_claimed", "operating_promotion": "not_claimed", "live_readiness": "not_claimed", "goal_achieve": "not_claimed", "effect": "FT 모델 단서를 운영 주장으로 올리지 않습니다."})


def write_docs(final: Mapping[str, Any], gates: Sequence[Mapping[str, Any]]) -> None:
    gate_lines = "\n".join(f"- {row['gate']}: {row['status']} -> {row['evidence']}" for row in gates)
    report = f"""# run364FT Regime Profit Density Reexpand(국면 수익 밀도 재확장)
...
"""
    decision_doc = f"""# Decision(결정): stage364FT Regime Profit Density Reexpand(국면 수익 밀도 재확장)
...
Action(행동): FT model/label/score(FT 모델/라벨/점수) 재시드를 실행하고 FU review(FU 검토)로 넘겼습니다.

Effect(효과): 수익 단서와 밀도 재확장 결과를 운영 주장 없이 다음 판정으로 보냅니다.
"""
    fn.write_text(REPORT_PATH, report, bom=True)
    fn.write_text(DECISION_DOC, decision_doc, bom=True)
    fn.append_text_once(REVIEW_INDEX, f"run364FT__{RUN_ID}", f"\n- run364FT__{RUN_ID}: [{REPORT_PATH.name}]({REPORT_PATH.name}) - regime profit density reexpand(국면 수익 밀도 재확장), next `{NEXT_RUN_ID}`.\n")
    fn.append_text_once(STAGE_BRIEF, f"run364FT__{RUN_ID}", f"\n<!-- run364FT__{RUN_ID} -->\n\n## run364FT Regime Profit Density Reexpand(국면 수익 밀도 재확장)\n\nAction(행동): 수익 회수 단서를 보존하면서 시간/필터 폭과 라벨 장벽을 다시 넓혔습니다.\n\nEffect(효과): `{NEXT_RUN_ID}`에서 패키지 가능성과 실패 경계를 검토합니다.\n")
    fn.append_text_once(STAGE_README, f"run364FT__{RUN_ID}", f"\n<!-- run364FT__{RUN_ID} -->\n## run364FT regime profit density reexpand(국면 수익 밀도 재확장)\n\nNext(다음): `{NEXT_RUN_ID}`.\n")
...
Next action(다음 행동): `{NEXT_RUN_ID}`에서 FT 결과를 review(검토)합니다.
...
Latest scout(최근 탐색): FT regime profit density reexpand(FT 국면 수익 밀도 재확장).
...
""", bom=True)
    fn.append_text_once(WORKSPACE_CHANGELOG, f"run364FT__{RUN_ID}", f"\n<!-- run364FT__{RUN_ID} -->\n- {final['created_at_utc']} `{RUN_ID}` completed regime profit density reexpand(국면 수익 밀도 재확장); strict candidates `{final['strict_candidate_count']}`; selected `{final['selected_model_id']}`; next `{NEXT_RUN_ID}`; no authority claim(권위 주장 없음).\n")
    fn.append_text_once(IDEA_REGISTRY, f"run364FT__{RUN_ID}", f"\n<!-- run364FT__{RUN_ID} -->\n- `{RUN_ID}`: regime profit density reexpand(국면 수익 밀도 재확장)를 학습했습니다. Effect(효과): FS 실패 기억을 수익 보존 + 밀도 재확장 점수로 공격했습니다.\n")
    if int(final["strict_candidate_count"]) == 0:
        fn.append_text_once(NEGATIVE_REGISTER, f"run364FT__strict_candidate_absent__{RUN_ID}", f"\n<!-- run364FT__strict_candidate_absent__{RUN_ID} -->\n- `{RUN_ID}`: regime profit density reexpand(국면 수익 밀도 재확장)는 strict candidate(엄격 후보)를 만들지 못했습니다. Effect(효과): FU에서 실패 경계와 회수 단서를 분리합니다.\n")
...
def write_ledgers(final: Mapping[str, Any], gates: Sequence[Mapping[str, Any]]) -> None:
    ...
    common = {
        ...
        "question": "Can profit salvage reexpand to density3?(수익 회수 단서가 밀도3으로 재확장될 수 있는가?)",
        ...
        rows.append(
            {
                **common,
                ...
                "view": record_view,
                "tier": tier_scope,
                "kpi_scope": "FT regime profit density reexpand(FT 국면 수익 밀도 재확장)",
        ...
        [
            {
                **common,
                ...
                "run_type": "regime_profit_density_reexpand(국면 수익 밀도 재확장)",
            ...
            rows.append(
                {
                    ...
                    "notes": "FT regime profit density reexpand artifact(FT 국면 수익 밀도 재확장 산출물)",
                }
            )
    fn.append_or_replace_csv(ARTIFACT_REGISTRY, ["artifact_id"], rows)


def apply_ft_patch() -> None:
    replacements = {
        "TODAY": TODAY,
        "RUN_NUMBER": RUN_NUMBER,
        "RUN_ID": RUN_ID,
    ...
    fn.fn_label_values = ft_label_values
    fn.fn_feature_sets = ft_feature_sets
    fn.fn_model_specs = ft_model_specs
    fn.fn_extra_mask = ft_extra_mask
    fn.fn_cost_values = ft_cost_values
    fn.fn_strict_success = ft_strict_success
    fn.fn_operational_stack = ft_operational_stack
    fn.fn_selection_score = ft_selection_score
