...
from stage_pipelines.stage364 import materialize_h17_short_source_profit_recovery_runtime_package_without_db as pkg


RUN_NUMBER = "run364DL"
RUN_ID = "run364DL_execute_h17_short_source_profit_recovery_mt5_runtime_probe_without_db_v1"
PARENT_RUN_ID = pkg.RUN_ID
NEXT_RUN_ID = "run364DM_review_h17_short_source_profit_recovery_mt5_runtime_probe_without_db_v1"

STATUS_COMPLETED = "completed_stage364DL_h17_short_source_profit_recovery_mt5_probe_outputs_available_review_required_no_authority"
STATUS_BLOCKED = "blocked_stage364DL_h17_short_source_profit_recovery_mt5_probe_attempt_recorded_repair_required_no_authority"
JUDGMENT_COMPLETED = "mt5_runtime_probe_outputs_available_di02_no19_short_source_proxy_diff_review_required_no_authority"
JUDGMENT_BLOCKED = "mt5_runtime_probe_attempt_recorded_di02_no19_outputs_missing_or_failed_repair_required_no_authority"
DECISION_COMPLETED = "stage364DL_open_run364DM_review_h17_short_source_profit_recovery_mt5_runtime_probe"
DECISION_BLOCKED = "stage364DL_open_run364DM_repair_or_review_h17_short_source_profit_recovery_mt5_runtime_probe"
CLAIM_BOUNDARY = (
    "research_development_mt5_runtime_probe_attempt_only_short_source_profit_recovery_"
...
EXECUTION_SUMMARY = RUN_DIR / "h17_short_source_profit_recovery_mt5_probe_summary.csv"
...
REPORT_PATH = REVIEW_DIR / "run364DL_h17_short_source_profit_recovery_mt5_runtime_probe.md"
DECISION_DOC = pkg.ROOT / "docs" / "decisions" / "2026-06-06_stage364DL_h17_short_source_profit_recovery_mt5_runtime_probe.md"
...
def write_docs(final: Mapping[str, Any], summaries: Sequence[Mapping[str, Any]], proxy_rows: Sequence[Mapping[str, Any]], gates: Sequence[Mapping[str, Any]]) -> None:
    report = f"""# run364DL h17 short-source profit recovery MT5 runtime probe(17시 숏 원천 수익 회복 MT5 런타임 탐침)
...
Action(행동): DK package(DK 패키지) `run364DK_di02_no19_short_source_profit_recovery`를 MT5 Strategy Tester(MT5 전략 테스터)로 실행 시도하고 telemetry/report(원격측정/보고서)를 수집했습니다.
...
"""
    ...
    pkg.write_text(
        DECISION_DOC,
        f"""# Stage364DL decision(결정): h17 short-source profit recovery MT5 runtime probe
...
- effect(효과): DM에서 proxy/MT5 diff(프록시/MT5 차이), equity DD(수익곡선 낙폭), side balance(방향 균형)를 검토합니다.
- claim_boundary(주장 경계): `{CLAIM_BOUNDARY}`
""",
        bom=True,
    )
    pkg.append_text_once(REVIEW_INDEX, f"run364DL__{RUN_ID}", f"\n- run364DL__{RUN_ID}: [{REPORT_PATH.name}]({REPORT_PATH.name}) - DI02 no19 short-source profit recovery MT5 runtime probe(DI02 19시 배제 숏 원천 수익 회복 MT5 런타임 탐침), next `{NEXT_RUN_ID}`.\n")
    pkg.append_text_once(STAGE_BRIEF, f"run364DL__{RUN_ID}", f"\n## run364DL MT5 Runtime Probe(MT5 런타임 탐침)\n\nAction(행동): DI02 no19 package(DI02 no19 패키지)를 Strategy Tester(전략 테스터)로 실행 시도했습니다.\n\nEffect(효과): `{NEXT_RUN_ID}`에서 proxy/MT5 diff(프록시/MT5 차이)를 검토할 수 있습니다.\n")
    pkg.append_text_once(STAGE_README, f"run364DL__{RUN_ID}", f"\n<!-- run364DL__{RUN_ID} -->\n## run364DL MT5 runtime probe(MT5 런타임 탐침)\n\n`run364DK_di02_no19_short_source_profit_recovery` probe(탐침) attempted(시도). Next(다음): `{NEXT_RUN_ID}`.\n")
...
""", bom=True)
    pkg.append_text_once(WORKSPACE_CHANGELOG, f"run364DL__{RUN_ID}", f"\n<!-- run364DL__{RUN_ID} -->\n- {final['created_at_utc']} `{RUN_ID}` attempted DI02 no19 short-source profit recovery MT5 runtime probe(DI02 19시 배제 숏 원천 수익 회복 MT5 런타임 탐침); judgment `{final['judgment']}`; no authority claim(권위 주장 없음).\n")
    pkg.append_text_once(IDEA_REGISTRY, f"run364DL__{RUN_ID}", f"\n<!-- run364DL__{RUN_ID} -->\n- `{RUN_ID}`: DI02 no19 short-source profit recovery(DI02 19시 배제 숏 원천 수익 회복)을 MT5 runtime probe(MT5 런타임 탐침)로 실행 시도했습니다.\n")
...
def write_ledgers(final: Mapping[str, Any], gates: Sequence[Mapping[str, Any]]) -> None:
    common = {
        ...
        "question": "Does DI02 no19 short-source profit recovery survive MT5 runtime probing?(DI02 19시 배제 숏 원천 수익 회복이 MT5 런타임 탐침에서 버티는가?)",
    ...
    for suffix, record_view, tier_scope, status in [
        ...
        row = {**common, "ledger_row_id": f"{RUN_ID}__{suffix}", "subrun_id": f"{RUN_ID}__{suffix}", "row_id": f"{RUN_ID}__{suffix}", "record_view": record_view, "tier_scope": tier_scope, "kpi_scope": "DL MT5 runtime probe(DL MT5 런타임 탐침)", "status": status, "view": record_view, "tier": tier_scope, "metric_scope": "mt5_runtime_probe(MT5 런타임 탐침)"}
