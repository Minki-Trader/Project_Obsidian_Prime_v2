...
from stage_pipelines.stage364 import train_h17_oos108_pf125_profit_floor_density_lift_without_db as gf  # noqa: E402


TODAY = "2026-06-07"
STAGE_ID = gf.STAGE_ID
RUN_NUMBER = "run364GG"
RUN_ID = "run364GG_review_h17_oos108_pf125_profit_floor_density_lift_without_db_v1"
PARENT_RUN_ID = gf.RUN_ID
NEXT_RUN_ID = "run364GH_train_h17_oos108_pf125_density3_profit_floor_repair_without_db_v1"

STATUS = "completed_stage364GG_profit_floor_density_lift_review_validation_improved_density_failed_open_gh_no_authority"
JUDGMENT = "negative_profit_floor_density_lift_review_validation_improved_density_failed_no_package_no_authority"
DECISION = "stage364GG_reject_package_open_run364GH_density3_profit_floor_repair"
CLAIM_BOUNDARY = (
    "research_development_profit_floor_density_lift_review_only_no_runtime_package_no_new_mt5_execution_"
...
REVIEW_SUMMARY = RUN_DIR / "gg_review_summary.csv"
SURFACE_DIAGNOSTIC = RUN_DIR / "gg_surface_diagnostic.csv"
FAILURE_ATTRIBUTION = RUN_DIR / "gg_failure_attribution.csv"
PACKAGE_DECISION = RUN_DIR / "package_decision.csv"
FAILURE_MEMORY = RUN_DIR / "gg_failure_memory.csv"
RUN364GH_QUEUE = RUN_DIR / "gg_gh_queue.csv"
...
REPORT_PATH = REVIEW_DIR / "run364GG_profit_floor_density_lift_review.md"
DECISION_DOC = ROOT / "docs" / "decisions" / f"{TODAY}_stage364GG_profit_floor_density_lift_review.md"
...
INPUT_FILES = [
    ...
    gf.RUN364GG_QUEUE,
    gf.RUN_EVIDENCE_RECEIPT,
    gf.MODEL_RECEIPT,
    gf.ATTRIBUTION_RECEIPT,
    gf.JUDGMENT_RECEIPT,
    gf.LINEAGE_RECEIPT,
    gf.CLAIM_RECEIPT,
    gf.RUN_MANIFEST,
...
def validate_inputs() -> Mapping[str, Any]:
    required = [gf.FINAL_DECISION, gf.GATE_AUDIT, gf.TRADE_SURFACE, gf.SELECTED_CANDIDATE, gf.RUN364GG_QUEUE]
    missing = [rel(path) for path in required if not exists(path)]
    if missing:
        raise FileNotFoundError(f"missing GF review inputs(누락된 GF 검토 입력): {missing}")
    parent = read_json(gf.FINAL_DECISION)
    if parent.get("run_id") != PARENT_RUN_ID:
        raise ValueError(f"parent run mismatch(상위 실행 불일치): {parent.get('run_id')} != {PARENT_RUN_ID}")
    if parent.get("next_run_id") != RUN_ID:
        raise ValueError(f"GF next_run_id mismatch(GF 다음 실행 ID 불일치): {parent.get('next_run_id')} != {RUN_ID}")
    parent_gates = read_csv(gf.GATE_AUDIT)
    if parent_gates.empty or not all(parent_gates["status"].astype(str) == "passed"):
        raise ValueError("GF gate audit did not fully pass(GF 게이트 감사가 모두 통과하지 않음)")
    for key in ["runtime_authority", "operating_promotion", "live_readiness", "goal_achieve"]:
        if parent.get(key, "not_claimed") != "not_claimed":
            raise RuntimeError(f"forbidden GF claim(금지된 GF 주장): {key}={parent.get(key)}")
    surface = read_csv(gf.TRADE_SURFACE)
    if surface.empty:
        raise ValueError("GF surface is empty(GF 표면이 비어 있음)")
    return parent
    ...
    return [
        {
            ...
            "effect": "GG review(GG 검토)의 계보 입력으로 고정했습니다.",
...
def main() -> None:
    ...
    write_csv(RUN364GH_QUEUE, queue)
