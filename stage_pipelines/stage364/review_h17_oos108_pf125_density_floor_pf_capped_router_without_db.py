...
RUN_NUMBER = "run364GQ"
RUN_ID = "run364GQ_review_h17_oos108_pf125_density_floor_pf_capped_router_without_db_v1"
PARENT_RUN_ID = gp.RUN_ID
NEXT_RUN_ID = "run364GR_train_h17_oos108_pf125_cost_near_density_floor_router_without_db_v1"

STATUS = "completed_stage364GQ_density_floor_pf_capped_router_review_selector_repaired_cost_density_incomplete_open_gr_no_authority"
JUDGMENT = "negative_density_floor_pf_capped_router_review_selector_repaired_cost_density_incomplete_no_package_no_authority"
DECISION = "stage364GQ_reject_package_open_run364GR_cost_near_density_floor_router"
CLAIM_BOUNDARY = (
    "research_development_density_floor_pf_capped_router_review_only_no_runtime_package_no_new_mt5_execution_"
...
REVIEW_SUMMARY = RUN_DIR / "gq_review_summary.csv"
SURFACE_DIAGNOSTIC = RUN_DIR / "gq_surface_diagnostic.csv"
FAILURE_ATTRIBUTION = RUN_DIR / "gq_failure_attribution.csv"
PACKAGE_DECISION = RUN_DIR / "package_decision.csv"
FAILURE_MEMORY = RUN_DIR / "gq_failure_memory.csv"
RUN364GR_QUEUE = RUN_DIR / "gq_gr_queue.csv"
...
REPORT_PATH = REVIEW_DIR / "run364GQ_density_floor_pf_capped_router_review.md"
DECISION_DOC = ROOT / "docs" / "decisions" / f"{TODAY}_stage364GQ_density_floor_pf_capped_router_review.md"
...
INPUT_FILES = [
    ...
    gp.GATE_AUDIT,
    gp.TRADE_SURFACE,
    gp.SELECTED_CANDIDATE,
    gp.SELECTED_TRADE_TAPE,
    gp.COST_STRESS,
    gp.SIDE_SESSION_REVIEW,
    gp.MONTH_STABILITY,
    gp.MODEL_SCORECARD,
    gp.MODEL_ARTIFACT_MANIFEST,
    gp.ONNX_SMOKE_REPORT,
    gp.DATA_INTEGRITY_AUDIT,
    gp.RUN364GQ_QUEUE,
    THIS_FILE,
]
...
def validate_inputs() -> dict[str, Any]:
    ...
    if missing:
        raise FileNotFoundError("missing GQ inputs(GQ 입력 누락): " + ", ".join(missing))
    parent = read_json(gp.FINAL_DECISION)
    ...
    if parent.get("next_run_id") != RUN_ID:
        raise RuntimeError(f"GP next_run_id mismatch(GP 다음 실행 ID 불일치): {parent.get('next_run_id')} != {RUN_ID}")
    for key in ["runtime_authority", "operating_promotion", "live_readiness", "goal_achieve"]:
        if parent.get(key, "not_claimed") != "not_claimed":
            raise RuntimeError(f"forbidden GP claim(금지된 GP 주장): {key}={parent.get(key)}")
    gates = pd.read_csv(fn.io_path(gp.GATE_AUDIT), encoding="utf-8-sig").fillna("")
    if gates.empty or any(gates["status"].astype(str) != "passed"):
        raise RuntimeError("GP gate audit(GP 게이트 감사)가 모두 passed(통과)가 아닙니다.")
    return parent
    ...
    return [
        {
            ...
            "input_role": "GQ density-floor PF-capped router review input(GQ 밀도 바닥 PF 캡 라우터 검토 입력)",
...
def build_outputs(parent: Mapping[str, Any]) -> tuple[list[dict[str, Any]], list[dict[str, Any]], list[dict[str, Any]], list[dict[str, Any]], list[dict[str, Any]], list[dict[str, Any]], dict[str, Any]]:
    surface = pd.read_csv(fn.io_path(gp.TRADE_SURFACE), encoding="utf-8-sig").fillna("")
    ...
    selector_repaired = (not sparse_selected) and (not pf999_selected)
    cost_density_incomplete = selected_cost06 < -10.0 or selected_cost09 < -230.0 or selected_density < 2.0
    package_eligible = (
        not sparse_selected
    ...
    summary_rows = [
        {
            ...
            "selector_repaired": str(selector_repaired).lower(),
            "cost_density_incomplete": str(cost_density_incomplete).lower(),
    ...
    attribution = [
        {
            "run_id": RUN_ID,
            "attribution_id": "gq01_selector_repaired_cost_density_still_incomplete",
            "observed_change": "selected candidate moved from 7 trades to 568 combined trades but combined cost0.9 remains negative(선택 후보가 7거래에서 568 합산 거래로 이동했지만 합산 비용0.9는 음수)",
            "comparison_baseline": "GO sparse PF999 failure reviewed before GP(GP 전 GO 희소 PF999 실패)",
            "likely_driver": "PF cap and density floor worked, but score still prefers density over combined cost stress(PF 캡과 밀도 바닥은 작동했지만 점수가 여전히 합산 비용 압박보다 밀도를 선호)",
            "segment_check": "surface density buckets show density>=1.2 rows with better cost and density>=1.8 rows with cost drag(표면 밀도 구간은 밀도1.2 비용 우수 행과 밀도1.8 비용 부담 행을 동시에 보임)",
            "attribution_confidence": "high",
            "claim_boundary": CLAIM_BOUNDARY,
        },
        {
            "run_id": RUN_ID,
            "attribution_id": "gq02_density2_frontier_absent",
            "observed_change": "density>=2.0 positive rows are absent(밀도2.0 이상 양수 행 없음)",
            "comparison_baseline": "GP selected combined density 1.8089(GP 선택 합산 밀도 1.8089)",
            "likely_driver": "density lift beyond 1.8 still needs source/session reblend(밀도 1.8 이후 상승은 소스/세션 재혼합 필요)",
    ...
    package = [
        {
            ...
            "reason": "selector repaired but cost-density frontier is not package-ready(선택기는 수리됐지만 비용-밀도 경계가 패키지 준비 상태가 아님)",
    ...
    failure = [
        {
            "run_id": RUN_ID,
            "memory_id": "gq01_selector_repaired_cost_density_incomplete",
            "failed_boundary": "package-ready density-cost frontier(패키지 준비 밀도-비용 경계)",
            "why_failed": "selected_density=1.8089171975;selected_trades=568;selected_combined_cost09=-286.862;density20_count=0",
            "salvage_value": "PF999 sparse selection is repaired and density>=1.2 cost-near rows exist(PF999 희소 선택은 수리됐고 밀도1.2 비용 근접 행은 있음)",
            "reopen_condition": "select cost-near density floor first, then lift density toward 2.0(비용 근접 밀도 바닥을 먼저 선택한 뒤 밀도를 2.0 쪽으로 올림)",
            "do_not_repeat": "Do not chase density 1.8+ when combined cost0.9 collapses(합산 비용0.9가 무너지면 밀도1.8 이상만 쫓지 말 것).",
            "claim_boundary": CLAIM_BOUNDARY,
        }
    ]
    queue = [
        {
            ...
            "queue_rank": 1,
            "queue_id": "gr01_cost_near_density_floor_router",
            "hypothesis": "cost-near density floor(비용 근접 밀도 바닥)를 먼저 고정하면 GP의 비용 붕괴를 줄이면서 다음 밀도 리프트 후보를 만들 수 있습니다.",
            "required_preserve": "PF cap and hard trade floor(PF 캡과 하드 거래수 바닥)",
            "required_repair": "prefer combined cost0.9 >= -80 and OOS cost0.6 >= -5 before density lift(밀도 리프트 전 합산 비용0.9 -80 이상과 표본외 비용0.6 -5 이상 선호)",
            "avoid": "density lift with combined cost0.9 below -230(합산 비용0.9 -230 아래의 밀도 리프트)",
            "effect": "GR은 GP 선택기 수리 효과를 보존하고 cost-near frontier(비용 근접 경계)를 먼저 고정합니다.",
            "claim_boundary": CLAIM_BOUNDARY,
        }
    ]
    final = {
...
def write_gates(final: Mapping[str, Any]) -> list[dict[str, Any]]:
    gates = [
        ("scope_completion_gate", REVIEW_SUMMARY, "GQ review summary(GQ 검토 요약)를 작성했습니다."),
        ("parent_integrity_gate", INPUT_MANIFEST, "GP 입력 계보를 확인했습니다."),
        ...
        ("failure_memory_gate", FAILURE_MEMORY, "cost-density incomplete failure memory(비용-밀도 미완 실패 기억)를 남겼습니다."),
        ("next_queue_gate", RUN364GR_QUEUE, "GR 수리 큐를 열었습니다."),
...
def write_receipts(final: Mapping[str, Any]) -> None:
    ...
    fn.write_json(RESULT_RECEIPT, {**common, "result_subject": PARENT_RUN_ID, "evidence_available": [rel(REVIEW_SUMMARY), rel(SURFACE_DIAGNOSTIC), rel(PACKAGE_DECISION)], "evidence_missing": ["MT5 runtime package(MT5 런타임 패키지)", "MT5 runtime probe(MT5 런타임 탐침)"], "judgment_label": JUDGMENT, "next_condition": NEXT_RUN_ID, "user_explanation_hook": "GP는 PF999 선택기를 고쳤지만 비용-밀도 경계가 아직 패키지 기준이 아닙니다."})
    fn.write_json(MODEL_RECEIPT, {**common, "model_family": "GP density-floor PF-capped router(GP 밀도 바닥 PF 캡 라우터)", "selection_metric": "PF-capped density-floor score(PF 캡 밀도 바닥 점수)", "validation_judgment": JUDGMENT, "next_condition": "cost-near density floor router(비용 근접 밀도 바닥 라우터)"})
    fn.write_json(ATTRIBUTION_RECEIPT, {**common, "observed_change": "PF999 sparse selection repaired but combined cost0.9 failed(PF999 희소 선택은 수리됐지만 합산 비용0.9 실패)", "comparison_baseline": PARENT_RUN_ID, "likely_drivers": ["PF cap(PF 캡)", "density floor(밀도 바닥)", "cost stress under density lift(밀도 상승 중 비용 압박)"], "segment_checks": [rel(SURFACE_DIAGNOSTIC), rel(gp.SIDE_SESSION_REVIEW)], "trade_shape": "combined trade count 568, density 1.8089, combined cost0.9 -286.862(합산 거래수 568, 밀도 1.8089, 합산 비용0.9 -286.862)", "attribution_confidence": "high", "next_probe": NEXT_RUN_ID})
...
def write_docs(final: Mapping[str, Any], gates: Sequence[Mapping[str, Any]]) -> None:
    ...
    report = f"""# run364GQ Density-Floor PF-Capped Router Review(밀도 바닥 PF 캡 라우터 검토)
...
- selector_repaired(선택기 수리): `{final['selector_repaired']}`
...
"""
    decision_doc = f"""# Decision(결정): stage364GQ Density-Floor PF-Capped Router Review(밀도 바닥 PF 캡 라우터 검토)
...
Action(행동): GP package(패키지)를 reject(거절)하고 GR cost-near density-floor router(GR 비용 근접 밀도 바닥 라우터)를 엽니다.

Effect(효과): PF cap(PF 캡) 수리 효과는 보존하되, 다음에는 비용 근접 경계를 먼저 고정합니다.
"""
    fn.write_text(REPORT_PATH, report, bom=True)
    fn.write_text(DECISION_DOC, decision_doc, bom=True)
    fn.append_text_once(REVIEW_INDEX, f"run364GQ__{RUN_ID}", f"\n- run364GQ__{RUN_ID}: [{REPORT_PATH.name}]({REPORT_PATH.name}) - GP selector repaired but cost-density incomplete(GP 선택기 수리, 비용-밀도 미완), next `{NEXT_RUN_ID}`.\n")
    fn.append_text_once(STAGE_BRIEF, f"run364GQ__{RUN_ID}", f"\n<!-- run364GQ__{RUN_ID} -->\n\n## run364GQ Density-Floor PF-Capped Router Review(밀도 바닥 PF 캡 라우터 검토)\n\nAction(행동): GP의 PF cap(PF 캡) 수리 효과와 비용-밀도 미완을 검토했습니다.\n\nEffect(효과): `{NEXT_RUN_ID}`에서 cost-near density floor(비용 근접 밀도 바닥)를 먼저 고정합니다.\n")
    fn.append_text_once(STAGE_README, f"run364GQ__{RUN_ID}", f"\n<!-- run364GQ__{RUN_ID} -->\n## run364GQ density-floor PF-capped router review(밀도 바닥 PF 캡 라우터 검토)\n\nNext(다음): `{NEXT_RUN_ID}`.\n")
...
Current truth(현재 진실): `run364GQ` reviewed(검토 완료) GP density-floor PF-capped router(GP 밀도 바닥 PF 캡 라우터). GP selected(선택) 후보는 combined density/trades(합산 밀도/거래수) `{final['selected_combined_trade_density']}` / `{final['selected_combined_trade_count']}`로 PF999 sparse selection(PF999 희소 선택)을 수리했지만, combined cost0.9(합산 비용0.9)가 `{final['selected_combined_cost09_net']}`라서 rejected(거절)입니다.

Failure truth(실패 진실): selector repair(선택기 수리)는 성공했지만 cost-density frontier(비용-밀도 경계)는 패키지 기준에 못 미칩니다.

Next action(다음 행동): `{NEXT_RUN_ID}`에서 cost-near density floor router(비용 근접 밀도 바닥 라우터)를 실행합니다.
...
Latest review(최근 검토): GQ rejected(거절) GP cost-density incomplete router(GP 비용-밀도 미완 라우터).

Selected GP model(선택 GP 모델): `{final['review_subject']}`
...
Next seed(다음 씨앗): cost-near density floor router(비용 근접 밀도 바닥 라우터).

Authority(권위): runtime authority(런타임 권위), operating promotion(운영 승격), live readiness(실거래 준비), Goal Achieve(목표 달성)는 모두 not claimed(주장 안 함)입니다.
""", bom=True)
    fn.append_text_once(WORKSPACE_CHANGELOG, f"run364GQ__{RUN_ID}", f"\n<!-- run364GQ__{RUN_ID} -->\n- {final['created_at_utc']} `{RUN_ID}` rejected GP cost-density incomplete selector(GP 비용-밀도 미완 선택기); next(다음) `{NEXT_RUN_ID}`; no authority claim(권위 주장 없음).\n")
    fn.append_text_once(IDEA_REGISTRY, f"run364GQ__{RUN_ID}", f"\n<!-- run364GQ__{RUN_ID} -->\n- `{RUN_ID}`: GP PF cap(PF 캡)은 효과가 있었지만 cost-near density floor(비용 근접 밀도 바닥) 수리가 필요합니다.\n")
    fn.append_text_once(NEGATIVE_REGISTER, f"run364GQ__cost_density_incomplete__{RUN_ID}", f"\n<!-- run364GQ__cost_density_incomplete__{RUN_ID} -->\n- `{RUN_ID}`: selector repaired but cost-density frontier incomplete(선택기 수리 완료, 비용-밀도 경계 미완). Effect(효과): GR에서 비용 근접 경계를 먼저 고정합니다.\n")
...
def write_ledgers(final: Mapping[str, Any]) -> None:
    ...
    common = {
        ...
        "question": "Is GP PF-capped router package-eligible after density-cost review?(GP PF 캡 라우터는 밀도-비용 검토 뒤 패키지 가능인가?)",
        "next_action": NEXT_RUN_ID,
        "notes": f"selector_repaired={final['selector_repaired']};cost_density_incomplete={final['cost_density_incomplete']};density15_cost_near_count={final['density15_cost_near_count']};density20_cost_near_count={final['density20_cost_near_count']};package_eligible={final['package_eligible']}",
        "runtime_authority": "not_claimed",
        "operating_promotion": "not_claimed",
        "goal_achieve": "not_claimed",
    }
        ...
        rows.append(
            {
                **common,
                ...
                "view": record_view,
                "tier": tier_scope,
                "kpi_scope": "GQ density-floor PF-capped router review(GQ 밀도 바닥 PF 캡 라우터 검토)",
        ...
        [
            {
                **common,
                ...
                "run_type": "density_floor_pf_capped_router_review(밀도 바닥 PF 캡 라우터 검토)",
            ...
            rows.append(
                {
                    ...
                    "notes": "GQ density-floor PF-capped router review artifact(GQ 밀도 바닥 PF 캡 라우터 검토 산출물)",
                }
            )
...
def main() -> None:
    ...
    write_csv(RUN364GR_QUEUE, queue)
