...
from stage_pipelines.stage364 import review_h17_oos108_pf125_session_side_loss_veto_rescue_without_db as gc
...
RUN_NUMBER = "run364GD"
RUN_ID = "run364GD_train_h17_oos108_pf125_profit_preserving_density_recovery_without_db_v1"
PARENT_RUN_ID = gc.RUN_ID
NEXT_RUN_ID = "run364GE_review_h17_oos108_pf125_profit_preserving_density_recovery_without_db_v1"

STATUS_NO_STRICT = "completed_stage364GD_profit_preserving_density_recovery_no_strict_pass_review_required_no_authority"
STATUS_STRICT = "completed_stage364GD_profit_preserving_density_recovery_strict_proxy_review_required_no_authority"
JUDGMENT_NO_STRICT = "inconclusive_profit_preserving_density_recovery_no_strict_pass_review_required_no_authority"
JUDGMENT_STRICT = "inconclusive_profit_preserving_density_recovery_strict_proxy_review_required_no_authority"
DECISION_NO_STRICT = "stage364GD_open_run364GE_profit_preserving_density_recovery_review"
DECISION_STRICT = "stage364GD_open_run364GE_profit_preserving_density_recovery_review"
CLAIM_BOUNDARY = (
    "research_development_profit_preserving_density_recovery_proxy_and_onnx_smoke_only_no_new_mt5_execution_"
...
FEATURE_AUDIT = RUN_DIR / "gd_feature_audit.csv"
LABEL_SUMMARY = RUN_DIR / "gd_label_summary.csv"
MODEL_SCORECARD = RUN_DIR / "gd_model_scorecard.csv"
TRADE_SURFACE = RUN_DIR / "gd_surface.csv"
SELECTED_CANDIDATE = RUN_DIR / "selected_gd_candidate.json"
SELECTED_TRADE_TAPE = RUN_DIR / "selected_gd_trade_tape.csv"
MONTH_STABILITY = RUN_DIR / "selected_gd_month_stability.csv"
COST_STRESS = RUN_DIR / "selected_gd_cost_stress.csv"
SIDE_SESSION_REVIEW = RUN_DIR / "selected_gd_side_session_review.csv"
...
RUN364GE_QUEUE = RUN_DIR / "gd_ge_queue.csv"
...
REPORT_PATH = REVIEW_DIR / "run364GD_profit_preserving_density_recovery.md"
DECISION_DOC = ROOT / "docs" / "decisions" / f"{TODAY}_stage364GD_profit_preserving_density_recovery.md"
...
INPUT_FILES = [
    ...
    gc.GATE_AUDIT,
    gc.REVIEW_SUMMARY,
    gc.SURFACE_DIAGNOSTIC,
    gc.FAILURE_ATTRIBUTION,
    gc.PACKAGE_DECISION,
    gc.FAILURE_MEMORY,
    gc.RUN364GD_QUEUE,
    gb.FINAL_DECISION,
...
LABEL_SPECS = [
    {"label_id": "gd_sym_h1_m0p55", "horizon_m5": 1, "threshold_points": 0.55, "mode": "symmetric"},
    {"label_id": "gd_sym_h1_m0p65", "horizon_m5": 1, "threshold_points": 0.65, "mode": "symmetric"},
    {"label_id": "gd_sym_h2_m0p55", "horizon_m5": 2, "threshold_points": 0.55, "mode": "symmetric"},
    {"label_id": "gd_asym_h1_l0p55_s1p10", "horizon_m5": 1, "threshold_points": 0.80, "long_threshold_points": 0.55, "short_threshold_points": 1.10, "mode": "asymmetric"},
]
TARGET_DENSITIES = [2.35, 2.55, 2.75, 2.95, 3.1]
MARGINS = [-0.20, -0.16, -0.12, -0.08, -0.04, 0.02]
HOUR_SETS = {
    "gd_profit_preserve_cash": [15, 16, 17, 18, 19, 20, 21, 22],
    "gd_density_restore_14_21": [14, 15, 16, 17, 18, 19, 20, 21],
    "gd_short18_anchor": [15, 16, 17, 18, 19, 20, 21],
    "gd_long16_17_bridge": [14, 16, 17, 18, 19, 20, 21, 22],
    "gd_cost_guard_cash": [15, 16, 17, 18, 19, 20, 21],
}
EXTRA_FILTERS = [
    "none",
    "gd_profit_preserve_veto",
    "gd_density_addback_guard",
    "gd_short18_cost_guard",
    "gd_validation_loss_veto",
]
...
def validate_inputs() -> dict[str, Any]:
    ...
    with fn.io_path(gc.FINAL_DECISION).open("r", encoding="utf-8-sig") as handle:
    ...
    gates = pd.read_csv(fn.io_path(gc.GATE_AUDIT), encoding="utf-8-sig").fillna("")
...
def gd_feature_sets(feature_order: Sequence[str]) -> dict[str, list[str]]:
    ...
    return {
        "gd_all72": list(dict.fromkeys(base + derived)),
        "gd_profit_density_blend": list(dict.fromkeys(price + macro + session + behavior + derived)),
        "gd_cost_session_regime": list(dict.fromkeys(price + macro + session + derived)),
    }
...
def gd_extra_mask(frame: pd.DataFrame, side: np.ndarray, extra_filter: str) -> np.ndarray:
    ...
    if extra_filter == "none":
        return mask
    if extra_filter == "gd_profit_preserve_veto":
        long_ok = (side == "long") & np.isin(hour, [16, 17, 18, 19, 20]) & (breadth >= 0.28) & (vix_stress <= 2.05)
        short_ok = (side == "short") & np.isin(hour, [17, 18, 20, 21]) & (vol_ratio >= 0.58) & ((breadth <= 0.66) | (log_return_3 < 0.00001))
        return mask & (long_ok | short_ok)
    if extra_filter == "gd_density_addback_guard":
        long_ok = (side == "long") & np.isin(hour, [14, 16, 17, 18, 19, 20]) & (breadth >= 0.25) & (vix_stress <= 2.15)
        short_ok = (side == "short") & np.isin(hour, [16, 17, 18, 20, 21]) & (vol_ratio >= 0.56) & (((hour == 16) & ((breadth <= 0.55) | (log_return_3 < -0.00004))) | (hour != 16))
        return mask & (long_ok | short_ok)
    if extra_filter == "gd_short18_cost_guard":
        long_ok = (side == "long") & np.isin(hour, [16, 17, 18, 20]) & (breadth >= 0.32) & (range_ratio >= 0.50) & (vix_stress <= 1.95)
        short_ok = (side == "short") & np.isin(hour, [17, 18, 20, 21]) & (vol_ratio >= 0.60) & ((hour == 18) | (breadth <= 0.62) | (log_return_3 < -0.00002))
        return mask & (long_ok | short_ok)
    if extra_filter == "gd_validation_loss_veto":
        veto = (
            ((side == "long") & np.isin(hour, [21, 22]))
            | ((side == "short") & (hour == 16) & (breadth > 0.56))
            | ((side == "long") & (vix_stress > 2.25))
            | ((side == "short") & (hour == 19) & (breadth > 0.68) & (vol_ratio < 0.70))
        )
...
def gd_selection_score(row: Mapping[str, Any]) -> float:
    ...
    profit_preserve = min(readiness(validation_net, 0.0, 120.0), readiness(oos_net, 0.0, 140.0), readiness(oos_pf, 1.05, 0.18))
    density_anchor = min(near_density, readiness(validation_net, -5.0, 160.0), readiness(values["combined_net"], 0.0, 260.0))
    profit_anchor = min(
        readiness(oos_net, -5.0, 170.0),
        readiness(oos_pf, 1.02, 0.22),
        readiness(oos_cost09, -180.0, 330.0),
    )
    cost_repair = min(readiness(values["combined_cost09_net"], -180.0, 520.0), readiness(oos_cost09, -60.0, 360.0))
    bridge = min(density_anchor, profit_anchor, max(0.35, profit_preserve))
    preserve_bonus = 1.0 if validation_net > 0.0 and oos_net > 0.0 and oos_pf >= 1.05 else 0.0
    density_addback_bonus = 1.0 if filter_id in {"gd_density_addback_guard", "gd_validation_loss_veto"} else 0.0
    profit_filter_bonus = 1.0 if filter_id in {"none", "gd_profit_preserve_veto", "gd_short18_cost_guard"} else 0.0
    density_scope_bonus = 1.0 if hour_id in {"gd_density_restore_14_21", "gd_profit_preserve_cash", "gd_cost_guard_cash"} else 0.0
    ...
    return (
        ...
        - 3600.0 * (1.0 if oos_pf < 1.05 else 0.0)
