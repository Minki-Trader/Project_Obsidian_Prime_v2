...
from stage_pipelines.stage364 import train_h17_oos108_pf125_profit_preserving_density_recovery_without_db as gd  # noqa: E402


TODAY = "2026-06-07"
STAGE_ID = gd.STAGE_ID
RUN_NUMBER = "run364GE"
RUN_ID = "run364GE_review_h17_oos108_pf125_profit_preserving_density_recovery_without_db_v1"
PARENT_RUN_ID = gd.RUN_ID
NEXT_RUN_ID = "run364GF_train_h17_oos108_pf125_profit_floor_density_lift_without_db_v1"

STATUS = "completed_stage364GE_profit_preserving_density_recovery_review_oos_profit_improved_density_validation_failed_open_gf_no_authority"
JUDGMENT = "negative_profit_preserving_density_recovery_review_oos_profit_improved_density_validation_failed_no_package_no_authority"
DECISION = "stage364GE_reject_package_open_run364GF_profit_floor_density_lift"
CLAIM_BOUNDARY = (
    "research_development_profit_preserving_density_recovery_review_only_no_runtime_package_no_new_mt5_execution_"
...
REVIEW_SUMMARY = RUN_DIR / "ge_review_summary.csv"
SURFACE_DIAGNOSTIC = RUN_DIR / "ge_surface_diagnostic.csv"
FAILURE_ATTRIBUTION = RUN_DIR / "ge_failure_attribution.csv"
PACKAGE_DECISION = RUN_DIR / "package_decision.csv"
FAILURE_MEMORY = RUN_DIR / "ge_failure_memory.csv"
RUN364GF_QUEUE = RUN_DIR / "ge_gf_queue.csv"
...
REPORT_PATH = REVIEW_DIR / "run364GE_profit_preserving_density_recovery_review.md"
DECISION_DOC = ROOT / "docs" / "decisions" / f"{TODAY}_stage364GE_profit_preserving_density_recovery_review.md"
...
INPUT_FILES = [
    ...
    gd.MONTH_STABILITY,
    gd.MODEL_SCORECARD,
    gd.MODEL_ARTIFACT_MANIFEST,
    gd.ONNX_SMOKE_REPORT,
    gd.DATA_INTEGRITY_AUDIT,
    gd.RUN364GE_QUEUE,
    gd.RUN_EVIDENCE_RECEIPT,
    gd.MODEL_RECEIPT,
    gd.ATTRIBUTION_RECEIPT,
    gd.JUDGMENT_RECEIPT,
    gd.LINEAGE_RECEIPT,
    gd.CLAIM_RECEIPT,
    gd.RUN_MANIFEST,
...
def validate_inputs() -> Mapping[str, Any]:
    required = [gd.FINAL_DECISION, gd.GATE_AUDIT, gd.TRADE_SURFACE, gd.SELECTED_CANDIDATE, gd.RUN364GE_QUEUE]
    missing = [rel(path) for path in required if not exists(path)]
    if missing:
        raise FileNotFoundError(f"missing GD review inputs(누락된 GD 검토 입력): {missing}")
    parent = read_json(gd.FINAL_DECISION)
    if parent.get("run_id") != PARENT_RUN_ID:
        raise ValueError(f"parent run mismatch(상위 실행 불일치): {parent.get('run_id')} != {PARENT_RUN_ID}")
    if parent.get("next_run_id") != RUN_ID:
        raise ValueError(f"GD next_run_id mismatch(GD 다음 실행 ID 불일치): {parent.get('next_run_id')} != {RUN_ID}")
    parent_gates = read_csv(gd.GATE_AUDIT)
    if parent_gates.empty or not all(parent_gates["status"].astype(str) == "passed"):
        raise ValueError("GD gate audit did not fully pass(GD 게이트 감사가 모두 통과하지 않음)")
    for key in ["runtime_authority", "operating_promotion", "live_readiness", "goal_achieve"]:
        if parent.get(key, "not_claimed") != "not_claimed":
            raise RuntimeError(f"forbidden GD claim(금지된 GD 주장): {key}={parent.get(key)}")
    surface = read_csv(gd.TRADE_SURFACE)
    if surface.empty:
        raise ValueError("GD surface is empty(GD 표면이 비어 있음)")
    return parent
    ...
    return [
        {
            ...
            "effect": "GE review(GE 검토)의 계보 입력으로 고정했습니다.",
...
def gate_rows(final_written: bool) -> list[dict[str, Any]]:
    gates = [
        ("input_lineage_gate", exists(INPUT_MANIFEST), INPUT_MANIFEST, "GD 입력 계보를 기록했습니다."),
        ("parent_gate_inheritance_gate", exists(gd.GATE_AUDIT), gd.GATE_AUDIT, "GD 게이트 통과 상태를 상속 확인했습니다."),
        ...
        ("failure_memory_gate", exists(FAILURE_MEMORY), FAILURE_MEMORY, "다음 GF 실행의 실패 기억을 기록했습니다."),
        ("next_queue_gate", exists(RUN364GF_QUEUE), RUN364GF_QUEUE, "GF 대기열을 기록했습니다."),
...
def write_ledgers(final: Mapping[str, Any]) -> None:
    ...
    try:
        gd.et.repair_run_registry_line_endings(RUN_ID)
...
def main() -> None:
    ...
    write_csv(RUN364GF_QUEUE, queue)
