...
from stage_pipelines.stage364 import review_h17_oos108_pf125_density_cost_decoupled_bridge_without_db as fo
...
RUN_NUMBER = "run364FP"
RUN_ID = "run364FP_train_h17_oos108_pf125_positive_density_floor_reseed_without_db_v1"
PARENT_RUN_ID = fo.RUN_ID
NEXT_RUN_ID = "run364FQ_review_h17_oos108_pf125_positive_density_floor_reseed_without_db_v1"

STATUS_NO_STRICT = "completed_stage364FP_positive_density_floor_reseed_no_strict_pass_review_required_no_authority"
STATUS_STRICT = "completed_stage364FP_positive_density_floor_reseed_strict_proxy_review_required_no_authority"
JUDGMENT_NO_STRICT = "inconclusive_positive_density_floor_reseed_no_strict_pass_review_required_no_authority"
JUDGMENT_STRICT = "inconclusive_positive_density_floor_reseed_strict_proxy_review_required_no_authority"
DECISION_NO_STRICT = "stage364FP_open_run364FQ_positive_density_floor_reseed_review"
DECISION_STRICT = "stage364FP_open_run364FQ_positive_density_floor_reseed_review"
CLAIM_BOUNDARY = (
    "research_development_positive_density_floor_reseed_proxy_and_onnx_smoke_only_no_new_mt5_execution_"
...
FEATURE_AUDIT = RUN_DIR / "fp_feature_audit.csv"
LABEL_SUMMARY = RUN_DIR / "fp_label_summary.csv"
MODEL_SCORECARD = RUN_DIR / "fp_model_scorecard.csv"
TRADE_SURFACE = RUN_DIR / "fp_surface.csv"
SELECTED_CANDIDATE = RUN_DIR / "selected_fp_candidate.json"
SELECTED_TRADE_TAPE = RUN_DIR / "selected_fp_trade_tape.csv"
MONTH_STABILITY = RUN_DIR / "selected_fp_month_stability.csv"
COST_STRESS = RUN_DIR / "selected_fp_cost_stress.csv"
SIDE_SESSION_REVIEW = RUN_DIR / "selected_fp_side_session_review.csv"
...
RUN364FQ_QUEUE = RUN_DIR / "fp_fq_queue.csv"
...
REPORT_PATH = REVIEW_DIR / "run364FP_positive_density_floor_reseed.md"
DECISION_DOC = ROOT / "docs" / "decisions" / f"{TODAY}_stage364FP_positive_density_floor_reseed.md"
...
INPUT_FILES = [
    ...
    fo.GATE_AUDIT,
    fo.REVIEW_SUMMARY,
    fo.SURFACE_DIAGNOSTIC,
    fo.FAILURE_ATTRIBUTION,
    fo.PACKAGE_DECISION,
    fo.FAILURE_MEMORY,
    fo.RUN364FP_QUEUE,
    fn.FINAL_DECISION,
...
LABEL_SPECS = [
    {"label_id": "fp_sym_h1_m1p5", "horizon_m5": 1, "threshold_points": 1.50, "mode": "symmetric"},
    {"label_id": "fp_sym_h2_m1p75", "horizon_m5": 2, "threshold_points": 1.75, "mode": "symmetric"},
    {"label_id": "fp_sym_h2_m2p0", "horizon_m5": 2, "threshold_points": 2.00, "mode": "symmetric"},
    {"label_id": "fp_asym_h2_l1p75_s2p75", "horizon_m5": 2, "threshold_points": 2.25, "long_threshold_points": 1.75, "short_threshold_points": 2.75, "mode": "asymmetric"},
]
TARGET_DENSITIES = [3.0, 3.05, 3.15, 3.3, 3.55, 4.0, 4.75]
MARGINS = [-0.14, -0.10, -0.06, -0.03, 0.0]
HOUR_SETS = {
    "fp_cash_15_to_22": [15, 16, 17, 18, 19, 20, 21, 22],
    "fp_dense_no_h19": [15, 16, 17, 18, 20, 21, 22],
    "fp_validation_density_15_16_17_18_20_22": [15, 16, 17, 18, 20, 22],
    "fp_oos_soft_bridge_15_16_18_20_22": [15, 16, 18, 20, 22],
    "fp_cost_scout_16_18_20_22": [16, 18, 20, 22],
}
EXTRA_FILTERS = [
    "none",
    "fp_broad_density",
    "fp_validation_profit",
    "fp_oos_soft_guard",
    "fp_balanced_density",
    "fp_no_bleed_soft",
]
...
def validate_inputs() -> dict[str, Any]:
    ...
    if missing:
        raise FileNotFoundError("missing FP inputs(FP 입력 누락): " + ", ".join(missing))
    with fn.io_path(fo.FINAL_DECISION).open("r", encoding="utf-8-sig") as handle:
        parent = json.load(handle)
    if parent.get("next_run_id") != RUN_ID:
        raise RuntimeError(f"FO next_run_id mismatch(FO 다음 실행 ID 불일치): {parent.get('next_run_id')} != {RUN_ID}")
    for key in ["runtime_authority", "operating_promotion", "live_readiness", "goal_achieve"]:
        if parent.get(key, "not_claimed") != "not_claimed":
            raise RuntimeError(f"forbidden FO claim(금지된 FO 주장): {key}={parent.get(key)}")
    gates = pd.read_csv(fn.io_path(fo.GATE_AUDIT), encoding="utf-8-sig").fillna("")
    if gates.empty or any(gates["status"].astype(str) != "passed"):
        raise RuntimeError("FO gate audit(FO 게이트 감사)가 모두 passed(통과)가 아닙니다.")
    return parent
    ...
    return [
        {
            ...
            "input_role": "FP positive density floor reseed input(FP 양수 밀도 바닥 재시드 입력)",
...
def fp_label_values(frame: pd.DataFrame, spec: Mapping[str, Any]) -> tuple[np.ndarray, np.ndarray]:
...
def fp_feature_sets(feature_order: Sequence[str]) -> dict[str, list[str]]:
    ...
    return {
        "fp_all72": list(dict.fromkeys(base + derived)),
        "fp_positive_density": list(dict.fromkeys(price + macro + session + behavior + derived)),
        "fp_validation_stability": list(dict.fromkeys(price + session + behavior + derived)),
    }


def fp_model_specs() -> list[tuple[str, str, Any]]:
    return [
        ("et7_l12_n176", "ExtraTrees(엑스트라트리)", et.dt.dp.ExtraTreesClassifier(n_estimators=176, max_depth=7, min_samples_leaf=12, class_weight="balanced", random_state=861, n_jobs=1)),
        ("et8_l18_n176", "ExtraTrees(엑스트라트리)", et.dt.dp.ExtraTreesClassifier(n_estimators=176, max_depth=8, min_samples_leaf=18, class_weight="balanced", random_state=862, n_jobs=1)),
        ("rf8_l22_n176", "RandomForest(랜덤포레스트)", et.dt.dp.RandomForestClassifier(n_estimators=176, max_depth=8, min_samples_leaf=22, class_weight="balanced_subsample", random_state=863, n_jobs=1)),
    ]
...
def fp_extra_mask(frame: pd.DataFrame, side: np.ndarray, extra_filter: str) -> np.ndarray:
    ...
    if extra_filter == "none":
        return mask
    if extra_filter == "fp_broad_density":
        long_ok = (side == "long") & np.isin(hour, [15, 16, 17, 18, 20, 21, 22]) & (breadth >= 0.30) & (vix_stress <= 1.85)
        short_ok = (side == "short") & np.isin(hour, [16, 17, 18, 20, 22]) & (vol_ratio >= 0.70) & ((breadth <= 0.61) | (log_return_3 < -0.00004))
        return mask & (long_ok | short_ok)
    if extra_filter == "fp_validation_profit":
        long_ok = (side == "long") & np.isin(hour, [15, 16, 18, 20, 21, 22]) & (breadth >= 0.38) & (range_ratio >= 0.66) & (vix_stress <= 1.65)
        short_ok = (side == "short") & np.isin(hour, [16, 18, 22]) & (vol_ratio >= 0.82) & ((breadth <= 0.56) | (log_return_3 < -0.00010))
        return mask & (long_ok | short_ok)
    if extra_filter == "fp_oos_soft_guard":
        long_ok = (side == "long") & np.isin(hour, [15, 16, 18, 21, 22]) & (breadth >= 0.40) & (vix_stress <= 1.55)
        short_ok = (side == "short") & np.isin(hour, [16, 18, 20, 22]) & (vol_ratio >= 0.84) & ((breadth <= 0.56) | (log_return_3 < -0.00010))
        return mask & (long_ok | short_ok)
    if extra_filter == "fp_balanced_density":
        long_ok = (side == "long") & np.isin(hour, [15, 16, 17, 18, 20, 22]) & (breadth >= 0.34) & (range_ratio >= 0.62)
        short_ok = (side == "short") & np.isin(hour, [16, 17, 18, 22]) & (vol_ratio >= 0.76) & (breadth <= 0.59)
        return mask & (long_ok | short_ok)
    if extra_filter == "fp_no_bleed_soft":
        veto = (
            ((side == "short") & (hour == 20) & (vol_ratio < 0.82))
            | ((side == "short") & (hour == 17) & (breadth > 0.62))
            | ((side == "long") & (hour == 19) & (breadth < 0.54))
            | ((side == "long") & (vix_stress > 1.95))
        )
        return mask & ~veto
    raise ValueError(f"unknown FP filter(알 수 없는 FP 필터): {extra_filter}")


def fp_cost_values(row: Mapping[str, Any]) -> dict[str, float]:
...
def fp_strict_success(row: Mapping[str, Any]) -> bool:
    values = fp_cost_values(row)
...
def fp_operational_stack(row: Mapping[str, Any]) -> bool:
    values = fp_cost_values(row)
    min_pf = min(as_float(row.get("validation_profit_factor")), as_float(row.get("oos_profit_factor")))
    return fp_strict_success(row) and values["combined_net"] >= RUNTIME_NET_REFERENCE and min_pf >= OPERATIONAL_MIN_PF_FLOOR
...
def fp_selection_score(row: Mapping[str, Any]) -> float:
    values = fp_cost_values(row)
    validation_net = as_float(row.get("validation_net"))
    oos_net = as_float(row.get("oos_net"))
    validation_pf = as_float(row.get("validation_profit_factor"))
    ...
    positive_density_ready = min(
        readiness(validation_density, DENSITY_FLOOR, 0.40),
        readiness(density, DENSITY_FLOOR, 0.45),
        readiness(oos_density, 2.90, 0.45),
        readiness(validation_net, 0.0, 160.0),
    )
    cross_positive_ready = min(
        readiness(oos_net, 0.0, 140.0),
        readiness(oos_pf, 1.05, 0.18),
        readiness(oos_density, 2.90, 0.45),
    )
    cost_scout_ready = min(readiness(oos_pf, OOS_PF_TARGET, 0.35), readiness(oos_cost09, 0.0, 260.0))
    ...
    low_density_cost_only = oos_pf >= OOS_PF_TARGET and oos_cost09 >= 0.0 and min_density < 2.80
    strict_shape_bonus = 1.0 if fp_strict_success(row) else 0.0
    return (
        7600.0 * positive_density_ready
        + 3900.0 * cross_positive_ready
        + 1800.0 * cost_scout_ready
        + 3600.0 * (1.0 if validation_positive_density else 0.0)
        + 5200.0 * (1.0 if all_positive_density else 0.0)
        + 1.65 * validation_net
        + 1.08 * oos_net
        + 0.72 * values["combined_net"]
        + 1650.0 * max(0.0, validation_pf - 1.0)
        + 1900.0 * max(0.0, oos_pf - 1.0)
        + 920.0 * max(0.0, min_pf - 1.0)
        + 1.20 * validation_cost09
        + 1.25 * values["combined_cost09_net"]
        + 1.90 * oos_cost09
        + 1600.0 * min(min_density, 4.4)
        + 780.0 * min(validation_density, 5.5)
        + 560.0 * min(oos_density, 5.5)
        + 600.0 * max(0.0, PRESERVE_SHORT_SHARE_FLOOR - short_share)
        + 8200.0 * strict_shape_bonus
        - 9000.0 * (1.0 if low_density_cost_only else 0.0)
        - 2300.0 * max(0.0, OOS_PF_TARGET - oos_pf)
        - 3600.0 * (1.0 if validation_net <= 0.0 else 0.0)
        - 1250.0 * (1.0 if oos_net <= 0.0 else 0.0)
        - 6.0 * max(0.0, -validation_cost09)
        - 5.0 * max(0.0, -oos_cost09)
        - 2.4 * max(0.0, -values["combined_cost09_net"] - 120.0)
        - 6200.0 * max(0.0, DENSITY_FLOOR - validation_density)
        - 5000.0 * max(0.0, 2.90 - oos_density)
...
def write_work_packet(parent: Mapping[str, Any]) -> None:
    fn.write_json(
        WORK_PACKET,
        {
            ...
            "hypothesis": "FP positive density floor reseed(FP 양수 밀도 바닥 재시드)가 validation positive density3(검증 양수 밀도3)를 먼저 복구하고 OOS PF/cost(표본외 수익 팩터/비용)를 scout clue(탐색 단서)로 붙입니다.",
            "preserve_conditions": ["OOS PF125/cost0.9 scout clue(표본외 PF125/비용0.9 탐색 단서)", "short_share<=0.77(숏 비중 0.77 이하)"],
            ...
            "avoid": "low-density cost-only package review(저밀도 비용 전용 패키지 검토)",
            "parent_summary": {
                ...
                "density3_all_splits_oos_pf105_count": parent.get("density3_all_splits_oos_pf105_count"),
                "oos_pf125_cost09_count": parent.get("oos_pf125_cost09_count"),
                "oos_pf125_cost09_density3_count": parent.get("oos_pf125_cost09_density3_count"),
            },
            "claim_boundary": CLAIM_BOUNDARY,
        },
    )
...
def write_queue(summary: Mapping[str, Any]) -> None:
    RUN364FQ_QUEUE.parent.mkdir(parents=True, exist_ok=True)
    et.write_csv(
        RUN364FQ_QUEUE,
        [
            {
                ...
                "queue_rank": 1,
                "queue_id": "fq01_positive_density_floor_reseed_review",
                ...
                "effect": "FQ review(FQ 검토)가 FP의 positive density floor reseed(양수 밀도 바닥 재시드)가 package(패키지) 후보인지 실패 기억인지 분리합니다.",
                "claim_boundary": CLAIM_BOUNDARY,
            }
        ],
    )
...
def write_receipts(final: Mapping[str, Any]) -> None:
    ...
    fn.write_json(EXPERIMENT_RECEIPT, {**base, "hypothesis": "FP positive density floor reseed(FP 양수 밀도 바닥 재시드)가 검증 양수 밀도3을 먼저 복구하는지 시험합니다.", "comparison_baseline": PARENT_RUN_ID, "decision_use": NEXT_RUN_ID})
    ...
    fn.write_json(MODEL_RECEIPT, {**base, "model_family": "RandomForest/ExtraTrees(랜덤포레스트/엑스트라트리)", "target_and_label": "3-class next-open horizon direction(3분류 다음 시가 horizon 방향)", "split_method": "chronological holdout(시간순 홀드아웃)", "selection_metric": "FP score repairs positive density floor first(FP 점수는 양수 밀도 바닥을 먼저 수리)", "threshold_policy": "searched threshold and density target(탐색 임계값과 밀도 목표)", "selected_model_id": final["selected_model_id"], "onnx_smoke_pass_rows": final["onnx_smoke_pass_rows"], "validation_judgment": final["judgment"]})
    fn.write_json(ATTRIBUTION_RECEIPT, {**base, "observed_change": f"selected validation/OOS net/PF/density {final['selected_validation_net']}/{final['selected_validation_profit_factor']}/{final['selected_validation_trade_density']} and {final['selected_oos_net']}/{final['selected_oos_profit_factor']}/{final['selected_oos_trade_density']}", "likely_drivers": ["positive density floor score(양수 밀도 바닥 점수)", "cost scout penalty(비용 탐색 벌점)", "broad density labels(넓은 밀도 라벨)"], "next_probe": NEXT_RUN_ID})
    ...
    fn.write_json(CLAIM_RECEIPT, {**base, "runtime_package": "not_opened", "runtime_authority": "not_claimed", "operating_promotion": "not_claimed", "live_readiness": "not_claimed", "goal_achieve": "not_claimed", "effect": "FP 모델 단서를 운영 주장으로 올리지 않습니다."})


def write_docs(final: Mapping[str, Any], gates: Sequence[Mapping[str, Any]]) -> None:
    gate_lines = "\n".join(f"- {row['gate']}: {row['status']} -> {row['evidence']}" for row in gates)
    report = f"""# run364FP Positive Density Floor Reseed(양수 밀도 바닥 재시드)
...
"""
    decision_doc = f"""# Decision(결정): stage364FP Positive Density Floor Reseed(양수 밀도 바닥 재시드)
...
Action(행동): FP model/label/score(모델/라벨/점수) 재시드를 실행하고 FQ review(FQ 검토)로 넘겼습니다.

Effect(효과): 양수 밀도 바닥 회복 여부를 운영 주장 없이 다음 판정으로 보냅니다.
"""
    fn.write_text(REPORT_PATH, report, bom=True)
    fn.write_text(DECISION_DOC, decision_doc, bom=True)
    fn.append_text_once(REVIEW_INDEX, f"run364FP__{RUN_ID}", f"\n- run364FP__{RUN_ID}: [{REPORT_PATH.name}]({REPORT_PATH.name}) - positive density floor reseed(양수 밀도 바닥 재시드), next `{NEXT_RUN_ID}`.\n")
    fn.append_text_once(STAGE_BRIEF, f"run364FP__{RUN_ID}", f"\n<!-- run364FP__{RUN_ID} -->\n\n## run364FP Positive Density Floor Reseed(양수 밀도 바닥 재시드)\n\nAction(행동): validation positive density3(검증 양수 밀도3)를 먼저 복구하도록 모델을 학습했습니다.\n\nEffect(효과): `{NEXT_RUN_ID}`에서 패키지 가능성과 실패 경계를 검토합니다.\n")
    fn.append_text_once(STAGE_README, f"run364FP__{RUN_ID}", f"\n<!-- run364FP__{RUN_ID} -->\n## run364FP positive density floor reseed(양수 밀도 바닥 재시드)\n\nNext(다음): `{NEXT_RUN_ID}`.\n")
...
Next action(다음 행동): `{NEXT_RUN_ID}`에서 FP 결과를 review(검토)합니다.
...
Latest scout(최근 탐색): FP positive density floor reseed(FP 양수 밀도 바닥 재시드).
...
""", bom=True)
    fn.append_text_once(WORKSPACE_CHANGELOG, f"run364FP__{RUN_ID}", f"\n<!-- run364FP__{RUN_ID} -->\n- {final['created_at_utc']} `{RUN_ID}` completed positive density floor reseed(양수 밀도 바닥 재시드); strict candidates `{final['strict_candidate_count']}`; selected `{final['selected_model_id']}`; next `{NEXT_RUN_ID}`; no authority claim(권위 주장 없음).\n")
    fn.append_text_once(IDEA_REGISTRY, f"run364FP__{RUN_ID}", f"\n<!-- run364FP__{RUN_ID} -->\n- `{RUN_ID}`: positive density floor reseed(양수 밀도 바닥 재시드)를 학습했습니다. Effect(효과): FO 실패 기억을 validation positive density3(검증 양수 밀도3) 우선 점수로 공격했습니다.\n")
    if int(final["strict_candidate_count"]) == 0:
        fn.append_text_once(NEGATIVE_REGISTER, f"run364FP__strict_candidate_absent__{RUN_ID}", f"\n<!-- run364FP__strict_candidate_absent__{RUN_ID} -->\n- `{RUN_ID}`: positive density floor reseed(양수 밀도 바닥 재시드)는 strict candidate(엄격 후보)를 만들지 못했습니다. Effect(효과): FQ에서 실패 경계와 회수 단서를 분리합니다.\n")
...
def write_ledgers(final: Mapping[str, Any], gates: Sequence[Mapping[str, Any]]) -> None:
    ...
    common = {
        ...
        "question": "Can positive density floor reseed recover validation-positive density3 before OOS PF125?(양수 밀도 바닥 재시드가 표본외 PF125 전 검증 양수 밀도3을 회복할 수 있는가?)",
        ...
        rows.append(
            {
                **common,
                ...
                "view": record_view,
                "tier": tier_scope,
                "kpi_scope": "FP positive density floor reseed(FP 양수 밀도 바닥 재시드)",
        ...
        [
            {
                **common,
                ...
                "run_type": "positive_density_floor_reseed(양수 밀도 바닥 재시드)",
            ...
            rows.append(
                {
                    ...
                    "notes": "FP positive density floor reseed artifact(FP 양수 밀도 바닥 재시드 산출물)",
                }
            )
    fn.append_or_replace_csv(ARTIFACT_REGISTRY, ["artifact_id"], rows)


def apply_fp_patch() -> None:
    replacements = {
        "TODAY": TODAY,
        "RUN_NUMBER": RUN_NUMBER,
        "RUN_ID": RUN_ID,
    ...
    fn.fn_label_values = fp_label_values
    fn.fn_feature_sets = fp_feature_sets
    fn.fn_model_specs = fp_model_specs
    fn.fn_extra_mask = fp_extra_mask
    fn.fn_cost_values = fp_cost_values
    fn.fn_strict_success = fp_strict_success
    fn.fn_operational_stack = fp_operational_stack
    fn.fn_selection_score = fp_selection_score
