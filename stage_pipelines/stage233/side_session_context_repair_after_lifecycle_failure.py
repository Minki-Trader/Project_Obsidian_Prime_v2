...
STAGE_ID = "233_adapter_research__side_session_context_repair_after_lifecycle_failure"
RUN_NUMBER = "run233A"
RUN_ID = "run233A_stage233_side_session_context_repair_after_lifecycle_failure_v1"
PACKET_ID = "stage233_side_session_context_repair_after_lifecycle_failure_v1"
PARENT_RUN_ID = "run232A_stage232_stage231_lifecycle_followup_review_v1"
SOURCE_STAGE_ID = "232_adapter_research__stage231_lifecycle_followup_review"
SOURCE_RUN_ID = PARENT_RUN_ID
SOURCE_STAGE232_EVIDENCE_COMMIT = "b31f4e73e8e233e03329c03ecb6100629588c74e"
SOURCE_STAGE232_HASH_RECORD_COMMIT = "15ecd17764cafd2090ec9b289665b71d35f28c89"
SOURCE_STAGE231_RUN_ID = "run231A_stage231_midpf_oos_repair_after_guard_blend_failure_v1"
NEXT_STAGE_ID = "234_adapter_research__stage233_side_session_context_followup_review"
NEXT_RUN_ID = "run234A_stage234_stage233_side_session_context_followup_review_v1"
NEXT_PACKET_ID = "stage234_stage233_side_session_context_followup_review_v1"
...
VALIDATION_RECOVERY_CLUE = {
    ...
    "validation_net": 982.55,
    "validation_early_pf": 1.746100566,
    "validation_mid_pf": 1.311844654,
    "validation_dd": 12.7188,
    "oos_net": 620.85,
    "oos_pf": 1.67,
    "oos_dd": 13.972,
}
...
COMMON_ROOT = f"OPV2/s233a/{RUN_NUMBER}"

SUMMARY_JSON_PATH = REVIEWS_ROOT / "stage233_summary.json"
SUMMARY_CSV_PATH = REVIEWS_ROOT / "stage233_side_session_context_kpi_summary.csv"
REPORT_PATH = REVIEWS_ROOT / "stage233_side_session_context_repair_report.md"
SEGMENT_KPI_PATH = REVIEWS_ROOT / "stage233_segment_kpi_summary.csv"
BALANCE_CURVE_AUDIT_PATH = REVIEWS_ROOT / "stage233_balance_curve_audit.csv"
MONTHLY_KPI_PATH = REVIEWS_ROOT / "stage233_monthly_kpi_summary.csv"
CONCENTRATION_PATH = REVIEWS_ROOT / "stage233_concentration_risk_summary.csv"
DRAWDOWN_PATH = REVIEWS_ROOT / "stage233_drawdown_recovery_summary.csv"
QUALITY_MATRIX_PATH = REVIEWS_ROOT / "stage233_quality_matrix.csv"
RISK_ATR_TELEMETRY_PATH = REVIEWS_ROOT / "stage233_risk_atr_telemetry.csv"
GATE_FEATURE_SUMMARY_PATH = REVIEWS_ROOT / "stage233_gate_feature_summary.csv"
PROBABILITY_BINDING_PATH = REVIEWS_ROOT / "stage233_probability_telemetry_summary.csv"
MODEL_SCORE_AUDIT_PATH = REVIEWS_ROOT / "stage233_model_score_audit.csv"
TIER_B_DIAGNOSTIC_PATH = REVIEWS_ROOT / "stage233_tier_b_diagnostic_summary.csv"
DECISION_PATH = REVIEWS_ROOT / "stage233_decision.md"
AUDIT_CSV_PATH = REVIEWS_ROOT / "stage233_trade_audit.csv"
...
PRODUCER_PATH = Path("stage_pipelines/stage233/side_session_context_repair_after_lifecycle_failure.py")
ARTIFACT_COLUMNS = prior.ARTIFACT_COLUMNS

VARIANTS = (
    repair.RepairVariant(
        adapter_id="s233_session_ref_h3_cd8",
        label="stage233_session_ref_h3_cd8",
        atr_enabled=True,
        ...
        max_hold_bars=3,
        notes="Stage233 control: Stage231 session-only OOS bound.",
    ),
    repair.RepairVariant(
        adapter_id="s233_session_p5_h3_cd8",
        label="stage233_session_p5_h3_cd8",
        atr_enabled=True,
        model_risk_enabled=True,
        fixed_lot=0.25,
        atr_stop_multiplier=2.0325,
        ...
        close_only_on_opposite_signal=False,
        max_hold_bars=3,
        notes="Stage233 repair: wider session long block, no lifecycle change.",
    ),
    repair.RepairVariant(
        adapter_id="s233_session_p10_h3_cd8",
        label="stage233_session_p10_h3_cd8",
        atr_enabled=True,
        model_risk_enabled=True,
        fixed_lot=0.25,
        atr_stop_multiplier=2.0325,
        ...
        close_only_on_opposite_signal=False,
        max_hold_bars=3,
        notes="Stage233 repair: shifted session long block, no lifecycle change.",
    ),
    repair.RepairVariant(
        adapter_id="s233_cashopen_long_h3_cd8",
        label="stage233_cashopen_long_h3_cd8",
        atr_enabled=True,
        model_risk_enabled=True,
        fixed_lot=0.25,
        atr_stop_multiplier=2.0325,
...
def apply_stage_values(context_specs: Mapping[str, Any]) -> None:
    values = {
        ...
        "SOURCE_ADAPTER_ID": "stage233_side_session_context_repair",
    ...
    for variant_index, variant in enumerate(VARIANTS, start=1):
        ...
        for split in ("validation_is", "oos"):
            ...
            for role_index, (tier, attempt_role, prefix, attempt_token) in enumerate(
                ...
                magic = 23310000 + variant_index * 100 + (1 if split == "validation_is" else 50) + role_index
                attempts.append(
                    s161.base.attempt_payload(
                        run_root=variant_root,
                        run_id=RUN_ID,
                        stage_number=233,
                        exploration_label="stage233_BaselineAdapter__SideSessionContextRepairAfterLifecycleFailure",
...
def configure_runner() -> None:
    ...
    for variant in VARIANTS:
        ...
        context_specs[variant.adapter_id] = {
            "gate_column": f"stage233_gate_{extra['axis']}",
            "gate_type": "encoded_stage233_side_session_context_repair_after_lifecycle_failure",
            ...
            "description": "Stage233 encoded gate: side/session/context repair after lifecycle failure.",
        }
...
def decide(rows: Sequence[Mapping[str, Any]], external: str) -> str:
    if external != "completed":
        return "continue_stage233_runtime_completion_due_to_incomplete_runtime_candidate_not_final"
    if any(pass_stage233(row) for row in rows):
        return "open_stage234_stage233_side_session_context_followup_review_candidate_not_final"
    return "open_stage234_bounded_followup_due_to_side_session_context_tradeoff_candidate_not_final"
...
def report_markdown(rows: Sequence[Mapping[str, Any]], decision: str, external: str) -> str:
    best = best_row(rows)
    return f"""# Stage233 Side Session Context Repair Report(233단계 방향/세션/문맥 수리 보고서)
...
def decision_markdown(decision: str, external: str) -> str:
    return f"""# Stage233 Decision(233단계 판정)
...
- source_stage(원천 단계): `{SOURCE_STAGE_ID}`
- source_run(원천 실행): `{SOURCE_RUN_ID}`
- source_stage232_evidence_commit(원천 232단계 근거 커밋): `{SOURCE_STAGE232_EVIDENCE_COMMIT}`
- source_stage232_hash_record_commit(원천 232단계 해시 기록 커밋): `{SOURCE_STAGE232_HASH_RECORD_COMMIT}`
- external_verification_status(외부 검증 상태): `{external}`
...
Stage233(233단계) closeout(종료)는 overall goal complete(전체 목표 완료)가 아니다.

Effect(효과): Stage234(234단계) follow-up review(후속 검토)에서 side/session/context repair(방향/세션/문맥 수리)의 KPI(핵심 성과 지표) 상충과 다음 bounded repair(경계 수리) 축을 분리 판정한다.
"""
...
def artifact_rows(result: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    for row in rows:
        if row.get("artifact_type") != "mt5_strategy_tester_report":
            row["artifact_type"] = "stage233_evidence"
            row["notes"] = "Stage233 side/session/context repair after lifecycle failure evidence."
        else:
            row["notes"] = "Stage233 MT5 Strategy Tester report."
    return rows
        ...
        [
            {
                "run_id": RUN_ID,
                "stage_id": STAGE_ID,
                "lane": "baseline_adapter_stage233_side_session_context_repair_after_lifecycle_failure",
                "status": status,
                "judgment": decision,
                "path": rel(DECISION_PATH),
                "notes": s172.ledger_pairs(
                    (
                        ("source_stage232_evidence_commit", SOURCE_STAGE232_EVIDENCE_COMMIT),
                        ("source_stage232_hash_record_commit", SOURCE_STAGE232_HASH_RECORD_COMMIT),
...
def write_packet_files(result: Mapping[str, Any], decision: str, ledger_payload: Mapping[str, Any], quality: Sequence[Mapping[str, Any]]) -> None:
    ...
    s172.write_md(
        ...
        f"""# Stage233 Closeout Packet(233단계 종료 작업 묶음)
...
Stage234(234단계)는 Stage233(233단계) side/session/context repair(방향/세션/문맥 수리) 결과를 follow-up review(후속 검토)하는 bounded review(경계 검토) 단계다.

## Bounded Question(경계 질문)

Did Stage233(233단계) improve validation early/mid PF(검증 초반/중반 수익요인) and validation net(검증 순손익) while preserving OOS net/PF/DD(표본외 순손익/수익요인/낙폭), model-controlled risk%(모델 제어 위험 비율), and ATR SL/TP(ATR 손절/익절)?

Effect(효과): Stage233(233단계) 안에서 다음 수리를 흡수하지 않고, 방향/세션/문맥 축의 KPI(핵심 성과 지표) 상충을 별도 검토로 닫는다.
...
- status(상태): `open_planned_from_stage233`
- source_stage(원천 단계): `{STAGE_ID}`
- source_decision(원천 판정): `{decision}`
...
- stage_status(단계 상태): `open_planned_from_stage233`
...
- >-
  Stage233(233단계) closed(종료) as `{decision}` and Stage234(234단계) `{NEXT_STAGE_ID}` is open_planned(개방 계획). Effect(효과): side/session/context repair(방향/세션/문맥 수리)의 KPI(핵심 성과 지표) 상충을 별도 review(검토)로 판정한다.
- >-
  Stage233 evidence(233단계 근거)는 `{rel(REPORT_PATH)}`, `{rel(SUMMARY_CSV_PATH)}`, `{rel(QUALITY_MATRIX_PATH)}`, `{rel(MONTHLY_KPI_PATH)}`, `{rel(CONCENTRATION_PATH)}`, `{rel(RISK_ATR_TELEMETRY_PATH)}`에 있다. Effect(효과): 방향/세션/문맥 축이 validation/OOS(검증/표본외)를 함께 끌어올렸는지 확인한다.
- >-
...
- adapter_under_review(검토 중 어댑터): `stage233_side_session_context_repair_after_lifecycle_failure`
- status(상태): `stage233_{decision}`
- claim_boundary(주장 경계): `{BOUNDARY}`

Stage233(233단계)는 side/session/context(방향/세션/문맥) 축으로 KPI(핵심 성과 지표) 회복을 측정했다. Effect(효과): Stage234(234단계)가 결과 상충과 다음 bounded repair(경계 수리)를 별도 review(검토)로 판정한다.

## Latest Stage233 Evidence(최신 233단계 근거)
...
def append_changelog(decision: str) -> None:
    ...
    if RUN_ID in existing:
        return
    entry = (
        f"\n## {s172.utc_now()} Stage233 side/session/context repair closeout(233단계 방향/세션/문맥 수리 종료)\n\n"
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    prepared = {
        ...
        "model_family": "baseline_adapter_stage233_v2_native_side_session_context_repair",
        "feature_set_id": "stage233_signal_plus_side_session_context_repair",
