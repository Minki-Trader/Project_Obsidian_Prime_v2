...
STAGE_ID = "stage_frontier_51__short_pf_edge_outcome_memory_recurrence_after_f50_loss_floor_transfer_memory"
PREV_STAGE_ID = "stage_frontier_50__short_pf_edge_loss_floor_regime_transfer_after_f49_state_machine_memory"
RUN_A = "frontier51A_stage_open_short_pf_edge_outcome_memory_recurrence_hypothesis_design_v1"
RUN_B = "frontier51B_train_only_outcome_memory_recurrence_order_path_proxy_v1"
RUN_C = "frontier51C_capped_outcome_memory_recurrence_order_path_repair_v1"
RUN_D = "frontier51D_stage_closeout_outcome_memory_recurrence_v1"
NEXT_STAGE_ID = "stage_frontier_52__short_pf_edge_order_path_cost_recurrence_after_f51_runtime_memory"
NEXT_RUN_ID = "frontier52A_stage_open_short_pf_edge_order_path_cost_recurrence_hypothesis_design_v1"
RUNTIME_REVIEW_RUN_ID = "frontier51E_pre_expensive_runtime_validation_review_v1"
RUNTIME_PROBE_RUN_ID = "frontier51Z_runtime_probe_backfill_v1"
...
GROK_OPEN_ROOT = Path("docs") / "agent_control" / "grok_reviews" / "2026-06-16_frontier51_stage_open" / "small_review"
GROK_CLOSE_ROOT = Path("docs") / "agent_control" / "grok_reviews" / "2026-06-16_frontier51_stage_closeout" / "small_review"
...
TRAIN_MIN_TRADES = 35
TRAIN_MIN_DENSITY = 0.35
TRAIN_MAX_DENSITY = 12.0
TRAIN_PF_FLOOR = 1.00
TRAIN_DD_CAP = 26.0
...
def main() -> None:
    ...
    write_json(INPUT_ROOT / "outcome_memory_recurrence_manifest.json", manifest)
    ...
    write_json(
        RUN_D_ROOT / "run_manifest.json",
        {
            ...
            "profile": "short_outcome_memory_recurrence_proxy",
            "open_review": open_review,
            "closeout_review": closeout_review,
            "repair_decision": repair_decision,
            "closeout": closeout,
...
def load_open_grok_review() -> dict[str, Any]:
    ...
    claim_ok = (
        ("claim_boundary_ok" in lowered and "yes" in lowered)
        or ("claim boundary" in lowered and "`accepted`" in lowered)
        or ("claim boundary" in lowered and "accepted" in lowered and "no completion" in lowered)
    )
    forbidden = any(
        phrase in lowered
    ...
    return {
        ...
        "classification": "accepted_stage_open_train_split_only_outcome_memory_recurrence_lock" if accepted and guardrail_seen and claim_ok and not rejected else "needs_local_verification",
...
def load_closeout_grok_review() -> dict[str, Any]:
    ...
    rejected = "`rejected`" in lowered or "verdict:** `rejected`" in lowered or "verdict:** rejected" in lowered or "verdict: rejected" in lowered
    boundary_ok = (
        ("closeout_boundary_ok" in lowered and "yes" in lowered)
        or ("claim boundary" in lowered and "`accepted`" in lowered)
        or ("claim boundary" in lowered and "accepted" in lowered and "no completion" in lowered)
    )
    forbidden = any(
    ...
    return {
        ...
        "metadata": metadata,
        "clean_output_path": clean_path.as_posix(),
        "classification": "accepted_closeout_outcome_memory_recurrence_boundary" if accepted and boundary_ok and not rejected else "needs_local_verification",
...
def context_checks(
    ...
    checks = {
        "workspace_points_to_f51": f"next_stage_id: {STAGE_ID}" in workspace or f"current_stage_id: {STAGE_ID}" in workspace,
        "prev_selection_next_matches": STAGE_ID in prev_selection,
    ...
    if not all(checks.values()):
        raise RuntimeError(f"Frontier51 context check failed: {json.dumps(checks, ensure_ascii=False)}")
    return {
        "checks": checks,
...
def build_input_manifest(
    ...
    return {
        "stage_id": STAGE_ID,
        "hypothesis": "train_only_outcome_memory_recurrence_after_f49_state_machine_memory",
        "idea_id": "IDEA-FR50-SHORT-LOSS-FLOOR-REGIME-TRANSFER",
        "decision_use": "scout_seed_runtime_candidate_screening_only",
        "comparison_baseline": "F49 best and closest nonwinner rows are reference-only clues, not inherited baseline",
        "control_variables": {
            "symbol": "US100",
            "timeframe": "M5",
            "side": SIDE_LABEL,
        ...
        "changed_variables": {
            "sequence_context": "lagged frozen event score plus causal outcome-memory tape and MFE/MAE decay memory",
            "event_surface": "future path event is segmented by train-only loss containment and causal prior outcome-memory recurrence families",
            "risk_sizing": "train-only first-hit SLTP caps with minimal liquidity/volatility hygiene admission",
            "novelty_line": "outcome-memory recurrence changes the input surface instead of relabeling F49 floor-state admission gates",
            "model_family": "class_weighted_sequence_context_classifier_with_outcome_memory_recurrence_and_decay_memory",
            "threshold_policy": "train_split_event_score_threshold_only; validation_oos_read_only",
        },
        "sample_scope": {
            "rows": int(len(frame)),
        ...
        "invalid_conditions": [
            ...
            "f49_floor_state_gate_relabeling_as_primary_lever",
        ...
        "evidence_plan": [
            ...
            "outcome_memory_recurrence_model_audit",
            "candidate_ledger",
            "split_metrics",
            "candidate_summary",
            "stage_run_ledger",
    ...
    rows: list[dict[str, Any]] = [
        {
            ...
            "gate_variant": "f49_reference_event_model_context_score_sltp",
            "threshold_name": "archived_f49_scaffold",
    ...
    for profile in ("initial", "repair"):
        for spec in risk_budget_specs(profile):
            ...
            for family, tag, values in (
                ("hygiene_gate_equals", "fixed_design_constant", dict(spec.get("equals_features", {}))),
                ("hygiene_gate_max", "fixed_design_constant", dict(spec.get("max_fixed_features", {}))),
                ("hygiene_gate_min", "fixed_design_constant", dict(spec.get("min_fixed_features", {}))),
                ("hygiene_gate_quantile", "train_only_fit", dict(spec.get("max_quantile_features", {}))),
            ):
                    ...
                    rows.append(
                        {
                            ...
                            "provenance_tag": tag,
                            "selection_role": "fixed_hygiene_admission" if tag == "fixed_design_constant" else "train_only_threshold",
                        }
                    )
...
def event_specs(frame: pd.DataFrame, labels: dict[str, np.ndarray], profile: str) -> list[dict[str, Any]]:
    ...
    if train_mfe.size < 100 or train_mae.size < 100 or train_ratio.size < 100 or train_horizon.size < 100:
        return []

    loss_sample = train_horizon[train_horizon < 0.0]
    gain_sample = train_horizon[train_horizon > 0.0]
    loss_floor = float(np.nanquantile(loss_sample, 0.35)) if loss_sample.size >= 50 else float(np.nanquantile(train_horizon, 0.25))
    gain_floor = float(np.nanquantile(gain_sample, 0.35)) if gain_sample.size >= 50 else float(np.nanquantile(train_horizon, 0.65))
    q = {
        ...
        "ratio60": float(np.nanquantile(train_ratio, 0.60)),
    ...
    known_horizon = pd.Series(horizon).shift(embargo)
    known_mfe = pd.Series(mfe).shift(embargo)
    known_mae = pd.Series(mae).shift(embargo)
    known_loss_floor = (known_horizon <= loss_floor).fillna(False).to_numpy(dtype=bool)
    known_gain_floor = (known_horizon >= gain_floor).fillna(False).to_numpy(dtype=bool)
    known_good_path = (
        (known_horizon > 0.0)
        & (known_mfe >= q["mfe60"])
        & (known_mae <= q["mae55"])
    ...
    known_bad_path = (
        (known_horizon <= loss_floor)
        | ((known_mae >= q["mae55"]) & (known_mfe < q["mfe60"]))
    ).fillna(False).to_numpy(dtype=bool)
    bars_since_gain = bars_since_true(known_gain_floor)
    bars_since_good = bars_since_true(known_good_path)
    bars_since_bad = bars_since_true(known_bad_path)
    known_mae_fast = known_mae.rolling(12, min_periods=6).mean()
    known_mae_slow = known_mae.rolling(36, min_periods=18).mean()
    known_mfe_fast = known_mfe.rolling(12, min_periods=6).mean()
    known_mfe_slow = known_mfe.rolling(36, min_periods=18).mean()
    mae_decay = (known_mae_fast <= known_mae_slow).fillna(False).to_numpy(dtype=bool)
    mfe_recovery = (known_mfe_fast >= known_mfe_slow).fillna(False).to_numpy(dtype=bool)
    ...
    def build(name: str, definition: str, event_mask: np.ndarray, thresholds: dict[str, float]) -> dict[str, Any]:
        ...
        return {
            ...
            "mfe": np.asarray(mfe, dtype="float64"),
            "mae": np.asarray(mae, dtype="float64"),
            "horizon": np.asarray(horizon, dtype="float64"),
            "train_thresholds": thresholds,
            "loss_floor_threshold": loss_floor,
            "gain_floor_threshold": gain_floor,
    ...
    specs = [
        build(
            "event_good_recurrence_mfe65_mae40_recent_good",
            "short path event(숏 경로 이벤트): MFE >= train q65, MAE <= train q40, and a confirmed good outcome recurred within 24 bars(24봉 내 확정 양호 결과 재발)",
            (mfe >= q["mfe65"]) & (mae <= q["mae40"]) & (bars_since_good <= 24.0),
            {"mfe65": q["mfe65"], "mae40": q["mae40"], "recent_good_bars": 24.0},
        ),
        build(
            "event_decay_memory_mfe70_mae45_ratio65",
            "short path event(숏 경로 이벤트): MFE >= train q70, MAE <= train q45, MFE/MAE ratio >= train q65, and confirmed MAE decay/MFE recovery memory(확정 MAE 감쇠/MFE 회복 기억)",
            (mfe >= q["mfe70"]) & (mae <= q["mae45"]) & (ratio >= q["ratio65"]) & mae_decay & mfe_recovery,
            {"mfe70": q["mfe70"], "mae45": q["mae45"], "ratio65": q["ratio65"]},
        ),
        build(
            "event_bad_to_good_recovery_mfe65_mae50",
            "short path event(숏 경로 이벤트): MFE >= train q65, MAE <= train q50, recent bad outcome is followed by recent good outcome(최근 불량 결과 뒤 양호 결과 회복)",
            (mfe >= q["mfe65"]) & (mae <= q["mae50"]) & (bars_since_bad <= 48.0) & (bars_since_good <= 24.0),
            {"mfe65": q["mfe65"], "mae50": q["mae50"], "recent_bad_bars": 48.0, "recent_good_bars": 24.0},
        ),
    ]
    if profile == "initial":
        return [item for item in specs if item["event_variant"] == "event_good_recurrence_mfe65_mae40_recent_good"]
    if profile == "repair":
        specs.extend(
            [
                build(
                    "repair_event_good_or_recovered_mfe60_mae55",
                    "looser repair event(완화 수리 이벤트): MFE >= train q60, MAE <= train q55, horizon pnl positive, and recent confirmed good/recovered outcome(최근 확정 양호/회복 결과)",
                    (mfe >= q["mfe60"])
                    & (mae <= q["mae55"])
                    & (horizon > 0.0)
                    & ((bars_since_good <= 60.0) | ((bars_since_bad <= 72.0) & (bars_since_gain <= 36.0))),
                    {"mfe60": q["mfe60"], "mae55": q["mae55"], "recent_good_bars": 60.0, "recent_bad_bars": 72.0},
                ),
                build(
                    "repair_event_decay_memory_mfe65_mae50_ratio60",
                    "repair decay-memory event(감쇠 기억 수리 이벤트): MFE >= train q65, MAE <= train q50, ratio >= q60, and known MAE decay(확정 MAE 감쇠)",
                    (mfe >= q["mfe65"]) & (mae <= q["mae50"]) & (ratio >= q["ratio60"]) & mae_decay,
                    {"mfe65": q["mfe65"], "mae50": q["mae50"], "ratio60": q["ratio60"]},
                ),
            ]
        )
        return [
...
def model_specs(profile: str) -> list[dict[str, Any]]:
    ...
    if profile == "repair":
        ...
        return [specs[1], specs[2], specs[3], specs[4]]
    return specs
...
def sequence_context_specs(profile: str) -> list[dict[str, Any]]:
    ...
    for high_q, fast, slow in pairs:
        specs.append(
            {
                "context_variant": f"outcome_memory_recurrence_decay_q{int(high_q * 100)}_w{fast}_{slow}",
...
def risk_budget_specs(profile: str) -> list[dict[str, Any]]:
    specs = [
        {
            "risk_budget_variant": "hygiene_squeeze_off_vol5_le2p25",
            "risk_budget_definition": "minimal hygiene gate(최소 위생 게이트): no squeeze and realized vol expansion <= 2.25; outcome-memory signal stays in model input(결과 기억 신호는 모델 입력에만 둠)",
            "equals_features": {
                "bb_squeeze": 0.0,
            },
            "max_fixed_features": {
                "historical_vol_5_over_20": 2.25,
            },
            "min_fixed_features": {},
        },
        {
            "risk_budget_variant": "hygiene_atr_le2p25_cash_open",
            "risk_budget_definition": "minimal hygiene gate(최소 위생 게이트): ATR ratio <= 2.25 during US cash session(미국 현금장)",
            "equals_features": {"is_us_cash_open": 1.0},
            "max_fixed_features": {
                "atr_14_over_atr_50": 2.25,
            },
            "min_fixed_features": {},
        },
        {
            "risk_budget_variant": "hygiene_vol_atr_le2p5",
            "risk_budget_definition": "minimal hygiene gate(최소 위생 게이트): broad volatility and ATR cap without outcome-memory admission(결과 기억 진입 필터 없음)",
            "equals_features": {},
            "max_fixed_features": {
                "historical_vol_5_over_20": 2.5,
                "atr_14_over_atr_50": 2.5,
            },
            "min_fixed_features": {},
        },
        {
            "risk_budget_variant": "hygiene_no_squeeze_cash_open",
            "risk_budget_definition": "minimal hygiene gate(최소 위생 게이트): no squeeze during US cash session(미국 현금장)",
            "equals_features": {
                "bb_squeeze": 0.0,
                "is_us_cash_open": 1.0,
            },
    ...
    if profile == "repair":
        specs.extend(
            [
                {
                    "risk_budget_variant": "repair_hygiene_squeeze_off_vol5_le2p75",
                    "risk_budget_definition": "repair hygiene gate(수리 위생 게이트): no squeeze and wider vol cap, not a floor-state repair(하한 상태 수리 아님)",
                    "equals_features": {"bb_squeeze": 0.0},
                    "max_fixed_features": {
                        "historical_vol_5_over_20": 2.75,
                    },
                    "min_fixed_features": {},
                },
                {
                    "risk_budget_variant": "repair_hygiene_atr_le2p75",
                    "risk_budget_definition": "repair hygiene gate(수리 위생 게이트): wider ATR cap, no outcome-memory admission(결과 기억 진입 필터 없음)",
                    "equals_features": {},
                    "max_fixed_features": {
                        "atr_14_over_atr_50": 2.75,
                    },
                    "min_fixed_features": {},
                },
                {
                    "risk_budget_variant": "repair_hygiene_cash_open_only",
                    "risk_budget_definition": "repair hygiene gate(수리 위생 게이트): US cash session only(미국 현금장만)",
                    "equals_features": {"is_us_cash_open": 1.0},
                    "max_fixed_features": {},
                    "min_fixed_features": {},
                },
                {
                    "risk_budget_variant": "repair_hygiene_unfiltered_input_surface",
                    "risk_budget_definition": "repair hygiene gate(수리 위생 게이트): no extra admission gate so the input surface carries the hypothesis(입력 표면이 가설을 담당)",
                    "equals_features": {},
                    "max_fixed_features": {},
                    "min_fixed_features": {},
                },
...
def build_event_surface(
    ...
    *,
                    ...
                    effective_model_spec = {
                        **model_spec,
                        ...
                        "sequence_context_features": "|".join(context["feature_names"]),
                            "past_outcome_embargo_bars": context["past_outcome_embargo_bars"],
                            "loss_floor_threshold": context["loss_floor_threshold"],
                            "gain_floor_threshold": context["gain_floor_threshold"],
    ...
    summary = summarize_f51_candidates(split_metrics, selected)
    return {
...
def build_sequence_context(
    *,
    ...
    horizon_series = pd.Series(np.asarray(event_spec.get("horizon", np.full(len(frame), np.nan)), dtype="float64"))
    mfe_series = pd.Series(np.asarray(event_spec.get("mfe", np.full(len(frame), np.nan)), dtype="float64"))
    mae_series = pd.Series(np.asarray(event_spec.get("mae", np.full(len(frame), np.nan)), dtype="float64"))
    known_horizon = horizon_series.shift(embargo)
    known_mfe = mfe_series.shift(embargo)
    known_mae = mae_series.shift(embargo)
    loss_floor_threshold = safe_float(event_spec.get("loss_floor_threshold"), -1e-6)
    gain_floor_threshold = safe_float(event_spec.get("gain_floor_threshold"), 1e-6)
    known_loss_floor = known_horizon <= loss_floor_threshold
    known_gain_floor = known_horizon >= gain_floor_threshold
    loss_magnitude = (-known_horizon).clip(lower=0.0)
    gain_magnitude = known_horizon.clip(lower=0.0)
    known_mfe_mae_ratio = known_mfe / known_mae.clip(lower=1e-8)
    ...
    known_loss_floor_flag = known_loss_floor.fillna(False).to_numpy(dtype=bool)
    known_gain_floor_flag = known_gain_floor.fillna(False).to_numpy(dtype=bool)
    data = {
        ...
        "seq_known_horizon_lag": known_horizon,
        "seq_loss_floor_rate_fast": known_loss_floor.astype(float).rolling(fast, min_periods=max(2, fast // 2)).mean(),
        "seq_loss_floor_rate_slow": known_loss_floor.astype(float).rolling(slow, min_periods=max(3, slow // 2)).mean(),
        "seq_gain_floor_rate_fast": known_gain_floor.astype(float).rolling(fast, min_periods=max(2, fast // 2)).mean(),
        "seq_loss_magnitude_mean_fast": loss_magnitude.rolling(fast, min_periods=max(2, fast // 2)).mean(),
        "seq_loss_magnitude_mean_slow": loss_magnitude.rolling(slow, min_periods=max(3, slow // 2)).mean(),
        "seq_gain_magnitude_mean_fast": gain_magnitude.rolling(fast, min_periods=max(2, fast // 2)).mean(),
        "seq_known_mfe_mean_fast": known_mfe.rolling(fast, min_periods=max(2, fast // 2)).mean(),
        "seq_known_mfe_mean_slow": known_mfe.rolling(slow, min_periods=max(3, slow // 2)).mean(),
        "seq_known_mae_mean_fast": known_mae.rolling(fast, min_periods=max(2, fast // 2)).mean(),
        "seq_known_mae_mean_slow": known_mae.rolling(slow, min_periods=max(3, slow // 2)).mean(),
        "seq_known_mfe_mae_ratio_fast": known_mfe_mae_ratio.rolling(fast, min_periods=max(2, fast // 2)).mean(),
        "seq_bars_since_loss_floor": pd.Series(bars_since_true(known_loss_floor_flag)),
        "seq_bars_since_gain_floor": pd.Series(bars_since_true(known_gain_floor_flag)),
    }
    ...
    context_frame["seq_loss_floor_rate_slope_fast_slow"] = (
        context_frame["seq_loss_floor_rate_fast"] - context_frame["seq_loss_floor_rate_slow"]
    )
    context_frame["seq_loss_recovery_transfer_fast"] = (
        context_frame["seq_gain_magnitude_mean_fast"] - context_frame["seq_loss_magnitude_mean_fast"]
    )
    context_frame["seq_mae_decay_fast_slow"] = (
        context_frame["seq_known_mae_mean_fast"] - context_frame["seq_known_mae_mean_slow"]
    )
    context_frame["seq_mfe_recovery_fast_slow"] = (
        context_frame["seq_known_mfe_mean_fast"] - context_frame["seq_known_mfe_mean_slow"]
    )
    ...
    return {
        ...
        "loss_floor_threshold": loss_floor_threshold,
...
def single_position_order_path_mask(
    frame: pd.DataFrame,
    mask: np.ndarray,
    side: int,
    stop_cap: float,
    ...
    signal = np.asarray(mask, dtype=bool) & labels["valid"]
    selected = np.zeros(len(frame), dtype=bool)
    next_available_pos = -1
    skipped_overlap = 0
    skipped_invalid = 0

    for idx in np.flatnonzero(signal):
        p = int(entry_pos[idx])
        q = int(future_pos[idx])
        if p < 0 or q <= p or q >= len(open_prices):
            skipped_invalid += 1
            continue
        if p < next_available_pos:
            skipped_overlap += 1
            continue
        ...
        selected[idx] = True
        next_available_pos = p + max(1, int(result["holding_bars"])) + 1

    train = f33b.split_mask(frame, "train")
    raw_train_count = int(np.sum(signal & train))
    order_train_count = int(np.sum(selected & train))
    raw_signal_count = int(np.sum(signal))
    order_signal_count = int(np.sum(selected))
    keep_rate = float(order_signal_count / max(raw_signal_count, 1))
    return selected, {
        "order_path_proxy_applied": True,
        "raw_signal_count": raw_signal_count,
        "order_path_signal_count": order_signal_count,
        "order_path_keep_rate": keep_rate,
        "order_path_block_rate": float(1.0 - keep_rate),
        "order_path_skipped_overlap": int(skipped_overlap),
        "order_path_skipped_invalid": int(skipped_invalid),
        "order_path_train_raw_signal_count": raw_train_count,
        "order_path_train_signal_count": order_train_count,
        "order_path_train_keep_rate": float(order_train_count / max(raw_train_count, 1)),
    }
...
def candidates_for_mask(
    ...
    condition = {
        ...
        "feature_family": "train_only_outcome_memory_recurrence_score",
    ...
    for row in threshold_rows(
        ...
        take_quantiles,
        rr_floors,
        f"f51_{profile}_train_only_outcome_memory_recurrence_sl_tp_caps",
    ):
        order_mask, order_meta = single_position_order_path_mask(
            frame,
            mask,
            SIDE_VALUE,
        ...
        candidate = f33b.candidate_from_conditions([condition], order_mask, SIDE_VALUE, row, metrics, score_value)
        ...
        candidate["loss_floor_threshold"] = model_spec.get("loss_floor_threshold", "")
        candidate["gain_floor_threshold"] = model_spec.get("gain_floor_threshold", "")
        ...
        candidate.update(order_meta)
        candidate["f51_train_selection_score"] = score_value
        candidate["selection_rank_basis"] = "train_only_outcome_memory_recurrence_single_position_order_path_proxy_no_validation_oos_feedback"
        variants.append(candidate)
    variants.sort(key=lambda item: float(item["f51_train_selection_score"]), reverse=True)
    return variants[:2]
...
def rank_candidates(candidates: list[dict[str, Any]], prefix: str, limit: int) -> list[dict[str, Any]]:
    ...
    for candidate in sorted(candidates, key=lambda item: float(item["f51_train_selection_score"]), reverse=True):
        key = (
...
def summarize_f51_candidates(split_metrics: pd.DataFrame, candidates: list[dict[str, Any]]) -> pd.DataFrame:
    ...
    metadata = {
        str(item["candidate_id"]): {
            ...
            "loss_floor_threshold": item.get("loss_floor_threshold", ""),
            "gain_floor_threshold": item.get("gain_floor_threshold", ""),
            ...
            "order_path_proxy_applied": item.get("order_path_proxy_applied", ""),
            "raw_signal_count": item.get("raw_signal_count", ""),
            "order_path_signal_count": item.get("order_path_signal_count", ""),
            "order_path_keep_rate": item.get("order_path_keep_rate", ""),
            "order_path_block_rate": item.get("order_path_block_rate", ""),
            "order_path_train_raw_signal_count": item.get("order_path_train_raw_signal_count", ""),
            "order_path_train_signal_count": item.get("order_path_train_signal_count", ""),
            "order_path_train_keep_rate": item.get("order_path_train_keep_rate", ""),
            "f51_train_selection_score": item.get("f51_train_selection_score", ""),
    ...
    for field in (
        ...
        "loss_floor_threshold",
        "gain_floor_threshold",
        "base_score_high_quantile",
        "onnx_friendly",
        "score_quantile",
        "score_threshold",
        "train_event_rate_selected",
        "order_path_proxy_applied",
        "raw_signal_count",
        "order_path_signal_count",
        "order_path_keep_rate",
        "order_path_block_rate",
        "order_path_train_raw_signal_count",
        "order_path_train_signal_count",
        "order_path_train_keep_rate",
        "f51_train_selection_score",
    ...
    summary["f51_scout_clue_flag"] = dual & density_scout & (f_min_pf >= SCOUT_MIN_PF) & (f_max_dd <= SCOUT_MAX_DD)
    summary["f51_seed_surface_flag"] = dual & density_seed & (f_min_pf >= SEED_MIN_PF) & (f_max_dd <= SEED_MAX_DD)
    summary["runtime_probe_candidate_flag"] = (
        summary["f51_seed_surface_flag"].astype(bool) & (f_min_pf >= RUNTIME_MIN_PF) & (f_max_dd <= RUNTIME_MAX_DD)
    )
    summary["f51_axis_gap_to_seed"] = (
...
def surface_counts(summary: pd.DataFrame) -> dict[str, Any]:
    ...
    return {
        "scout_clue_rows": int(summary["f51_scout_clue_flag"].sum()),
        "seed_surface_rows": int(summary["f51_seed_surface_flag"].sum()),
...
def closest_nonwinner_forward_observation(combined: pd.DataFrame, best: dict[str, Any]) -> dict[str, Any]:
    ...
    frame["__seed_gap"] = numeric_column("f51_axis_gap_to_seed", 999999.0)
...
def build_repair_decision(initial_summary: pd.DataFrame) -> dict[str, Any]:
    if initial_summary.empty:
        return {
            ...
            "repair_action": "run_capped_outcome_memory_recurrence_input_surface_repair",
            "repair_reason": "Initial outcome-memory recurrence surface produced no candidate summary.",
        }
    runtime = int(initial_summary["runtime_probe_candidate_flag"].sum())
    seed = int(initial_summary["f51_seed_surface_flag"].sum())
    scout_rows = int(initial_summary["f51_scout_clue_flag"].sum())
    if runtime:
        return {
    ...
    if seed:
        return {
            ...
            "repair_reason": "Seed surface exists; avoid input-surface overfit repair before runtime validation planning.",
        }
    return {
        "run_repair_grid": True,
        "repair_action": "run_capped_outcome_memory_recurrence_input_surface_repair",
        "repair_reason": f"Initial outcome-memory recurrence surface scout={scout_rows}, seed=0, runtime=0; run bounded input-surface repair.",
...
def classify_closeout(initial_summary: pd.DataFrame, repair_summary: pd.DataFrame) -> dict[str, Any]:
    ...
    if combined.empty:
        ...
        scout_rows = int(combined["f51_scout_clue_flag"].sum())
        seed_rows = int(combined["f51_seed_surface_flag"].sum())
    ...
    if runtime_rows:
        ...
        runtime_status = "mandatory_runtime_probe_required_select_seed_surface_representative_after_f51_proxy"
        next_stage = STAGE_ID
        next_run = RUNTIME_PROBE_RUN_ID
    elif scout_rows:
        closeout_class = "preserved_clue_negative_memory"
        runtime_status = "mandatory_runtime_probe_required_select_scout_representative_after_f51_proxy"
        next_stage = STAGE_ID
        next_run = RUNTIME_PROBE_RUN_ID
    else:
        closeout_class = "negative_memory"
        runtime_status = "mandatory_runtime_probe_required_select_best_available_after_f51_proxy"
        next_stage = STAGE_ID
        next_run = RUNTIME_PROBE_RUN_ID
    return {
...
def build_closeout_prompt(closeout: dict[str, Any], best_rows: list[dict[str, Any]], repair_decision: dict[str, Any]) -> str:
    ...
    compact_rows = "\n".join(
        (
            ...
            f"fwd_dd={row.get('forward_max_dd')}; order_keep={row.get('order_path_keep_rate')}; "
            f"scout={row.get('f51_scout_clue_flag')}; seed={row.get('f51_seed_surface_flag')}; runtime={row.get('runtime_probe_candidate_flag')}"
        )
        for row in best_rows[:6]
    )
    return f"""# Frontier51 Closeout Grok Review(전선 51단계 마감 그록 검토)
...
- order_path_keep_rate(주문 경로 유지율): {best.get("order_path_keep_rate")}
- scout/seed/runtime(탐색/씨앗/런타임): {best.get("f51_scout_clue_flag")}/{best.get("f51_seed_surface_flag")}/{best.get("runtime_probe_candidate_flag")}
...
F51 opened train-only outcome-memory recurrence(학습 전용 결과 기억 재발) hypothesis(가설). F49 is reference only(참조 전용), not baseline/winner(기준선/승자 아님).
"""
...
- scope_completion_gate(범위 완료 게이트): pass(통과), F51 hypothesis/proxy/repair/closeout(가설/프록시/수리/마감) materialized.
...
- data_integrity(데이터 무결성): pass(통과), closed-bar feature order(닫힌 봉 피처 순서), train-only frozen base scorer(학습 전용 고정 기본 채점기), outcome-memory/outcome-memory context(결과 기억/결과 기억 문맥), horizon+1 embargo(예측수평+1 유예), and minimal hygiene gates(최소 위생 게이트) verified.
...
- final_codex_direction(최종 코덱스 방향): run F51 outcome-memory recurrence proxy with validation/OOS read-only(검증/표본외 읽기 전용)
"""
...
- final_codex_direction(최종 코덱스 방향): close F51 as `{closeout.get("closeout_class")}` with no authority claim(권위 주장 없음)
"""
    return {
...
F51 preserved clue(보존 단서)는 train-only outcome-memory recurrence(학습 전용 결과 기억 재발)이 PF/DD/density(수익 팩터/손실폭/밀도)를 얼마나 바꿀 수 있는지에 대한 근거다.
...
F51 negative memory(부정 기억)는 train-only outcome-memory recurrence(학습 전용 결과 기억 재발)이 seed/runtime(씨앗/런타임) 후보를 만들었는지 여부와 반복 금지 경계를 기록한다.
    ...
    rows = [
        {
            ...
            "notes": "F51 opened with train-only outcome-memory recurrence hypothesis and Grok guardrails.",
        },
        {
            "stage_id": STAGE_ID,
            "run_id": RUN_B,
            ...
            "notes": "Initial train-only outcome-memory recurrence proxy surface.",
        },
        {
            "stage_id": STAGE_ID,
            "run_id": RUN_C,
            ...
            "runtime_probe_status": "evaluated_for_runtime_candidate",
            "notes": "Capped outcome-memory recurrence repair diagnostic.",
        },
        {
            "stage_id": STAGE_ID,
            "run_id": RUN_D,
        ...
        },
        {
            ...
            "closeout_class": closeout.get("closeout_class"),
            "runtime_probe_status": "out_of_scope_by_claim_tier_a_outcome_memory_recurrence_proxy_only",
            "notes": "F51 used Tier A outcome-memory recurrence proxy only; Tier B not claimed.",
        },
        {
            "stage_id": STAGE_ID,
            "run_id": RUN_D,
...
def project_ledger_row(row: dict[str, Any], fields: list[str]) -> dict[str, Any]:
    ...
    values = {
        ...
        "kpi_scope": "short_outcome_memory_recurrence_proxy",
        ...
        "run_family": "frontier_short_outcome_memory_recurrence_proxy",
...
current_judgment: {closeout.get("closeout_class")}(F51 train-only outcome-memory recurrence proxy no operating authority)
...
Frontier51(F51, 전선 50단계)가 `{closeout.get("closeout_class")}`로 닫혔다.
...
F51 carry-forward(이월) 기록은 train-only outcome-memory recurrence(학습 전용 결과 기억 재발)이 PF/DD/density(수익 팩터/손실폭/밀도)를 얼마나 바꾸었는지와 seed/runtime(씨앗/런타임) 후보가 생겼는지 여부다.
"""
...
def artifact_map() -> dict[str, str]:
    return {
        "input_manifest": (INPUT_ROOT / "outcome_memory_recurrence_manifest.json").as_posix(),
