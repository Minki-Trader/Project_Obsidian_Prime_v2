...
STAGE_ID = "79_adapter_research__v41_atr_stop_lifecycle_repair"
RUN_NUMBER = "run79A"
RUN_ID = "run79A_stage79_v41_atr_stop_lifecycle_repair_v1"
PACKET_ID = "stage79_v41_atr_stop_lifecycle_repair_v1"
PARENT_RUN_ID = "run78A_stage78_v41_entry_quality_followup_review_v1"
SOURCE_STAGE78_ID = "78_adapter_research__v41_entry_quality_followup_review"
SOURCE_STAGE78_CLOSEOUT_COMMIT = "91eb1e26ce16013fc555166a76a27685f859b5dc"
SOURCE_STAGE78_LATEST_COMMIT = "7dd8cea104784a812b97bdd2601770d4d79d4966"
SOURCE_STAGE73_LATEST_COMMIT = "76db6f199ff917da2f8311544f68dc6f24612e0e"
NEXT_STAGE_ID = "80_adapter_research__v41_atr_stop_followup_review"
NEXT_RUN_ID = "run80A_stage80_v41_atr_stop_followup_review_v1"
NEXT_PACKET_ID = "stage80_v41_atr_stop_followup_review_v1"
...
COMMON_ROOT = f"OPV2/s79a/{RUN_NUMBER}"

SUMMARY_JSON_PATH = REVIEWS_ROOT / "stage79_v41_atr_stop_lifecycle_summary.json"
SUMMARY_CSV_PATH = REVIEWS_ROOT / "stage79_v41_atr_stop_lifecycle_summary.csv"
REPORT_PATH = REVIEWS_ROOT / "stage79_v41_atr_stop_lifecycle_report.md"
SEGMENT_KPI_PATH = REVIEWS_ROOT / "stage79_segment_kpi_summary.csv"
RISK_ATR_TELEMETRY_PATH = REVIEWS_ROOT / "stage79_risk_atr_telemetry.csv"
GATE_FEATURE_SUMMARY_PATH = REVIEWS_ROOT / "stage79_gate_feature_summary.csv"
TIER_B_DIAGNOSTIC_PATH = REVIEWS_ROOT / "stage79_tier_b_diagnostic_summary.csv"
DECISION_PATH = REVIEWS_ROOT / "stage79_decision.md"
AUDIT_CSV_PATH = REVIEWS_ROOT / "stage79_trade_audit.csv"
...
VARIANTS = (
    repair.RepairVariant(
        adapter_id="s79_v41_h3_risk5_gate08_sl20_tp35",
        label="run50BN_v41_h3_risk5_gate08_sl20_tp35",
        atr_enabled=True,
        ...
        max_hold_bars=3,
        notes="Stage79 ATR stop repair: Stage73 risk5 gate08 TP3.5 with tighter SL 2.0 and hold3.",
    ),
    repair.RepairVariant(
        adapter_id="s79_v41_h3_risk5_gate08_sl225_tp35",
        label="run50BN_v41_h3_risk5_gate08_sl225_tp35",
        atr_enabled=True,
        model_risk_enabled=True,
        fixed_lot=0.25,
        ...
        close_only_on_opposite_signal=True,
        max_hold_bars=3,
        notes="Stage79 ATR stop repair: Stage73 risk5 gate08 TP3.5 with moderate SL 2.25 and hold3.",
    ),
    repair.RepairVariant(
        adapter_id="s79_v41_h3_risk5_gate08_sl20_tp40",
        label="run50BN_v41_h3_risk5_gate08_sl20_tp40",
        atr_enabled=True,
        model_risk_enabled=True,
        fixed_lot=0.25,
        atr_stop_multiplier=2.0,
        ...
        close_only_on_opposite_signal=True,
        max_hold_bars=3,
        notes="Stage79 ATR stop repair: Stage73 risk5 gate08 TP4.0 with tighter SL 2.0 and hold3.",
    ),
)
...
CONTEXT_GATE_SPECS = {
    variant.adapter_id: {
        "gate_column": "stage79_gate_margin_lt_008_short",
        "gate_type": "margin",
        "threshold": 0.08,
        "block_mode": "short",
...
def decide(summary_rows: Sequence[Mapping[str, Any]], segment_rows: Sequence[Mapping[str, Any]], external: str) -> str:
    if external != "completed":
        return "continue_stage79_due_to_incomplete_runtime"
    ...
    if val_net >= 900 and oos_net >= 520 and val_dd <= 18.0 and oos_dd <= 19.0 and val_pf >= 1.50 and oos_pf >= 1.30 and not reasons:
        return "proceed_to_stage80_candidate_review"
    if val_net >= 650 and oos_net >= 420 and val_dd <= 24.0 and oos_dd <= 24.0 and val_pf >= 1.35 and oos_pf >= 1.25:
        return "continue_atr_stop_lifecycle_review_in_stage80"
    if val_pf < 1.0 or val_net <= 0:
        return "open_new_model_source_branch_in_stage80"
    return "continue_atr_stop_lifecycle_review_in_stage80"
...
def report_markdown(
    ...
    return f"""# Stage79 V41 ATR Stop/Lifecycle Repair Report(79단계 V41 ATR 손절/생명주기 수리 보고서)

- run(실행): `{RUN_ID}`
- target_surface(목표 표면): `{TARGET_SURFACE}`
- source_stage(원천 단계): `{SOURCE_STAGE78_ID}`
- source_stage78_closeout_commit(원천 78단계 종료 커밋): `{SOURCE_STAGE78_CLOSEOUT_COMMIT}`
- source_stage78_latest_commit(원천 78단계 최신 커밋): `{SOURCE_STAGE78_LATEST_COMMIT}`
...
def decision_markdown(decision: str, external: str) -> str:
    return f"""# Stage79 Decision(79단계 판정)

decision(판정): `{decision}`

Stage79(79단계)는 Stage73(73단계) v41(브이41) risk5 gate08(위험 5%, 게이트 0.08) 표면에서 ATR stop multiplier(ATR 손절 배수)를 2.0/2.25로 낮춰 PF/net/DD(수익 팩터/순손익/손실률) 균형을 측정했다.
            ...
            rows.append(
                {
                    ...
                    "split": split_name,
                    "tier_b_policy": "diagnostic_missing_required_but_disabled_for_this_v41_atr_stop_lifecycle_repair",
                    ...
                    "reason": "Stage79 isolates v41 ATR stop/lifecycle repair first; Tier B fallback remains diagnostic and disabled.",
                }
            )
    return rows
            ...
            rows.append(
                {
                    ...
                    "artifact_type": "stage79_v41_atr_stop_lifecycle_repair_evidence",
                    "path": rel(path),
                    "sha256": sha256_file_lf_normalized(path),
                    "stage_id": STAGE_ID,
                    "run_id": RUN_ID,
                    "created_at_utc": created,
                    "notes": "Stage79 v2-native v41 ATR stop/lifecycle repair artifact.",
                }
            )
            ...
            rows.append(
                {
                    ...
                    "run_id": RUN_ID,
                    "created_at_utc": created,
                    "notes": "Actual Stage79 MT5 Strategy Tester HTML report.",
                }
            )
    return rows
...
def write_run_identity(result: Mapping[str, Any]) -> None:
    write_json(
        RUN_ROOT / "run_manifest.json",
        {
            ...
            "source_stage78_closeout_commit": SOURCE_STAGE78_CLOSEOUT_COMMIT,
            "source_stage78_latest_commit": SOURCE_STAGE78_LATEST_COMMIT,
        ...
        [
            {
                "run_id": RUN_ID,
                "stage_id": STAGE_ID,
                "lane": "baseline_adapter_v2_native_v41_atr_stop_lifecycle_repair",
                "status": status,
                "judgment": decision,
        ...
        alpha_rows = [
            {
                ...
                "tier_scope": "Tier A+B",
                "kpi_scope": "stage79_v41_atr_stop_lifecycle_repair",
                ...
                "external_verification_status": external,
                "notes": "Stage79 run materialized or blocked before KPI records were available.",
            }
        ]
...
def write_packet_files(result: Mapping[str, Any], decision: str, ledger_payload: Mapping[str, Any]) -> None:
    ...
    write_json(PACKET_ROOT / "aggregate_summary.json", {"packet_id": PACKET_ID, "stage_id": STAGE_ID, "run_id": RUN_ID, "decision": decision, "source_stage78_closeout_commit": SOURCE_STAGE78_CLOSEOUT_COMMIT, "source_stage78_latest_commit": SOURCE_STAGE78_LATEST_COMMIT, "source_stage73_latest_commit": SOURCE_STAGE73_LATEST_COMMIT, "gate_feature_summary_path": rel(GATE_FEATURE_SUMMARY_PATH), "ledger_payload": ledger_payload, "overall_goal_complete": False})
...
Stage80(80단계)는 Stage79(79단계)의 v41 ATR stop/lifecycle repair(V41 ATR 손절/생명주기 수리) 결과를 받아 candidate review(후보 검토), continued repair(계속 수리), 또는 another model source(다른 모델 원천)를 고르는 follow-up(후속) 단계다.

## Bounded Question(경계 질문)

Stage79(79단계)의 tighter ATR stop(더 좁은 ATR 손절)이 Stage73(73단계)의 net(순손익)을 보존하면서 validation DD(검증 손실률)를 낮출 만큼 강한가?

Effect(효과): Stage80(80단계)는 다음 판단을 하나의 경계 질문으로 좁힌다.
...
""",
    ...
    write_md(
        ...
        f"""# Stage80 Input References(80단계 입력 참조)

- source_stage(원천 단계): `{STAGE_ID}`
- source_run(원천 실행): `{RUN_ID}`
- source_decision(원천 판정): `{decision}`
- source_external_verification_status(원천 외부 검증 상태): `{external}`
- stage79_report(79단계 보고서): `{rel(REPORT_PATH)}`
- stage79_decision(79단계 판정): `{rel(DECISION_PATH)}`
- summary(요약): `{rel(SUMMARY_CSV_PATH)}`
- target_surface(목표 표면): `{TARGET_SURFACE}`

Effect(효과): Stage80(80단계)는 v2 고유 근거만 이어받아 34D KPI(34D 핵심 성과 지표) 격차 축소를 계속한다.
""",
    )
    write_md(NEXT_STAGE_ROOT / "03_reviews" / "review_index.md", f"""# Stage80 Review Index(80단계 검토 색인)
...
Effect(효과): Stage80(80단계)는 Stage79(79단계) closeout(종료 기록)을 이어받아 다음 bounded batch(경계 묶음 실행)만 검토한다.
""")
    write_md(NEXT_STAGE_ROOT / "04_selected" / "selection_status.md", f"""# Stage80 Selection Status(80단계 선택 상태)
...
Effect(효과): Stage80(80단계)는 34D KPI(34D 핵심 성과 지표) 격차 축소를 계속하지만, 운영 의미 없이 연구개발로만 이어진다.
""")
...
- >-
  Stage79(79단계) closed(종료) as `{decision}` and Stage80(80단계) `{NEXT_STAGE_ID}` is open_planned(열림 계획). Effect(효과): v41 ATR stop/lifecycle(V41 ATR 손절/생명주기) 근거를 보존하고 다음 경계 연구 질문으로만 넘긴다.
- >-
  Stage79 result(79단계 결과): validation/OOS(검증/표본외), PF/net/DD(수익 팩터/순손익/손실률), risk/ATR telemetry(위험/ATR 텔레메트리)는 `{rel(SUMMARY_CSV_PATH)}`와 `{rel(RISK_ATR_TELEMETRY_PATH)}`에 기록된다. Effect(효과): 34D target surface(34D 목표 표면) 대비 KPI(핵심 성과 지표) 차이를 다음 단계 입력으로 보존한다.
- >-
...
stage79_v41_atr_stop_lifecycle_repair:
  packet_id: {PACKET_ID}
  stage_id: {STAGE_ID}
  ...
  boundary: {BOUNDARY}
"""
    if "stage79_v41_atr_stop_lifecycle_repair:" in text:
        text = re.sub(r"\nstage79_v41_atr_stop_lifecycle_repair:\n(?:  .*\n)+", block + "\n", text, count=1)
    else:
        text = text.rstrip() + block
    io_path(WORKSPACE_STATE_PATH).write_text(text + ("\n" if not text.endswith("\n") else ""), encoding="utf-8-sig")
    write_md(SELECTED_ROOT / "selection_status.md", f"""# Stage79 Selection Status(79단계 선택 상태)
...
- target_surface(목표 표면): `{TARGET_SURFACE}`
- stage79_decision(79단계 판정): `{decision}`
- next_stage_or_branch(다음 단계/분기): `{NEXT_STAGE_ID}`
- claim_boundary(주장 경계): `{BOUNDARY}`

Effect(효과): Stage79(79단계)는 v41 ATR stop/lifecycle repair(V41 ATR 손절/생명주기 수리)를 측정하고, 운영 의미 없이 Stage80(80단계)로 넘긴다.
""")
...
- target_surface(목표 표면): `{TARGET_SURFACE}`
- adapter_under_review(검토 중 어댑터): `{SOURCE_ADAPTER_ID}`
- status(상태): `stage79_closed_{decision}`
- claim_boundary(주장 경계): `{BOUNDARY}`

Stage79(79단계) closed(종료) as v2-native v41 ATR stop/lifecycle repair batch(브이투 고유 브이41 ATR 손절/생명주기 수리 묶음). Effect(효과): legacy 34D(레거시 34D)는 target surface(목표 표면)로만 쓰였고, 다음 연구는 Stage80(80단계)로 이어진다.

## Latest Stage79 Evidence(최신 79단계 근거)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    prepared = {
        ...
        "model_family": "baseline_adapter_stage79_v2_native_v41_atr_stop_lifecycle_repair_ebm_table",
        "feature_set_id": "stage79_v41_atr_stop_lifecycle_signal",
