...
SOURCE_STAGE59AC_ID = "59AC_adapter_repair__bounded_followup_from_stage59ab"
STAGE59AD_ID = "59AD_adapter_repair__bounded_followup_from_stage59ac"
NEXT_REPAIR_STAGE_ID = "59AE_adapter_repair__bounded_followup_from_stage59ad"
RUN_NUMBER = "run59Y"
RUN_ID = "run59Y_stage59ad_bounded_followup_from_stage59ac_v1"
PACKET_ID = "stage59ad_bounded_followup_from_stage59ac_v1"
PARENT_RUN_ID = "run59X_stage59ac_bounded_followup_from_stage59ab_v1"
SOURCE_ADAPTER_ID = "s59ac_v64_gap14_t59_h4_entrytrans_sd5"
DEVELOPMENT_ANCHOR = "v64_v47_ctxgap14_refill_etfw_h2_no_b"
BACKUP_ANCHOR = "v60_v47_et_stable_damage_firewall_h2c0_no_b"
SOURCE_STAGE59AC_PUSHED_COMMIT = "7b9a1f2dcf27b2715f3f91ec75a302376ca49db2"
BOUNDARY = (
    "research_development_only_no_live_readiness_no_runtime_authority_"
    "no_operating_promotion_no_operating_reference_no_production_baseline_no_deployment"
)

STAGE_ROOT = Path("stages") / STAGE59AD_ID
...
SOURCE_STAGE59AC_ROOT = Path("stages") / SOURCE_STAGE59AC_ID
SOURCE_STAGE59AC_DECISION = SOURCE_STAGE59AC_ROOT / "03_reviews/stage59ac_decision.md"
SOURCE_STAGE59AC_REPORT = SOURCE_STAGE59AC_ROOT / "03_reviews/bounded_followup_from_stage59ab_report.md"
SOURCE_STAGE59AC_SUMMARY = SOURCE_STAGE59AC_ROOT / "03_reviews/bounded_followup_summary.csv"
SOURCE_STAGE59AC_JSON = SOURCE_STAGE59AC_ROOT / "03_reviews/bounded_followup_summary.json"
...
COMMON_ROOT = f"Project_Obsidian_Prime_v2/s59ad/{RUN_NUMBER}"

REPORT_PATH = REVIEWS_ROOT / "bounded_followup_from_stage59ac_report.md"
...
DECISION_PATH = REVIEWS_ROOT / "stage59ad_decision.md"
AUDIT_CSV_PATH = REVIEWS_ROOT / "stage59ad_trade_audit.csv"
...
STAGE59AD_VARIANTS = (
    repair.RepairVariant(
        adapter_id="s59ad_v64_gap14_t60_h4_entrytrans_sd5",
        label="run50BR_v64_context_gap14_t60_hold4_entry_transition_risk3pct_wide_atr_sd5",
        atr_enabled=True,
        ...
        max_hold_bars=4,
        notes="Stage59AD bounded follow-up: tighten both thresholds to 0.60 on the Stage59AC hold4 adapter to test whether validation PF and cost weakness are low-confidence entry driven.",
    ),
    repair.RepairVariant(
        adapter_id="s59ad_v64_gap14_t61_h4_entrytrans_sd5",
        label="run50BR_v64_context_gap14_t61_hold4_entry_transition_risk3pct_wide_atr_sd5",
        atr_enabled=True,
        model_risk_enabled=True,
        fixed_lot=0.25,
        atr_stop_multiplier=2.5,
        ...
        close_only_on_opposite_signal=True,
        max_hold_bars=4,
        notes="Stage59AD bounded follow-up: tighten both thresholds to 0.61 on the Stage59AC hold4 adapter as a bounded entry-quality repair check.",
    ),
    repair.RepairVariant(
        adapter_id="s59ad_v64_gap14_t62_h4_entrytrans_sd5",
        label="run50BR_v64_context_gap14_t62_hold4_entry_transition_risk3pct_wide_atr_sd5",
        atr_enabled=True,
        model_risk_enabled=True,
        fixed_lot=0.25,
        atr_stop_multiplier=2.5,
        ...
        close_only_on_opposite_signal=True,
        max_hold_bars=4,
        notes="Stage59AD bounded follow-up: tighten both thresholds to 0.62 on the Stage59AC hold4 adapter as a bounded stress check for entry quality, not a broad search.",
    ),
)
...
def source_specs() -> dict[str, dict[str, Any]]:
    return {
        "s59ad_v64_gap14_t60_h4_entrytrans_sd5": {
        ...
        "s59ad_v64_gap14_t61_h4_entrytrans_sd5": {
            "label": "run50br_v64_context_gap14_h2",
            "run_root": RUN50BR_ROOT,
            "variant_root": RUN50BR_ROOT / RUN50BR_V64_ANCHOR,
            "anchor": RUN50BR_V64_ANCHOR,
        ...
        "s59ad_v64_gap14_t62_h4_entrytrans_sd5": {
            "label": "run50br_v64_context_gap14_h2",
            "run_root": RUN50BR_ROOT,
            "variant_root": RUN50BR_ROOT / RUN50BR_V64_ANCHOR,
            "anchor": RUN50BR_V64_ANCHOR,
...
def configure_reused_engine() -> None:
    engine.STAGE59_ID = STAGE59AD_ID
    ...
    engine.STAGE59_VARIANTS = STAGE59AD_VARIANTS
    engine.SOURCE_SPECS = source_specs()
    engine.MODEL_RISK_MIN_PCT = {variant.adapter_id: 0.005 for variant in STAGE59AD_VARIANTS}

    repair.STAGE_ID = STAGE59AD_ID
    repair.RUN_NUMBER = RUN_NUMBER
    repair.RUN_ID = RUN_ID
    repair.RUN_ROOT = RUN_ROOT
    repair.REPAIR_VARIANTS = STAGE59AD_VARIANTS
    s58.STAGE58_ID = STAGE59AD_ID
    ...
    s58.STAGE58_VARIANTS = STAGE59AD_VARIANTS
...
def build_attempts(inputs: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    for variant_index, variant in enumerate(STAGE59AD_VARIANTS, start=1):
            ...
            for role_index, (tier, attempt_role, prefix, attempt_token) in enumerate(
                ...
                attempts.append(
                    attempt_payload(
                        ...
                        run_id=RUN_ID,
                        stage_number=59,
                        exploration_label="stage59AD_BaselineAdapter__ThresholdTightening",
...
def execute_prepared_run_checkpointed(prepared: Mapping[str, Any], args: argparse.Namespace) -> dict[str, Any]:
    ...
    return {
        **dict(prepared),
        ...
        "judgment": "inconclusive_stage59ad_runtime_probe_completed" if completed and report_completed else "blocked_stage59ad_runtime_probe",
    }
...
def upsert_csv_rows_retry(path: Path, columns: Sequence[str], rows: Sequence[Mapping[str, Any]], *, key: str) -> dict[str, Any]:
    ...
    for attempt in range(3):
        try:
            ...
            last_error = exc
            if path == ARTIFACT_REGISTRY_PATH and getattr(exc, "errno", None) == 22:
                return upsert_csv_rows_atomic(path, columns, rows, key=key)
            time.sleep(0.5 * (attempt + 1))
    assert last_error is not None
    raise last_error


def upsert_csv_rows_atomic(path: Path, columns: Sequence[str], rows: Sequence[Mapping[str, Any]], *, key: str) -> dict[str, Any]:
    ...
    with io_path(tmp).open("w", encoding="utf-8", newline="") as handle:
        writer = csv.DictWriter(handle, fieldnames=list(columns), lineterminator="\n")
        writer.writeheader()
        for row in merged:
            writer.writerow({column: ledger_value(row.get(column, "")) for column in columns})
    io_path(tmp).replace(io_path(path))
    return {
        "path": path.as_posix(),
...
def next_stage_for_decision(decision: str) -> str:
    ...
    if decision == "demote_current_adapter_and_select_backup":
        return "59AE_adapter_repair__backup_or_demote_from_stage59ad"
    if decision == "open_new_model_branch":
        return "59AE_adapter_repair__new_model_branch_from_stage59ad"
...
def next_packet_for_decision(decision: str) -> tuple[str, str]:
    ...
    return "stage59ae_bounded_followup_from_stage59ad_v1", "run59Z_stage59ae_bounded_followup_from_stage59ad_v1"
            ...
            rows.append(
                {
                    ...
                    "stage_id": STAGE59AD_ID,
                    "run_id": RUN_ID,
                    "artifact_type": "stage59ad_bounded_followup_evidence",
                    "path": rel(path),
                    "sha256": sha256_file_lf_normalized(path),
                    "created_at_utc": created,
                    "notes": "Stage59AD bounded threshold-tightening follow-up from Stage59AC artifact.",
                }
            )
            ...
            rows.append(
                {
                    ...
                    "created_at_utc": created,
                    "notes": "Actual Stage59AD MT5 Strategy Tester HTML report.",
                }
            )
    return rows
...
def write_run_identity(result: Mapping[str, Any]) -> None:
    engine.write_json(
        RUN_ROOT / "run_manifest.json",
        {
            ...
            "bounded_question": "Can bounded max-hold changes on the Stage59AC best adapter repair validation late flatline and cost weakness without damaging OOS behavior?",
            "source_stage59ac_decision": rel(SOURCE_STAGE59AC_DECISION),
            "source_stage59ac_pushed_commit": SOURCE_STAGE59AC_PUSHED_COMMIT,
            "variants": [
                {
                    **variant.__dict__,
                ...
                for variant in STAGE59AD_VARIANTS
            ],
...
def write_ledgers(
        ...
        [
            {
                "run_id": RUN_ID,
                "stage_id": STAGE59AD_ID,
                "lane": "baseline_adapter_bounded_followup_from_stage59ac",
                "status": status,
                "judgment": decision,
    ...
    ledger_rows.append(
        {
            "ledger_row_id": f"{RUN_ID}__aggregate_bounded_followup_from_stage59ac",
            "stage_id": STAGE59AD_ID,
            "run_id": RUN_ID,
            "subrun_id": "aggregate_bounded_followup_from_stage59ac",
            "parent_run_id": PARENT_RUN_ID,
            "record_view": "bounded_followup_from_stage59ac",
            "tier_scope": "Tier A+B",
            ...
            "external_verification_status": external,
            "notes": "Stage59AD bounded threshold-tightening follow-up from Stage59AC evidence; not final package completion.",
        }
    )
    ...
    engine.write_json(
        SUMMARY_JSON_PATH,
        {
            ...
            "required_outputs": {
                ...
                "stage59ad_decision": rel(DECISION_PATH),
            },
...
def report_markdown(summary_rows: Sequence[Mapping[str, Any]], decision: str, reasons: Sequence[str], external: str) -> str:
    ...
    best = best_repaired_variant(summary_rows)
    return f"""# Stage59AD Bounded Follow-Up From Stage59AC Report(59AD단계 Stage59AC 기반 경계 후속 보고서)

- stage(단계): `{STAGE59AD_ID}`
- run(실행): `{RUN_ID}`
...
def equity_audit_markdown(summary_rows: Sequence[Mapping[str, Any]], segment_rows: Sequence[Mapping[str, Any]]) -> str:
    ...
    return f"""# Stage59AD Bounded Follow-Up Equity Curve Audit(59AD단계 경계 후속 자금 곡선 감사)
...
Effect(효과): threshold(임계값) 차이가 final net(최종 순손익)만 올렸는지, 아니면 validation/OOS(검증/표본외) 구간 품질과 drawdown recovery(손실 회복)를 같이 개선했는지 판정 근거로 남긴다.
"""


def decision_markdown(decision: str, reasons: Sequence[str], best: Mapping[str, Any], external: str) -> str:
    return f"""# Stage59AD Decision(59AD단계 판정)

decision(판정): `{decision}`

Stage59AD(59AD단계)는 Stage59AC best adapter evidence(Stage59AC 최선 어댑터 근거)를 사용한 bounded threshold-tightening follow-up(경계 임계값 강화 후속)이다. Effect(효과): Stage60 ONNX(60단계 ONNX)로 넘어가기 전 validation PF/cost weakness(검증 수익 팩터/비용 약점)를 먼저 확인한다.
...
Stage59AD closeout(59AD단계 종료)는 overall goal completion(전체 목표 완료)이 아니다. Effect(효과): strong full adapter evidence(강한 전체 어댑터 근거)가 없으면 다음 bounded stage(경계 다음 단계)로 넘긴다.
...
def write_packet_files(
    ...
    files = {
        ...
        "experiment_design_receipt.json": {
            "hypothesis": "Bounded symmetric threshold tightening on the Stage59AC best adapter may repair validation PF and cost-stressed expectancy weakness while preserving OOS net, PF, ATR/bracket telemetry, and model-risk telemetry.",
            ...
            "comparison_baseline": "Stage59AC best adapter s59ac_v64_gap14_t59_h4_entrytrans_sd5 with validation PF 1.02, validation cost-stressed expectancy below zero, and strong but concentrated OOS behavior.",
            "control_variables": ["US100", "M5", "split_v1", "run50BR v64 source", "Tier B disabled", "entry_transition_only", "entry_transition_rearm_min_confidence_delta=0.002", "max_hold_bars=4", "close_only_on_opposite", "wide ATR bracket", "3% model-controlled risk cap", "ONNX deferred"],
            "changed_variables": ["short_threshold", "long_threshold"],
            ...
            "stop_conditions": "three bounded Stage59AD symmetric threshold variants only",
        ...
        "artifact_lineage_audit.json": {
            "source_inputs": [
                rel(SOURCE_STAGE59AC_DECISION),
                rel(SOURCE_STAGE59AC_REPORT),
                rel(SOURCE_STAGE59AC_SUMMARY),
                rel(SOURCE_STAGE59AC_JSON),
                rel(RUN50BR_ROOT / "models/stage56_context_timed_event_signal_discrete_score_table.csv"),
                rel(RUN50BR_ROOT / RUN50BR_V64_ANCHOR),
            ],
            "producer": rel(Path("stage_pipelines/stage59ad/bounded_followup_from_stage59ac.py")),
...
def write_stage_docs(decision: str) -> None:
    ...
    engine.write_md(
        ...
        f"""# Stage59AD Brief(59AD단계 개요)

- stage_id(단계 ID): `{STAGE59AD_ID}`
- source_stage(원천 단계): `{SOURCE_STAGE59AC_ID}`
- source_decision(원천 판정): `continue_repair_in_new_bounded_stage`
- bounded_question(경계 질문): `Can bounded symmetric threshold tightening on the Stage59AC best adapter repair validation PF/cost weakness without damaging OOS behavior?`
- boundary(경계): `{BOUNDARY}`

Stage59AD(59AD단계)는 Stage59AC(59AC단계)의 best adapter(최선 어댑터) `{SOURCE_ADAPTER_ID}`에서 short/long threshold(매도/매수 임계값) 세 변형만 측정한다. Effect(효과): Stage60 ONNX(60단계 ONNX)로 조기 이동하지 않고 검증 품질 약점을 먼저 좁게 확인한다.
""",
    )
    engine.write_md(
        INPUT_ROOT / "input_refs.md",
        f"""# Stage59AD Input References(59AD단계 입력 참조)

- stage59ac_decision(59AC단계 판정): `{rel(SOURCE_STAGE59AC_DECISION)}`
- stage59ac_report(59AC단계 보고서): `{rel(SOURCE_STAGE59AC_REPORT)}`
- stage59ac_summary(59AC단계 요약): `{rel(SOURCE_STAGE59AC_SUMMARY)}`
- stage59ac_pushed_commit(59AC단계 푸시 커밋): `{SOURCE_STAGE59AC_PUSHED_COMMIT}`
...
Effect(효과): Stage59AD(59AD단계)는 Stage59AC(59AC단계) 약점과 run50BR v64(run50BR 변형64) 원천을 명시적으로 연결한다.
""",
    )
    engine.write_md(
        SELECTED_ROOT / "selection_status.md",
        f"""# Stage59AD Selection Status(59AD단계 선택 상태)

- stage_status(단계 상태): `closed_bounded_followup_from_stage59ac`
- source_stage(원천 단계): `{SOURCE_STAGE59AC_ID}`
- source_decision(원천 판정): `continue_repair_in_new_bounded_stage`
- stage59ad_decision(59AD단계 판정): `{decision}`
- next_stage_or_branch(다음 단계/분기): `{next_stage}`
- selected_research_baseline(선택 연구 기준선): `none`
- claim_boundary(주장 경계): `{BOUNDARY}`

Effect(효과): Stage59AD(59AD단계)는 측정 결과를 보존하지만 final package(최종 패키지)나 operating claim(운영 주장)을 만들지 않는다.
""",
    )
    engine.write_md(
        REVIEWS_ROOT / "review_index.md",
        f"""# Stage59AD Review Index(59AD단계 검토 색인)
...
- bounded_followup_risk_atr_telemetry(경계 후속 위험/ATR 기록): `{rel(RISK_ATR_TELEMETRY_PATH)}`
- stage59ad_decision(59AD단계 판정): `{rel(DECISION_PATH)}`
""",
    )
...
- source_stage(원천 단계): `{STAGE59AD_ID}`
- source_decision(원천 판정): `{decision}`
- bounded_question(경계 질문): `What bounded follow-up should repair, demote, or branch from the Stage59AD threshold-tightening evidence without starting ONNX prematurely?`
- boundary(경계): `{BOUNDARY}`

{next_stage_label}({next_stage_label}단계)는 Stage59AD(59AD단계) 결과가 Stage60 ONNX(60단계 ONNX)에 충분하지 않을 때 이어지는 bounded follow-up(경계 후속)이다. Effect(효과): repair(수리), demotion(강등), branch(분기)를 다음 작은 질문으로 제한한다.
""",
        )
        engine.write_md(
...
- stage59ad_decision(59AD단계 판정): `{rel(DECISION_PATH)}`
- stage59ad_report(59AD단계 보고서): `{rel(REPORT_PATH)}`
- stage59ad_summary(59AD단계 요약): `{rel(SUMMARY_CSV_PATH)}`
- stage59ad_segment_kpi(59AD단계 구간 KPI): `{rel(SEGMENT_KPI_PATH)}`
- stage59ad_risk_atr_telemetry(59AD단계 위험/ATR 기록): `{rel(RISK_ATR_TELEMETRY_PATH)}`
""",
        )
        engine.write_md(
...
{next_stage_label}({next_stage_label}단계)는 planned(계획) 상태다. Effect(효과): Stage59AD(59AD단계) 약점을 다음 bounded repair(경계 수리) 근거로 넘긴다.
""",
        )
        engine.write_md(
...
- stage_status(단계 상태): `active_planned_from_stage59ad`
- source_stage(원천 단계): `{STAGE59AD_ID}`
- source_decision(원천 판정): `{decision}`
- selected_research_baseline(선택 연구 기준선): `none`
- claim_boundary(주장 경계): `{BOUNDARY}`

Effect(효과): {next_stage_label}({next_stage_label}단계)는 Stage59AD(59AD단계) 이후 필요한 다음 작은 질문만 다룬다.
""",
        )
...
- status(상태): `stage59ad_closed_{decision}`
- claim_boundary(주장 경계): research/development only(연구/개발 전용)

Stage59AD(59AD단계) closed(종료) as bounded threshold-tightening follow-up from Stage59AC(Stage59AC 기반 경계 임계값 강화 후속). Effect(효과): Stage59AC(59AC단계)의 validation PF/cost weakness(검증 수익 팩터/비용 약점)를 보존하고, Stage60 ONNX(60단계 ONNX)는 품질 근거가 강할 때만 열린다.

## Latest Stage59AD Evidence(최신 59AD단계 근거)
...
""",
    ...
    focus = (
        "current_focus:\n"
        f"- >-\n"
        f"  Stage59AD(59AD단계) `{STAGE59AD_ID}` closed(종료) as bounded threshold-tightening follow-up from Stage59AC(Stage59AC 기반 경계 임계값 강화 후속); decision(판정)=`{decision}`. "
        ...
        f"- >-\n"
        f"  Next stage_or_branch(다음 단계/분기) `{next_stage}` is active/planned(활성/계획). Effect(효과): Stage59AD(59AD단계) 결과를 다음 bounded step(경계 다음 단계)으로 넘긴다.\n"
    )
    text = re.sub(r"current_focus:\n(?:- >-\n(?:  .*\n)+)+", focus, text, count=1)
    block = f"""

stage59ad_bounded_followup_from_stage59ac:
  packet_id: {PACKET_ID}
  stage_id: {STAGE59AD_ID}
  status: closed_bounded_followup_from_stage59ac
  ...
  boundary: {BOUNDARY}
"""
    if "stage59ad_bounded_followup_from_stage59ac:" in text:
        text = re.sub(r"\nstage59ad_bounded_followup_from_stage59ac:\n(?:  .*\n)*", block, text, count=1)
    else:
        text += block
...
def parse_args(argv: Sequence[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run Stage59AD bounded follow-up from Stage59AC.")
...
def load_existing_result() -> dict[str, Any]:
    ...
    if not path_exists(manifest) or not path_exists(kpi):
        raise FileNotFoundError("Stage59AD existing run_manifest.json or kpi_record.json is missing")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.finalize_existing:
        ...
        prepared = {
            ...
            "model_family": "baseline_adapter_stage59ad_threshold_tightening_ebm_table",
            "feature_set_id": "stage59ad_threshold_tightening_signal",
