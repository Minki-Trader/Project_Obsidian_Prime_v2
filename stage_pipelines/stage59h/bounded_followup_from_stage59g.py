...
SOURCE_STAGE59E_ID = "59E_adapter_repair__demotion_or_new_branch"
...
NEXT_REPAIR_STAGE_ID = "59I_adapter_repair__bounded_followup_from_stage59h"
RUN_NUMBER = "run59C"
RUN_ID = "run59C_stage59h_bounded_followup_from_stage59g_v1"
PACKET_ID = "stage59h_bounded_followup_from_stage59g_v1"
PARENT_RUN_ID = "run59B_stage59g_bounded_followup_from_stage59f_v1"
SOURCE_ADAPTER_ID = "s59g_v54_sd10"
DEVELOPMENT_ANCHOR = "v64_v47_ctxgap14_refill_etfw_h2_no_b"
BACKUP_ANCHOR = "v60_v47_et_stable_damage_firewall_h2c0_no_b"
SOURCE_STAGE59E_PUSHED_COMMIT = "1c38672d733c3fab2b924f0ad99fee6305af81bb"
SOURCE_STAGE59F_PUSHED_COMMIT = "b39e8fdd642c3112899730e258fa27945a0a8ff4"
SOURCE_STAGE59G_PUSHED_COMMIT = "45efe77d06f75fcb6063d957fc902dde5f240439"
BOUNDARY = (
    "research_development_only_no_live_readiness_no_runtime_authority_"
    "no_operating_promotion_no_operating_reference_no_production_baseline_no_deployment"
)

STAGE_ROOT = Path("stages") / STAGE59H_ID
...
SOURCE_STAGE59E_ROOT = Path("stages") / SOURCE_STAGE59E_ID
SOURCE_STAGE59E_DECISION = SOURCE_STAGE59E_ROOT / "03_reviews/stage59e_decision.md"
SOURCE_STAGE59E_REPORT = SOURCE_STAGE59E_ROOT / "03_reviews/demotion_or_new_branch_report.md"
SOURCE_STAGE59E_SUMMARY = SOURCE_STAGE59E_ROOT / "03_reviews/demotion_or_new_branch_summary.csv"
SOURCE_STAGE59E_JSON = SOURCE_STAGE59E_ROOT / "03_reviews/demotion_or_new_branch_summary.json"
SOURCE_STAGE59F_ROOT = Path("stages") / STAGE59F_ID
SOURCE_STAGE59F_DECISION = SOURCE_STAGE59F_ROOT / "03_reviews/stage59f_decision.md"
SOURCE_STAGE59F_REPORT = SOURCE_STAGE59F_ROOT / "03_reviews/new_model_branch_from_failure_memory_report.md"
SOURCE_STAGE59F_SUMMARY = SOURCE_STAGE59F_ROOT / "03_reviews/new_model_branch_summary.csv"
SOURCE_STAGE59F_SEGMENT_KPI = SOURCE_STAGE59F_ROOT / "03_reviews/new_model_branch_segment_kpi_summary.csv"
SOURCE_STAGE59F_RISK_ATR_TELEMETRY = SOURCE_STAGE59F_ROOT / "03_reviews/new_model_branch_risk_atr_telemetry.csv"
SOURCE_STAGE59F_JSON = SOURCE_STAGE59F_ROOT / "03_reviews/new_model_branch_summary.json"
SOURCE_STAGE59G_ROOT = Path("stages") / STAGE59G_ID
SOURCE_STAGE59G_DECISION = SOURCE_STAGE59G_ROOT / "03_reviews/stage59g_decision.md"
SOURCE_STAGE59G_REPORT = SOURCE_STAGE59G_ROOT / "03_reviews/bounded_followup_from_stage59f_report.md"
SOURCE_STAGE59G_SUMMARY = SOURCE_STAGE59G_ROOT / "03_reviews/bounded_followup_summary.csv"
SOURCE_STAGE59G_SEGMENT_KPI = SOURCE_STAGE59G_ROOT / "03_reviews/bounded_followup_segment_kpi_summary.csv"
SOURCE_STAGE59G_RISK_ATR_TELEMETRY = SOURCE_STAGE59G_ROOT / "03_reviews/bounded_followup_risk_atr_telemetry.csv"
SOURCE_STAGE59G_JSON = SOURCE_STAGE59G_ROOT / "03_reviews/bounded_followup_summary.json"

RUN50BN_ROOT = SOURCE_STAGE_ROOT / "02_runs/run50BN"
RUN50BO_ROOT = SOURCE_STAGE_ROOT / "02_runs/run50BO"
RUN50BP_ROOT = SOURCE_STAGE_ROOT / "02_runs/run50BP"
RUN50BN_ANCHOR = "v47_v22_topup_plus_et40_slotfill_h2c0_no_b"
RUN50BO_ANCHOR = "v50_topup_slotfill_sd2_h2c0_no_b"
RUN50BP_ANCHOR = "v54_et40_slot20_first_h2c0_no_b"
RUN50BN_SIGNAL = "stage56_context_et_event_signal"
RUN50BO_SIGNAL = "stage56_context_et_event_signal"
RUN50BP_SIGNAL = "stage56_et40_slot_lifecycle_signal"

COMMON_ROOT = f"Project_Obsidian_Prime_v2/s59h/{RUN_NUMBER}"

REPORT_PATH = REVIEWS_ROOT / "bounded_followup_from_stage59g_report.md"
...
DECISION_PATH = REVIEWS_ROOT / "stage59h_decision.md"
AUDIT_CSV_PATH = REVIEWS_ROOT / "stage59h_trade_audit.csv"
...
STAGE59H_VARIANTS = (
    repair.RepairVariant(
        adapter_id="s59h_v54_th60_sd10",
        label="run50BP_v54_stage59g_threshold60_cooldown10",
        atr_enabled=True,
        ...
        max_hold_bars=2,
        notes="Stage59H bounded follow-up: combine Stage59G threshold60 validation clue with cooldown10 same-move reduction.",
    ),
    repair.RepairVariant(
        adapter_id="s59h_v54_th62_sd10",
        label="run50BP_v54_stage59g_threshold62_cooldown10",
        atr_enabled=True,
        model_risk_enabled=True,
        fixed_lot=0.25,
        atr_stop_multiplier=2.5,
        ...
        close_only_on_opposite_signal=True,
        max_hold_bars=2,
        notes="Stage59H bounded follow-up: apply stronger confidence pressure while keeping cooldown10.",
    ),
    repair.RepairVariant(
        adapter_id="s59h_v54_th60_sd12",
        label="run50BP_v54_stage59g_threshold60_cooldown12",
        atr_enabled=True,
        model_risk_enabled=True,
        fixed_lot=0.25,
        atr_stop_multiplier=2.5,
        ...
        close_only_on_opposite_signal=True,
        max_hold_bars=2,
        notes="Stage59H bounded follow-up: keep threshold60 but push same-direction cooldown to 12 bars.",
    ),
)
...
def source_specs() -> dict[str, dict[str, Any]]:
    v54 = {
        "label": "run50bp_v54",
        "run_root": RUN50BP_ROOT,
        "variant_root": RUN50BP_ROOT / RUN50BP_ANCHOR,
        "anchor": RUN50BP_ANCHOR,
        "model": RUN50BP_ROOT / "models/stage56_context_timed_event_signal_discrete_score_table.csv",
        "signal_column": RUN50BP_SIGNAL,
        "validation_ini": RUN50BP_ROOT / RUN50BP_ANCHOR / "mt5/x01_ta_val.ini",
        "oos_ini": RUN50BP_ROOT / RUN50BP_ANCHOR / "mt5/x01_ta_oos.ini",
        "source_note": "Stage56 run50BP v54 ET40 slot-lifecycle source",
    }
    return {variant.adapter_id: dict(v54) for variant in STAGE59H_VARIANTS}


def configure_reused_engine() -> None:
    engine.STAGE59_ID = STAGE59H_ID
    ...
    engine.STAGE59_VARIANTS = STAGE59H_VARIANTS
    engine.SOURCE_SPECS = source_specs()
    engine.MODEL_RISK_MIN_PCT = {variant.adapter_id: 0.005 for variant in STAGE59H_VARIANTS}

    repair.STAGE_ID = STAGE59H_ID
    repair.RUN_NUMBER = RUN_NUMBER
    repair.RUN_ID = RUN_ID
    repair.RUN_ROOT = RUN_ROOT
    repair.REPAIR_VARIANTS = STAGE59H_VARIANTS
    s58.STAGE58_ID = STAGE59H_ID
    ...
    s58.STAGE58_VARIANTS = STAGE59H_VARIANTS
...
def build_attempts(inputs: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    for variant_index, variant in enumerate(STAGE59H_VARIANTS, start=1):
            ...
            for role_index, (tier, attempt_role, prefix, attempt_token) in enumerate(
                ...
                attempts.append(
                    attempt_payload(
                        ...
                        run_id=RUN_ID,
                        stage_number=59,
                        exploration_label="stage59H_BaselineAdapter__BoundedFollowupFromStage59G",
...
def execute_prepared_run_checkpointed(prepared: Mapping[str, Any], args: argparse.Namespace) -> dict[str, Any]:
    ...
    return {
        **dict(prepared),
        ...
        "judgment": "inconclusive_stage59h_runtime_probe_completed" if completed and report_completed else "blocked_stage59h_runtime_probe",
    }
...
def next_stage_for_decision(decision: str) -> str:
    ...
    if decision == "demote_current_adapter_and_select_backup":
        return "59I_adapter_repair__backup_or_demote_from_stage59h"
    if decision == "open_new_model_branch":
        return "59I_adapter_repair__new_model_branch_from_stage59h"
...
def next_packet_for_decision(decision: str) -> tuple[str, str]:
    ...
    return "stage59i_bounded_followup_from_stage59h_v1", "run59D_stage59i_bounded_followup_from_stage59h_v1"
            ...
            rows.append(
                {
                    ...
                    "stage_id": STAGE59H_ID,
                    "run_id": RUN_ID,
                    "artifact_type": "stage59h_bounded_followup_evidence",
                    "path": rel(path),
                    "sha256": sha256_file_lf_normalized(path),
                    "created_at_utc": created,
                    "notes": "Stage59H bounded follow-up from Stage59G artifact.",
                }
            )
            ...
            rows.append(
                {
                    ...
                    "created_at_utc": created,
                    "notes": "Actual Stage59H MT5 Strategy Tester HTML report.",
                }
            )
    return rows
...
def write_run_identity(result: Mapping[str, Any]) -> None:
    engine.write_json(
        RUN_ROOT / "run_manifest.json",
        {
            ...
            "bounded_question": "Can combined Stage59G threshold and same-direction cooldown clues repair validation weakness without starting ONNX?",
            "source_stage59g_decision": rel(SOURCE_STAGE59G_DECISION),
            "source_stage59g_pushed_commit": SOURCE_STAGE59G_PUSHED_COMMIT,
            "variants": [
                {
                    **variant.__dict__,
                ...
                for variant in STAGE59H_VARIANTS
            ],
...
def write_ledgers(
        ...
        [
            {
                "run_id": RUN_ID,
                "stage_id": STAGE59H_ID,
                "lane": "baseline_adapter_bounded_followup_from_stage59g",
                "status": status,
                "judgment": decision,
    ...
    ledger_rows.append(
        {
            "ledger_row_id": f"{RUN_ID}__aggregate_bounded_followup_from_stage59g",
            "stage_id": STAGE59H_ID,
            "run_id": RUN_ID,
            "subrun_id": "aggregate_bounded_followup_from_stage59g",
            "parent_run_id": PARENT_RUN_ID,
            "record_view": "bounded_followup_from_stage59g",
            "tier_scope": "Tier A+B",
            ...
            "external_verification_status": external,
            "notes": "Stage59H bounded follow-up from Stage59G threshold/cooldown clues; not final package completion.",
        }
    )
...
def report_markdown(summary_rows: Sequence[Mapping[str, Any]], decision: str, reasons: Sequence[str], external: str) -> str:
    ...
    best = best_repaired_variant(summary_rows)
    return f"""# Stage59F New Model Branch From Failure Memory Report(59F단계 실패 기억 기반 새 모델 분기 보고서)

- stage(단계): `{STAGE59F_ID}`
- run(실행): `{RUN_ID}`
...
- new_model_branch_summary(새 모델 분기 요약): `{rel(SUMMARY_CSV_PATH)}`
- new_model_branch_segment_kpi_summary(새 모델 분기 구간 KPI 요약): `{rel(SEGMENT_KPI_PATH)}`
- new_model_branch_risk_atr_telemetry(새 모델 분기 위험/ATR 텔레메트리): `{rel(RISK_ATR_TELEMETRY_PATH)}`
...
def equity_audit_markdown(summary_rows: Sequence[Mapping[str, Any]], segment_rows: Sequence[Mapping[str, Any]]) -> str:
    ...
    return f"""# Stage59F New Model Branch Equity Curve Audit(59F단계 새 모델 분기 자금 곡선 감사)
...
def decision_markdown(decision: str, reasons: Sequence[str], best: Mapping[str, Any], external: str) -> str:
    return f"""# Stage59F Decision(59F단계 판정)

decision(판정): `{decision}`

Stage59F(59F단계)는 Stage59D/59E failure memory(59D/59E단계 실패 기억)를 사용한 bounded new model branch(경계 새 모델 분기)로 기록한다. Effect(효과): 새 source family(원천 계열)의 성공/실패를 다음 bounded stage(경계 다음 단계)의 입력 근거로 넘긴다.
...
Stage59F closeout(59F단계 종료)는 overall goal completion(전체 목표 완료)이 아니다. Effect(효과): Stage60 ONNX hardening(60단계 ONNX 경화)은 adapter quality(어댑터 품질)가 강할 때만 열린다.
...
def report_markdown(summary_rows: Sequence[Mapping[str, Any]], decision: str, reasons: Sequence[str], external: str) -> str:
    rows = [row for row in summary_rows if row.get("view") == "actual_routed_total"]
    table = "\n".join(
        "| {adapter} | {split} | {pf} | {net} | {dd} | {cost} | {same} | {risk} | {lot} |".format(
            adapter=row.get("adapter_id"),
            split=row.get("split"),
            pf=aw.fmt(row.get("profit_factor")),
            net=aw.fmt(row.get("net_profit")),
    ...
    best = best_repaired_variant(summary_rows)
    return f"""# Stage59G Bounded Follow-up From Stage59F Report(59G단계 59F단계 후속 경계 보고서)

- stage(단계): `{STAGE59G_ID}`
- run(실행): `{RUN_ID}`
- source_adapter(원천 어댑터): `{SOURCE_ADAPTER_ID}`
- source_stage59f_decision(원천 59F단계 판정): `{rel(SOURCE_STAGE59F_DECISION)}`
- external_verification_status(외부 검증 상태): `{external}`
- decision(판정): `{decision}`
- boundary(경계): `{BOUNDARY}`
...
def equity_audit_markdown(summary_rows: Sequence[Mapping[str, Any]], segment_rows: Sequence[Mapping[str, Any]]) -> str:
    ...
    return f"""# Stage59G Bounded Follow-up Equity Curve Audit(59G단계 경계 후속 자금 곡선 감사)
...
def decision_markdown(decision: str, reasons: Sequence[str], best: Mapping[str, Any], external: str) -> str:
    return f"""# Stage59G Decision(59G단계 판정)
    ...
    engine.write_json(
        SUMMARY_JSON_PATH,
        {
            ...
            "required_outputs": {
                ...
                "stage59h_decision": rel(DECISION_PATH),
            },
...
def write_packet_files(
    ...
    files = {
        ...
        "experiment_design_receipt.json": {
            "hypothesis": "A bounded new model branch using Stage59D close-only-on-opposite failure memory may improve post-ATR/risk validation and OOS behavior versus the demoted v64 adapter path.",
            ...
            "comparison_baseline": "Stage59D best_balanced_failure_memory s59d_v64_closeonlyopp_thr57_mr03_wideatr_sd5 and Stage59E demotion decision.",
            "control_variables": ["US100", "M5", "split_v1", "Tier B disabled", "close_only_on_opposite", "wide ATR bracket", "3% model-controlled risk cap", "ONNX deferred"],
            "changed_variables": ["model_source_family", "signal_column", "source_feature_csv"],
            ...
            "stop_conditions": "three bounded Stage59F source-family variants only",
        ...
        "artifact_lineage_audit.json": {
            "source_inputs": [rel(SOURCE_STAGE59E_DECISION), rel(SOURCE_STAGE59E_REPORT), rel(SOURCE_STAGE59E_SUMMARY), rel(SOURCE_STAGE59E_JSON)],
        ...
        "aggregate_summary.json": {
            "packet_id": PACKET_ID,
            "stage_id": STAGE59F_ID,
            "run_id": RUN_ID,
            "decision": decision,
            "external_verification_status": external,
            "required_outputs": {
                "new_model_branch_report": rel(REPORT_PATH),
                "new_model_branch_summary_json": rel(SUMMARY_JSON_PATH),
                "new_model_branch_summary_csv": rel(SUMMARY_CSV_PATH),
                "new_model_branch_segment_kpi_summary": rel(SEGMENT_KPI_PATH),
                "new_model_branch_equity_curve_audit": rel(EQUITY_AUDIT_PATH),
                "new_model_branch_risk_atr_telemetry": rel(RISK_ATR_TELEMETRY_PATH),
                "stage59f_decision": rel(DECISION_PATH),
            },
            "claim_boundary": BOUNDARY,
            "overall_goal_complete": False,
        },
...
- stage_id(단계 ID): `{STAGE59F_ID}`
- source_stage(원천 단계): `{SOURCE_STAGE59E_ID}`
- source_decision(원천 판정): `open_new_model_branch`
...
Stage59F(59F단계)는 three source-family variants(세 원천 계열 변형)만 측정한다. Effect(효과): Stage59D(59D단계)의 close-only-on-opposite(반대 신호에서만 청산) 단서를 사용하되 Stage60 ONNX(60단계 ONNX)나 deployment(배포)로 넘어가지 않는다.
""",
    )
    engine.write_md(
...
- stage59e_decision(59E단계 판정): `{rel(SOURCE_STAGE59E_DECISION)}`
- stage59e_report(59E단계 보고서): `{rel(SOURCE_STAGE59E_REPORT)}`
- stage59e_summary(59E단계 요약): `{rel(SOURCE_STAGE59E_SUMMARY)}`
- stage59e_pushed_commit(59E단계 푸시 커밋): `{SOURCE_STAGE59E_PUSHED_COMMIT}`
- run50bn_source_model(run50BN 원천 모델): `{rel(RUN50BN_ROOT / "models/stage56_context_timed_event_signal_discrete_score_table.csv")}`
- run50bo_source_model(run50BO 원천 모델): `{rel(RUN50BO_ROOT / "models/stage56_context_timed_event_signal_discrete_score_table.csv")}`
- run50bp_source_model(run50BP 원천 모델): `{rel(RUN50BP_ROOT / "models/stage56_context_timed_event_signal_discrete_score_table.csv")}`

Effect(효과): Stage59F(59F단계)는 failure memory(실패 기억)와 새 source family(원천 계열)를 명시적으로 연결한다.
""",
    )
    engine.write_md(
...
- stage_status(단계 상태): `closed_bounded_new_model_branch_from_failure_memory`
- source_stage(원천 단계): `{SOURCE_STAGE59E_ID}`
- source_decision(원천 판정): `open_new_model_branch`
- stage59f_decision(59F단계 판정): `{decision}`
- next_stage_or_branch(다음 단계/분기): `{next_stage}`
- selected_research_baseline(선택 연구 기준선): `none`
- claim_boundary(주장 경계): `{BOUNDARY}`

Effect(효과): Stage59F(59F단계)는 측정 결과를 보존하지만 final package(최종 패키지)나 operating claim(운영 주장)을 만들지 않는다.
""",
    )
    engine.write_md(
        REVIEWS_ROOT / "review_index.md",
        f"""# Stage59F Review Index(59F단계 검토 색인)

- new_model_branch_report(새 모델 분기 보고서): `{rel(REPORT_PATH)}`
- new_model_branch_summary(새 모델 분기 요약): `{rel(SUMMARY_CSV_PATH)}`
- new_model_branch_segment_kpi(새 모델 분기 구간 KPI): `{rel(SEGMENT_KPI_PATH)}`
- new_model_branch_equity_curve_audit(새 모델 분기 자금 곡선 감사): `{rel(EQUITY_AUDIT_PATH)}`
- new_model_branch_risk_atr_telemetry(새 모델 분기 위험/ATR 텔레메트리): `{rel(RISK_ATR_TELEMETRY_PATH)}`
- stage59f_decision(59F단계 판정): `{rel(DECISION_PATH)}`
""",
    )
    if not next_stage.startswith("60_"):
        ...
        engine.write_md(
            ...
            f"""# Stage59G Brief(59G단계 개요)

- stage_id(단계 ID): `{next_stage}`
- source_stage(원천 단계): `{STAGE59F_ID}`
- source_decision(원천 판정): `{decision}`
- bounded_question(경계 질문): `What bounded follow-up should repair, demote, or branch from the Stage59F new-model evidence without starting ONNX prematurely?`
- boundary(경계): `{BOUNDARY}`

Stage59G(59G단계)는 Stage59F(59F단계) 결과가 Stage60 ONNX(60단계 ONNX)에 충분하지 않을 때만 이어지는 bounded follow-up(경계 후속)이다. Effect(효과): repair(수리), demotion(강등), branch(분기)를 다음 작은 질문으로 제한한다.
""",
        )
        engine.write_md(
            next_root / "01_inputs/input_refs.md",
            f"""# Stage59G Input References(59G단계 입력 참조)

- stage59f_decision(59F단계 판정): `{rel(DECISION_PATH)}`
- stage59f_report(59F단계 보고서): `{rel(REPORT_PATH)}`
- stage59f_summary(59F단계 요약): `{rel(SUMMARY_CSV_PATH)}`
- stage59f_segment_kpi(59F단계 구간 KPI): `{rel(SEGMENT_KPI_PATH)}`
- stage59f_risk_atr_telemetry(59F단계 위험/ATR 텔레메트리): `{rel(RISK_ATR_TELEMETRY_PATH)}`
""",
        )
        engine.write_md(
            next_root / "03_reviews/review_index.md",
            """# Stage59G Review Index(59G단계 검토 색인)

Stage59G(59G단계)는 planned(계획) 상태다. Effect(효과): Stage59F(59F단계) 약점을 다음 bounded repair(경계 수리) 근거로 넘긴다.
""",
        )
        engine.write_md(
            next_root / "04_selected/selection_status.md",
            f"""# Stage59G Selection Status(59G단계 선택 상태)

- stage_status(단계 상태): `active_planned_from_stage59f`
- source_stage(원천 단계): `{STAGE59F_ID}`
- source_decision(원천 판정): `{decision}`
- selected_research_baseline(선택 연구 기준선): `none`
- claim_boundary(주장 경계): `{BOUNDARY}`

Effect(효과): Stage59G(59G단계)는 Stage59F(59F단계) 이후 필요한 다음 작은 질문만 다룬다.
""",
        )
...
- status(상태): `stage59f_closed_{decision}`
...
""",
    ...
    focus = (
        "current_focus:\n"
        f"- >-\n"
        f"  Stage59F(59F단계) `{STAGE59F_ID}` closed(종료) as bounded new model branch from failure memory(실패 기억 기반 경계 새 모델 분기); decision(판정)=`{decision}`. "
        f"Effect(효과): full BaselineAdapter repair(전체 기준선 어댑터 수리)는 계속 연구/개발 경계 안에 있으며 final(최종) 또는 operating(운영) 주장은 없다.\n"
        f"- >-\n"
        f"  Next stage_or_branch(다음 단계/분기) `{next_stage}` is active/planned(활성/계획). Effect(효과): Stage59F(59F단계) 결과를 다음 bounded step(경계 다음 단계)으로 넘긴다.\n"
    )
    text = re.sub(
...
stage59f_new_model_branch_from_failure_memory:
  packet_id: {PACKET_ID}
  stage_id: {STAGE59F_ID}
  status: closed_bounded_new_model_branch_from_failure_memory
  current_run_id: {RUN_ID}
  source_adapter: {SOURCE_ADAPTER_ID}
  source_stage59e_pushed_commit: {SOURCE_STAGE59E_PUSHED_COMMIT}
  best_repaired_adapter: {best.get('adapter_id', 'none')}
  decision: {decision}
  ...
  boundary: {BOUNDARY}
"""
    if "stage59f_new_model_branch_from_failure_memory:" in text:
        text = re.sub(r"\nstage59f_new_model_branch_from_failure_memory:\n(?:  .*\n)*", block, text, count=1)
    else:
        text += block
...
def write_packet_files(
    ...
    files = {
        "routing_receipt.json": {
            ...
            "support_skills": ["obsidian-experiment-design", "obsidian-backtest-forensics", "obsidian-result-judgment", "obsidian-artifact-lineage"],
            "required_gates": ["experiment_design_receipt", "runtime_evidence_gate", "kpi_contract_audit", "result_judgment_gate", "artifact_lineage_audit", "final_claim_guard"],
            "status": "completed",
        },
        "experiment_design_receipt.json": {
            "hypothesis": "A narrow same-direction re-entry and threshold follow-up around Stage59F s59f_v54_coo may repair validation weakness while preserving OOS behavior after ATR and model-controlled risk.",
            "decision_use": "Route to Stage60 only if validation/OOS, segment KPI, risk telemetry, ATR bracket behavior, and equity shape are strong; otherwise continue repair, demote, or open a new branch.",
            "comparison_baseline": "Stage59F best_repaired_adapter s59f_v54_coo.",
            "control_variables": ["US100", "M5", "split_v1", "run50BP v54 source", "Tier B disabled", "close_only_on_opposite", "wide ATR bracket 2.5/3.5", "3% model-controlled risk cap", "ONNX deferred"],
            "changed_variables": ["same_direction_reentry_cooldown_bars", "short_threshold", "long_threshold", "entry_transition_only", "entry_transition_rearm_min_confidence_delta"],
            "sample_scope": "FPMarkets US100 M5 validation 2025-01-01 to 2025-09-30 and OOS 2025-10-01 to 2026-04-13.",
            "success_criteria": ["validation and OOS net positive", "validation and OOS PF >= 1.10", "validation and OOS cost-stressed expectancy positive", "same-move concentration reduced or justified", "ATR and model risk telemetry present", "no severe segment flags"],
            "failure_criteria": ["negative validation/OOS net", "PF < 1.10", "cost-stressed expectancy <= 0", "same-move concentration remains harmful", "missing risk or ATR telemetry", "segment flags remain"],
            "invalid_conditions": ["missing source Stage59F evidence", "missing model table", "missing feature files", "missing MT5 report", "telemetry parse failure"],
            "stop_conditions": "three bounded Stage59G variants only; do not broaden into open-ended hunting.",
            "evidence_plan": [rel(SUMMARY_CSV_PATH), rel(SEGMENT_KPI_PATH), rel(EQUITY_AUDIT_PATH), rel(RISK_ATR_TELEMETRY_PATH), rel(DECISION_PATH)],
            "status": "completed",
        },
        "runtime_evidence_gate.json": {"external_verification_status": external, "mt5_reports": result.get("strategy_tester_reports", []), "status": external},
        ...
        "artifact_lineage_audit.json": {
            "source_inputs": [rel(SOURCE_STAGE59F_DECISION), rel(SOURCE_STAGE59F_REPORT), rel(SOURCE_STAGE59F_SUMMARY), rel(SOURCE_STAGE59F_SEGMENT_KPI), rel(SOURCE_STAGE59F_RISK_ATR_TELEMETRY), rel(SOURCE_STAGE59F_JSON)],
            "consumers": [rel(REPORT_PATH), rel(DECISION_PATH), rel(SUMMARY_JSON_PATH)],
            "ledger_links": ledger_payload,
        },
        "final_claim_guard.json": {
        ...
        "required_gate_coverage_audit.json": {
            "required_gates": ["experiment_design_receipt", "runtime_evidence_gate", "kpi_contract_audit", "result_judgment_gate", "artifact_lineage_audit", "final_claim_guard"],
            "covered_by": ["experiment_design_receipt.json", "runtime_evidence_gate.json", "kpi_contract_audit.json", "result_judgment_gate.json", "artifact_lineage_audit.json", "final_claim_guard.json"],
            "status": "completed",
        },
        "aggregate_summary.json": {
            "packet_id": PACKET_ID,
            "stage_id": STAGE59G_ID,
            "run_id": RUN_ID,
            "decision": decision,
            "external_verification_status": external,
            "required_outputs": {
                ...
                "bounded_followup_equity_curve_audit": rel(EQUITY_AUDIT_PATH),
                "bounded_followup_risk_atr_telemetry": rel(RISK_ATR_TELEMETRY_PATH),
                "stage59g_decision": rel(DECISION_PATH),
            },
            "claim_boundary": BOUNDARY,
            "overall_goal_complete": False,
        },
...
- stage_id(단계 ID): `{STAGE59G_ID}`
- source_stage(원천 단계): `{STAGE59F_ID}`
- source_decision(원천 판정): `continue_repair_in_new_bounded_stage`
- bounded_question(경계 질문): `Can a narrow same-direction re-entry and threshold follow-up repair Stage59F v54 validation weakness without starting ONNX?`
- boundary(경계): `{BOUNDARY}`

Stage59G(59G단계)는 Stage59F(59F단계)의 `s59f_v54_coo` 주변만 좁게 시험한다. Effect(효과): repair(수리) 판단을 작게 유지하고 Stage60 ONNX(60단계 ONNX) 조기 진입을 막는다.
""",
    )
    engine.write_md(
        INPUT_ROOT / "input_refs.md",
...
- stage59f_decision(59F단계 판정): `{rel(SOURCE_STAGE59F_DECISION)}`
- stage59f_report(59F단계 보고서): `{rel(SOURCE_STAGE59F_REPORT)}`
- stage59f_summary(59F단계 요약): `{rel(SOURCE_STAGE59F_SUMMARY)}`
- stage59f_segment_kpi(59F단계 구간 KPI): `{rel(SOURCE_STAGE59F_SEGMENT_KPI)}`
- stage59f_risk_atr_telemetry(59F단계 위험/ATR 텔레메트리): `{rel(SOURCE_STAGE59F_RISK_ATR_TELEMETRY)}`
- stage59f_pushed_commit(59F단계 푸시 커밋): `{SOURCE_STAGE59F_PUSHED_COMMIT}`
- run50bp_source_model(run50BP 원천 모델): `{rel(RUN50BP_ROOT / "models/stage56_context_timed_event_signal_discrete_score_table.csv")}`

Effect(효과): Stage59G(59G단계)는 59F 실패 근거와 이번 작은 수리 축을 명시적으로 연결한다.
""",
    )
    engine.write_md(
        SELECTED_ROOT / "selection_status.md",
...
- stage_status(단계 상태): `closed_bounded_followup_from_stage59f`
- source_stage(원천 단계): `{STAGE59F_ID}`
- source_decision(원천 판정): `continue_repair_in_new_bounded_stage`
- stage59g_decision(59G단계 판정): `{decision}`
- next_stage_or_branch(다음 단계/분기): `{next_stage}`
- selected_research_baseline(선택 연구 기준선): `none`
- claim_boundary(주장 경계): `{BOUNDARY}`

Effect(효과): Stage59G(59G단계)는 결과를 보존하지만 final package(최종 패키지)나 operating claim(운영 주장)을 만들지 않는다.
""",
    )
    engine.write_md(
        REVIEWS_ROOT / "review_index.md",
        f"""# Stage59G Review Index(59G단계 검토 색인)
...
- bounded_followup_risk_atr_telemetry(경계 후속 위험/ATR 텔레메트리): `{rel(RISK_ATR_TELEMETRY_PATH)}`
- stage59g_decision(59G단계 판정): `{rel(DECISION_PATH)}`
""",
    )
    if not next_stage.startswith("60_"):
        next_root = Path("stages") / next_stage
        engine.write_md(
            next_root / "00_spec/stage_brief.md",
            f"""# Stage59H Brief(59H단계 개요)

- stage_id(단계 ID): `{next_stage}`
- source_stage(원천 단계): `{STAGE59G_ID}`
- source_decision(원천 판정): `{decision}`
...
""",
        ...
        engine.write_md(
            next_root / "01_inputs/input_refs.md",
            f"""# Stage59H Input References(59H단계 입력 참조)

- stage59g_decision(59G단계 판정): `{rel(DECISION_PATH)}`
- stage59g_report(59G단계 보고서): `{rel(REPORT_PATH)}`
- stage59g_summary(59G단계 요약): `{rel(SUMMARY_CSV_PATH)}`
- stage59g_segment_kpi(59G단계 구간 KPI): `{rel(SEGMENT_KPI_PATH)}`
- stage59g_risk_atr_telemetry(59G단계 위험/ATR 텔레메트리): `{rel(RISK_ATR_TELEMETRY_PATH)}`
""",
        )
        engine.write_md(
            next_root / "03_reviews/review_index.md",
            """# Stage59H Review Index(59H단계 검토 색인)

Stage59H(59H단계)는 planned(계획) 상태다. Effect(효과): Stage59G(59G단계) 약점을 다음 bounded repair(경계 수리) 근거로 넘긴다.
""",
        )
        engine.write_md(
            next_root / "04_selected/selection_status.md",
            f"""# Stage59H Selection Status(59H단계 선택 상태)

- stage_status(단계 상태): `active_planned_from_stage59g`
- source_stage(원천 단계): `{STAGE59G_ID}`
- source_decision(원천 판정): `{decision}`
- selected_research_baseline(선택 연구 기준선): `none`
- claim_boundary(주장 경계): `{BOUNDARY}`

Effect(효과): Stage59H(59H단계)는 Stage59G(59G단계) 이후 필요한 다음 작은 질문만 다룬다.
""",
        )
...
- backup_anchor(예비 기준점): `{BACKUP_ANCHOR}`
- adapter_under_review(검토 중 어댑터): `{best.get('adapter_id', SOURCE_ADAPTER_ID)}`
- status(상태): `stage59g_closed_{decision}`
- claim_boundary(주장 경계): research/development only(연구/개발 전용)
...
""",
    ...
    focus = (
        "current_focus:\n"
        f"- >-\n"
        f"  Stage59G(59G단계) `{STAGE59G_ID}` closed(종료) as bounded follow-up from Stage59F(59F단계); decision(판정)=`{decision}`. Effect(효과): full BaselineAdapter repair(전체 기준선 어댑터 수리)는 계속 연구/개발 경계 안에 있다.\n"
        f"- >-\n"
        f"  Next stage_or_branch(다음 단계/분기) `{next_stage}` is active/planned(활성/계획). Effect(효과): Stage59G(59G단계) 결과를 다음 bounded step(경계 다음 단계)으로 넘긴다.\n"
    )
    text = re.sub(r"current_focus:\n(?:- >-\n  Stage59G[^\n]*\n- >-\n  Next stage_or_branch[^\n]*\n)+", "current_focus:\n", text, count=1)
    text = re.sub(r"current_focus:\n", focus, text, count=1)
    block = f"""

stage59g_bounded_followup_from_stage59f:
  packet_id: {PACKET_ID}
  stage_id: {STAGE59G_ID}
  status: closed_bounded_followup_from_stage59f
  current_run_id: {RUN_ID}
  source_adapter: {SOURCE_ADAPTER_ID}
  source_stage59f_pushed_commit: {SOURCE_STAGE59F_PUSHED_COMMIT}
  best_repaired_adapter: {best.get('adapter_id', 'none')}
  decision: {decision}
  next_stage_or_branch: {next_stage}
  report_path: {rel(DECISION_PATH)}
  ...
  boundary: {BOUNDARY}
"""
    if "stage59g_bounded_followup_from_stage59f:" in text:
        text = re.sub(r"\nstage59g_bounded_followup_from_stage59f:\n(?:  .*\n)*", block, text, count=1)
    else:
        text += block
    io_path(WORKSPACE_STATE_PATH).write_text(text, encoding="utf-8-sig")
...
def report_markdown(summary_rows: Sequence[Mapping[str, Any]], decision: str, reasons: Sequence[str], external: str) -> str:
    ...
    best = best_repaired_variant(summary_rows)
    return f"""# Stage59H Bounded Follow-up From Stage59G Report(59H단계 59G단계 후속 경계 보고서)

- stage(단계): `{STAGE59H_ID}`
- run(실행): `{RUN_ID}`
- source_adapter(원천 어댑터): `{SOURCE_ADAPTER_ID}`
- source_stage59g_decision(원천 59G단계 판정): `{rel(SOURCE_STAGE59G_DECISION)}`
- external_verification_status(외부 검증 상태): `{external}`
- decision(판정): `{decision}`
- boundary(경계): `{BOUNDARY}`
...
def equity_audit_markdown(summary_rows: Sequence[Mapping[str, Any]], segment_rows: Sequence[Mapping[str, Any]]) -> str:
    ...
    return f"""# Stage59H Bounded Follow-up Equity Curve Audit(59H단계 경계 후속 자금 곡선 감사)
...
def decision_markdown(decision: str, reasons: Sequence[str], best: Mapping[str, Any], external: str) -> str:
    return f"""# Stage59H Decision(59H단계 판정)

decision(판정): `{decision}`

Stage59H(59H단계)는 Stage59G(59G단계)의 threshold/cooldown clues(문턱값/쿨다운 단서)를 결합해 validation weakness(검증 약점)을 시험한 단계다. Effect(효과): repair(수리), demotion(강등), branch(분기), 또는 Stage60 ONNX hardening(60단계 ONNX 경화) 여부를 다음 작은 판단으로 제한한다.
...
Stage59H closeout(59H단계 종료)는 overall goal completion(전체 목표 완료)이 아니다. Effect(효과): Stage60 ONNX hardening(60단계 ONNX 경화)은 full adapter quality(전체 어댑터 품질)가 강할 때만 열린다.
...
def write_packet_files(
    ...
    files = {
        ...
        "experiment_design_receipt.json": {
            "hypothesis": "Combining Stage59G threshold60 with stronger same-direction cooldown may repair validation weakness while preserving OOS behavior after ATR and model-controlled risk.",
            "decision_use": "Route to Stage60 only if validation/OOS, segment KPI, risk telemetry, ATR bracket behavior, and equity shape are strong; otherwise continue repair, demote, or open a new branch.",
            "comparison_baseline": "Stage59G best_repaired_adapter s59g_v54_sd10 and validation-nearest variant s59g_v54_th60_sd8.",
            "control_variables": ["US100", "M5", "split_v1", "run50BP v54 source", "Tier B disabled", "close_only_on_opposite", "wide ATR bracket 2.5/3.5", "3% model-controlled risk cap", "ONNX deferred"],
            "changed_variables": ["same_direction_reentry_cooldown_bars", "short_threshold", "long_threshold"],
            "sample_scope": "FPMarkets US100 M5 validation 2025-01-01 to 2025-09-30 and OOS 2025-10-01 to 2026-04-13.",
            "success_criteria": ["validation and OOS net positive", "validation and OOS PF >= 1.10", "validation and OOS cost-stressed expectancy positive", "same-move concentration reduced or justified", "ATR and model risk telemetry present", "no severe segment flags"],
            "failure_criteria": ["negative validation/OOS net", "PF < 1.10", "cost-stressed expectancy <= 0", "same-move concentration remains harmful", "missing risk or ATR telemetry", "segment flags remain"],
            "invalid_conditions": ["missing source Stage59G evidence", "missing model table", "missing feature files", "missing MT5 report", "telemetry parse failure"],
            "stop_conditions": "three bounded Stage59H variants only; do not broaden into open-ended hunting.",
            "evidence_plan": [rel(SUMMARY_CSV_PATH), rel(SEGMENT_KPI_PATH), rel(EQUITY_AUDIT_PATH), rel(RISK_ATR_TELEMETRY_PATH), rel(DECISION_PATH)],
            "status": "completed",
        },
        "runtime_evidence_gate.json": {"external_verification_status": external, "mt5_reports": result.get("strategy_tester_reports", []), "status": external},
        ...
        "artifact_lineage_audit.json": {
            "source_inputs": [rel(SOURCE_STAGE59G_DECISION), rel(SOURCE_STAGE59G_REPORT), rel(SOURCE_STAGE59G_SUMMARY), rel(SOURCE_STAGE59G_SEGMENT_KPI), rel(SOURCE_STAGE59G_RISK_ATR_TELEMETRY), rel(SOURCE_STAGE59G_JSON)],
            "consumers": [rel(REPORT_PATH), rel(DECISION_PATH), rel(SUMMARY_JSON_PATH)],
            "ledger_links": ledger_payload,
        },
        "final_claim_guard.json": {
...
- stage_id(단계 ID): `{STAGE59H_ID}`
- source_stage(원천 단계): `{STAGE59G_ID}`
- source_decision(원천 판정): `continue_repair_in_new_bounded_stage`
- bounded_question(경계 질문): `Can combined Stage59G threshold and same-direction cooldown clues repair validation weakness without starting ONNX?`
- boundary(경계): `{BOUNDARY}`

Stage59H(59H단계)는 Stage59G(59G단계)의 threshold60(문턱값60)과 cooldown(쿨다운) 단서 조합만 좁게 시험한다. Effect(효과): repair(수리) 판단을 작게 유지하고 Stage60 ONNX(60단계 ONNX) 조기 진입을 막는다.
""",
    )
    engine.write_md(
        INPUT_ROOT / "input_refs.md",
...
- stage59g_decision(59G단계 판정): `{rel(SOURCE_STAGE59G_DECISION)}`
- stage59g_report(59G단계 보고서): `{rel(SOURCE_STAGE59G_REPORT)}`
- stage59g_summary(59G단계 요약): `{rel(SOURCE_STAGE59G_SUMMARY)}`
- stage59g_segment_kpi(59G단계 구간 KPI): `{rel(SOURCE_STAGE59G_SEGMENT_KPI)}`
- stage59g_risk_atr_telemetry(59G단계 위험/ATR 텔레메트리): `{rel(SOURCE_STAGE59G_RISK_ATR_TELEMETRY)}`
- stage59g_pushed_commit(59G단계 푸시 커밋): `{SOURCE_STAGE59G_PUSHED_COMMIT}`
- run50bp_source_model(run50BP 원천 모델): `{rel(RUN50BP_ROOT / "models/stage56_context_timed_event_signal_discrete_score_table.csv")}`

Effect(효과): Stage59H(59H단계)는 59G 실패 근거와 이번 작은 수리 축을 명시적으로 연결한다.
""",
    )
    engine.write_md(
        SELECTED_ROOT / "selection_status.md",
...
- stage_status(단계 상태): `closed_bounded_followup_from_stage59g`
- source_stage(원천 단계): `{STAGE59G_ID}`
- source_decision(원천 판정): `continue_repair_in_new_bounded_stage`
- stage59h_decision(59H단계 판정): `{decision}`
- next_stage_or_branch(다음 단계/분기): `{next_stage}`
- selected_research_baseline(선택 연구 기준선): `none`
- claim_boundary(주장 경계): `{BOUNDARY}`

Effect(효과): Stage59H(59H단계)는 결과를 보존하지만 final package(최종 패키지)나 operating claim(운영 주장)을 만들지 않는다.
""",
    )
    engine.write_md(
        REVIEWS_ROOT / "review_index.md",
        f"""# Stage59H Review Index(59H단계 검토 색인)
...
- bounded_followup_equity_curve_audit(경계 후속 자금 곡선 감사): `{rel(EQUITY_AUDIT_PATH)}`
- bounded_followup_risk_atr_telemetry(경계 후속 위험/ATR 텔레메트리): `{rel(RISK_ATR_TELEMETRY_PATH)}`
- stage59h_decision(59H단계 판정): `{rel(DECISION_PATH)}`
""",
    )
    if not next_stage.startswith("60_"):
        next_root = Path("stages") / next_stage
...
- source_decision(원천 판정): `{decision}`
- bounded_question(경계 질문): `What bounded follow-up should repair, demote, or branch from Stage59H without starting ONNX prematurely?`
- boundary(경계): `{BOUNDARY}`

Stage59I(59I단계)는 Stage59H(59H단계) 결과가 Stage60 ONNX(60단계 ONNX)에 충분하지 않을 때만 이어진다. Effect(효과): 다음 repair(수리), demotion(강등), branch(분기)를 다시 작은 질문으로 제한한다.
""",
        )
        engine.write_md(
            next_root / "01_inputs/input_refs.md",
...
- stage59h_decision(59H단계 판정): `{rel(DECISION_PATH)}`
- stage59h_report(59H단계 보고서): `{rel(REPORT_PATH)}`
- stage59h_summary(59H단계 요약): `{rel(SUMMARY_CSV_PATH)}`
- stage59h_segment_kpi(59H단계 구간 KPI): `{rel(SEGMENT_KPI_PATH)}`
- stage59h_risk_atr_telemetry(59H단계 위험/ATR 텔레메트리): `{rel(RISK_ATR_TELEMETRY_PATH)}`
""",
        )
        engine.write_md(
            next_root / "03_reviews/review_index.md",
...
- stage_status(단계 상태): `active_planned_from_stage59h`
- source_stage(원천 단계): `{STAGE59H_ID}`
- source_decision(원천 판정): `{decision}`
- selected_research_baseline(선택 연구 기준선): `none`
- claim_boundary(주장 경계): `{BOUNDARY}`

Effect(효과): Stage59I(59I단계)는 Stage59H(59H단계) 이후 필요한 다음 작은 질문만 다룬다.
""",
        )
...
- backup_anchor(예비 기준점): `{BACKUP_ANCHOR}`
- adapter_under_review(검토 중 어댑터): `{best.get('adapter_id', SOURCE_ADAPTER_ID)}`
- status(상태): `stage59h_closed_{decision}`
- claim_boundary(주장 경계): research/development only(연구/개발 전용)

Stage59H(59H단계) closed(종료) as bounded follow-up from Stage59G(59G단계 경계 후속). Effect(효과): threshold/cooldown clue(문턱값/쿨다운 단서) 조합 결과를 보존하고, Stage60 ONNX(60단계 ONNX)는 품질 근거가 강할 때만 연다.

## Latest Stage59H Evidence(최신 59H단계 근거)
...
""",
    ...
    focus = (
        "current_focus:\n"
        f"- >-\n"
        f"  Stage59H(59H단계) `{STAGE59H_ID}` closed(종료) as bounded follow-up from Stage59G(59G단계); decision(판정)=`{decision}`. Effect(효과): full BaselineAdapter repair(전체 기준선 어댑터 수리)는 계속 연구/개발 경계 안에 있다.\n"
        f"- >-\n"
        f"  Next stage_or_branch(다음 단계/분기) `{next_stage}` is active/planned(활성/계획). Effect(효과): Stage59H(59H단계) 결과를 다음 bounded step(경계 다음 단계)으로 넘긴다.\n"
    )
    text = re.sub(r"current_focus:\n(?:- >-\n  Stage59H[^\n]*\n- >-\n  Next stage_or_branch[^\n]*\n)+", "current_focus:\n", text, count=1)
    text = re.sub(r"current_focus:\n", focus, text, count=1)
    block = f"""

stage59h_bounded_followup_from_stage59g:
  packet_id: {PACKET_ID}
  stage_id: {STAGE59H_ID}
  status: closed_bounded_followup_from_stage59g
  current_run_id: {RUN_ID}
  source_adapter: {SOURCE_ADAPTER_ID}
  source_stage59g_pushed_commit: {SOURCE_STAGE59G_PUSHED_COMMIT}
  best_repaired_adapter: {best.get('adapter_id', 'none')}
  ...
  boundary: {BOUNDARY}
"""
    if "stage59h_bounded_followup_from_stage59g:" in text:
        text = re.sub(r"\nstage59h_bounded_followup_from_stage59g:\n(?:  .*\n)*", block, text, count=1)
    else:
        text += block
    io_path(WORKSPACE_STATE_PATH).write_text(text, encoding="utf-8-sig")
...
def parse_args(argv: Sequence[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run Stage59H bounded follow-up from Stage59G.")
...
def load_existing_result() -> dict[str, Any]:
    ...
    if not path_exists(manifest) or not path_exists(kpi):
        raise FileNotFoundError("Stage59H existing run_manifest.json or kpi_record.json is missing")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.finalize_existing:
        ...
        prepared = {
            ...
            "model_family": "baseline_adapter_stage59h_bounded_followup_ebm_table",
            "feature_set_id": "stage59h_v54_threshold_cooldown_combination",
