...
NEXT_REPAIR_STAGE_ID = "59A_adapter_repair__risk_sizing_quality_recalibration"
RUN_NUMBER = "run53A"
RUN_ID = "run53A_stage59_post_risk_atr_repair_v1"
PACKET_ID = "stage59_post_risk_atr_repair_v1"
PARENT_RUN_ID = "run52A_stage58_adapter_repair_before_risk_atr_v1"
...
SOURCE_STAGE58_ROOT = Path("stages") / STAGE58_ID
SOURCE_STAGE58_DECISION = SOURCE_STAGE58_ROOT / "03_reviews/stage58_decision.md"
SOURCE_STAGE58_REPORT = SOURCE_STAGE58_ROOT / "03_reviews/risk_atr_integration_report.md"
SOURCE_STAGE58_SUMMARY = SOURCE_STAGE58_ROOT / "03_reviews/stage58_raw_attempt_summary.csv"
SOURCE_STAGE58_SEGMENTS = SOURCE_STAGE58_ROOT / "03_reviews/risk_atr_segment_kpi_summary.csv"
SOURCE_STAGE58_RISK = SOURCE_STAGE58_ROOT / "03_reviews/risk_telemetry_summary.csv"

SIGNAL_COLUMN = "stage56_context_gap_refill_signal"
FEATURE_ORDER_HASH = ordered_hash((SIGNAL_COLUMN,))
COMMON_ROOT = f"Project_Obsidian_Prime_v2/stage59/{RUN_NUMBER}_post_risk_atr_repair"

REPORT_PATH = REVIEWS_ROOT / "adapter_repair_report.md"
SUMMARY_JSON_PATH = REVIEWS_ROOT / "repaired_adapter_summary.json"
SUMMARY_CSV_PATH = REVIEWS_ROOT / "repaired_adapter_summary.csv"
SEGMENT_KPI_PATH = REVIEWS_ROOT / "repaired_segment_kpi_summary.csv"
EQUITY_AUDIT_PATH = REVIEWS_ROOT / "repaired_equity_curve_audit.md"
RISK_ATR_TELEMETRY_PATH = REVIEWS_ROOT / "repaired_risk_atr_telemetry.csv"
DECISION_PATH = REVIEWS_ROOT / "stage59_decision.md"
AUDIT_CSV_PATH = REVIEWS_ROOT / "stage59_trade_audit.csv"
...
STAGE58_PUSHED_COMMIT = "91286dc66751e947d77e88b456c5ab51ae6cbf7c"

STAGE59_VARIANTS = (
    repair.RepairVariant(
        adapter_id="s59_mr02_atr_sd5",
        label="risk_damped_2pct_atr_sd5",
        atr_enabled=True,
        ...
        notes="Stage59 bounded repair: keep Stage58 bracket and damp model-controlled risk% to 2%.",
    ),
    repair.RepairVariant(
        adapter_id="s59_mr02_atr_sd8",
        label="risk_damped_2pct_atr_sd8",
        atr_enabled=True,
        model_risk_enabled=True,
        fixed_lot=0.25,
        atr_stop_multiplier=1.5,
        ...
        notes="Stage59 bounded repair: damp model risk and lengthen same-direction re-entry cooldown.",
    ),
    repair.RepairVariant(
        adapter_id="s59_mr02_wideatr_sd5",
        label="risk_damped_2pct_wide_atr_sd5",
        atr_enabled=True,
        model_risk_enabled=True,
        fixed_lot=0.25,
        ...
        model_risk_max_pct=0.02,
        same_direction_reentry_cooldown_bars=5,
        notes="Stage59 bounded repair: damp model risk and test the wider Stage58 bracket.",
    ),
)
...
def prepare_inputs(common_files_root: Path) -> dict[str, Any]:
    ...
    for split in ("validation_is", "oos"):
        token = "val" if split == "validation_is" else "oos"
        feature_local = RUN_ROOT / DEVELOPMENT_ANCHOR / "features" / f"{DEVELOPMENT_ANCHOR}_stage59_adapter_a_{token}.csv"
    ...
    for variant_index, variant in enumerate(STAGE59_VARIANTS, start=1):
        ...
        for split in ("validation_is", "oos"):
            ...
            for role_index, (tier, attempt_role, prefix, attempt_token) in enumerate(
                ...
                magic = 5905300 + variant_index * 100 + (1 if split == "validation_is" else 50) + role_index
                attempts.append(
                    attempt_payload(
                        run_root=variant_root,
                        run_id=RUN_ID,
                        stage_number=59,
                        exploration_label="stage59_BaselineAdapter__PostRiskAtrRepair",
...
def load_existing_result() -> dict[str, Any]:
    ...
    if not path_exists(manifest) or not path_exists(kpi):
        raise FileNotFoundError("Stage59 existing run_manifest.json or kpi_record.json is missing")
            ...
            rows.append(
                {
                    ...
                    "created_at_utc": created,
                    "notes": "Actual Stage59 MT5 Strategy Tester HTML report.",
                }
            )
    return rows
...
def write_run_identity(result: Mapping[str, Any]) -> None:
    write_json(
        RUN_ROOT / "run_manifest.json",
        {
            ...
            "bounded_question": "Can risk-damped model-controlled risk% repair Stage58 ATR/risk validation damage without changing the model source?",
            "source_stage58_decision": rel(SOURCE_STAGE58_DECISION),
            "source_stage58_pushed_commit": STAGE58_PUSHED_COMMIT,
...
def write_ledgers(
        ...
        [
            {
                "run_id": RUN_ID,
                "stage_id": STAGE59_ID,
                "lane": "baseline_adapter_post_risk_atr_repair",
                "status": status,
                "judgment": decision,
    ...
    ledger_rows.append(
        {
            "ledger_row_id": f"{RUN_ID}__aggregate_post_risk_atr_repair",
            "stage_id": STAGE59_ID,
            "run_id": RUN_ID,
            "subrun_id": "aggregate_post_risk_atr_repair",
            "parent_run_id": PARENT_RUN_ID,
            "record_view": "post_risk_atr_repair",
            "tier_scope": "Tier A+B",
            ...
            "external_verification_status": external,
            "notes": "Stage59 bounded risk-damped repair; not final package completion.",
        }
    )
...
def report_markdown(summary_rows: Sequence[Mapping[str, Any]], decision: str, reasons: Sequence[str], external: str) -> str:
    ...
    best = best_repaired_variant(summary_rows)
    return f"""# Stage59 Adapter Repair Report(59단계 어댑터 수리 보고서)

- stage(단계): `{STAGE59_ID}`
- run(실행): `{RUN_ID}`
- source_adapter(원천 어댑터): `{SOURCE_ADAPTER_ID}`
- source_stage58_commit(원천 58단계 커밋): `{STAGE58_PUSHED_COMMIT}`
...
def equity_audit_markdown(summary_rows: Sequence[Mapping[str, Any]], segment_rows: Sequence[Mapping[str, Any]]) -> str:
    ...
    return f"""# Stage59 Repaired Equity Curve Audit(59단계 수리 자금 곡선 감사)
...
def decision_markdown(decision: str, reasons: Sequence[str], best: Mapping[str, Any], external: str) -> str:
    return f"""# Stage59 Decision(59단계 판정)
    ...
    write_json(
        SUMMARY_JSON_PATH,
        {
            ...
            "required_outputs": {
                ...
                "stage59_decision": rel(DECISION_PATH),
            },
...
def write_packet_files(
    ...
    files = {
        ...
        "experiment_design_receipt.json": {
            "hypothesis": "risk-damped model-controlled risk% may reduce Stage58 validation damage while preserving ATR bracket behavior",
            "control_variables": ["US100", "M5", "split_v1", "Tier B disabled", "source model unchanged"],
            "stop_condition": "three bounded Stage59 variants only",
            "status": "completed",
        },
        ...
        "artifact_lineage_audit.json": {
            "source_inputs": [rel(SOURCE_STAGE58_DECISION), rel(SOURCE_STAGE58_REPORT), rel(SOURCE_MODEL)],
...
- source_decision(원천 판정): `demote_adapter_due_to_risk_atr_damage`
- bounded_question(경계 질문): `risk_damped_model_controlled_risk_pct_repair`
- boundary(경계): `{BOUNDARY}`

Stage59(59단계)는 risk-damped post-ATR repair(위험 완화 ATR 이후 수리)를 측정한다. Effect(효과): model source(모델 원천)를 바꾸기 전, risk sizing(위험 크기 조절)과 bracket(브래킷)이 손상의 주된 원인인지 좁힌다.
""",
    )
    write_md(
...
- stage58_decision(58단계 판정): `{rel(SOURCE_STAGE58_DECISION)}`
- stage58_report(58단계 보고서): `{rel(SOURCE_STAGE58_REPORT)}`
- stage58_summary(58단계 요약): `{rel(SOURCE_STAGE58_SUMMARY)}`
- stage58_segment_kpi(58단계 구간 KPI): `{rel(SOURCE_STAGE58_SEGMENTS)}`
- stage58_risk_telemetry(58단계 위험 텔레메트리): `{rel(SOURCE_STAGE58_RISK)}`
- stage58_pushed_commit(58단계 푸시 커밋): `{STAGE58_PUSHED_COMMIT}`
""",
    )
    write_md(
...
- stage_status(단계 상태): `closed_bounded_post_risk_atr_repair`
- source_stage(원천 단계): `{STAGE58_ID}`
- source_decision(원천 판정): `demote_adapter_due_to_risk_atr_damage`
- stage59_decision(59단계 판정): `{decision}`
- next_stage_or_branch(다음 단계/분기): `{next_stage}`
- selected_research_baseline(선택 연구 기준선): `none`
- claim_boundary(주장 경계): `{BOUNDARY}`

Effect(효과): Stage59(59단계)는 bounded repair result(경계 수리 결과)를 보존하지만 final package(최종 패키지)나 operating claim(운영 주장)을 만들지 않는다.
""",
    )
...
- bounded_question(경계 질문): `Can risk sizing calibration and quality filtering repair Stage59 cost-stressed weakness without starting ONNX?`
- boundary(경계): `{BOUNDARY}`

Stage59A(59A단계)는 risk sizing calibration and quality filtering(위험 크기 보정과 품질 필터링) 후속 수리 단계다. Effect(효과): Stage60 ONNX(60단계 ONNX)는 adapter quality(어댑터 품질)가 강해질 때까지 열지 않는다.
""",
        )
        write_md(
...
- stage59_decision(59단계 판정): `{rel(DECISION_PATH)}`
...
""",
        ...
        write_md(
            ...
            """# Stage59A Review Index(59A단계 검토 색인)

Stage59A(59A단계)는 아직 planned(계획) 상태다. Effect(효과): Stage59(59단계) 약점 기록을 다음 bounded repair(경계 수리)에 연결한다.
""",
        )
        write_md(
...
- stage_status(단계 상태): `active_planned_from_stage59`
- source_stage(원천 단계): `{STAGE59_ID}`
- source_decision(원천 판정): `{decision}`
- selected_research_baseline(선택 연구 기준선): `none`
- claim_boundary(주장 경계): `{BOUNDARY}`

Effect(효과): Stage59A(59A단계)는 Stage59(59단계)의 cost-stressed weakness(비용 압박 약점)를 risk sizing calibration(위험 크기 보정)과 quality filtering(품질 필터링)으로만 다룬다.
""",
        )
    write_md(
        REVIEWS_ROOT / "review_index.md",
        f"""# Stage59 Review Index(59단계 검토 색인)
...
def update_current_truth(decision: str, summary_rows: Sequence[Mapping[str, Any]], external: str) -> None:
    best = best_repaired_variant(summary_rows)
    next_stage = next_stage_for_decision(decision)
    next_packet = "stage59a_risk_sizing_quality_recalibration_v1" if decision == "continue_repair_in_new_bounded_stage" else "stage60_onnx_hardening_v1"
    next_run = "run54A_stage59a_risk_sizing_quality_recalibration_v1" if decision == "continue_repair_in_new_bounded_stage" else "run54A_stage60_onnx_hardening_v1"
    write_md(
...
- status(상태): `stage59_closed_{decision}`
- claim_boundary(주장 경계): research/development only(연구/개발 전용)

Stage59(59단계) closed(종료) as bounded post-risk/ATR repair measurement(경계 위험/ATR 이후 수리 측정). Effect(효과): risk-damped model-controlled risk%(위험 완화 모델 제어 위험률)는 measured(측정됨)됐지만 final adapter(최종 어댑터) 또는 overall goal complete(전체 목표 완료)는 아니다.

## Latest Stage59 Evidence(최신 59단계 근거)
...
""",
    ...
    focus = (
        "current_focus:\n"
        f"- >-\n"
        f"  Stage59(59단계) `{STAGE59_ID}` closed(종료) as bounded post-risk/ATR repair measurement(경계 위험/ATR 이후 수리 측정); decision(판정)=`{decision}`. "
        f"Effect(효과): full adapter repair(전체 어댑터 수리)는 측정됐지만 final(최종) 또는 operating(운영) 주장은 없다.\n"
        f"- >-\n"
        f"  Next stage_or_branch(다음 단계/분기) `{next_stage}` is active/planned(활성/계획). Effect(효과): Stage59(59단계) 결과를 다음 bounded step(경계 다음 단계)으로 넘긴다.\n"
    )
    text = re.sub(
...
stage59_post_risk_atr_repair:
  packet_id: {PACKET_ID}
  stage_id: {STAGE59_ID}
  status: closed_bounded_post_risk_atr_repair
  ...
  boundary: {BOUNDARY}
"""
    if "stage59_post_risk_atr_repair:" in text:
        text = re.sub(r"\nstage59_post_risk_atr_repair:\n(?:  .*\n)*", block, text, count=1)
    else:
        text += block
...
def append_changelog(decision: str) -> None:
    entry = (
        ...
        f"- effect(효과): Stage58(58단계) risk/ATR damage(위험/ATR 손상)를 bounded risk dampening repair(경계 위험 완화 수리)로 측정하고 다음 단계/분기 조건을 남겼다.\n"
    )
...
def parse_args(argv: Sequence[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run Stage59 bounded post-risk/ATR repair.")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.finalize_existing:
        ...
        prepared = {
            ...
            "model_family": "baseline_adapter_stage59_risk_damped_ebm_table",
            "feature_set_id": "stage59_context_gap_refill_signal_risk_atr_repair",
