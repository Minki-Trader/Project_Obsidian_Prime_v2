...
STAGE_ID = "stage_frontier_50__short_pf_edge_loss_floor_regime_transfer_after_f49_state_machine_memory"
PREV_STAGE_ID = "stage_frontier_49__short_pf_edge_forward_floor_state_machine_after_f48_event_risk_memory"
RUN_A = "frontier50A_stage_open_short_pf_edge_loss_floor_regime_transfer_hypothesis_design_v1"
RUN_B = "frontier50B_train_only_loss_floor_regime_transfer_proxy_v1"
RUN_C = "frontier50C_capped_loss_floor_regime_transfer_repair_v1"
RUN_D = "frontier50D_stage_closeout_loss_floor_regime_transfer_v1"
NEXT_STAGE_ID = "stage_frontier_51__short_pf_edge_outcome_memory_recurrence_after_f50_loss_floor_transfer_memory"
NEXT_RUN_ID = "frontier51A_stage_open_short_pf_edge_outcome_memory_recurrence_hypothesis_design_v1"
RUNTIME_REVIEW_RUN_ID = "frontier50E_pre_expensive_runtime_validation_review_v1"
...
GROK_OPEN_ROOT = Path("docs") / "agent_control" / "grok_reviews" / "2026-06-15_frontier50_stage_open" / "small_review"
GROK_CLOSE_ROOT = Path("docs") / "agent_control" / "grok_reviews" / "2026-06-15_frontier50_stage_closeout" / "small_review"
...
def main() -> None:
    ...
    write_json(INPUT_ROOT / "loss_floor_regime_transfer_manifest.json", manifest)
    ...
    write_json(
        RUN_D_ROOT / "run_manifest.json",
        {
            ...
            "profile": "short_loss_floor_regime_transfer_proxy",
            "open_review": open_review,
            "closeout_review": closeout_review,
            "repair_decision": repair_decision,
            "closeout": closeout,
...
def load_open_grok_review() -> dict[str, Any]:
    ...
    return {
        ...
        "classification": "accepted_stage_open_train_split_only_loss_floor_regime_transfer_lock" if accepted and guardrail_seen and claim_ok and not rejected else "needs_local_verification",
...
def load_closeout_grok_review() -> dict[str, Any]:
    ...
    return {
        ...
        "metadata": metadata,
        "clean_output_path": clean_path.as_posix(),
        "classification": "accepted_closeout_loss_floor_regime_transfer_boundary" if accepted and boundary_ok and not rejected else "needs_local_verification",
...
def context_checks(
    ...
    checks = {
        "workspace_points_to_f50": f"next_stage_id: {STAGE_ID}" in workspace or f"current_stage_id: {STAGE_ID}" in workspace,
    ...
    if not all(checks.values()):
        raise RuntimeError(f"Frontier50 context check failed: {json.dumps(checks, ensure_ascii=False)}")
    return {
        "checks": checks,
...
def build_input_manifest(
    ...
    return {
        "stage_id": STAGE_ID,
        "hypothesis": "train_only_loss_floor_regime_transfer_after_f49_state_machine_memory",
        ...
        "changed_variables": {
            "sequence_context": "lagged frozen event score plus causal loss-floor tape and MFE/MAE decay memory",
            "event_surface": "future path event is segmented by train-only loss containment and causal prior loss-floor transfer families",
            "risk_sizing": "train-only first-hit SLTP caps with minimal liquidity/volatility hygiene admission",
            "novelty_line": "loss-floor regime transfer changes the input surface instead of relabeling F49 floor-state admission gates",
            "model_family": "class_weighted_sequence_context_classifier_with_loss_floor_transfer_and_decay_memory",
        ...
        "evidence_plan": [
            ...
            "loss_floor_regime_transfer_model_audit",
            "candidate_ledger",
            "split_metrics",
            "candidate_summary",
            "stage_run_ledger",
...
def event_specs(frame: pd.DataFrame, labels: dict[str, np.ndarray], profile: str) -> list[dict[str, Any]]:
    ...
    bars_since_loss = bars_since_true(known_loss_floor)
    ...
    specs = [
        build(
            "event_loss_floor_transfer_mfe65_mae40_recent_loss",
            "short path event(숏 경로 이벤트) if MFE >= train q65, MAE <= train q40, and a known loss-floor state transferred within 36 bars(36봉 내 확정 손실 하한 전이)",
            (mfe >= q["mfe65"]) & (mae <= q["mae40"]) & (bars_since_loss <= 36.0),
            {"mfe65": q["mfe65"], "mae40": q["mae40"], "loss_floor": q["loss_floor"], "recent_loss_bars": 36.0},
        ),
        build(
            "event_decay_memory_mfe70_mae45_ratio65",
            "short path event(숏 경로 이벤트) if MFE >= train q70, MAE <= train q45, MFE/MAE ratio >= train q65, and known MAE decay/MFE recovery memory(확정 MAE 감쇠/MFE 회복 기억)",
            (mfe >= q["mfe70"]) & (mae <= q["mae45"]) & (ratio >= q["ratio65"]) & mae_decay & mfe_recovery,
            {"mfe70": q["mfe70"], "mae45": q["mae45"], "ratio65": q["ratio65"]},
        ),
        build(
            "event_gain_after_loss_floor_mfe65_mae50",
            "short path event(숏 경로 이벤트) if MFE >= train q65, MAE <= train q50, and known gain follows prior loss floor(확정 손실 하한 뒤 이익 전이)",
            (mfe >= q["mfe65"]) & (mae <= q["mae50"]) & (bars_since_loss <= 48.0) & (bars_since_gain <= 24.0),
            {"mfe65": q["mfe65"], "mae50": q["mae50"], "loss_floor": q["loss_floor"], "gain_floor": q["gain_floor"]},
        ),
    ]
    if profile == "initial":
        return [item for item in specs if item["event_variant"] == "event_loss_floor_transfer_mfe65_mae40_recent_loss"]
    if profile == "repair":
        specs.extend(
            [
                build(
                    "repair_event_loss_transfer_mfe60_mae55_horizon_pos",
                    "looser repair event(완화 수리 이벤트) if MFE >= train q60, MAE <= train q55, horizon pnl positive, and recent known loss floor(최근 확정 손실 하한)",
                    (mfe >= q["mfe60"]) & (mae <= q["mae55"]) & (horizon > 0.0) & (bars_since_loss <= 60.0),
                    {"mfe60": q["mfe60"], "mae55": q["mae55"], "loss_floor": q["loss_floor"], "recent_loss_bars": 60.0},
                ),
                build(
                    "repair_event_decay_memory_mfe65_mae50_ratio60",
                    "repair decay-memory event(감쇠 기억 수리 이벤트) if MFE >= train q65, MAE <= train q50, ratio >= q60, and known MAE decay(확정 MAE 감쇠)",
...
def sequence_context_specs(profile: str) -> list[dict[str, Any]]:
    ...
    for high_q, fast, slow in pairs:
        specs.append(
            {
                "context_variant": f"loss_floor_transfer_decay_q{int(high_q * 100)}_w{fast}_{slow}",
    ...
    specs = [
        {
            ...
            "risk_budget_definition": "minimal hygiene gate(최소 위생 게이트): no squeeze and realized vol expansion <= 2.25; loss-floor signal stays in model input(손실 하한 신호는 모델 입력에만 둠)",
            "equals_features": {
                "bb_squeeze": 0.0,
            },
            "max_fixed_features": {
...
def build_event_surface(
    ...
    *,
    ...
    summary = summarize_f50_candidates(split_metrics, selected)
    return {
...
def candidates_for_mask(
    ...
    condition = {
        ...
        "feature_family": "train_only_loss_floor_regime_transfer_score",
    ...
    for row in threshold_rows(
        ...
        take_quantiles,
        rr_floors,
        f"f50_{profile}_train_only_loss_floor_regime_transfer_sl_tp_caps",
    ):
        ...
        candidate["f50_train_selection_score"] = score_value
        candidate["selection_rank_basis"] = "train_only_event_loss_floor_regime_transfer_no_validation_oos_feedback"
        variants.append(candidate)
    variants.sort(key=lambda item: float(item["f50_train_selection_score"]), reverse=True)
    return variants[:2]
...
def rank_candidates(candidates: list[dict[str, Any]], prefix: str, limit: int) -> list[dict[str, Any]]:
    ...
    for candidate in sorted(candidates, key=lambda item: float(item["f50_train_selection_score"]), reverse=True):
        key = (
...
def summarize_f50_candidates(split_metrics: pd.DataFrame, candidates: list[dict[str, Any]]) -> pd.DataFrame:
    ...
    metadata = {
        str(item["candidate_id"]): {
            ...
            "f50_train_selection_score": item.get("f50_train_selection_score", ""),
    ...
    summary["f50_scout_clue_flag"] = dual & density_scout & (f_min_pf >= SCOUT_MIN_PF) & (f_max_dd <= SCOUT_MAX_DD)
    summary["f50_seed_surface_flag"] = dual & density_seed & (f_min_pf >= SEED_MIN_PF) & (f_max_dd <= SEED_MAX_DD)
    summary["runtime_probe_candidate_flag"] = (
        summary["f50_seed_surface_flag"].astype(bool) & (f_min_pf >= RUNTIME_MIN_PF) & (f_max_dd <= RUNTIME_MAX_DD)
    )
    summary["f50_axis_gap_to_seed"] = (
...
def surface_counts(summary: pd.DataFrame) -> dict[str, Any]:
    ...
    return {
        "scout_clue_rows": int(summary["f50_scout_clue_flag"].sum()),
        "seed_surface_rows": int(summary["f50_seed_surface_flag"].sum()),
...
def closest_nonwinner_forward_observation(combined: pd.DataFrame, best: dict[str, Any]) -> dict[str, Any]:
    ...
    frame["__seed_gap"] = numeric_column("f50_axis_gap_to_seed", 999999.0)
...
def build_repair_decision(initial_summary: pd.DataFrame) -> dict[str, Any]:
    if initial_summary.empty:
        return {
            ...
            "repair_action": "run_capped_loss_floor_transfer_input_surface_repair",
            "repair_reason": "Initial loss-floor regime transfer surface produced no candidate summary.",
        }
    runtime = int(initial_summary["runtime_probe_candidate_flag"].sum())
    seed = int(initial_summary["f50_seed_surface_flag"].sum())
    scout_rows = int(initial_summary["f50_scout_clue_flag"].sum())
    if runtime:
        return {
    ...
    return {
        "run_repair_grid": True,
        "repair_action": "run_capped_loss_floor_transfer_input_surface_repair",
        "repair_reason": f"Initial loss-floor regime transfer surface scout={scout_rows}, seed=0, runtime=0; run bounded input-surface repair.",
...
def classify_closeout(initial_summary: pd.DataFrame, repair_summary: pd.DataFrame) -> dict[str, Any]:
    ...
    if combined.empty:
        ...
        scout_rows = int(combined["f50_scout_clue_flag"].sum())
        seed_rows = int(combined["f50_seed_surface_flag"].sum())
    ...
    if runtime_rows:
        ...
        runtime_status = "runtime_probe_out_of_scope_by_claim_seed_only_no_runtime_candidate_after_f50_loss_floor_regime_transfer_proxy"
        next_stage = NEXT_STAGE_ID
        next_run = NEXT_RUN_ID
    elif scout_rows:
        closeout_class = "preserved_clue_negative_memory"
        runtime_status = "runtime_probe_ineligible_no_seed_or_runtime_candidate_after_f50_loss_floor_regime_transfer_proxy"
        next_stage = NEXT_STAGE_ID
        next_run = NEXT_RUN_ID
    else:
        closeout_class = "negative_memory"
        runtime_status = "runtime_probe_ineligible_no_scout_seed_or_runtime_candidate_after_f50_loss_floor_regime_transfer_proxy"
        next_stage = NEXT_STAGE_ID
        next_run = NEXT_RUN_ID
    return {
...
def build_closeout_prompt(closeout: dict[str, Any], best_rows: list[dict[str, Any]], repair_decision: dict[str, Any]) -> str:
    ...
    compact_rows = "\n".join(
        (
            ...
            f"fwd_dd={row.get('forward_max_dd')}; scout={row.get('f50_scout_clue_flag')}; "
            f"seed={row.get('f50_seed_surface_flag')}; runtime={row.get('runtime_probe_candidate_flag')}"
        )
        for row in best_rows[:6]
    )
    return f"""# Frontier50 closeout Grok review(그록 마감 검토)
...
- hygiene_variant(위생 변형): {best.get("risk_budget_variant")}
...
- scout/seed/runtime(탐색/씨앗/런타임): {best.get("f50_scout_clue_flag")}/{best.get("f50_seed_surface_flag")}/{best.get("runtime_probe_candidate_flag")}
...
- loss_floor_threshold(손실 하한 임계값): {best.get("loss_floor_threshold")}
...
F50 opened train-only loss-floor regime transfer(학습 전용 손실 하한 체제 전이) hypothesis(가설). F49 is reference only(참조 전용), not baseline/winner(기준선/승자 아님).
"""
...
- final_codex_direction(최종 코덱스 방향): run F50 loss-floor regime transfer proxy with validation/OOS read-only(검증/표본외 읽기 전용)
"""
...
- final_codex_direction(최종 코덱스 방향): close F50 as `{closeout.get("closeout_class")}` with no authority claim(권위 주장 없음)
"""
    return {
    ...
    rows = [
        {
            ...
            "notes": "F50 opened with train-only loss-floor regime transfer hypothesis and Grok guardrails.",
        },
        {
            "stage_id": STAGE_ID,
            "run_id": RUN_B,
            ...
            "notes": "Initial train-only loss-floor regime transfer proxy surface.",
        },
        {
            "stage_id": STAGE_ID,
            "run_id": RUN_C,
            ...
            "runtime_probe_status": "evaluated_for_runtime_candidate",
            "notes": "Capped loss-floor regime transfer repair diagnostic.",
        },
        {
            "stage_id": STAGE_ID,
            "run_id": RUN_D,
        ...
        },
        {
            ...
            "closeout_class": closeout.get("closeout_class"),
            "runtime_probe_status": "out_of_scope_by_claim_tier_a_loss_floor_regime_transfer_proxy_only",
            "notes": "F50 used Tier A loss-floor regime transfer proxy only; Tier B not claimed.",
        },
        {
            "stage_id": STAGE_ID,
            "run_id": RUN_D,
...
def project_ledger_row(row: dict[str, Any], fields: list[str]) -> dict[str, Any]:
    ...
    values = {
        ...
        "kpi_scope": "short_loss_floor_regime_transfer_proxy",
        ...
        "run_family": "frontier_short_loss_floor_regime_transfer_proxy",
...
current_judgment: {closeout.get("closeout_class")}(F50 train-only loss-floor regime transfer proxy no operating authority)
...
Frontier50(F50, 전선 50단계)가 `{closeout.get("closeout_class")}`로 닫혔다.
...
F50 carry-forward(이월) 기록은 train-only loss-floor regime transfer(학습 전용 손실 하한 체제 전이)이 PF/DD/density(수익 팩터/손실폭/밀도)를 얼마나 바꾸었는지와 seed/runtime(씨앗/런타임) 후보가 생겼는지 여부다.
"""
...
def artifact_map() -> dict[str, str]:
    return {
        "input_manifest": (INPUT_ROOT / "loss_floor_regime_transfer_manifest.json").as_posix(),
