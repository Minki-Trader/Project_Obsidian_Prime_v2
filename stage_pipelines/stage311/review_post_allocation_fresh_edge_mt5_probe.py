...
RUN_ID = "run311C_review_post_allocation_fresh_edge_mt5_probe_v1"
RUN_NUMBER = "run311C"
SOURCE_RUN_ID = "run311B_execute_post_allocation_fresh_edge_mt5_probe_v1"
PARENT_RUN_ID = "run311A_design_post_allocation_fresh_edge_rebuild_packet_v1"
UPDATED_ON = "2026-05-24"
BOUNDARY = r310.BOUNDARY

NEXT_REBUILD_STAGE_ID = "312_onnx_candidate_campaign__fresh_model_asymmetry_rebuild"
NEXT_ADAPTER_STAGE_ID = "312_onnx_candidate_campaign__adapter_package_for_post_allocation_fresh_edge"

STAGE_ROOT = ROOT / "stages" / STAGE_ID
RUN311B = STAGE_ROOT / "02_runs" / "run311B"
...
SOURCE_KPI = RUN311B / "mt5_kpi_summary.csv"
SOURCE_ATTEMPT_SUMMARY = RUN311B / "attempt_summary.csv"
PRODUCER = Path("stage_pipelines/stage311/review_post_allocation_fresh_edge_mt5_probe.py")

SCOREBOARD = RUN_ROOT / "post_allocation_fresh_edge_review_scoreboard.csv"
...
NEXT_STAGE_QUEUE = RUN_ROOT / "stage312_seed_queue.csv"
RESULT_JUDGMENT = RUN_ROOT / "result_judgment.csv"
GATE_AUDIT = RUN_ROOT / "required_gate_coverage_audit.csv"
RUN_MANIFEST = RUN_ROOT / "run_manifest.json"
LINEAGE = RUN_ROOT / "artifact_lineage_receipt.json"
REPORT = REVIEWS / "run311C_review_stage312_open.md"
DECISION = ROOT / "docs" / "decisions" / "2026-05-24_stage311_post_allocation_fresh_edge_review_stage312_open.md"
...
def next_stage_rows(scoreboard: Sequence[Mapping[str, Any]]) -> list[dict[str, Any]]:
    ...
    return [
        {
            "seed_id": "stage311_fresh_edge_review_seed",
            "source_candidates": ";".join(row["package_id"] for row in positive[:3]) or "none",
            "fresh_thesis": "fresh_model_asymmetry_rebuild",
            "use_as": "failure_memory_or_candidate_seed",
            "upside": "adverse-hour mirror may repair validation loss only if actual MT5 confirms scale and curve.",
            "failure_mode": "fails if validation/OOS, density, efficiency, or curve cannot pass together.",
            "discard_condition": "discard narrow hour mirror repair; require model/asymmetry feature surface if no gate pass.",
            "next_action": "run312A_design_fresh_model_asymmetry_rebuild_packet",
        }
    ]
...
def scaffold_next_stage(selected_rows: Sequence[Mapping[str, Any]]) -> tuple[str, str, str, str]:
    if selected_rows:
        ...
        question = "selected post-allocation fresh edge(선택된 배분 이후 새 엣지)를 Adapter package(어댑터 패키지)로 정리할 수 있는가?"
        status = "opened_adapter_package_stage_after_stage311_candidate_selection"
        judgment = "candidate_selected_adapter_package_stage_opened_no_onnx_yet"
        next_action = "run312A_build_post_allocation_fresh_edge_adapter_package"
    else:
        next_stage_id = NEXT_REBUILD_STAGE_ID
        question = "Stage311 adverse-hour mirror(311단계 불리 시간 반전) 실패 기억을 넘어 fresh model asymmetry(새 모델 비대칭)를 만들 수 있는가?"
        status = "opened_fresh_model_asymmetry_rebuild_after_stage311_no_selection"
        judgment = "no_candidate_selected_fresh_model_asymmetry_stage_opened"
        next_action = "run312A_design_fresh_model_asymmetry_rebuild_packet"
    ...
    write_text(
        ...
        "\n".join(
            [
                ...
                "",
                "Effect(효과): Stage311(311단계)의 결과를 운영 의미가 아니라 다음 연구 질문의 근거로 넘긴다.",
            ]
        ),
    )
    write_text(
        ...
        "\n".join(
            [
                "# Stage312 Selection Status(312단계 선택 상태)",
                "",
                ...
                f"- stage311_review(311단계 검토): `{rel(REPORT)}`",
            ]
        ),
    )
    write_text(next_root / "03_reviews" / "review_index.md", f"# Stage312 Review Index(312단계 검토 색인)\n\n- stage311_review(311단계 검토): `{rel(REPORT)}`\n")
    write_csv(
        next_root / "03_reviews" / "stage_run_ledger.csv",
        STAGE_LEDGER_COLUMNS,
        [
            {
                "row_id": f"{RUN_ID}__stage312_open",
...
def report_markdown(scoreboard: Sequence[Mapping[str, Any]], selected_rows: Sequence[Mapping[str, Any]], next_stage_id: str, next_action: str) -> str:
    ...
    lines = [
        ...
        f"- selected_candidate(선택 후보): `{selected_rows[0]['package_id'] if selected_rows else 'none'}`",
        f"- Adapter package(어댑터 패키지): `{'deferred_to_stage312' if selected_rows else 'none'}`",
        "- ONNX readiness(온엑스 준비): `not_started`",
        "- Goal Achieve(목표 달성): `not_claimed`",
    ...
    lines.extend(
        [
            ...
            "## Decision(결정)",
            "",
            "Stage311(311단계)은 selected candidate(선택 후보) 없이 닫는다." if not selected_rows else "Stage311(311단계)은 Adapter(어댑터) 패키지 단계로 넘긴다.",
...
def update_docs(status: str, judgment: str, next_stage_id: str, next_action: str, selected_rows: Sequence[Mapping[str, Any]]) -> None:
    ...
    selected = r310.replace_line(selected, "- Adapter package(", f"- Adapter package(어댑터 패키지): `{'deferred_to_stage312' if selected_rows else 'none'}`")
    selected = r310.replace_line(selected, "- next_action(", f"- next_action(다음 행동): `{next_action}`")
    selected = r310.drop_prefixed_lines(selected, ("- run311C_report(", "- stage312_opened("))
    selected += f"- run311C_report(311C 보고서): `{rel(REPORT)}`\n"
    selected += f"- stage312_opened(312단계 열림): `{next_stage_id}`\n"
    write_text(SELECTED, selected)

    review_index = read_text(REVIEW_INDEX)
    review_index = r310.drop_prefixed_lines(review_index, ("- run311C_report(", "- run311C_scoreboard(", "- stage312_seed_queue("))
    review_index += f"- run311C_report(311C 보고서): `{rel(REPORT)}`\n"
    review_index += f"- run311C_scoreboard(311C 점수판): `{rel(SCOREBOARD)}`\n"
    review_index += f"- stage312_seed_queue(312단계 씨앗 대기열): `{rel(NEXT_STAGE_QUEUE)}`\n"
    ...
    current = r310.drop_prefixed_lines(current, ("- run311C_summary(",))
    current = current.rstrip() + f"\n- run311C_summary(311C 요약): Stage311(311단계) actual MT5(실제 메타트레이더5) 검토를 완료했다. Effect(효과): selected_candidate(선택 후보)는 `{selected_rows[0]['package_id'] if selected_rows else 'none'}`이고 next_stage(다음 단계)는 `{next_stage_id}`다.\n"
    ...
    focus = (
        f"- >-\n"
        f"  Stage311(311단계) run311C(311C 실행) post-allocation fresh edge review(배분 이후 새 엣지 검토) `{RUN_ID}` closed Stage311 and opened `{next_stage_id}`. "
        f"Effect(효과): selected candidate(선택 후보)는 `{selected_rows[0]['package_id'] if selected_rows else 'none'}`이고 Adapter package(어댑터 패키지)는 `{'deferred_to_stage312' if selected_rows else 'none'}`, ONNX readiness(온엑스 준비)는 `not_started`다.\n"
    )
    ...
    if RUN_ID not in changelog:
        changelog += (
            f"\n## {UPDATED_ON} run311C Post-allocation fresh edge review(311C 배분 이후 새 엣지 검토)\n\n"
            f"- status(상태): `{status}`\n"
            f"- judgment(판정): `{judgment}`\n"
            f"- effect(효과): Stage311(311단계)을 닫고 `{next_stage_id}`를 열었다.\n"
...
def update_memory_registers(failures: Sequence[Mapping[str, Any]], selected_rows: Sequence[Mapping[str, Any]]) -> None:
    ...
    if RUN_ID not in idea:
        idea += (
            f"\n## {RUN_ID} post_allocation_fresh_edge(배분 이후 새 엣지)\n\n"
    ...
    if failures:
        ...
        if RUN_ID not in negative:
            negative += (
                f"\n## {RUN_ID} Stage311 fresh edge failure memory(311단계 새 엣지 실패 기억)\n\n"
...
def update_registers(status: str, judgment: str, next_action: str) -> None:
    safe_upsert(RUN_REGISTRY, RUN_REGISTRY_COLUMNS, [{"run_id": RUN_ID, "stage_id": STAGE_ID, "lane": "post_allocation_fresh_edge_review", "status": status, "judgment": judgment, "path": rel(REPORT), "notes": f"selected_candidate_reviewed;next_action={next_action}."}], "run_id")
    safe_upsert(
        ALPHA_LEDGER,
        ...
        [
            {
                ...
                "record_view": "post_allocation_fresh_edge_review",
    ...
    safe_upsert(STAGE_LEDGER, STAGE_LEDGER_COLUMNS, [{"row_id": f"{RUN_ID}__review", "stage_id": STAGE_ID, "run_id": RUN_ID, "view": "post_allocation_fresh_edge_review", "tier_scope": "Tier A used/Tier B fallback/actual routed total", "scoreboard": "post_allocation_fresh_edge_review_scoreboard", "status": status, "judgment": judgment, "evidence_boundary": "runtime_probe_review_no_onnx", "report_path": rel(REPORT), "notes": "Stage312 opened if no selected candidate; ONNX not started."}], "row_id")
        ...
        rows.append(
            {
                ...
                "artifact_type": "stage311_post_allocation_fresh_edge_review_artifact",
                "path": rel(path),
                "sha256": sha256_file(path),
                "stage_id": STAGE_ID,
                "run_id": RUN_ID,
                "created_at_utc": "2026-05-24T23:59:00Z",
                "notes": "Stage311 review and Stage312 open handoff",
            }
        )
...
def main() -> None:
    ...
    stage312_rows = next_stage_rows(scoreboard)
    next_stage_id, _next_status, _next_judgment, next_action = scaffold_next_stage(selected_rows)
    status = "completed_post_allocation_fresh_edge_review_stage312_opened"
    judgment = "actual_mt5_candidate_gate_passed_adapter_stage_opened" if selected_rows else "actual_mt5_no_onnx_worthy_candidate_fresh_model_asymmetry_rebuild_opened"
    ...
    write_csv(NEXT_STAGE_QUEUE, list(stage312_rows[0].keys()), stage312_rows)
    write_csv(
        RESULT_JUDGMENT,
        ("run_id", "status", "judgment", "selected_candidate", "adapter_package", "onnx_readiness", "next_action", "claim_boundary"),
        [{"run_id": RUN_ID, "status": status, "judgment": judgment, "selected_candidate": selected_rows[0]["package_id"] if selected_rows else "none", "adapter_package": "deferred_to_stage312" if selected_rows else "none", "onnx_readiness": "not_started", "next_action": next_action, "claim_boundary": BOUNDARY}],
    )
    gate_rows = [
    ...
    manifest = {
        ...
        "adapter_package": "deferred_to_stage312" if selected_rows else "none",
    ...
    print(json.dumps({"status": status, "judgment": judgment, "scoreboard_rows": len(scoreboard), "failure_rows": len(failure_rows), "selected_candidate": selected_rows[0]["package_id"] if selected_rows else "none", "adapter_package": "deferred_to_stage312" if selected_rows else "none", "onnx_readiness": "not_started", "goal_achieve": "not_claimed", "next_stage_id": next_stage_id, "next_action": next_action}, ensure_ascii=False, indent=2, sort_keys=True))
