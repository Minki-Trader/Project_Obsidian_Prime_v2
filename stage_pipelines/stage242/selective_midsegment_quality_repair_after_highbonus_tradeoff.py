...
STAGE_ID = "242_adapter_research__selective_midsegment_quality_repair_after_highbonus_tradeoff"
RUN_NUMBER = "run242A"
RUN_ID = "run242A_stage242_selective_midsegment_quality_repair_after_highbonus_tradeoff_v1"
PACKET_ID = "stage242_selective_midsegment_quality_repair_after_highbonus_tradeoff_v1"
PARENT_RUN_ID = "run241A_stage241_stage240_highbonus_repair_followup_review_v1"
SOURCE_STAGE_ID = "241_adapter_research__stage240_highbonus_repair_followup_review"
SOURCE_RUN_ID = PARENT_RUN_ID
SOURCE_STAGE241_EVIDENCE_COMMIT = "d005c4f7dcb4c95c6ac4d6c774205fec8df61d95"
SOURCE_STAGE241_HASH_RECORD_COMMIT = "978cccf09d6ddbb684edbfb52e320333093d1ff4"
NEXT_STAGE_ID = "243_adapter_research__stage242_selective_midsegment_followup_review"
NEXT_RUN_ID = "run243A_stage243_stage242_selective_midsegment_followup_review_v1"
NEXT_PACKET_ID = "stage243_stage242_selective_midsegment_followup_review_v1"
...
COMMON_ROOT = f"OPV2/s242a/{RUN_NUMBER}"

SUMMARY_JSON_PATH = REVIEWS_ROOT / "stage242_summary.json"
SUMMARY_CSV_PATH = REVIEWS_ROOT / "stage242_selective_midsegment_kpi_summary.csv"
REPORT_PATH = REVIEWS_ROOT / "stage242_selective_midsegment_report.md"
SEGMENT_KPI_PATH = REVIEWS_ROOT / "stage242_segment_kpi_summary.csv"
BALANCE_CURVE_AUDIT_PATH = REVIEWS_ROOT / "stage242_balance_curve_audit.csv"
MONTHLY_KPI_PATH = REVIEWS_ROOT / "stage242_monthly_kpi_summary.csv"
CONCENTRATION_PATH = REVIEWS_ROOT / "stage242_concentration_risk_summary.csv"
DRAWDOWN_PATH = REVIEWS_ROOT / "stage242_drawdown_recovery_summary.csv"
QUALITY_MATRIX_PATH = REVIEWS_ROOT / "stage242_quality_matrix.csv"
RISK_ATR_TELEMETRY_PATH = REVIEWS_ROOT / "stage242_risk_atr_telemetry.csv"
GATE_FEATURE_SUMMARY_PATH = REVIEWS_ROOT / "stage242_gate_feature_summary.csv"
PROBABILITY_BINDING_PATH = REVIEWS_ROOT / "stage242_probability_telemetry_summary.csv"
MODEL_SCORE_AUDIT_PATH = REVIEWS_ROOT / "stage242_model_score_audit.csv"
TIER_B_DIAGNOSTIC_PATH = REVIEWS_ROOT / "stage242_tier_b_diagnostic_summary.csv"
DECISION_PATH = REVIEWS_ROOT / "stage242_decision.md"
AUDIT_CSV_PATH = REVIEWS_ROOT / "stage242_trade_audit.csv"
...
PRODUCER_PATH = Path("stage_pipelines/stage242/selective_midsegment_quality_repair_after_highbonus_tradeoff.py")
ARTIFACT_COLUMNS = stage238.ARTIFACT_COLUMNS

SIGNAL_COLUMN = stage238.SIGNAL_COLUMN
RANK_COLUMN = "stage242_margin_rank_bucket"
...
VARIANTS = (
    variant(
        "s242_samecap_control",
        "stage242_samecap_control",
        bonus_high=0.10,
        bonus_vhigh=0.15,
        risk_cap=0.031375,
        mid_guard_mode="none",
        note="Stage242 control: repeat Stage240 highbonus same-cap behavior.",
    ),
    variant(
        "s242_midlow_guard",
        "stage242_midlow_guard",
        bonus_high=0.10,
        bonus_vhigh=0.15,
        risk_cap=0.031375,
        mid_guard_mode="low",
        note="Stage242 repair: block only low-margin signals inside the middle window.",
    ),
    variant(
        "s242_midlowmid_guard",
        "stage242_midlowmid_guard",
        bonus_high=0.10,
        bonus_vhigh=0.15,
        risk_cap=0.031375,
        mid_guard_mode="low_mid",
        note="Stage242 repair: block low and mid margin signals inside the middle window.",
    ),
    variant(
        "s242_midlowmid_guard_cap0305",
        "stage242_midlowmid_guard_cap0305",
        bonus_high=0.10,
        bonus_vhigh=0.15,
        risk_cap=0.0305,
        mid_guard_mode="low_mid",
        note="Stage242 repair: combine selective middle-window guard with a mild cap only.",
    ),
)
...
VARIANT_EXTRAS: dict[str, dict[str, Any]] = {
    "s242_samecap_control": extra("samecap_control", 0.10, 0.15, "none"),
    "s242_midlow_guard": extra("midlow_guard", 0.10, 0.15, "low"),
    "s242_midlowmid_guard": extra("midlowmid_guard", 0.10, 0.15, "low_mid"),
    "s242_midlowmid_guard_cap0305": extra("midlowmid_guard_cap0305", 0.10, 0.15, "low_mid"),
}
...
def write_selective_feature(source: Path, destination: Path, variant: Any, split: str) -> dict[str, Any]:
    gate_column = f"stage242_gate_{VARIANT_EXTRAS[variant.adapter_id]['axis']}"
    ...
    return {
        ...
        "gate_description": "Stage242 reference side filter plus selective middle-window low/mid margin guard.",
...
def prepare_inputs(common_files_root: Path) -> dict[str, Any]:
    ...
    for item in VARIANTS:
        gate_column = f"stage242_gate_{VARIANT_EXTRAS[item.adapter_id]['axis']}"
        ...
        copied.append(
            {
                ...
                "transform": "stage242_three_feature_selective_midsegment_score_shape_model",
            }
        )
            ...
            copied.append(
                {
                    ...
                    "transform": "stage242_margin_rank_plus_midwindow_selective_guard_feature",
                }
            )
            ...
            for role_index, (tier, attempt_role, prefix, attempt_token) in enumerate(
                ...
                attempts.append(
                    stage238.s161.base.attempt_payload(
                        ...
                        run_id=RUN_ID,
                        stage_number=242,
                        exploration_label="stage242_BaselineAdapter__SelectiveMidsegmentRepair",
...
def decide(quality_rows: Sequence[Mapping[str, Any]], external: str) -> str:
    if external != "completed":
        return "continue_stage242_runtime_completion_due_to_incomplete_runtime_candidate_not_final"
    if any(pass_stage242(row) for row in quality_rows):
        return "open_stage243_bounded_followup_due_to_selective_midsegment_repair_candidate_not_final"
    return "open_stage243_bounded_followup_due_to_selective_midsegment_tradeoff_candidate_not_final"
            ...
            rows.append(
                {
                    ...
                    "split": split,
                    "status": "diagnostic_missing_required_but_disabled_for_stage242_selective_midsegment_repair",
                    "fallback_enabled": 0,
                    "fallback_used_count": 0,
                    "notes": "Stage242 isolates Tier A routed selective midsegment repair; Tier B fallback remains disabled by prior fallback-only damage memory.",
                }
            )
    return rows
...
def report_markdown(quality_rows: Sequence[Mapping[str, Any]], decision: str, external: str) -> str:
    ...
    lines = [
        ...
        f"- source_stage241_evidence_commit(원천 241단계 근거 커밋): `{SOURCE_STAGE241_EVIDENCE_COMMIT}`",
        f"- source_stage241_hash_record_commit(원천 241단계 해시 기록 커밋): `{SOURCE_STAGE241_HASH_RECORD_COMMIT}`",
...
def decision_markdown(decision: str, external: str) -> str:
    ...
    return f"""# Stage242 Decision(242단계 판정)
...
Stage243(243단계)는 Stage242(242단계) selective midsegment repair(선택적 중간 구간 수리) 결과를 follow-up review(후속 검토)하는 bounded review(경계 검토) 단계다.

## Bounded Question(경계 질문)

Did Stage242(242단계) improve validation DD(검증 낙폭), mid PF(중간 수익요인), and cost-stressed behavior(비용 압박 행동) while preserving validation/OOS net(검증/표본외 순손익), ATR SL/TP(ATR 손절/익절), model-controlled risk%(모델 제어 위험 비율), and segment behavior(구간 행동)?

Effect(효과): Stage242(242단계) 안에서 다음 수리를 흡수하지 않고 selective midsegment repair(선택적 중간 구간 수리) 결과를 별도 review(검토)로 닫는다.
...
- status(상태): `open_planned_from_stage242`
- source_stage(원천 단계): `{STAGE_ID}`
- source_decision(원천 판정): `{decision}`
...
- stage_status(단계 상태): `open_planned_from_stage242`
...
- >-
  Stage242(242단계) closed(종료) as `{decision}` and Stage243(243단계) `{NEXT_STAGE_ID}` is open_planned(개방 계획). Effect(효과): selective midsegment repair(선택적 중간 구간 수리)의 KPI(핵심 성과 지표) 상충을 별도 review(검토)로 판정한다.
- >-
  Stage242 evidence(242단계 근거)는 `{rel(REPORT_PATH)}`, `{rel(SUMMARY_CSV_PATH)}`, `{rel(QUALITY_MATRIX_PATH)}`, `{rel(GATE_FEATURE_SUMMARY_PATH)}`, `{rel(RISK_ATR_TELEMETRY_PATH)}`에 있다. Effect(효과): 중간 기간 보호문이 34D(34D 기준)에 가까워졌는지 확인한다.
- >-
...
"""
  ...
  boundary: {BOUNDARY}
"""
    stage243_block = f"""stage243_stage242_selective_midsegment_followup_review:
  packet_id: {NEXT_PACKET_ID}
  stage_id: {NEXT_STAGE_ID}
  status: open_planned_from_stage242
...
"""
    ...
    if external == "completed":
        state = replace_stage_block(state, "stage243_stage242_selective_midsegment_followup_review", stage243_block)
...
- adapter_under_review(검토 중 어댑터): `stage242_selective_midsegment_quality_repair`
- status(상태): `stage242_{decision}`
- claim_boundary(주장 경계): `{BOUNDARY}`

Stage242(242단계)는 selective midsegment repair(선택적 중간 구간 수리)를 MT5(MetaTrader 5, 메타트레이더5)로 측정했다. Effect(효과): Stage243(243단계)가 결과 상충과 다음 bounded repair(경계 수리)를 별도 review(검토)로 판정한다.

## Latest Stage242 Evidence(최신 242단계 근거)
...
def append_changelog(decision: str) -> None:
    ...
    if RUN_ID in existing:
        return
    entry = (
        f"\n## {utc_now()} Stage242 selective midsegment repair closeout(242단계 선택적 중간 구간 수리 종료)\n\n"
        f"- action(행동): closed(종료) `{STAGE_ID}` with decision(판정) `{decision}`.\n"
        "- effect(효과): 중간 기간 low/mid margin guard(저/중간 마진 보호문)를 MT5(MetaTrader 5, 메타트레이더5)로 측정하고 Stage243(243단계) follow-up review(후속 검토)로 넘겼다.\n"
        f"- boundary(주장 경계): `{BOUNDARY}`.\n"
    )
            ...
            rows.append(
                {
                    ...
                    "artifact_type": "stage242_selective_midsegment_repair_evidence",
                    "path": rel(path),
                    "sha256": sha256_file_lf_normalized(path),
                    "stage_id": STAGE_ID,
                    "run_id": RUN_ID,
                    "created_at_utc": created,
                    "notes": "Stage242 selective midsegment repair evidence.",
                }
            )
    return rows
...
def write_ledgers(result: Mapping[str, Any], decision: str, artifacts: Sequence[Mapping[str, Any]]) -> dict[str, Any]:
    ...
    for row in alpha_rows:
        ...
        row["scoreboard_lane"] = "baseline_adapter_stage242_selective_midsegment_repair"
    ...
    run_rows = [
        {
            "run_id": RUN_ID,
            "stage_id": STAGE_ID,
            "lane": "baseline_adapter_stage242_selective_midsegment_repair",
            "status": "completed" if result.get("external_verification_status") == "completed" else "blocked",
            "judgment": decision,
            "path": rel(DECISION_PATH),
            "notes": ledger_pairs(
                [
                    ("source_stage241_evidence_commit", SOURCE_STAGE241_EVIDENCE_COMMIT),
                    ("source_stage241_hash_record_commit", SOURCE_STAGE241_HASH_RECORD_COMMIT),
                    ("target_surface", TARGET_SURFACE),
                    ("overall_goal_complete", 0),
                ]
            ),
...
def write_packet_files(result: Mapping[str, Any], decision: str, ledger_payload: Mapping[str, Any], quality: Sequence[Mapping[str, Any]]) -> None:
    ...
    files = {
        ...
        "result_judgment_gate.json": {
            **base_payload,
            ...
            "evidence_missing": ["Stage243 follow-up review not run yet(243단계 후속 검토 미실행)", "ONNX/runtime hardening not in scope(ONNX/런타임 경화 범위 밖)"],
        ...
        "artifact_lineage_audit.json": {
            **base_payload,
            "source_inputs": [rel(SOURCE_STAGE_ID), rel("stages/241_adapter_research__stage240_highbonus_repair_followup_review/03_reviews/stage241_decision.md")],
    ...
    write_md(
        ...
        f"""# Stage242 Closeout Packet(242단계 종료 작업 묶음)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    prepared = {
        ...
        "model_family": "baseline_adapter_stage242_v2_native_selective_midsegment_repair",
        "feature_set_id": "stage242_signal_margin_rank_plus_midwindow_guard",
