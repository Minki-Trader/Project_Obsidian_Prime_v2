...
from stage_pipelines.stage219 import entry_lifecycle_repair_after_bracket_axis_failure as s219  # noqa: E402
...
STAGE_ID = "221_adapter_research__entry_signal_gate_repair_after_lifecycle_axis_failure"
RUN_NUMBER = "run221A"
RUN_ID = "run221A_stage221_entry_signal_gate_repair_after_lifecycle_axis_failure_v1"
PACKET_ID = "stage221_entry_signal_gate_repair_after_lifecycle_axis_failure_v1"
PARENT_RUN_ID = "run220A_stage220_stage219_entry_lifecycle_followup_review_v1"
SOURCE_STAGE_ID = "220_adapter_research__stage219_entry_lifecycle_followup_review"
SOURCE_RUN_ID = "run220A_stage220_stage219_entry_lifecycle_followup_review_v1"
SOURCE_STAGE220_EVIDENCE_COMMIT = "cc387629951effd1cafc154e7d5617b9ffa6f936"
SOURCE_STAGE220_HASH_RECORD_COMMIT = "e00bc5234c0b1d332f0fc5632d1743b4dd1316a5"
SOURCE_STAGE219_CONTROL_ID = "s219_life_control_h3_sd8"
NEXT_STAGE_ID = "222_adapter_research__stage221_entry_signal_gate_followup_review"
NEXT_RUN_ID = "run222A_stage222_stage221_entry_signal_gate_followup_review_v1"
NEXT_PACKET_ID = "stage222_stage221_entry_signal_gate_followup_review_v1"
...
STAGE219_CONTROL = {
    "adapter_id": SOURCE_STAGE219_CONTROL_ID,
...
COMMON_ROOT = f"OPV2/s221a/{RUN_NUMBER}"

SUMMARY_JSON_PATH = REVIEWS_ROOT / "stage221_summary.json"
SUMMARY_CSV_PATH = REVIEWS_ROOT / "stage221_entry_signal_gate_kpi_summary.csv"
REPORT_PATH = REVIEWS_ROOT / "stage221_entry_signal_gate_repair_report.md"
SEGMENT_KPI_PATH = REVIEWS_ROOT / "stage221_segment_kpi_summary.csv"
BALANCE_CURVE_AUDIT_PATH = REVIEWS_ROOT / "stage221_balance_curve_audit.csv"
MONTHLY_KPI_PATH = REVIEWS_ROOT / "stage221_monthly_kpi_summary.csv"
CONCENTRATION_PATH = REVIEWS_ROOT / "stage221_concentration_risk_summary.csv"
DRAWDOWN_PATH = REVIEWS_ROOT / "stage221_drawdown_recovery_summary.csv"
QUALITY_MATRIX_PATH = REVIEWS_ROOT / "stage221_quality_matrix.csv"
RISK_ATR_TELEMETRY_PATH = REVIEWS_ROOT / "stage221_risk_atr_telemetry.csv"
GATE_FEATURE_SUMMARY_PATH = REVIEWS_ROOT / "stage221_gate_feature_summary.csv"
PROBABILITY_BINDING_PATH = REVIEWS_ROOT / "stage221_probability_telemetry_summary.csv"
MODEL_SCORE_AUDIT_PATH = REVIEWS_ROOT / "stage221_model_score_audit.csv"
TIER_B_DIAGNOSTIC_PATH = REVIEWS_ROOT / "stage221_tier_b_diagnostic_summary.csv"
DECISION_PATH = REVIEWS_ROOT / "stage221_decision.md"
AUDIT_CSV_PATH = REVIEWS_ROOT / "stage221_trade_audit.csv"
...
PRODUCER_PATH = Path("stage_pipelines/stage221/entry_signal_gate_repair_after_lifecycle_axis_failure.py")
ARTIFACT_COLUMNS = s172.ARTIFACT_COLUMNS

VARIANTS = (
    repair.RepairVariant(
        adapter_id="s221_gate_control",
        label="stage221_gate_control",
        atr_enabled=True,
        ...
        max_hold_bars=3,
        notes="Stage221 control: Stage219 control lifecycle and gate.",
    ),
    repair.RepairVariant(
        adapter_id="s221_gate_short_broad",
        label="stage221_gate_short_broadened",
        atr_enabled=True,
        model_risk_enabled=True,
        fixed_lot=0.25,
        atr_stop_multiplier=2.0325,
        ...
        close_only_on_opposite_signal=False,
        max_hold_bars=3,
        notes="Stage221 gate repair: broaden short low-edge block with all bracket/risk/lifecycle settings fixed.",
    ),
    repair.RepairVariant(
        adapter_id="s221_gate_short_narrow",
        label="stage221_gate_short_narrowed",
        atr_enabled=True,
        model_risk_enabled=True,
        fixed_lot=0.25,
        atr_stop_multiplier=2.0325,
        ...
        close_only_on_opposite_signal=False,
        max_hold_bars=3,
        notes="Stage221 gate repair: narrow short low-edge block with all bracket/risk/lifecycle settings fixed.",
    ),
    repair.RepairVariant(
        adapter_id="s221_gate_no_long_block",
        label="stage221_gate_no_long_block",
        atr_enabled=True,
        model_risk_enabled=True,
        fixed_lot=0.25,
        atr_stop_multiplier=2.0325,
        ...
        close_only_on_opposite_signal=False,
        max_hold_bars=3,
        notes="Stage221 gate repair: disable long session block with all bracket/risk/lifecycle settings fixed.",
    ),
)

BASE_EXTRA = {**s213.BASE_EXTRA, "block_mode": "encoded_side_context"}
VARIANT_EXTRAS: dict[str, dict[str, Any]] = {
    "s221_gate_control": {**BASE_EXTRA, "short_block_rule": "midwide_lowedge", "long_block_rule": "session_only", "axis": "control"},
    "s221_gate_short_broad": {
        **BASE_EXTRA,
        "short_block_rule": "midwide_lowedge",
        "long_block_rule": "session_only",
        "wide_session_min": 145.0,
        "wide_session_max": 300.0,
        "wide_margin_min": 0.0300,
        "wide_margin_max": 0.0900,
        "axis": "short_broad",
    },
    "s221_gate_short_narrow": {
        **BASE_EXTRA,
        "short_block_rule": "midwide_lowedge",
        "long_block_rule": "session_only",
        "wide_session_min": 175.0,
        "wide_session_max": 255.0,
        "wide_margin_min": 0.0525,
        "wide_margin_max": 0.0675,
        "axis": "short_narrow",
    },
    "s221_gate_no_long_block": {**BASE_EXTRA, "short_block_rule": "midwide_lowedge", "long_block_rule": "none", "axis": "no_long_block"},
}
...
def apply_stage_values(context_specs: Mapping[str, Any]) -> None:
    values = {
        ...
        "SOURCE_ADAPTER_ID": SOURCE_STAGE219_CONTROL_ID,
    ...
    for module in (s219, s213, s210, s192, s190, base, s188, s184, s180, s178, s176, s174, s172, s161):
    ...
    for variant_index, variant in enumerate(VARIANTS, start=1):
        ...
        for split in ("validation_is", "oos"):
            ...
            for role_index, (tier, attempt_role, prefix, attempt_token) in enumerate(
                ...
                magic = 22110000 + variant_index * 100 + (1 if split == "validation_is" else 50) + role_index
                attempts.append(
                    s161.base.attempt_payload(
                        run_root=variant_root,
                        run_id=RUN_ID,
                        stage_number=221,
                        exploration_label="stage221_BaselineAdapter__EntrySignalGateRepairAfterLifecycleAxisFailure",
...
def configure_runner() -> None:
    ...
    for variant in VARIANTS:
        ...
        context_specs[variant.adapter_id] = {
            "gate_column": f"stage221_gate_{extra['axis']}",
            "gate_type": "encoded_stage221_entry_signal_gate_repair",
            ...
            "description": "Stage221 encoded gate: entry signal/gate selectivity changes with bracket/risk/lifecycle fixed.",
        }
...
def pass_entry_signal_gate(row: Mapping[str, Any]) -> bool:
    return (
...
def decide(rows: Sequence[Mapping[str, Any]], external: str) -> str:
    if external != "completed":
        return "continue_stage221_runtime_completion_due_to_incomplete_runtime_candidate_not_final"
    if any(pass_entry_signal_gate(row) for row in rows):
        return "open_stage222_stage221_entry_signal_gate_followup_review_candidate_not_final"
    return "open_stage222_bounded_followup_due_to_entry_signal_gate_tradeoff_candidate_not_final"
...
def gate_label(adapter_id: str) -> str:
    labels = {
        "s221_gate_control": "control(대조군)",
        "s221_gate_short_broad": "short_broad(숏 차단 확대)",
        "s221_gate_short_narrow": "short_narrow(숏 차단 축소)",
        "s221_gate_no_long_block": "no_long_block(롱 차단 해제)",
    }
...
def report_markdown(rows: Sequence[Mapping[str, Any]], decision: str, external: str) -> str:
    best = best_row(rows)
    return f"""# Stage221 Entry Signal/Gate Repair Report(221단계 진입 신호/게이트 수리 보고서)

- stage(단계): `{STAGE_ID}`
- run(실행): `{RUN_ID}`
- source_stage(원천 단계): `{SOURCE_STAGE_ID}`
- source_run(원천 실행): `{SOURCE_RUN_ID}`
- source_stage220_evidence_commit(원천 220단계 근거 커밋): `{SOURCE_STAGE220_EVIDENCE_COMMIT}`
- source_stage220_hash_record_commit(원천 220단계 해시 기록 커밋): `{SOURCE_STAGE220_HASH_RECORD_COMMIT}`
- source_stage219_control(원천 219단계 대조군): `{SOURCE_STAGE219_CONTROL_ID}`
...
- comparison_baseline(비교 기준): Stage219 control(219단계 대조군) `{SOURCE_STAGE219_CONTROL_ID}`와 Stage210 anchor(210단계 기준 후보) `{STAGE210_ANCHOR["adapter_id"]}`다.
...
- baseline_gap(기준 차이): Stage219 control(219단계 대조군)은 validation net(검증 순손익) `{STAGE219_CONTROL["validation_net"]}`, early PF(초반 수익요인) `{STAGE219_CONTROL["validation_early_pf"]}`, mid PF(중반 수익요인) `{STAGE219_CONTROL["validation_mid_pf"]}`, OOS net(표본외 순손익) `{STAGE219_CONTROL["oos_net"]}`였다.
...
def decision_markdown(decision: str, external: str) -> str:
    return f"""# Stage221 Decision(221단계 판정)
...
Stage221(221단계) closeout(종료)은 overall goal complete(전체 목표 완료)가 아니다.

Effect(효과): Stage222(222단계) follow-up review(후속 검토)에서 entry signal/gate repair(진입 신호/게이트 수리)의 KPI(핵심 성과 지표) 상충을 별도로 판정한다.
"""
...
def artifact_rows(result: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    for row in rows:
        if row.get("artifact_type") != "mt5_strategy_tester_report":
            row["artifact_type"] = "stage221_evidence"
            row["notes"] = "Stage221 entry signal/gate repair after lifecycle-axis failure evidence."
        else:
            row["notes"] = "Stage221 MT5 Strategy Tester report."
    return rows
        ...
        [
            {
                "run_id": RUN_ID,
                "stage_id": STAGE_ID,
                "lane": "baseline_adapter_stage221_entry_signal_gate_repair",
                "status": status,
                "judgment": decision,
                "path": rel(DECISION_PATH),
                "notes": s172.ledger_pairs(
                    (
                        ("source_stage220_evidence_commit", SOURCE_STAGE220_EVIDENCE_COMMIT),
                        ("source_stage220_hash_record_commit", SOURCE_STAGE220_HASH_RECORD_COMMIT),
                        ("source_stage219_control", SOURCE_STAGE219_CONTROL_ID),
                        ("target_surface", TARGET_SURFACE),
...
def write_packet_files(result: Mapping[str, Any], decision: str, ledger_payload: Mapping[str, Any], quality: Sequence[Mapping[str, Any]]) -> None:
    ...
    s172.write_md(
        ...
        f"""# Stage221 Closeout Packet(221단계 종료 작업 묶음)
...
Stage222(222단계)은 Stage221(221단계) entry signal/gate repair(진입 신호/게이트 수리) 결과를 follow-up review(후속 검토)한다.

## Bounded Question(경계 질문)

Did Stage221(221단계) improve validation net(검증 순손익), early PF(초반 수익요인), mid PF(중반 수익요인), OOS net(표본외 순손익), drawdown(낙폭), segment KPI(구간 핵심 성과 지표), concentration risk(집중 위험), and risk/ATR telemetry(위험/ATR 기록) enough to continue this repair axis?

Effect(효과): Stage221(221단계) 안에서 추가 수리를 하지 않고, KPI(핵심 성과 지표) 판정을 별도 review(검토)로 닫는다.
...
""",
    ...
    s172.write_md(NEXT_STAGE_ROOT / "03_reviews" / "review_index.md", f"# Stage222 Review Index(222단계 검토 색인)\n\n- status(상태): `open_planned_from_stage221`\n- source_stage(원천 단계): `{STAGE_ID}`\n- source_decision(원천 판정): `{decision}`\n- next_action(다음 행동): `{NEXT_RUN_ID}`\n")
    s172.write_md(NEXT_STAGE_ROOT / "04_selected" / "selection_status.md", f"# Stage222 Selection Status(222단계 선택 상태)\n\n- stage_status(단계 상태): `open_planned_from_stage221`\n- current_packet(현재 작업 묶음): `{NEXT_PACKET_ID}`\n- current_run(현재 실행): `{NEXT_RUN_ID}`\n- source_stage(원천 단계): `{STAGE_ID}`\n- source_run(원천 실행): `{RUN_ID}`\n- source_decision(원천 판정): `{decision}`\n- claim_boundary(주장 경계): `{BOUNDARY}`\n")
...
- >-
  Stage221(221단계) closed(종료) as `{decision}` and Stage222(222단계) `{NEXT_STAGE_ID}` is open_planned(개방 계획). Effect(효과): entry signal/gate repair(진입 신호/게이트 수리)의 KPI(핵심 성과 지표) 상충을 별도 follow-up review(후속 검토)로 판정한다.
- >-
  Stage221 evidence(221단계 근거)는 `{rel(REPORT_PATH)}`, `{rel(SUMMARY_CSV_PATH)}`, `{rel(QUALITY_MATRIX_PATH)}`, `{rel(MONTHLY_KPI_PATH)}`, `{rel(CONCENTRATION_PATH)}`, `{rel(RISK_ATR_TELEMETRY_PATH)}`에 있다. Effect(효과): gate selectivity(게이트 선별성)가 34D(34D 기준) KPI(핵심 성과 지표) 간극을 줄였는지 확인한다.
- >-
...
"""
    if re.search(r"(?ms)^current_focus:\r?\n.*?(?=\r?\nstage\d+_)", state):
        ...
        state = state.rstrip() + "\n" + focus
    state = re.sub(r"(?ms)^stage221_entry_signal_gate_repair_after_lifecycle_axis_failure:\r?\n.*?(?=^stage\d+_|\Z)", "", state)
    block = f"""
stage221_entry_signal_gate_repair_after_lifecycle_axis_failure:
  packet_id: {PACKET_ID}
  stage_id: {STAGE_ID}
...
- adapter_under_review(검토 중 어댑터): `stage221_entry_signal_gate_repair`
- status(상태): `stage221_{decision}`
- claim_boundary(주장 경계): `{BOUNDARY}`

Stage221(221단계)는 Stage220(220단계) 판정에 따라 bracket/risk/lifecycle(브래킷/위험/생애주기)을 고정하고 entry signal/gate(진입 신호/게이트)만 MT5 Strategy Tester(MetaTrader 5 전략 테스터)로 측정했다. Effect(효과): Stage222(222단계)가 KPI(핵심 성과 지표) 상충을 별도 review(검토)로 판정한다.

## Latest Stage221 Evidence(최신 221단계 근거)
...
def append_changelog(decision: str) -> None:
    ...
    if RUN_ID in existing:
        return
    entry = (
        f"\n## {s172.utc_now()} Stage221 entry signal/gate repair closeout(221단계 진입 신호/게이트 수리 종료)\n\n"
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    prepared = {
        ...
        "model_family": "baseline_adapter_stage221_v2_native_entry_signal_gate_repair",
        "feature_set_id": "stage221_signal_plus_encoded_entry_gate_repair",
