...
STAGE_ID = "206_adapter_research__stage204_long_session_dd_micro_repair"
RUN_NUMBER = "run206A"
RUN_ID = "run206A_stage206_stage204_long_session_dd_micro_repair_v1"
PACKET_ID = "stage206_stage204_long_session_dd_micro_repair_v1"
PARENT_RUN_ID = "run205A_stage205_stage204_selective_probability_margin_followup_review_v1"
SOURCE_STAGE_ID = "205_adapter_research__stage204_selective_probability_margin_followup_review"
SOURCE_RUN_ID = "run205A_stage205_stage204_selective_probability_margin_followup_review_v1"
SOURCE_STAGE205_EVIDENCE_COMMIT = "4c09dec5cef0860628e44c754bcf0684697d9800"
SOURCE_STAGE205_HASH_RECORD_COMMIT = "77665b62b3a801fb6596de391f4f1b2fbacdc77c"
SOURCE_ADAPTER_ID = "s204_cd8_long_session_r0325"
NEXT_STAGE_ID = "207_adapter_research__stage206_long_session_dd_micro_repair_followup_review"
NEXT_RUN_ID = "run207A_stage207_stage206_long_session_dd_micro_repair_followup_review_v1"
NEXT_PACKET_ID = "stage207_stage206_long_session_dd_micro_repair_followup_review_v1"
...
COMMON_ROOT = f"OPV2/s206a/{RUN_NUMBER}"

SUMMARY_JSON_PATH = REVIEWS_ROOT / "stage206_dd_micro_repair_summary.json"
SUMMARY_CSV_PATH = REVIEWS_ROOT / "stage206_dd_micro_repair_kpi_summary.csv"
REPORT_PATH = REVIEWS_ROOT / "stage206_dd_micro_repair_report.md"
SEGMENT_KPI_PATH = REVIEWS_ROOT / "stage206_segment_kpi_summary.csv"
BALANCE_CURVE_AUDIT_PATH = REVIEWS_ROOT / "stage206_balance_curve_audit.csv"
MONTHLY_KPI_PATH = REVIEWS_ROOT / "stage206_monthly_kpi_summary.csv"
CONCENTRATION_PATH = REVIEWS_ROOT / "stage206_concentration_risk_summary.csv"
DRAWDOWN_PATH = REVIEWS_ROOT / "stage206_drawdown_recovery_summary.csv"
QUALITY_MATRIX_PATH = REVIEWS_ROOT / "stage206_quality_matrix.csv"
RISK_ATR_TELEMETRY_PATH = REVIEWS_ROOT / "stage206_risk_atr_telemetry.csv"
GATE_FEATURE_SUMMARY_PATH = REVIEWS_ROOT / "stage206_gate_feature_summary.csv"
PROBABILITY_BINDING_PATH = REVIEWS_ROOT / "stage206_probability_telemetry_summary.csv"
MODEL_SCORE_AUDIT_PATH = REVIEWS_ROOT / "stage206_model_score_audit.csv"
TIER_B_DIAGNOSTIC_PATH = REVIEWS_ROOT / "stage206_tier_b_diagnostic_summary.csv"
DECISION_PATH = REVIEWS_ROOT / "stage206_decision.md"
AUDIT_CSV_PATH = REVIEWS_ROOT / "stage206_trade_audit.csv"
...
PRODUCER_PATH = Path("stage_pipelines/stage206/long_session_dd_micro_repair.py")
ARTIFACT_COLUMNS = s172.ARTIFACT_COLUMNS

VARIANTS = (
    repair.RepairVariant(
        adapter_id="s206_ls_ref_r0325",
        label="stage206_ls_ref_r0325",
        atr_enabled=True,
        ...
        max_hold_bars=3,
        notes="Stage206 reference: Stage204 long_session shape with unchanged risk/ATR settings.",
    ),
    repair.RepairVariant(
        adapter_id="s206_ls_session_p5_r0325",
        label="stage206_ls_session_p5_r0325",
        atr_enabled=True,
        model_risk_enabled=True,
        fixed_lot=0.25,
        atr_stop_multiplier=2.075,
        ...
        close_only_on_opposite_signal=False,
        max_hold_bars=3,
        notes="Stage206 micro repair: widen only the long-session block by five minutes on each side.",
    ),
    repair.RepairVariant(
        adapter_id="s206_ls_session_p10_r0325",
        label="stage206_ls_session_p10_r0325",
        atr_enabled=True,
        model_risk_enabled=True,
        fixed_lot=0.25,
        atr_stop_multiplier=2.075,
        ...
        close_only_on_opposite_signal=False,
        max_hold_bars=3,
        notes="Stage206 micro repair: widen only the long-session block by ten minutes on each side.",
    ),
    repair.RepairVariant(
        adapter_id="s206_ls_risk0250",
        label="stage206_ls_risk0250",
        atr_enabled=True,
        model_risk_enabled=True,
        fixed_lot=0.25,
        atr_stop_multiplier=2.075,
        ...
        close_only_on_opposite_signal=False,
        max_hold_bars=3,
        notes="Stage206 micro repair: keep long-session gate and compress model risk cap to 2.5%.",
    ),
)
...
VARIANT_EXTRAS: dict[str, dict[str, Any]] = {
    "s206_ls_ref_r0325": {
        **BASE_EXTRA,
        ...
        "axis": "long_session_ref",
    },
    "s206_ls_session_p5_r0325": {
        **BASE_EXTRA,
        "wide_session_min": 165.0,
        "wide_session_max": 270.0,
        "short_block_rule": "midwide_lowedge",
        "long_block_rule": "session_only_p5",
        "axis": "session_p5",
    },
    "s206_ls_session_p10_r0325": {
        **BASE_EXTRA,
        "short_block_rule": "midwide_lowedge",
    ...
    for variant_index, variant in enumerate(VARIANTS, start=1):
        ...
        for split in ("validation_is", "oos"):
            ...
            for role_index, (tier, attempt_role, prefix, attempt_token) in enumerate(
                ...
                magic = 20610000 + variant_index * 100 + (1 if split == "validation_is" else 50) + role_index
                attempts.append(
                    s161.base.attempt_payload(
                        run_root=variant_root,
                        run_id=RUN_ID,
                        stage_number=206,
                        exploration_label="stage206_BaselineAdapter__LongSessionDDMicroRepair",
...
def configure_runner() -> None:
    ...
    for variant in VARIANTS:
        ...
        context_specs[variant.adapter_id] = {
            "gate_column": f"stage206_gate_{extra['axis']}",
            "gate_type": "encoded_stage206_long_session_dd_micro_repair",
            ...
            "description": "Stage206 encoded gate: long-session DD micro repair.",
        }
...
def decide(rows: Sequence[Mapping[str, Any]], external: str) -> str:
    if external != "completed":
        return "continue_stage206_runtime_completion_due_to_incomplete_runtime_candidate_not_final"
    if any(row.get("hard_quality_pass") for row in rows):
        return "open_stage207_stage206_dd_micro_repair_followup_review_candidate_not_final"
    return "open_stage207_bounded_followup_due_to_long_session_dd_tradeoff_candidate_not_final"
...
def report_markdown(rows: Sequence[Mapping[str, Any]], decision: str, external: str) -> str:
    best = best_row(rows)
    ref = next((row for row in rows if row.get("adapter_id") == "s206_ls_ref_r0325"), {})
    return f"""# Stage206 Long Session DD Micro Repair Report(206단계 롱 세션 낙폭 미세 수리 보고서)
...
- source_stage205_evidence_commit(원천 205단계 근거 커밋): `{SOURCE_STAGE205_EVIDENCE_COMMIT}`
- source_stage205_hash_record_commit(원천 205단계 해시 기록 커밋): `{SOURCE_STAGE205_HASH_RECORD_COMMIT}`
...
def decision_markdown(decision: str, external: str) -> str:
    return f"""# Stage206 Decision(206단계 판정)
...
Stage206(206단계) closeout(종료)은 overall goal complete(전체 목표 완료)가 아니다. Effect(효과): Stage207(207단계) follow-up review(후속 검토)에서 DD micro repair(낙폭 미세 수리) 상충을 따로 판정한다.
"""
...
def artifact_rows(result: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    for row in rows:
        if row.get("artifact_type") != "mt5_strategy_tester_report":
            row["artifact_type"] = "stage206_evidence"
            row["notes"] = "Stage206 long-session DD micro repair evidence."
        else:
            row["notes"] = "Stage206 MT5 Strategy Tester report."
    return rows
        ...
        [
            {
                "run_id": RUN_ID,
                "stage_id": STAGE_ID,
                "lane": "baseline_adapter_stage206_long_session_dd_micro_repair",
                "status": status,
                "judgment": decision,
                "path": rel(DECISION_PATH),
                "notes": s172.ledger_pairs(
                    (
                        ("source_stage205_evidence_commit", SOURCE_STAGE205_EVIDENCE_COMMIT),
                        ("source_stage205_hash_record_commit", SOURCE_STAGE205_HASH_RECORD_COMMIT),
        ...
        alpha_rows = [
            {
                ...
                "tier_scope": "Tier A+B",
                "kpi_scope": "stage206_long_session_dd_micro_repair",
                ...
                "external_verification_status": external,
                "notes": "Stage206 materialized or blocked before KPI records were available.",
            }
        ]
...
def write_packet_files(result: Mapping[str, Any], decision: str, ledger_payload: Mapping[str, Any], quality: Sequence[Mapping[str, Any]]) -> None:
    ...
    s172.write_md(
        ...
        f"""# Stage206 Closeout Packet(206단계 종료 작업 묶음)
...
Stage207(207단계)는 Stage206(206단계) long-session DD micro repair(롱 세션 낙폭 미세 수리) 결과를 follow-up review(후속 검토)한다.

## Bounded Question(경계 질문)

Did Stage206(206단계) reduce validation DD(검증 낙폭) below 34D(34D) without damaging validation net/PF/midPF(검증 순손익/수익요인/중반 수익요인), trade supply(거래 공급), OOS(표본외), and risk/ATR telemetry(위험/ATR 기록)?

Effect(효과): Stage206(206단계) 안에서 추가 수리를 하지 않고, DD micro repair(낙폭 미세 수리) 측정 결과를 별도 판독 단계에서 과장 없이 판단한다.
...
- status(상태): `open_planned_from_stage206`
- source_stage(원천 단계): `{STAGE_ID}`
- source_decision(원천 판정): `{decision}`
...
- stage_status(단계 상태): `open_planned_from_stage206`
...
- >-
  Stage206(206단계) closed(종료) as `{decision}` and Stage207(207단계) `{NEXT_STAGE_ID}` is open_planned(개방 계획). Effect(효과): DD micro repair(낙폭 미세 수리) 결과를 별도 follow-up review(후속 검토)로 넘긴다.
- >-
  Stage206 evidence(206단계 근거)는 `{rel(REPORT_PATH)}`, `{rel(SUMMARY_CSV_PATH)}`, `{rel(QUALITY_MATRIX_PATH)}`, `{rel(BALANCE_CURVE_AUDIT_PATH)}`, `{rel(RISK_ATR_TELEMETRY_PATH)}`에 있다. Effect(효과): session window/risk cap(세션 창/위험 상한)이 DD/PF/OOS(낙폭/수익요인/표본외)에 준 영향을 같이 본다.
- >-
...
"""
    if re.search(r"(?ms)^current_focus:\r?\n.*?(?=\r?\nstage\d+_)", state):
        ...
        state = state.rstrip() + "\n" + focus
    state = re.sub(r"(?ms)^stage206_long_session_dd_micro_repair:\r?\n.*?(?=^stage\d+_|\Z)", "", state)
    block = f"""
stage206_long_session_dd_micro_repair:
  packet_id: {PACKET_ID}
  stage_id: {STAGE_ID}
...
- adapter_under_review(검토 중 어댑터): `stage206_long_session_dd_micro_repair`
- status(상태): `stage206_{decision}`
- claim_boundary(주장 경계): `{BOUNDARY}`

Stage206(206단계)는 Stage205(205단계) 판정에 따라 long-session DD micro repair(롱 세션 낙폭 미세 수리)를 MT5 Strategy Tester(메타트레이더5 전략 테스터)로 측정했다. Effect(효과): Stage207(207단계)가 KPI(핵심 성과 지표) 상충을 별도 review(검토)로 판정한다.

## Latest Stage206 Evidence(최신 206단계 근거)
...
def append_changelog(decision: str) -> None:
    ...
    if RUN_ID in existing:
        return
    entry = (
        f"\n## {s172.utc_now()} Stage206 long-session DD micro repair closeout(206단계 롱 세션 낙폭 미세 수리 종료)\n\n"
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    prepared = {
        ...
        "model_family": "baseline_adapter_stage206_v2_native_long_session_dd_micro_repair",
        "feature_set_id": "stage206_signal_plus_encoded_long_session_dd_gate",
