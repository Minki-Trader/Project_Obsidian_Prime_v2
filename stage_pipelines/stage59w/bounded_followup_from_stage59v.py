...
SOURCE_STAGE59V_ID = "59V_adapter_repair__bounded_followup_from_stage59u"
STAGE59W_ID = "59W_adapter_repair__bounded_followup_from_stage59v"
NEXT_REPAIR_STAGE_ID = "59X_adapter_repair__bounded_followup_from_stage59w"
RUN_NUMBER = "run59R"
RUN_ID = "run59R_stage59w_bounded_followup_from_stage59v_v1"
PACKET_ID = "stage59w_bounded_followup_from_stage59v_v1"
PARENT_RUN_ID = "run59Q_stage59v_bounded_followup_from_stage59u_v1"
SOURCE_ADAPTER_ID = "s59v_s59s_mr025_sl20_tp30_sd12_h5_rearm002"
...
STAGE_ROOT = Path("stages") / STAGE59W_ID
...
SOURCE_STAGE59V_ROOT = Path("stages") / SOURCE_STAGE59V_ID
SOURCE_STAGE59V_DECISION = SOURCE_STAGE59V_ROOT / "03_reviews/stage59v_decision.md"
SOURCE_STAGE59V_REPORT = SOURCE_STAGE59V_ROOT / "03_reviews/bounded_followup_from_stage59u_report.md"
SOURCE_STAGE59V_SUMMARY = SOURCE_STAGE59V_ROOT / "03_reviews/bounded_followup_summary.csv"
SOURCE_STAGE59V_JSON = SOURCE_STAGE59V_ROOT / "03_reviews/bounded_followup_summary.json"
SOURCE_STAGE59V_SEGMENT = SOURCE_STAGE59V_ROOT / "03_reviews/bounded_followup_segment_kpi_summary.csv"
SOURCE_STAGE59V_RISK_ATR = SOURCE_STAGE59V_ROOT / "03_reviews/bounded_followup_risk_atr_telemetry.csv"
...
COMMON_ROOT = f"Project_Obsidian_Prime_v2/s59w/{RUN_NUMBER}"

REPORT_PATH = REVIEWS_ROOT / "bounded_followup_from_stage59v_report.md"
...
DECISION_PATH = REVIEWS_ROOT / "stage59w_decision.md"
AUDIT_CSV_PATH = REVIEWS_ROOT / "stage59w_trade_audit.csv"
...
STAGE59W_VARIANTS = (
    repair.RepairVariant(
        adapter_id="s59w_s59v_st54_mr025_sl20_tp30_sd12_h5_rearm002",
        label="run50BQ_v61_transition_h2_close_only_short54_long54_sl20_tp30_sd12_h5_rearm002_risk2p5pct_atr",
        atr_enabled=True,
        ...
        max_hold_bars=5,
        notes="Stage59W bounded follow-up: keep Stage59V best risk/ATR/lifecycle settings and tighten short_threshold to 0.54.",
    ),
    repair.RepairVariant(
        adapter_id="s59w_s59v_st56_mr025_sl20_tp30_sd12_h5_rearm002",
        label="run50BQ_v61_transition_h2_close_only_short56_long54_sl20_tp30_sd12_h5_rearm002_risk2p5pct_atr",
        atr_enabled=True,
        model_risk_enabled=True,
        fixed_lot=0.25,
        atr_stop_multiplier=2.0,
        ...
        close_only_on_opposite_signal=True,
        max_hold_bars=5,
        notes="Stage59W bounded follow-up: keep Stage59V best risk/ATR/lifecycle settings and tighten short_threshold to 0.56.",
    ),
    repair.RepairVariant(
        adapter_id="s59w_s59v_st58_mr025_sl20_tp30_sd12_h5_rearm002",
        label="run50BQ_v61_transition_h2_close_only_short58_long54_sl20_tp30_sd12_h5_rearm002_risk2p5pct_atr",
        atr_enabled=True,
        model_risk_enabled=True,
        fixed_lot=0.25,
        atr_stop_multiplier=2.0,
        ...
        close_only_on_opposite_signal=True,
        max_hold_bars=5,
        notes="Stage59W bounded follow-up: keep Stage59V best risk/ATR/lifecycle settings and tighten short_threshold to 0.58.",
    ),
)
...
def source_specs() -> dict[str, dict[str, Any]]:
    v61 = {
        ...
        "source_note": "Stage59W source anchor: Stage59V best risk-cap-compressed adapter on Stage56 run50BQ v61 context ExtraTrees firewall transition hold2 source",
    }
    return {variant.adapter_id: dict(v61) for variant in STAGE59W_VARIANTS}

def configure_reused_engine() -> None:
    engine.STAGE59_ID = STAGE59W_ID
    ...
    engine.STAGE59_VARIANTS = STAGE59W_VARIANTS
    engine.SOURCE_SPECS = source_specs()
    engine.MODEL_RISK_MIN_PCT = {variant.adapter_id: 0.005 for variant in STAGE59W_VARIANTS}

    repair.STAGE_ID = STAGE59W_ID
    repair.RUN_NUMBER = RUN_NUMBER
    repair.RUN_ID = RUN_ID
    repair.RUN_ROOT = RUN_ROOT
    repair.REPAIR_VARIANTS = STAGE59W_VARIANTS
    s58.STAGE58_ID = STAGE59W_ID
    ...
    s58.STAGE58_VARIANTS = STAGE59W_VARIANTS
...
def build_attempts(inputs: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    for variant_index, variant in enumerate(STAGE59W_VARIANTS, start=1):
            ...
            for role_index, (tier, attempt_role, prefix, attempt_token) in enumerate(
                ...
                attempts.append(
                    attempt_payload(
                        ...
                        run_id=RUN_ID,
                        stage_number=59,
                        exploration_label="stage59W_BaselineAdapter__ShortThresholdTightenFromStage59V",
...
def execute_prepared_run_checkpointed(prepared: Mapping[str, Any], args: argparse.Namespace) -> dict[str, Any]:
    ...
    return {
        **dict(prepared),
        ...
        "judgment": "inconclusive_stage59w_runtime_probe_completed" if completed and report_completed else "blocked_stage59w_runtime_probe",
    }
...
def next_stage_for_decision(decision: str) -> str:
    ...
    if decision == "demote_current_adapter_and_select_backup":
        return "59X_adapter_repair__backup_or_demote_from_stage59w"
    if decision == "open_bounded_followup":
        return "59X_adapter_repair__bounded_followup_from_stage59w"
...
def next_packet_for_decision(decision: str) -> tuple[str, str]:
    ...
    return "stage59x_bounded_followup_from_stage59w_v1", "run59S_stage59x_bounded_followup_from_stage59w_v1"
            ...
            rows.append(
                {
                    ...
                    "stage_id": STAGE59W_ID,
                    "run_id": RUN_ID,
                    "artifact_type": "stage59w_bounded_followup_evidence",
                    "path": rel(path),
                    "sha256": sha256_file_lf_normalized(path),
                    "created_at_utc": created,
                    "notes": "Stage59W bounded short-threshold tightening follow-up from Stage59V artifact.",
                }
            )
            ...
            rows.append(
                {
                    ...
                    "created_at_utc": created,
                    "notes": "Actual Stage59W MT5 Strategy Tester HTML report.",
                }
            )
    return rows
...
def write_run_identity(result: Mapping[str, Any]) -> None:
    engine.write_json(
        RUN_ROOT / "run_manifest.json",
        {
            ...
            "bounded_question": "Can bounded short-threshold tightening on the Stage59V best adapter reduce validation early negative and OOS mid weak segment behavior while preserving validation/OOS PF, net, drawdown, cost-stressed expectancy, ATR/bracket telemetry, and model-controlled risk behavior without starting ONNX?",
            "source_stage59v_decision": rel(SOURCE_STAGE59V_DECISION),
            ...
            "variants": [
                ...
                for variant in STAGE59W_VARIANTS
            ],
...
def write_ledgers(
        ...
        [
            {
                "run_id": RUN_ID,
                "stage_id": STAGE59W_ID,
                "lane": "baseline_adapter_bounded_followup_from_stage59v",
                "status": status,
                "judgment": decision,
    ...
    ledger_rows.append(
        {
            "ledger_row_id": f"{RUN_ID}__aggregate_bounded_followup_from_stage59v",
            "stage_id": STAGE59W_ID,
            "run_id": RUN_ID,
            "subrun_id": "aggregate_bounded_followup_from_stage59v",
            "parent_run_id": PARENT_RUN_ID,
            "record_view": "bounded_followup_from_stage59v",
            "tier_scope": "Tier A+B",
            ...
            "external_verification_status": external,
            "notes": "Stage59W bounded short-threshold tightening follow-up from Stage59V evidence; not final package completion.",
        }
    )
...
def report_markdown(summary_rows: Sequence[Mapping[str, Any]], decision: str, reasons: Sequence[str], external: str) -> str:
    ...
    best = best_repaired_variant(summary_rows)
    return f"""# Stage59W Short Threshold Tightening From Stage59V Report(59W단계 59V단계 기반 숏 임계값 강화 보고서)

- stage(단계): `{STAGE59W_ID}`
- run(실행): `{RUN_ID}`
...
def equity_audit_markdown(summary_rows: Sequence[Mapping[str, Any]], segment_rows: Sequence[Mapping[str, Any]]) -> str:
    ...
    return f"""# Stage59W Bounded Followup Equity Curve Audit(59W단계 경계 후속 자금 곡선 감사)
...
def decision_markdown(decision: str, reasons: Sequence[str], best: Mapping[str, Any], external: str) -> str:
    return f"""# Stage59W Decision(59W단계 판정)

decision(판정): `{decision}`

Stage59W(59W단계)는 Stage59V evidence(59V단계 근거)를 사용한 bounded short-threshold tightening follow-up(경계 숏 임계값 강화 후속)으로 기록한다. Effect(효과): short_threshold(숏 임계값) 강화의 성공/실패를 다음 bounded stage(경계 다음 단계)의 입력 근거로 넘긴다.
...
Stage59W closeout(59W단계 종료)는 overall goal completion(전체 목표 완료)이 아니다. Effect(효과): Stage60 ONNX hardening(60단계 ONNX 경화)은 adapter quality(어댑터 품질)가 강할 때만 열린다.
    ...
    engine.write_json(
        SUMMARY_JSON_PATH,
        {
            ...
            "required_outputs": {
                ...
                "stage59w_decision": rel(DECISION_PATH),
            },
...
def write_packet_files(
    ...
    files = {
        ...
        "experiment_design_receipt.json": {
            "hypothesis": "A bounded short-threshold tightening on the Stage59V best adapter may reduce validation early negative and OOS mid weak segment behavior while preserving validation/OOS PF, net, drawdown, cost-stressed expectancy, ATR/bracket telemetry, and model-controlled risk behavior.",
            ...
            "comparison_baseline": "Stage59V best candidate s59v_s59s_mr025_sl20_tp30_sd12_h5_rearm002.",
            "control_variables": ["US100", "M5", "split_v1", "Tier B disabled", "run50BQ v61 source anchor", "long threshold 0.54", "same-direction cooldown 12", "ATR SL 2.0", "ATR TP 3.0", "2.5% model-controlled risk cap", "max hold 5", "close-only-on-opposite lifecycle", "entry transition rearm 0.02", "ONNX deferred"],
            "changed_variables": ["short_threshold"],
            ...
            "stop_conditions": "three bounded Stage59W short-threshold variants only",
        ...
        "artifact_lineage_audit.json": {
            "source_inputs": [
                rel(SOURCE_STAGE59V_DECISION),
                rel(SOURCE_STAGE59V_REPORT),
                rel(SOURCE_STAGE59V_SUMMARY),
                rel(SOURCE_STAGE59V_JSON),
                rel(SOURCE_STAGE59V_SEGMENT),
                rel(SOURCE_STAGE59V_RISK_ATR),
            ],
...
- stage_id(단계 ID): `{STAGE59W_ID}`
- source_stage(원천 단계): `{SOURCE_STAGE59V_ID}`
- source_decision(원천 판정): `continue_repair_in_new_bounded_stage`
- bounded_question(경계 질문): `Can bounded short-threshold tightening on the Stage59V best adapter reduce validation early negative and OOS mid weak segment behavior while preserving validation/OOS PF, net, drawdown, cost-stressed expectancy, ATR/bracket telemetry, and model-controlled risk behavior without starting ONNX?`
- boundary(경계): `{BOUNDARY}`

Stage59W(59W단계)는 Stage59V(59V단계) best adapter(최선 어댑터)의 risk cap(위험 상한), ATR bracket(ATR 브래킷), long_threshold(롱 임계값), cooldown(쿨다운), max_hold_bars(최대 보유 봉수), close-only lifecycle(청산 전용 생명주기)를 유지하고 short_threshold(숏 임계값) 세 변형만 측정한다. Effect(효과): Stage60 ONNX(60단계 ONNX)나 deployment(배포)로 이어가지 않고 숏 임계값 강화가 구간 약점을 줄이는지만 본다.
""",
    )
    engine.write_md(
...
- stage59v_decision(59V단계 판정): `{rel(SOURCE_STAGE59V_DECISION)}`
- stage59v_report(59V단계 보고서): `{rel(SOURCE_STAGE59V_REPORT)}`
- stage59v_summary(59V단계 요약): `{rel(SOURCE_STAGE59V_SUMMARY)}`
- stage59v_segment_kpi(59V단계 구간 KPI): `{rel(SOURCE_STAGE59V_SEGMENT)}`
- stage59v_risk_atr_telemetry(59V단계 위험/ATR 텔레메트리): `{rel(SOURCE_STAGE59V_RISK_ATR)}`
...
- stage59w_changed_variable(59W단계 변경 변수): `short_threshold_only`

Effect(효과): Stage59W(59W단계)는 Stage59V(59V단계) 구간 약점과 run50BQ(run50BQ) v61 원천을 고정한 short_threshold(숏 임계값) 변형을 명시적으로 연결한다.
""",
    )
    engine.write_md(
...
- stage_status(단계 상태): `closed_bounded_followup_from_stage59v`
- source_stage(원천 단계): `{SOURCE_STAGE59V_ID}`
- source_decision(원천 판정): `continue_repair_in_new_bounded_stage`
- stage59w_decision(59W단계 판정): `{decision}`
- next_stage_or_branch(다음 단계/분기): `{next_stage}`
- selected_research_baseline(선택 연구 기준선): `none`
- claim_boundary(주장 경계): `{BOUNDARY}`

Effect(효과): Stage59W(59W단계)는 측정 결과를 보존하지만 final package(최종 패키지)나 operating claim(운영 주장)을 만들지 않는다.
""",
    )
    engine.write_md(
        REVIEWS_ROOT / "review_index.md",
        f"""# Stage59W Review Index(59W단계 검토 색인)
...
- bounded_followup_risk_atr_telemetry(경계 후속 위험/ATR 텔레메트리): `{rel(RISK_ATR_TELEMETRY_PATH)}`
- stage59w_decision(59W단계 판정): `{rel(DECISION_PATH)}`
""",
    )
...
- bounded_question(경계 질문): `What bounded follow-up should repair, demote, or branch from the Stage59W evidence without starting ONNX prematurely?`
- boundary(경계): `{BOUNDARY}`

Stage59X(59X단계)는 Stage59W(59W단계) 결과가 Stage60 ONNX(60단계 ONNX)에 충분하지 않을 때 이어지는 bounded follow-up(경계 후속)이다. Effect(효과): repair(수리), demotion(강등), branch(분기)를 다음 작은 질문으로 제한한다.
""",
        )
        engine.write_md(
...
- stage59w_decision(59W단계 판정): `{rel(DECISION_PATH)}`
- stage59w_report(59W단계 보고서): `{rel(REPORT_PATH)}`
- stage59w_summary(59W단계 요약): `{rel(SUMMARY_CSV_PATH)}`
- stage59w_segment_kpi(59W단계 구간 KPI): `{rel(SEGMENT_KPI_PATH)}`
- stage59w_risk_atr_telemetry(59W단계 위험/ATR 텔레메트리): `{rel(RISK_ATR_TELEMETRY_PATH)}`
""",
        )
        engine.write_md(
            next_root / "03_reviews/review_index.md",
            """# Stage59X Review Index(59X단계 검토 색인)

Stage59X(59X단계)는 planned(계획) 상태다. Effect(효과): Stage59W(59W단계) 약점을 다음 bounded repair(경계 수리) 근거로 넘긴다.
""",
        )
        engine.write_md(
...
- stage_status(단계 상태): `active_planned_from_stage59w`
- source_stage(원천 단계): `{STAGE59W_ID}`
- source_decision(원천 판정): `{decision}`
- selected_research_baseline(선택 연구 기준선): `none`
- claim_boundary(주장 경계): `{BOUNDARY}`

Effect(효과): Stage59X(59X단계)는 Stage59W(59W단계) 이후 필요한 다음 작은 질문만 다룬다.
""",
        )
...
- status(상태): `stage59w_closed_{decision}`
- claim_boundary(주장 경계): research/development only(연구/개발 전용)

Stage59W(59W단계) closed(종료) as bounded short-threshold tightening follow-up from Stage59V(59V단계 기반 경계 숏 임계값 강화 후속). Effect(효과): Stage59V(59V단계) segment weakness(구간 약점)는 보존하고, short_threshold(숏 임계값) 강화 결과를 다음 판단에 넘긴다.

## Latest Stage59W Evidence(최신 59W단계 근거)
...
""",
    ...
    focus = (
        "current_focus:\n"
        f"- >-\n"
        f"  Stage59W(59W단계) `{STAGE59W_ID}` closed(종료) as bounded short-threshold tightening follow-up from Stage59V(59V단계 기반 경계 숏 임계값 강화 후속); decision(판정)=`{decision}`. "
        ...
        f"- >-\n"
        f"  Next stage_or_branch(다음 단계/분기) `{next_stage}` is active/planned(활성/계획). Effect(효과): Stage59W(59W단계) 결과를 다음 bounded step(경계 다음 단계)으로 넘긴다.\n"
    )
    text = re.sub(
...
stage59w_bounded_followup_from_stage59v:
  packet_id: {PACKET_ID}
  stage_id: {STAGE59W_ID}
  status: closed_bounded_followup_from_stage59v
  ...
  boundary: {BOUNDARY}
"""
    if "stage59w_bounded_followup_from_stage59v:" in text:
        text = re.sub(r"\nstage59w_bounded_followup_from_stage59v:\n(?:  .*\n)*", block, text, count=1)
    else:
        text += block
...
def parse_args(argv: Sequence[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run Stage59W bounded follow-up from Stage59V.")
...
def load_existing_result() -> dict[str, Any]:
    ...
    if not path_exists(manifest) or not path_exists(kpi):
        raise FileNotFoundError("Stage59W existing run_manifest.json or kpi_record.json is missing")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.finalize_existing:
        ...
        prepared = {
            ...
            "model_family": "baseline_adapter_stage59w_short_threshold_tightening_ebm_table",
            "feature_set_id": "stage59w_run50bq_v61_short_threshold_signal",
