...
STAGE_ID = "77_adapter_research__v41_entry_quality_dd_guard"
RUN_NUMBER = "run77A"
RUN_ID = "run77A_stage77_v41_entry_quality_dd_guard_v1"
PACKET_ID = "stage77_v41_entry_quality_dd_guard_v1"
PARENT_RUN_ID = "run76A_stage76_v41_dd_balance_followup_review_v1"
SOURCE_STAGE76_ID = "76_adapter_research__v41_dd_balance_followup_review"
SOURCE_STAGE76_COMMIT = "cb58585f64cb944570215cb2fb51f98be7c458fc"
NEXT_STAGE_ID = "78_adapter_research__v41_entry_quality_followup_review"
NEXT_RUN_ID = "run78A_stage78_v41_entry_quality_followup_review_v1"
NEXT_PACKET_ID = "stage78_v41_entry_quality_followup_review_v1"
...
COMMON_ROOT = f"OPV2/s77a/{RUN_NUMBER}"

SUMMARY_JSON_PATH = REVIEWS_ROOT / "stage77_v41_entry_quality_dd_guard_summary.json"
SUMMARY_CSV_PATH = REVIEWS_ROOT / "stage77_v41_entry_quality_dd_guard_summary.csv"
REPORT_PATH = REVIEWS_ROOT / "stage77_v41_entry_quality_dd_guard_report.md"
SEGMENT_KPI_PATH = REVIEWS_ROOT / "stage77_segment_kpi_summary.csv"
RISK_ATR_TELEMETRY_PATH = REVIEWS_ROOT / "stage77_risk_atr_telemetry.csv"
GATE_FEATURE_SUMMARY_PATH = REVIEWS_ROOT / "stage77_gate_feature_summary.csv"
TIER_B_DIAGNOSTIC_PATH = REVIEWS_ROOT / "stage77_tier_b_diagnostic_summary.csv"
DECISION_PATH = REVIEWS_ROOT / "stage77_decision.md"
AUDIT_CSV_PATH = REVIEWS_ROOT / "stage77_trade_audit.csv"
...
VARIANTS = (
    repair.RepairVariant(
        adapter_id="s77_v41_h3_risk5_gate10_tp35",
        label="run50BN_v41_h3_risk5_gate10_tp35",
        atr_enabled=True,
        ...
        max_hold_bars=3,
        notes="Stage77 entry quality/DD guard: Stage73 risk5 TP3.5 with stricter 0.10 low-margin short block.",
    ),
    repair.RepairVariant(
        adapter_id="s77_v41_h3_risk5_gate12_tp35",
        label="run50BN_v41_h3_risk5_gate12_tp35",
        atr_enabled=True,
        model_risk_enabled=True,
        fixed_lot=0.25,
        atr_stop_multiplier=2.5,
        ...
        close_only_on_opposite_signal=True,
        max_hold_bars=3,
        notes="Stage77 entry quality/DD guard: Stage73 risk5 TP3.5 with stricter 0.12 low-margin short block.",
    ),
    repair.RepairVariant(
        adapter_id="s77_v41_h3_risk5_gate10_tp40",
        label="run50BN_v41_h3_risk5_gate10_tp40",
        atr_enabled=True,
        model_risk_enabled=True,
        fixed_lot=0.25,
        atr_stop_multiplier=2.5,
        ...
        close_only_on_opposite_signal=True,
        max_hold_bars=3,
        notes="Stage77 entry quality/DD guard: Stage73 risk5 TP4.0 with stricter 0.10 low-margin short block.",
    ),
)
...
CONTEXT_GATE_SPECS = {
    "s77_v41_h3_risk5_gate10_tp35": {
        "gate_column": "stage77_gate_margin_lt_010_short",
        "gate_type": "margin",
        "threshold": 0.10,
        "block_mode": "short",
        "description": "v41 source; block shorts if et40_decision_margin < 0.10; risk cap 5%, TP 3.5, hold 3",
    },
    "s77_v41_h3_risk5_gate12_tp35": {
        "gate_column": "stage77_gate_margin_lt_012_short",
        "gate_type": "margin",
        "threshold": 0.12,
        "block_mode": "short",
        "description": "v41 source; block shorts if et40_decision_margin < 0.12; risk cap 5%, TP 3.5, hold 3",
    },
    "s77_v41_h3_risk5_gate10_tp40": {
        "gate_column": "stage77_gate_margin_lt_010_short",
        "gate_type": "margin",
        "threshold": 0.10,
        "block_mode": "short",
        "description": "v41 source; block shorts if et40_decision_margin < 0.10; risk cap 5%, TP 4.0, hold 3",
    },
}
...
def decide(summary_rows: Sequence[Mapping[str, Any]], segment_rows: Sequence[Mapping[str, Any]], external: str) -> str:
    if external != "completed":
        return "continue_stage77_due_to_incomplete_runtime"
    ...
    if val_net >= 900 and oos_net >= 520 and val_dd <= 18.0 and oos_dd <= 19.0 and val_pf >= 1.50 and oos_pf >= 1.30 and not reasons:
        return "proceed_to_stage78_candidate_review"
    if val_net >= 650 and oos_net >= 420 and val_dd <= 24.0 and oos_dd <= 24.0 and val_pf >= 1.35 and oos_pf >= 1.25:
        return "continue_entry_quality_dd_guard_in_stage78"
    if val_pf < 1.0 or val_net <= 0:
        return "open_new_model_source_branch_in_stage78"
    return "continue_entry_quality_dd_guard_in_stage78"
...
def report_markdown(
    ...
    return f"""# Stage77 V41 Entry Quality/DD Guard Report(77단계 V41 진입 품질/손실률 보호 보고서)

- run(실행): `{RUN_ID}`
- target_surface(목표 표면): `{TARGET_SURFACE}`
- source_stage(원천 단계): `{SOURCE_STAGE76_ID}`
- source_stage76_commit(원천 76단계 커밋): `{SOURCE_STAGE76_COMMIT}`
...
def decision_markdown(decision: str, external: str) -> str:
    return f"""# Stage77 Decision(77단계 판정)
            ...
            rows.append(
                {
                    ...
                    "split": split_name,
                    "tier_b_policy": "diagnostic_missing_required_but_disabled_for_this_v41_entry_quality_dd_guard",
                    ...
                    "reason": "Stage77 isolates v41 entry quality/DD guard first; Tier B fallback remains diagnostic and disabled.",
                }
            )
    return rows
            ...
            rows.append(
                {
                    ...
                    "artifact_type": "stage77_v41_entry_quality_dd_guard_evidence",
                    "path": rel(path),
                    "sha256": sha256_file_lf_normalized(path),
                    "stage_id": STAGE_ID,
                    "run_id": RUN_ID,
                    "created_at_utc": created,
                    "notes": "Stage77 v2-native v41 entry quality/DD guard artifact.",
                }
            )
            ...
            rows.append(
                {
                    ...
                    "run_id": RUN_ID,
                    "created_at_utc": created,
                    "notes": "Actual Stage77 MT5 Strategy Tester HTML report.",
                }
            )
    return rows
...
def write_run_identity(result: Mapping[str, Any]) -> None:
    write_json(
        RUN_ROOT / "run_manifest.json",
        {
            ...
            "source_stage76_pushed_commit": SOURCE_STAGE76_COMMIT,
        ...
        [
            {
                "run_id": RUN_ID,
                "stage_id": STAGE_ID,
                "lane": "baseline_adapter_v2_native_v41_entry_quality_dd_guard",
                "status": status,
                "judgment": decision,
                ...
                "notes": ledger_pairs(
                    (
                        ...
                        ("source_stage76_commit", SOURCE_STAGE76_COMMIT),
                        ("target_surface", TARGET_SURFACE),
        ...
        alpha_rows = [
            {
                ...
                "tier_scope": "Tier A+B",
                "kpi_scope": "stage77_v41_entry_quality_dd_guard",
                ...
                "external_verification_status": external,
                "notes": "Stage77 run materialized or blocked before KPI records were available.",
            }
        ]
...
def write_packet_files(result: Mapping[str, Any], decision: str, ledger_payload: Mapping[str, Any]) -> None:
    ...
    write_json(PACKET_ROOT / "aggregate_summary.json", {"packet_id": PACKET_ID, "stage_id": STAGE_ID, "run_id": RUN_ID, "decision": decision, "gate_feature_summary_path": rel(GATE_FEATURE_SUMMARY_PATH), "ledger_payload": ledger_payload, "overall_goal_complete": False})
...
Stage78(78단계)는 Stage77(77단계)의 v41 entry quality/DD guard(V41 진입 품질/손실률 보호) 결과를 받아 candidate review(후보 검토), continued repair(계속 수리), 또는 another model source(다른 모델 원천)를 고르는 follow-up(후속) 단계다.

## Bounded Question(경계 질문)

Stage77(77단계)의 stricter low-margin short gate(더 엄격한 낮은 마진 숏 게이트)가 34D KPI(34D 핵심 성과 지표) 격차를 줄일 만큼 강한가?

Effect(효과): Stage78(78단계)는 다음 판단을 하나의 경계 질문으로 좁힌다.
...
""",
    ...
    write_md(
        ...
        f"""# Stage78 Input References(78단계 입력 참조)

- source_stage(원천 단계): `{STAGE_ID}`
- source_run(원천 실행): `{RUN_ID}`
- source_decision(원천 판정): `{decision}`
- source_external_verification_status(원천 외부 검증 상태): `{external}`
- stage77_report(77단계 보고서): `{rel(REPORT_PATH)}`
- stage77_decision(77단계 판정): `{rel(DECISION_PATH)}`
- summary(요약): `{rel(SUMMARY_CSV_PATH)}`
- target_surface(목표 표면): `{TARGET_SURFACE}`

Effect(효과): Stage78(78단계)는 v2 고유 근거만 이어받아 34D KPI(34D 핵심 성과 지표) 격차 축소를 계속한다.
""",
    )
    write_md(NEXT_STAGE_ROOT / "03_reviews" / "review_index.md", f"""# Stage78 Review Index(78단계 검토 색인)
...
Effect(효과): Stage78(78단계)는 Stage77(77단계) closeout(종료 기록)을 이어받아 다음 bounded batch(경계 묶음 실행)만 검토한다.
""")
    write_md(NEXT_STAGE_ROOT / "04_selected" / "selection_status.md", f"""# Stage78 Selection Status(78단계 선택 상태)
...
Effect(효과): Stage78(78단계)는 34D KPI(34D 핵심 성과 지표) 격차 축소를 계속하지만, 운영 의미 없이 연구개발로만 이어진다.
""")
...
- >-
  Stage77(77단계) closed(종료) as `{decision}` and Stage78(78단계) `{NEXT_STAGE_ID}` is open_planned(열림 계획). Effect(효과): v41 entry quality/DD guard(V41 진입 품질/손실률 보호) 근거를 보존하고 다음 경계 연구 질문으로만 넘긴다.
- >-
  Stage77 result(77단계 결과): validation/OOS(검증/표본외), PF/net/DD(수익 팩터/순손익/손실률), risk/ATR telemetry(위험/ATR 텔레메트리)는 `{rel(SUMMARY_CSV_PATH)}`와 `{rel(RISK_ATR_TELEMETRY_PATH)}`에 기록된다. Effect(효과): 34D target surface(34D 목표 표면) 대비 KPI(핵심 성과 지표) 차이를 다음 단계 입력으로 보존한다.
- >-
...
stage77_v41_entry_quality_dd_guard:
  packet_id: {PACKET_ID}
  stage_id: {STAGE_ID}
  ...
  boundary: {BOUNDARY}
"""
    if "stage77_v41_entry_quality_dd_guard:" in text:
        text = re.sub(r"\nstage77_v41_entry_quality_dd_guard:\n(?:  .*\n)+", block + "\n", text, count=1)
    else:
        text = text.rstrip() + block
    io_path(WORKSPACE_STATE_PATH).write_text(text + ("\n" if not text.endswith("\n") else ""), encoding="utf-8-sig")
    write_md(SELECTED_ROOT / "selection_status.md", f"""# Stage77 Selection Status(77단계 선택 상태)

- stage_status(단계 상태): `closed_{decision}`
- source_stage(원천 단계): `{SOURCE_STAGE76_ID}`
- source_decision(원천 판정): `continue_entry_quality_dd_guard_in_stage77`
- source_stage76_pushed_commit(원천 76단계 푸시 커밋): `{SOURCE_STAGE76_COMMIT}`
- current_run(현재 실행): `{RUN_ID}`
- adapter_under_review(검토 중 어댑터): `{SOURCE_ADAPTER_ID}`
- target_surface(목표 표면): `{TARGET_SURFACE}`
- stage77_decision(77단계 판정): `{decision}`
- next_stage_or_branch(다음 단계/분기): `{NEXT_STAGE_ID}`
- claim_boundary(주장 경계): `{BOUNDARY}`

Effect(효과): Stage77(77단계)는 v41 entry quality/DD guard(V41 진입 품질/손실률 보호)를 측정하고, 운영 의미 없이 Stage78(78단계)로 넘긴다.
""")
...
- target_surface(목표 표면): `{TARGET_SURFACE}`
- adapter_under_review(검토 중 어댑터): `{SOURCE_ADAPTER_ID}`
- status(상태): `stage77_closed_{decision}`
- claim_boundary(주장 경계): `{BOUNDARY}`

Stage77(77단계) closed(종료) as v2-native v41 entry quality/DD guard batch(브이투 고유 브이41 진입 품질/손실률 보호 묶음). Effect(효과): legacy 34D(레거시 34D)는 target surface(목표 표면)로만 쓰였고, 다음 연구는 Stage78(78단계)로 이어진다.

## Latest Stage77 Evidence(최신 77단계 근거)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    prepared = {
        ...
        "model_family": "baseline_adapter_stage77_v2_native_v41_entry_quality_dd_guard_ebm_table",
        "feature_set_id": "stage77_v41_entry_quality_dd_guard_signal",
