...
RUN_NUMBER = "run50BO"
PARENT_RUN_ID = "run50BO_stage56_context_extratrees_same_direction_cooldown_v1"
PACKET_ID = "stage56_run50BO_context_extratrees_same_direction_cooldown_v1"
EXPLORATION_LABEL = "stage56_BaseEngine__ContextExtraTreesSameDirectionCooldown"
REPORT_PATH = ctx.REVIEWS_ROOT / "run50BO_context_extratrees_same_direction_cooldown.md"
RESULTS_CSV_PATH = ctx.REVIEWS_ROOT / "run50BO_summary.csv"
AUDIT_CSV_PATH = ctx.REVIEWS_ROOT / "run50BO_audit.csv"
SOURCE_SUMMARY_CSV_PATH = ctx.REVIEWS_ROOT / "run50BO_source_summary.csv"
PACKET_ROOT = Path("docs/agent_control/packets") / PACKET_ID
AGGREGATE_SUMMARY_PATH = PACKET_ROOT / "aggregate_summary.json"
RUN_ROOT = ctx.STAGE_ROOT / "02_runs" / RUN_NUMBER
COMMON_ROOT = f"Project_Obsidian_Prime_v2/stage56/{RUN_NUMBER}_context_extratrees_same_direction_cooldown"
WORKSPACE_BLOCK_KEY = "stage56_run50bo_context_extratrees_same_direction_cooldown:"
RUN50BN_SUMMARY = ctx.REVIEWS_ROOT / "run50BN_summary.csv"
RUN50BN_AUDIT = ctx.REVIEWS_ROOT / "run50BN_audit.csv"


DEFAULT_VARIANTS = (
    ctx.ContextTimedVariant(
        "v48_midcov_slotfill_sd2_h2c0_no_b",
        "midcov_slotfill_same_direction_cooldown2",
        40,
        bn.be.bd.W40_ESOL_MIDCOV_RULES,
        2,
        0,
        False,
        "Run50BN v46 source with same-direction cooldown2 in MT5 execution.",
        "context_plus_et40_slot_fill",
    ),
    ctx.ContextTimedVariant(
        "v49_midcov_slotfill_sd3_h2c0_no_b",
        "midcov_slotfill_same_direction_cooldown3",
        40,
        bn.be.bd.W40_ESOL_MIDCOV_RULES,
        2,
        0,
        False,
        "Run50BN v46 source with same-direction cooldown3 in MT5 execution.",
        "context_plus_et40_slot_fill",
    ),
    ctx.ContextTimedVariant(
        "v50_topup_slotfill_sd2_h2c0_no_b",
        "topup_slotfill_same_direction_cooldown2",
        40,
        bn.be.V22_SLOT3_5_8_TOPUP,
        2,
        0,
        False,
        "Run50BN v47 source with same-direction cooldown2 in MT5 execution.",
        "context_plus_et40_slot_fill",
    ),
    ctx.ContextTimedVariant(
        "v51_topup_slotfill_sd3_h2c0_no_b",
        "topup_slotfill_same_direction_cooldown3",
        40,
        bn.be.V22_SLOT3_5_8_TOPUP,
        2,
        0,
        False,
        "Run50BN v47 source with same-direction cooldown3 in MT5 execution.",
        "context_plus_et40_slot_fill",
    ),
    ctx.ContextTimedVariant(
        "v52_topup_slotfill_sd4_h2c0_no_b",
        "topup_slotfill_same_direction_cooldown4",
        40,
        bn.be.V22_SLOT3_5_8_TOPUP,
        2,
        0,
        False,
        "Run50BN v47 source with same-direction cooldown4 in MT5 execution.",
        "context_plus_et40_slot_fill",
    ),
    ctx.ContextTimedVariant(
        "v53_topup_slotfill_sd2_h3c0_no_b",
        "topup_slotfill_hold3_same_direction_cooldown2",
        40,
        bn.be.V22_SLOT3_5_8_TOPUP,
        3,
        0,
        False,
        "Run50BN v47 source with hold3 and same-direction cooldown2 in MT5 execution.",
        "context_plus_et40_slot_fill",
    ),
)

SAME_DIRECTION_COOLDOWN_BY_VARIANT = {
    "v48_midcov_slotfill_sd2_h2c0_no_b": 2,
    "v49_midcov_slotfill_sd3_h2c0_no_b": 3,
    "v50_topup_slotfill_sd2_h2c0_no_b": 2,
    "v51_topup_slotfill_sd3_h2c0_no_b": 3,
    "v52_topup_slotfill_sd4_h2c0_no_b": 4,
    "v53_topup_slotfill_sd2_h3c0_no_b": 2,
}


def same_direction_cooldown(variant: ctx.ContextTimedVariant) -> int:
    return int(SAME_DIRECTION_COOLDOWN_BY_VARIANT.get(variant.variant_id, 0))
...
def build_variant_frames(
    ...
    frames, summary_rows, lineage = bn.build_variant_frames(variants, common)
    for row in summary_rows:
        row["same_direction_reentry_cooldown_bars"] = SAME_DIRECTION_COOLDOWN_BY_VARIANT.get(str(row.get("variant_id")), 0)
    for role, path, affects in (
        ("run50BN_context_extratrees_summary", RUN50BN_SUMMARY, "pre-cooldown slot-fill frontier comparison"),
        ("run50BN_context_extratrees_audit", RUN50BN_AUDIT, "pre-cooldown same-move and cost-stress audit"),
    ):
        lineage.append(
            {
                "role": role,
...
def build_attempts(
    ...
    feature_hash = ctx.ordered_hash((ctx.SIGNAL_COLUMN,))
    extra_set_values = {
    ...
    for variant_index, variant in enumerate(variants, 1):
        ...
        variant_extra = {
            **extra_set_values,
            ...
            "InpSameDirectionReentryCooldownBars": same_direction_cooldown(variant),
        }
        for source_split, runtime_split in (("validation", "validation_is"), ("oos", "oos")):
            ...
            base_kwargs = {
                ...
                "model_id": f"{PARENT_RUN_ID}_{variant.variant_id}_context_et_sd_signal_table",
            ...
            for role, role_token, tier, feature_path, primary_tier, record_prefix, fallback in (
                ...
                payload = ctx.attempt_payload(
                    **base_kwargs,
                    ...
                    fallback_model_id=f"{PARENT_RUN_ID}_{variant.variant_id}_tier_b_context_et_sd_signal_table" if fallback else None,
                ...
                payload["same_direction_reentry_cooldown_bars"] = same_direction_cooldown(variant)
...
def write_report(rows: Sequence[Mapping[str, Any]], audit_rows: Sequence[Mapping[str, Any]], result: Mapping[str, Any]) -> None:
    ...
    for row in rows:
        sd = SAME_DIRECTION_COOLDOWN_BY_VARIANT.get(str(row.get("variant_id")), "")
        lines.append(
            "| {variant} | {sd} | {vday} | {oday} | {vpf} | {opf} | {vnet} | {onet} | {samev}/{sameo} | {coolv}/{coolo} | {fail} |".format(
...
## {ctx.utc_now()} run50BO Context ExtraTrees Same-Direction Cooldown(문맥 ExtraTrees 동일 방향 쿨다운)
...
def update_workspace_state(best: Mapping[str, Any]) -> None:
    ...
    focus = (
        f"- >-\n"
        f"  Stage56(56단계) `{ctx.STAGE_ID}`: run50BO(실행50BO) context ExtraTrees same-direction cooldown(문맥 ExtraTrees 동일 방향 쿨다운) 완료; "
    ...
    block = (
        ...
        "  next_action: evaluate_same_direction_cooldown_failure_or_open_next_model_branch\n"
    )
...
- run50BO judgment(실행50BO 판정): selected_research_baseline(선택 연구 기준선)은 `none`이다. Effect(효과): failure_reasons(실패 이유) `{failures}` 때문에 hard condition(강한 완료 조건)을 통과하지 못했다.
