...
RUN_NUMBER = "run50BE"
PARENT_RUN_ID = "run50BE_stage56_context_timed_v22_density_topup_v1"
PACKET_ID = "stage56_run50BE_context_timed_v22_density_topup_v1"
EXPLORATION_LABEL = "stage56_BaseEngine__ContextTimedV22DensityTopup"
REPORT_PATH = ctx.REVIEWS_ROOT / "run50BE_context_timed_v22_density_topup.md"
RESULTS_CSV_PATH = ctx.REVIEWS_ROOT / "run50BE_summary.csv"
AUDIT_CSV_PATH = ctx.REVIEWS_ROOT / "run50BE_audit.csv"
SOURCE_SUMMARY_CSV_PATH = ctx.REVIEWS_ROOT / "run50BE_source_summary.csv"
PACKET_ROOT = Path("docs/agent_control/packets") / PACKET_ID
AGGREGATE_SUMMARY_PATH = PACKET_ROOT / "aggregate_summary.json"
RUN_ROOT = ctx.STAGE_ROOT / "02_runs" / RUN_NUMBER
COMMON_ROOT = f"Project_Obsidian_Prime_v2/stage56/{RUN_NUMBER}_context_timed_v22_density_topup"
WORKSPACE_BLOCK_KEY = "stage56_run50be_context_timed_v22_density_topup:"


def _replace_rules(replacements: Mapping[int, ctx.SlotRule]) -> tuple[ctx.SlotRule, ...]:
    return tuple(replacements.get(rule.slot_index, rule) for rule in bd.W40_ESOL_MIDCOV_RULES)


HIGHCOV_BY_SLOT = {rule.slot_index: rule for rule in bd.W40_ESOL_HIGHCOV_RULES}
V22_SLOT8_TOPUP = _replace_rules({8: HIGHCOV_BY_SLOT[8]})
V22_SLOT3_8_TOPUP = _replace_rules({3: HIGHCOV_BY_SLOT[3], 8: HIGHCOV_BY_SLOT[8]})
V22_SLOT5_8_TOPUP = _replace_rules({5: HIGHCOV_BY_SLOT[5], 8: HIGHCOV_BY_SLOT[8]})
V22_SLOT3_5_8_TOPUP = _replace_rules({3: HIGHCOV_BY_SLOT[3], 5: HIGHCOV_BY_SLOT[5], 8: HIGHCOV_BY_SLOT[8]})


DEFAULT_VARIANTS = (
    ctx.ContextTimedVariant(
        "v26_v22_slot8_relax_h2c0_no_b",
        "v22_density_topup_slot8",
        40,
        V22_SLOT8_TOPUP,
        2,
        0,
        False,
        "Run50BD v22 with only slot 8 relaxed toward v21 high-coverage rule; Tier B disabled.",
        "v22_slot8_relax_no_runtime_cooldown",
    ),
    ctx.ContextTimedVariant(
        "v27_v22_slot3_8_relax_h2c0_no_b",
        "v22_density_topup_slot3_8",
        40,
        V22_SLOT3_8_TOPUP,
        2,
        0,
        False,
        "Run50BD v22 with slots 3 and 8 relaxed toward v21 high-coverage rules; Tier B disabled.",
        "v22_slot3_8_relax_no_runtime_cooldown",
    ),
    ctx.ContextTimedVariant(
        "v28_v22_slot5_8_relax_h2c0_no_b",
        "v22_density_topup_slot5_8",
        40,
        V22_SLOT5_8_TOPUP,
        2,
        0,
        False,
        "Run50BD v22 with slots 5 and 8 relaxed toward v21 high-coverage rules; Tier B disabled.",
        "v22_slot5_8_relax_no_runtime_cooldown",
    ),
    ctx.ContextTimedVariant(
        "v29_v22_slot3_5_8_relax_h2c0_no_b",
        "v22_density_topup_slot3_5_8",
        40,
        V22_SLOT3_5_8_TOPUP,
        2,
        0,
        False,
        "Run50BD v22 with slots 3, 5, and 8 relaxed toward v21 high-coverage rules; Tier B disabled.",
        "v22_slot3_5_8_relax_no_runtime_cooldown",
    ),
    ctx.ContextTimedVariant(
        "v30_v22_midcov_h2c0_with_b",
        "v22_tier_b_damage_probe",
        40,
        bd.W40_ESOL_MIDCOV_RULES,
        2,
        0,
        True,
        "Run50BD v22 unchanged with Tier B fallback enabled to test whether fallback can top up density without hidden OOS damage.",
        "v22_midcov_no_runtime_cooldown_with_b",
    ),
)
...
## {ctx.utc_now()} run50BE V22 Density Top-Up(밀도 보강)
...
def update_workspace_state(best: Mapping[str, Any]) -> None:
    ...
    text = "\n".join(
        ...
        if not (line.startswith("- Stage56") and "run50BE(" in line)
    ) + "\n"
    focus = (
        f"- Stage56(56단계) `{ctx.STAGE_ID}`: run50BE(실행50BE) v22 density top-up(밀도 보강) 완료; "
    ...
    block = (
        ...
        "  next_action: decide_v22_density_repair_or_open_new_model_branch\n"
    )
...
Stage56(56단계)는 unfinished optimization campaign(미완성 최적화 캠페인)으로 계속 열린다. Effect(효과): run50BE(실행50BE)는 v22 density top-up(밀도 보강)이 OOS-positive under-dense(표본외 양수이나 밀도 부족) 후보를 유용한 후보로 끌어올릴 수 있는지 확인한 중간 근거다.
...
- run50BE judgment(실행50BE 판정): selected_research_baseline(선택 연구 기준선)은 `none`이다. Effect(효과): failure_reasons(실패 사유) `{failures}` 때문에 hard condition(강한 완료 조건)을 통과하지 못했다.
- next_hypothesis_branch(다음 가설 분기): `decide_v22_density_repair_or_open_new_model_branch`
