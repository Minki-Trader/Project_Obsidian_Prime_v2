...
from stage_pipelines.stage25 import hazard_trade_lifecycle_scout as scout
...
PACKET_ID = "stage25_run19B_hazard_trade_lifecycle_runtime_probe_v1"
EXPLORATION_LABEL = "stage25_Exit__HazardTradeLifecycleRuntimeProbe"
MODEL_FAMILY = "sklearn_discrete_time_logistic_hazard_permission_table_runtime_probe"
MODEL_BACKEND = "ebm_table"
FEATURE_SET_ID = "feature_set_v2_hazard_permission_runtime_features"
LABEL_ID = scout.LABEL_ID
SPLIT_CONTRACT = scout.SPLIT_CONTRACT
RUNTIME_FEATURE_ORDER = ("direction_proxy", "hazard_risk_z")
RUNTIME_FEATURE_HASH = ordered_hash(RUNTIME_FEATURE_ORDER)
THRESHOLD_QUANTILE = 0.80
MAX_HOLD_BARS = scout.MAX_HORIZON_BARS
HAZARD_SENTINEL_ELAPSED_BAR = 6
MIN_MARGIN = 0.0
BOUNDARY = "hazard_permission_runtime_probe_only_not_edge_not_alpha_quality_not_baseline_not_promotion_not_runtime_authority"
JUDGMENT_COMPLETED = "inconclusive_hazard_permission_runtime_probe_completed"
JUDGMENT_BLOCKED = "blocked_hazard_permission_runtime_probe_after_attempt"
...
REVIEW_PATH = STAGE_ROOT / "03_reviews/run19B_hazard_trade_lifecycle_runtime_probe_packet.md"
DECISION_PATH = ROOT / "docs/decisions/2026-05-05_stage25_run19B_hazard_runtime_probe.md"
...
def load_source_summary() -> dict[str, Any]:
    ...
    if str(summary.get("selected_variant_id")) != "v04_logit_core24_reversal_after_favorable_1x":
        raise RuntimeError(f"Unexpected Stage25 selected variant: {summary.get('selected_variant_id')}")
    return summary
...
def load_model_payloads(source_summary: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if not io_path(tier_a_path).exists() or not io_path(tier_b_path).exists():
        raise FileNotFoundError(f"Missing Stage25 run19A model artifacts: {tier_a_path}, {tier_b_path}")
    return {
...
def raw_hazard_risk(model_payload: Mapping[str, Any], frame: pd.DataFrame) -> np.ndarray:
...
def attach_runtime_features(frame: pd.DataFrame, model_payload: Mapping[str, Any], scaler: Mapping[str, float], base_threshold: float) -> pd.DataFrame:
    ...
    risk = raw_hazard_risk(model_payload, out)
    ...
    out["hazard_runtime_raw_risk"] = risk
    out["hazard_risk_z"] = np.clip((risk - float(scaler["median"])) / float(scaler["iqr"]), -4.0, 4.0)
    out["hazard_elapsed_runtime_bar"] = float(HAZARD_SENTINEL_ELAPSED_BAR)
    out["hazard_elapsed_runtime_frac"] = float(HAZARD_SENTINEL_ELAPSED_BAR) / float(MAX_HOLD_BARS)
    return out
...
def runtime_logits(values: np.ndarray) -> np.ndarray:
    ...
    logits[:, 0] += -1.15 * direction
    logits[:, 2] += 1.15 * direction
    logits[:, 0] += -0.40 * risk
    logits[:, 1] += 0.85 * risk
    logits[:, 2] += -0.40 * risk
    return logits
...
def export_hazard_permission_score_table(reference_frame: pd.DataFrame, output_path: Path) -> dict[str, Any]:
        ...
        for feature_index, feature_name in enumerate(RUNTIME_FEATURE_ORDER):
            ...
            for score_index in range(len(cuts) + 2):
                ...
                if feature_name == "direction_proxy":
                    row = np.asarray([[-1.15 * reps[rep_index], 0.0, 1.15 * reps[rep_index]]], dtype="float64")
                else:
                    risk = reps[rep_index]
                    row = np.asarray([[-0.40 * risk, 0.85 * risk, -0.40 * risk]], dtype="float64")
                writer.writerow(
                    {
                        "record_type": "score",
                        "feature_index": feature_index,
    ...
    return {
        ...
        "format": "hazard_permission_ebm_score_table_csv_v1",
        "feature_count": len(RUNTIME_FEATURE_ORDER),
        "feature_names": list(RUNTIME_FEATURE_ORDER),
        "runtime_policy": "direction_proxy_plus_fixed_elapsed_hazard_risk_flat_close_permission_probe",
        "claim_boundary": "runtime_probe_not_hazard_runtime_authority",
    }
...
def check_score_table_parity(table_path: Path, sample: pd.DataFrame) -> dict[str, Any]:
    ...
    return {
        ...
        "claim_boundary": "piecewise_hazard_permission_table_check_not_runtime_authority",
    }
...
def probability_frame(frame: pd.DataFrame, table_path: Path, threshold: float | None = None) -> pd.DataFrame:
    ...
    keep = [
        ...
        "direction_proxy",
        "hazard_risk_z",
        "hazard_runtime_raw_risk",
        "hazard_elapsed_runtime_bar",
        "hazard_elapsed_runtime_frac",
    ]
    ...
    out["hazard_runtime_threshold"] = threshold if threshold is not None else np.nan
    return out
...
def materialize_runtime_surfaces(
...
) -> tuple[dict[str, Any], list[dict[str, Any]], dict[str, Any], dict[str, pd.DataFrame]]:
    ...
    tier_a_train_risk = raw_hazard_risk(model_payloads["tier_a"], context["tier_a_frame"].loc[context["tier_a_frame"]["split"].astype(str).eq("train")])
    tier_b_train_risk = raw_hazard_risk(model_payloads["tier_b"], context["tier_b_training_frame"].loc[context["tier_b_training_frame"]["split"].astype(str).eq("train")])
    ...
    tier_a_table_path = model_root / "tier_a_hazard_permission_score_table.csv"
    tier_b_table_path = model_root / "tier_b_hazard_permission_score_table.csv"
    tier_a_table = export_hazard_permission_score_table(tier_a_features.loc[tier_a_features["split"].astype(str).eq("train")], tier_a_table_path)
    tier_b_table = export_hazard_permission_score_table(tier_b_train_features.loc[tier_b_train_features["split"].astype(str).eq("train")], tier_b_table_path)
    ...
    a_path = pred_root / "tier_a_hazard_permission_predictions.parquet"
    b_path = pred_root / "tier_b_hazard_permission_predictions.parquet"
    ab_path = pred_root / "tier_ab_hazard_permission_predictions.parquet"
    ...
    model_artifacts = {
        ...
        "hazard_sentinel_elapsed_bar": HAZARD_SENTINEL_ELAPSED_BAR,
        "hazard_sentinel_elapsed_frac": float(HAZARD_SENTINEL_ELAPSED_BAR) / float(MAX_HOLD_BARS),
        ...
        "runtime_policy": "direction_proxy supplies side; fixed elapsed hazard_risk_z supplies flat/close pressure; close_on_flat_signal enabled.",
        "known_runtime_difference": "MT5 runtime_probe uses a fixed elapsed-bar hazard snapshot, not a dynamic position-age hazard clock.",
    }
...
def export_feature_matrices(runtime_frames: Mapping[str, pd.DataFrame]) -> dict[str, Any]:
    ...
    for source_split, runtime_split in (("validation", "validation_is"), ("oos", "oos")):
        ...
        payload[f"tier_a_{runtime_split}"] = mt5.export_mt5_feature_matrix_csv(
            ...
            root / f"tier_a_{runtime_split}_hazard_permission_features.csv",
        ...
        payload[f"tier_b_fallback_{runtime_split}"] = mt5.export_mt5_feature_matrix_csv(
            tier_b_frame,
            RUNTIME_FEATURE_ORDER,
            root / f"tier_b_fallback_{runtime_split}_hazard_permission_features.csv",
            metadata_columns=("partial_context_subtype", "route_role"),
        )
    return payload
        ...
        attempts.append(
            attempt_payload(
                **common_kwargs,
                ...
                model_id=f"{RUN_ID}_tier_a_hazard_permission",
                model_backend=MODEL_BACKEND,
        ...
        attempts.append(
            attempt_payload(
                **common_kwargs,
                ...
                model_id=f"{RUN_ID}_tier_b_hazard_permission",
                model_backend=MODEL_BACKEND,
        ...
        attempts.append(
            attempt_payload(
                **common_kwargs,
                ...
                fallback_model_id=f"{RUN_ID}_tier_b_hazard_permission",
...
def execute_or_block(prepared: Mapping[str, Any], args: argparse.Namespace) -> dict[str, Any]:
    ...
    for record in result.get("mt5_kpi_records", []):
        ...
        record["topic_read"] = "hazard_permission_fixed_elapsed_flat_close_runtime_handoff"
        record["threshold_quantile"] = f"q{THRESHOLD_QUANTILE:.2f}"
        record["max_hold_bars"] = MAX_HOLD_BARS
        record["hazard_sentinel_elapsed_bar"] = HAZARD_SENTINEL_ELAPSED_BAR
    return result
...
def build_summary(result: Mapping[str, Any], model_artifacts: Mapping[str, Any], prediction_artifacts: Mapping[str, Any], tier_records: Sequence[Mapping[str, Any]], source_summary: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "topic_read": "hazard_permission_fixed_elapsed_flat_close_runtime_handoff",
        ...
        "model_characteristic_strength": "hazard_permission_runtime_axis_visible" if visible else "hazard_permission_runtime_axis_weak_or_blocked",
...
def packet_markdown(summary: Mapping[str, Any], kpi: Mapping[str, Any]) -> str:
    ...
    return f"""# RUN19B Hazard Runtime Probe Packet(실행19B 위험률 런타임 탐침 묶음)
...
- Tier A table parity(Tier A 점수표 동등성): `{parity.get('tier_a', {}).get('passed')}`; max_abs_diff(최대 절대 차이) `{parity.get('tier_a', {}).get('max_abs_diff')}`
- Tier B table parity(Tier B 점수표 동등성): `{parity.get('tier_b', {}).get('passed')}`; max_abs_diff(최대 절대 차이) `{parity.get('tier_b', {}).get('max_abs_diff')}`
    ...
    return [
        {
            ...
            "status": status,
            "research_path": rel(SOURCE_RUN_ROOT / "summary.json"),
            "runtime_path": rel(ROOT / "foundation/mt5/ObsidianPrimeV2_RuntimeProbeEA.mq5"),
            "shared_contract": "two-feature hazard permission score table, q0.80 non-flat thresholds, close-on-flat enabled, Tier A primary plus Tier B fallback routing",
            "known_differences": summary.get("model_artifacts", {}).get("known_runtime_difference"),
        ...
        },
        {
            ...
            "claim_boundary": BOUNDARY,
            "next_condition": "Stage25 closeout/open Stage26 if runtime_probe evidence remains sufficient and no repair is required.",
        },
    ]
...
def write_run_outputs(result: Mapping[str, Any], model_artifacts: Mapping[str, Any], prediction_artifacts: Mapping[str, Any], tier_records: Sequence[Mapping[str, Any]], source_summary: Mapping[str, Any], kpi: Mapping[str, Any], created_at: str) -> dict[str, Any]:
    ...
    run_row = {
        ...
        "notes": ledger_pairs(
            (
                ...
                ("hazard_elapsed_bar", HAZARD_SENTINEL_ELAPSED_BAR),
    ...
    kpi_record = {
        **manifest,
        "kpi_scope": "hazard_permission_mt5_runtime_probe",
        "python_tier_records": list(tier_records),
        "mt5": {
...
def update_workspace_state(summary: Mapping[str, Any]) -> None:
    completed = summary.get("external_verification_status") == "completed"
    status = "active_run19B_mt5_runtime_probe_completed" if completed else "active_run19B_mt5_runtime_probe_blocked_after_attempt"
    next_action = "stage25_closeout_and_stage26_open_only" if completed else "repair_run19B_hazard_runtime_probe_then_rerun_exact_attempts"
    state = io_path(WORKSPACE_STATE_PATH).read_text(encoding="utf-8-sig")
    state = state.replace(f"current_run_id: {SOURCE_RUN_ID}", f"current_run_id: {RUN_ID}", 1)
    lines = []
    for line in state.splitlines():
        if line.startswith("- treat Stage 25 as active after "):
            lines.append(
                f"- treat Stage 25 as {status} after Hazard model(위험률 모델) MT5 runtime_probe(MT5 런타임 탐침); next action is {next_action}, and no baseline, promotion, or runtime authority exists"
            )
        else:
            lines.append(line)
    state = "\n".join(lines) + "\n"
    state = state.replace(
        f"      status: active_run19A_python_structural_scout_completed\n      current_run_id: {SOURCE_RUN_ID}",
    ...
    stage_block = f"""stage25_hazard_model:
  stage_id: {STAGE_ID}
  status: {status}
  current_run_id: {RUN_ID}
  ...
  report_path: stages/{STAGE_ID}/03_reviews/run19B_hazard_trade_lifecycle_runtime_probe_packet.md
  packet_summary_path: docs/agent_control/packets/{PACKET_ID}/aggregate_summary.json
  next_action: {next_action}
"""
    state = replace_top_level_yaml_block(state, "stage25_hazard_model:", stage_block)
    run_block = f"""stage25_hazard_run19B_runtime_probe:
  packet_id: {PACKET_ID}
  ...
  next_action: {next_action}
"""
    state = replace_top_level_yaml_block(state, "stage25_hazard_run19B_runtime_probe:", run_block)
...
- stage(단계): `{STAGE_ID}`
- status(상태): `{'active_run19B_mt5_runtime_probe_completed' if completed else 'active_run19B_mt5_runtime_probe_blocked_after_attempt'}`
...
효과(effect, 효과): Stage25(25단계)는 Hazard model(위험률 모델)의 Python-side evidence(파이썬 근거)와 MT5 runtime_probe(MT5 런타임 탐침)를 기록했지만, closeout(마감), baseline(기준선), promotion(승격), runtime authority(런타임 권위)는 아직 없다.
...
""",
    ...
    write_md(
        DECISION_PATH,
        f"""# 2026-05-05 Stage25 RUN19B Hazard Runtime Probe Decision(25단계 실행19B 위험률 런타임 탐침 결정)
...
- validation net/PF/trades(검증 순손익/수익 팩터/거래 수): `{validation.get('net_profit')}` / `{validation.get('profit_factor')}` / `{validation.get('trade_count')}`
- OOS net/PF/trades(표본외 순손익/수익 팩터/거래 수): `{oos.get('net_profit')}` / `{oos.get('profit_factor')}` / `{oos.get('trade_count')}`
- MT5 KPI records(MT5 핵심 성과 지표 기록): `{summary.get('mt5_kpi_record_count')}`
- normalized KPI records(정규화 핵심 성과 지표 기록): `{kpi.get('normalized_records')}`
...
""",
    ...
    update = f"""## Latest Stage25 RUN19B Hazard Runtime Update(최신 25단계 실행19B 위험률 런타임 업데이트)

Stage25(25단계) `{RUN_ID}`를 MT5 runtime_probe(MT5 런타임 탐침)로 실행했다.
...
"""
    ...
    plan = plan.replace(
        f"현재 첫 미완료 milestone(마일스톤)은 Stage25(25단계) `{RUN_ID}` MT5 runtime_probe(MT5 런타임 탐침)이다.",
        f"현재 첫 미완료 milestone(마일스톤)은 Stage25(25단계) `{next_action}`이다.",
        1,
    )
    plan = plan.replace(
        f"- [ ] Stage25(25단계) hazard model(위험률 모델) scout/probe/closeout/open Stage26. Completed(완료): `{SOURCE_RUN_ID}`; remaining(남음): MT5 runtime_probe(MT5 런타임 탐침), closeout/open Stage26.",
        f"- [ ] Stage25(25단계) hazard model(위험률 모델) scout/probe/closeout/open Stage26. Completed(완료): `{SOURCE_RUN_ID}`, `{RUN_ID}`; remaining(남음): closeout/open Stage26.",
        1,
    )
    plan = plan.replace(
        f"Current active milestone(현재 활성 마일스톤): Stage25(25단계) `{RUN_ID}` narrow MT5 runtime_probe(좁은 MT5 런타임 탐침).",
        f"Current active milestone(현재 활성 마일스톤): Stage25(25단계) `{next_action}`.",
        1,
    )
...
- changed files(변경 파일): hazard runtime probe pipeline(위험률 런타임 탐침 파이프라인), MT5 run evidence(MT5 실행 근거), normalized KPI(정규화 핵심 성과 지표), ledgers(장부), current truth docs(현재 진실 문서).
...
- blocker(차단 사유): `{'none(없음)' if completed else 'runtime_probe_blocked_after_attempt(런타임 탐침 시도 후 차단)'}`.
- exact next action(정확한 다음 행동): `{next_action}`.
- git status(깃 상태): checkpoint commit/push(중간 지점 커밋/푸시) pending(대기).

효과(effect, 효과): 다음 재개는 Stage25(25단계) closeout/open Stage26(마감/26단계 개방) 또는 run19B(19B 실행) 복구 조건에서 시작한다.
"""
    plan = replace_markdown_section(plan, "## Latest Stop Resume State", resume)
    outcome = f"- `2026-05-05`: Stage25(25단계) `{RUN_ID}` MT5 runtime_probe(런타임 탐침)를 기록했다. judgment(판정): `{summary.get('closure_judgment')}`."
...
def materialize_bundle(args: argparse.Namespace) -> dict[str, Any]:
    ...
    prepared = {
        ...
        "feature_matrix_artifacts": list(feature_matrices.values()),
        "model_artifacts": model_artifacts,
        "prediction_artifacts": prediction_artifacts,
        "tier_records": tier_records,
    ...
    if bool(args.materialize_only):
        write_json(RUN_ROOT / "materialized_bundle_summary.json", prepared)
    return prepared
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if bool(args.materialize_only):
        return {
            ...
            "prepared_bundle_path": rel(RUN_ROOT / "materialized_bundle_summary.json"),
        }
    result = execute_or_block(bundle, args)
    provisional = {
    ...
    if not bool(args.materialize_only):
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run Stage25 hazard trade-lifecycle MT5 runtime probe.")
