...
SOURCE_STAGE59N_ID = "59N_adapter_repair__bounded_followup_from_stage59m"
STAGE59O_ID = "59O_adapter_repair__bounded_followup_from_stage59n"
NEXT_REPAIR_STAGE_ID = "59P_adapter_repair__bounded_followup_from_stage59o"
RUN_NUMBER = "run59J"
RUN_ID = "run59J_stage59o_bounded_followup_from_stage59n_v1"
PACKET_ID = "stage59o_bounded_followup_from_stage59n_v1"
PARENT_RUN_ID = "run59I_stage59n_bounded_followup_from_stage59m_v1"
SOURCE_ADAPTER_ID = "s59n_v62_sl20_tp30_sd12"
DEVELOPMENT_ANCHOR = "v64_v47_ctxgap14_refill_etfw_h2_no_b"
BACKUP_ANCHOR = "v60_v47_et_stable_damage_firewall_h2c0_no_b"
SOURCE_STAGE59N_PUSHED_COMMIT = "5c551c8f0258dec5341cf96c95ef63b6ea7bc73a"
BOUNDARY = (
    "research_development_only_no_live_readiness_no_runtime_authority_"
    "no_operating_promotion_no_operating_reference_no_production_baseline_no_deployment"
)

STAGE_ROOT = Path("stages") / STAGE59O_ID
...
SOURCE_STAGE59N_ROOT = Path("stages") / SOURCE_STAGE59N_ID
SOURCE_STAGE59N_DECISION = SOURCE_STAGE59N_ROOT / "03_reviews/stage59n_decision.md"
SOURCE_STAGE59N_REPORT = SOURCE_STAGE59N_ROOT / "03_reviews/bounded_followup_from_stage59m_report.md"
SOURCE_STAGE59N_SUMMARY = SOURCE_STAGE59N_ROOT / "03_reviews/bounded_followup_summary.csv"
SOURCE_STAGE59N_JSON = SOURCE_STAGE59N_ROOT / "03_reviews/bounded_followup_summary.json"
...
COMMON_ROOT = f"Project_Obsidian_Prime_v2/s59o/{RUN_NUMBER}"

REPORT_PATH = REVIEWS_ROOT / "bounded_followup_from_stage59n_report.md"
...
DECISION_PATH = REVIEWS_ROOT / "stage59o_decision.md"
AUDIT_CSV_PATH = REVIEWS_ROOT / "stage59o_trade_audit.csv"
...
STAGE59O_VARIANTS = (
    repair.RepairVariant(
        adapter_id="s59o_v62_sl20_tp30_sd12_t52",
        label="run50BQ_v62_transition_h4_sl20_tp30_sd12_t52_risk3pct_atr",
        atr_enabled=True,
        ...
        max_hold_bars=4,
        notes="Stage59O bounded follow-up: keep Stage59N bracket/cooldown and raise both confidence thresholds to 0.52.",
    ),
    repair.RepairVariant(
        adapter_id="s59o_v62_sl20_tp30_sd12_t55",
        label="run50BQ_v62_transition_h4_sl20_tp30_sd12_t55_risk3pct_atr",
        atr_enabled=True,
        model_risk_enabled=True,
        fixed_lot=0.25,
        atr_stop_multiplier=2.0,
        ...
        close_only_on_opposite_signal=True,
        max_hold_bars=4,
        notes="Stage59O bounded follow-up: keep Stage59N bracket/cooldown and raise both confidence thresholds to 0.55.",
    ),
    repair.RepairVariant(
        adapter_id="s59o_v62_sl20_tp30_sd12_t58",
        label="run50BQ_v62_transition_h4_sl20_tp30_sd12_t58_risk3pct_atr",
        atr_enabled=True,
        model_risk_enabled=True,
        fixed_lot=0.25,
        atr_stop_multiplier=2.0,
        ...
        close_only_on_opposite_signal=True,
        max_hold_bars=4,
        notes="Stage59O bounded follow-up: keep Stage59N bracket/cooldown and raise both confidence thresholds to 0.58.",
    ),
)
...
def source_specs() -> dict[str, dict[str, Any]]:
    ...
    return {variant.adapter_id: dict(v62) for variant in STAGE59O_VARIANTS}


def configure_reused_engine() -> None:
    engine.STAGE59_ID = STAGE59O_ID
    ...
    engine.STAGE59_VARIANTS = STAGE59O_VARIANTS
    engine.SOURCE_SPECS = source_specs()
    engine.MODEL_RISK_MIN_PCT = {variant.adapter_id: 0.005 for variant in STAGE59O_VARIANTS}

    repair.STAGE_ID = STAGE59O_ID
    repair.RUN_NUMBER = RUN_NUMBER
    repair.RUN_ID = RUN_ID
    repair.RUN_ROOT = RUN_ROOT
    repair.REPAIR_VARIANTS = STAGE59O_VARIANTS
    s58.STAGE58_ID = STAGE59O_ID
    ...
    s58.STAGE58_VARIANTS = STAGE59O_VARIANTS
...
def build_attempts(inputs: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    for variant_index, variant in enumerate(STAGE59O_VARIANTS, start=1):
            ...
            for role_index, (tier, attempt_role, prefix, attempt_token) in enumerate(
                ...
                attempts.append(
                    attempt_payload(
                        ...
                        run_id=RUN_ID,
                        stage_number=59,
                        exploration_label="stage59O_BaselineAdapter__ThresholdPressureFromStage59N",
...
def execute_prepared_run_checkpointed(prepared: Mapping[str, Any], args: argparse.Namespace) -> dict[str, Any]:
    ...
    return {
        **dict(prepared),
        ...
        "judgment": "inconclusive_stage59o_runtime_probe_completed" if completed and report_completed else "blocked_stage59o_runtime_probe",
    }
...
def next_stage_for_decision(decision: str) -> str:
    ...
    if decision == "demote_current_adapter_and_select_backup":
        return "59P_adapter_repair__backup_or_demote_from_stage59o"
    if decision == "open_bounded_followup":
        return "59P_adapter_repair__bounded_followup_from_stage59o"
...
def next_packet_for_decision(decision: str) -> tuple[str, str]:
    ...
    return "stage59p_bounded_followup_from_stage59o_v1", "run59K_stage59p_bounded_followup_from_stage59o_v1"
            ...
            rows.append(
                {
                    ...
                    "stage_id": STAGE59O_ID,
                    "run_id": RUN_ID,
                    "artifact_type": "stage59o_bounded_followup_evidence",
                    "path": rel(path),
                    "sha256": sha256_file_lf_normalized(path),
                    "created_at_utc": created,
                    "notes": "Stage59O bounded follow-up from Stage59N artifact.",
                }
            )
            ...
            rows.append(
                {
                    ...
                    "created_at_utc": created,
                    "notes": "Actual Stage59O MT5 Strategy Tester HTML report.",
                }
            )
    return rows
...
def write_run_identity(result: Mapping[str, Any]) -> None:
    engine.write_json(
        RUN_ROOT / "run_manifest.json",
        {
            ...
            "bounded_question": "Can bounded confidence threshold pressure on the Stage59N bracket/cooldown candidate repair OOS PF and cost-stressed expectancy without starting ONNX?",
            "source_stage59n_decision": rel(SOURCE_STAGE59N_DECISION),
            "source_stage59n_pushed_commit": SOURCE_STAGE59N_PUSHED_COMMIT,
            "variants": [
                {
                    **variant.__dict__,
                ...
                for variant in STAGE59O_VARIANTS
            ],
...
def write_ledgers(
        ...
        [
            {
                "run_id": RUN_ID,
                "stage_id": STAGE59O_ID,
                "lane": "baseline_adapter_bounded_followup_from_stage59n",
                "status": status,
                "judgment": decision,
    ...
    ledger_rows.append(
        {
            "ledger_row_id": f"{RUN_ID}__aggregate_bounded_followup_from_stage59n",
            "stage_id": STAGE59O_ID,
            "run_id": RUN_ID,
            "subrun_id": "aggregate_bounded_followup_from_stage59n",
            "parent_run_id": PARENT_RUN_ID,
            "record_view": "bounded_followup_from_stage59n",
            "tier_scope": "Tier A+B",
            ...
            "external_verification_status": external,
            "notes": "Stage59O bounded confidence threshold follow-up from Stage59N evidence; not final package completion.",
        }
    )
...
def report_markdown(summary_rows: Sequence[Mapping[str, Any]], decision: str, reasons: Sequence[str], external: str) -> str:
    ...
    best = best_repaired_variant(summary_rows)
    return f"""# Stage59O Bounded Followup From Stage59N Report(59O단계 59N단계 기반 경계 후속 보고서)

- stage(단계): `{STAGE59O_ID}`
- run(실행): `{RUN_ID}`
...
def equity_audit_markdown(summary_rows: Sequence[Mapping[str, Any]], segment_rows: Sequence[Mapping[str, Any]]) -> str:
    ...
    return f"""# Stage59O Bounded Followup Equity Curve Audit(59O단계 경계 후속 자금 곡선 감사)
...
def decision_markdown(decision: str, reasons: Sequence[str], best: Mapping[str, Any], external: str) -> str:
    return f"""# Stage59O Decision(59O단계 판정)

decision(판정): `{decision}`

Stage59O(59O단계)는 Stage59N evidence(59N단계 근거)를 사용한 bounded follow-up(경계 후속)으로 기록한다. Effect(효과): confidence threshold(신뢰도 임계값)의 성공/실패를 다음 bounded stage(경계 다음 단계)의 입력 근거로 넘긴다.
...
Stage59O closeout(59O단계 종료)는 overall goal completion(전체 목표 완료)이 아니다. Effect(효과): Stage60 ONNX hardening(60단계 ONNX 경화)은 adapter quality(어댑터 품질)가 강할 때만 열린다.
    ...
    engine.write_json(
        SUMMARY_JSON_PATH,
        {
            ...
            "required_outputs": {
                ...
                "stage59o_decision": rel(DECISION_PATH),
            },
...
def write_packet_files(
    ...
    files = {
        ...
        "experiment_design_receipt.json": {
            "hypothesis": "A bounded confidence threshold follow-up on the Stage59N bracket/cooldown candidate may improve OOS PF and cost-stressed expectancy while preserving validation behavior.",
            ...
            "comparison_baseline": "Stage59N best candidate s59n_v62_sl20_tp30_sd12.",
            "control_variables": ["US100", "M5", "split_v1", "Tier B disabled", "close_only_on_opposite", "hold4", "ATR SL 2.0", "ATR TP 3.0", "3% model-controlled risk cap", "ONNX deferred"],
            ...
            "stop_conditions": "three bounded Stage59O confidence threshold variants only",
        ...
        "artifact_lineage_audit.json": {
            "source_inputs": [rel(SOURCE_STAGE59N_DECISION), rel(SOURCE_STAGE59N_REPORT), rel(SOURCE_STAGE59N_SUMMARY), rel(SOURCE_STAGE59N_JSON)],
...
def write_stage_docs(decision: str) -> None:
    ...
    engine.write_md(
        ...
        f"""# Stage59O Brief(59O단계 개요)

- stage_id(단계 ID): `{STAGE59O_ID}`
- source_stage(원천 단계): `{SOURCE_STAGE59N_ID}`
- source_decision(원천 판정): `continue_repair_in_new_bounded_stage`
- bounded_question(경계 질문): `Can bounded confidence threshold pressure on the Stage59N bracket/cooldown candidate repair OOS PF and cost-stressed expectancy without starting ONNX?`
- boundary(경계): `{BOUNDARY}`

Stage59O(59O단계)는 Stage59N(59N단계) bracket/cooldown candidate(브래킷/쿨다운 후보)를 유지하고 confidence threshold(신뢰도 임계값) 세 변형만 측정한다. Effect(효과): Stage60 ONNX(60단계 ONNX)나 deployment(배포)로 이어가지 않고 OOS 약점 수리 여부만 본다.
""",
    )
    engine.write_md(
        INPUT_ROOT / "input_refs.md",
        f"""# Stage59O Input References(59O단계 입력 참조)

- stage59n_decision(59N단계 판정): `{rel(SOURCE_STAGE59N_DECISION)}`
- stage59n_report(59N단계 보고서): `{rel(SOURCE_STAGE59N_REPORT)}`
- stage59n_summary(59N단계 요약): `{rel(SOURCE_STAGE59N_SUMMARY)}`
- stage59n_pushed_commit(59N단계 푸시 커밋): `{SOURCE_STAGE59N_PUSHED_COMMIT}`
...
Effect(효과): Stage59O(59O단계)는 Stage59N(59N단계) 실패 기억과 run50BQ(run50BQ) v62 전환 원천을 명시적으로 연결한다.
""",
    )
    engine.write_md(
        SELECTED_ROOT / "selection_status.md",
        f"""# Stage59O Selection Status(59O단계 선택 상태)

- stage_status(단계 상태): `closed_bounded_followup_from_stage59n`
- source_stage(원천 단계): `{SOURCE_STAGE59N_ID}`
- source_decision(원천 판정): `continue_repair_in_new_bounded_stage`
- stage59o_decision(59O단계 판정): `{decision}`
- next_stage_or_branch(다음 단계/분기): `{next_stage}`
- selected_research_baseline(선택 연구 기준선): `none`
- claim_boundary(주장 경계): `{BOUNDARY}`

Effect(효과): Stage59O(59O단계)는 측정 결과를 보존하지만 final package(최종 패키지)나 operating claim(운영 주장)을 만들지 않는다.
""",
    )
    engine.write_md(
        REVIEWS_ROOT / "review_index.md",
        f"""# Stage59O Review Index(59O단계 검토 색인)
...
- bounded_followup_risk_atr_telemetry(경계 후속 위험/ATR 텔레메트리): `{rel(RISK_ATR_TELEMETRY_PATH)}`
- stage59o_decision(59O단계 판정): `{rel(DECISION_PATH)}`
""",
    )
    if not next_stage.startswith("60_"):
        ...
        engine.write_md(
            ...
            f"""# Stage59P Brief(59P단계 개요)

- stage_id(단계 ID): `{next_stage}`
- source_stage(원천 단계): `{STAGE59O_ID}`
- source_decision(원천 판정): `{decision}`
- bounded_question(경계 질문): `What bounded follow-up should repair, demote, or branch from the Stage59O evidence without starting ONNX prematurely?`
- boundary(경계): `{BOUNDARY}`

Stage59P(59P단계)는 Stage59O(59O단계) 결과가 Stage60 ONNX(60단계 ONNX)에 충분하지 않을 때 이어지는 bounded follow-up(경계 후속)이다. Effect(효과): repair(수리), demotion(강등), branch(분기)를 다음 작은 질문으로 제한한다.
""",
        )
        engine.write_md(
            next_root / "01_inputs/input_refs.md",
            f"""# Stage59P Input References(59P단계 입력 참조)

- stage59o_decision(59O단계 판정): `{rel(DECISION_PATH)}`
- stage59o_report(59O단계 보고서): `{rel(REPORT_PATH)}`
- stage59o_summary(59O단계 요약): `{rel(SUMMARY_CSV_PATH)}`
- stage59o_segment_kpi(59O단계 구간 KPI): `{rel(SEGMENT_KPI_PATH)}`
- stage59o_risk_atr_telemetry(59O단계 위험/ATR 텔레메트리): `{rel(RISK_ATR_TELEMETRY_PATH)}`
""",
        )
        engine.write_md(
            next_root / "03_reviews/review_index.md",
            """# Stage59P Review Index(59P단계 검토 색인)

Stage59P(59P단계)는 planned(계획) 상태다. Effect(효과): Stage59O(59O단계) 약점을 다음 bounded repair(경계 수리) 근거로 넘긴다.
""",
        )
        engine.write_md(
            next_root / "04_selected/selection_status.md",
            f"""# Stage59P Selection Status(59P단계 선택 상태)

- stage_status(단계 상태): `active_planned_from_stage59o`
- source_stage(원천 단계): `{STAGE59O_ID}`
- source_decision(원천 판정): `{decision}`
- selected_research_baseline(선택 연구 기준선): `none`
- claim_boundary(주장 경계): `{BOUNDARY}`

Effect(효과): Stage59P(59P단계)는 Stage59O(59O단계) 이후 필요한 다음 작은 질문만 다룬다.
""",
        )
...
- status(상태): `stage59o_closed_{decision}`
- claim_boundary(주장 경계): research/development only(연구/개발 전용)

Stage59O(59O단계) closed(종료) as bounded confidence threshold follow-up from Stage59N(59N단계 기반 경계 신뢰도 임계값 후속). Effect(효과): Stage59N(59N단계) OOS weakness(OOS 약점)는 보존하고, Stage60 ONNX(60단계 ONNX)는 품질 근거가 강할 때만 열린다.

## Latest Stage59O Evidence(최신 59O단계 근거)
...
""",
    ...
    focus = (
        "current_focus:\n"
        f"- >-\n"
        f"  Stage59O(59O단계) `{STAGE59O_ID}` closed(종료) as bounded confidence threshold follow-up from Stage59N(59N단계 기반 경계 신뢰도 임계값 후속); decision(판정)=`{decision}`. "
        ...
        f"- >-\n"
        f"  Next stage_or_branch(다음 단계/분기) `{next_stage}` is active/planned(활성/계획). Effect(효과): Stage59O(59O단계) 결과를 다음 bounded step(경계 다음 단계)으로 넘긴다.\n"
    )
    text = re.sub(
...
stage59o_bounded_followup_from_stage59n:
  packet_id: {PACKET_ID}
  stage_id: {STAGE59O_ID}
  status: closed_bounded_followup_from_stage59n
  ...
  boundary: {BOUNDARY}
"""
    if "stage59o_bounded_followup_from_stage59n:" in text:
        text = re.sub(r"\nstage59o_bounded_followup_from_stage59n:\n(?:  .*\n)*", block, text, count=1)
    else:
        text += block
...
def parse_args(argv: Sequence[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run Stage59O bounded follow-up from Stage59N.")
...
def load_existing_result() -> dict[str, Any]:
    ...
    if not path_exists(manifest) or not path_exists(kpi):
        raise FileNotFoundError("Stage59O existing run_manifest.json or kpi_record.json is missing")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.finalize_existing:
        ...
        prepared = {
            ...
            "model_family": "baseline_adapter_stage59o_bounded_followup_ebm_table",
            "feature_set_id": "stage59o_v62_confidence_threshold_discrete_signal",
