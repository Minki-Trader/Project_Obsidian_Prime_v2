...
RUN_ID = "run309C_review_split_coherent_profit_curve_source_mt5_probe_v1"
RUN_NUMBER = "run309C"
SOURCE_RUN_ID = "run309B_execute_split_coherent_profit_curve_source_mt5_probe_v1"
PARENT_RUN_ID = "run309A_design_split_coherent_profit_curve_source_rebuild_packet_v1"
...
NEXT_REBUILD_STAGE_ID = "310_onnx_candidate_campaign__runtime_positive_fragment_allocation_rebuild"
NEXT_ADAPTER_STAGE_ID = "310_onnx_candidate_campaign__adapter_package_for_split_coherent_profit_curve_source"

STAGE_ROOT = ROOT / "stages" / STAGE_ID
RUN309B = STAGE_ROOT / "02_runs" / "run309B"
...
SOURCE_KPI = RUN309B / "mt5_kpi_summary.csv"
SOURCE_ATTEMPT_SUMMARY = RUN309B / "attempt_summary.csv"
PRODUCER = Path("stage_pipelines/stage309/review_split_coherent_profit_curve_source_mt5_probe.py")

SCOREBOARD = RUN_ROOT / "split_coherent_profit_curve_review_scoreboard.csv"
...
NEXT_STAGE_QUEUE = RUN_ROOT / "stage310_seed_queue.csv"
RESULT_JUDGMENT = RUN_ROOT / "result_judgment.csv"
GATE_AUDIT = RUN_ROOT / "required_gate_coverage_audit.csv"
RUN_MANIFEST = RUN_ROOT / "run_manifest.json"
LINEAGE = RUN_ROOT / "artifact_lineage_receipt.json"
REPORT = REVIEWS / "run309C_review_stage310_open.md"
DECISION = ROOT / "docs" / "decisions" / "2026-05-24_stage309_split_coherent_profit_curve_review_stage310_open.md"
...
def feature_trading_days(attempt: Mapping[str, str]) -> int:
    ...
    local = RUN309B / "features" / Path(feature_path).name
...
def build_scoreboard(rows: Sequence[Mapping[str, Any]]) -> tuple[list[dict[str, Any]], list[dict[str, Any]], list[dict[str, Any]]]:
    ...
    for candidate_id, split_rows in sorted(by_candidate.items()):
        ...
        profit_gate = "passed" if val_net >= 350.0 and oos_net >= 250.0 and combined >= 900.0 else "failed"
        efficiency_gate = "passed"
        if number(val.get("profit_factor")) < 1.10 or number(oos.get("profit_factor")) < 1.08:
        ...
        if selected_gate:
            ...
            salvage = "positive_fragment_allocation_seed" if combined > 0 else "failure_memory_only"
            failures.append(
                {
                    ...
                    "reopen_condition": "only_with_runtime_allocation_layer_that_lifts_scale_and_curve_together",
                    "do_not_repeat": "do_not_only_retune_hold_density_or_lot_on_stage309_surface",
                }
            )
...
def next_stage_rows(scoreboard: Sequence[Mapping[str, Any]]) -> list[dict[str, Any]]:
    ...
    return [
        {
            "seed_id": "stage309_positive_fragment_allocation_seed",
            "source_candidates": ";".join(row["package_id"] for row in positive[:3]) or "none",
            "fresh_thesis": "runtime_positive_fragment_allocation",
            "use_as": "fragments_not_candidates",
            "upside": "cp309A and cp309E made positive actual MT5 fragments.",
            "failure_mode": "scale, trade-count, efficiency, or curve pocket gates did not pass together.",
            "discard_condition": "discard if allocation layer cannot produce 4-10 trades/day, higher net scale, and shallow local pockets.",
            "next_action": "run310A_design_runtime_positive_fragment_allocation_rebuild_packet",
        }
    ]
...
def scaffold_next_stage(selected_rows: Sequence[Mapping[str, Any]]) -> tuple[str, str, str, str]:
    if selected_rows:
        ...
        question = "selected split-coherent profit curve source(선택된 분할 일관 수익 곡선 원천)를 Adapter(어댑터)로 패키징할 수 있는가?"
        status = "opened_adapter_package_stage_after_stage309_candidate_selection"
        judgment = "candidate_selected_adapter_package_stage_opened_no_onnx_yet"
        next_action = "run310A_build_split_coherent_profit_curve_source_adapter_package"
    else:
        next_stage_id = NEXT_REBUILD_STAGE_ID
        question = "runtime positive fragments(런타임 양수 조각)을 새 allocation layer(배분 계층)로 묶어 수익 규모, 4-10 trades/day(일 4-10거래), shallow curve pocket(얕은 곡선 포켓)을 동시에 만들 수 있는가?"
        status = "opened_runtime_positive_fragment_allocation_rebuild_after_stage309_no_selection"
        judgment = "no_candidate_selected_positive_fragment_allocation_stage_opened"
        next_action = "run310A_design_runtime_positive_fragment_allocation_rebuild_packet"
    ...
    write_text(
        ...
        "\n".join(
            [
                ...
                "",
                "Effect(효과): Stage309(309단계)의 양수 조각을 후보로 승격하지 않고, 새 allocation(배분) 구조의 seed(씨앗)로만 쓴다.",
            ]
        ),
    )
    write_text(
        ...
        "\n".join(
            [
                "# Stage310 Selection Status(310단계 선택 상태)",
                "",
                ...
                f"- stage309_review(309단계 검토): `{rel(REPORT)}`",
            ]
        ),
    )
    write_text(next_root / "03_reviews" / "review_index.md", f"# Stage310 Review Index(310단계 검토 색인)\n\n- stage309_review(309단계 검토): `{rel(REPORT)}`\n")
    write_csv(
        next_root / "03_reviews" / "stage_run_ledger.csv",
        STAGE_LEDGER_COLUMNS,
        [
            {
                "row_id": f"{RUN_ID}__stage310_open",
...
def report_markdown(scoreboard: Sequence[Mapping[str, Any]], failures: Sequence[Mapping[str, Any]], selected_rows: Sequence[Mapping[str, Any]], next_stage_id: str, next_action: str) -> str:
    ...
    lines = [
        ...
        f"- selected_candidate(선택 후보): `{selected_rows[0]['package_id'] if selected_rows else 'none'}`",
        f"- Adapter package(어댑터 패키지): `{'deferred_to_stage310' if selected_rows else 'none'}`",
        "- ONNX readiness(온엑스 준비): `not_started`",
        "- Goal Achieve(목표 달성): `not_claimed`",
    ...
    lines.extend(
        [
            ...
            "## Decision(결정)",
            "",
            "Stage309(309단계)는 selected candidate(선택 후보) 없이 닫는다." if not selected_rows else "Stage309(309단계)는 Adapter(어댑터) 패키지 단계로 넘긴다.",
...
def update_docs(status: str, judgment: str, next_stage_id: str, next_action: str, selected_rows: Sequence[Mapping[str, Any]]) -> None:
    ...
    selected = replace_line(selected, "- Adapter package(", f"- Adapter package(어댑터 패키지): `{'deferred_to_stage310' if selected_rows else 'none'}`")
    selected = replace_line(selected, "- next_action(", f"- next_action(다음 행동): `{next_action}`")
    selected = drop_prefixed_lines(selected, ("- run309C_report(", "- stage310_opened("))
    selected += f"- run309C_report(309C 보고): `{rel(REPORT)}`\n"
    selected += f"- stage310_opened(310단계 열림): `{next_stage_id}`\n"
    write_text(SELECTED, selected)

    review_index = read_text(REVIEW_INDEX)
    review_index = drop_prefixed_lines(review_index, ("- run309C_report(", "- run309C_scoreboard(", "- stage310_seed_queue("))
    review_index += f"- run309C_report(309C 보고): `{rel(REPORT)}`\n"
    review_index += f"- run309C_scoreboard(309C 점수표): `{rel(SCOREBOARD)}`\n"
    review_index += f"- stage310_seed_queue(310단계 씨앗 대기열): `{rel(NEXT_STAGE_QUEUE)}`\n"
    ...
    current = drop_prefixed_lines(current, ("- run309C_summary(",))
    current = current.rstrip() + f"\n- run309C_summary(309C 요약): Stage309(309단계) actual MT5(실제 메타트레이더5) 검토를 완료했다. Effect(효과): selected_candidate(선택 후보)는 `{selected_rows[0]['package_id'] if selected_rows else 'none'}`이고 next_stage(다음 단계)는 `{next_stage_id}`다.\n"
    ...
    focus = (
        f"- >-\n"
        f"  Stage309(309단계) run309C(309C 실행) split-coherent profit curve source review(분할 일관 수익 곡선 원천 검토) `{RUN_ID}` closed Stage309 and opened `{next_stage_id}`. "
        f"Effect(효과): selected candidate(선택 후보)는 `{selected_rows[0]['package_id'] if selected_rows else 'none'}`이고 Adapter package(어댑터 패키지)는 `{'deferred_to_stage310' if selected_rows else 'none'}`, ONNX readiness(온엑스 준비)는 `not_started`다.\n"
    )
    ...
    if RUN_ID not in changelog:
        changelog += (
            f"\n## {UPDATED_ON} run309C Split-coherent profit curve source review(309C 분할 일관 수익 곡선 원천 검토)\n\n"
            f"- status(상태): `{status}`\n"
            f"- judgment(판정): `{judgment}`\n"
            f"- effect(효과): Stage309(309단계)를 닫고 `{next_stage_id}`를 열었다.\n"
...
def update_registers(status: str, judgment: str, next_action: str) -> None:
    safe_upsert(
        ...
        [{"run_id": RUN_ID, "stage_id": STAGE_ID, "lane": "split_coherent_profit_curve_source_review", "status": status, "judgment": judgment, "path": rel(REPORT), "notes": f"selected_candidate_reviewed;next_action={next_action}."}],
        "run_id",
    )
    safe_upsert(
        ALPHA_LEDGER,
        ...
        [
            {
                ...
                "record_view": "split_coherent_profit_curve_source_review",
    ...
    safe_upsert(
        STAGE_LEDGER,
        STAGE_LEDGER_COLUMNS,
        [{"row_id": f"{RUN_ID}__review", "stage_id": STAGE_ID, "run_id": RUN_ID, "view": "split_coherent_profit_curve_source_review", "tier_scope": "Tier A used/Tier B fallback/actual routed total", "scoreboard": "split_coherent_profit_curve_review_scoreboard", "status": status, "judgment": judgment, "evidence_boundary": "runtime_probe_review_no_onnx", "report_path": rel(REPORT), "notes": "Stage310 opened if no selected candidate; ONNX not started."}],
        "row_id",
    )
...
def update_memory_registers(failures: Sequence[Mapping[str, Any]], selected_rows: Sequence[Mapping[str, Any]]) -> None:
    ...
    if RUN_ID not in idea:
        idea += (
            f"\n## {RUN_ID} split-coherent profit curve source(분할 일관 수익 곡선 원천)\n\n"
    ...
    if failures:
        ...
        if RUN_ID not in negative:
            negative += (
                f"\n## {RUN_ID} Stage309 split-coherent source failure memory(309단계 분할 일관 원천 실패 기억)\n\n"
        ...
        rows.append(
            {
                ...
                "artifact_type": "stage309_split_coherent_profit_curve_source_review_artifact",
                "path": rel(path),
                "sha256": sha256_file(path),
                "stage_id": STAGE_ID,
                "run_id": RUN_ID,
                "created_at_utc": "2026-05-24T23:30:00Z",
                "notes": "Stage309 review and Stage310 open handoff",
            }
        )
...
def main() -> None:
    ...
    stage310_rows = next_stage_rows(scoreboard)
    next_stage_id, _next_status, _next_judgment, next_action = scaffold_next_stage(selected_rows)
    status = "completed_split_coherent_profit_curve_source_review_stage310_opened"
    judgment = (
        "actual_mt5_candidate_gate_passed_adapter_stage_opened"
        if selected_rows
        else "actual_mt5_no_onnx_worthy_candidate_runtime_positive_fragment_allocation_rebuild_opened"
    )
    ...
    write_csv(NEXT_STAGE_QUEUE, list(stage310_rows[0].keys()), stage310_rows)
    write_csv(
        RESULT_JUDGMENT,
        ("run_id", "status", "judgment", "selected_candidate", "adapter_package", "onnx_readiness", "next_action", "claim_boundary"),
        [{"run_id": RUN_ID, "status": status, "judgment": judgment, "selected_candidate": selected_rows[0]["package_id"] if selected_rows else "none", "adapter_package": "deferred_to_stage310" if selected_rows else "none", "onnx_readiness": "not_started", "next_action": next_action, "claim_boundary": BOUNDARY}],
    )
    gate_rows = [
    ...
    manifest = {
        ...
        "adapter_package": "deferred_to_stage310" if selected_rows else "none",
    ...
    write_text(
        ...
        "\n".join(
            [
                ...
                "",
                "Effect(효과): cp309A/cp309E(309A/309E 후보)의 양수 조각은 보존하지만 전체 ONNX-worthy(온엑스 가치 있음) 관문을 통과하지 못해 선택 후보로 선언하지 않는다.",
            ]
        ),
    )
