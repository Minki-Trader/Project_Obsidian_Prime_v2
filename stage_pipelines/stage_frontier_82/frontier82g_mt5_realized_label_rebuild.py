...
RUN_ID = "frontier82G_mt5_realized_label_rebuild_v1"
PARENT_RUN_ID = "frontier82F_deal_reconciled_runtime_label_preflight_v1"
NEXT_RUN_ID = "frontier82H_capped_repair_closeout_or_f83_rotation_decision_v1"
STATUS = "f82g_realized_label_rebuild_positive_low_density_seed_no_materialization_ready_no_authority"
JUDGMENT = "realized_label_filter_found_positive_low_density_seed_no_materialization_repair_cap_consumed_rotation_decision_required_no_authority"
CLAIM_BOUNDARY = (
...
FEATURES_CSV = STAGE_DIR / "02_runs/frontier82C_mt5_runtime_materialization_v1/features/f82c_runtime_f82b_07295_features.csv"
FEATURE_ORDER = STAGE_DIR / "02_runs/frontier82C_mt5_runtime_materialization_v1/models/f82c_runtime_f82b_07295_feature_order.txt"
F82C_RECEIPT = STAGE_DIR / "02_runs/frontier82C_mt5_runtime_materialization_v1/f82c_runtime_receipt.csv"
F82F_SUMMARY = REVIEW_DIR / "f82f_deal_reconciliation_summary.json"
F82F_TRADES = STAGE_DIR / "02_runs/frontier82F_deal_reconciled_runtime_label_preflight_v1/f82f_trade_rows.csv"

LABEL_DATASET = RUN_DIR / "f82g_mt5_realized_label_dataset.csv"
UNMATCHED_TRADES = RUN_DIR / "f82g_unmatched_trade_rows.csv"
CANDIDATE_ROWS = RUN_DIR / "f82g_realized_label_candidate_rows.csv"
TOP_CANDIDATES = REVIEW_DIR / "f82g_realized_label_top_candidates.csv"
SUMMARY = REVIEW_DIR / "f82g_mt5_realized_label_rebuild_summary.json"
REPORT = REVIEW_DIR / "frontier82G_mt5_realized_label_rebuild_report.md"
GATE_AUDIT = REVIEW_DIR / "required_gate_coverage_audit_f82g.md"
RUN_MANIFEST = RUN_DIR / "run_manifest.json"
EXPERIMENT_RECEIPT = REVIEW_DIR / "f82g_experiment_design_receipt.yaml"
DATA_RECEIPT = REVIEW_DIR / "f82g_data_integrity_receipt.yaml"
MODEL_RECEIPT = REVIEW_DIR / "f82g_model_validation_receipt.yaml"
RUN_EVIDENCE_RECEIPT = REVIEW_DIR / "f82g_run_evidence_receipt.yaml"
ARTIFACT_RECEIPT = REVIEW_DIR / "f82g_artifact_lineage_receipt.yaml"
TASK_FORCE_RECEIPT = REVIEW_DIR / "f82g_task_force_review_receipt.yaml"
LOCAL_VERIFICATION = REVIEW_DIR / "f82g_local_verification.json"
CLAIM_RECEIPT = REVIEW_DIR / "f82g_claim_discipline_receipt.yaml"
...
SCRIPT_REL = "stage_pipelines/stage_frontier_82/frontier82g_mt5_realized_label_rebuild.py"
...
def artifact_entries(created_at: str) -> list[dict[str, Any]]:
    artifacts = [
        ("label_dataset", LABEL_DATASET, "ignored_02_runs_with_tracked_hash_receipts"),
        ("unmatched_trades", UNMATCHED_TRADES, "ignored_02_runs_with_tracked_hash_receipts"),
        ("candidate_rows", CANDIDATE_ROWS, "ignored_02_runs_with_tracked_hash_receipts"),
        ("top_candidates", TOP_CANDIDATES, "tracked_review_artifact"),
        ("summary", SUMMARY, "tracked_review_artifact"),
        ("report", REPORT, "tracked_review_artifact"),
        ("gate_audit", GATE_AUDIT, "tracked_review_artifact"),
        ("run_manifest", RUN_MANIFEST, "ignored_02_runs_with_tracked_hash_receipts"),
        ("experiment_receipt", EXPERIMENT_RECEIPT, "tracked_review_artifact"),
        ("data_receipt", DATA_RECEIPT, "tracked_review_artifact"),
        ("model_receipt", MODEL_RECEIPT, "tracked_review_artifact"),
        ("run_evidence_receipt", RUN_EVIDENCE_RECEIPT, "tracked_review_artifact"),
        ("artifact_lineage_receipt", ARTIFACT_RECEIPT, "tracked_review_artifact"),
        ("task_force_receipt", TASK_FORCE_RECEIPT, "tracked_review_artifact"),
        ("claim_receipt", CLAIM_RECEIPT, "tracked_review_artifact"),
        ("local_verification", LOCAL_VERIFICATION, "tracked_review_artifact"),
        ("work_packet", WORK_PACKET, "tracked_control_packet"),
        ("skill_receipts", SKILL_RECEIPTS, "tracked_control_packet"),
        ("packet_gate_audit", PACKET_GATE_AUDIT, "tracked_control_packet"),
        ("final_claim_guard", FINAL_CLAIM_GUARD, "tracked_control_packet"),
    ]
    rows = []
        ...
        rows.append(
            {
                ...
                "notes": availability,
                "effect": "F82G evidence identity preserved without claiming runtime authority(F82G 근거 정체성을 보존하되 런타임 권위를 주장하지 않음).",
            }
        )
    return rows
...
def update_artifact_registry(created_at: str) -> None:
    for row in artifact_entries(created_at):
...
def feature_order() -> list[str]:
    return [line.strip() for line in open_path(FEATURE_ORDER).read_text(encoding="utf-8-sig").splitlines() if line.strip()]
...
def models() -> dict[str, Any]:
    return {
        ...
        "logreg_balanced_diagnostic": make_pipeline(
            StandardScaler(),
            LogisticRegression(C=0.25, max_iter=1000, class_weight="balanced", random_state=8252),
        ),
    }
...
def candidate_status(row: Mapping[str, Any]) -> dict[str, Any]:
    ...
    exportable = str(row.get("exportability")) == "exportable_current_path(현재 경로 내보내기 가능)"
...
def build_candidates(dataset: pd.DataFrame, features_used: Sequence[str], calendar_days: Mapping[str, float]) -> list[dict[str, Any]]:
    ...
    for model_name, model in models().items():
        ...
        for quantile in [0.40, 0.50, 0.60, 0.70, 0.80, 0.85, 0.90, 0.95]:
            ...
            row: dict[str, Any] = {
                "candidate_id": f"f82g_{len(rows) + 1:04d}",
                "model": model_name,
...
def build_payload(created_at: str) -> tuple[dict[str, Any], pd.DataFrame, pd.DataFrame, list[dict[str, Any]]]:
    ...
    trades = pd.read_csv(io_path(F82F_TRADES))
    ...
    return (
        {
            ...
            "selection_boundary": (
                "Post-F82C realized-trade filter only; not a standalone strategy and not runtime authority"
                "(F82C 이후 실현 거래 필터 전용, 독립 전략이나 런타임 권위 아님)."
            ),
            "next_condition": (
                "No exportable density-sufficient materialization candidate; record positive low-density seed and decide F82 closeout or F83 rotation"
                "(내보내기 가능하고 밀도 충분한 물질화 후보 없음; 양수 저밀도 씨앗을 기록하고 F82 마감 또는 F83 회전 결정)."
            ),
...
def report_text(payload: Mapping[str, Any]) -> str:
    ...
    return f"""# F82G MT5-Realized Label Rebuild(F82G MT5 실현 손익 라벨 재구축)
...
Action(행동): F82F trade rows(거래 행)를 F82C runtime feature rows(런타임 피처 행)에 붙이고, MT5 realized win/loss label(MT5 실현 승/패 라벨)로 diagnostic filter models(진단 필터 모델)를 학습했다.

Effect(효과): 기존 F82C의 손실 거래를 사후 라벨로 분석해 repair value(수리 가치)를 확인했다. 단, validation-as-train/OOS-holdout(검증 훈련/표본외 보류) 진단이므로 runtime authority(런타임 권위)나 baseline(기준선)이 아니다.
...
Interpretation(해석): F82G found a positive low-density seed(F82G는 양수 저밀도 씨앗을 찾음) but no exportable density-sufficient candidate(내보내기 가능하고 밀도 충분한 후보 없음). Effect(효과): F82 repair cap(수리 상한)은 소모됐고 F82H(전선82H)는 closeout or rotation decision(마감 또는 회전 결정)을 해야 한다.
...
def gate_audit_text(payload: Mapping[str, Any]) -> str:
    return f"""# F82G Required Gate Coverage Audit(F82G 필수 게이트 커버리지 감사)
...
| `skill_receipt_lint` | `passed(통과)` | `{rel(RUN_EVIDENCE_RECEIPT)}`, `{rel(MODEL_RECEIPT)}` | experiment/data/model/artifact/task-force receipts(실험/데이터/모델/산출물/태스크포스 영수증)를 남겼다. |
| `codex_task_force_review_packet` | `passed(통과)` | `{rel(TASK_FORCE_RECEIPT)}` | 8명 agent(요원) 검토를 Project-native Codex Task Force(프로젝트 전용 코덱스 태스크포스) 경로로 기록했다. |
...
hypothesis: "MT5 realized PnL labels can identify a subset of F82C trades worth preserving(MT5 실현 손익 라벨이 F82C 거래 중 보존할 부분집합을 찾을 수 있다)."
decision_use: F82 closeout or F82 rotation decision(F82 마감 또는 F82 회전 결정)
comparison_baseline: F82C unfiltered runtime trades(F82C 무필터 런타임 거래)
control_variables:
  - F82C feature set(피처 묶음)
  - F82C executed trade set(실행 거래 집합)
changed_variables:
...
negative_memory_required: pending_f82h_decision(F82H 결정 대기)
...
status: passed_project_native_review_no_authority
...
agent_opinions:
  - agent: agent_01_system_governor
    opinion: "F82G may record a diagnostic seed only; forbid completion/baseline/promotion/runtime authority(F82G는 진단 씨앗만 기록 가능, 완성/기준선/승격/런타임 권위 금지)."
    disposition: accepted
  - agent: agent_04_evidence_control_plane
    opinion: "Ignored 02_runs artifacts require tracked hashes and registry identity(무시된 02_runs 산출물은 추적 해시와 등록부 정체성이 필요)."
    disposition: accepted
  - agent: agent_05_data_feature_contract
    opinion: "Entry-time feature and post-trade label boundary must be explicit(진입 시점 피처와 거래 후 라벨 경계를 명시해야 함)."
    disposition: accepted
  - agent: agent_06_quant_research
    opinion: "Positive low-density seed is useful clue, not a materialization candidate(양수 저밀도 씨앗은 단서이지 물질화 후보가 아님)."
    disposition: accepted
  - agent: agent_07_model_validation_risk
    opinion: "Validation-as-train diagnostic has high overfit risk and cannot promote(검증 훈련 진단은 과적합 위험이 높아 승격 불가)."
    disposition: accepted
  - agent: agent_08_mt5_onnx_runtime
    opinion: "No exportable density-sufficient candidate means no ONNX handoff in this packet(내보내기 가능하고 밀도 충분한 후보가 없으므로 이 묶음의 ONNX 인계 없음)."
    disposition: accepted
local_verification_required: {rel(LOCAL_VERIFICATION)}
task_force_judgment: accepted_with_local_verification_boundary(로컬 검증 경계로 수용)
effect: "Agent review is attached to gates and receipts instead of being treated as informal commentary(요원 검토를 비공식 코멘트가 아니라 게이트와 영수증에 연결)."
""",
...
required_gates:
  ...
  - final_claim_guard
scope: "Build MT5 realized label diagnostic filters from F82F trade evidence(F82F 거래 근거로 MT5 실현 라벨 진단 필터 생성)."
status: {STATUS}
judgment: {JUDGMENT}
next_run_id: {NEXT_RUN_ID}
claim_boundary: {CLAIM_BOUNDARY}
...
def packet_receipts_json(payload: Mapping[str, Any]) -> dict[str, Any]:
    return {
        ...
        "support_skills": [
            ...
            {"name": "obsidian-task-force-review", "status": "passed_project_native_review_no_authority", "evidence": rel(TASK_FORCE_RECEIPT)},
        ],
...
def packet_gate_json() -> dict[str, Any]:
    return {
        ...
        "gates": [
            ...
            {"gate": "skill_receipt_lint", "status": "passed", "evidence": [rel(EXPERIMENT_RECEIPT), rel(DATA_RECEIPT), rel(MODEL_RECEIPT), rel(RUN_EVIDENCE_RECEIPT), rel(ARTIFACT_RECEIPT), rel(TASK_FORCE_RECEIPT)]},
...
def final_claim_guard_json(payload: Mapping[str, Any]) -> dict[str, Any]:
    return {
        ...
        "claim_boundary": CLAIM_BOUNDARY,
        "forbidden_claims": payload.get("forbidden_claims"),
        "effect": "F82G is diagnostic seed evidence only(F82G는 진단 씨앗 근거 전용).",
    }


def local_verification_json(payload: Mapping[str, Any], created_at: str) -> dict[str, Any]:
    checks = [
        {
            "name": "matched_trade_rows_present",
            "passed": int(payload.get("matched_trade_rows") or 0) > 0,
            "observed": payload.get("matched_trade_rows"),
            "effect": "Label join produced analyzable rows(라벨 결합이 분석 가능한 행을 생성).",
        },
        {
            "name": "unmatched_trade_rows_recorded",
            "passed": payload.get("unmatched_trade_rows") is not None and path_exists(UNMATCHED_TRADES),
            "observed": payload.get("unmatched_trade_rows"),
            "effect": "Join gap is recorded instead of hidden(결합 간극을 숨기지 않고 기록).",
        },
        {
            "name": "positive_seed_no_materialization_boundary",
            "passed": int(payload.get("positive_low_density_seed_count") or 0) > 0 and int(payload.get("materialization_candidate_count") or 0) == 0,
            "observed": {
            ...
            "effect": "Useful clue is separated from ONNX handoff readiness(유용한 단서와 ONNX 인계 준비를 분리).",
        },
        {
            "name": "task_force_receipt_written",
            "passed": path_exists(TASK_FORCE_RECEIPT),
            "observed": rel(TASK_FORCE_RECEIPT),
            "effect": "Agent review is materialized as evidence(요원 검토가 근거로 물질화).",
        },
        {
            "name": "final_claim_guard_written",
            "passed": path_exists(FINAL_CLAIM_GUARD),
            "observed": rel(FINAL_CLAIM_GUARD),
            "effect": "Forbidden claims remain blocked(금지 주장을 계속 차단).",
        },
    ]
    return {
        "packet_id": RUN_ID,
        "created_at_utc": created_at,
        "status": "passed_with_boundary" if all(check["passed"] for check in checks) else "failed",
        "checks": checks,
        "claim_boundary": CLAIM_BOUNDARY,
        "next_run_id": NEXT_RUN_ID,
        "local_verification_judgment": "positive_low_density_seed_only_no_authority",
    }
...
def ledger_rows(payload: Mapping[str, Any], created_at: str) -> list[dict[str, Any]]:
    ...
    base = {
        ...
        "source_authority": "F82F realized trade labels(F82F 실현 거래 라벨)",
        ...
        "materialization_candidate_count": payload.get("materialization_candidate_count"),
        "oos_net_profit": best.get("oos_net_profit"),
        ...
        "oos_drawdown_percent": best.get("oos_drawdown_percent"),
    }
    return [
        {
            **base,
        ...
        },
        {
            **base,
            ...
            "guardrail_kpi": "No Tier B realized trade source in F82C/F82F(F82C/F82F에 티어 B 실현 거래 원천 없음)",
...
next_run_id: {NEXT_RUN_ID}
runtime_probe_status: f82g_positive_low_density_seed_rotation_decision_required_no_authority
runtime_authority: not_claimed
operating_promotion: not_claimed
...
notes:
  - "Action(행동): F82G MT5-realized label rebuild(MT5 실현 라벨 재구축) 진단을 완료했다."
...
Action(행동): F82G realized label diagnostic(F82G 실현 라벨 진단)을 기록했다.

Effect(효과): F82 repair cycle(F82 수리 회차)은 저밀도 씨앗을 남겼지만 materialization-ready candidate(물질화 준비 후보)는 없어서 F82H closeout or rotation decision(F82H 마감 또는 회전 결정)이 필요하다.
...
def update_changelog(payload: Mapping[str, Any]) -> None:
    ...
    if RUN_ID in text:
        return
    best = payload.get("best_candidate") or {}
    entry = f"""# 2026-06-18 - F82G MT5-Realized Label Diagnostic(F82G MT5 실현 라벨 진단)

- Action(행동): `{RUN_ID}`로 F82F trade labels(거래 라벨)을 F82C features(피처)에 붙여 진단 필터를 학습했다.
- Effect(효과): best seed(최선 씨앗) `{best.get('candidate_id')}`는 OOS net/PF/trades-day(표본외 순손익/수익 팩터/일 거래) `{best.get('oos_net_profit')}/{best.get('oos_profit_factor')}/{best.get('oos_trades_per_day')}`를 보였지만, materialization-ready candidate(물질화 준비 후보)는 `{payload.get('materialization_candidate_count')}`개라 F82H 결정이 필요하다.
- Next(다음): `{NEXT_RUN_ID}`.
- Boundary(경계): no completion/baseline/promotion/runtime authority/live readiness/Goal Achieve(완성/기준선/승격/런타임 권위/실거래 준비/목표 달성 없음).
...
def main() -> int:
    ...
    write_json(LOCAL_VERIFICATION, local_verification_json(payload, created_at))
