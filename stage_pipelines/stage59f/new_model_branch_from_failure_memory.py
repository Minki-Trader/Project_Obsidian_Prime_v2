...
NEXT_REPAIR_STAGE_ID = "59G_adapter_repair__bounded_followup_from_stage59f"
RUN_NUMBER = "run59A"
RUN_ID = "run59A_stage59f_new_model_branch_from_failure_memory_v1"
PACKET_ID = "stage59f_new_model_branch_from_failure_memory_v1"
PARENT_RUN_ID = "run58A_stage59e_demotion_or_new_branch_v1"
...
STAGE_ROOT = Path("stages") / STAGE59F_ID
...
COMMON_ROOT = f"Project_Obsidian_Prime_v2/s59f/{RUN_NUMBER}"

REPORT_PATH = REVIEWS_ROOT / "new_model_branch_from_failure_memory_report.md"
SUMMARY_JSON_PATH = REVIEWS_ROOT / "new_model_branch_summary.json"
SUMMARY_CSV_PATH = REVIEWS_ROOT / "new_model_branch_summary.csv"
SEGMENT_KPI_PATH = REVIEWS_ROOT / "new_model_branch_segment_kpi_summary.csv"
EQUITY_AUDIT_PATH = REVIEWS_ROOT / "new_model_branch_equity_curve_audit.md"
RISK_ATR_TELEMETRY_PATH = REVIEWS_ROOT / "new_model_branch_risk_atr_telemetry.csv"
DECISION_PATH = REVIEWS_ROOT / "stage59f_decision.md"
AUDIT_CSV_PATH = REVIEWS_ROOT / "stage59f_trade_audit.csv"
...
STAGE59F_VARIANTS = (
    repair.RepairVariant(
        adapter_id="s59f_v47_coo",
        label="run50BN_v47_close_only_opposite_threshold57_risk3pct_wide_atr_sd5",
        atr_enabled=True,
        ...
        max_hold_bars=2,
        notes="Stage59F bounded new branch: run50BN v47 source with Stage59D close-only-on-opposite failure-memory clue, 3% model risk cap, and wide ATR bracket.",
    ),
    repair.RepairVariant(
        adapter_id="s59f_v50_coo",
        label="run50BO_v50_close_only_opposite_threshold57_risk3pct_wide_atr_sd5",
        atr_enabled=True,
        model_risk_enabled=True,
        fixed_lot=0.25,
        atr_stop_multiplier=2.5,
        ...
        close_only_on_opposite_signal=True,
        max_hold_bars=2,
        notes="Stage59F bounded new branch: run50BO v50 same-direction-cooldown source with Stage59D close-only-on-opposite clue.",
    ),
    repair.RepairVariant(
        adapter_id="s59f_v54_coo",
        label="run50BP_v54_slot_lifecycle_close_only_opposite_threshold57_risk3pct_wide_atr_sd5",
        atr_enabled=True,
        model_risk_enabled=True,
        fixed_lot=0.25,
        atr_stop_multiplier=2.5,
        ...
        close_only_on_opposite_signal=True,
        max_hold_bars=2,
        notes="Stage59F bounded new branch: run50BP v54 ET40 slot-lifecycle source with Stage59D close-only-on-opposite clue.",
    ),
)
...
def source_specs() -> dict[str, dict[str, Any]]:
    return {
        "s59f_v47_coo": {
            ...
            "variant_root": RUN50BN_ROOT / RUN50BN_ANCHOR,
            "anchor": RUN50BN_ANCHOR,
            "model": RUN50BN_ROOT / "models/stage56_context_timed_event_signal_discrete_score_table.csv",
            "signal_column": RUN50BN_SIGNAL,
            "validation_ini": RUN50BN_ROOT / RUN50BN_ANCHOR / "mt5/x07_ta_val.ini",
            "oos_ini": RUN50BN_ROOT / RUN50BN_ANCHOR / "mt5/x07_ta_oos.ini",
            "source_note": "Stage56 run50BN v47 topup plus ET40 slot-fill source",
        },
        "s59f_v50_coo": {
            "label": "run50bo_v50",
            "run_root": RUN50BO_ROOT,
            "variant_root": RUN50BO_ROOT / RUN50BO_ANCHOR,
            "anchor": RUN50BO_ANCHOR,
            "model": RUN50BO_ROOT / "models/stage56_context_timed_event_signal_discrete_score_table.csv",
            "signal_column": RUN50BO_SIGNAL,
            "validation_ini": RUN50BO_ROOT / RUN50BO_ANCHOR / "mt5/x03_ta_val.ini",
            "oos_ini": RUN50BO_ROOT / RUN50BO_ANCHOR / "mt5/x03_ta_oos.ini",
            "source_note": "Stage56 run50BO v50 topup slot-fill same-direction cooldown source",
        },
        "s59f_v54_coo": {
            "label": "run50bp_v54",
...
def configure_reused_engine() -> None:
    ...
    engine.STAGE59_VARIANTS = STAGE59F_VARIANTS
    engine.SOURCE_SPECS = source_specs()
    engine.MODEL_RISK_MIN_PCT = {variant.adapter_id: 0.005 for variant in STAGE59F_VARIANTS}

    repair.STAGE_ID = STAGE59F_ID
    repair.RUN_NUMBER = RUN_NUMBER
    repair.RUN_ID = RUN_ID
    repair.RUN_ROOT = RUN_ROOT
    repair.REPAIR_VARIANTS = STAGE59F_VARIANTS
    s58.STAGE58_ID = STAGE59F_ID
    ...
    s58.STAGE58_VARIANTS = STAGE59F_VARIANTS
...
def build_attempts(inputs: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    for variant_index, variant in enumerate(STAGE59F_VARIANTS, start=1):
            ...
            for role_index, (tier, attempt_role, prefix, attempt_token) in enumerate(
                ...
                attempts.append(
                    attempt_payload(
                        ...
                        run_id=RUN_ID,
                        stage_number=59,
                        exploration_label="stage59F_BaselineAdapter__NewModelBranchFromFailureMemory",
...
def execute_prepared_run_checkpointed(prepared: Mapping[str, Any], args: argparse.Namespace) -> dict[str, Any]:
    ...
    return {
        **dict(prepared),
        ...
        "judgment": "inconclusive_stage59f_runtime_probe_completed" if completed and report_completed else "blocked_stage59f_runtime_probe",
    }
...
def next_stage_for_decision(decision: str) -> str:
    ...
    if decision == "demote_current_adapter_and_select_backup":
        return "59G_adapter_repair__backup_or_demote_from_stage59f"
    if decision == "open_new_model_branch":
        return "59G_adapter_repair__new_model_branch_from_stage59f"
...
def next_packet_for_decision(decision: str) -> tuple[str, str]:
    ...
    return "stage59g_bounded_followup_from_stage59f_v1", "run59B_stage59g_bounded_followup_from_stage59f_v1"
            ...
            rows.append(
                {
                    ...
                    "run_id": RUN_ID,
                    "artifact_type": "stage59f_new_model_branch_evidence",
                    "path": rel(path),
                    "sha256": sha256_file_lf_normalized(path),
                    "created_at_utc": created,
                    "notes": "Stage59F bounded new model branch from failure memory artifact.",
                }
            )
            ...
            rows.append(
                {
                    ...
                    "created_at_utc": created,
                    "notes": "Actual Stage59F MT5 Strategy Tester HTML report.",
                }
            )
    return rows
...
def write_run_identity(result: Mapping[str, Any]) -> None:
    engine.write_json(
        RUN_ROOT / "run_manifest.json",
        {
            ...
            "bounded_question": "Can a new bounded model branch, informed by Stage59D/59E failure memory, produce a post-ATR/risk adapter candidate without starting ONNX?",
            "source_stage59e_decision": rel(SOURCE_STAGE59E_DECISION),
            ...
            "variants": [
                ...
                for variant in STAGE59F_VARIANTS
            ],
...
def write_ledgers(
        ...
        [
            {
                "run_id": RUN_ID,
                "stage_id": STAGE59F_ID,
                "lane": "baseline_adapter_new_model_branch_from_failure_memory",
                "status": status,
                "judgment": decision,
    ...
    ledger_rows.append(
        {
            "ledger_row_id": f"{RUN_ID}__aggregate_new_model_branch_from_failure_memory",
            "stage_id": STAGE59F_ID,
            "run_id": RUN_ID,
            "subrun_id": "aggregate_new_model_branch_from_failure_memory",
            "parent_run_id": PARENT_RUN_ID,
            "record_view": "new_model_branch_from_failure_memory",
            "tier_scope": "Tier A+B",
            ...
            "external_verification_status": external,
            "notes": "Stage59F bounded new model branch from Stage59D/59E failure memory; not final package completion.",
        }
    )
...
def parse_args(argv: Sequence[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run Stage59F bounded new model branch from failure memory.")
...
def load_existing_result() -> dict[str, Any]:
    ...
    if not path_exists(manifest) or not path_exists(kpi):
        raise FileNotFoundError("Stage59F existing run_manifest.json or kpi_record.json is missing")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.finalize_existing:
        ...
        prepared = {
            ...
            "model_family": "baseline_adapter_stage59f_new_model_branch_ebm_table",
            "feature_set_id": "stage59f_source_family_discrete_signal",
