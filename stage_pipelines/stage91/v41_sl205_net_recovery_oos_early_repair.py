...
STAGE_ID = "91_adapter_research__v41_sl205_net_recovery_oos_early_repair"
RUN_NUMBER = "run91A"
RUN_ID = "run91A_stage91_v41_sl205_net_recovery_oos_early_repair_v1"
PACKET_ID = "stage91_v41_sl205_net_recovery_oos_early_repair_v1"
PARENT_RUN_ID = "run90A_stage90_v41_drawdown_oos_early_followup_review_v1"
SOURCE_STAGE90_ID = "90_adapter_research__v41_drawdown_oos_early_followup_review"
SOURCE_STAGE90_CLOSEOUT_COMMIT = "7a3f7a77ef1b96908c52547af3247a0c899f0be5"
SOURCE_STAGE90_LATEST_COMMIT = "80930456ad3951f9ab4ec4a52d1dc9583a0fcf96"
SOURCE_STAGE89_CLOSEOUT_COMMIT = "50f767c3ae9c18f36a53e4ec95588299e61f5dc0"
SOURCE_STAGE89_LATEST_COMMIT = "f0b6a5eb755b750cb5bc805c5d74bebbba23b1c3"
NEXT_STAGE_ID = "92_adapter_research__v41_sl205_net_recovery_followup_review"
NEXT_RUN_ID = "run92A_stage92_v41_sl205_net_recovery_followup_review_v1"
NEXT_PACKET_ID = "stage92_v41_sl205_net_recovery_followup_review_v1"
SOURCE_ADAPTER_ID = "s89_v41_h3_risk475_gate08_sl205_tp38_cd10"
...
COMMON_ROOT = f"OPV2/s91a/{RUN_NUMBER}"

SUMMARY_JSON_PATH = REVIEWS_ROOT / "stage91_v41_sl205_net_recovery_oos_early_repair_summary.json"
SUMMARY_CSV_PATH = REVIEWS_ROOT / "stage91_v41_sl205_net_recovery_oos_early_repair_summary.csv"
REPORT_PATH = REVIEWS_ROOT / "stage91_v41_sl205_net_recovery_oos_early_repair_report.md"
SEGMENT_KPI_PATH = REVIEWS_ROOT / "stage91_segment_kpi_summary.csv"
RISK_ATR_TELEMETRY_PATH = REVIEWS_ROOT / "stage91_risk_atr_telemetry.csv"
GATE_FEATURE_SUMMARY_PATH = REVIEWS_ROOT / "stage91_gate_feature_summary.csv"
TIER_B_DIAGNOSTIC_PATH = REVIEWS_ROOT / "stage91_tier_b_diagnostic_summary.csv"
DECISION_PATH = REVIEWS_ROOT / "stage91_decision.md"
AUDIT_CSV_PATH = REVIEWS_ROOT / "stage91_trade_audit.csv"
...
VARIANTS = (
    repair.RepairVariant(
        adapter_id="s91_v41_h3_risk475_gate08_sl205_tp40_cd10",
        label="run50BN_v41_h3_risk475_gate08_sl205_tp40_cd10",
        atr_enabled=True,
        ...
        max_hold_bars=3,
        notes="Stage91 SL2.05 net recovery repair: keep SL2.05 and risk4.75 while testing TP4.0 recovery.",
    ),
    repair.RepairVariant(
        adapter_id="s91_v41_h3_risk45_gate08_sl205_tp38_cd10",
        label="run50BN_v41_h3_risk45_gate08_sl205_tp38_cd10",
        atr_enabled=True,
        model_risk_enabled=True,
        fixed_lot=0.25,
        atr_stop_multiplier=2.05,
        ...
        close_only_on_opposite_signal=True,
        max_hold_bars=3,
        notes="Stage91 SL2.05 net recovery repair: combine SL2.05 with lower risk cap 4.5 and TP3.8.",
    ),
    repair.RepairVariant(
        adapter_id="s91_v41_h3_risk475_gate08_sl210_tp38_cd10",
        label="run50BN_v41_h3_risk475_gate08_sl210_tp38_cd10",
        atr_enabled=True,
        model_risk_enabled=True,
        fixed_lot=0.25,
        ...
        close_only_on_opposite_signal=True,
        max_hold_bars=3,
        notes="Stage91 SL2.05 net recovery repair: test middle stop SL2.10 to recover net while limiting drawdown.",
    ),
)
...
CONTEXT_GATE_SPECS = {
    variant.adapter_id: {
        "gate_column": "stage91_gate_margin_lt_008_short",
        "gate_type": "margin",
        "threshold": 0.08,
        "block_mode": "short",
...
def decide(summary_rows: Sequence[Mapping[str, Any]], segment_rows: Sequence[Mapping[str, Any]], external: str) -> str:
    if external != "completed":
        return "continue_stage91_due_to_incomplete_runtime"
    ...
    if val_net >= 900 and oos_net >= 560 and val_dd <= 22.8 and oos_dd <= 20.8 and val_pf >= 1.50 and oos_pf >= 1.50 and not reasons:
        return "proceed_to_stage92_sl205_net_recovery_followup_review"
    if val_net >= 820 and oos_net >= 500 and val_dd <= 24.8 and oos_dd <= 21.8 and val_pf >= 1.42 and oos_pf >= 1.45:
        return "continue_sl205_net_recovery_followup_review_in_stage92"
    if val_pf < 1.0 or val_net <= 0:
        return "open_new_model_source_branch_in_stage92"
    return "continue_sl205_net_recovery_followup_review_in_stage92"
...
def report_markdown(
    ...
    return f"""# Stage91 V41 SL2.05 Net Recovery/OOS Early Repair Report(91단계 V41 손절 2.05 순손익 회복/표본외 초반 수리 보고서)

- run(실행): `{RUN_ID}`
- target_surface(목표 표면): `{TARGET_SURFACE}`
- source_stage(원천 단계): `{SOURCE_STAGE90_ID}`
- source_stage90_closeout_commit(원천 90단계 종료 커밋): `{SOURCE_STAGE90_CLOSEOUT_COMMIT}`
- source_stage90_latest_commit(원천 90단계 최신 커밋): `{SOURCE_STAGE90_LATEST_COMMIT}`
- source_stage89_closeout_commit(원천 89단계 종료 커밋): `{SOURCE_STAGE89_CLOSEOUT_COMMIT}`
- source_stage89_latest_commit(원천 89단계 최신 커밋): `{SOURCE_STAGE89_LATEST_COMMIT}`
...
def decision_markdown(decision: str, external: str) -> str:
    return f"""# Stage91 Decision(91단계 판정)

decision(판정): `{decision}`

Stage91(91단계)는 Stage90(90단계) 판정에 따라 SL2.05(손절 2.05)의 DD(손실률) 압축을 보존하면서 validation net/PF(검증 순손익/수익 팩터)와 OOS early(표본외 초반)를 좁게 수리했다.
            ...
            rows.append(
                {
                    ...
                    "split": split_name,
                    "tier_b_policy": "diagnostic_missing_required_but_disabled_for_this_v41_sl205_net_recovery_oos_early_repair",
                    ...
                    "reason": "Stage91 isolates v41 SL2.05 net recovery/OOS early first; Tier B fallback remains diagnostic and disabled.",
                }
            )
    return rows
            ...
            rows.append(
                {
                    ...
                    "artifact_type": "stage91_v41_sl205_net_recovery_oos_early_repair_evidence",
                    "path": rel(path),
                    "sha256": sha256_file_lf_normalized(path),
                    "stage_id": STAGE_ID,
                    "run_id": RUN_ID,
                    "created_at_utc": created,
                    "notes": "Stage91 v2-native v41 SL2.05 net recovery/OOS early repair artifact.",
                }
            )
            ...
            rows.append(
                {
                    ...
                    "run_id": RUN_ID,
                    "created_at_utc": created,
                    "notes": "Actual Stage91 MT5 Strategy Tester HTML report.",
                }
            )
    return rows
...
def write_run_identity(result: Mapping[str, Any]) -> None:
    write_json(
        RUN_ROOT / "run_manifest.json",
        {
            ...
            "source_stage90_closeout_commit": SOURCE_STAGE90_CLOSEOUT_COMMIT,
            "source_stage90_latest_commit": SOURCE_STAGE90_LATEST_COMMIT,
            "source_stage89_closeout_commit": SOURCE_STAGE89_CLOSEOUT_COMMIT,
            "source_stage89_latest_commit": SOURCE_STAGE89_LATEST_COMMIT,
        ...
        [
            {
                "run_id": RUN_ID,
                "stage_id": STAGE_ID,
                "lane": "baseline_adapter_v2_native_v41_sl205_net_recovery_oos_early_repair",
                "status": status,
                "judgment": decision,
        ...
        alpha_rows = [
            {
                ...
                "tier_scope": "Tier A+B",
                "kpi_scope": "stage91_v41_sl205_net_recovery_oos_early_repair",
                ...
                "external_verification_status": external,
                "notes": "Stage91 run materialized or blocked before KPI records were available.",
            }
        ]
...
def write_packet_files(result: Mapping[str, Any], decision: str, ledger_payload: Mapping[str, Any]) -> None:
    ...
    write_json(PACKET_ROOT / "aggregate_summary.json", {"packet_id": PACKET_ID, "stage_id": STAGE_ID, "run_id": RUN_ID, "decision": decision, "source_stage90_closeout_commit": SOURCE_STAGE90_CLOSEOUT_COMMIT, "source_stage90_latest_commit": SOURCE_STAGE90_LATEST_COMMIT, "source_stage89_closeout_commit": SOURCE_STAGE89_CLOSEOUT_COMMIT, "source_stage89_latest_commit": SOURCE_STAGE89_LATEST_COMMIT, "gate_feature_summary_path": rel(GATE_FEATURE_SUMMARY_PATH), "ledger_payload": ledger_payload, "overall_goal_complete": False})
...
Stage92(92단계)는 Stage91(91단계)의 SL2.05 net recovery/OOS early repair(손절 2.05 순손익 회복/표본외 초반 수리) 결과를 review gate(검토 게이트)로만 판독하는 follow-up(후속) 단계다.

## Bounded Question(경계 질문)

Stage91(91단계)의 TP recovery/risk balance/middle stop(익절 회복/위험 균형/중간 손절) 조합이 SL2.05 DD compression(손절 2.05 손실률 압축)을 보존하면서 validation net/PF(검증 순손익/수익 팩터)와 OOS early(표본외 초반)를 충분히 회복했는가?

Effect(효과): Stage92(92단계)는 Stage91(91단계)의 결과를 과장하지 않고 다음 판단으로만 좁힌다.
...
""",
    ...
    write_md(
        ...
        f"""# Stage92 Input References(92단계 입력 참조)

- source_stage(원천 단계): `{STAGE_ID}`
- source_run(원천 실행): `{RUN_ID}`
- source_decision(원천 판정): `{decision}`
- source_external_verification_status(원천 외부 검증 상태): `{external}`
- stage91_report(91단계 보고서): `{rel(REPORT_PATH)}`
- stage91_decision(91단계 판정): `{rel(DECISION_PATH)}`
- summary(요약): `{rel(SUMMARY_CSV_PATH)}`
- target_surface(목표 표면): `{TARGET_SURFACE}`

Effect(효과): Stage92(92단계)는 v2 고유 근거만 이어받아 34D KPI(34D 핵심 성과 지표) 격차 축소를 계속한다.
""",
    )
    write_md(NEXT_STAGE_ROOT / "03_reviews" / "review_index.md", f"""# Stage92 Review Index(92단계 검토 색인)
...
Effect(효과): Stage92(92단계)는 Stage91(91단계) closeout(종료 기록)을 이어받아 다음 bounded review(경계 검토)만 수행한다.
""")
    write_md(NEXT_STAGE_ROOT / "04_selected" / "selection_status.md", f"""# Stage92 Selection Status(92단계 선택 상태)
...
Effect(효과): Stage92(92단계)는 34D KPI(34D 핵심 성과 지표) 격차 축소를 계속하지만, 운영 의미 없이 연구개발로만 이어진다.
""")
...
- >-
  Stage91(91단계) closed(종료) as `{decision}` and Stage92(92단계) `{NEXT_STAGE_ID}` is open_planned(열림 계획). Effect(효과): v41 SL2.05 net recovery/OOS early repair(V41 손절 2.05 순손익 회복/표본외 초반 수리) 근거를 보존하고 다음 경계 연구 질문으로만 넘긴다.
- >-
  Stage91 result(91단계 결과): validation/OOS(검증/표본외), PF/net/DD(수익 팩터/순손익/손실률), risk/ATR telemetry(위험/ATR 텔레메트리)는 `{rel(SUMMARY_CSV_PATH)}`와 `{rel(RISK_ATR_TELEMETRY_PATH)}`에 기록된다. Effect(효과): 34D target surface(34D 목표 표면) 대비 KPI(핵심 성과 지표) 차이를 다음 단계 입력으로 보존한다.
- >-
...
stage91_v41_sl205_net_recovery_oos_early_repair:
  packet_id: {PACKET_ID}
  stage_id: {STAGE_ID}
  ...
  boundary: {BOUNDARY}
"""
    if "stage91_v41_sl205_net_recovery_oos_early_repair:" in text:
        text = re.sub(r"\nstage91_v41_sl205_net_recovery_oos_early_repair:\n(?:  .*\n)+", block + "\n", text, count=1)
    else:
        text = text.rstrip() + block
    io_path(WORKSPACE_STATE_PATH).write_text(text + ("\n" if not text.endswith("\n") else ""), encoding="utf-8-sig")
    write_md(SELECTED_ROOT / "selection_status.md", f"""# Stage91 Selection Status(91단계 선택 상태)
...
- target_surface(목표 표면): `{TARGET_SURFACE}`
- stage91_decision(91단계 판정): `{decision}`
- next_stage_or_branch(다음 단계/분기): `{NEXT_STAGE_ID}`
- claim_boundary(주장 경계): `{BOUNDARY}`

Effect(효과): Stage91(91단계)는 v41 SL2.05 net recovery/OOS early repair(V41 손절 2.05 순손익 회복/표본외 초반 수리)를 측정하고, 운영 의미 없이 Stage92(92단계)로 넘긴다.
""")
...
- target_surface(목표 표면): `{TARGET_SURFACE}`
- adapter_under_review(검토 중 어댑터): `{SOURCE_ADAPTER_ID}`
- status(상태): `stage91_closed_{decision}`
- claim_boundary(주장 경계): `{BOUNDARY}`

Stage91(91단계) closed(종료) as v2-native v41 SL2.05 net recovery/OOS early repair batch(브이투 고유 브이41 손절 2.05 순손익 회복/표본외 초반 수리 묶음). Effect(효과): legacy 34D(레거시 34D)는 target surface(목표 표면)로만 쓰였고, 다음 연구는 Stage92(92단계)로 이어진다.

## Latest Stage91 Evidence(최신 91단계 근거)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    prepared = {
        ...
        "model_family": "baseline_adapter_stage91_v2_native_v41_sl205_net_recovery_oos_early_repair_ebm_table",
        "feature_set_id": "stage91_v41_sl205_net_recovery_oos_early_signal",
