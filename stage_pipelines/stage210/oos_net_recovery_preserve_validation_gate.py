...
STAGE_ID = "210_adapter_research__oos_net_recovery_preserve_stage208_validation_gate"
RUN_NUMBER = "run210A"
RUN_ID = "run210A_stage210_oos_net_recovery_preserve_stage208_validation_gate_v1"
PACKET_ID = "stage210_oos_net_recovery_preserve_stage208_validation_gate_v1"
PARENT_RUN_ID = "run209A_stage209_stage208_risk_cap_interpolation_followup_review_v1"
SOURCE_STAGE_ID = "209_adapter_research__stage208_risk_cap_interpolation_followup_review"
SOURCE_RUN_ID = "run209A_stage209_stage208_risk_cap_interpolation_followup_review_v1"
SOURCE_STAGE209_EVIDENCE_COMMIT = "dd8b5b16624a94518982f3705e57bb42f4935eb3"
SOURCE_STAGE209_HASH_RECORD_COMMIT = "e1ec3157f10ade76ed6e907f830454530f40d2ee"
SOURCE_ADAPTER_ID = "s208_ls_r0305"
NEXT_STAGE_ID = "211_adapter_research__stage210_oos_net_recovery_followup_review"
NEXT_RUN_ID = "run211A_stage211_stage210_oos_net_recovery_followup_review_v1"
NEXT_PACKET_ID = "stage211_stage210_oos_net_recovery_followup_review_v1"
...
COMMON_ROOT = f"OPV2/s210a/{RUN_NUMBER}"

SUMMARY_JSON_PATH = REVIEWS_ROOT / "stage210_oos_net_recovery_summary.json"
SUMMARY_CSV_PATH = REVIEWS_ROOT / "stage210_oos_net_recovery_kpi_summary.csv"
REPORT_PATH = REVIEWS_ROOT / "stage210_oos_net_recovery_report.md"
SEGMENT_KPI_PATH = REVIEWS_ROOT / "stage210_segment_kpi_summary.csv"
BALANCE_CURVE_AUDIT_PATH = REVIEWS_ROOT / "stage210_balance_curve_audit.csv"
MONTHLY_KPI_PATH = REVIEWS_ROOT / "stage210_monthly_kpi_summary.csv"
CONCENTRATION_PATH = REVIEWS_ROOT / "stage210_concentration_risk_summary.csv"
DRAWDOWN_PATH = REVIEWS_ROOT / "stage210_drawdown_recovery_summary.csv"
QUALITY_MATRIX_PATH = REVIEWS_ROOT / "stage210_quality_matrix.csv"
RISK_ATR_TELEMETRY_PATH = REVIEWS_ROOT / "stage210_risk_atr_telemetry.csv"
GATE_FEATURE_SUMMARY_PATH = REVIEWS_ROOT / "stage210_gate_feature_summary.csv"
PROBABILITY_BINDING_PATH = REVIEWS_ROOT / "stage210_probability_telemetry_summary.csv"
MODEL_SCORE_AUDIT_PATH = REVIEWS_ROOT / "stage210_model_score_audit.csv"
TIER_B_DIAGNOSTIC_PATH = REVIEWS_ROOT / "stage210_tier_b_diagnostic_summary.csv"
DECISION_PATH = REVIEWS_ROOT / "stage210_decision.md"
AUDIT_CSV_PATH = REVIEWS_ROOT / "stage210_trade_audit.csv"
...
PRODUCER_PATH = Path("stage_pipelines/stage210/oos_net_recovery_preserve_validation_gate.py")
ARTIFACT_COLUMNS = s172.ARTIFACT_COLUMNS

VARIANTS = (
    repair.RepairVariant(
        adapter_id="s210_ls_r0310",
        label="stage210_ls_r0310",
        atr_enabled=True,
        ...
        max_hold_bars=3,
        notes="Stage210 upper-risk OOS recovery: long-session structure with model risk cap 3.10%.",
    ),
    repair.RepairVariant(
        adapter_id="s210_ls_r03125",
        label="stage210_ls_r03125",
        atr_enabled=True,
        model_risk_enabled=True,
        fixed_lot=0.25,
        atr_stop_multiplier=2.075,
        ...
        close_only_on_opposite_signal=False,
        max_hold_bars=3,
        notes="Stage210 upper-risk OOS recovery: long-session structure with model risk cap 3.125%.",
    ),
    repair.RepairVariant(
        adapter_id="s210_ls_r0315",
        label="stage210_ls_r0315",
        atr_enabled=True,
        model_risk_enabled=True,
        fixed_lot=0.25,
        atr_stop_multiplier=2.075,
        ...
        close_only_on_opposite_signal=False,
        max_hold_bars=3,
        notes="Stage210 upper-risk OOS recovery: long-session structure with model risk cap 3.15%.",
    ),
    repair.RepairVariant(
        adapter_id="s210_ls_r03175",
        label="stage210_ls_r03175",
        atr_enabled=True,
        model_risk_enabled=True,
        fixed_lot=0.25,
        atr_stop_multiplier=2.075,
        atr_take_profit_multiplier=4.75,
        model_risk_max_pct=0.03175,
        same_direction_reentry_cooldown_bars=8,
        short_threshold=0.54,
        long_threshold=0.52,
        close_on_flat_signal=False,
        ...
        close_only_on_opposite_signal=False,
        max_hold_bars=3,
        notes="Stage210 upper-risk OOS recovery: long-session structure with model risk cap 3.175%.",
    ),
)
...
BASE_EXTRA = {
    ...
    "tight_session_min": 160.0,
    "tight_session_max": 275.0,
    ...
    for variant_index, variant in enumerate(VARIANTS, start=1):
        ...
        for split in ("validation_is", "oos"):
            ...
            for role_index, (tier, attempt_role, prefix, attempt_token) in enumerate(
                ...
                magic = 20810000 + variant_index * 100 + (1 if split == "validation_is" else 50) + role_index
                attempts.append(
                    s161.base.attempt_payload(
                        run_root=variant_root,
                        run_id=RUN_ID,
                        stage_number=208,
                        exploration_label="stage210_BaselineAdapter__OOSNetRecoveryPreserveValidationGate",
...
def gate_value(row: Mapping[str, str], variant: repair.RepairVariant) -> float:
    ...
    if signal > 0:
        ...
        if rule == "session_only" and bounded_lowedge_hit(row, spec, "", use_margin=False):
            return 2.0
        if rule == "session_only_p5" and bounded_lowedge_hit(row, spec, "wide", use_margin=False):
            return 2.0
        if rule == "session_only_p10" and bounded_lowedge_hit(row, spec, "tight", use_margin=False):
            return 2.0
    if signal < 0:
...
def configure_runner() -> None:
    ...
    for variant in VARIANTS:
        ...
        context_specs[variant.adapter_id] = {
            "gate_column": f"stage210_gate_{extra['axis']}",
            "gate_type": "encoded_stage210_oos_net_recovery_preserve_validation_gate",
            ...
            "description": "Stage210 encoded gate: long-session structure with upper risk-cap OOS net recovery.",
        }
...
def decide(rows: Sequence[Mapping[str, Any]], external: str) -> str:
    if external != "completed":
        return "continue_stage210_runtime_completion_due_to_incomplete_runtime_candidate_not_final"
    if any(row.get("hard_quality_pass") for row in rows):
        return "open_stage211_stage210_oos_net_recovery_followup_review_candidate_not_final"
    return "open_stage211_bounded_followup_due_to_upper_risk_oos_recovery_tradeoff_candidate_not_final"
...
def report_markdown(rows: Sequence[Mapping[str, Any]], decision: str, external: str) -> str:
    best = best_row(rows)
    ref = next((row for row in rows if row.get("adapter_id") == "s210_ls_r03175"), {})
    return f"""# Stage210 OOS Net Recovery Preserve Validation Gate Report(210단계 표본외 순손익 회복 검증 관문 보존 보고서)
...
- source_stage209_evidence_commit(원천 209단계 근거 커밋): `{SOURCE_STAGE209_EVIDENCE_COMMIT}`
- source_stage209_hash_record_commit(원천 209단계 해시 기록 커밋): `{SOURCE_STAGE209_HASH_RECORD_COMMIT}`
...
def decision_markdown(decision: str, external: str) -> str:
    return f"""# Stage210 Decision(210단계 판정)
...
Stage210(210단계) closeout(종료)은 overall goal complete(전체 목표 완료)가 아니다. Effect(효과): Stage211(211단계) follow-up review(후속 검토)에서 upper risk OOS recovery(상단 위험 표본외 회복) 상충을 따로 판정한다.
"""
...
def artifact_rows(result: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    for row in rows:
        if row.get("artifact_type") != "mt5_strategy_tester_report":
            row["artifact_type"] = "stage210_evidence"
            row["notes"] = "Stage210 OOS net recovery preserve validation gate evidence."
        else:
            row["notes"] = "Stage210 MT5 Strategy Tester report."
    return rows
        ...
        [
            {
                "run_id": RUN_ID,
                "stage_id": STAGE_ID,
                "lane": "baseline_adapter_stage210_oos_net_recovery_preserve_validation_gate",
                "status": status,
                "judgment": decision,
                "path": rel(DECISION_PATH),
                "notes": s172.ledger_pairs(
                    (
                        ("source_stage209_evidence_commit", SOURCE_STAGE209_EVIDENCE_COMMIT),
                        ("source_stage209_hash_record_commit", SOURCE_STAGE209_HASH_RECORD_COMMIT),
        ...
        alpha_rows = [
            {
                ...
                "tier_scope": "Tier A+B",
                "kpi_scope": "stage210_oos_net_recovery_preserve_validation_gate",
                ...
                "external_verification_status": external,
                "notes": "Stage210 materialized or blocked before KPI records were available.",
            }
        ]
...
def write_packet_files(result: Mapping[str, Any], decision: str, ledger_payload: Mapping[str, Any], quality: Sequence[Mapping[str, Any]]) -> None:
    ...
    s172.write_md(
        ...
        f"""# Stage210 Closeout Packet(210단계 종료 작업 묶음)
...
Stage211(211단계)는 Stage210(210단계) OOS net recovery preserve validation gate(표본외 순손익 회복 검증 관문 보존) 결과를 follow-up review(후속 검토)한다.

## Bounded Question(경계 질문)

Did Stage210(210단계) improve OOS net(표본외 순손익) by upper risk cap(상단 위험 상한) without breaking validation DD/net/PF/midPF(검증 낙폭/순손익/수익요인/중반 수익요인)?

Effect(효과): Stage210(210단계) 안에서 추가 수리를 하지 않고, OOS recovery tradeoff(표본외 회복 상충)를 별도 판독 단계에서 과장 없이 판단한다.
...
""",
    ...
    s172.write_md(
        ...
        f"""# Stage211 Review Index(211단계 검토 색인)

- status(상태): `open_planned_from_stage210`
- source_stage(원천 단계): `{STAGE_ID}`
- source_decision(원천 판정): `{decision}`
...
""",
    ...
    s172.write_md(
        ...
        f"""# Stage211 Selection Status(211단계 선택 상태)

- stage_status(단계 상태): `open_planned_from_stage210`
...
- >-
  Stage210(210단계) closed(종료) as `{decision}` and Stage211(211단계) `{NEXT_STAGE_ID}` is open_planned(개방 계획). Effect(효과): upper risk OOS recovery(상단 위험 표본외 회복) 결과를 별도 follow-up review(후속 검토)로 넘긴다.
- >-
  Stage210 evidence(210단계 근거)는 `{rel(REPORT_PATH)}`, `{rel(SUMMARY_CSV_PATH)}`, `{rel(QUALITY_MATRIX_PATH)}`, `{rel(BALANCE_CURVE_AUDIT_PATH)}`, `{rel(RISK_ATR_TELEMETRY_PATH)}`에 있다. Effect(효과): upper risk cap(상단 위험 상한)이 DD/PF/OOS(낙폭/수익요인/표본외)에 준 영향을 같이 본다.
- >-
...
"""
    if re.search(r"(?ms)^current_focus:\r?\n.*?(?=\r?\nstage\d+_)", state):
        ...
        state = state.rstrip() + "\n" + focus
    state = re.sub(r"(?ms)^stage210_oos_net_recovery_preserve_validation_gate:\r?\n.*?(?=^stage\d+_|\Z)", "", state)
    block = f"""
stage210_oos_net_recovery_preserve_validation_gate:
  packet_id: {PACKET_ID}
  stage_id: {STAGE_ID}
...
- adapter_under_review(검토 중 어댑터): `stage210_oos_net_recovery_preserve_validation_gate`
- status(상태): `stage210_{decision}`
- claim_boundary(주장 경계): `{BOUNDARY}`

Stage210(210단계)는 Stage209(209단계) 판정에 따라 upper risk OOS net recovery(상단 위험 표본외 순손익 회복)를 MT5 Strategy Tester(메타트레이더5 전략 테스터)로 측정했다. Effect(효과): Stage211(211단계)가 KPI(핵심 성과 지표) 상충을 별도 review(검토)로 판정한다.

## Latest Stage210 Evidence(최신 210단계 근거)
...
def append_changelog(decision: str) -> None:
    ...
    if RUN_ID in existing:
        return
    entry = (
        f"\n## {s172.utc_now()} Stage210 OOS net recovery preserve validation gate closeout(210단계 표본외 순손익 회복 검증 관문 보존 종료)\n\n"
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    prepared = {
        ...
        "model_family": "baseline_adapter_stage210_v2_native_oos_net_recovery_preserve_validation_gate",
        "feature_set_id": "stage210_signal_plus_encoded_long_session_upper_risk_gate",
