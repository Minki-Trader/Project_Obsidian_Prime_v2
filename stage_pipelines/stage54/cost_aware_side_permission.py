...
STAGE_NUMBER = 54
STAGE_ID = "54_adapter_signal__cost_aware_side_permission_filter"
IDEA_ID = "IDEA-ST54-COST-AWARE-SIDE-PERMISSION-FILTER"
RUN_ID = "run48A_cost_aware_side_permission_filter_v1"
RUN_DIR_NAME = "run48A"
PACKET_ID = "stage54_run48A_cost_aware_side_permission_filter_v1"
BOUNDARY = (
    "stage54_cost_aware_side_permission_runtime_probe_only_no_baseline_no_promotion_"
...
COMMON_RUN_ROOT = "Project_Obsidian_Prime_v2/stage54/run48A"
...
SIDE_FILTER_CANDIDATES: tuple[dict[str, str], ...] = (
    {
        ...
        "description": "no additional cost-aware side permission filter",
    },
    {
    ...
    for candidate in SIDE_FILTER_CANDIDATES:
        ...
        for tier_scope, source, columns, source_files, token in payloads:
            for window in WFO_WINDOWS:
                ...
                output_name = f"{RUN_DIR_NAME}_{token}_{adapter_id}_{window['window_id']}_s54.csv"
        ...
        for window in WFO_WINDOWS:
            for tier_scope, token, route_view in (
                ...
                payload = attempt_payload(
                    ...
                    exploration_label="stage54_AdapterSignal__SideSpecificShortPermissionFilter",
...
def route_coverage(features: Mapping[str, Any]) -> dict[str, Any]:
    ...
    for window in WFO_WINDOWS:
        ...
        subtype_by_split[window["window_id"]] = {"Stage54_side_filter_Tier_B_accounting": tier_b}
...
def execute_mt5(attempts: Sequence[Mapping[str, Any]], coverage: Mapping[str, Any], args: argparse.Namespace) -> dict[str, Any]:
    ...
    for attempt in attempts:
        ...
        result = mt5.run_mt5_tester(
            ...
            tester_profile_ini_path=Path(args.tester_profile_root) / f"opv2_s54_{safe_name(attempt['attempt_name'], 80)}.ini",
            ...
            row.update(
                {
                    ...
                    "source_label": "stage54_cost_aware_side_permission_filter",
                    "adapter_id": execution.get("adapter_id", ""),
...
def evaluate_candidates(summary_rows: Sequence[Mapping[str, Any]], trade_rows: Sequence[Mapping[str, Any]], mt5_result: Mapping[str, Any]) -> dict[str, Any]:
    ...
    best: dict[str, Any] = {}
    for adapter_id in sorted({str(row.get("adapter_id") or "") for row in summary_rows if str(row.get("adapter_id") or "").startswith("csp")}):
        if adapter_id == "csp00_stage51_control":
            continue
    ...
    if max(float(validation.get("max_drawdown_amount") or 0.0), float(oos.get("max_drawdown_amount") or 0.0)) > 160.0:
        adapter_failures.append("max_drawdown_over_stage54_limit_160")
    ...
    practical_gate = {
        ...
        "deterministic_mapping": "adapter_id maps to stage54 feature CSV signal permission rules and MT5 .set files",
...
def write_stage_docs(results: Mapping[str, Any], judgment: str) -> None:
    ...
    write_md(
        ...
        f"""# Stage54 Cost-Aware Side Permission Filter(53?④퀎 諛⑺뼢 ?덉슜 ?꾪꽣)
...
""",
    ...
    write_md(
        ...
        f"""# Stage54 Input References(53?④퀎 ?낅젰 李몄“)
...
""",
    ...
    write_md(
        ...
        f"""# Stage54 Run Packet(53?④퀎 ?ㅽ뻾 ?⑦궥)
...
""",
    ...
    write_md(
        ...
        f"""# Stage54 Review Index(53?④퀎 寃???됱씤)
...
""",
    ...
    write_md(
        ...
        f"""# Stage54 Selection Status(53?④퀎 ?좏깮 ?곹깭)
...
def ledger_rows(judgment: str, evaluation: Mapping[str, Any]) -> tuple[list[dict[str, Any]], list[dict[str, Any]], list[dict[str, Any]]]:
        ...
        alpha_rows.append(
            {
                ...
                "kpi_scope": "stage54_side_permission_validation_oos",
    ...
    alpha_rows.append(
        {
            "ledger_row_id": f"{RUN_ID}_{adapter_id}_stage54_closeout",
            "stage_id": STAGE_ID,
            "run_id": RUN_ID,
            "subrun_id": "stage54_closeout",
            "parent_run_id": RUN_ID,
            "record_view": "stage54_closeout",
            "tier_scope": "Tier A and Tier B separate",
            "kpi_scope": "stage54_side_permission_stage_closeout",
    ...
    artifact_rows = [
        {"artifact_id": "stage54_run48A_manifest", "type": "manifest", "path": rel(MANIFEST_PATH), "status": "generated", "notes": "Stage54 side permission run manifest."},
        {"artifact_id": "stage54_run48A_mt5_summary", "type": "result_table", "path": rel(MT5_SUMMARY_PATH), "status": "generated", "notes": "Imported MT5 KPI summary."},
        {"artifact_id": "stage54_run48A_trade_coverage", "type": "result_table", "path": rel(TRADE_COVERAGE_PATH), "status": "generated", "notes": "Period-adjusted trade-count gate evidence."},
        {"artifact_id": "stage54_run48A_concentration", "type": "result_table", "path": rel(CONCENTRATION_PATH), "status": "generated", "notes": "Trade/day/week/month concentration audit."},
    ]
...
def update_workspace_state(judgment: str, evaluation: Mapping[str, Any]) -> None:
    ...
    focus = (
        f"- Stage54(53?④퀎) `{STAGE_ID}`: side-specific permission filter(諛⑺뼢蹂??덉슜 ?꾪꽣)瑜?`{judgment}`濡?湲곕줉?덈떎; "
    ...
    block = f"""
stage54_cost_aware_side_permission_filter:
  packet_id: {PACKET_ID}
  stage_id: {STAGE_ID}
  idea_id: {IDEA_ID}
  status: {stage_status}
  ...
  boundary: {BOUNDARY}
"""
    if "stage54_cost_aware_side_permission_filter:" not in text:
        text = text.rstrip() + "\n\n" + block
    else:
        text = re.sub(r"\nstage54_cost_aware_side_permission_filter:\n(?:  .+\n)+", "\n" + block, text)
    io_path(WORKSPACE_STATE_PATH).write_text(text.rstrip() + "\n", encoding="utf-8")

    current_block = f"""## Latest Stage54 Cost-Aware Side Permission Filter(理쒖떊 53?④퀎 諛⑺뼢 ?덉슜 ?꾪꽣)

- current run(?꾩옱 ?ㅽ뻾): `{RUN_ID}`

Stage54(53?④퀎) `{STAGE_ID}`??side-specific permission filter(諛⑺뼢蹂??덉슜 ?꾪꽣)瑜?`{judgment}`濡?湲곕줉?덈떎. selected candidate(?좏깮 ?꾨낫)??`{adapter_id}`?닿퀬, boundary(寃쎄퀎)??runtime_probe_only(?고????먯묠 ?꾩슜)?대떎. baseline(湲곗???, promotion(?밴꺽), runtime authority(?고???沅뚯쐞), live readiness(?ㅺ굅??以鍮?, operating reference(?댁쁺 李몄“)???녿떎.

"""
    current = read_text(CURRENT_WORKING_STATE_PATH)
    if "## Latest Stage54 Cost-Aware Side Permission Filter(理쒖떊 53?④퀎 諛⑺뼢 ?덉슜 ?꾪꽣)" not in current:
        io_path(CURRENT_WORKING_STATE_PATH).write_text(current_block + current, encoding="utf-8-sig")
    append_once(
        CHANGELOG_PATH,
        f"- 2026-05-11T00:00:00Z `{STAGE_ID}` recorded(湲곕줉) cost-aware side permission filter(諛⑺뼢 ?덉슜 ?꾪꽣) as `{judgment}`; no baseline(湲곗???, promotion(?밴꺽), runtime authority(?고???沅뚯쐞), live readiness(?ㅺ굅??以鍮?, or operating reference(?댁쁺 李몄“) created(?앹꽦).",
        bom=True,
    )
...
def write_packet_files(results: Mapping[str, Any], judgment: str, ledger_sync: Mapping[str, Any]) -> None:
    ...
    validation_commands = [
        run_command(["python", "-m", "py_compile", "stage_pipelines/stage54/cost_aware_side_permission.py", "foundation/pipelines/run_stage54_cost_aware_side_permission.py"]),
        run_command(["python", "-m", "pytest", "tests/test_stage54_cost_aware_side_permission.py", "tests/test_stage52_atr_sltp_adapter.py", "-q"]),
    ]
    ...
    write_json(PACKET_ROOT / "artifact_lineage_audit.json", {"status": "passed", "source_inputs": [rel(SOURCE_MODEL_PATH), rel(FEATURE_AUDIT_PATH)], "producer": rel(Path("stage_pipelines/stage54/cost_aware_side_permission.py")), "runtime_path": rel(mt5.EA_SOURCE_PATH), "lineage_judgment": "connected_with_boundary"})
...
def write_adapter_candidate_review_packet(results: Mapping[str, Any], judgment: str) -> str | None:
    ...
    packet_path = common.ROOT / "docs" / "agent_control" / "packets" / f"adapter_candidate_review_stage54_{adapter_id}_v1"
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run Stage54 cost-aware side permission filter MT5 probe.")
