...
RUN_ID = "frontier81C_mt5_runtime_materialization_v1"
PARENT_RUN_ID = "frontier81B_mt5_native_order_intent_cost_shape_proxy_scout_v1"
NEXT_RUN_ID = "frontier81D_proxy_runtime_gap_attribution_v1"
COMMON_RUN_ROOT = "Project_Obsidian_Prime_v2/frontier81C_mt5_runtime_materialization"
RUNTIME_CANDIDATE_PREFIX = "f81c_runtime"
ATTEMPT_PREFIX = "f81c_runtime_materialization"
EXPLORATION_LABEL = "frontier81C_runtime_materialization"
ATTEMPT_ROLE = "runtime_materialization"
RECORD_VIEW_PREFIX = "mt5_f81c_runtime_materialization"
...
TARGET_SELECTION = REVIEW_DIR / "f81c_runtime_materialization_target_selection.json"
F81B_SUMMARY = REVIEW_DIR / "f81b_order_intent_cost_shape_proxy_summary.json"
F81B_RANKED_TOP = REVIEW_DIR / "f81b_order_intent_cost_shape_ranked_top200.csv"
REPORT = REVIEW_DIR / "frontier81C_mt5_runtime_materialization_report.md"
GATE_AUDIT = REVIEW_DIR / "required_gate_coverage_audit_f81c.md"
SUMMARY = REVIEW_DIR / "f81c_mt5_runtime_materialization_summary.json"
RUNTIME_PARITY = REVIEW_DIR / "f81c_runtime_parity_receipt.json"
BACKTEST_FORENSICS = REVIEW_DIR / "f81c_backtest_forensics_receipt.json"
...
SCRIPT_REL = "stage_pipelines/stage_frontier_81/frontier81c_mt5_runtime_materialization.py"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="F81C MT5 runtime materialization.")
...
def target_row() -> dict[str, Any]:
    payload = read_json(F81B_SUMMARY)
    ranked_rows = read_csv(F81B_RANKED_TOP)
    ...
    for row in ranked_rows:
        ...
        break
    if not target:
        raise RuntimeError("f81b_exportable_materialization_target_missing")
    if int(target.get("materialization_candidate") or 0) != 1:
        raise RuntimeError(f"target_not_materialization_candidate:{target.get('candidate_id')}")
    target["selection_source"] = rel(F81B_RANKED_TOP)
    ...
    target["exportability_boundary"] = "HistGBM candidates hit current skl2onnx TreeEnsembleClassifier attribute conversion failure; selected next ranked exportable ExtraTrees candidate."
    write_json(
        TARGET_SELECTION,
        {
            "run_id": RUN_ID,
            ...
            "selection_boundary": "F81B ranked exportable materialization target only; no baseline, promotion, or runtime authority.",
        },
    )
    return target
...
def build_context(target: Mapping[str, Any]) -> dict[str, Any]:
    f78b.INITIAL_BALANCE = f81b.INITIAL_BALANCE
    frame, raw, features = f78b.load_inputs()
    spec = next(item for item in f81b.runtime_specs() if item.name == str(target["label_name"]))
    outcome = f79b.compute_outcome(raw, f79b.entry_indices(frame, raw, spec.entry_mode), spec)
    label = f81b.make_label(frame, outcome, spec)
    train_valid = (frame["split"] == "train").to_numpy() & np.asarray(outcome["valid"], dtype=bool)
    feature_columns = f81b.feature_sets(features)[str(target["feature_set"])]
    ...
    model = f81b.model_builders()[str(target["model"])]()
    ...
    for split, prefix in (("validation", "val"), ("oos", "oos")):
        ...
        event_filter = valid & f81b.regime_mask(split_df, str(target["regime"]), spec.side, thresholds) & f81b.risk_mask(split_df, str(target["risk_filter"]), spec.side, thresholds)
...
def materialize(context: Mapping[str, Any], common_files_root: Path) -> tuple[dict[str, Any], list[dict[str, Any]], list[dict[str, Any]], list[dict[str, Any]]]:
    ...
    artifact = {
        ...
        "materialization_mode": f"{side}_binary_onnx_three_column_runtime_materialization",
        ...
        "trade_shape": f"{side}_{target.get('entry_mode')}_hold{target.get('hold_bars')}_tp{target.get('tp_broker_points')}_sl{target.get('sl_broker_points')}",
...
def build_attempts(context: Mapping[str, Any], artifact: Mapping[str, Any], *, include_oos: bool) -> list[dict[str, Any]]:
    ...
    for split in splits:
        ...
        extra = {
            ...
            "InpAtrMinStopPoints": float(target.get("sl_broker_points", spec.sl_price_units * f81b.SLTP_POINT_SCALE)),
            "InpAtrMaxStopPoints": float(target.get("sl_broker_points", spec.sl_price_units * f81b.SLTP_POINT_SCALE)),
            "InpAtrMinTakeProfitPoints": float(target.get("tp_broker_points", spec.tp_price_units * f81b.SLTP_POINT_SCALE)),
            "InpAtrMaxTakeProfitPoints": float(target.get("tp_broker_points", spec.tp_price_units * f81b.SLTP_POINT_SCALE)),
        ...
        attempt = runtime_base.attempt_payload(
            ...
            model_id=f"F81C_{artifact['candidate_id']}",
            model_backend="onnx",
...
def report_text(payload: Mapping[str, Any], summary: Mapping[str, Any], created_at: str) -> str:
    ...
    return f"""# F81C MT5 Runtime Materialization Report(F81C MT5 런타임 물질화 보고서)
...
def gate_audit_text(payload: Mapping[str, Any], summary: Mapping[str, Any]) -> str:
    return f"""# F81C Required Gate Coverage Audit(F81C 필수 게이트 커버리지 감사)

Status(상태): `{payload.get('status')}`

| gate(게이트) | status(상태) | evidence(근거) | effect(효과) |
|---|---|---|---|
| `materialization_target` | `passed(통과)` | `{rel(TARGET_SELECTION)}` | F81B(전선81B) 대상만 물질화한다. |
| `onnx_probability_parity` | `{summary.get('probability_parity_pass_rows')}` | `{rel(RUN_DIR / 'f81c_probability_parity.csv')}` | Python/ONNX(파이썬/온엑스) 확률 차이를 확인한다. |
| `runtime_signal_veto_parity` | `{summary.get('signal_parity_pass_rows')}` | `{rel(RUN_DIR / 'f81c_signal_parity.csv')}` | 선택 진입 시각이 런타임 입력으로 보존되는지 확인한다. |
...
next_run_id: {NEXT_RUN_ID}
runtime_probe_status: f81_runtime_materialization_recorded_completed_attempts_{summary.get('completed_attempt_count')}
runtime_authority: not_claimed
operating_promotion: not_claimed
...
notes:
  - "Action(행동): F81C MT5 runtime materialization(MT5 런타임 물질화)을 만들고 실행을 시도했다."
...
Action(행동): F81C MT5 runtime materialization(F81C MT5 런타임 물질화)을 실행했다.
...
- `{RUN_ID}` attempted F81 MT5 runtime materialization(F81 MT5 런타임 물질화). Target(대상): `{target.get('candidate_id')}`. Attempts(시도): `{summary.get('attempt_count')}`, completed(완료): `{summary.get('completed_attempt_count')}`. Boundary(경계): runtime materialization only, no authority(런타임 물질화만, 권위 없음).
"""
...
def main() -> int:
    ...
    write_csv(RUN_DIR / "f81c_probability_parity.csv", probability)
    write_csv(RUN_DIR / "f81c_signal_parity.csv", signal)
    write_csv(RUN_DIR / "f81c_feature_readiness_parity.csv", feature_parity)
    write_csv(RUN_DIR / "f81c_source_reproduction.csv", context["reproduction_rows"])
    write_csv(RUN_DIR / "f81c_runtime_receipt.csv", runtime_receipt, f71d.RUNTIME_RECEIPT_COLUMNS)
    write_json(RUN_DIR / "f81c_execution_results.json", execution_results)
