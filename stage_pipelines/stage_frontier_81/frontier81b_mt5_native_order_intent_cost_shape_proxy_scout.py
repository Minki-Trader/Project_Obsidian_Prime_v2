...
RUN_ID = "frontier81B_mt5_native_order_intent_cost_shape_proxy_scout_v1"
PARENT_RUN_ID = "frontier81A_stage_open_mt5_native_order_intent_cost_shape_rebuild_v1"
NEXT_RUN_IF_MATERIAL = "frontier81C_mt5_runtime_materialization_v1"
NEXT_RUN_IF_WEAK = "frontier81C_capped_repair_or_rotation_decision_v1"
NEXT_RUN_IF_ZERO = "frontier81F_no_signal_negative_evidence_closeout_decision_v1"

STATUS_MATERIAL = "f81b_proxy_material_signal_mt5_materialization_required_no_authority"
STATUS_WEAK = "f81b_proxy_weak_nonzero_signal_capped_repair_or_rotation_required_no_authority"
STATUS_ZERO = "f81b_proxy_zero_signal_negative_evidence_closeout_decision_required_no_authority"
JUDGMENT_MATERIAL = "mt5_native_order_intent_cost_shape_material_proxy_clue_requires_mt5_runtime_no_authority"
JUDGMENT_WEAK = "mt5_native_order_intent_cost_shape_weak_proxy_clue_requires_capped_repair_no_authority"
JUDGMENT_ZERO = "mt5_native_order_intent_cost_shape_zero_signal_requires_negative_evidence_closeout_no_authority"
CLAIM_BOUNDARY = (
...
SUMMARY = REVIEW_DIR / "f81b_order_intent_cost_shape_proxy_summary.json"
CANDIDATES_ALL = RUN_DIR / "f81b_order_intent_cost_shape_candidates_all.csv"
CANDIDATES_TOP = REVIEW_DIR / "f81b_order_intent_cost_shape_ranked_top200.csv"
AXIS_SUMMARY = REVIEW_DIR / "f81b_order_intent_cost_shape_axis_summary.csv"
SURFACE_SUMMARY = REVIEW_DIR / "f81b_surface_family_summary.csv"
MODEL_FIT_SUMMARY = REVIEW_DIR / "f81b_model_fit_summary.csv"
LABEL_AUDIT = REVIEW_DIR / "f81b_label_audit.csv"
TIER_AUDIT = REVIEW_DIR / "f81b_tier_record_audit.csv"
DATA_CONTRACT = REVIEW_DIR / "f81b_data_feature_contract_preflight.json"
MODEL_VALIDATION = REVIEW_DIR / "f81b_model_validation_review.json"
ARTIFACT_LINEAGE = REVIEW_DIR / "f81b_artifact_lineage.json"
REPORT = REVIEW_DIR / "frontier81B_mt5_native_order_intent_cost_shape_proxy_scout_report.md"
GATE_AUDIT = REVIEW_DIR / "required_gate_coverage_audit_f81b.md"
...
SCRIPT_REL = "stage_pipelines/stage_frontier_81/frontier81b_mt5_native_order_intent_cost_shape_proxy_scout.py"
...
def model_builders(random_state: int = 8102) -> dict[str, Callable[[], Any]]:
    return {
        "extra_trees_d6_l160": lambda: ExtraTreesClassifier(
            n_estimators=28,
            max_depth=6,
        ...
        "histgbm_cost_shape_shallow": lambda: HistGradientBoostingClassifier(
            max_iter=50,
            learning_rate=0.060,
            max_leaf_nodes=15,
...
def runtime_specs() -> list[F81Spec]:
    seeds = [
        ("mt5_native_cost_shape_guard", "long", "same_bar_open", "pessimistic", 8, 10.0, 6.0, "cost_shape_guard", 0.62),
        ("mt5_native_cost_shape_guard", "short", "same_bar_open", "pessimistic", 8, 10.0, 6.0, "cost_shape_guard", 0.62),
        ("order_intent_exit_efficiency", "long", "same_bar_open", "close_direction", 14, 18.0, 8.0, "exit_efficiency", 0.64),
        ("order_intent_exit_efficiency", "short", "same_bar_open", "close_direction", 14, 18.0, 8.0, "exit_efficiency", 0.64),
        ("spread_tail_cost_budget", "long", "next_bar_open_control", "pessimistic", 10, 14.0, 8.0, "spread_tail_budget", 0.66),
        ("spread_tail_cost_budget", "short", "next_bar_open_control", "pessimistic", 10, 14.0, 8.0, "spread_tail_budget", 0.66),
        ("intent_cost_asymmetry", "long", "same_bar_open", "close_direction", 20, 24.0, 10.0, "intent_cost_asymmetry", 0.60),
        ("intent_cost_asymmetry", "short", "same_bar_open", "close_direction", 20, 24.0, 10.0, "intent_cost_asymmetry", 0.60),
        ("smooth_density_cost_shape", "long", "next_bar_open_control", "close_direction", 18, 20.0, 10.0, "smooth_density_cost_shape", 0.58),
        ("smooth_density_cost_shape", "short", "next_bar_open_control", "close_direction", 18, 20.0, 10.0, "smooth_density_cost_shape", 0.58),
    ]
    return [
        F81Spec(
...
def make_label(df: pd.DataFrame, outcome: Mapping[str, np.ndarray], spec: F81Spec) -> np.ndarray:
    ...
    exit_efficiency = np.divide(pnl, mfe + spread + 1e-9)
    if spec.label_mode == "cost_shape_guard":
        score = pnl + 0.04 * mfe - 0.85 * mae - 2.25 * spread - 0.003 * exit_offset
        guard = (mae <= np.nanquantile(mae[train_mask], 0.64)) & (spread <= np.nanquantile(spread[train_mask], 0.72))
    elif spec.label_mode == "exit_efficiency":
        score = pnl + 0.18 * mfe - 0.48 * mae - 1.40 * spread + 0.08 * exit_efficiency - 0.003 * exit_offset
        guard = (exit_efficiency >= np.nanquantile(exit_efficiency[train_mask], 0.55)) & (mae <= np.nanquantile(mae[train_mask], 0.78))
    elif spec.label_mode == "spread_tail_budget":
        score = pnl - 1.05 * mae - 2.80 * spread - 0.006 * exit_offset
        guard = (spread <= np.nanquantile(spread[train_mask], 0.62)) & (mae <= np.nanquantile(mae[train_mask], 0.58))
    elif spec.label_mode == "intent_cost_asymmetry":
        ratio = np.divide(mfe, mae + 1e-9)
        score = pnl + 0.15 * mfe - 0.55 * mae - 1.75 * spread - 0.0025 * exit_offset
        guard = (ratio >= np.nanquantile(ratio[train_mask], 0.60)) & (spread_pressure <= np.nanquantile(spread_pressure[train_mask], 0.70))
    else:
        score = pnl + 0.06 * mfe - 0.40 * mae - 1.60 * spread - 0.010 * exit_offset
        guard = (exit_offset <= np.nanquantile(exit_offset[train_mask], 0.70)) & (spread_pressure <= np.nanquantile(spread_pressure[train_mask], 0.75))
...
def fit_and_score() -> tuple[list[dict[str, Any]], list[dict[str, Any]], list[dict[str, Any]], dict[str, Any]]:
    ...
    try:
        ...
        specs = runtime_specs()
        regimes = ["all", "cash_open", "cash_mid", "high_vol", "trend"]
        risks = ["none", "liquidity_release", "order_intent_guard"]
        prob_quantiles = [0.72, 0.84]
        cooldowns = [0, 6]
                        ...
                        for regime in regimes:
                            for risk in risks:
                                for cooldown in cooldowns:
                                    ...
                                    candidate_id += 1
                                    row: dict[str, Any] = {
                                        "candidate_id": f"f81b_{candidate_id:05d}",
                                        "surface_family": spec.surface_family,
                                        "label_name": spec.name,
                                        "side": spec.side,
                                        "entry_mode": spec.entry_mode,
...
def report_text(created_at: str, summary: Mapping[str, Any], top_rows: Sequence[Mapping[str, Any]]) -> str:
    ...
    return f"""# F81B MT5 Native Order Intent Cost Shape Proxy Scout Report(F81B MT5 원형 주문 의도 비용 형태 프록시 탐색 보고서)
...
def data_contract(summary: Mapping[str, Any]) -> dict[str, Any]:
    return {
        ...
        "label_only_future_fields": "Post-entry OHLC path(진입 이후 OHLC 경로), spread cost(스프레드 비용), MAE/MFE(최대 불리/유리 이동), spread pressure/exit efficiency/cost shape(스프레드 압력/청산 효율/비용 형태).",
        ...
        "f64_f67_f80_lesson": "Parity(동등성), ONNX handoff(온엑스 인계), proxy PF(프록시 수익 팩터), and signal count(신호 수) do not guarantee MT5 economics(MT5 경제성).",
    }
...
def model_validation_review(summary: Mapping[str, Any]) -> dict[str, Any]:
    return {
        ...
        "risk_boundary": "Deposit=500 DD(예치금 500 손실폭), spread cost/pressure(스프레드 비용/압력), exit efficiency(청산 효율), density(밀도), lifecycle occupancy(생명주기 점유)를 같이 본다.",
        "summary_counts": {
...
def artifact_lineage(summary: Mapping[str, Any], next_run: str) -> dict[str, Any]:
    return {
        ...
        "source_inputs": [rel(f78b.DATASET_PATH), rel(f78b.FEATURE_ORDER_PATH), rel(f78b.RAW_BARS_PATH), rel(STAGE_DIR / "00_spec/stage_brief.md"), rel(STAGE_DIR / "01_inputs/input_refs.md")],
...
def gate_audit_text(status: str, summary: Mapping[str, Any], next_run: str) -> str:
    return f"""# F81B Required Gate Coverage Audit(F81B 필수 게이트 커버리지 감사)
...
def selection_status_text(created_at: str, status: str, judgment: str, next_run: str, summary: Mapping[str, Any]) -> str:
    return f"""# F81 Selection Status(F81 선택 상태)
...
Action(행동): F81B MT5-native order intent cost-shape proxy scout(F81B MT5 원형 주문 의도 비용 형태 프록시 탐색)를 실행했다.

Effect(효과): spread pressure/exit efficiency/cost shape(스프레드 압력/청산 효율/비용 형태)를 label(라벨)에 넣어, F80 runtime gap(F80 런타임 간극)을 다른 축에서 다시 압박한 후보 표면을 기록했다.
...
def ledger_rows(created_at: str, status: str, judgment: str, next_run: str, summary: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    base = {
        ...
        "run_family": "mt5_native_order_intent_cost_shape_proxy_scout",
    ...
    return [
        {
            **base,
            ...
            "lane": "order_intent_cost_shape_proxy_scout(주문 의도 비용 형태 프록시 탐색)",
            ...
            "question": "Can MT5-native order intent cost shape labels create material proxy clues?(MT5 원형 주문 의도 비용 형태 라벨이 물질적 프록시 단서를 만들 수 있는가?)",
            "artifact_count": 11,
        },
        {
            **base,
            ...
            "notes": "Tier B separate(티어 B 분리) source was not available in F81B; not omitted.",
        },
        {
            **base,
            ...
            "notes": "No routed Tier A primary + Tier B fallback(티어 A 우선 + 티어 B 대체) run exists in F81B.",
        },
    ]
...
next_run_id: {next_run}
runtime_probe_status: f81_runtime_probe_required_if_material_candidate_exists_not_yet_run
runtime_authority: not_claimed
operating_promotion: not_claimed
...
notes:
  - "Action(행동): F81B MT5-native order intent cost-shape proxy scout(MT5 원형 주문 의도 비용 형태 프록시 탐색)를 실행했다."
...
Effect(효과): F81(전선81)의 spread pressure/exit efficiency/cost shape(스프레드 압력/청산 효율/비용 형태) label axis(라벨 축), feature set/model family/trade shape/risk logic/regime split(피처 묶음/모델 계열/거래 형태/위험 로직/장세 분할)을 함께 기록했다.
...
def receipts_texts(status: str, judgment: str, next_run: str, summary: Mapping[str, Any]) -> dict[Path, str]:
    return {
        REVIEW_DIR / "f81b_run_evidence_receipt.yaml": f"""packet_id: {RUN_ID}
...
forbidden_claims:
  ...
  - goal_achieve
""",
        REVIEW_DIR / "f81b_data_integrity_receipt.yaml": f"""packet_id: {RUN_ID}
...
""",
        REVIEW_DIR / "f81b_model_validation_receipt.yaml": f"""packet_id: {RUN_ID}
skill: obsidian-model-validation
status: proxy_validation_recorded_no_selection
model_or_threshold_surface: "F81B proxy candidates(F81B 프록시 후보)"
...
forbidden_claims:
  ...
  - goal_achieve
""",
        REVIEW_DIR / "f81b_artifact_lineage_receipt.yaml": f"""packet_id: {RUN_ID}
...
""",
        REVIEW_DIR / "f81b_claim_discipline_receipt.yaml": f"""packet_id: {RUN_ID}
skill: obsidian-claim-discipline
status: passed_proxy_only
requested_claims:
  - "F81B proxy scout executed(F81B 프록시 탐색 실행)."
allowed_claims:
  - proxy_scout_executed
...
def update_review_index() -> None:
    text = f"""# F81 Review Index(F81 검토 색인)
...
def context_anchor_text(created_at: str, status: str, judgment: str, next_run: str, summary: Mapping[str, Any]) -> str:
    return f"""# F81 Context Anchor(F81 문맥 앵커)
