...
STAGE_NUMBER = 12
RUN_NUMBER = "run03Q"
RUN_ID = "run03Q_et_macro_proxy_regime_probe_v1"
PACKET_ID = "stage12_run03q_macro_proxy_regime_probe_v1"
EXPLORATION_LABEL = "stage12_Model__ExtraTreesMacroProxyRegime"
IDEA_ID = "IDEA-ST12-ET-MACRO-PROXY-REGIME"
...
FEATURE_SET_ID = "feature_set_v2_mt5_price_proxy_58_and_core42_partial_context_macro_proxy_regime"
LABEL_ID = "label_v1_fwd12_m5_logret_train_q33_3class"
SPLIT_CONTRACT = "rolling_wfo_fold01_to_fold06_macro_proxy_regime_buckets_mt5_fold05_no_fold07"
BOUNDARY = "runtime_probe_macro_proxy_regime_not_alpha_quality_not_promotion_not_runtime_authority"
JUDGMENT = "inconclusive_macro_proxy_regime_runtime_probe_completed"
HIT_FLOOR = 0.48
MT5_FOLD_ID = "fold05"
...
PLAN_PATH = STAGE_ROOT / "00_spec/run03Q_macro_proxy_regime_probe_plan.md"
STAGE_REVIEW_PATH = STAGE_ROOT / "03_reviews/run03Q_macro_proxy_regime_probe_packet.md"
...
SESSION_BUCKETS = (
    {"id": "risk_on", "label": "macro_risk_on_relief", "flag_min": -3, "flag_max": -1},
    {"id": "mixed", "label": "macro_mixed_neutral", "flag_min": 0, "flag_max": 0},
    {"id": "risk_off", "label": "macro_risk_off_pressure", "flag_min": 1, "flag_max": 3},
)
...
MACRO_VIX_CHANGE_THRESHOLD = 0.003
MACRO_RATE_CHANGE_THRESHOLD = 0.00018
MACRO_USDX_CHANGE_THRESHOLD = 0.00015
MACRO_ZSCORE_THRESHOLD = 1.0
...
def macro_proxy_regime_flag_count(frame: pd.DataFrame) -> pd.Series:
    vix_z = pd.to_numeric(frame["vix_zscore_20"], errors="coerce")
    vix_change = pd.to_numeric(frame["vix_change_1"], errors="coerce")
    rate_z = pd.to_numeric(frame["us10yr_zscore_20"], errors="coerce")
    rate_change = pd.to_numeric(frame["us10yr_change_1"], errors="coerce")
    usdx_z = pd.to_numeric(frame["usdx_zscore_20"], errors="coerce")
    usdx_change = pd.to_numeric(frame["usdx_change_1"], errors="coerce")
    risk_off = (
        (vix_z.ge(MACRO_ZSCORE_THRESHOLD) | vix_change.ge(MACRO_VIX_CHANGE_THRESHOLD)).fillna(False).astype("int64")
        + (rate_z.ge(MACRO_ZSCORE_THRESHOLD) | rate_change.ge(MACRO_RATE_CHANGE_THRESHOLD)).fillna(False).astype("int64")
        + (usdx_z.ge(MACRO_ZSCORE_THRESHOLD) | usdx_change.ge(MACRO_USDX_CHANGE_THRESHOLD)).fillna(False).astype("int64")
    )
    risk_on = (
        (vix_z.le(-MACRO_ZSCORE_THRESHOLD) | vix_change.le(-MACRO_VIX_CHANGE_THRESHOLD)).fillna(False).astype("int64")
        + (rate_z.le(-MACRO_ZSCORE_THRESHOLD) | rate_change.le(-MACRO_RATE_CHANGE_THRESHOLD)).fillna(False).astype("int64")
        + (usdx_z.le(-MACRO_ZSCORE_THRESHOLD) | usdx_change.le(-MACRO_USDX_CHANGE_THRESHOLD)).fillna(False).astype("int64")
    )
    return risk_off - risk_on


def assign_session_bucket(frame: pd.DataFrame) -> pd.Series:
    score = macro_proxy_regime_flag_count(frame)
    out = pd.Series("mixed", index=frame.index, dtype="object")
    out.loc[score.lt(0)] = "risk_on"
    out.loc[score.gt(0)] = "risk_off"
    return out
...
def decision_frame(
    *,
    ...
    vol_flags = macro_proxy_regime_flag_count(source).to_numpy()
    return pd.DataFrame(
        {
            "run_id": RUN_ID,
            "fold_id": fold_id,
            "variant_id": "macro_proxy_regime_v01_base_leaf20_q90",
            ...
            "macro_proxy_regime_flag_count": vol_flags,
            "label_class": labels,
...
def metric_row(frame: pd.DataFrame, fold: Any, split_role: str, bucket_id: str) -> dict[str, Any]:
    ...
    return {
        ...
        "variant_id": first.get("variant_id", "macro_proxy_regime_v01_base_leaf20_q90"),
        "source_variant_id": REFERENCE_VARIANT_ID,
        "idea_id": IDEA_ID,
        "split_role": split_role,
...
def make_attempts(
    *,
    ...
    for attempt in attempts:
        ...
        attempt["source_variant_id"] = "macro_proxy_regime_v01_base_leaf20_q90"
...
def prepare_mt5_run() -> dict[str, Any]:
    ...
    tier_a_joblib = RUN_ROOT / "models/tier_a_macro_proxy_regime_fold05_model.joblib"
    tier_b_joblib = RUN_ROOT / "models/tier_b_macro_proxy_regime_fold05_core42_model.joblib"
    ...
    if not onnx_parity["passed"]:
        raise RuntimeError("RUN03Q ONNX parity failed")
    ...
    summary = {
        ...
        "macro_proxy_regime_buckets": SESSION_BUCKETS,
    ...
    return {
        **summary,
        ...
        "route_coverage": route_coverage,
        "completion_goal": "macro_proxy_regime_probe_no_fold07_with_fold05_mt5",
...
def repair_mt5_record_identities(result: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    for execution in completed_executions:
        ...
        for _ in range(expected_records):
            ...
            record["source_variant_id"] = "macro_proxy_regime_v01_base_leaf20_q90"
...
def finalize_result(result: Mapping[str, Any]) -> dict[str, Any]:
    ...
    out["judgment"] = JUDGMENT if completed else "blocked_macro_proxy_regime_mt5_probe"
...
def write_preliminary_kpi(result: Mapping[str, Any]) -> None:
    ...
    write_json(
        ...
        {
            **manifest,
            "kpi_scope": "macro_proxy_regime_probe_preliminary",
            "model_family": MODEL_FAMILY,
            "feature_set_id": FEATURE_SET_ID,
            "label_id": LABEL_ID,
            "split_contract": SPLIT_CONTRACT,
            "mt5": {"scoreboard_lane": "runtime_probe_macro_proxy_regime", "kpi_records": result.get("mt5_kpi_records", [])},
        },
    )
...
def build_packet_summary(
    ...
    return {
        ...
        "judgment": JUDGMENT if completed else "blocked_macro_proxy_regime_mt5_probe",
        "boundary": BOUNDARY,
        "allowed_claims": ["runtime_probe_macro_proxy_regime_completed", "no_fold07_macro_proxy_regime_probe_completed"] if completed else ["blocked"],
...
def runtime_evidence_gate(summary: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "allowed_claims": ["runtime_probe_macro_proxy_regime_completed"] if passed else ["blocked"],
    ...
    run_rows = [
        {
            "run_id": RUN_ID,
            "stage_id": STAGE_ID,
            "lane": "macro_proxy_regime_runtime_probe",
            "status": status,
        ...
        rows.append(
            {
                ...
                "tier_scope": record.get("tier_scope"),
                "kpi_scope": "macro_proxy_regime_python_wfo_no_fold07",
                "scoreboard_lane": "runtime_probe_macro_proxy_regime",
                ...
                "notes": "RUN03Q Python macro-proxy-regime-regime WFO row; not alpha quality or promotion.",
            }
        )
    return rows
        ...
        rows.append(
            {
                ...
                "record_view": record.get("record_view"),
                "tier_scope": record.get("tier_scope"),
                "kpi_scope": "macro_proxy_regime_mt5_fold05",
                "scoreboard_lane": "runtime_probe_macro_proxy_regime",
                "status": record.get("status"),
                "judgment": summary["judgment"],
                ...
                "external_verification_status": summary["external_verification_status"],
                "notes": "RUN03Q MT5 fold05 macro-proxy-regime-regime row; runtime probe only.",
            }
        )
    return rows
...
def write_work_packet(created_at: str, summary: Mapping[str, Any]) -> None:
    packet = {
        ...
        "user_request": {"user_quote": "하나씩 run으로 구성해서 mt5까지 연동해서 검증해줘", "requested_action": "run macro proxy regime regime probe without fold07 and include MT5 verification", "requested_count": {"value": 1, "n_a_reason": None}, "ambiguous_terms": ["one at a time means RUN03Q first."]},
        ...
        "decision_lock": {"mode": "user_explicit_one_run_no_variant", "assumptions": {"fold07_excluded": True, "python_folds": "fold01-fold06", "mt5_fold": MT5_FOLD_ID, "macro_proxy_regime_buckets": [b["label"] for b in SESSION_BUCKETS]}, "questions": [], "required_user_decisions": []},
        "interpreted_scope": {"work_families": ["runtime_backtest"], "target_surfaces": [rel(RUN_ROOT), rel(PACKET_ROOT), rel(STAGE_RUN_LEDGER_PATH), rel(PROJECT_ALPHA_LEDGER_PATH)], "scope_units": ["single_idea", "macro_proxy_regime_bucket", "fold", "tier_view", "mt5_attempt", "kpi_record", "ledger"], "execution_layers": ["python_wfo", "onnx_export", "mt5_strategy_tester", "kpi_recording", "ledger_update", "document_edit"], "mutation_policy": {"allowed": True, "boundary": "new RUN03Q artifacts and current truth only"}, "evidence_layers": ["python_macro_proxy_regime_metrics", "mt5_reports", "normalized_kpi", "trade_attribution", "ledgers", "work_packet"], "reduction_policy": {"reduction_allowed": False, "requires_user_quote": False}, "claim_boundary": {"allowed_claims": summary["allowed_claims"], "forbidden_claims": summary["forbidden_claims"]}},
        "acceptance_criteria": [
            ...
            {"id": "AC-002", "text": "Three macro proxy regime buckets are recorded for Tier A, Tier B, and Tier A+B.", "expected_artifact": rel(RUN_ROOT / "results/session_bucket_summary.csv"), "verification_method": "kpi_contract_audit", "required": True},
...
def skill_receipts(created_at: str, summary: Mapping[str, Any]) -> list[dict[str, Any]]:
    return [
        {"packet_id": PACKET_ID, "created_at_utc": created_at, "triggered": True, "blocking": False, "skill": "obsidian-runtime-parity", "status": "executed", "python_artifact": rel(RUN_ROOT / "results/session_bucket_summary.csv"), "runtime_artifact": rel(RUN_ROOT / "run_manifest.json"), "compared_surface": "Python macro proxy regime bucket WFO, ONNX exports, bucket feature matrices, and MT5 Strategy Tester reports", "parity_level": "runtime_probe", "tester_identity": f"US100 M5, deposit=500, leverage=1:100, mt5_fold={MT5_FOLD_ID}, attempts={summary.get('mt5_attempts_total')}", "missing_evidence": "none" if summary["external_verification_status"] == "completed" else "blocked_mt5_attempts", "allowed_claims": ["runtime_probe_macro_proxy_regime_completed"], "forbidden_claims": ["alpha_quality", "promotion_candidate", "operating_promotion", "runtime_authority"]},
        {"packet_id": PACKET_ID, "created_at_utc": created_at, "triggered": True, "blocking": False, "skill": "obsidian-experiment-design", "status": "executed", "hypothesis": "ExtraTrees signal quality may depend on broad macro_proxy_regime regimes.", "baseline": SOURCE_WFO_RUN_ID, "changed_variables": ["post-score macro proxy regime bucket segmentation", "fold07 exclusion"], "invalid_conditions": ["fold07 included", "missing tier view", "missing MT5 reports"], "evidence_plan": [rel(RUN_ROOT / "results/session_bucket_summary.csv"), rel(PACKET_ROOT / "normalized_kpi_records.jsonl")]},
        {"packet_id": PACKET_ID, "created_at_utc": created_at, "triggered": True, "blocking": False, "skill": "obsidian-model-validation", "status": "executed", "model_or_threshold_surface": "ExtraTrees v01 base leaf20 q90 with macro-proxy-regime-regime post-score segmentation", "validation_split": "Python fold01-fold06 only and MT5 fold05 only; fold07 excluded", "overfit_checks": "bucket hit-rate gaps, both-positive folds, collapse folds, and MT5 validation/test bucket comparison", "selection_metric_boundary": "exploratory bucket read only; no operating selection metric", "allowed_claims": ["runtime_probe_macro_proxy_regime_completed"], "forbidden_claims": ["alpha_quality", "promotion_candidate", "operating_promotion", "runtime_authority"]},
        ...
        {"packet_id": PACKET_ID, "created_at_utc": created_at, "triggered": True, "blocking": False, "skill": "obsidian-exploration-mandate", "status": "executed", "exploration_lane": "Stage 12 ExtraTrees macro-proxy-regime-regime probe without fold07", "idea_boundary": "market macro proxy regime structure axis, not model variant or baseline", "negative_memory_effect": "Weak bucket behavior is retained as failure memory and should not trigger micro tuning.", "operating_claim_boundary": "No alpha quality, promotion candidate, operating promotion, or runtime authority."},
        {"packet_id": PACKET_ID, "created_at_utc": created_at, "triggered": True, "blocking": False, "skill": "obsidian-result-judgment", "status": "executed", "judgment_boundary": BOUNDARY, "allowed_claims": ["runtime_probe_macro_proxy_regime_completed", "no_fold07_macro_proxy_regime_probe_completed"], "forbidden_claims": ["alpha_quality", "promotion_candidate", "operating_promotion", "runtime_authority"], "evidence_used": [rel(RUN_ROOT / "results/session_bucket_summary.csv"), rel(PACKET_ROOT / "normalized_kpi_records.jsonl")]},
    ]
...
def write_reports(summary: Mapping[str, Any]) -> None:
    ...
    lines = [
        "# RUN03Q Macro Proxy Regime Regime Probe(RUN03Q 거시 대리 국면 국면 탐침)",
        "",
        f"- run(실행): `{RUN_ID}`",
        "- fold07(접힘 7): `excluded(제외)`",
        f"- Python folds(파이썬 접힘): `{','.join(summary['folds_used'])}`",
        f"- MT5 fold(MT5 접힘): `{MT5_FOLD_ID}`",
        "- macro proxy regime bucket(거시 대리 국면 구간): `risk_on/mixed/risk_off`",
    ...
    lines.extend(["", "효과(effect, 효과): macro proxy regime regime(거시 대리 국면 국면)이 신호 품질을 가르는지 확인했지만, alpha quality(알파 품질), promotion candidate(승격 후보), runtime authority(런타임 권위)는 아니다."])
    write_text(RUN_ROOT / "reports/result_summary.md", "\n".join(lines))
    closeout = f"""# {PACKET_ID} Closeout Report(마감 보고서)

## Conclusion(결론)

RUN03Q(실행 03Q)는 fold07(접힘 7)을 쓰지 않고 macro proxy regime regime(거시 대리 국면 국면)을 탐침했다.

## What changed(무엇이 바뀌었나)

- fold01~fold06(접힘 1~6)만 Python WFO(파이썬 워크포워드 최적화)에 사용했다.
- MT5(메타트레이더5)는 fold05(접힘 5)만 사용했다.
- 신호를 3개 macro proxy regime bucket(거시 대리 국면 구간)으로 나눴다.
...
full MT5 WFO(전체 MT5 워크포워드 최적화)는 실행하지 않았다. 이 packet(묶음)은 fold07(접힘 7) 없는 macro proxy regime runtime probe(거시 대리 국면 런타임 탐침)다.

## Allowed claims(허용 주장)

`runtime_probe_macro_proxy_regime_completed`, `no_fold07_macro_proxy_regime_probe_completed`.
...
"""
    ...
    plan = "# RUN03Q Macro Proxy Regime Regime Probe Plan(RUN03Q 거시 대리 국면 국면 탐침 계획)\n\nfold07(접힘 7)을 제외하고 macro proxy regime regime(거시 대리 국면 국면) 주제를 탐색한다.\n\n효과(effect, 효과): 모델 변형이 아니라 시장 거시 대리 국면 구조를 본다.\n"
    review = f"# RUN03Q Review Packet(RUN03Q 검토 묶음)\n\n`{RUN_ID}` completed(완료). 효과(effect, 효과): fold07(접힘 7) 없이 macro proxy regime bucket(거시 대리 국면 구간) 근거를 남겼다.\n"
...
def write_run_files(result: Mapping[str, Any], summary: Mapping[str, Any]) -> None:
    ...
    write_json(RUN_ROOT / "run_manifest.json", manifest)
    write_json(RUN_ROOT / "kpi_record.json", {**manifest, "kpi_scope": "macro_proxy_regime_probe", "model_family": MODEL_FAMILY, "feature_set_id": FEATURE_SET_ID, "label_id": LABEL_ID, "split_contract": SPLIT_CONTRACT, "python_wfo": summary.get("best_python_routed_bucket"), "mt5": {"scoreboard_lane": "runtime_probe_macro_proxy_regime", "kpi_records": result.get("mt5_kpi_records", [])}})
...
def update_workspace_state(summary: Mapping[str, Any]) -> None:
  ...
  stage_id: {STAGE_ID}
  status: active_macro_proxy_regime_probe_completed
  lane: macro_proxy_regime_runtime_probe
...
def update_current_working_state(summary: Mapping[str, Any]) -> None:
    ...
    section = f"""## RUN03Q macro proxy regime regime probe(거시 대리 국면 국면 탐침)
...
"""
    write_text(CURRENT_STATE_PATH, replace_section(text, "## RUN03Q macro proxy regime regime probe(거시 대리 국면 국면 탐침)", section))
...
## Current Read - RUN03Q macro proxy regime regime probe(현재 판독 - RUN03Q 거시 대리 국면 국면 탐침)
...
Effect(효과): fold07(접힘 7) 없이 macro proxy regime regime(거시 대리 국면 국면) 주제를 확인했다. baseline(기준선)이나 promotion candidate(승격 후보)는 아니다.
"""
...
def update_changelog(summary: Mapping[str, Any]) -> None:
    ...
    if RUN_ID in text:
        return
    entry = f"- 2026-05-01: `{RUN_ID}` completed(완료). fold07(접힘 7) 없이 macro proxy regime-regime probe(거시 대리 국면 국면 탐침)를 실행했다. attempts(시도) `{summary['mt5_attempts_total']}`, normalized KPI(정규화 KPI) `{summary['normalized_records']}`. Effect(효과): 모델 미세조정이 아니라 거시 대리 국면 구조 축을 failure/salvage memory(실패/회수 기억)로 남겼다.\n"
...
def update_idea_registry(summary: Mapping[str, Any]) -> None:
    ...
    if IDEA_ID in text:
        return
    best = summary["best_python_routed_bucket"]
    row = f"| `{IDEA_ID}` | `{STAGE_ID}` | ExtraTrees(엑스트라 트리) 신호는 특정 macro proxy regime regime(거시 대리 국면 국면)에 집중될 수 있다 | `Tier A + Tier B combined(Tier A + Tier B 합산)` | `runtime_probe_completed_inconclusive` | RUN03Q(실행 03Q); fold07(접힘 7) 제외; best bucket(최상위 구간) `{best.get('session_bucket_label')}` |\n"
...
def update_negative_register(summary: Mapping[str, Any]) -> None:
    ...
    if IDEA_ID in text:
        return
    row = f"| `NR-023` | `{IDEA_ID}` | macro proxy regime regime(거시 대리 국면 국면)만으로 ExtraTrees(엑스트라 트리) 신호가 안정될 수 있다 | RUN03Q(실행 03Q) best bucket(최상위 구간)의 both-positive fold(양쪽 양수 접힘)는 `{best.get('both_positive_fold_count')}`다 | macro proxy regime bucket(거시 대리 국면 구간)은 보조 설명 축으로 남긴다 | 특정 bucket(구간)이 다른 label/model(라벨/모델)에서 반복될 때 |\n"
...
def run_closeout_gate(summary: Mapping[str, Any]) -> None:
    ...
    for check in (
        ("session_summary_rows", "9", RUN_ROOT / "results/session_bucket_summary.csv", "three-macro_proxy_regime-buckets-three-tier-summary"),
        ("session_metric_rows", "108", RUN_ROOT / "results/session_bucket_metrics.csv", "fold01-fold06-macro_proxy_regime-bucket-metrics"),
        ("mt5_attempt_rows", "18", PACKET_ROOT / "attempt_summary.csv", "fold05-macro_proxy_regime-bucket-mt5-attempts"),
...
def repair_existing_run() -> dict[str, Any]:
    ...
    if not path_exists(manifest_path) or not path_exists(kpi_path) or not path_exists(bucket_summary_path):
        raise RuntimeError("RUN03Q existing artifacts are incomplete; cannot repair without rerunning.")
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run Stage 12 macro_proxy_regime regime probe without fold07.")
