...
STAGE_NUMBER = 12
RUN_NUMBER = "run03M"
RUN_ID = "run03M_et_session_age_regime_probe_v1"
PACKET_ID = "stage12_run03m_session_age_regime_probe_v1"
EXPLORATION_LABEL = "stage12_Model__ExtraTreesSessionAgeRegime"
IDEA_ID = "IDEA-ST12-ET-SESSION-AGE-REGIME"
...
FEATURE_SET_ID = "feature_set_v2_mt5_price_proxy_58_and_core42_partial_context_session_age"
LABEL_ID = "label_v1_fwd12_m5_logret_train_q33_3class"
SPLIT_CONTRACT = "rolling_wfo_fold01_to_fold06_session_age_buckets_mt5_fold05_no_fold07"
BOUNDARY = "runtime_probe_session_age_regime_not_alpha_quality_not_promotion_not_runtime_authority"
JUDGMENT = "inconclusive_session_age_regime_runtime_probe_completed"
HIT_FLOOR = 0.48
MT5_FOLD_ID = "fold05"
...
PLAN_PATH = STAGE_ROOT / "00_spec/run03M_session_age_regime_probe_plan.md"
STAGE_REVIEW_PATH = STAGE_ROOT / "03_reviews/run03M_session_age_regime_probe_packet.md"
...
SESSION_BUCKETS = (
    {"id": "b000060", "label": "0-60", "start": 0.0, "end": 60.0},
    {"id": "b060120", "label": "60-120", "start": 60.0, "end": 120.0},
    {"id": "b120180", "label": "120-180", "start": 120.0, "end": 180.0},
    {"id": "b180240", "label": "180-240", "start": 180.0, "end": 240.0},
    {"id": "b240plus", "label": "240+", "start": 240.0, "end": None},
)
...
def assign_session_bucket(frame: pd.DataFrame) -> pd.Series:
    ...
    out = pd.Series("outside", index=frame.index, dtype="object")
    for bucket in SESSION_BUCKETS:
        mask = minutes >= float(bucket["start"])
        if bucket["end"] is not None:
            mask &= minutes < float(bucket["end"])
        out.loc[mask] = str(bucket["id"])
    return out
    ...
    return pd.DataFrame(
        {
            "run_id": RUN_ID,
            "fold_id": fold_id,
            "variant_id": "session_age_v01_base_leaf20_q90",
...
def metric_row(frame: pd.DataFrame, fold: Any, split_role: str, bucket_id: str) -> dict[str, Any]:
    ...
    return {
        ...
        "session_bucket_start": bucket["start"],
        "session_bucket_end": "" if bucket["end"] is None else bucket["end"],
        "variant_id": first.get("variant_id", "session_age_v01_base_leaf20_q90"),
        "source_variant_id": REFERENCE_VARIANT_ID,
        "idea_id": IDEA_ID,
        "split_role": split_role,
...
def make_attempts(
    *,
    ...
    for attempt in attempts:
        ...
        attempt["source_variant_id"] = "session_age_v01_base_leaf20_q90"
...
def prepare_mt5_run() -> dict[str, Any]:
    ...
    tier_a_joblib = RUN_ROOT / "models/tier_a_session_age_fold05_model.joblib"
    tier_b_joblib = RUN_ROOT / "models/tier_b_session_age_fold05_core42_model.joblib"
    ...
    if not onnx_parity["passed"]:
        raise RuntimeError("RUN03M ONNX parity failed")
    ...
    return {
        **summary,
        ...
        "route_coverage": route_coverage,
        "completion_goal": "session_age_regime_probe_no_fold07_with_fold05_mt5",
...
def repair_mt5_record_identities(result: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    for execution in completed_executions:
        ...
        for _ in range(expected_records):
            ...
            record["source_variant_id"] = "session_age_v01_base_leaf20_q90"
...
def finalize_result(result: Mapping[str, Any]) -> dict[str, Any]:
    ...
    out["judgment"] = JUDGMENT if completed else "blocked_session_age_regime_mt5_probe"
...
def write_preliminary_kpi(result: Mapping[str, Any]) -> None:
    ...
    write_json(
        ...
        {
            **manifest,
            "kpi_scope": "session_age_regime_probe_preliminary",
            "model_family": MODEL_FAMILY,
            "feature_set_id": FEATURE_SET_ID,
            "label_id": LABEL_ID,
            "split_contract": SPLIT_CONTRACT,
            "mt5": {"scoreboard_lane": "runtime_probe_session_age_regime", "kpi_records": result.get("mt5_kpi_records", [])},
        },
    )
...
def build_packet_summary(
    ...
    return {
        ...
        "judgment": JUDGMENT if completed else "blocked_session_age_regime_mt5_probe",
        "boundary": BOUNDARY,
        "allowed_claims": ["runtime_probe_session_age_completed", "no_fold07_session_age_probe_completed"] if completed else ["blocked"],
...
def runtime_evidence_gate(summary: Mapping[str, Any]) -> dict[str, Any]:
    passed = summary.get("external_verification_status") == "completed" and int(summary.get("mt5_attempts_total", 0)) == 30
    return {
        ...
        "allowed_claims": ["runtime_probe_session_age_completed"] if passed else ["blocked"],
    ...
    run_rows = [
        {
            "run_id": RUN_ID,
            "stage_id": STAGE_ID,
            "lane": "session_age_regime_runtime_probe",
            "status": status,
        ...
        rows.append(
            {
                ...
                "tier_scope": record.get("tier_scope"),
                "kpi_scope": "session_age_regime_python_wfo_no_fold07",
                "scoreboard_lane": "runtime_probe_session_age_regime",
                ...
                "notes": "RUN03M Python session-age WFO row; not alpha quality or promotion.",
            }
        )
    return rows
        ...
        rows.append(
            {
                ...
                "record_view": record.get("record_view"),
                "tier_scope": record.get("tier_scope"),
                "kpi_scope": "session_age_regime_mt5_fold05",
                "scoreboard_lane": "runtime_probe_session_age_regime",
                "status": record.get("status"),
                "judgment": summary["judgment"],
                ...
                "external_verification_status": summary["external_verification_status"],
                "notes": "RUN03M MT5 fold05 session-age row; runtime probe only.",
            }
        )
    return rows
...
def write_work_packet(created_at: str, summary: Mapping[str, Any]) -> None:
    packet = {
        ...
        "user_request": {"user_quote": "그럼 또 아예 주제를 다르게 파볼만한거 fold7 안쓰고 / ㄱㄱ", "requested_action": "run session-age regime probe without fold07", "requested_count": {"value": 1, "n_a_reason": None}, "ambiguous_terms": ["fold7 means fold07 is excluded from both Python WFO and MT5 verification."]},
        ...
        "decision_lock": {"mode": "user_explicit_no_fold07", "assumptions": {"fold07_excluded": True, "python_folds": "fold01-fold06", "mt5_fold": MT5_FOLD_ID, "session_buckets": [b["label"] for b in SESSION_BUCKETS]}, "questions": [], "required_user_decisions": []},
        "interpreted_scope": {"work_families": ["runtime_backtest"], "target_surfaces": [rel(RUN_ROOT), rel(PACKET_ROOT), rel(STAGE_RUN_LEDGER_PATH), rel(PROJECT_ALPHA_LEDGER_PATH)], "scope_units": ["single_idea", "session_bucket", "fold", "tier_view", "mt5_attempt", "kpi_record", "ledger"], "execution_layers": ["python_wfo", "onnx_export", "mt5_strategy_tester", "kpi_recording", "ledger_update", "document_edit"], "mutation_policy": {"allowed": True, "boundary": "new RUN03M artifacts and current truth only"}, "evidence_layers": ["python_session_metrics", "mt5_reports", "normalized_kpi", "trade_attribution", "ledgers", "work_packet"], "reduction_policy": {"reduction_allowed": False, "requires_user_quote": False}, "claim_boundary": {"allowed_claims": summary["allowed_claims"], "forbidden_claims": summary["forbidden_claims"]}},
        "acceptance_criteria": [
            ...
            {"id": "AC-002", "text": "Five session buckets are recorded for Tier A, Tier B, and Tier A+B.", "expected_artifact": rel(RUN_ROOT / "results/session_bucket_summary.csv"), "verification_method": "kpi_contract_audit", "required": True},
        ...
        "work_plan": {"phases": [{"id": "python_wfo", "actions": ["train_fold01_to_fold06", "score_session_buckets"]}, {"id": "execute_mt5", "actions": ["train_fold05_models", "export_bucket_feature_matrices", "run_30_strategy_tester_attempts"]}, {"id": "closeout", "actions": ["record_kpi", "record_trade_attribution", "update_ledgers", "sync_current_truth", "run_gates"]}], "expected_outputs": [rel(RUN_ROOT / "reports/result_summary.md"), rel(PACKET_ROOT / "closeout_gate.json")], "stop_conditions": ["fold07_used", "missing_mt5_reports", "parser_errors", "ledger_rows_missing"]},
...
def skill_receipts(created_at: str, summary: Mapping[str, Any]) -> list[dict[str, Any]]:
    return [
        {"packet_id": PACKET_ID, "created_at_utc": created_at, "triggered": True, "blocking": False, "skill": "obsidian-runtime-parity", "status": "executed", "python_artifact": rel(RUN_ROOT / "results/session_bucket_summary.csv"), "runtime_artifact": rel(RUN_ROOT / "run_manifest.json"), "compared_surface": "Python session bucket WFO, ONNX exports, bucket feature matrices, and MT5 Strategy Tester reports", "parity_level": "runtime_probe", "tester_identity": f"US100 M5, deposit=500, leverage=1:100, mt5_fold={MT5_FOLD_ID}, attempts={summary.get('mt5_attempts_total')}", "missing_evidence": "none" if summary["external_verification_status"] == "completed" else "blocked_mt5_attempts", "allowed_claims": ["runtime_probe_session_age_completed"], "forbidden_claims": ["alpha_quality", "promotion_candidate", "operating_promotion", "runtime_authority"]},
        {"packet_id": PACKET_ID, "created_at_utc": created_at, "triggered": True, "blocking": False, "skill": "obsidian-experiment-design", "status": "executed", "hypothesis": "ExtraTrees signal quality may be concentrated in a broad session-age regime.", "baseline": SOURCE_WFO_RUN_ID, "changed_variables": ["post-score session bucket segmentation", "fold07 exclusion"], "invalid_conditions": ["fold07 included", "missing tier view", "missing MT5 reports"], "evidence_plan": [rel(RUN_ROOT / "results/session_bucket_summary.csv"), rel(PACKET_ROOT / "normalized_kpi_records.jsonl")]},
        {"packet_id": PACKET_ID, "created_at_utc": created_at, "triggered": True, "blocking": False, "skill": "obsidian-model-validation", "status": "executed", "model_or_threshold_surface": "ExtraTrees v01 base leaf20 q90 with session-age post-score segmentation", "validation_split": "Python fold01-fold06 only and MT5 fold05 only; fold07 excluded", "overfit_checks": "bucket hit-rate gaps, both-positive folds, collapse folds, and MT5 validation/test bucket comparison", "selection_metric_boundary": "exploratory bucket read only; no operating selection metric", "allowed_claims": ["runtime_probe_session_age_completed"], "forbidden_claims": ["alpha_quality", "promotion_candidate", "operating_promotion", "runtime_authority"]},
        ...
        {"packet_id": PACKET_ID, "created_at_utc": created_at, "triggered": True, "blocking": False, "skill": "obsidian-exploration-mandate", "status": "executed", "exploration_lane": "Stage 12 ExtraTrees session-age regime probe without fold07", "idea_boundary": "market/time structure axis, not model variant or baseline", "negative_memory_effect": "Weak bucket behavior is retained as failure memory and should not trigger micro tuning.", "operating_claim_boundary": "No alpha quality, promotion candidate, operating promotion, or runtime authority."},
        {"packet_id": PACKET_ID, "created_at_utc": created_at, "triggered": True, "blocking": False, "skill": "obsidian-result-judgment", "status": "executed", "judgment_boundary": BOUNDARY, "allowed_claims": ["runtime_probe_session_age_completed", "no_fold07_session_age_probe_completed"], "forbidden_claims": ["alpha_quality", "promotion_candidate", "operating_promotion", "runtime_authority"], "evidence_used": [rel(RUN_ROOT / "results/session_bucket_summary.csv"), rel(PACKET_ROOT / "normalized_kpi_records.jsonl")]},
    ]


def write_reports(summary: Mapping[str, Any]) -> None:
    best = summary["best_python_routed_bucket"]
    routed = [row for row in summary["mt5_routed_rows"] if row.get("route_role") == "total"]
    lines = [
        "# RUN03M Session Age Regime Probe(RUN03M 세션 경과 국면 탐침)",
        "",
        f"- run(실행): `{RUN_ID}`",
    ...
    lines.extend(["", "효과(effect, 효과): session age(세션 경과 시간)가 신호 품질을 가르는지 확인했지만, alpha quality(알파 품질), promotion candidate(승격 후보), runtime authority(런타임 권위)는 아니다."])
...
"""
    ...
    plan = f"# RUN03M Session Age Regime Probe Plan(RUN03M 세션 경과 국면 탐침 계획)\n\nfold07(접힘 7)을 제외하고 session age(세션 경과 시간) 주제를 탐색한다.\n\n효과(effect, 효과): 모델 변형이 아니라 시장 시간 구조를 본다.\n"
    review = f"# RUN03M Review Packet(RUN03M 검토 묶음)\n\n`{RUN_ID}` completed(완료). 효과(effect, 효과): fold07(접힘 7) 없이 session bucket(세션 구간) 근거를 남겼다.\n"
...
def write_run_files(result: Mapping[str, Any], summary: Mapping[str, Any]) -> None:
    ...
    write_json(RUN_ROOT / "run_manifest.json", manifest)
    write_json(RUN_ROOT / "kpi_record.json", {**manifest, "kpi_scope": "session_age_regime_probe", "model_family": MODEL_FAMILY, "feature_set_id": FEATURE_SET_ID, "label_id": LABEL_ID, "split_contract": SPLIT_CONTRACT, "python_wfo": summary.get("best_python_routed_bucket"), "mt5": {"scoreboard_lane": "runtime_probe_session_age_regime", "kpi_records": result.get("mt5_kpi_records", [])}})
...
def update_workspace_state(summary: Mapping[str, Any]) -> None:
  ...
  stage_id: {STAGE_ID}
  status: active_session_age_regime_probe_completed
  lane: session_age_regime_runtime_probe
  ...
  current_summary:
    ...
    next_action: decide_session_bucket_followup_or_other_axis
"""
...
def update_current_working_state(summary: Mapping[str, Any]) -> None:
    best = summary["best_python_routed_bucket"]
    text = re.sub(r"- current run\(현재 실행\): `[^`]+`", f"- current run(현재 실행): `{RUN_ID}`", read_text(CURRENT_STATE_PATH), count=1)
    section = f"""## RUN03M session age regime probe(세션 경과 국면 탐침)
...
"""
    write_text(CURRENT_STATE_PATH, replace_section(text, "## RUN03M session age regime probe(세션 경과 국면 탐침)", section))
...
## Current Read - RUN03M session age regime probe(현재 판독 - RUN03M 세션 경과 국면 탐침)
...
Effect(효과): fold07(접힘 7) 없이 session age(세션 경과 시간) 주제를 확인했다. baseline(기준선)이나 promotion candidate(승격 후보)는 아니다.
"""
...
def update_changelog(summary: Mapping[str, Any]) -> None:
    ...
    if RUN_ID in text:
        return
    entry = f"- 2026-05-01: `{RUN_ID}` completed(완료). fold07(접힘 7) 없이 session-age regime probe(세션 경과 국면 탐침)를 실행했다. attempts(시도) `{summary['mt5_attempts_total']}`, normalized KPI(정규화 KPI) `{summary['normalized_records']}`. Effect(효과): 모델 미세조정이 아니라 시간 구조 축을 failure/salvage memory(실패/회수 기억)로 남겼다.\n"
...
def update_idea_registry(summary: Mapping[str, Any]) -> None:
    ...
    if IDEA_ID in text:
        return
    best = summary["best_python_routed_bucket"]
    row = f"| `{IDEA_ID}` | `{STAGE_ID}` | ExtraTrees(엑스트라 트리) 신호는 특정 session age(세션 경과 시간)에 집중될 수 있다 | `Tier A + Tier B combined(Tier A + Tier B 합산)` | `runtime_probe_completed_inconclusive` | RUN03M(실행 03M); fold07(접힘 7) 제외; best bucket(최상위 구간) `{best.get('session_bucket_label')}` |\n"
...
def update_negative_register(summary: Mapping[str, Any]) -> None:
    ...
    if IDEA_ID in text:
        return
    row = f"| `NR-019` | `{IDEA_ID}` | session age(세션 경과 시간)만으로 ExtraTrees(엑스트라 트리) 신호가 안정될 수 있다 | RUN03M(실행 03M) best bucket(최상위 구간)도 both-positive fold(양쪽 양수 접힘)가 `{best.get('both_positive_fold_count')}`라 반복성이 약했다 | session bucket(세션 구간)은 보조 설명 축으로 남긴다 | 특정 bucket(구간)이 다른 label/model(라벨/모델)에서 반복될 때 |\n"
...
def run_closeout_gate(summary: Mapping[str, Any]) -> None:
    ...
    for check in (
        ("session_summary_rows", "15", RUN_ROOT / "results/session_bucket_summary.csv", "five-buckets-three-tier-summary"),
        ("session_metric_rows", "180", RUN_ROOT / "results/session_bucket_metrics.csv", "fold01-fold06-bucket-metrics"),
        ("mt5_attempt_rows", "30", PACKET_ROOT / "attempt_summary.csv", "fold05-bucket-mt5-attempts"),
        ("normalized_rows", "50", PACKET_ROOT / "normalized_kpi_summary.csv", "normalized-mt5-kpi-summary"),
    ):
    ...
    cmd.extend(["--skill-receipt-json", str(PACKET_ROOT / "skill_receipts.json"), "--skill-receipt-schema", "docs/agent_control/skill_receipt_schema.yaml", "--extra-audit-json", str(PACKET_ROOT / "runtime_evidence_gate.json"), "--required-gate-coverage", "--closeout-report", str(PACKET_ROOT / "closeout_report.md"), "--kpi-run-id", RUN_ID, "--kpi-stage-id", STAGE_ID, "--kpi-run-root", str(RUN_ROOT), "--stage-ledger", str(STAGE_RUN_LEDGER_PATH), "--project-ledger", str(PROJECT_ALPHA_LEDGER_PATH), "--expected-stage-ledger-rows", "65", "--expected-project-ledger-rows", "65", "--output-json", str(PACKET_ROOT / "closeout_gate.json")])
...
def repair_existing_run() -> dict[str, Any]:
    ...
    if not path_exists(manifest_path) or not path_exists(kpi_path) or not path_exists(bucket_summary_path):
        raise RuntimeError("RUN03M existing artifacts are incomplete; cannot repair without rerunning.")
    manifest = json.loads(io_path(manifest_path).read_text(encoding="utf-8"))
    kpi_payload = json.loads(io_path(kpi_path).read_text(encoding="utf-8"))
    result = dict(manifest)
    reports_by_name = {report.get("report_name"): report for report in result.get("strategy_tester_reports", [])}
    attempts_by_name = {attempt.get("attempt_name"): attempt for attempt in result.get("attempts", [])}
    ...
    result["execution_results"] = corrected_executions
    result["mt5_kpi_records"] = build_mt5_kpi_records(result.get("execution_results", []))
    if not result["mt5_kpi_records"]:
        result["mt5_kpi_records"] = kpi_payload.get("mt5", {}).get("kpi_records", [])
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run Stage 12 session-age regime probe without fold07.")
