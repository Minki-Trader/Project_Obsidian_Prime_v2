...
STAGE_NUMBER = 12
RUN_NUMBER = "run03S"
RUN_ID = "run03S_et_probability_shape_attribution_probe_v1"
PACKET_ID = "stage12_run03s_probability_shape_attribution_probe_v1"
EXPLORATION_LABEL = "stage12_Model__ExtraTreesProbabilityShapeAttribution"
IDEA_ID = "IDEA-ST12-ET-PROBABILITY-SHAPE-ATTRIBUTION"
...
FEATURE_SET_ID = "feature_set_v2_mt5_price_proxy_58_and_core42_partial_context_probability_shape_attribution"
LABEL_ID = "label_v1_fwd12_m5_logret_train_q33_3class"
SPLIT_CONTRACT = "rolling_wfo_fold01_to_fold06_probability_shape_attribution_buckets_mt5_fold05_no_fold07"
BOUNDARY = "runtime_probe_probability_shape_attribution_not_alpha_quality_not_promotion_not_runtime_authority"
JUDGMENT = "inconclusive_probability_shape_attribution_runtime_probe_completed"
HIT_FLOOR = 0.48
MT5_FOLD_ID = "fold05"
...
PLAN_PATH = STAGE_ROOT / "00_spec/run03S_probability_shape_attribution_probe_plan.md"
STAGE_REVIEW_PATH = STAGE_ROOT / "03_reviews/run03S_probability_shape_attribution_probe_packet.md"
...
SESSION_BUCKETS = (
    {"id": "thin_edge", "label": "thin_probability_edge", "flag_min": 0, "flag_max": 0},
    {"id": "middle_edge", "label": "middle_probability_edge", "flag_min": 1, "flag_max": 1},
    {"id": "wide_edge", "label": "wide_probability_edge", "flag_min": 2, "flag_max": 2},
)
...
PROBABILITY_MARGIN_THIN_MAX = 0.09
PROBABILITY_MARGIN_WIDE_MIN = 0.13
...
def probability_shape_scores(probs: np.ndarray) -> tuple[pd.Series, np.ndarray, np.ndarray]:
    p_short = probs[:, 0]
    p_flat = probs[:, 1]
    p_long = probs[:, 2]
    ...
    score = pd.Series(1, index=range(len(probs)), dtype="int64")
    score.loc[margin < PROBABILITY_MARGIN_THIN_MAX] = 0
    score.loc[margin >= PROBABILITY_MARGIN_WIDE_MIN] = 2
    return score, nonflat_conf, margin


def probability_shape_attribution_flag_count(frame: pd.DataFrame) -> pd.Series:
    if "probability_shape_score" in frame.columns:
        return pd.to_numeric(frame["probability_shape_score"], errors="coerce").fillna(0).astype("int64")
    return pd.Series(0, index=frame.index, dtype="int64")


def assign_session_bucket(frame: pd.DataFrame) -> pd.Series:
    score = probability_shape_attribution_flag_count(frame)
    out = pd.Series("middle_edge", index=frame.index, dtype="object")
    out.loc[score.eq(0)] = "thin_edge"
    out.loc[score.ge(2)] = "wide_edge"
    return out


def attach_probability_shape(frame: pd.DataFrame, probs: np.ndarray) -> pd.DataFrame:
    score, nonflat_conf, margin = probability_shape_scores(probs)
    out = frame.copy().reset_index(drop=True)
    score = score.reset_index(drop=True)
    out["probability_shape_score"] = score.to_numpy()
    out["probability_nonflat_conf"] = nonflat_conf
    out["probability_margin"] = margin
    out["session_bucket_id"] = assign_session_bucket(out).to_numpy()
    out["session_bucket_label"] = [BUCKET_BY_ID.get(str(item), {}).get("label", "outside") for item in out["session_bucket_id"]]
    return out
...
def decision_frame(
    *,
    ...
    shape_score, _shape_conf, _shape_margin = probability_shape_scores(probs)
    bucket_series = pd.Series("middle_edge", index=range(len(source)), dtype="object")
    bucket_series.loc[shape_score.eq(0)] = "thin_edge"
    bucket_series.loc[shape_score.ge(2)] = "wide_edge"
    return pd.DataFrame(
        {
            "run_id": RUN_ID,
            "fold_id": fold_id,
            "variant_id": "probability_shape_attribution_v01_base_leaf20_q90",
            ...
            "session_bucket_id": bucket_series.to_numpy(),
            "session_bucket_label": [BUCKET_BY_ID.get(str(item), {}).get("label", "outside") for item in bucket_series],
            "probability_shape_attribution_flag_count": shape_score.to_numpy(),
            "label_class": labels,
...
def metric_row(frame: pd.DataFrame, fold: Any, split_role: str, bucket_id: str) -> dict[str, Any]:
    ...
    return {
        ...
        "variant_id": first.get("variant_id", "probability_shape_attribution_v01_base_leaf20_q90"),
        "source_variant_id": REFERENCE_VARIANT_ID,
        "idea_id": IDEA_ID,
        "split_role": split_role,
...
def make_attempts(
    *,
    ...
    for attempt in attempts:
        ...
        attempt["source_variant_id"] = "probability_shape_attribution_v01_base_leaf20_q90"
...
def prepare_mt5_run() -> dict[str, Any]:
    ...
    tier_a_test_probs = ordered_probs(tier_a_model, a_test, tier_a_order)
    tier_b_test_probs = ordered_probs(tier_b_model, b_test, tier_b_order)
    ...
    a_validation_bucketed = attach_probability_shape(a_validation, tier_a_val_probs)
    a_test_bucketed = attach_probability_shape(a_test, tier_a_test_probs)
    b_validation_bucketed = attach_probability_shape(b_validation, tier_b_val_probs)
    b_test_bucketed = attach_probability_shape(b_test, tier_b_test_probs)
    tier_a_hash = mt5.ordered_hash(tier_a_order)
    tier_b_hash = mt5.ordered_hash(tier_b_order)
    tier_a_joblib = RUN_ROOT / "models/tier_a_probability_shape_attribution_fold05_model.joblib"
    tier_b_joblib = RUN_ROOT / "models/tier_b_probability_shape_attribution_fold05_core42_model.joblib"
    ...
    if not onnx_parity["passed"]:
        raise RuntimeError("RUN03S ONNX parity failed")
    ...
    for split_label, wfo_role, a_frame, b_frame in (
        ("validation_is", "validation", a_validation_bucketed, b_validation_bucketed),
        ("oos", "test", a_test_bucketed, b_test_bucketed),
    ):
    ...
    summary = {
        ...
        "probability_shape_attribution_buckets": SESSION_BUCKETS,
    ...
    return {
        **summary,
        ...
        "route_coverage": route_coverage,
        "completion_goal": "probability_shape_attribution_probe_no_fold07_with_fold05_mt5",
...
def repair_mt5_record_identities(result: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    for execution in completed_executions:
        ...
        for _ in range(expected_records):
            ...
            record["source_variant_id"] = "probability_shape_attribution_v01_base_leaf20_q90"
...
def finalize_result(result: Mapping[str, Any]) -> dict[str, Any]:
    ...
    out["judgment"] = JUDGMENT if completed else "blocked_probability_shape_attribution_mt5_probe"
...
def write_preliminary_kpi(result: Mapping[str, Any]) -> None:
    ...
    write_json(
        ...
        {
            **manifest,
            "kpi_scope": "probability_shape_attribution_probe_preliminary",
            "model_family": MODEL_FAMILY,
            "feature_set_id": FEATURE_SET_ID,
            "label_id": LABEL_ID,
            "split_contract": SPLIT_CONTRACT,
            "mt5": {"scoreboard_lane": "runtime_probe_probability_shape_attribution", "kpi_records": result.get("mt5_kpi_records", [])},
        },
    )
...
def build_packet_summary(
    ...
    return {
        ...
        "judgment": JUDGMENT if completed else "blocked_probability_shape_attribution_mt5_probe",
        "boundary": BOUNDARY,
        "allowed_claims": ["runtime_probe_probability_shape_attribution_completed", "no_fold07_probability_shape_attribution_probe_completed"] if completed else ["blocked"],
...
def runtime_evidence_gate(summary: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "allowed_claims": ["runtime_probe_probability_shape_attribution_completed"] if passed else ["blocked"],
    ...
    run_rows = [
        {
            "run_id": RUN_ID,
            "stage_id": STAGE_ID,
            "lane": "probability_shape_attribution_runtime_probe",
            "status": status,
        ...
        rows.append(
            {
                ...
                "tier_scope": record.get("tier_scope"),
                "kpi_scope": "probability_shape_attribution_python_wfo_no_fold07",
                "scoreboard_lane": "runtime_probe_probability_shape_attribution",
                ...
                "notes": "RUN03S Python probability-shape-attribution-regime WFO row; not alpha quality or promotion.",
            }
        )
    return rows
        ...
        rows.append(
            {
                ...
                "record_view": record.get("record_view"),
                "tier_scope": record.get("tier_scope"),
                "kpi_scope": "probability_shape_attribution_mt5_fold05",
                "scoreboard_lane": "runtime_probe_probability_shape_attribution",
                "status": record.get("status"),
                "judgment": summary["judgment"],
                ...
                "external_verification_status": summary["external_verification_status"],
                "notes": "RUN03S MT5 fold05 probability-shape-attribution-regime row; runtime probe only.",
            }
        )
    return rows
...
def write_work_packet(created_at: str, summary: Mapping[str, Any]) -> None:
    packet = {
        ...
        "user_request": {"user_quote": "하나씩 run으로 구성해서 mt5까지 연동해서 검증해줘", "requested_action": "run probability-shape attribution regime probe without fold07 and include MT5 verification", "requested_count": {"value": 1, "n_a_reason": None}, "ambiguous_terms": ["one at a time means RUN03S first."]},
        ...
        "decision_lock": {"mode": "user_explicit_one_run_no_variant", "assumptions": {"fold07_excluded": True, "python_folds": "fold01-fold06", "mt5_fold": MT5_FOLD_ID, "probability_shape_attribution_buckets": [b["label"] for b in SESSION_BUCKETS]}, "questions": [], "required_user_decisions": []},
        "interpreted_scope": {"work_families": ["runtime_backtest"], "target_surfaces": [rel(RUN_ROOT), rel(PACKET_ROOT), rel(STAGE_RUN_LEDGER_PATH), rel(PROJECT_ALPHA_LEDGER_PATH)], "scope_units": ["single_idea", "probability_shape_attribution_bucket", "fold", "tier_view", "mt5_attempt", "kpi_record", "ledger"], "execution_layers": ["python_wfo", "onnx_export", "mt5_strategy_tester", "kpi_recording", "ledger_update", "document_edit"], "mutation_policy": {"allowed": True, "boundary": "new RUN03S artifacts and current truth only"}, "evidence_layers": ["python_probability_shape_attribution_metrics", "mt5_reports", "normalized_kpi", "trade_attribution", "ledgers", "work_packet"], "reduction_policy": {"reduction_allowed": False, "requires_user_quote": False}, "claim_boundary": {"allowed_claims": summary["allowed_claims"], "forbidden_claims": summary["forbidden_claims"]}},
        "acceptance_criteria": [
            ...
            {"id": "AC-002", "text": "Three probability-shape attribution buckets are recorded for Tier A, Tier B, and Tier A+B.", "expected_artifact": rel(RUN_ROOT / "results/session_bucket_summary.csv"), "verification_method": "kpi_contract_audit", "required": True},
...
def skill_receipts(created_at: str, summary: Mapping[str, Any]) -> list[dict[str, Any]]:
    return [
        {"packet_id": PACKET_ID, "created_at_utc": created_at, "triggered": True, "blocking": False, "skill": "obsidian-runtime-parity", "status": "executed", "python_artifact": rel(RUN_ROOT / "results/session_bucket_summary.csv"), "runtime_artifact": rel(RUN_ROOT / "run_manifest.json"), "compared_surface": "Python probability-shape attribution bucket WFO, ONNX exports, bucket feature matrices, and MT5 Strategy Tester reports", "parity_level": "runtime_probe", "tester_identity": f"US100 M5, deposit=500, leverage=1:100, mt5_fold={MT5_FOLD_ID}, attempts={summary.get('mt5_attempts_total')}", "missing_evidence": "none" if summary["external_verification_status"] == "completed" else "blocked_mt5_attempts", "allowed_claims": ["runtime_probe_probability_shape_attribution_completed"], "forbidden_claims": ["alpha_quality", "promotion_candidate", "operating_promotion", "runtime_authority"]},
        {"packet_id": PACKET_ID, "created_at_utc": created_at, "triggered": True, "blocking": False, "skill": "obsidian-experiment-design", "status": "executed", "hypothesis": "ExtraTrees signal quality may depend on broad probability_shape_attribution regimes.", "baseline": SOURCE_WFO_RUN_ID, "changed_variables": ["post-score probability-shape attribution bucket segmentation", "fold07 exclusion"], "invalid_conditions": ["fold07 included", "missing tier view", "missing MT5 reports"], "evidence_plan": [rel(RUN_ROOT / "results/session_bucket_summary.csv"), rel(PACKET_ROOT / "normalized_kpi_records.jsonl")]},
        {"packet_id": PACKET_ID, "created_at_utc": created_at, "triggered": True, "blocking": False, "skill": "obsidian-model-validation", "status": "executed", "model_or_threshold_surface": "ExtraTrees v01 base leaf20 q90 with probability-shape-attribution-regime post-score segmentation", "validation_split": "Python fold01-fold06 only and MT5 fold05 only; fold07 excluded", "overfit_checks": "bucket hit-rate gaps, both-positive folds, collapse folds, and MT5 validation/test bucket comparison", "selection_metric_boundary": "exploratory bucket read only; no operating selection metric", "allowed_claims": ["runtime_probe_probability_shape_attribution_completed"], "forbidden_claims": ["alpha_quality", "promotion_candidate", "operating_promotion", "runtime_authority"]},
        ...
        {"packet_id": PACKET_ID, "created_at_utc": created_at, "triggered": True, "blocking": False, "skill": "obsidian-exploration-mandate", "status": "executed", "exploration_lane": "Stage 12 ExtraTrees probability-shape-attribution-regime probe without fold07", "idea_boundary": "market probability-shape attribution structure axis, not model variant or baseline", "negative_memory_effect": "Weak bucket behavior is retained as failure memory and should not trigger micro tuning.", "operating_claim_boundary": "No alpha quality, promotion candidate, operating promotion, or runtime authority."},
        {"packet_id": PACKET_ID, "created_at_utc": created_at, "triggered": True, "blocking": False, "skill": "obsidian-result-judgment", "status": "executed", "judgment_boundary": BOUNDARY, "allowed_claims": ["runtime_probe_probability_shape_attribution_completed", "no_fold07_probability_shape_attribution_probe_completed"], "forbidden_claims": ["alpha_quality", "promotion_candidate", "operating_promotion", "runtime_authority"], "evidence_used": [rel(RUN_ROOT / "results/session_bucket_summary.csv"), rel(PACKET_ROOT / "normalized_kpi_records.jsonl")]},
    ]
...
def write_reports(summary: Mapping[str, Any]) -> None:
    ...
    lines = [
        "# RUN03S Probability Shape Attribution Regime Probe(RUN03S 확률 모양 귀속 국면 탐침)",
        "",
        f"- run(실행): `{RUN_ID}`",
        "- fold07(접힘 7): `excluded(제외)`",
        f"- Python folds(파이썬 접힘): `{','.join(summary['folds_used'])}`",
        f"- MT5 fold(MT5 접힘): `{MT5_FOLD_ID}`",
        "- probability-shape attribution bucket(확률 모양 귀속 구간): `thin_edge/middle_edge/wide_edge`",
    ...
    lines.extend(["", "효과(effect, 효과): probability-shape attribution regime(확률 모양 귀속 국면)이 신호 품질을 가르는지 확인했지만, alpha quality(알파 품질), promotion candidate(승격 후보), runtime authority(런타임 권위)는 아니다."])
    write_text(RUN_ROOT / "reports/result_summary.md", "\n".join(lines))
    closeout = f"""# {PACKET_ID} Closeout Report(마감 보고서)

## Conclusion(결론)

RUN03S(실행 03S)는 fold07(접힘 7)을 쓰지 않고 probability-shape attribution regime(확률 모양 귀속 국면)을 탐침했다.

## What changed(무엇이 바뀌었나)

- fold01~fold06(접힘 1~6)만 Python WFO(파이썬 워크포워드 최적화)에 사용했다.
- MT5(메타트레이더5)는 fold05(접힘 5)만 사용했다.
- 신호를 3개 probability-shape attribution bucket(확률 모양 귀속 구간)으로 나눴다.
...
full MT5 WFO(전체 MT5 워크포워드 최적화)는 실행하지 않았다. 이 packet(묶음)은 fold07(접힘 7) 없는 probability-shape attribution runtime probe(확률 모양 귀속 런타임 탐침)다.

## Allowed claims(허용 주장)

`runtime_probe_probability_shape_attribution_completed`, `no_fold07_probability_shape_attribution_probe_completed`.
...
"""
    ...
    plan = "# RUN03S Probability Shape Attribution Regime Probe Plan(RUN03S 확률 모양 귀속 국면 탐침 계획)\n\nfold07(접힘 7)을 제외하고 probability-shape attribution regime(확률 모양 귀속 국면) 주제를 탐색한다.\n\n효과(effect, 효과): 모델 변형이 아니라 시장 확률 모양 귀속 구조를 본다.\n"
    review = f"# RUN03S Review Packet(RUN03S 검토 묶음)\n\n`{RUN_ID}` completed(완료). 효과(effect, 효과): fold07(접힘 7) 없이 probability-shape attribution bucket(확률 모양 귀속 구간) 근거를 남겼다.\n"
...
def write_run_files(result: Mapping[str, Any], summary: Mapping[str, Any]) -> None:
    ...
    write_json(RUN_ROOT / "run_manifest.json", manifest)
    write_json(RUN_ROOT / "kpi_record.json", {**manifest, "kpi_scope": "probability_shape_attribution_probe", "model_family": MODEL_FAMILY, "feature_set_id": FEATURE_SET_ID, "label_id": LABEL_ID, "split_contract": SPLIT_CONTRACT, "python_wfo": summary.get("best_python_routed_bucket"), "mt5": {"scoreboard_lane": "runtime_probe_probability_shape_attribution", "kpi_records": result.get("mt5_kpi_records", [])}})
...
def update_workspace_state(summary: Mapping[str, Any]) -> None:
  ...
  stage_id: {STAGE_ID}
  status: active_probability_shape_attribution_probe_completed
  lane: probability_shape_attribution_runtime_probe
...
def update_current_working_state(summary: Mapping[str, Any]) -> None:
    ...
    section = f"""## RUN03S probability-shape attribution regime probe(확률 모양 귀속 국면 탐침)
...
"""
    write_text(CURRENT_STATE_PATH, replace_section(text, "## RUN03S probability-shape attribution regime probe(확률 모양 귀속 국면 탐침)", section))
...
## Current Read - RUN03S probability-shape attribution regime probe(현재 판독 - RUN03S 확률 모양 귀속 국면 탐침)
...
def update_changelog(summary: Mapping[str, Any]) -> None:
    ...
    if RUN_ID in text:
        return
    entry = f"- 2026-05-01: `{RUN_ID}` completed(완료). fold07(접힘 7) 없이 probability-shape attribution-regime probe(확률 모양 귀속 국면 탐침)를 실행했다. attempts(시도) `{summary['mt5_attempts_total']}`, normalized KPI(정규화 KPI) `{summary['normalized_records']}`. Effect(효과): 모델 미세조정이 아니라 확률 모양 귀속 구조 축을 failure/salvage memory(실패/회수 기억)로 남겼다.\n"
...
def update_idea_registry(summary: Mapping[str, Any]) -> None:
    ...
    if IDEA_ID in text:
        return
    best = summary["best_python_routed_bucket"]
    row = f"| `{IDEA_ID}` | `{STAGE_ID}` | ExtraTrees(엑스트라 트리) 신호는 특정 probability-shape attribution regime(확률 모양 귀속 국면)에 집중될 수 있다 | `Tier A + Tier B combined(Tier A + Tier B 합산)` | `runtime_probe_completed_inconclusive` | RUN03S(실행 03S); fold07(접힘 7) 제외; best bucket(최상위 구간) `{best.get('session_bucket_label')}` |\n"
...
def update_negative_register(summary: Mapping[str, Any]) -> None:
    ...
    if IDEA_ID in text:
        return
    row = f"| `NR-025` | `{IDEA_ID}` | probability-shape attribution regime(확률 모양 귀속 국면)만으로 ExtraTrees(엑스트라 트리) 신호가 안정될 수 있다 | RUN03S(실행 03S) best bucket(최상위 구간)의 both-positive fold(양쪽 양수 접힘)는 `{best.get('both_positive_fold_count')}`다 | probability-shape attribution bucket(확률 모양 귀속 구간)은 보조 설명 축으로 남긴다 | 특정 bucket(구간)이 다른 label/model(라벨/모델)에서 반복될 때 |\n"
...
def run_closeout_gate(summary: Mapping[str, Any]) -> None:
    ...
    for check in (
        ("session_summary_rows", "9", RUN_ROOT / "results/session_bucket_summary.csv", "three-probability_shape_attribution-buckets-three-tier-summary"),
        ("session_metric_rows", "108", RUN_ROOT / "results/session_bucket_metrics.csv", "fold01-fold06-probability_shape_attribution-bucket-metrics"),
        ("mt5_attempt_rows", "18", PACKET_ROOT / "attempt_summary.csv", "fold05-probability_shape_attribution-bucket-mt5-attempts"),
...
def repair_existing_run() -> dict[str, Any]:
    ...
    if not path_exists(manifest_path) or not path_exists(kpi_path) or not path_exists(bucket_summary_path):
        raise RuntimeError("RUN03S existing artifacts are incomplete; cannot repair without rerunning.")
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run Stage 12 probability_shape_attribution regime probe without fold07.")
