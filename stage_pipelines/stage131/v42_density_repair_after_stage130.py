...
STAGE_ID = "131_adapter_research__new_v2_model_branch_followup_review"
RUN_NUMBER = "run131A"
RUN_ID = "run131A_stage131_new_v2_model_branch_followup_review_v1"
PACKET_ID = "stage131_new_v2_model_branch_followup_review_v1"
PARENT_RUN_ID = "run130A_stage130_new_v2_model_branch_after_v41_tradeoff_failure_v1"
SOURCE_ADAPTER_ID = "s130_v42_veto_sd2_h2_mr03_wideatr"
SOURCE_STAGE130_PUSHED_COMMIT = "9deecfcf34ef007107f5120f670d02016ad1601d"
NEXT_STAGE_ID = "132_adapter_research__v42_density_repair_followup"
NEXT_RUN_ID = "run132A_stage132_v42_density_repair_followup_v1"
NEXT_PACKET_ID = "stage132_v42_density_repair_followup_v1"
...
RUN50BN_V42_ANCHOR = "v42_v22_midcov_et40_veto_conflict_h2c0_no_b"
...
SOURCE_STAGE130_REPORT = Path("stages") / SOURCE_STAGE130_ID / "03_reviews/stage130_new_v2_model_branch_report.md"
SOURCE_STAGE130_SUMMARY = Path("stages") / SOURCE_STAGE130_ID / "03_reviews/stage130_new_v2_model_branch_summary.csv"
SOURCE_STAGE130_SEGMENTS = Path("stages") / SOURCE_STAGE130_ID / "03_reviews/stage130_segment_kpi_summary.csv"
SOURCE_STAGE130_RISK = Path("stages") / SOURCE_STAGE130_ID / "03_reviews/stage130_risk_atr_telemetry.csv"

COMMON_ROOT = f"OPV2/stage131/{RUN_NUMBER}"
MIN_MARGIN = 0.0

REPORT_PATH = REVIEWS_ROOT / "stage131_v42_density_repair_report.md"
SUMMARY_JSON_PATH = REVIEWS_ROOT / "stage131_v42_density_repair_summary.json"
SUMMARY_CSV_PATH = REVIEWS_ROOT / "stage131_v42_density_repair_summary.csv"
SEGMENT_KPI_PATH = REVIEWS_ROOT / "stage131_segment_kpi_summary.csv"
EQUITY_AUDIT_PATH = REVIEWS_ROOT / "stage131_equity_curve_audit.md"
RISK_ATR_TELEMETRY_PATH = REVIEWS_ROOT / "stage131_risk_atr_telemetry.csv"
DECISION_PATH = REVIEWS_ROOT / "stage131_decision.md"
AUDIT_CSV_PATH = REVIEWS_ROOT / "stage131_trade_audit.csv"
...
VARIANTS = (
    repair.RepairVariant(
        adapter_id="s131_v42_sd5_h2_mr03_wideatr",
        label="stage131_v42_same_direction_cooldown5",
        atr_enabled=True,
        ...
        max_hold_bars=2,
        notes="Stage131 bounded density repair: v42 source, same-direction cooldown 5.",
    ),
    repair.RepairVariant(
        adapter_id="s131_v42_cd5_sd5_h2_mr03_wideatr",
        label="stage131_v42_global_cooldown5_same_direction5",
        atr_enabled=True,
        model_risk_enabled=True,
        fixed_lot=0.25,
        atr_stop_multiplier=2.5,
        ...
        close_only_on_opposite_signal=True,
        max_hold_bars=2,
        notes="Stage131 bounded density repair: v42 source, global and same-direction cooldown 5.",
    ),
    repair.RepairVariant(
        adapter_id="s131_v42_cd10_sd10_h2_mr03_wideatr",
        label="stage131_v42_global_cooldown10_same_direction10",
        atr_enabled=True,
        model_risk_enabled=True,
        fixed_lot=0.25,
        atr_stop_multiplier=2.5,
        ...
        close_only_on_opposite_signal=True,
        max_hold_bars=2,
        notes="Stage131 bounded density repair: v42 source, stricter cooldown 10.",
    ),
    repair.RepairVariant(
        adapter_id="s131_v42_transition_sd5_h2_mr03_wideatr",
        label="stage131_v42_entry_transition_only_same_direction5",
        atr_enabled=True,
        model_risk_enabled=True,
        fixed_lot=0.25,
        atr_stop_multiplier=2.5,
        ...
        close_only_on_opposite_signal=True,
        max_hold_bars=2,
        notes="Stage131 bounded density repair: v42 source, entry only when signal transitions.",
    ),
)
...
def source_specs() -> dict[str, dict[str, Any]]:
    root = RUN50BN_ROOT / RUN50BN_V42_ANCHOR
    return {
        variant.adapter_id: {
            "label": RUN50BN_V42_ANCHOR,
            "run_root": RUN50BN_ROOT,
            "variant_root": root,
            "anchor": RUN50BN_V42_ANCHOR,
            "model": RUN50BN_MODEL,
            "signal_column": RUN50BN_SIGNAL,
            "validation_ini": root / "mt5/x02_ta_val.ini",
            "oos_ini": root / "mt5/x02_ta_oos.ini",
            "source_note": "Stage56 run50BN v42 source, reused only as v2-native research input.",
        }
...
def configure_reused_engine() -> None:
    ...
    engine.DEVELOPMENT_ANCHOR = RUN50BN_V42_ANCHOR
    ...
    for variant_index, variant in enumerate(VARIANTS, start=1):
        ...
        for split in ("validation_is", "oos"):
            ...
            for role_index, (tier, attempt_role, prefix, attempt_token) in enumerate(
                ...
                magic = 13113100 + variant_index * 100 + (1 if split == "validation_is" else 50) + role_index
                attempts.append(
                    attempt_payload(
                        run_root=variant_root,
                        run_id=RUN_ID,
                        stage_number=131,
                        exploration_label="stage131_BaselineAdapter__V42DensityRepairAfterStage130",
...
def load_existing_result() -> dict[str, Any]:
    ...
    if not path_exists(manifest) or not path_exists(kpi):
        raise FileNotFoundError("Stage131 existing run_manifest.json or kpi_record.json is missing")
...
def decide(summary_rows: Sequence[Mapping[str, Any]], segment_rows: Sequence[Mapping[str, Any]], external: str) -> str:
    ...
    if not reasons:
        return "proceed_to_stage132_candidate_confirmation"
    return "continue_v42_density_repair_in_stage132_due_to_34d_gap"
...
def line_table(summary_rows: Sequence[Mapping[str, Any]]) -> str:
    ...
    for row in summary_rows:
        if row.get("view") != "actual_routed_total":
            continue
        lines.append(
            "| {adapter} | {split} | {pf:.2f} | {net:.2f} | {dd:.2f} | {trades:.0f} | {cost:.4f} |".format(
                ...
                cost=metric(row, "cost_stressed_expectancy"),
            )
        )
    return "\n".join(lines)
...
def report_markdown(summary_rows: Sequence[Mapping[str, Any]], segment_rows: Sequence[Mapping[str, Any]], decision: str, external: str) -> str:
    best = best_variant(summary_rows)
    reasons = failure_reasons(summary_rows, segment_rows, external)
    return f"""# Stage131 V42 Density Repair Report(131단계 v42 밀도 수리 보고서)
...
def decision_markdown(summary_rows: Sequence[Mapping[str, Any]], segment_rows: Sequence[Mapping[str, Any]], decision: str, external: str) -> str:
    best = best_variant(summary_rows)
    reasons = failure_reasons(summary_rows, segment_rows, external)
    return f"""# Stage131 Decision(131단계 판정)

decision(판정): `{decision}`

Stage131(131단계)는 Stage130(130단계) 최선 후보인 v42 source(v42 원천)의 density/cost weakness(밀도/비용 약점)를 bounded repair(경계 수리)로 측정했다. Effect(효과): 수리 효과를 근거로 보존하고 Stage132(132단계)로 넘긴다.
...
Stage131 closeout(131단계 종료)는 overall goal completion(전체 목표 완료)이 아니다. Effect(효과): 34D KPI(34D 핵심 성과 지표) 차이를 계속 줄이기 위해 Stage132(132단계)로 넘긴다.
"""
...
def equity_audit_markdown(summary_rows: Sequence[Mapping[str, Any]], segment_rows: Sequence[Mapping[str, Any]]) -> str:
    ...
    for row in flagged[:30]:
        lines.append(f"- `{row.get('adapter_id')}` `{row.get('split')}` `{row.get('segment_type')}` `{row.get('segment')}`: `{row.get('quality_flag')}`")
    return "\n".join(lines) + "\n"
...
def write_run_identity(result: Mapping[str, Any]) -> None:
    ...
    write_json(
        RUN_ROOT / "run_manifest.json",
        {
            ...
            "bounded_question": "Can cooldown or transition-only controls repair Stage130 v42 density and cost damage?",
            "source_stage130_decision": rel(SOURCE_STAGE130_DECISION),
            "source_stage130_pushed_commit": SOURCE_STAGE130_PUSHED_COMMIT,
            ...
            rows.append(
                {
                    ...
                    "stage_id": STAGE_ID,
                    "run_id": RUN_ID,
                    "artifact_type": "stage131_v42_density_repair_evidence",
                    "path": rel(path),
                    "sha256": sha256_file_lf_normalized(path),
                    "hash_policy": "lf_normalized_text" if path.suffix.lower() in {".csv", ".json", ".md"} else "raw_file",
                    "created_at_utc": created,
                    "notes": "Stage131 bounded v42 density repair artifact.",
                }
            )
            ...
            rows.append(
                {
                    ...
                    "created_at_utc": created,
                    "notes": "Actual Stage131 MT5 Strategy Tester HTML report.",
                }
            )
    return rows
...
def write_ledgers(
        ...
        [
            {
                "run_id": RUN_ID,
                "stage_id": STAGE_ID,
                "lane": "baseline_adapter_v42_density_repair",
                "status": status,
                "judgment": decision,
                "path": rel(DECISION_PATH),
                "notes": ledger_pairs((("source_adapter", SOURCE_ADAPTER_ID), ("best_candidate", best.get("adapter_id")), ("validation_net", val.get("net_profit")), ("oos_net", oos.get("net_profit")), ("boundary", BOUNDARY))),
            }
        ],
        key="run_id",
    )
    ...
    ledger_rows.append(
        {
            "ledger_row_id": f"{RUN_ID}__aggregate_v42_density_repair",
            "stage_id": STAGE_ID,
            "run_id": RUN_ID,
            "subrun_id": "aggregate_v42_density_repair",
            "parent_run_id": PARENT_RUN_ID,
            "record_view": "v42_density_repair",
            "tier_scope": "Tier A+B",
            ...
            "judgment": decision,
            "path": rel(DECISION_PATH),
            "primary_kpi": ledger_pairs((("best_candidate", best.get("adapter_id")), ("validation_net", val.get("net_profit")), ("oos_net", oos.get("net_profit")), ("validation_pf", val.get("profit_factor")), ("oos_pf", oos.get("profit_factor")))),
            "guardrail_kpi": ledger_pairs((("failure_reasons", failure_reasons(summary_rows, segment_rows, external)), ("atr_sltp", "measured"), ("model_controlled_risk_pct", "measured"), ("overall_goal_complete", False))),
            "external_verification_status": external,
            "notes": "Stage131 bounded v42 density repair; not final package completion.",
        }
    )
    ...
    write_json(
        SUMMARY_JSON_PATH,
        {
            ...
            "required_outputs": {
                ...
                "stage131_decision": rel(DECISION_PATH),
            },
...
def write_packet_files(result: Mapping[str, Any], summary_rows: Sequence[Mapping[str, Any]], segment_rows: Sequence[Mapping[str, Any]], risk_rows: Sequence[Mapping[str, Any]], decision: str, ledger_payload: Mapping[str, Any]) -> None:
    external = str(result.get("external_verification_status") or "blocked")
    reasons = failure_reasons(summary_rows, segment_rows, external)
    best = best_variant(summary_rows)
    files = {
        "routing_receipt.json": {
            ...
            "support_skills": ["obsidian-backtest-forensics", "obsidian-model-validation", "obsidian-result-judgment"],
        ...
        "experiment_design_receipt.json": {
            "hypothesis": "Cooldown and transition-only controls may repair Stage130 v42 density and cost damage without opening a new source family.",
            "decision_use": "route the branch to Stage132 confirmation, further bounded repair, or demotion; do not claim final package completion.",
            "comparison_baseline": SOURCE_ADAPTER_ID,
            "control_variables": ["US100", "M5", "split_v1", "run50BN v42 source", "wide ATR bracket", "3% model-controlled risk cap", "ONNX deferred"],
            "changed_variables": ["same-direction cooldown", "global cooldown", "entry transition only"],
            "success_criteria": ["validation PF/net reaches 34D target", "OOS remains positive and credible", "cost-stressed expectancy improves", "risk/ATR telemetry present"],
            "failure_criteria": ["34D KPI gap remains", "OOS weak or damaged", "cost expectancy remains negative", "segment instability remains"],
            "stop_condition": "four bounded v42 density variants only",
            "status": "completed",
        },
        ...
        "result_judgment_gate.json": {"result_subject": RUN_ID, "judgment_label": decision, "failure_or_gap_reasons": reasons, "best_candidate": best.get("adapter_id", "none"), "claim_boundary": BOUNDARY, "status": "passed_with_boundary"},
        "artifact_lineage_audit.json": {"source_inputs": [rel(SOURCE_STAGE130_DECISION), rel(SOURCE_STAGE130_REPORT), rel(SOURCE_STAGE130_SUMMARY), rel(SOURCE_STAGE130_SEGMENTS), rel(SOURCE_STAGE130_RISK), rel(RUN50BN_MODEL)], "consumers": [rel(REPORT_PATH), rel(DECISION_PATH), rel(SUMMARY_JSON_PATH)], "ledger_links": ledger_payload},
        ...
        "required_gate_coverage_audit.json": {"required_gates": ["experiment_design_receipt", "runtime_evidence_gate", "kpi_contract_audit", "result_judgment_gate", "artifact_lineage_audit", "final_claim_guard"], "covered_by": ["experiment_design_receipt.json", "runtime_evidence_gate.json", "kpi_contract_audit.json", "result_judgment_gate.json", "artifact_lineage_audit.json", "final_claim_guard.json"], "status": "completed"},
        "aggregate_summary.json": {"packet_id": PACKET_ID, "stage_id": STAGE_ID, "run_id": RUN_ID, "decision": decision, "external_verification_status": external, "best_candidate": best.get("adapter_id", "none"), "required_outputs": {"report": rel(REPORT_PATH), "summary_json": rel(SUMMARY_JSON_PATH), "summary_csv": rel(SUMMARY_CSV_PATH), "segment_kpi_summary": rel(SEGMENT_KPI_PATH), "equity_curve_audit": rel(EQUITY_AUDIT_PATH), "risk_atr_telemetry": rel(RISK_ATR_TELEMETRY_PATH), "stage131_decision": rel(DECISION_PATH)}, "claim_boundary": BOUNDARY, "overall_goal_complete": False},
    }
...
`{BOUNDARY}`
""")
    write_md(INPUT_ROOT / "input_refs.md", f"""# Stage131 Input References(131단계 입력 참조)

- stage130_decision(130단계 판정): `{rel(SOURCE_STAGE130_DECISION)}`
- stage130_report(130단계 보고서): `{rel(SOURCE_STAGE130_REPORT)}`
- stage130_summary(130단계 요약): `{rel(SOURCE_STAGE130_SUMMARY)}`
- stage130_segments(130단계 구간): `{rel(SOURCE_STAGE130_SEGMENTS)}`
- stage130_risk(130단계 위험): `{rel(SOURCE_STAGE130_RISK)}`
- stage130_pushed_commit(130단계 푸시 커밋): `{SOURCE_STAGE130_PUSHED_COMMIT}`
""")
    write_md(SELECTED_ROOT / "selection_status.md", f"""# Stage131 Selection Status(131단계 선택 상태)

- stage_status(단계 상태): `closed_bounded_v42_density_repair`
- current_packet(현재 작업 묶음): `{PACKET_ID}`
- current_run(현재 실행): `{RUN_ID}`
- source_stage(원천 단계): `{SOURCE_STAGE130_ID}`
- source_decision(원천 판정): `continue_new_v2_model_branch_repair_in_stage131_due_to_34d_gap`
- stage131_decision(131단계 판정): `{decision}`
- next_stage_or_branch(다음 단계/분기): `{NEXT_STAGE_ID}`
- target_surface(목표 표면): `{TARGET_SURFACE}`
- selected_research_baseline(선택 연구 기준): `none`
- claim_boundary(주장 경계): `{BOUNDARY}`
""")
    write_md(REVIEWS_ROOT / "review_index.md", f"""# Stage131 Review Index(131단계 검토 색인)
...
`{BOUNDARY}`
""")
    write_md(NEXT_STAGE_ROOT / "01_inputs/input_refs.md", f"""# Stage132 Input References(132단계 입력 참조)

- stage131_decision(131단계 판정): `{rel(DECISION_PATH)}`
- stage131_report(131단계 보고서): `{rel(REPORT_PATH)}`
- stage131_summary(131단계 요약): `{rel(SUMMARY_CSV_PATH)}`
- stage131_segment_kpi(131단계 구간 KPI): `{rel(SEGMENT_KPI_PATH)}`
- stage131_risk_atr_telemetry(131단계 위험/ATR 원격측정): `{rel(RISK_ATR_TELEMETRY_PATH)}`
""")
    write_md(NEXT_STAGE_ROOT / "03_reviews/review_index.md", """# Stage132 Review Index(132단계 검토 색인)

Stage132(132단계)는 planned(계획) 상태다. Effect(효과): Stage131(131단계) 결과를 다음 bounded decision(경계 판정) 입력으로 연결한다.
""")
    write_md(NEXT_STAGE_ROOT / "04_selected/selection_status.md", f"""# Stage132 Selection Status(132단계 선택 상태)

- stage_status(단계 상태): `active_planned_from_stage131`
...
- selected_research_baseline(선택 연구 기준): `none`
- target_surface(목표 표면): `{TARGET_SURFACE}`
- adapter_under_review(검토 중 어댑터): `stage132_followup_from_{best.get("adapter_id", "stage131_none")}`
- status(상태): `stage131_closed_{decision}_stage132_open_planned`
- claim_boundary(주장 경계): `{BOUNDARY}`

Stage131(131단계)는 Stage130(130단계) v42 후보의 density/cost weakness(밀도/비용 약점)를 bounded repair(경계 수리)로 측정하고 종료했다. Effect(효과): 최선 결과와 실패 이유를 Stage132(132단계) 입력으로 넘긴다.

## Latest Stage131 Evidence(최신 131단계 근거)
...
- >-
  Stage131(131단계) closed(종료) as `{decision}` and Stage132(132단계) `{NEXT_STAGE_ID}` is active_planned(활성 계획). Effect(효과): v42 density repair(v42 밀도 수리)의 성공/실패를 보존하고 다음 경계 판정으로 넘긴다.
- >-
  Stage131 evidence(131단계 근거)는 `{rel(REPORT_PATH)}`, `{rel(SUMMARY_CSV_PATH)}`, `{rel(SEGMENT_KPI_PATH)}`에 있다. Effect(효과): KPI(핵심 성과 지표), segment(구간), risk/ATR telemetry(위험/ATR 원격측정)를 함께 추적한다.
- >-
...
stage131_new_v2_model_branch_followup_review:
  packet_id: {PACKET_ID}
  stage_id: {STAGE_ID}
  status: closed_bounded_v42_density_repair
  current_run_id: {RUN_ID}
  source_stage: {SOURCE_STAGE130_ID}
  decision: {decision}
...
stage132_v42_density_repair_followup:
  ...
  status: active_planned_from_stage131
  current_run_id: {NEXT_RUN_ID}
  source_stage: {STAGE_ID}
  source_decision: {decision}
  next_action: run_stage132_bounded_followup
  boundary: {BOUNDARY}
"""
    text = re.sub(r"(?ms)\nstage131_new_v2_model_branch_followup_review:.*?(?=\nstage\d+_|$)", "\n", text)
    text = re.sub(r"(?ms)\nstage132_v42_density_repair_followup:.*?(?=\nstage\d+_|$)", "\n", text)
...
def append_changelog(decision: str) -> None:
    entry = (
        f"\n## {utc_now()} Stage131 v42 density repair closeout(131단계 v42 밀도 수리 종료)\n\n"
        f"- action(행동): closed(종료) `{STAGE_ID}` with decision(판정) `{decision}`.\n"
        f"- effect(효과): v42 density/cost repair(v42 밀도/비용 수리) evidence(근거)를 보존하고 Stage132(132단계)로 넘겼다.\n"
...
def parse_args(argv: Sequence[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run Stage131 bounded v42 density repair after Stage130.")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.finalize_existing:
        ...
        prepared = {
            ...
            "model_family": "baseline_adapter_stage131_v42_density_repair_ebm_table",
            "feature_set_id": "stage131_run50bn_v42_density_repair_discrete_signal",
    ...
    print(json.dumps(json_ready({"status": "ok" if external == "completed" else "blocked", "run_id": RUN_ID, "decision": decision, "best_candidate": best_variant(summary_rows), "summary_json": rel(SUMMARY_JSON_PATH), "decision_path": rel(DECISION_PATH)}), ensure_ascii=False, indent=2, sort_keys=True))
    return 0
