...
RUN_ID = "run318B_execute_post_non_time_curve_stability_mt5_probe_v1"
RUN_NUMBER = "run318B"
SOURCE_RUN_ID = "run318A_design_post_non_time_curve_stability_rebuild_packet_v1"
PARENT_RUN_ID = "run317C_review_fresh_non_time_profit_source_mt5_probe_v1"
STATUS_PREPARED = "prepared_post_non_time_curve_stability_mt5_probe_no_runtime_kpi"
UPDATED_ON = "2026-05-24"
EXPLORATION_LABEL = "stage318_Model__PostNonTimeCurveStabilityReplay"
SIGNAL_COLUMN = "run318b_route_signal"
FEATURE_ORDER = (SIGNAL_COLUMN,)
COMMON_ROOT = "Project_Obsidian_Prime_v2/stage318/run318B_post_non_time_curve_stability"
...
MT5_QUEUE = RUN318A / "mt5_probe_queue.csv"
SOURCE_MANIFEST = RUN318A / "candidate_payload_manifest.csv"
SOURCE_RUN_MANIFEST = RUN318A / "run_manifest.json"
SOURCE_REPORT = REVIEWS / "run318A_materialization.md"
PRODUCER = Path("stage_pipelines/stage318/execute_post_non_time_curve_stability_mt5_probe.py")
...
REPORT = REVIEWS / "run318B_mt5_probe.md"
...
def classify_status(result: Mapping[str, Any], materialize_only: bool) -> tuple[str, str, str, str]:
    ...
    if materialize_only:
        return (
            STATUS_PREPARED,
            "post_non_time_curve_stability_runtime_probe_prepared_no_external_execution",
            "out_of_scope_by_claim_materialize_only",
            "run318B_execute_post_non_time_curve_stability_mt5_probe_external_check",
        )
    ...
    if planned and completed_exec >= planned and len(kpis) >= planned:
        return (
            "completed_post_non_time_curve_stability_mt5_probe_no_selection",
            "runtime_probe_completed_requires_profit_scale_curve_review_no_selection",
            "completed",
            "run318C_review_post_non_time_curve_stability_mt5_probe",
        )
    if kpis:
        return (
            "partial_post_non_time_curve_stability_mt5_probe_no_selection",
            "runtime_probe_partial_requires_continuation_or_review_no_selection",
            "partial_or_blocked",
            "run318B_continue_post_non_time_curve_stability_mt5_probe" if limited else "run318C_review_with_runtime_gaps",
        )
    return (
        "blocked_post_non_time_curve_stability_mt5_probe_no_kpi",
        "runtime_probe_blocked_no_kpi_no_selection",
        "blocked_or_invalid",
        "run318B_repair_or_block_post_non_time_curve_stability_mt5_probe",
    )
...
def report_markdown(result: Mapping[str, Any], status: str, judgment: str, external_status: str, next_action: str) -> str:
    ...
    return "\n".join(
        [
            "# run318B Post Non-Time Curve Stability MT5 Probe(318B 비시간 이후 곡선 안정성 MT5 탐침)",
            "",
            ...
            "",
            "Effect(효과): run318A(318A 실행)의 curve stability route_signal_value(곡선 안정성 경로 신호)를 MT5 tester(MT5 테스터)에 넣어 실제 trade count(거래수), net profit(순수익), PF(수익 팩터), DD(drawdown, 손실폭), recovery(회복)를 확인한다.",
            "",
            f"`{e310.BOUNDARY}`",
        ]
    )
        ...
        [
            {
                "run_id": RUN_ID,
                "stage_id": STAGE_ID,
                "lane": "post_non_time_curve_stability_mt5_probe",
                "status": status,
                "judgment": judgment,
        ...
        [
            {
                ...
                "record_view": "post_non_time_curve_stability_mt5_probe",
                "tier_scope": "Tier A used/Tier B fallback stress/actual routed total",
                "kpi_scope": "runtime_probe",
                "scoreboard_lane": "post_non_time_curve_stability",
                "status": status,
                "judgment": judgment,
                "path": base.rel(REPORT),
        ...
        [
            {
                ...
                "stage_id": STAGE_ID,
                "run_id": RUN_ID,
                "view": "post_non_time_curve_stability_mt5_probe",
...
def update_docs(status: str, judgment: str, next_action: str, kpi_count: int, attempt_count: int) -> None:
    ...
    selected = base.append_once(selected, "run318B_report", f"- run318B_report(318B 보고서): `{base.rel(REPORT)}`")
    selected = base.append_once(selected, "run318B_execution_result", f"- run318B_execution_result(318B 실행 결과): `{base.rel(EXECUTION_RESULT)}`")
    save_md(SELECTED, selected)

    review_index = base.io_path(REVIEW_INDEX).read_text(encoding="utf-8-sig") if base.path_exists(REVIEW_INDEX) else "# Stage318 Review Index(318단계 검토 색인)\n"
    review_index = base.append_once(review_index, "run318B_report", f"- run318B_report(318B 보고서): `{base.rel(REPORT)}`\n- run318B_execution_result(318B 실행 결과): `{base.rel(EXECUTION_RESULT)}`\n- run318B_mt5_kpi_summary(318B MT5 핵심 성과 지표 요약): `{base.rel(MT5_KPI_SUMMARY)}`")
    ...
    current = base.append_once(current, "run318B_summary", f"- run318B_summary(318B 요약): post non-time curve stability MT5 probe(비시간 이후 곡선 안정성 MT5 탐침)를 실행했다. Effect(효과): attempts(시도) `{attempt_count}`개와 MT5 KPI records(MT5 핵심 성과 지표 기록) `{kpi_count}`개를 만들었고 후보/Adapter(어댑터)/ONNX(온엑스)는 주장하지 않는다.")
    ...
    focus = f"- >-\n  Stage318(318단계) run318B(318B 실행) post non-time curve stability MT5 probe(비시간 이후 곡선 안정성 MT5 탐침) `{RUN_ID}`. Effect(효과): attempts(시도) `{attempt_count}`개와 MT5 KPI records(MT5 핵심 성과 지표 기록) `{kpi_count}`개를 기록했고 selected candidate(선택 후보), Adapter package(어댑터 패키지), ONNX readiness(온엑스 준비)는 주장하지 않는다.\n"
