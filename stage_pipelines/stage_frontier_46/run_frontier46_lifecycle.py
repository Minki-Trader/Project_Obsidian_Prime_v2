...
STAGE_ID = "stage_frontier_46__short_pf_edge_event_sequence_context_pivot_after_f45_event_classifier_memory"
PREV_STAGE_ID = "stage_frontier_45__short_pf_edge_event_utility_model_pivot_after_f44_label_model_memory"
RUN_A = "frontier46A_stage_open_short_pf_edge_event_sequence_context_hypothesis_design_v1"
RUN_B = "frontier46B_train_only_lagged_event_sequence_context_proxy_v1"
RUN_C = "frontier46C_capped_sequence_context_repair_v1"
RUN_D = "frontier46D_stage_closeout_event_sequence_context_v1"
NEXT_STAGE_ID = "stage_frontier_47__short_pf_edge_sequence_state_risk_budget_after_f46_sequence_context_memory"
NEXT_RUN_ID = "frontier47A_stage_open_short_pf_edge_sequence_state_risk_budget_hypothesis_design_v1"
RUNTIME_REVIEW_RUN_ID = "frontier46E_pre_expensive_runtime_validation_review_v1"
...
GROK_OPEN_ROOT = Path("docs") / "agent_control" / "grok_reviews" / "2026-06-15_frontier46_stage_open" / "small_review"
GROK_CLOSE_ROOT = Path("docs") / "agent_control" / "grok_reviews" / "2026-06-15_frontier46_stage_closeout" / "small_review"
...
def main() -> None:
    ...
    write_json(INPUT_ROOT / "event_sequence_context_manifest.json", manifest)
    ...
    write_json(
        RUN_D_ROOT / "run_manifest.json",
        {
            ...
            "profile": "short_event_sequence_context_proxy",
            "open_review": open_review,
            "closeout_review": closeout_review,
            "repair_decision": repair_decision,
            "closeout": closeout,
...
def load_closeout_grok_review() -> dict[str, Any]:
    ...
    return {
        ...
        "metadata": metadata,
        "clean_output_path": clean_path.as_posix(),
        "classification": "accepted_closeout_sequence_context_boundary" if accepted and boundary_ok and not rejected else "needs_local_verification",
...
def context_checks(
    ...
    checks = {
        "workspace_points_to_f46": f"next_stage_id: {STAGE_ID}" in workspace or f"current_stage_id: {STAGE_ID}" in workspace,
    ...
    if not all(checks.values()):
        raise RuntimeError(f"Frontier46 context check failed: {json.dumps(checks, ensure_ascii=False)}")
    return {
        "checks": checks,
...
def build_input_manifest(
    ...
    return {
        "stage_id": STAGE_ID,
        "hypothesis": "train_only_lagged_short_event_sequence_context",
        "idea_id": "IDEA-FR46-SHORT-EVENT-SEQUENCE-CONTEXT",
        "decision_use": "scout_seed_runtime_candidate_screening_only",
        "comparison_baseline": "F45 best and nonwinner rows are reference-only clues, not inherited baseline",
        "control_variables": {
            "symbol": "US100",
            "timeframe": "M5",
            "side": SIDE_LABEL,
        ...
        "changed_variables": {
            "sequence_context": "lagged_frozen_event_score_rolls_cooldown_and_horizon_embargoed_past_outcome_tape",
            "model_family": "class_weighted_sequence_context_classifier",
            "threshold_policy": "train_split_sequence_probability_quantiles_only",
        },
        "sample_scope": {
            "rows": int(len(frame)),
...
def sequence_context_specs(profile: str) -> list[dict[str, Any]]:
    high_quantiles = REPAIR_BASE_SCORE_HIGH_QUANTILES if profile == "repair" else INITIAL_BASE_SCORE_HIGH_QUANTILES
    if profile == "repair":
        windows = ((4, 18), (8, 30))
    else:
        windows = ((6, 24), (12, 36))
...
def build_event_surface(
    ...
    *,
    ...
    summary = summarize_f46_candidates(split_metrics, selected)
    return {
...
def candidates_for_mask(
    ...
    condition = {
        ...
        "feature_family": "train_only_lagged_event_sequence_context_score",
    ...
    for row in threshold_rows(
        ...
        take_quantiles,
        rr_floors,
        f"f46_{profile}_train_only_event_sequence_sl_tp_caps",
    ):
        ...
        candidate["f46_train_selection_score"] = score_value
        candidate["selection_rank_basis"] = "train_only_event_metrics_no_validation_oos_feedback"
        variants.append(candidate)
    variants.sort(key=lambda item: float(item["f46_train_selection_score"]), reverse=True)
    return variants[:2]
...
def rank_candidates(candidates: list[dict[str, Any]], prefix: str, limit: int) -> list[dict[str, Any]]:
    ...
    for candidate in sorted(candidates, key=lambda item: float(item["f46_train_selection_score"]), reverse=True):
        key = (
...
def summarize_f46_candidates(split_metrics: pd.DataFrame, candidates: list[dict[str, Any]]) -> pd.DataFrame:
    ...
    metadata = {
        str(item["candidate_id"]): {
            ...
            "f46_train_selection_score": item.get("f46_train_selection_score", ""),
    ...
    summary["f46_scout_clue_flag"] = dual & density_scout & (f_min_pf >= SCOUT_MIN_PF) & (f_max_dd <= SCOUT_MAX_DD)
    summary["f46_seed_surface_flag"] = dual & density_seed & (f_min_pf >= SEED_MIN_PF) & (f_max_dd <= SEED_MAX_DD)
    summary["runtime_probe_candidate_flag"] = (
        summary["f46_seed_surface_flag"].astype(bool) & (f_min_pf >= RUNTIME_MIN_PF) & (f_max_dd <= RUNTIME_MAX_DD)
    )
    summary["f46_axis_gap_to_seed"] = (
...
def surface_counts(summary: pd.DataFrame) -> dict[str, Any]:
    ...
    return {
        "scout_clue_rows": int(summary["f46_scout_clue_flag"].sum()),
        "seed_surface_rows": int(summary["f46_seed_surface_flag"].sum()),
...
def closest_nonwinner_forward_observation(combined: pd.DataFrame, best: dict[str, Any]) -> dict[str, Any]:
    ...
    frame["__seed_gap"] = numeric_column("f46_axis_gap_to_seed", 999999.0)
...
def build_repair_decision(initial_summary: pd.DataFrame) -> dict[str, Any]:
    if initial_summary.empty:
        return {
            ...
            "repair_reason": "Initial event sequence-context surface produced no candidate summary.",
        }
    runtime = int(initial_summary["runtime_probe_candidate_flag"].sum())
    seed = int(initial_summary["f46_seed_surface_flag"].sum())
    scout_rows = int(initial_summary["f46_scout_clue_flag"].sum())
    if runtime:
        return {
    ...
    if seed:
        return {
            ...
            "repair_reason": "Seed surface exists; avoid sequence-context overfit repair before runtime validation planning.",
        }
    return {
        "run_repair_grid": True,
        "repair_action": "run_capped_event_rarity_threshold_repair",
        "repair_reason": f"Initial event sequence-context surface scout={scout_rows}, seed=0, runtime=0; run bounded context repair.",
...
def classify_closeout(initial_summary: pd.DataFrame, repair_summary: pd.DataFrame) -> dict[str, Any]:
    ...
    if combined.empty:
        ...
        scout_rows = int(combined["f46_scout_clue_flag"].sum())
        seed_rows = int(combined["f46_seed_surface_flag"].sum())
    ...
    if runtime_rows:
        ...
        runtime_status = "runtime_probe_out_of_scope_by_claim_seed_only_no_runtime_candidate_after_f46_sequence_context_proxy"
        next_stage = NEXT_STAGE_ID
        next_run = NEXT_RUN_ID
    elif scout_rows:
        closeout_class = "preserved_clue_negative_memory"
        runtime_status = "runtime_probe_ineligible_no_seed_or_runtime_candidate_after_f46_sequence_context_proxy"
        next_stage = NEXT_STAGE_ID
        next_run = NEXT_RUN_ID
    else:
        closeout_class = "negative_memory"
        runtime_status = "runtime_probe_ineligible_no_scout_seed_or_runtime_candidate_after_f46_sequence_context_proxy"
        next_stage = NEXT_STAGE_ID
        next_run = NEXT_RUN_ID
    return {
...
def build_closeout_prompt(closeout: dict[str, Any], best_rows: list[dict[str, Any]], repair_decision: dict[str, Any]) -> str:
    ...
    compact_rows = "\n".join(
        (
            ...
            f"fwd_dd={row.get('forward_max_dd')}; scout={row.get('f46_scout_clue_flag')}; "
            f"seed={row.get('f46_seed_surface_flag')}; runtime={row.get('runtime_probe_candidate_flag')}"
        )
        for row in best_rows[:6]
    )
    return f"""# Frontier46 closeout Grok review(그록 마감 검토)
...
- scout/seed/runtime(탐색/씨앗/런타임): {best.get("f46_scout_clue_flag")}/{best.get("f46_seed_surface_flag")}/{best.get("runtime_probe_candidate_flag")}
...
F46 opened train-only lagged short event sequence-context(학습 전용 지연 숏 이벤트 순서 문맥) hypothesis(가설). F45 is reference only(참조 전용), not baseline/winner(기준선/승자 아님).
"""
...
- scope_completion_gate(범위 완료 게이트): pass(통과), F46 hypothesis/proxy/repair/closeout(가설/프록시/수리/마감) materialized.
...
- final_codex_direction(최종 코덱스 방향): run F46 event sequence-context proxy with validation/OOS read-only(검증/표본외 읽기 전용)
"""
...
- final_codex_direction(최종 코덱스 방향): close F46 as `{closeout.get("closeout_class")}` with no authority claim(권위 주장 없음)
"""
    return {
...
F46 negative memory(부정 기억)는 train-only lagged short event sequence context(학습 전용 지연 숏 이벤트 순서 문맥)가 seed/runtime(씨앗/런타임) 후보를 만들었는지 여부와 반복 금지 경계를 기록한다.
    ...
    rows = [
        {
            ...
            "notes": "F46 opened with train-only lagged event sequence-context hypothesis and Grok guardrails.",
        },
        {
            "stage_id": STAGE_ID,
            "run_id": RUN_B,
            ...
            "notes": "Initial train-only lagged event sequence-context proxy surface.",
        },
        {
            "stage_id": STAGE_ID,
            "run_id": RUN_C,
            ...
            "runtime_probe_status": "evaluated_for_runtime_candidate",
            "notes": "Capped sequence context repair diagnostic.",
        },
        {
            "stage_id": STAGE_ID,
            "run_id": RUN_D,
        ...
        },
        {
            ...
            "closeout_class": closeout.get("closeout_class"),
            "runtime_probe_status": "out_of_scope_by_claim_tier_a_sequence_context_proxy_only",
            "notes": "F46 used Tier A sequence-context proxy only; Tier B not claimed.",
        },
        {
            "stage_id": STAGE_ID,
            "run_id": RUN_D,
...
def project_ledger_row(row: dict[str, Any], fields: list[str]) -> dict[str, Any]:
    ...
    values = {
        ...
        "kpi_scope": "short_event_sequence_context_proxy",
        ...
        "run_family": "frontier_short_event_sequence_context_proxy",
...
current_judgment: {closeout.get("closeout_class")}(F46 train-only event sequence-context proxy no operating authority)
...
Frontier46(F46, 전선 46단계)가 `{closeout.get("closeout_class")}`로 닫혔다.
...
def artifact_map() -> dict[str, str]:
    return {
        "input_manifest": (INPUT_ROOT / "event_sequence_context_manifest.json").as_posix(),
