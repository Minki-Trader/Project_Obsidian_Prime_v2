...
RUN_ID = "run322B_execute_cp321b_curve_stability_pressure_mt5_probe_v1"
RUN_NUMBER = "run322B"
SOURCE_RUN_ID = "run322A_design_cp321b_curve_stability_pressure_packet_v1"
PARENT_RUN_ID = "run321C_review_post_controller_profit_curve_mt5_probe_v1"
STATUS_PREPARED = "prepared_cp321b_curve_stability_pressure_mt5_probe_no_runtime_kpi"
UPDATED_ON = "2026-05-26"
EXPLORATION_LABEL = "stage322_Model__Cp321BCurveStabilityPressure"
SIGNAL_COLUMN = "run322b_route_signal"
FEATURE_ORDER = (SIGNAL_COLUMN,)
COMMON_ROOT = "Project_Obsidian_Prime_v2/stage322/run322B_cp321b_curve_stability_pressure"
...
MT5_QUEUE = RUN322A / "mt5_probe_queue.csv"
SOURCE_MANIFEST = RUN322A / "candidate_payload_manifest.csv"
SOURCE_RUN_MANIFEST = RUN322A / "run_manifest.json"
SOURCE_REPORT = REVIEWS / "run322A_materialization.md"
PRODUCER = Path("stage_pipelines/stage322/execute_cp321b_curve_stability_pressure_mt5_probe.py")
...
REPORT = REVIEWS / "run322B_mt5_probe.md"
...
def classify_status(result: Mapping[str, Any], materialize_only: bool) -> tuple[str, str, str, str]:
    ...
    if materialize_only:
        return (
            STATUS_PREPARED,
            "cp321b_curve_stability_pressure_runtime_probe_prepared_no_external_execution",
            "out_of_scope_by_claim_materialize_only",
            "run322B_execute_cp321b_curve_stability_pressure_mt5_probe_external_check",
        )
    ...
    if planned and completed_exec >= planned and len(kpis) >= planned:
        return (
            "completed_cp321b_curve_stability_pressure_mt5_probe_no_selection",
            "runtime_probe_completed_requires_stability_review_no_selection",
            "completed",
            "run322C_review_cp321b_curve_stability_pressure_mt5_probe",
        )
    if kpis:
        return (
            "partial_cp321b_curve_stability_pressure_mt5_probe_no_selection",
            "runtime_probe_partial_requires_continuation_or_review_no_selection",
            "partial_or_blocked",
            "run322B_continue_cp321b_curve_stability_pressure_mt5_probe" if limited else "run322C_review_with_runtime_gaps",
        )
    return (
        "blocked_cp321b_curve_stability_pressure_mt5_probe_no_kpi",
        "runtime_probe_blocked_no_kpi_no_selection",
        "blocked_or_invalid",
        "run322B_repair_or_block_cp321b_curve_stability_pressure_mt5_probe",
    )
...
def report_markdown(result: Mapping[str, Any], status: str, judgment: str, external_status: str, next_action: str) -> str:
    ...
    return "\n".join(
        [
            "# run322B cp321B Curve Stability Pressure MT5 Probe(322B cp321B 곡선 안정성 압박 MT5 탐침)",
            "",
            ...
            "",
            "Effect(효과): run322A(322A 실행)의 cp321B stability pressure(안정성 압박) route_signal_value(경로 신호)를 MT5 tester(MT5 테스터)에 넣어 trade count(거래 수), net profit(순수익), PF(수익 팩터), DD(drawdown, 손실폭), recovery(회복)를 확인한다.",
            "",
            f"`{e310.BOUNDARY}`",
        ]
    )
...
def upsert_ledgers(result: Mapping[str, Any], status: str, judgment: str, external_status: str, next_action: str) -> None:
    ...
    ledger.upsert_csv_rows(RUN_REGISTRY, base.RUN_REGISTRY_COLUMNS, [{"run_id": RUN_ID, "stage_id": STAGE_ID, "lane": "cp321b_curve_stability_pressure_mt5_probe", "status": status, "judgment": judgment, "path": base.rel(REPORT), "notes": f"attempts={attempt_count};mt5_kpi_records={kpi_count};selected_candidate=none;onnx_readiness=not_claimed;next_action={next_action}."}], key="run_id")
    ledger.upsert_csv_rows(ALPHA_LEDGER, base.ALPHA_LEDGER_COLUMNS, [{"ledger_row_id": f"{RUN_ID}__mt5_probe", "stage_id": STAGE_ID, "run_id": RUN_ID, "subrun_id": RUN_NUMBER, "parent_run_id": SOURCE_RUN_ID, "record_view": "cp321b_curve_stability_pressure_mt5_probe", "tier_scope": "Tier A used/Tier B fallback stress/actual routed total", "kpi_scope": "runtime_probe", "scoreboard_lane": "cp321b_curve_stability_pressure", "status": status, "judgment": judgment, "path": base.rel(REPORT), "primary_kpi": f"attempts={attempt_count};mt5_kpi_records={kpi_count}", "guardrail_kpi": "selected_candidate=none;onnx_readiness=not_claimed", "external_verification_status": external_status, "notes": f"next_action={next_action}."}], key="ledger_row_id")
    ledger.upsert_csv_rows(STAGE_LEDGER, e310.runner.prev.e295.e294.base.STAGE_LEDGER_COLUMNS, [{"row_id": f"{RUN_ID}__mt5_probe", "stage_id": STAGE_ID, "run_id": RUN_ID, "view": "cp321b_curve_stability_pressure_mt5_probe", "tier_scope": "Tier A used/Tier B fallback/actual routed total", "scoreboard": "runtime_probe", "status": status, "judgment": judgment, "evidence_boundary": "runtime_probe_no_candidate_no_onnx", "report_path": base.rel(REPORT), "notes": f"attempts={attempt_count};mt5_kpi_records={kpi_count}."}], key="row_id")
...
def update_docs(status: str, judgment: str, next_action: str, kpi_count: int, attempt_count: int) -> None:
    ...
    selected = base.append_once(selected, "run322B_report", f"- run322B_report(322B 보고서): `{base.rel(REPORT)}`")
    selected = base.append_once(selected, "run322B_execution_result", f"- run322B_execution_result(322B 실행 결과): `{base.rel(EXECUTION_RESULT)}`")
    save_md(SELECTED, selected)

    review_index = base.io_path(REVIEW_INDEX).read_text(encoding="utf-8-sig") if base.path_exists(REVIEW_INDEX) else "# Stage322 Review Index(322단계 검토 색인)\n"
    review_index = base.append_once(review_index, "run322B_report", f"- run322B_report(322B 보고서): `{base.rel(REPORT)}`\n- run322B_execution_result(322B 실행 결과): `{base.rel(EXECUTION_RESULT)}`\n- run322B_mt5_kpi_summary(322B MT5 KPI 요약): `{base.rel(MT5_KPI_SUMMARY)}`")
    ...
    current = base.append_once(current, "run322B_summary", f"- run322B_summary(322B 요약): cp321B curve stability pressure MT5 probe(cp321B 곡선 안정성 압박 MT5 탐침)를 실행했다. Effect(효과): attempts(시도) `{attempt_count}`개와 MT5 KPI records(MT5 KPI 기록) `{kpi_count}`개를 만들었고 선택 후보/Adapter(어댑터)/ONNX(온엑스)는 주장하지 않는다.")
    ...
    focus = f"- >-\n  Stage322(322단계) run322B(322B 실행) cp321B curve stability pressure MT5 probe(cp321B 곡선 안정성 압박 MT5 탐침) `{RUN_ID}`. Effect(효과): attempts(시도) `{attempt_count}`개와 MT5 KPI records(MT5 KPI 기록) `{kpi_count}`개를 기록했고 selected candidate(선택 후보), Adapter package(어댑터 패키지), ONNX readiness(온엑스 준비)는 주장하지 않는다.\n"
    ...
    changelog = base.append_once(changelog, RUN_ID, f"## {UPDATED_ON} run322B cp321B Curve Stability Pressure MT5 Probe(322B cp321B 곡선 안정성 압박 MT5 탐침)\n\n- status(상태): `{status}`\n- judgment(판정): `{judgment}`\n- effect(효과): attempts(시도) `{attempt_count}`개와 MT5 KPI records(MT5 KPI 기록) `{kpi_count}`개를 기록했다.\n")
