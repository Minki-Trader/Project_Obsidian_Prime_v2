...
from stage_pipelines.stage267 import (
    run267DR_runtime_gap_aware_fourth_followup_or_prune_mt5_executor as source_executor,
)
...
RUN_NUMBER = "run267DS"
RUN_ID = "run267DS_stage267_runtime_gap_aware_fourth_followup_or_prune_balance_timeslice_trade_quality_with_init_failures_v1"
STATUS = "run267DS_runtime_gap_aware_fourth_followup_or_prune_balance_timeslice_trade_quality_with_init_failures_completed"
PARTIAL_STATUS = "run267DS_runtime_gap_aware_fourth_followup_or_prune_balance_timeslice_trade_quality_with_init_failures_partial_parser_errors"
NEXT_ACTION = "run267DT_design_runtime_gap_aware_fifth_followup_or_prune_from_run267DS_review"
NEXT_ACTION_PARTIAL = "run267DS_repair_trade_report_parser_before_init_failure_review"
...
RUN_ROOT = STAGE_ROOT / "02_runs" / RUN_NUMBER / "runtime_gap_aware_fourth_followup_or_prune_balance_timeslice_trade_quality_with_init_failures"
...
REPORT_PATH = REVIEWS_ROOT / "stage267_run267DS_runtime_gap_aware_fourth_followup_or_prune_balance_timeslice_trade_quality_with_init_failures.md"
PRODUCER_PATH = Path("stage_pipelines/stage267/run267DS_runtime_gap_aware_fourth_followup_or_prune_balance_timeslice_trade_quality_review.py")
...
def init_summary_read(alias: str, attempts: Sequence[Mapping[str, Any]], completed: Sequence[Mapping[str, Any]]) -> str:
    ...
    if alias == "s258_stc" and init_failures and completed:
        return "mixed_taper_survives_supply_continuity_init_failure_no_selection(완화형은 일부 살아남지만 공급 연속성은 초기화 실패, 선택 아님)"
    if alias == "s264_lc" and completed:
        return "defensive_control_profitable_but_dd_uncomfortable_no_selection(방어 대조는 수익이나 손실폭 불편, 선택 아님)"
...
def next_probe_for_candidate(alias: str) -> str:
    if alias == "s258_stc":
        return "separate_monday_late_taper_from_supply_continuity_table_repair(월요일/후반 완화와 공급 연속성 테이블 수리를 분리)"
    if alias == "s264_lc":
        return "keep_as_defensive_control_with_drawdown_zoom_only(손실폭 확대 검토용 방어 대조로만 유지)"
        ...
        output.append(
            {
                ...
                "comparison_baseline": "run267DR MT5 execution(267DR MT5 실행) source KPI(원천 핵심 성과 지표) and telemetry(텔레메트리)",
...
def build_result_judgment(result: Mapping[str, Any]) -> list[dict[str, Any]]:
    return [
        {
            "result_subject": "run267DS runtime gap aware fourth follow-up/prune balance/time-slice/trade-quality review(267DS 런타임 공백 반영 4차 후속/가지치기 잔액/시간구간/거래품질 검토)",
            "evidence_available": "run267DR Strategy Tester reports(267DR 전략 테스터 보고서), runtime telemetry(런타임 텔레메트리), trade records(거래 기록), time-slice KPI(시간구간 핵심 성과 지표), init failure summary(초기화 실패 요약)",
...
def artifact_rows(created_at: str) -> list[dict[str, Any]]:
    entries = (
        ("stage267_run267DS_producer", "producer_script", PRODUCER_PATH, "Builds run267DS balance/time-slice review with init failures."),
        ("stage267_run267DS_source_execution_result", "source_execution_result", SOURCE_EXECUTION_RESULT_PATH, "Source run267DR execution result."),
        ("stage267_run267DS_trade_records", "trade_records", TRADE_RECORDS_PATH, "Parsed completed runtime trade records."),
        ("stage267_run267DS_time_slice_kpi", "time_slice_kpi", TIME_SLICE_KPI_PATH, "Time-slice KPI for completed runtime rows."),
        ("stage267_run267DS_curve_diagnostics", "curve_diagnostics", CURVE_DIAGNOSTICS_PATH, "Curve diagnostics for completed runtime rows."),
        ("stage267_run267DS_candidate_profile_review", "candidate_profile_review", CANDIDATE_PROFILE_REVIEW_PATH, "Candidate-profile completed runtime review."),
        ("stage267_run267DS_candidate_init_failure_summary", "candidate_init_failure_summary", CANDIDATE_INIT_FAILURE_SUMMARY_PATH, "Candidate init failure summary."),
        ("stage267_run267DS_attempt_outcome_review", "attempt_outcome_review", ATTEMPT_OUTCOME_REVIEW_PATH, "Attempt-level runtime/report outcome review."),
        ("stage267_run267DS_negative_slice", "negative_slice_summary", NEGATIVE_SLICE_PATH, "Weak negative time slices."),
        ("stage267_run267DS_attribution", "performance_attribution_summary", ATTRIBUTION_SUMMARY_PATH, "Performance attribution with init failure boundary."),
        ("stage267_run267DS_result_judgment", "result_judgment", RESULT_JUDGMENT_PATH, "Result judgment."),
        ("stage267_run267DS_parser_checks", "parser_checks", PARSER_CHECKS_PATH, "Parser checks."),
        ("stage267_run267DS_review_result", "review_result", REVIEW_RESULT_PATH, "Review result payload."),
        ("stage267_run267DS_report", "review_report", REPORT_PATH, "User-facing review report."),
    )
    return [
        {
...
def update_ledgers(created_at: str, result: Mapping[str, Any]) -> None:
    ...
    stage_row = {
        "row_id": "stage267_run267DS_runtime_gap_aware_fourth_followup_or_prune_balance_timeslice_trade_quality_with_init_failures",
        "stage_id": STAGE_ID,
        "run_id": RUN_ID,
        "view": "runtime_gap_aware_fourth_followup_or_prune_balance_timeslice_trade_quality_with_init_failures",
    ...
    project_row = {
        ...
        "record_view": "runtime_gap_aware_fourth_followup_or_prune_balance_timeslice_trade_quality_with_init_failures",
...
def update_stage267_workspace_block(text: str, *, status: str, next_action: str) -> str:
    report_entry = f"  run267DS_runtime_gap_aware_fourth_followup_or_prune_balance_timeslice_trade_quality_review_report_path: {rel(REPORT_PATH)}"
    lines = text.splitlines()
    output: list[str] = []
    in_stage267 = False
...
def update_current_docs(result: Mapping[str, Any]) -> None:
    ...
    report_line = (
        ...
        f"(267DS 런타임 공백 반영 4차 후속/가지치기 잔액/시간구간/거래품질 검토): `{rel(REPORT_PATH)}`"
    )
    latest_line = (
        f"- latest_review(최신 검토): run267DS(267DS 실행) candidate_profile_rows(후보-프로필 행) "
    ...
    block = "\n".join(
        [
            ...
            f"Effect(효과): candidate_profile_rows(후보-프로필 행) `{len(result['candidate_profile_review'])}`, init_failure_attempts(초기화 실패 시도) `{result['init_failure_attempts']}`, negative_slices(음수 구간) `{len(result['negative_slices'])}`를 만들었고, 다음은 runtime gap aware fifth follow-up/prune design(런타임 공백 반영 5차 후속/가지치기 설계)이다.",
    ...
    current = replace_line_prefix(
        ...
        "- adapter_under_review(검토 중 어댑터): `runtime_gap_aware_fourth_followup_or_prune_balance_timeslice_trade_quality_with_init_failures`",
    )
    current = replace_line_prefix(current, "- status(상태):", f"- status(상태): `{status}`")
    current = replace_line_prefix(current, "- next_action(다음 행동):", f"- next_action(다음 행동): `{next_action}`")
    current = append_after_contains(current, "stage267_run267DR_runtime_gap_aware_fourth_followup_or_prune_mt5_execution.md", report_line)
    current = append_after_contains(current, "## Current Next Action", latest_line)
    current = append_block_once(current, "Run267DS(267DS 실행)는 run267DR", block)
    ...
    selection = append_after_contains(selection, "stage267_run267DR_runtime_gap_aware_fourth_followup_or_prune_mt5_execution", report_line)
    selection = append_block_once(selection, "Run267DS(267DS 실행)는 run267DR", block)
    ...
    review_index = append_after_contains(review_index, "stage267_run267DR_runtime_gap_aware_fourth_followup_or_prune_mt5_execution.md", report_line)
    review_index = append_block_once(review_index, "Run267DS(267DS 실행)는 run267DR", block)
    ...
    focus_line = (
        "- >-\n"
        f"  Stage267(267단계) run267DS(267DS 실행) runtime gap aware fourth follow-up/prune balance/time-slice/trade-quality review"
        f"(런타임 공백 반영 4차 후속/가지치기 잔액/시간구간/거래품질 검토) `{status}`. "
        f"Effect(효과): run267DR(267DR 실행)의 completed runtime(완료 런타임) `5`개와 init failure(초기화 실패) `{result['init_failure_attempts']}`개를 분리했고, "
