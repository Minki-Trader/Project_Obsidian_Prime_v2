...
from stage_pipelines.stage267 import run267AX_pool_wide_state_feature_engineering_second_followup_mt5_executor as source_executor


STAGE_ID = source_executor.STAGE_ID
RUN_NUMBER = "run267AY"
RUN_ID = "run267AY_stage267_pool_wide_state_feature_engineering_second_followup_balance_timeslice_trade_quality_review_v1"
PARENT_RUN_ID = source_executor.RUN_ID
SOURCE_MATERIALIZATION_RUN_ID = materializer.RUN_ID
STATUS = "run267AY_pool_wide_state_feature_engineering_second_followup_balance_timeslice_trade_quality_review_completed"
PARTIAL_STATUS = "run267AY_pool_wide_state_feature_engineering_second_followup_balance_timeslice_trade_quality_review_partial_parser_errors"
...
NEXT_ACTION = "run267AZ_design_pool_wide_state_feature_engineering_second_followup_or_adapter_branch_from_run267AY_review"
NEXT_ACTION_PARTIAL = "run267AY_repair_second_followup_trade_report_parser_errors"

STAGE_ROOT = source_executor.STAGE_ROOT
REVIEWS_ROOT = source_executor.REVIEWS_ROOT
RUN_ROOT = STAGE_ROOT / "02_runs" / RUN_NUMBER / "pool_wide_state_feature_engineering_second_followup_balance_timeslice_trade_quality_review"
...
SOURCE_FOLLOWUP_REVIEW_PATH = (
    STAGE_ROOT
    / "02_runs"
    / "run267AU"
    / "pool_wide_state_feature_engineering_followup_balance_timeslice_trade_quality_review"
    / "candidate_followup_profile_review.csv"
)
...
CANDIDATE_SECOND_REVIEW_PATH = RUN_ROOT / "candidate_second_followup_review.csv"
CANDIDATE_SUMMARY_PATH = RUN_ROOT / "candidate_second_followup_summary.csv"
SECOND_PROFILE_SUMMARY_PATH = RUN_ROOT / "second_followup_profile_summary.csv"
...
REPORT_PATH = REVIEWS_ROOT / "stage267_run267AY_pool_wide_state_feature_engineering_second_followup_balance_timeslice_trade_quality_review.md"
PRODUCER_PATH = Path("stage_pipelines/stage267/run267AY_pool_wide_state_feature_engineering_second_followup_balance_timeslice_trade_quality_review.py")
...
def curve_read(item: Mapping[str, Any], report_metrics: Mapping[str, Any], month_rows: Sequence[Mapping[str, Any]]) -> str:
    ...
    if equity_dd >= 20.0 or worst_month_net <= -180.0:
        return "headline_good_but_hole_uncomfortable(대표 숫자는 좋지만 구멍 불편)"
    if trades >= 275 and net >= 1000.0 and pf >= 1.60 and equity_dd <= 17.5 and positive_month_ratio >= 0.58:
        return "constructive_curve_watch_not_selection(건설적 곡선 관찰, 선택 아님)"
    if trades >= 255 and net >= 800.0 and pf >= 1.50 and equity_dd < 20.0:
            ...
            output.append(
                {
                    ...
                    "source_followup_profile": source_followup_profile,
                    "second_followup_profile": second_followup_profile,
...
def source_followup_by_key() -> dict[tuple[str, str, str], Mapping[str, Any]]:
    rows: dict[tuple[str, str, str], Mapping[str, Any]] = {}
    for row in read_csv(SOURCE_FOLLOWUP_REVIEW_PATH):
        key = (str(row.get("candidate_alias")), str(row.get("state_profile")), str(row.get("followup_profile")))
        rows[key] = row
    return rows
...
def fragility_flags(curve: Mapping[str, Any], worst_slice: Mapping[str, Any], source: Mapping[str, Any]) -> str:
    flags: list[str] = []
    source_net = as_float(source.get("net_profit"))
    source_pf = as_float(source.get("profit_factor"))
    source_dd = as_float(source.get("report_equity_drawdown_percent"))
    if as_float(curve.get("report_equity_drawdown_percent")) >= 18.0:
        flags.append("dd_watch(손실폭 관찰)")
    if as_float(curve.get("worst_month_net")) <= -160.0:
        flags.append("month_hole(월별 구멍)")
    if as_float(worst_slice.get("net_profit")) <= -180.0:
        flags.append("deep_slice_hole(깊은 구간 구멍)")
    if as_int(curve.get("trade_count")) < 270:
        flags.append("trade_count_watch(거래 수 관찰)")
    if as_float(curve.get("profit_factor")) < 1.60:
        flags.append("pf_watch(수익 팩터 관찰)")
    if source and as_float(curve.get("net_profit")) < source_net - 120.0:
        flags.append("source_net_regression(원천 후속 대비 순수익 후퇴)")
    if source and as_float(curve.get("profit_factor")) < source_pf - 0.08:
        flags.append("source_pf_regression(원천 후속 대비 수익 팩터 후퇴)")
    if source and as_float(curve.get("report_equity_drawdown_percent")) > source_dd + 1.0:
        flags.append("source_dd_worse(원천 후속 대비 손실폭 악화)")
    if not flags:
        flags.append("no_major_flag_in_this_review(이번 검토 주요 경고 없음)")
    return ";".join(flags)


def decision_read(curve: Mapping[str, Any], worst_slice: Mapping[str, Any], source: Mapping[str, Any]) -> str:
    flags = fragility_flags(curve, worst_slice, source)
    if "hole" in flags:
        return "headline_or_mixed_with_slice_hole_no_selection"
    if "source_net_regression" in flags and as_float(curve.get("net_profit")) < 1000.0:
        return "second_followup_regressed_no_selection"
    if str(curve.get("curve_read")).startswith("constructive_curve_watch") and "source_pf_regression" not in flags:
        return "constructive_second_followup_watch_no_selection"
    if str(curve.get("curve_read")).startswith("mixed_constructive"):
        return "mixed_second_followup_needs_wider_pressure"
    return "diagnostic_only_or_prune_pressure"


def build_candidate_second_review(
    curve_rows: Sequence[Mapping[str, Any]],
    time_rows: Sequence[Mapping[str, Any]],
) -> list[dict[str, Any]]:
    base_kpi = base_kpi_by_alias()
    source_followup = source_followup_by_key()
    output: list[dict[str, Any]] = []
    for curve in curve_rows:
        if curve.get("tier_scope") != "Tier A":
            continue
        alias = str(curve.get("candidate_alias"))
        source_key = (alias, str(curve.get("state_profile")), str(curve.get("source_followup_profile")))
        source = source_followup.get(source_key, {})
        ...
        output.append(
            {
                ...
                "source_followup_profile": curve.get("source_followup_profile"),
                "second_followup_profile": curve.get("second_followup_profile"),
                "pressure_group": curve.get("pressure_group"),
                "record_view": curve.get("record_view"),
                "net_profit": curve.get("net_profit"),
                "source_followup_net_profit": source.get("net_profit", ""),
                "net_delta_vs_source_followup": as_float(curve.get("net_profit")) - as_float(source.get("net_profit")),
                ...
                "source_followup_profit_factor": source.get("profit_factor", ""),
                "pf_delta_vs_source_followup": as_float(curve.get("profit_factor")) - as_float(source.get("profit_factor")),
                "trade_count": curve.get("trade_count"),
                "source_followup_trade_count": source.get("trade_count", ""),
                "trade_delta_vs_source_followup": as_float(curve.get("trade_count")) - as_float(source.get("trade_count")),
                "expectancy": curve.get("expectancy"),
                "report_equity_drawdown_percent": curve.get("report_equity_drawdown_percent"),
                "source_followup_dd_percent": source.get("report_equity_drawdown_percent", ""),
                "dd_delta_vs_source_followup": as_float(curve.get("report_equity_drawdown_percent")) - as_float(source.get("report_equity_drawdown_percent")),
                "recovery_factor": curve.get("report_recovery_factor"),
                "source_followup_recovery_factor": source.get("recovery_factor", ""),
                ...
                "source_followup_worst_month": source.get("worst_month", ""),
                "source_followup_worst_month_net": source.get("worst_month_net", ""),
                "worst_month_delta_vs_source": as_float(curve.get("worst_month_net")) - as_float(source.get("worst_month_net")),
                ...
                "fragility_flags": fragility_flags(curve, worst, source),
                "curve_read": curve.get("curve_read"),
                "decision_read": decision_read(curve, worst, source),
    ...
    return sorted(
        output,
        key=lambda row: (
            0 if row.get("decision_read") == "constructive_second_followup_watch_no_selection" else 1,
...
def build_candidate_summary(rows: Sequence[Mapping[str, Any]]) -> list[dict[str, Any]]:
    ...
    for (candidate_id, alias, role), grouped in group_rows(rows, ("candidate_id", "candidate_alias", "candidate_role")).items():
        constructive = [row for row in grouped if row.get("decision_read") == "constructive_second_followup_watch_no_selection"]
        mixed = [row for row in grouped if row.get("decision_read") == "mixed_second_followup_needs_wider_pressure"]
        holes = [row for row in grouped if "hole" in str(row.get("fragility_flags"))]
        regressions = [row for row in grouped if "source_net_regression" in str(row.get("fragility_flags"))]
        ...
        if constructive and not holes and not regressions:
            summary_read = "candidate_second_followup_watch_no_selection(2차 후속 관찰, 선택 아님)"
        ...
        output.append(
            {
                ...
                "source_regression_count": len(regressions),
...
def build_second_profile_summary(rows: Sequence[Mapping[str, Any]]) -> list[dict[str, Any]]:
    output: list[dict[str, Any]] = []
    for (profile, pressure_group), grouped in group_rows(rows, ("second_followup_profile", "pressure_group")).items():
        net_values = [as_float(row.get("net_profit")) for row in grouped]
        dd_values = [as_float(row.get("report_equity_drawdown_percent")) for row in grouped]
        holes = [row for row in grouped if "hole" in str(row.get("fragility_flags"))]
        constructive = [row for row in grouped if row.get("decision_read") == "constructive_second_followup_watch_no_selection"]
        ...
        output.append(
            {
                "second_followup_profile": profile,
                "pressure_group": pressure_group,
...
def update_workspace_state_text(text: str, result: Mapping[str, Any]) -> str:
    ...
    path_key = "run267AY_pool_wide_state_feature_engineering_second_followup_balance_timeslice_trade_quality_review_report_path"
    text = remove_workspace_focus_item(text, "run267AY(267AY 실행)")
    lines = text.splitlines()
    output: list[str] = []
    in_stage267 = False
    ...
    focus_block = [
        "- >-",
        f"  Stage267(267단계) run267AY(267AY 실행) pool-wide state feature engineering second follow-up balance/time-slice/trade-quality review(후보군 전체 상태 피처 엔지니어링 2차 후속 잔액/시간구간/거래품질 검토) `{status}`. Effect(효과): run267AX(267AX 실행)의 8개 MT5(MetaTrader 5, 메타트레이더5) 보고서를 거래 단위로 다시 읽어 curve(곡선), weak slice(약한 구간), trade quality(거래 품질) 판단을 만들었고 selected candidate(선택 후보), ONNX readiness(ONNX 준비), Goal Achieve(목표 달성)는 주장하지 않는다.",
    ]
    for line in lines:
        ...
        if in_stage267:
            ...
            if stripped.startswith("next_action:"):
                ...
                continue
            if "run267AX_pool_wide_state_feature_engineering_second_followup_mt5_execution_report_path" in stripped and not inserted_path:
                output.append(line)
...
def update_current_truth_docs(result: Mapping[str, Any]) -> None:
    status = str(result["status"])
    next_action = str(result["next_action"])
    report_line = f"- run267AY_pool_wide_state_feature_engineering_second_followup_balance_timeslice_trade_quality_review(267AY 후보군 전체 상태 피처 엔지니어링 2차 후속 잔액/시간구간/거래품질 검토): `{rel(REPORT_PATH)}`"
    latest_line = (
        f"- latest_mt5_review(최신 MT5 검토): run267AY(267AY 실행) trade records(거래 기록) `{result['trade_record_count']}`, "
        f"candidate-second rows(후보-2차 행) `{len(result['candidate_second_review'])}`, "
    ...
    for path in (CURRENT_WORKING_STATE_PATH, SELECTION_STATUS_PATH, REVIEW_INDEX_PATH):
        ...
        if path == CURRENT_WORKING_STATE_PATH:
            ...
            text = replace_line_prefix(text, "- adapter_under_review(검토 중 어댑터):", "- adapter_under_review(검토 중 어댑터): `pool_wide_state_feature_engineering_second_followup_balance_timeslice_trade_quality_review`")
            text = append_after_contains(text, "stage267_run267AX_pool_wide_state_feature_engineering_second_followup_mt5_execution.md", report_line)
            ...
            text = replace_line_prefix(text, "- next_action(다음 행동):", f"- next_action(다음 행동): `{next_action}`")
            text = append_after_contains(text, "stage267_run267AX_pool_wide_state_feature_engineering_second_followup_mt5_execution.md", report_line)
        text = append_block_once(text, "Run267AY(267AY 실행)는 run267AX", closing_block)
        write_md(path, text)
...
def update_ledgers(created_at: str, result: Mapping[str, Any]) -> None:
    ...
    upsert_csv_rows(
        ...
        [
            {
                "row_id": "stage267_run267AY_pool_wide_state_feature_engineering_second_followup_balance_timeslice_trade_quality_review",
                "stage_id": STAGE_ID,
                "run_id": RUN_ID,
                "view": "pool_wide_state_feature_engineering_second_followup_balance_timeslice_trade_quality_review",
                "tier_scope": "Tier A second follow-up review; Tier B and actual routed total blocked by route gap audit",
                ...
                "notes": (
                    ...
                    f"candidate_second_rows={len(result['candidate_second_review'])};"
        ...
        [
            {
                "run_id": RUN_ID,
                "stage_id": STAGE_ID,
                "lane": "baseline_candidate_racing_pool_wide_state_feature_engineering_second_followup_balance_timeslice_trade_quality_review",
                "status": status,
                "judgment": judgment,
                "path": rel(REPORT_PATH),
                "notes": (
                    "Run267AY second follow-up MT5 curve/time-slice/trade-quality review from run267AX reports; "
    ...
    upsert_csv_rows(
        ...
        [
            {
                "ledger_row_id": f"{RUN_ID}__pool_wide_state_feature_engineering_second_followup_balance_timeslice_trade_quality_review",
                "stage_id": STAGE_ID,
                "run_id": RUN_ID,
                "subrun_id": "pool_wide_state_feature_engineering_second_followup_balance_timeslice_trade_quality_review",
                "parent_run_id": PARENT_RUN_ID,
                "record_view": "pool_wide_state_feature_engineering_second_followup_balance_timeslice_trade_quality_review",
                "tier_scope": "Tier A second follow-up review; Tier B and actual routed total blocked by route gap audit",
                ...
                "external_verification_status": "completed_for_run267AX_mt5_report_review",
    ...
    entries = (
        ("stage267_run267AY_review_script", "producer_script", PRODUCER_PATH, "Builds run267AY second follow-up curve/time-slice/trade-quality review."),
        ("stage267_run267AY_source_execution_result", "source_execution_result", SOURCE_EXECUTION_RESULT_PATH, "Source run267AX execution result."),
        ("stage267_run267AY_source_kpi_summary", "source_kpi_summary", SOURCE_KPI_SUMMARY_PATH, "Source run267AX KPI summary."),
        ("stage267_run267AY_source_forensics", "source_forensics", SOURCE_FORENSICS_PATH, "Source run267AX backtest forensics."),
        ("stage267_run267AY_source_attempts", "source_attempt_manifest", SOURCE_EXECUTED_ATTEMPTS_PATH, "Source run267AX executed attempts."),
        ("stage267_run267AY_trade_records", "trade_records", TRADE_RECORDS_PATH, "Run267AY paired trade records from run267AX reports."),
        ("stage267_run267AY_time_slice_kpi", "time_slice_kpi", TIME_SLICE_KPI_PATH, "Run267AY month/weekday/hour/session/direction/chron-segment KPI."),
        ("stage267_run267AY_curve_diagnostics", "curve_diagnostics", CURVE_DIAGNOSTICS_PATH, "Run267AY closed-balance curve diagnostics."),
        ("stage267_run267AY_candidate_second_review", "candidate_second_review", CANDIDATE_SECOND_REVIEW_PATH, "Run267AY candidate second follow-up curve and weak-slice review."),
        ("stage267_run267AY_candidate_summary", "candidate_summary", CANDIDATE_SUMMARY_PATH, "Run267AY candidate second follow-up summary."),
        ("stage267_run267AY_second_profile_summary", "second_profile_summary", SECOND_PROFILE_SUMMARY_PATH, "Run267AY second follow-up profile summary."),
        ("stage267_run267AY_negative_slice_summary", "negative_slice_summary", NEGATIVE_SLICE_PATH, "Run267AY worst negative Tier A slices."),
        ("stage267_run267AY_parser_checks", "parser_checks", PARSER_CHECKS_PATH, "Run267AY parser reconciliation checks."),
        ("stage267_run267AY_parser_errors", "parser_errors", PARSER_ERRORS_PATH, "Run267AY parser errors."),
        ("stage267_run267AY_review_result", "review_result", REVIEW_RESULT_PATH, "Run267AY review JSON payload."),
        ("stage267_run267AY_review_report", "review_report", REPORT_PATH, "User-facing run267AY balance/time-slice/trade-quality review."),
    )
    rows = [
        {
...
def report_markdown(result: Mapping[str, Any]) -> str:
    candidate_rows = result["candidate_second_review"]
    candidate_summary = result["candidate_summary"]
    profile_summary = result["second_profile_summary"]
    ...
    lines = [
        ...
        "",
        f"- action(행동): run267AX(267AX 실행)의 `{len(result['parser_checks'])}`개 MT5(MetaTrader 5, 메타트레이더5) report(보고서)를 trade list(거래 목록) 단위로 다시 읽었다.",
        ...
        f"- candidate_second_rows(후보-2차 행): `{len(candidate_rows)}`",
    ...
    for index, row in enumerate(candidate_rows[:12], start=1):
        lines.append(
            f"| {index} | `{row.get('candidate_alias')}` | `{row.get('candidate_role')}` | `{row.get('source_followup_profile')}` | `{row.get('second_followup_profile')}` | "
            f"{fmt(row.get('net_profit'))} | {fmt(row.get('net_delta_vs_source_followup'))} | {fmt(row.get('profit_factor'))} | "
            f"{as_int(row.get('trade_count'))} | {fmt(row.get('report_equity_drawdown_percent'))} | "
            f"`{row.get('worst_month')}` {fmt(row.get('worst_month_net'))} | `{row.get('worst_slice_axis')}`/`{row.get('worst_slice_bucket')}` {fmt(row.get('worst_slice_net'))} | `{row.get('decision_read')}` |"
        )
    lines.extend(
        [
            "",
    ...
    for row in candidate_summary:
        lines.append(
            ...
            f"{as_int(row.get('slice_hole_flag_count'))} | {as_int(row.get('source_regression_count'))} | "
            f"{fmt(row.get('net_profit_mean'))} | {fmt(row.get('net_profit_min'))} | "
    ...
    for row in profile_summary:
        lines.append(
            f"| `{row.get('second_followup_profile')}` | `{row.get('pressure_group')}` | {as_int(row.get('candidate_count'))} | "
    ...
    for row in negative:
        lines.append(
            f"| `{row.get('candidate_alias')}` | `{row.get('second_followup_profile')}` | `{row.get('axis')}` | `{row.get('bucket')}` | "
...
def review() -> dict[str, Any]:
    ...
    candidate_review = build_candidate_second_review(curve_rows, time_rows)
    candidate_summary = build_candidate_summary(candidate_review)
    profile_summary = build_second_profile_summary(candidate_review)
    ...
    watch_rows = sum(
        ...
        if row.get("decision_read") in {"constructive_second_followup_watch_no_selection", "mixed_second_followup_needs_wider_pressure"}
    )
    result = {
        "status": status,
        "judgment": judgment,
        ...
        "candidate_second_review": candidate_review,
        "candidate_summary": candidate_summary,
        "second_profile_summary": profile_summary,
        ...
        "outputs": {
            ...
            "candidate_second_review": rel(CANDIDATE_SECOND_REVIEW_PATH),
            "candidate_summary": rel(CANDIDATE_SUMMARY_PATH),
            "second_profile_summary": rel(SECOND_PROFILE_SUMMARY_PATH),
        ...
        "sources": {
            ...
            "source_followup_review": rel(SOURCE_FOLLOWUP_REVIEW_PATH),
    ...
    write_csv(CANDIDATE_SECOND_REVIEW_PATH, candidate_review, tuple(candidate_review[0].keys()) if candidate_review else ())
    write_csv(CANDIDATE_SUMMARY_PATH, candidate_summary, tuple(candidate_summary[0].keys()) if candidate_summary else ())
    write_csv(SECOND_PROFILE_SUMMARY_PATH, profile_summary, tuple(profile_summary[0].keys()) if profile_summary else ())
        ...
        json.dumps(
            {
                ...
                "candidate_second_rows": len(result["candidate_second_review"]),
