...
from stage_pipelines.stage267 import run267AH_noncalendar_state_guard_followup_mt5_executor as source_executor


STAGE_ID = source_executor.STAGE_ID
RUN_NUMBER = "run267AI"
RUN_ID = "run267AI_stage267_noncalendar_state_guard_followup_balance_timeslice_trade_quality_review_v1"
PARENT_RUN_ID = source_executor.RUN_ID
STATUS = "run267AI_noncalendar_state_guard_followup_balance_timeslice_trade_quality_review_completed"
PARTIAL_STATUS = "run267AI_noncalendar_state_guard_followup_balance_timeslice_trade_quality_review_partial_parser_errors"
...
NEXT_ACTION = "run267AJ_design_followup_from_run267AI_curve_time_slice_review"
NEXT_ACTION_PARTIAL = "run267AI_repair_followup_trade_report_parser_errors"

STAGE_ROOT = source_executor.STAGE_ROOT
REVIEWS_ROOT = source_executor.REVIEWS_ROOT
RUN_ROOT = STAGE_ROOT / "02_runs" / RUN_NUMBER / "noncalendar_state_guard_followup_balance_timeslice_trade_quality_review"
...
REPORT_PATH = REVIEWS_ROOT / "stage267_run267AI_noncalendar_state_guard_followup_balance_timeslice_trade_quality_review.md"
PRODUCER_PATH = Path("stage_pipelines/stage267/run267AI_noncalendar_state_guard_followup_balance_timeslice_trade_quality_review.py")
...
def curve_read(item: Mapping[str, Any], report_metrics: Mapping[str, Any], month_rows: Sequence[Mapping[str, Any]]) -> str:
    ...
    if trades >= 290 and net >= 1000.0 and pf >= 1.5 and equity_dd <= 16.0 and positive_month_ratio >= 0.5:
        return "constructive_curve_watch_not_selection"
    if trades >= 250 and net >= 700.0 and pf >= 1.35 and equity_dd < 22.0:
            ...
            rows.append(
                {
                    ...
                    "materialization_boundary": "noncalendar_state_guard_followup_pressure_not_retrained",
                    "record_view": record.get("record_view"),
                    "attempt_name": attempt_name,
                    "tier_scope": record.get("tier_scope"),
...
def build_time_slice_rows(trade_rows: Sequence[Mapping[str, Any]]) -> list[dict[str, Any]]:
    ...
    for axis in AXES:
        ...
        for key, rows in group_rows(trade_rows, keys).items():
            record_view, tier_scope, candidate_id, alias, role, test_id, profile, boundary, route_role, bucket = key
            item = metrics(rows)
            output.append(
                {
...
def build_curve_rows(
    ...
    for key, rows in group_rows(trade_rows, keys).items():
        record_view, tier_scope, candidate_id, alias, role, test_id, profile, boundary, route_role = key
        item = metrics(rows)
...
def worst_slice_for(curve_row: Mapping[str, Any], time_rows: Sequence[Mapping[str, Any]]) -> Mapping[str, Any]:
    related = [
        row
        for row in time_rows
        if row.get("record_view") == curve_row.get("record_view") and as_int(row.get("trade_count")) >= 3
    ]
    return min(related, key=lambda row: as_float(row.get("net_profit"))) if related else {}


def build_candidate_test_review(curve_rows: Sequence[Mapping[str, Any]], time_rows: Sequence[Mapping[str, Any]]) -> list[dict[str, Any]]:
    rows: list[dict[str, Any]] = []
    for curve in curve_rows:
        if curve.get("tier_scope") != "Tier A":
            continue
        worst_slice = worst_slice_for(curve, time_rows)
        rows.append(
            {
                ...
                "recovery_factor_closed": curve.get("recovery_factor_closed"),
                ...
                "worst_slice_axis": worst_slice.get("axis", ""),
                "worst_slice_bucket": worst_slice.get("bucket", ""),
                "worst_slice_net": worst_slice.get("net_profit", ""),
                "worst_slice_trade_count": worst_slice.get("trade_count", ""),
                "curve_read": curve.get("curve_read"),
                "decision_read": decision_read(curve),
            }
        )
    return sorted(
        rows,
        key=lambda row: (
            0 if row.get("curve_read") == "constructive_curve_watch_not_selection" else 1,
...
def decision_read(curve: Mapping[str, Any]) -> str:
    ...
    if read == "constructive_curve_watch_not_selection":
        return "watch_for_next_pressure_no_selection"
    if read == "mixed_constructive_needs_more_pressure":
        return "salvage_watch_needs_wider_pressure"
    if read == "dd_or_month_hole_uncomfortable":
        return "do_not_select_repair_or_prune_boundary"
    return "diagnostic_only"


def build_candidate_summary(candidate_tests: Sequence[Mapping[str, Any]]) -> list[dict[str, Any]]:
    rows: list[dict[str, Any]] = []
    for (candidate_id, alias, role), grouped in group_rows(candidate_tests, ("candidate_id", "candidate_alias", "candidate_role")).items():
        constructive = [row for row in grouped if row.get("curve_read") == "constructive_curve_watch_not_selection"]
        deep_holes = [row for row in grouped if row.get("curve_read") == "dd_or_month_hole_uncomfortable"]
        ...
        if constructive and not deep_holes:
            summary_read = "constructive_watch_not_selection"
        elif constructive:
            summary_read = "mixed_watch_with_holes"
        elif deep_holes:
            summary_read = "fragile_or_prune_pressure"
        else:
            summary_read = "diagnostic_only"
        rows.append(
            {
                "candidate_id": candidate_id,
                "candidate_alias": alias,
                ...
                "deep_hole_flag_count": len(deep_holes),
    ...
    return sorted(rows, key=lambda row: (-as_int(row.get("constructive_curve_count")), -as_float(row.get("net_profit_max"))))


def build_profile_summary(candidate_tests: Sequence[Mapping[str, Any]]) -> list[dict[str, Any]]:
    rows: list[dict[str, Any]] = []
    for (profile, test_id), grouped in group_rows(candidate_tests, ("followup_profile", "source_test_id")).items():
        rows.append(
            {
                "followup_profile": profile,
                "source_test_id": test_id,
                ...
                "constructive_curve_count": sum(1 for row in grouped if row.get("curve_read") == "constructive_curve_watch_not_selection"),
                "net_profit_mean": mean([as_float(row.get("net_profit")) for row in grouped]) if grouped else None,
                "equity_drawdown_percent_worst": max([as_float(row.get("report_equity_drawdown_percent")) for row in grouped]) if grouped else None,
                "worst_month_net_min": min([as_float(row.get("worst_month_net")) for row in grouped]) if grouped else None,
            }
        )
    return sorted(rows, key=lambda row: (-as_int(row.get("constructive_curve_count")), -as_float(row.get("net_profit_mean"))))
...
def negative_slices(time_rows: Sequence[Mapping[str, Any]]) -> list[dict[str, Any]]:
    rows = [
        dict(row)
        for row in time_rows
        if row.get("tier_scope") == "Tier A" and as_int(row.get("trade_count")) >= 3 and as_float(row.get("net_profit")) < 0.0
    ]
    return sorted(rows, key=lambda row: (as_float(row.get("net_profit")), -as_int(row.get("trade_count"))))


def build_tier_duplicate_review(curve_rows: Sequence[Mapping[str, Any]]) -> list[dict[str, Any]]:
    by_key = group_rows(curve_rows, ("candidate_alias", "source_test_id", "followup_profile"))
    output: list[dict[str, Any]] = []
    for (alias, test_id, profile), rows in by_key.items():
        ...
        output.append(
            {
                ...
                "net_profit_delta": as_float(routed.get("net_profit")) - as_float(tier_a.get("net_profit")),
                "trade_count_delta": as_int(routed.get("trade_count")) - as_int(tier_a.get("trade_count")),
                "fallback_used_count": as_int(routed.get("tier_b_fallback_used_count")),
                "duplicate_boundary": "duplicate_when_fallback_disabled" if as_int(routed.get("tier_b_fallback_used_count")) == 0 else "routed_total_has_fallback",
            }
        )
    return output
...
def forensics_summary(rows: Sequence[Mapping[str, Any]]) -> dict[str, Any]:
    ...
    return {
        ...
        "to_dates": to_dates,
        "cost_assumption_boundary": "MT5 tester broker-history costs; no separate cost authority claimed",
        "backtest_judgment": "usable_with_boundary" if rows and statuses == ["completed"] and report_statuses == ["completed"] else "inconclusive",
    }
...
def update_workspace_state_text(text: str, result: Mapping[str, Any]) -> str:
    ...
    output: list[str] = []
    in_stage267 = False
    inserted_focus = "run267AI(" in text
    inserted_path = "run267AI_noncalendar_state_guard_followup_balance_timeslice_trade_quality_review_report_path" in text
    focus_block = [
        "- >-",
        f"  Stage267(267단계) run267AI(267AI 실행) noncalendar state guard follow-up balance/time-slice/trade-quality review(비달력 상태 방어 후속 잔액/시간구간/거래품질 검토) `{status}`. Effect(효과): run267AH(267AH 실행)의 6개 MT5(MetaTrader 5, 메타트레이더5) 보고서를 거래 단위로 다시 읽어 curve(곡선), weak slice(약한 구간), trade quality(거래 품질) 판단을 만들었고 selected candidate(선택 후보), ONNX readiness(ONNX 준비), Goal Achieve(목표 달성)는 주장하지 않는다.",
    ]
    for line in lines:
        ...
        if in_stage267:
            ...
            if stripped.startswith("next_action:"):
                ...
                continue
            if "run267AH_noncalendar_state_guard_followup_mt5_execution_report_path" in stripped and not inserted_path:
                output.append(line)
                output.append(f"  run267AI_noncalendar_state_guard_followup_balance_timeslice_trade_quality_review_report_path: {rel(REPORT_PATH)}")
                inserted_path = True
                continue
        output.append(line)
...
def update_current_truth_docs(result: Mapping[str, Any]) -> None:
    ...
    next_action = str(result["next_action"])
    report_line = (
        "- run267AI_noncalendar_state_guard_followup_balance_timeslice_trade_quality_review"
        f"(267AI 비달력 상태 방어 후속 잔액/시간구간/거래품질 검토): `{rel(REPORT_PATH)}`"
    )
    latest_line = (
        "- latest_mt5_review(최신 MT5 검토): run267AI(267AI 실행) "
    ...
    for path in (CURRENT_WORKING_STATE_PATH, SELECTION_STATUS_PATH, REVIEW_INDEX_PATH):
        ...
        if path == CURRENT_WORKING_STATE_PATH:
            ...
            text = replace_line_prefix(
                text,
                "- action(",
                "- action(행동): run267AI(267AI 실행)는 run267AH(267AH 실행)의 6개 noncalendar state guard follow-up MT5 reports(비달력 상태 방어 후속 MT5 보고서)를 거래 단위로 검토했다.",
            )
            text = replace_line_prefix(
                text,
                "- effect(",
                "- effect(효과): 다음 run267AJ(267AJ 실행)에서 s264_aia(264 AIA) replacement pressure(대체 압박)는 관찰 유지하고, s264_aih(264 AIH) core role pressure(핵심 역할 압박)는 DD(drawdown, 손실폭)와 2024-12 약점 때문에 가지치기/수리 경계로 분리할 수 있다.",
            )
            text = replace_line_prefix(
                text,
                "- adapter_under_review(",
                "- adapter_under_review(검토 중 어댑터): `noncalendar_state_guard_followup_balance_timeslice_trade_quality_review`",
            )
            text = append_after_contains(text, "run267AH_noncalendar_state_guard_followup_mt5_execution", report_line)
            ...
            if path == REVIEW_INDEX_PATH:
                text = replace_line_prefix(text, "- status(", f"- status(상태): `{status}`")
            text = append_after_contains(text, "run267AH_noncalendar_state_guard_followup_mt5_execution", report_line)
        text = append_block_once(text, "Run267AI(267AI 실행)는 run267AH", closing_block)
        write_md(path, text)
...
def update_ledgers(created_at: str, result: Mapping[str, Any]) -> None:
    ...
    constructive_count = int(result["constructive_curve_rows"])
    ...
    upsert_csv_rows(
        ...
        [
            {
                "row_id": f"{RUN_ID}__noncalendar_state_guard_followup_balance_timeslice_trade_quality_review",
                "stage_id": STAGE_ID,
                "run_id": RUN_ID,
                "view": "noncalendar_state_guard_followup_balance_timeslice_trade_quality_review",
                "tier_scope": "Tier A primary read plus Tier A+B duplicate boundary",
                "scoreboard": "curve_time_slice_trade_quality_trade_shape_review",
                "status": status,
                "judgment": judgment,
                "evidence_boundary": "diagnostic_curve_timeslice_trade_quality_review_no_candidate_selection",
                "report_path": rel(REPORT_PATH),
                "notes": f"trade_records={trade_count};candidate_test_rows={candidate_count};next_action={next_action}.",
            }
        ],
        key="row_id",
    )
        ...
        [
            {
                "run_id": RUN_ID,
                "stage_id": STAGE_ID,
                "lane": "baseline_candidate_racing_noncalendar_state_guard_followup_review",
                "status": status,
                "judgment": judgment,
                "path": rel(REPORT_PATH),
                "notes": (
                    "Run267AI follow-up MT5 curve/time-slice/trade-quality review; "
    ...
    upsert_csv_rows(
        ...
        [
            {
                "ledger_row_id": f"{RUN_ID}__noncalendar_state_guard_followup_balance_timeslice_trade_quality_review",
                "stage_id": STAGE_ID,
                "run_id": RUN_ID,
                "subrun_id": "noncalendar_state_guard_followup_balance_timeslice_trade_quality_review",
                "parent_run_id": PARENT_RUN_ID,
                "record_view": "noncalendar_state_guard_followup_balance_timeslice_trade_quality_review",
                "tier_scope": "Tier A primary read plus Tier A+B duplicate boundary",
                ...
                "primary_kpi": (
                    f"trade_records={trade_count};candidate_test_rows={candidate_count};"
                    f"constructive_rows={constructive_count};negative_slices={negative_count}"
                ),
                "guardrail_kpi": "selected_candidate=none;onnx_readiness=not_claimed;goal_achieve=not_claimed",
                "external_verification_status": "completed_for_run267AH_mt5_report_review",
    ...
    entries = (
        ("stage267_run267AI_review_script", "producer_script", PRODUCER_PATH, "Builds run267AI follow-up curve/time-slice/trade-quality review."),
        ("stage267_run267AI_source_execution_result", "source_execution_result", SOURCE_EXECUTION_RESULT_PATH, "Source run267AH execution result."),
        ("stage267_run267AI_source_kpi_summary", "source_kpi_summary", SOURCE_KPI_SUMMARY_PATH, "Source run267AH KPI summary."),
        ("stage267_run267AI_source_forensics", "source_forensics", SOURCE_FORENSICS_PATH, "Source run267AH backtest forensics."),
        ("stage267_run267AI_source_attempts", "source_attempt_manifest", SOURCE_EXECUTED_ATTEMPTS_PATH, "Source run267AH executed attempts."),
        ("stage267_run267AI_trade_records", "trade_records", TRADE_RECORDS_PATH, "Run267AI paired trade records from run267AH reports."),
        ("stage267_run267AI_time_slice_kpi", "time_slice_kpi", TIME_SLICE_KPI_PATH, "Run267AI month/weekday/hour/session/direction/chron-segment KPI."),
        ("stage267_run267AI_curve_diagnostics", "curve_diagnostics", CURVE_DIAGNOSTICS_PATH, "Run267AI closed-balance curve diagnostics."),
        ("stage267_run267AI_candidate_test_review", "candidate_test_review", CANDIDATE_TEST_REVIEW_PATH, "Run267AI candidate-test curve and weak-slice review."),
        ("stage267_run267AI_candidate_summary", "candidate_summary", CANDIDATE_SUMMARY_PATH, "Run267AI candidate balance/time-slice summary."),
        ("stage267_run267AI_followup_profile_summary", "followup_profile_summary", FOLLOWUP_PROFILE_SUMMARY_PATH, "Run267AI follow-up profile summary."),
        ("stage267_run267AI_negative_slice_summary", "negative_slice_summary", NEGATIVE_SLICE_PATH, "Run267AI worst negative Tier A slices."),
        ("stage267_run267AI_parser_checks", "parser_checks", PARSER_CHECKS_PATH, "Run267AI parser reconciliation checks."),
        ("stage267_run267AI_parser_errors", "parser_errors", PARSER_ERRORS_PATH, "Run267AI parser errors."),
        ("stage267_run267AI_tier_duplicate_review", "audit_matrix", TIER_DUPLICATE_REVIEW_PATH, "Run267AI Tier A versus Tier A+B duplicate boundary."),
        ("stage267_run267AI_review_result", "review_result", REVIEW_RESULT_PATH, "Run267AI review JSON payload."),
        ("stage267_run267AI_review_report", "review_report", REPORT_PATH, "User-facing run267AI balance/time-slice/trade-quality review."),
    )
    rows = [
        {
...
def report_markdown(result: Mapping[str, Any]) -> str:
    ...
    lines = [
        ...
        "",
        f"- action(행동): run267AH(267AH 실행)의 `{len(result['parser_checks'])}`개 MT5(MetaTrader 5, 메타트레이더5) 보고서를 trade list(거래 목록) 단위로 다시 읽었다.",
    ...
    for index, row in enumerate(top_rows(candidate_tests), start=1):
        lines.append(
            f"| {index} | `{row.get('candidate_alias')}` | `{row.get('candidate_role')}` | `{row.get('source_test_id')}` | "
...
def review() -> dict[str, Any]:
    ...
    profile_summary = build_profile_summary(candidate_tests)
    ...
    constructive_count = sum(1 for row in candidate_tests if row.get("curve_read") == "constructive_curve_watch_not_selection")
    result = {
        "status": status,
        "judgment": judgment,
        "stage_id": STAGE_ID,
