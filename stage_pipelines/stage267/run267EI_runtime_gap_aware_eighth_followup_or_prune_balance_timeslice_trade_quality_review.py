...
from stage_pipelines.stage267 import run267EH_runtime_gap_aware_eighth_followup_or_prune_mt5_executor as source_executor
...
RUN_NUMBER = "run267EI"
RUN_ID = "run267EI_stage267_runtime_gap_aware_eighth_followup_or_prune_balance_timeslice_trade_quality_review_v1"
...
PARTIAL_STATUS = "run267EI_runtime_gap_aware_eighth_followup_or_prune_balance_timeslice_trade_quality_review_partial_parser_errors"
NEXT_ACTION = "run267EJ_design_runtime_gap_aware_ninth_followup_or_prune_from_run267EI_review"
NEXT_ACTION_PARTIAL = "run267EI_repair_trade_report_parser_before_ninth_followup_design"
...
RUN_ROOT = STAGE_ROOT / "02_runs" / RUN_NUMBER / "runtime_gap_aware_eighth_followup_or_prune_balance_timeslice_trade_quality_review"
...
REPORT_PATH = REVIEWS_ROOT / "stage267_run267EI_runtime_gap_aware_eighth_followup_or_prune_balance_timeslice_trade_quality_review.md"
PRODUCER_PATH = Path("stage_pipelines/stage267/run267EI_runtime_gap_aware_eighth_followup_or_prune_balance_timeslice_trade_quality_review.py")
...
def build_result_judgment(result: Mapping[str, Any]) -> list[dict[str, Any]]:
    return [
        {
            "result_subject": "run267EI balance/time-slice/trade-quality review(267EI 잔액/시간구간/거래품질 검토)",
...
def artifact_rows(created_at: str) -> list[dict[str, Any]]:
    entries = (
        ("stage267_run267EI_producer", "producer_script", PRODUCER_PATH, "Builds run267EI balance/time-slice/trade-quality review."),
        ("stage267_run267EI_source_execution", "source_execution_result", SOURCE_EXECUTION_RESULT_PATH, "Source run267EH execution result."),
        ("stage267_run267EI_trade_records", "trade_records", TRADE_RECORDS_PATH, "Parsed trade records."),
        ("stage267_run267EI_time_slice_kpi", "time_slice_kpi", TIME_SLICE_KPI_PATH, "Time-slice KPI."),
        ("stage267_run267EI_curve_diagnostics", "curve_diagnostics", CURVE_DIAGNOSTICS_PATH, "Curve diagnostics."),
        ("stage267_run267EI_candidate_profile_review", "candidate_profile_review", CANDIDATE_PROFILE_REVIEW_PATH, "Candidate profile review."),
        ("stage267_run267EI_attempt_outcome_review", "attempt_outcome_review", ATTEMPT_OUTCOME_REVIEW_PATH, "Attempt outcome review."),
        ("stage267_run267EI_init_failure_summary", "init_failure_summary", INIT_FAILURE_SUMMARY_PATH, "Init/runtime output gap summary."),
        ("stage267_run267EI_negative_slices", "negative_slice_summary", NEGATIVE_SLICE_PATH, "Negative slice summary."),
        ("stage267_run267EI_attribution_summary", "performance_attribution_summary", ATTRIBUTION_SUMMARY_PATH, "Performance attribution summary."),
        ("stage267_run267EI_followup_queue", "followup_decision_queue", FOLLOWUP_DECISION_QUEUE_PATH, "Follow-up decision queue."),
        ("stage267_run267EI_result_judgment", "result_judgment", RESULT_JUDGMENT_PATH, "Result judgment."),
        ("stage267_run267EI_review_result", "review_result", REVIEW_RESULT_PATH, "Review result payload."),
        ("stage267_run267EI_report", "review_report", REPORT_PATH, "User-facing review report."),
    )
    return [
        {
...
def update_ledgers(created_at: str, result: Mapping[str, Any]) -> None:
    ...
    stage_row = {
        "row_id": "stage267_run267EI_runtime_gap_aware_eighth_followup_or_prune_balance_timeslice_trade_quality_review",
        "stage_id": STAGE_ID,
        "run_id": RUN_ID,
        "view": "runtime_gap_aware_eighth_followup_or_prune_balance_timeslice_trade_quality_review",
    ...
    run_row = {
        "run_id": RUN_ID,
        "stage_id": STAGE_ID,
        "lane": "runtime_gap_aware_eighth_followup_or_prune_balance_timeslice_trade_quality_review",
        "status": result["status"],
        "judgment": "review_completed_with_init_failures_no_candidate_selection",
    ...
    project_row = {
        ...
        "record_view": "runtime_gap_aware_eighth_followup_or_prune_balance_timeslice_trade_quality_review",
        "tier_scope": "Tier A completed rows; init/runtime gaps separated",
        "kpi_scope": "trade_curve_time_slice_trade_quality",
        "scoreboard_lane": "runtime_gap_aware_eighth_followup_or_prune_review",
        "status": result["status"],
        "judgment": "review_completed_with_init_failures_no_candidate_selection",
        "path": rel(REPORT_PATH),
...
def update_stage267_workspace_block(text: str, *, status: str, next_action: str) -> str:
    report_entry = f"  run267EI_runtime_gap_aware_eighth_followup_or_prune_balance_timeslice_trade_quality_review_report_path: {rel(REPORT_PATH)}"
    lines = text.splitlines()
    output: list[str] = []
    in_stage267 = False
...
def update_current_docs(result: Mapping[str, Any]) -> None:
    ...
    report_line = (
        ...
        f"(267EI 런타임 공백 반영 8차 후속/가지치기 잔액/시간구간/거래품질 검토): `{rel(REPORT_PATH)}`"
    )
    block = "\n".join(
        [
    ...
    for path in (CURRENT_WORKING_STATE_PATH, SELECTION_STATUS_PATH, REVIEW_INDEX_PATH):
        ...
        if path == CURRENT_WORKING_STATE_PATH:
            ...
            text = replace_line_prefix(
                ...
                "- adapter_under_review(검토 중 어댑터): `runtime_gap_aware_eighth_followup_or_prune_balance_timeslice_trade_quality_review`",
            )
            text = replace_line_prefix(text, "- next_action(다음 행동):", f"- next_action(다음 행동): `{next_action}`")
            text = append_after_contains(text, "stage267_run267EH_runtime_gap_aware_eighth_followup_or_prune_mt5_execution.md", report_line)
            ...
            text = replace_line_prefix(text, "- next_action(다음 행동):", f"- next_action(다음 행동): `{next_action}`")
            text = append_after_contains(text, "stage267_run267EH_runtime_gap_aware_eighth_followup_or_prune_mt5_execution", report_line)
        else:
            ...
            text = append_after_contains(text, "stage267_run267EH_runtime_gap_aware_eighth_followup_or_prune_mt5_execution.md", report_line)
        text = append_block_once(text, "Run267EI(267EI 실행)는 run267EH", block)
    ...
    focus = (
        "- >-\n"
        "  Stage267(267단계) run267EI(267EI 실행) runtime gap aware eighth follow-up/prune balance/time-slice/trade-quality review"
        f"(런타임 공백 반영 8차 후속/가지치기 잔액/시간구간/거래품질 검토) `{status}`. "
        f"Effect(효과): run267EH(267EH 실행)의 candidate_profile_rows(후보-프로필 행) `{len(result['candidate_profile_review'])}`, "
