...
from stage_pipelines.stage267 import run267I_soft_noncalendar_adapter_executor as executor
...
CANDIDATE_REVIEW_PATH = DESIGN_ROOT / "candidate_soft_adapter_review.csv"
NEGATIVE_SLICE_PATH = DESIGN_ROOT / "negative_slice_summary.csv"
REVIEW_RESULT_PATH = DESIGN_ROOT / "review_result.json"
REPORT_PATH = REVIEWS_ROOT / "stage267_run267I_soft_noncalendar_adapter_mt5_review.md"
PRODUCER_PATH = Path("stage_pipelines/stage267/run267I_soft_noncalendar_adapter_review.py")

STATUS = "run267I_p0_soft_noncalendar_adapter_mt5_review_completed"
PARTIAL_STATUS = "run267I_p0_soft_noncalendar_adapter_mt5_review_partial_parser_errors"
NEXT_ACTION = "run267J_design_retrained_soft_context_adapter_probe_with_stop_rules"
NEXT_ACTION_PARTIAL = "run267I_repair_soft_adapter_review_parser_errors"
DEPOSIT = 500.0
...
def review_read(curve: Mapping[str, Any], base: Mapping[str, Any], weak_month: Mapping[str, Any]) -> str:
    ...
    if net_delta > 40.0 and pf_delta > 0.03 and dd_delta < -4.0 and equity_dd <= 31.0 and weak_month_net > -110.0:
        return "constructive_soft_adapter_watch_not_selection(건설적 부드러운 어댑터 관찰, 선택 아님)"
...
def report_markdown(result: Mapping[str, Any]) -> str:
    review_rows = list(result["candidate_soft_adapter_review"])
...
def update_ledgers(created_at: str, result: Mapping[str, Any]) -> None:
    ...
    stage_row = {
        "row_id": "stage267_run267I_soft_noncalendar_adapter_mt5_review",
        "stage_id": STAGE_ID,
        "run_id": RUN_ID,
        "view": "p0_soft_noncalendar_adapter_mt5_review",
        "tier_scope": "Tier A and Tier A+B historical 2024 P0 soft adapter review",
        ...
        "notes": f"review_rows={len(result['candidate_soft_adapter_review'])};negative_slices={len(result['negative_slices'])};next_action={next_action}.",
    }
    ...
    upsert_csv(
        ...
        "run_id",
        {
            "run_id": RUN_ID,
            "stage_id": STAGE_ID,
            "lane": "baseline_candidate_racing_p0_soft_adapter_mt5_review",
            "status": status,
            "judgment": judgment,
            "path": rel(REPORT_PATH),
            "notes": f"Run267I soft adapter MT5 review; selected_candidate=none; onnx_readiness=not_claimed; next_action={next_action}.",
        },
    ...
    upsert_csv(
        ...
        "ledger_row_id",
        {
            "ledger_row_id": f"{RUN_ID}__p0_soft_noncalendar_adapter_mt5_review",
            "stage_id": STAGE_ID,
            "run_id": RUN_ID,
            "subrun_id": "p0_soft_noncalendar_adapter_mt5_review",
            "parent_run_id": RUN_ID,
            "record_view": "p0_soft_noncalendar_adapter_mt5_review",
            "tier_scope": "Tier A and Tier A+B historical 2024 P0 soft adapter review",
            ...
            "judgment": judgment,
            "path": rel(REPORT_PATH),
            "primary_kpi": f"review_rows={len(result['candidate_soft_adapter_review'])};negative_slices={len(result['negative_slices'])}",
            "guardrail_kpi": "selected_candidate=none;onnx_readiness=not_claimed;goal_achieve=not_claimed",
            "external_verification_status": "completed_for_run267I_mt5_batch_review",
    ...
    entries = (
        ("stage267_run267I_soft_adapter_review_script", "producer_script", PRODUCER_PATH, "Builds run267I soft adapter curve/time-slice review."),
        ("stage267_run267I_soft_adapter_trade_records", "trade_records", TRADE_RECORDS_PATH, "Run267I paired trade records."),
        ("stage267_run267I_soft_adapter_time_slice_kpi", "time_slice_kpi", TIME_SLICE_KPI_PATH, "Run267I month/week/hour/session/direction/chron-segment KPI."),
        ("stage267_run267I_soft_adapter_curve_diagnostics", "curve_diagnostics", CURVE_DIAGNOSTICS_PATH, "Run267I closed-balance curve diagnostics."),
        ("stage267_run267I_soft_adapter_candidate_review", "candidate_soft_adapter_review", CANDIDATE_REVIEW_PATH, "Run267I candidate soft adapter review."),
        ("stage267_run267I_soft_adapter_negative_slice_summary", "negative_slice_summary", NEGATIVE_SLICE_PATH, "Run267I worst negative slices."),
        ("stage267_run267I_soft_adapter_review_result", "review_result", REVIEW_RESULT_PATH, "Run267I review JSON payload."),
        ("stage267_run267I_soft_adapter_mt5_review_report", "review_report", REPORT_PATH, "User-facing run267I soft adapter MT5 review."),
    )
...
def update_docs(result: Mapping[str, Any]) -> None:
    status = str(result["status"])
    next_action = str(result["next_action"])
    current_line = f"- Stage267(267단계) run267I P0 soft non-calendar Adapter MT5 review(P0 부드러운 비달력 어댑터 MT5 검토): `{rel(REPORT_PATH)}`"
    index_line = f"- run267I_soft_noncalendar_adapter_mt5_review(267I 부드러운 비달력 어댑터 MT5 검토): `{rel(REPORT_PATH)}`"
    summary_line = (
    ...
    for path, line, anchor in (
        (CURRENT_WORKING_STATE_PATH, current_line, "stage267_run267I_soft_noncalendar_adapter_mt5_execution.md"),
        (SELECTION_STATUS_PATH, index_line, "run267I_soft_noncalendar_adapter_mt5_execution"),
        (REVIEW_INDEX_PATH, index_line, "run267I_soft_noncalendar_adapter_mt5_execution"),
    ):
        text = io_path(path).read_text(encoding="utf-8-sig")
        text = replace_status_and_next(text, status, next_action)
        text = append_after_contains(text, anchor, line)
        text = append_after_contains(text, "Run267I(267I 실행)는", summary_line)
        write_md(path, text)
    ...
    workspace = append_after_contains(
        workspace,
        "run267I_soft_noncalendar_adapter_mt5_execution_report_path",
        f"  run267I_soft_noncalendar_adapter_mt5_review_path: {rel(REPORT_PATH)}",
    )
...
def review() -> dict[str, Any]:
    ...
    result = {
        ...
        "candidate_soft_adapter_review": candidate_review,
        ...
        "outputs": {
            ...
            "candidate_soft_adapter_review": rel(CANDIDATE_REVIEW_PATH),
        ...
        json.dumps(
            {
                ...
                "review_rows": len(result["candidate_soft_adapter_review"]),
