...
from stage_pipelines.stage267 import run267BX_aggressive_impulse_dd_shape_cross_period_mt5_executor as source_executor


STAGE_ID = source_executor.STAGE_ID
RUN_NUMBER = "run267BY"
RUN_ID = "run267BY_stage267_aggressive_impulse_dd_shape_cross_period_balance_timeslice_trade_quality_review_v1"
PARENT_RUN_ID = source_executor.RUN_ID
SOURCE_MATERIALIZATION_RUN_ID = source_executor.SOURCE_RUN_ID
STATUS = "run267BY_aggressive_impulse_dd_shape_cross_period_balance_timeslice_trade_quality_review_completed"
PARTIAL_STATUS = "run267BY_aggressive_impulse_dd_shape_cross_period_balance_timeslice_trade_quality_review_partial_parser_errors"
JUDGMENT = "cross_period_curve_timeslice_trade_quality_review_completed_no_candidate_selection"
PARTIAL_JUDGMENT = "cross_period_curve_timeslice_trade_quality_review_partial_parser_errors_no_candidate_selection"
CLAIM_BOUNDARY = source_executor.CLAIM_BOUNDARY
NEXT_ACTION = "run267BZ_design_aggressive_impulse_dd_shape_cross_period_followup_or_prune"
NEXT_ACTION_PARTIAL = "run267BY_repair_trade_report_parser_errors_before_followup_design"

STAGE_ROOT = source_executor.STAGE_ROOT
REVIEWS_ROOT = source_executor.REVIEWS_ROOT
RUN_ROOT = STAGE_ROOT / "02_runs" / RUN_NUMBER / "aggressive_impulse_dd_shape_cross_period_balance_timeslice_trade_quality_review"
...
CANDIDATE_SUMMARY_PATH = RUN_ROOT / "candidate_cross_period_summary.csv"
...
REPORT_PATH = REVIEWS_ROOT / "stage267_run267BY_aggressive_impulse_dd_shape_cross_period_balance_timeslice_trade_quality_review.md"
PRODUCER_PATH = Path("stage_pipelines/stage267/run267BY_aggressive_impulse_dd_shape_cross_period_balance_timeslice_trade_quality_review.py")
...
def candidate_read(rows: Sequence[Mapping[str, Any]]) -> str:
    ...
    if len(rows) < 3 or any(net <= 0.0 for net in nets) or min(pfs or [0.0]) <= 1.0:
        return "insufficient_or_broken_prune_no_selection(부족하거나 깨짐, 선택 아님)"
...
def build_followup_queue(candidate_summary: Sequence[Mapping[str, Any]], period_summary: Sequence[Mapping[str, Any]]) -> list[dict[str, Any]]:
    ...
    best_net = sorted(candidate_summary, key=lambda row: -as_float(row.get("total_net_profit")))
    if best_stability:
        row = best_stability[0]
        rows.append(
            {
                "queue_id": f"run267by_q01_{row.get('candidate_alias')}_stability_deepen",
                "priority": "P0",
                "workstream": "aggressive_followup",
                "candidate_alias": row.get("candidate_alias"),
                "reason": "lowest worst DD among positive cross-period aggressive impulse rows(양수 확장 기간 행 중 최저 최악 손실폭)",
                "next_probe": "design run267BZ follow-up with curve-shape and weak-slice pressure; no selection",
    ...
    if best_net:
        ...
        rows.append(
            {
                "queue_id": f"run267by_q02_{row.get('candidate_alias')}_high_net_dd_watch",
                "priority": "P0",
                "workstream": "drawdown_shape_watch",
                "candidate_alias": row.get("candidate_alias"),
                "reason": "highest total net but still needs DD/late-segment review(최고 총 순수익이지만 손실폭/후반 검토 필요)",
                "next_probe": "do not promote; compare curve zoom and time-slice holes before any Adapter extension",
                "claim_boundary": CLAIM_BOUNDARY,
            }
        )
    weak_periods = [row for row in period_summary if str(row.get("period_read")).startswith("dd_watch")]
    for row in weak_periods:
        rows.append(
            {
                "queue_id": f"run267by_q03_{str(row.get('target_period')).lower()}_period_dd_watch",
                "priority": "P1",
                "workstream": "period_pressure",
                "target_period": row.get("target_period"),
                "reason": f"period worst DD percent={cell(row.get('worst_dd_percent'))}",
                "next_probe": "review negative slices and avoid single-period tuning loop",
                "claim_boundary": CLAIM_BOUNDARY,
            }
        )
    return rows
...
def build_failure_memory(candidate_summary: Sequence[Mapping[str, Any]], period_summary: Sequence[Mapping[str, Any]]) -> list[dict[str, Any]]:
    ...
    rows: list[dict[str, Any]] = [
        {
            "memory_id": "run267by_headline_positive_not_selection",
            "scope": "aggressive_impulse_replacement_cross_period(공격형 임펄스 대체 확장 기간)",
            "evidence": rel(CANDIDATE_SUMMARY_PATH),
            "failure_mode": "all candidates are positive across the three periods, but this is still a narrow aggressive profile and Tier B routed evidence is blocked",
            ...
            "salvage_condition": "use only for run267BZ design with curve zoom, DD-shape, and weak-slice checks",
        }
    ]
    if high_dd_candidates:
        rows.append(
            {
                "memory_id": "run267by_dd_watch_candidates",
    ...
    if any(str(row.get("target_period")) == "2025H2" and str(row.get("period_read")).startswith("dd_watch") for row in period_summary):
        rows.append(
            {
                "memory_id": "run267by_2025h2_dd_watch",
                "scope": "2025H2",
    ...
    rows: list[dict[str, Any]] = [
        {
            ...
            "observed_change": "aggressive impulse cross-period rows are all positive, but DD and period shape remain the gating issue",
            ...
            "segment_checks": "month/weekday/hour/session/direction/chron_segment and 2023H2/2025H1/2025H2 checked",
        ...
        rows.append(
            {
                ...
                "comparison_baseline": "same candidate across 2023H2/2025H1/2025H2",
        ...
        rows.append(
            {
                ...
                "comparison_baseline": "other adjacent periods in run267BY",
...
def update_current_truth_docs(result: Mapping[str, Any]) -> None:
    ...
    report_line = f"- run267BY_aggressive_impulse_dd_shape_cross_period_balance_timeslice_trade_quality_review(267BY 공격형 임펄스 손실폭 형태 확장 기간 잔액/시간구간/거래품질 검토): `{rel(REPORT_PATH)}`"
    block = "\n".join(
        [
            "Run267BY(267BY 실행)는 run267BX(267BX 실행)의 9개 MT5(MetaTrader 5, 메타트레이더5) 보고서를 trade list(거래 목록)로 다시 읽었다.",
            f"Effect(효과): trade records(거래 기록) `{result['trade_record_count']}`개, time-slice rows(시간 구간 행) `{result['time_slice_row_count']}`개, negative slices(음수 구간) `{result['negative_slice_count']}`개를 만들고 후보별 확장 기간 DD-shape(손실폭 형태)를 다음 설계 입력으로 고정했다.",
    ...
    for path in (CURRENT_WORKING_STATE_PATH, SELECTION_STATUS_PATH, REVIEW_INDEX_PATH):
        ...
        text = replace_line_prefix(text, "- adapter_under_review(검토 중 어댑터):", "- adapter_under_review(검토 중 어댑터): `aggressive_impulse_dd_shape_cross_period_balance_timeslice_trade_quality_review`")
        text = replace_line_prefix(text, "- next_action(다음 행동):", f"- next_action(다음 행동): `{next_action}`")
        text = append_after_contains(text, "stage267_run267BX_aggressive_impulse_dd_shape_cross_period_mt5_execution.md", report_line)
        text = append_block_once(text, "Run267BY(267BY 실행)는", block)
        write_md(path, text)

    workspace = io_path(WORKSPACE_STATE_PATH).read_text(encoding="utf-8-sig")
    focus = (
        "- >-\n"
        f"  Stage267(267단계) run267BY(267BY 실행) aggressive impulse DD-shape cross-period balance/time-slice/trade-quality review(공격형 임펄스 손실폭 형태 확장 기간 잔액/시간구간/거래품질 검토) `{status}`. "
        f"Effect(효과): run267BX(267BX 실행)의 9개 MT5(MetaTrader 5, 메타트레이더5) report(보고서)를 trade list(거래 목록), curve(곡선), weak slice(약한 구간)로 다시 읽었고 selected candidate(선택 후보), selected research baseline(선택 연구 기준 후보), ONNX readiness(ONNX 준비), Goal Achieve(목표 달성)는 주장하지 않는다.\n"
    )
    ...
    workspace = update_stage267_workspace_block(
        ...
        report_entry=f"  run267BY_aggressive_impulse_dd_shape_cross_period_balance_timeslice_trade_quality_review_report_path: {rel(REPORT_PATH)}",
    )
...
def update_ledgers_and_artifacts(created_at: str, result: Mapping[str, Any]) -> None:
    ...
    stage_row = {
        "row_id": "stage267_run267BY_aggressive_impulse_dd_shape_cross_period_balance_timeslice_trade_quality_review",
        "stage_id": STAGE_ID,
        "run_id": RUN_ID,
        "view": "aggressive_impulse_dd_shape_cross_period_balance_timeslice_trade_quality_review",
        "tier_scope": "Tier A run267BX review; Tier B and actual routed total blocked until true fallback manifest exists",
        "scoreboard": "trade_shape_curve_time_slice_cross_period_review",
        "status": status,
        "judgment": judgment,
    ...
    run_row = {
        "run_id": RUN_ID,
        "stage_id": STAGE_ID,
        "lane": "baseline_candidate_racing_aggressive_impulse_cross_period_balance_timeslice_trade_quality_review",
        "status": status,
        "judgment": judgment,
    ...
    project_row = {
        "ledger_row_id": f"{RUN_ID}__aggressive_impulse_dd_shape_cross_period_balance_timeslice_trade_quality_review",
        "stage_id": STAGE_ID,
        "run_id": RUN_ID,
        "subrun_id": "aggressive_impulse_dd_shape_cross_period_balance_timeslice_trade_quality_review",
        "parent_run_id": PARENT_RUN_ID,
        "record_view": "aggressive_impulse_dd_shape_cross_period_balance_timeslice_trade_quality_review",
        "tier_scope": "Tier A run267BX review; true fallback blocked",
        "kpi_scope": "balance_curve_time_slice_trade_quality",
        "scoreboard_lane": "aggressive_impulse_cross_period_trade_quality_review",
        "status": status,
        "judgment": judgment,
        "path": rel(REPORT_PATH),
        ...
        "external_verification_status": "completed_for_run267BX_trade_report_review",
    ...
    entries = (
        ("stage267_run267BY_producer", "producer_script", PRODUCER_PATH, "Builds run267BY aggressive impulse cross-period balance/time-slice/trade-quality review."),
        ("stage267_run267BY_source_execution_result", "source_execution_result", SOURCE_EXECUTION_RESULT_PATH, "Source run267BX execution result."),
        ("stage267_run267BY_trade_records", "trade_records", TRADE_RECORDS_PATH, "Parsed trade records."),
        ("stage267_run267BY_time_slice_kpi", "time_slice_kpi", TIME_SLICE_KPI_PATH, "Time-slice KPI."),
        ("stage267_run267BY_curve_diagnostics", "curve_diagnostics", CURVE_DIAGNOSTICS_PATH, "Curve diagnostics."),
        ("stage267_run267BY_candidate_period_review", "candidate_period_review", CANDIDATE_PERIOD_REVIEW_PATH, "Candidate-period review."),
        ("stage267_run267BY_candidate_summary", "candidate_summary", CANDIDATE_SUMMARY_PATH, "Candidate cross-period summary."),
        ("stage267_run267BY_period_summary", "period_summary", PERIOD_SUMMARY_PATH, "Period summary."),
        ("stage267_run267BY_negative_slice_summary", "negative_slice_summary", NEGATIVE_SLICE_PATH, "Negative slice summary."),
        ("stage267_run267BY_followup_queue", "followup_queue", FOLLOWUP_QUEUE_PATH, "Follow-up queue."),
        ("stage267_run267BY_failure_memory", "failure_memory", FAILURE_MEMORY_PATH, "Failure memory."),
        ("stage267_run267BY_parser_checks", "parser_checks", PARSER_CHECKS_PATH, "Parser checks."),
        ("stage267_run267BY_forensic_summary", "forensic_summary", FORENSIC_SUMMARY_PATH, "Backtest forensic summary."),
        ("stage267_run267BY_performance_attribution", "performance_attribution", PERFORMANCE_ATTRIBUTION_PATH, "Performance attribution."),
        ("stage267_run267BY_result_judgment", "result_judgment", RESULT_JUDGMENT_PATH, "Result judgment."),
        ("stage267_run267BY_review_result", "review_result", REVIEW_RESULT_PATH, "Review result payload."),
        ("stage267_run267BY_run_manifest", "run_manifest", RUN_MANIFEST_PATH, "Run manifest."),
        ("stage267_run267BY_lineage", "lineage", LINEAGE_PATH, "Lineage map."),
        ("stage267_run267BY_report", "review_report", REPORT_PATH, "User-facing report."),
    )
    rows = [
        {
