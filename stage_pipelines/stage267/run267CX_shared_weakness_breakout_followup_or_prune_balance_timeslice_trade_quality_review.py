...
from stage_pipelines.stage267 import (
    run267CW_shared_weakness_breakout_followup_or_prune_mt5_executor as source_executor,
)
...
RUN_NUMBER = "run267CX"
RUN_ID = "run267CX_stage267_shared_weakness_breakout_followup_or_prune_balance_timeslice_trade_quality_review_v1"
STATUS = "run267CX_shared_weakness_breakout_followup_or_prune_balance_timeslice_trade_quality_review_completed"
PARTIAL_STATUS = "run267CX_shared_weakness_breakout_followup_or_prune_balance_timeslice_trade_quality_review_partial_parser_errors"
NEXT_ACTION = "run267CY_design_shared_weakness_breakout_followup_or_prune_from_run267CX_review"
NEXT_ACTION_PARTIAL = "run267CX_repair_shared_weakness_breakout_followup_or_prune_trade_parser_errors"
...
RUN_ROOT = STAGE_ROOT / "02_runs" / RUN_NUMBER / "shared_weakness_breakout_followup_or_prune_balance_timeslice_trade_quality_review"
...
REPORT_PATH = REVIEWS_ROOT / "stage267_run267CX_shared_weakness_breakout_followup_or_prune_balance_timeslice_trade_quality_review.md"
PRODUCER_PATH = Path("stage_pipelines/stage267/run267CX_shared_weakness_breakout_followup_or_prune_balance_timeslice_trade_quality_review.py")
        ...
        output.append(
            {
                ...
                "likely_drivers": "run267CV(267CV 실행)가 run267CU(267CU 실행) queue(대기열)를 redzone(위험 구역), aggressive supply(공격형 공급), explosive shock-state(폭발형 충격 상태) follow-up/prune(후속/가지치기) profile(프로필)로 물질화했다.",
...
def next_probe_for_row(row: Mapping[str, Any], weakest: Mapping[str, Any]) -> str:
    ...
    if profile == "aih_aggressive_supply_repair" and trades < 300:
        return "s264_aih_aggressive_supply_needs_trade_count_repair_or_prune(공격형 공급 거래 수 수리 또는 가지치기)"
    if dd >= 20.0 or weakest_net <= -170.0:
        return "inspect_balance_curve_and_weak_slice_before_followup(후속 전 곡선과 약점 구간 확대 검토)"
    if profile == "redzone_monday_dd_pressure":
        return "redzone_monday_dd_survived_but_needs_day_dd_zoom(위험 구역 월요일 손실폭 압박 생존, 요일/손실폭 확대 필요)"
    if profile == "explosive_shock_state_combo":
        return "explosive_combo_needs_cross_period_and_prune_gate(폭발형 조합 확장 기간 및 가지치기 게이트 필요)"
...
def build_result_judgment(result: Mapping[str, Any]) -> list[dict[str, Any]]:
    return [
        {
            "result_subject": "run267CX shared weakness follow-up/prune balance/time-slice/trade-quality review(267CX 공유 약점 후속/가지치기 잔액/시간구간/거래품질 검토)",
            "evidence_available": "trade records(거래 기록), time-slice KPI(시간구간 핵심 성과 지표), curve diagnostics(곡선 진단), negative slices(음수 구간)",
            "evidence_missing": "adjacent-period pressure(인접 기간 압박), full Adapter package(전체 어댑터 패키지), ONNX parity(ONNX 동등성)",
...
def artifact_rows(created_at: str) -> list[dict[str, Any]]:
    entries = (
        ("stage267_run267CX_producer", "producer_script", PRODUCER_PATH, "Builds run267CX balance/time-slice/trade-quality review."),
        ("stage267_run267CX_trade_records", "trade_records", TRADE_RECORDS_PATH, "Parsed trade records."),
        ("stage267_run267CX_time_slice_kpi", "time_slice_kpi", TIME_SLICE_KPI_PATH, "Time-slice KPI."),
        ("stage267_run267CX_curve_diagnostics", "curve_diagnostics", CURVE_DIAGNOSTICS_PATH, "Curve diagnostics."),
        ("stage267_run267CX_candidate_profile_review", "candidate_profile_review", CANDIDATE_PROFILE_REVIEW_PATH, "Candidate-profile review."),
        ("stage267_run267CX_candidate_summary", "candidate_summary", CANDIDATE_SUMMARY_PATH, "Candidate summary."),
        ("stage267_run267CX_profile_axis_summary", "profile_axis_summary", PROFILE_AXIS_SUMMARY_PATH, "Profile axis summary."),
        ("stage267_run267CX_negative_slice", "negative_slice_summary", NEGATIVE_SLICE_PATH, "Negative slice summary."),
        ("stage267_run267CX_attribution", "performance_attribution_summary", ATTRIBUTION_SUMMARY_PATH, "Performance attribution."),
        ("stage267_run267CX_result_judgment", "result_judgment", RESULT_JUDGMENT_PATH, "Result judgment."),
        ("stage267_run267CX_parser_checks", "parser_checks", PARSER_CHECKS_PATH, "Parser checks."),
        ("stage267_run267CX_review_result", "review_result", REVIEW_RESULT_PATH, "Review result payload."),
        ("stage267_run267CX_report", "review_report", REPORT_PATH, "User-facing review report."),
    )
    return [
        {
...
def update_ledgers(created_at: str, result: Mapping[str, Any]) -> None:
    ...
    stage_row = {
        "row_id": "stage267_run267CX_shared_weakness_breakout_followup_or_prune_balance_timeslice_trade_quality_review",
        "stage_id": STAGE_ID,
        "run_id": RUN_ID,
        "view": "shared_weakness_breakout_followup_or_prune_balance_timeslice_trade_quality_review",
    ...
    run_row = {
        "run_id": RUN_ID,
        "stage_id": STAGE_ID,
        "lane": "shared_weakness_breakout_followup_or_prune_balance_timeslice_trade_quality_review",
        "status": status,
        "judgment": "review_completed_no_candidate_selection",
    ...
    project_row = {
        "ledger_row_id": f"{RUN_ID}__shared_weakness_breakout_followup_or_prune_balance_timeslice_trade_quality_review",
        "stage_id": STAGE_ID,
        "run_id": RUN_ID,
        "subrun_id": "shared_weakness_breakout_followup_or_prune_balance_timeslice_trade_quality_review",
        "parent_run_id": SOURCE_RUN_ID,
        "record_view": "shared_weakness_breakout_followup_or_prune_balance_timeslice_trade_quality_review",
...
def update_current_docs(result: Mapping[str, Any]) -> None:
    ...
    report_line = (
        ...
        f"(267CX 공유 약점 후속/가지치기 잔액/시간구간/거래품질 검토): `{rel(REPORT_PATH)}`"
    )
    latest_line = (
        f"- latest_review(최신 검토): run267CX(267CX 실행) candidate_profile_rows(후보-프로필 행) "
    ...
    current = replace_line_prefix(
        ...
        "- adapter_under_review(검토 중 어댑터): `shared_weakness_breakout_followup_or_prune_balance_timeslice_trade_quality_review`",
    )
    current = replace_line_prefix(current, "- status(상태):", f"- status(상태): `{status}`")
    current = replace_line_prefix(current, "- next_action(다음 행동):", f"- next_action(다음 행동): `{next_action}`")
    current = append_after_contains(current, "stage267_run267CW_shared_weakness_breakout_followup_or_prune_mt5_execution.md", report_line)
    current = append_after_contains(current, "## Current Next Action", latest_line)
    current = append_block_once(current, "Run267CX(267CX 실행)는 run267CW", block)
    ...
    selection = append_after_contains(selection, "run267CW_shared_weakness_breakout_followup_or_prune_mt5_execution", report_line)
    selection = append_block_once(selection, "Run267CX(267CX 실행)는 run267CW", block)
    ...
    review_index = append_after_contains(review_index, "run267CW_shared_weakness_breakout_followup_or_prune_mt5_execution", report_line)
    review_index = append_block_once(review_index, "Run267CX(267CX 실행)는 run267CW", block)
    ...
    workspace = append_after_contains(
        workspace,
        "run267CW_shared_weakness_breakout_followup_or_prune_mt5_execution_report_path",
        f"  run267CX_shared_weakness_breakout_followup_or_prune_balance_timeslice_trade_quality_review_report_path: {rel(REPORT_PATH)}",
    )
    focus_line = (
        "- >-\n"
        f"  Stage267(267단계) run267CX(267CX 실행) shared weakness breakout follow-up/prune balance/time-slice/trade-quality review"
        f"(공유 약점 돌파 후속/가지치기 잔액/시간구간/거래품질 검토) `{status}`. "
        f"Effect(효과): run267CW(267CW 실행)의 MT5(MetaTrader 5, 메타트레이더5) report(보고서)를 거래 목록과 약한 구간으로 다시 읽었고, "
