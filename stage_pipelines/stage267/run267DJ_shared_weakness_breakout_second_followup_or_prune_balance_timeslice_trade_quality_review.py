...
from stage_pipelines.stage267 import (
    run267DI_shared_weakness_breakout_second_followup_or_prune_mt5_executor as source_executor,
)
...
RUN_NUMBER = "run267DJ"
RUN_ID = "run267DJ_stage267_shared_weakness_breakout_second_followup_or_prune_balance_timeslice_trade_quality_review_v1"
STATUS = "run267DJ_shared_weakness_breakout_second_followup_or_prune_balance_timeslice_trade_quality_review_completed"
PARTIAL_STATUS = "run267DJ_shared_weakness_breakout_second_followup_or_prune_balance_timeslice_trade_quality_review_partial_parser_errors"
NEXT_ACTION = "run267DK_design_shared_weakness_breakout_second_followup_or_prune_from_run267DJ_review"
NEXT_ACTION_PARTIAL = "run267DJ_repair_shared_weakness_breakout_second_followup_or_prune_trade_parser_errors"
...
REPORT_PATH = REVIEWS_ROOT / "stage267_run267DJ_shared_weakness_breakout_second_followup_or_prune_balance_timeslice_trade_quality_review.md"
PRODUCER_PATH = Path("stage_pipelines/stage267/run267DJ_shared_weakness_breakout_second_followup_or_prune_balance_timeslice_trade_quality_review.py")
...
def curve_read(item: Mapping[str, Any], report_metrics: Mapping[str, Any], month_rows: Sequence[Mapping[str, Any]]) -> str:
    ...
    if equity_dd >= 26.0 or worst_month_net <= -240.0:
        return "profit_with_deep_drawdown_or_month_hole(수익은 있으나 깊은 손실폭 또는 월별 구멍)"
    ...
    if trades < 300 and pf >= 1.45:
        return "high_profit_factor_but_supply_thin_no_selection(높은 수익 팩터이나 거래 공급 얇음, 선택 아님)"
...
def review_read(curve: Mapping[str, Any], base: Mapping[str, Any], weak_month: Mapping[str, Any]) -> str:
    ...
    if equity_dd >= 26.0 or weak_month_net <= -240.0:
        return "profit_but_drawdown_or_month_hole_uncomfortable(수익은 있으나 손실폭 또는 월별 구멍 불편)"
    if net > base_net and equity_dd <= max(base_dd + 3.0, 18.0) and pf >= 1.45 and trade_count >= 350:
        return "constructive_stability_clue_no_selection(건설적 안정성 단서, 선택 아님)"
    if net >= 1700.0 and pf >= 1.40 and trade_count >= 350 and equity_dd < 18.0:
        return "high_profit_needs_curve_zoom_no_selection(높은 수익이나 곡선 확대 검토 필요, 선택 아님)"
...
def profile_read(items: Sequence[Mapping[str, Any]]) -> str:
    ...
    if risk_rows:
        return "profile_profitable_but_risk_rows_present(프로필 수익성은 있으나 위험 행 존재)"
    if thin_rows:
        ...
        output.append(
            {
                ...
                "likely_drivers": "run267DI(267DI 실행)는 run267DH(267DH 실행)의 survivor gate(생존 관문), validation control(검증 대조), thin-supply impulse stress(얇은 공급 충격 압박), DD de-escalation(손실폭 완화)을 MT5 report(MetaTrader 5 보고서)와 KPI(핵심 성과 지표)로 연결했다.",
...
def next_probe_for_row(row: Mapping[str, Any], weakest: Mapping[str, Any]) -> str:
    ...
    if "s258_session_cross_period_stress" in profile and (dd >= 14.0 or weakest_net <= -170.0):
        return "s258_cross_period_stress_needs_curve_zoom_or_short_repair(s258 기간 교차 압박 곡선 확대 또는 숏 수리 필요)"
    if "similar_replacement_watch" in profile and net > 0.0 and pf >= 1.30:
        return "s264_aia_replacement_survived_enter_survivor_gate_no_selection(s264_aia 유사 대체 생존, 생존 관문 진입 가능, 선택 아님)"
    ...
    return "candidate_profile_can_enter_followup_design_no_selection(후보 프로필 후속 설계 진입 가능, 선택 아님)"

def build_result_judgment(result: Mapping[str, Any]) -> list[dict[str, Any]]:
    return [
        {
            "result_subject": "run267DJ shared weakness second follow-up/prune balance/time-slice/trade-quality review(267DJ 공유 약점 2차 후속/가지치기 잔액/시간구간/거래품질 검토)",
            "evidence_available": "trade records(거래 기록), time-slice KPI(시간구간 핵심 성과 지표), curve diagnostics(곡선 진단), negative slices(음수 구간)",
            "evidence_missing": "feature ablation/replacement follow-up gates(피처 제거/대체 후속 관문), Adapter package(어댑터 패키지), runtime reproduction(런타임 재현), ONNX parity(ONNX 동등성)",
...
def artifact_rows(created_at: str) -> list[dict[str, Any]]:
    entries = (
        ("stage267_run267DJ_producer", "producer_script", PRODUCER_PATH, "Builds run267DJ balance/time-slice/trade-quality review."),
        ("stage267_run267DJ_trade_records", "trade_records", TRADE_RECORDS_PATH, "Parsed trade records."),
        ("stage267_run267DJ_time_slice_kpi", "time_slice_kpi", TIME_SLICE_KPI_PATH, "Time-slice KPI."),
        ("stage267_run267DJ_curve_diagnostics", "curve_diagnostics", CURVE_DIAGNOSTICS_PATH, "Curve diagnostics."),
        ("stage267_run267DJ_candidate_profile_review", "candidate_profile_review", CANDIDATE_PROFILE_REVIEW_PATH, "Candidate-profile review."),
        ("stage267_run267DJ_candidate_summary", "candidate_summary", CANDIDATE_SUMMARY_PATH, "Candidate summary."),
        ("stage267_run267DJ_profile_axis_summary", "profile_axis_summary", PROFILE_AXIS_SUMMARY_PATH, "Profile axis summary."),
        ("stage267_run267DJ_negative_slice", "negative_slice_summary", NEGATIVE_SLICE_PATH, "Negative slice summary."),
        ("stage267_run267DJ_attribution", "performance_attribution_summary", ATTRIBUTION_SUMMARY_PATH, "Performance attribution."),
        ("stage267_run267DJ_result_judgment", "result_judgment", RESULT_JUDGMENT_PATH, "Result judgment."),
        ("stage267_run267DJ_parser_checks", "parser_checks", PARSER_CHECKS_PATH, "Parser checks."),
        ("stage267_run267DJ_review_result", "review_result", REVIEW_RESULT_PATH, "Review result payload."),
        ("stage267_run267DJ_report", "review_report", REPORT_PATH, "User-facing review report."),
    )
    return [
        {
...
def update_ledgers(created_at: str, result: Mapping[str, Any]) -> None:
    ...
    stage_row = {
        "row_id": "stage267_run267DJ_shared_weakness_breakout_second_followup_or_prune_balance_timeslice_trade_quality_review",
        "stage_id": STAGE_ID,
        "run_id": RUN_ID,
...
def update_current_docs(result: Mapping[str, Any]) -> None:
    ...
    report_line = (
        "- run267DJ_shared_weakness_breakout_second_followup_or_prune_balance_timeslice_trade_quality_review"
        f"(267DJ 공유 약점 후속/가지치기 잔액/시간구간/거래품질 검토): `{rel(REPORT_PATH)}`"
    )
    latest_line = (
        f"- latest_review(최신 검토): run267DJ(267DJ 실행) candidate_profile_rows(후보-프로필 행) "
    ...
    current = append_after_contains(current, "Effect(효과): run267DH(267DH 실행)의 MT5", report_line)
    current = append_after_contains(current, "## Current Next Action", latest_line)
    current = append_block_once(current, "Run267DJ(267DJ 실행)는 run267DI", block)
    ...
    selection = append_after_contains(selection, "run267DI_shared_weakness_breakout_second_followup_or_prune_mt5_execution", report_line)
    selection = append_block_once(selection, "Run267DJ(267DJ 실행)는 run267DI", block)
    ...
    review_index = append_after_contains(review_index, "run267DI_shared_weakness_breakout_second_followup_or_prune_mt5_execution", report_line)
    review_index = append_block_once(review_index, "Run267DJ(267DJ 실행)는 run267DI", block)
    ...
    workspace = append_after_contains(
        workspace,
        "run267DI_shared_weakness_breakout_second_followup_or_prune_mt5_execution_report_path",
        f"  run267DJ_shared_weakness_breakout_second_followup_or_prune_balance_timeslice_trade_quality_review_report_path: {rel(REPORT_PATH)}",
    )
    focus_line = (
        "- >-\n"
        f"  Stage267(267단계) run267DJ(267DJ 실행) shared weakness breakout second follow-up/prune balance/time-slice/trade-quality review"
        f"(공유 약점 돌파 후속/가지치기 잔액/시간구간/거래품질 검토) `{status}`. "
        f"Effect(효과): run267DI(267DI 실행)의 MT5 report(MetaTrader 5 보고서)를 거래 목록과 약한 구간으로 다시 읽었고, "
