...
from stage_pipelines.stage267 import run267ED_runtime_gap_aware_seventh_followup_or_prune_mt5_executor as source_executor
...
RUN_NUMBER = "run267EE"
RUN_ID = "run267EE_stage267_runtime_gap_aware_seventh_followup_or_prune_balance_timeslice_trade_quality_review_v1"
STATUS = "run267EE_runtime_gap_aware_seventh_followup_or_prune_balance_timeslice_trade_quality_review_completed"
PARTIAL_STATUS = "run267EE_runtime_gap_aware_seventh_followup_or_prune_balance_timeslice_trade_quality_review_partial_parser_errors"
NEXT_ACTION = "run267EF_design_runtime_gap_aware_eighth_followup_or_prune_from_run267EE_review"
NEXT_ACTION_PARTIAL = "run267EE_repair_trade_report_parser_before_eighth_followup_design"
...
RUN_ROOT = STAGE_ROOT / "02_runs" / RUN_NUMBER / "runtime_gap_aware_seventh_followup_or_prune_balance_timeslice_trade_quality_review"
...
REPORT_PATH = REVIEWS_ROOT / "stage267_run267EE_runtime_gap_aware_seventh_followup_or_prune_balance_timeslice_trade_quality_review.md"
PRODUCER_PATH = Path("stage_pipelines/stage267/run267EE_runtime_gap_aware_seventh_followup_or_prune_balance_timeslice_trade_quality_review.py")
...
def build_attribution_summary(candidate_rows: Sequence[Mapping[str, Any]]) -> list[dict[str, Any]]:
    ...
    for alias, rows in group_by(candidate_rows, "candidate_alias").items():
        ...
        if alias == "s258_stc":
            ...
            next_probe = "eighth follow-up(8차 후속)에서는 DD shape(손실폭 형태)와 adverse state(불리 상태)를 합치기보다 약한 기간별 생존 조건을 먼저 판정한다."
        elif alias == "s264_aih":
            ...
            observed = "s262_lih는 validation coverage rejoin(검증 커버리지 재합류)에서는 양수지만 2026.04 coverage rejoin(커버리지 재합류)에서 음수라 validation-heavy(검증 중심) 역할은 유지하되 최종월 약점이 남는다."
            next_probe = "validation strength(검증 강점)과 final-month fragility(마지막 달 취약성)를 분리해 대조 후보로 유지할지 판단한다."
        elif alias == "s264_aia":
            observed = "s264_aia는 validation coverage rejoin(검증 커버리지 재합류)에서는 s262_lih와 같은 양수 형태지만 2026.04 coverage rejoin(커버리지 재합류)에서 음수라 OOS anchor(표본외 앵커) 역할을 바로 회복했다고 볼 수 없다."
            next_probe = "OOS anchor(표본외 앵커) 역할은 final-month hole(마지막 달 구멍)과 validation damage(검증 손상)를 같이 본 뒤 유지 여부를 정한다."
        else:
            observed = "이 후보는 run267ED(267ED 실행) 검토에서 별도 역할 판독이 필요하다."
            next_probe = "다음 설계에서 후보 역할과 실패 조건을 분리한다."
        output.append(
            {
...
def build_result_judgment(result: Mapping[str, Any]) -> list[dict[str, Any]]:
    return [
        {
            "result_subject": "run267EE balance/time-slice/trade-quality review(267EE 잔액/시간구간/거래품질 검토)",
            ...
            "user_explanation_hook": "이번 검토는 후보 선택이 아니라 run267ED의 거래·곡선·약한 구간을 다음 설계 재료로 바꾸는 작업이다.",
        }
    ]
...
def artifact_rows(created_at: str) -> list[dict[str, Any]]:
    entries = (
        ("stage267_run267EE_producer", "producer_script", PRODUCER_PATH, "Builds run267EE balance/time-slice/trade-quality review."),
        ("stage267_run267EE_source_execution", "source_execution_result", SOURCE_EXECUTION_RESULT_PATH, "Source run267ED execution result."),
        ("stage267_run267EE_trade_records", "trade_records", TRADE_RECORDS_PATH, "Parsed trade records."),
        ("stage267_run267EE_time_slice_kpi", "time_slice_kpi", TIME_SLICE_KPI_PATH, "Time-slice KPI."),
        ("stage267_run267EE_curve_diagnostics", "curve_diagnostics", CURVE_DIAGNOSTICS_PATH, "Curve diagnostics."),
        ("stage267_run267EE_candidate_profile_review", "candidate_profile_review", CANDIDATE_PROFILE_REVIEW_PATH, "Candidate profile review."),
        ("stage267_run267EE_attempt_outcome_review", "attempt_outcome_review", ATTEMPT_OUTCOME_REVIEW_PATH, "Attempt outcome review."),
        ("stage267_run267EE_negative_slices", "negative_slice_summary", NEGATIVE_SLICE_PATH, "Negative slice summary."),
        ("stage267_run267EE_attribution_summary", "performance_attribution_summary", ATTRIBUTION_SUMMARY_PATH, "Performance attribution summary."),
        ("stage267_run267EE_result_judgment", "result_judgment", RESULT_JUDGMENT_PATH, "Result judgment."),
        ("stage267_run267EE_review_result", "review_result", REVIEW_RESULT_PATH, "Review result payload."),
        ("stage267_run267EE_report", "review_report", REPORT_PATH, "User-facing review report."),
    )
    return [
        {
...
def update_ledgers(created_at: str, result: Mapping[str, Any]) -> None:
    ...
    stage_row = {
        "row_id": "stage267_run267EE_runtime_gap_aware_seventh_followup_or_prune_balance_timeslice_trade_quality_review",
        "stage_id": STAGE_ID,
        "run_id": RUN_ID,
        "view": "runtime_gap_aware_seventh_followup_or_prune_balance_timeslice_trade_quality_review",
    ...
    run_row = {
        "run_id": RUN_ID,
        "stage_id": STAGE_ID,
        "lane": "runtime_gap_aware_seventh_followup_or_prune_balance_timeslice_trade_quality_review",
        "status": result["status"],
        "judgment": "review_completed_no_candidate_selection",
    ...
    project_row = {
        ...
        "record_view": "runtime_gap_aware_seventh_followup_or_prune_balance_timeslice_trade_quality_review",
        "tier_scope": "Tier A completed rows",
        "kpi_scope": "trade_curve_time_slice_trade_quality",
        "scoreboard_lane": "runtime_gap_aware_seventh_followup_or_prune_review",
        "status": result["status"],
        "judgment": "review_completed_no_candidate_selection",
        "path": rel(REPORT_PATH),
...
def update_stage267_workspace_block(text: str, *, status: str, next_action: str) -> str:
    report_entry = f"  run267EE_runtime_gap_aware_seventh_followup_or_prune_balance_timeslice_trade_quality_review_report_path: {rel(REPORT_PATH)}"
    lines = text.splitlines()
    output: list[str] = []
    in_stage267 = False
...
def update_current_docs(result: Mapping[str, Any]) -> None:
    ...
    report_line = (
        ...
        f"(267EE 런타임 공백 반영 7차 후속/가지치기 잔액/시간구간/거래품질 검토): `{rel(REPORT_PATH)}`"
    )
    block = "\n".join(
        [
            ...
            f"Effect(효과): candidate_profile_rows(후보-프로필 행) `{len(result['candidate_profile_review'])}`, negative_slices(음수 구간) `{len(result['negative_slices'])}`를 만들었고, 다음은 eighth follow-up/prune design(8차 후속/가지치기 설계)이다.",
    ...
    for path in (CURRENT_WORKING_STATE_PATH, SELECTION_STATUS_PATH, REVIEW_INDEX_PATH):
        ...
        if path == CURRENT_WORKING_STATE_PATH:
            ...
            text = replace_line_containing(text, "- adapter_under_review(", "- adapter_under_review(검토 중 어댑터): `runtime_gap_aware_seventh_followup_or_prune_balance_timeslice_trade_quality_review`")
            text = replace_line_containing(text, "- next_action(", f"- next_action(다음 행동): `{next_action}`")
            text = append_after_contains(text, "stage267_run267ED_runtime_gap_aware_seventh_followup_or_prune_mt5_execution.md", report_line)
            ...
            text = replace_line_containing(text, "- next_action(", f"- next_action(다음 행동): `{next_action}`")
            text = append_after_contains(text, "stage267_run267ED_runtime_gap_aware_seventh_followup_or_prune_mt5_execution", report_line)
        else:
            text = replace_line_containing(text, "- status(", f"- status(상태): `{status}`")
            text = replace_line_containing(text, "- current_run(", f"- current_run(현재 실행): `{RUN_ID}`")
            text = replace_line_containing(text, "- last_completed_run(", f"- last_completed_run(마지막 완료 실행): `{RUN_ID}`")
            text = append_after_contains(text, "stage267_run267ED_runtime_gap_aware_seventh_followup_or_prune_mt5_execution.md", report_line)
        text = append_block_once(text, "Run267EE(267EE 실행)는 run267ED", block)
    ...
    focus = (
        "- >-\n"
        f"  Stage267(267단계) run267EE(267EE 실행) runtime gap aware seventh follow-up/prune balance/time-slice/trade-quality review"
        f"(런타임 공백 반영 7차 후속/가지치기 잔액/시간구간/거래품질 검토) `{status}`. "
        f"Effect(효과): run267ED(267ED 실행)의 candidate_profile_rows(후보-프로필 행) `{len(result['candidate_profile_review'])}`와 negative_slices(음수 구간) `{len(result['negative_slices'])}`를 만들었고, "
