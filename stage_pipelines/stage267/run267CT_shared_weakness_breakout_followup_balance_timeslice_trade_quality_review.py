...
from stage_pipelines.stage267 import (
    run267CS_shared_weakness_breakout_followup_mt5_executor as source_executor,
)
...
RUN_NUMBER = "run267CT"
RUN_ID = "run267CT_stage267_shared_weakness_breakout_followup_balance_timeslice_trade_quality_review_v1"
STATUS = "run267CT_shared_weakness_breakout_followup_balance_timeslice_trade_quality_review_completed"
PARTIAL_STATUS = "run267CT_shared_weakness_breakout_followup_balance_timeslice_trade_quality_review_partial_parser_errors"
NEXT_ACTION = "run267CU_design_shared_weakness_breakout_followup_or_prune_from_run267CT_review"
NEXT_ACTION_PARTIAL = "run267CT_repair_shared_weakness_breakout_followup_trade_parser_errors"
...
RUN_ROOT = STAGE_ROOT / "02_runs" / RUN_NUMBER / "shared_weakness_breakout_followup_balance_timeslice_trade_quality_review"
...
REPORT_PATH = REVIEWS_ROOT / "stage267_run267CT_shared_weakness_breakout_followup_balance_timeslice_trade_quality_review.md"
PRODUCER_PATH = Path("stage_pipelines/stage267/run267CT_shared_weakness_breakout_followup_balance_timeslice_trade_quality_review.py")
        ...
        output.append(
            {
                ...
                "likely_drivers": "run267CR added noncalendar state-phase, shock-supply, or stress-blast feature on top of run267CN surface",
...
def next_probe_for_row(row: Mapping[str, Any], weakest: Mapping[str, Any]) -> str:
    ...
    if profile == "aggressive_shock_supply_expansion" and trades < 300:
    ...
    if profile == "redzone_stress_blast":
        return "redzone_stress_survived_first_pass_but_needs_curve_zoom(고위험 압박 1차 생존, 곡선 확대 필요)"
...
def build_result_judgment(result: Mapping[str, Any]) -> list[dict[str, Any]]:
    return [
        {
            "result_subject": "run267CT shared weakness follow-up balance/time-slice/trade-quality review(267CT 공유 약점 후속 잔액/시간구간/거래품질 검토)",
...
def artifact_rows(created_at: str) -> list[dict[str, Any]]:
    entries = (
        ("stage267_run267CT_producer", "producer_script", PRODUCER_PATH, "Builds run267CT balance/time-slice/trade-quality review."),
        ("stage267_run267CT_trade_records", "trade_records", TRADE_RECORDS_PATH, "Parsed trade records."),
        ("stage267_run267CT_time_slice_kpi", "time_slice_kpi", TIME_SLICE_KPI_PATH, "Time-slice KPI."),
        ("stage267_run267CT_curve_diagnostics", "curve_diagnostics", CURVE_DIAGNOSTICS_PATH, "Curve diagnostics."),
        ("stage267_run267CT_candidate_profile_review", "candidate_profile_review", CANDIDATE_PROFILE_REVIEW_PATH, "Candidate-profile review."),
        ("stage267_run267CT_candidate_summary", "candidate_summary", CANDIDATE_SUMMARY_PATH, "Candidate summary."),
        ("stage267_run267CT_profile_axis_summary", "profile_axis_summary", PROFILE_AXIS_SUMMARY_PATH, "Profile axis summary."),
        ("stage267_run267CT_negative_slice", "negative_slice_summary", NEGATIVE_SLICE_PATH, "Negative slice summary."),
        ("stage267_run267CT_attribution", "performance_attribution_summary", ATTRIBUTION_SUMMARY_PATH, "Performance attribution."),
        ("stage267_run267CT_result_judgment", "result_judgment", RESULT_JUDGMENT_PATH, "Result judgment."),
        ("stage267_run267CT_parser_checks", "parser_checks", PARSER_CHECKS_PATH, "Parser checks."),
        ("stage267_run267CT_review_result", "review_result", REVIEW_RESULT_PATH, "Review result payload."),
        ("stage267_run267CT_report", "review_report", REPORT_PATH, "User-facing review report."),
    )
    return [
        {
...
def update_ledgers(created_at: str, result: Mapping[str, Any]) -> None:
    ...
    stage_row = {
        "row_id": "stage267_run267CT_shared_weakness_breakout_followup_balance_timeslice_trade_quality_review",
        "stage_id": STAGE_ID,
        "run_id": RUN_ID,
        "view": "shared_weakness_breakout_followup_balance_timeslice_trade_quality_review",
    ...
    run_row = {
        "run_id": RUN_ID,
        "stage_id": STAGE_ID,
        "lane": "shared_weakness_breakout_followup_balance_timeslice_trade_quality_review",
        "status": status,
        "judgment": "review_completed_no_candidate_selection",
    ...
    project_row = {
        "ledger_row_id": f"{RUN_ID}__shared_weakness_breakout_followup_balance_timeslice_trade_quality_review",
        "stage_id": STAGE_ID,
        "run_id": RUN_ID,
        "subrun_id": "shared_weakness_breakout_followup_balance_timeslice_trade_quality_review",
        "parent_run_id": SOURCE_RUN_ID,
        "record_view": "shared_weakness_breakout_followup_balance_timeslice_trade_quality_review",
...
def update_current_docs(result: Mapping[str, Any]) -> None:
    ...
    report_line = (
        "- run267CT_shared_weakness_breakout_followup_balance_timeslice_trade_quality_review"
        f"(267CT 공유 약점 후속 잔액/시간구간/거래품질 검토): `{rel(REPORT_PATH)}`"
    )
    latest_line = (
        f"- latest_review(최신 검토): run267CT(267CT 실행) candidate_profile_rows(후보-프로필 행) "
    ...
    current = replace_line_prefix(
        ...
        "- adapter_under_review(검토 중 어댑터): `shared_weakness_breakout_followup_balance_timeslice_trade_quality_review`",
    )
    current = replace_line_prefix(current, "- status(상태):", f"- status(상태): `{status}`")
    current = replace_line_prefix(current, "- next_action(다음 행동):", f"- next_action(다음 행동): `{next_action}`")
    current = append_after_contains(current, "stage267_run267CS_shared_weakness_breakout_followup_mt5_execution.md", report_line)
    current = append_after_contains(current, "## Current Next Action", latest_line)
    current = append_block_once(current, "Run267CT(267CT 실행)는 run267CS", block)
    ...
    selection = append_after_contains(selection, "run267CS_shared_weakness_breakout_followup_mt5_execution", report_line)
    selection = append_block_once(selection, "Run267CT(267CT 실행)는 run267CS", block)
    review_base.write_md(SELECTION_STATUS_PATH, selection)

    review_index = io_path(REVIEW_INDEX_PATH).read_text(encoding="utf-8-sig")
    review_index = append_after_contains(review_index, "run267CS_shared_weakness_breakout_followup_mt5_execution", report_line)
    review_index = append_block_once(review_index, "Run267CT(267CT 실행)는 run267CS", block)
    ...
    workspace = append_after_contains(
        workspace,
        "run267CS_shared_weakness_breakout_followup_mt5_execution_report_path",
        f"  run267CT_shared_weakness_breakout_followup_balance_timeslice_trade_quality_review_report_path: {rel(REPORT_PATH)}",
    )
    focus_line = (
        "- >-\n"
        f"  Stage267(267단계) run267CT(267CT 실행) shared weakness breakout follow-up balance/time-slice/trade-quality review"
        f"(공유 약점 돌파 후속 잔액/시간구간/거래품질 검토) `{status}`. "
        f"Effect(효과): run267CS(267CS 실행)의 MT5(MetaTrader 5, 메타트레이더5) report(보고서)를 거래 목록과 약한 구간으로 다시 읽었고, "
