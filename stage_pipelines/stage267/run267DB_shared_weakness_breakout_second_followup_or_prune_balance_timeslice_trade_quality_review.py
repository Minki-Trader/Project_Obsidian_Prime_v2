...
from stage_pipelines.stage267 import (
    run267DA_shared_weakness_breakout_second_followup_or_prune_mt5_executor as source_executor,
)
...
RUN_NUMBER = "run267DB"
RUN_ID = "run267DB_stage267_shared_weakness_breakout_second_followup_or_prune_balance_timeslice_trade_quality_review_v1"
STATUS = "run267DB_shared_weakness_breakout_second_followup_or_prune_balance_timeslice_trade_quality_review_completed"
PARTIAL_STATUS = "run267DB_shared_weakness_breakout_second_followup_or_prune_balance_timeslice_trade_quality_review_partial_parser_errors"
NEXT_ACTION = "run267DC_design_shared_weakness_breakout_second_followup_or_prune_from_run267DB_review"
NEXT_ACTION_PARTIAL = "run267DB_repair_shared_weakness_breakout_second_followup_or_prune_trade_parser_errors"
...
REPORT_PATH = REVIEWS_ROOT / "stage267_run267DB_shared_weakness_breakout_second_followup_or_prune_balance_timeslice_trade_quality_review.md"
PRODUCER_PATH = Path("stage_pipelines/stage267/run267DB_shared_weakness_breakout_second_followup_or_prune_balance_timeslice_trade_quality_review.py")
        ...
        output.append(
            {
                ...
                "likely_drivers": "run267DA(267DA 실행)가 run267CZ(267CZ 실행)의 second follow-up/prune(2차 후속/가지치기) queue(대기열)를 MT5(MetaTrader 5, 메타트레이더5) report(보고서)와 KPI(핵심 성과 지표)로 물질화했다.",
...
def build_result_judgment(result: Mapping[str, Any]) -> list[dict[str, Any]]:
    return [
        {
            "result_subject": "run267DB shared weakness second follow-up/prune balance/time-slice/trade-quality review(267DB 공유 약점 2차 후속/가지치기 잔액/시간구간/거래품질 검토)",
...
def artifact_rows(created_at: str) -> list[dict[str, Any]]:
    entries = (
        ("stage267_run267DB_producer", "producer_script", PRODUCER_PATH, "Builds run267DB balance/time-slice/trade-quality review."),
        ("stage267_run267DB_trade_records", "trade_records", TRADE_RECORDS_PATH, "Parsed trade records."),
        ("stage267_run267DB_time_slice_kpi", "time_slice_kpi", TIME_SLICE_KPI_PATH, "Time-slice KPI."),
        ("stage267_run267DB_curve_diagnostics", "curve_diagnostics", CURVE_DIAGNOSTICS_PATH, "Curve diagnostics."),
        ("stage267_run267DB_candidate_profile_review", "candidate_profile_review", CANDIDATE_PROFILE_REVIEW_PATH, "Candidate-profile review."),
        ("stage267_run267DB_candidate_summary", "candidate_summary", CANDIDATE_SUMMARY_PATH, "Candidate summary."),
        ("stage267_run267DB_profile_axis_summary", "profile_axis_summary", PROFILE_AXIS_SUMMARY_PATH, "Profile axis summary."),
        ("stage267_run267DB_negative_slice", "negative_slice_summary", NEGATIVE_SLICE_PATH, "Negative slice summary."),
        ("stage267_run267DB_attribution", "performance_attribution_summary", ATTRIBUTION_SUMMARY_PATH, "Performance attribution."),
        ("stage267_run267DB_result_judgment", "result_judgment", RESULT_JUDGMENT_PATH, "Result judgment."),
        ("stage267_run267DB_parser_checks", "parser_checks", PARSER_CHECKS_PATH, "Parser checks."),
        ("stage267_run267DB_review_result", "review_result", REVIEW_RESULT_PATH, "Review result payload."),
        ("stage267_run267DB_report", "review_report", REPORT_PATH, "User-facing review report."),
    )
    return [
        {
...
def update_ledgers(created_at: str, result: Mapping[str, Any]) -> None:
    ...
    stage_row = {
        "row_id": "stage267_run267DB_shared_weakness_breakout_second_followup_or_prune_balance_timeslice_trade_quality_review",
        "stage_id": STAGE_ID,
        "run_id": RUN_ID,
...
def update_current_docs(result: Mapping[str, Any]) -> None:
    ...
    report_line = (
        ...
        f"(267DB 공유 약점 후속/가지치기 잔액/시간구간/거래품질 검토): `{rel(REPORT_PATH)}`"
    )
    latest_line = (
        f"- latest_review(최신 검토): run267DB(267DB 실행) candidate_profile_rows(후보-프로필 행) "
    ...
    current = append_after_contains(current, "stage267_run267DA_shared_weakness_breakout_second_followup_or_prune_mt5_execution.md", report_line)
    current = append_after_contains(current, "## Current Next Action", latest_line)
    current = append_block_once(current, "Run267DB(267DB 실행)는 run267DA", block)
    ...
    selection = append_after_contains(selection, "run267DA_shared_weakness_breakout_second_followup_or_prune_mt5_execution", report_line)
    selection = append_block_once(selection, "Run267DB(267DB 실행)는 run267DA", block)
    ...
    review_index = append_after_contains(review_index, "run267DA_shared_weakness_breakout_second_followup_or_prune_mt5_execution", report_line)
    review_index = append_block_once(review_index, "Run267DB(267DB 실행)는 run267DA", block)
    ...
    workspace = append_after_contains(
        workspace,
        "run267DA_shared_weakness_breakout_second_followup_or_prune_mt5_execution_report_path",
        f"  run267DB_shared_weakness_breakout_second_followup_or_prune_balance_timeslice_trade_quality_review_report_path: {rel(REPORT_PATH)}",
    )
    focus_line = (
        "- >-\n"
        f"  Stage267(267단계) run267DB(267DB 실행) shared weakness breakout second follow-up/prune balance/time-slice/trade-quality review"
        f"(공유 약점 돌파 후속/가지치기 잔액/시간구간/거래품질 검토) `{status}`. "
        f"Effect(효과): run267DA(267DA 실행)의 MT5(MetaTrader 5, 메타트레이더5) report(보고서)를 거래 목록과 약한 구간으로 다시 읽었고, "
