...
SOURCE_STAGE59J_ID = "59J_adapter_repair__new_model_branch_from_stage59i"
STAGE59K_ID = "59K_adapter_repair__bounded_followup_from_stage59j"
NEXT_REPAIR_STAGE_ID = "59L_adapter_repair__bounded_followup_from_stage59k"
RUN_NUMBER = "run59F"
RUN_ID = "run59F_stage59k_bounded_followup_from_stage59j_v1"
PACKET_ID = "stage59k_bounded_followup_from_stage59j_v1"
PARENT_RUN_ID = "run59E_stage59j_new_model_branch_from_stage59i_v1"
SOURCE_ADAPTER_ID = "s59j_v62_trn_h4_sd5"
DEVELOPMENT_ANCHOR = "v64_v47_ctxgap14_refill_etfw_h2_no_b"
BACKUP_ANCHOR = "v60_v47_et_stable_damage_firewall_h2c0_no_b"
SOURCE_STAGE59J_PUSHED_COMMIT = "270d8958387fdd4390b7ac93734066027d9488a7"
BOUNDARY = (
    "research_development_only_no_live_readiness_no_runtime_authority_"
    "no_operating_promotion_no_operating_reference_no_production_baseline_no_deployment"
)

STAGE_ROOT = Path("stages") / STAGE59K_ID
...
SOURCE_STAGE59J_ROOT = Path("stages") / SOURCE_STAGE59J_ID
SOURCE_STAGE59J_DECISION = SOURCE_STAGE59J_ROOT / "03_reviews/stage59j_decision.md"
SOURCE_STAGE59J_REPORT = SOURCE_STAGE59J_ROOT / "03_reviews/new_model_branch_from_stage59i_report.md"
SOURCE_STAGE59J_SUMMARY = SOURCE_STAGE59J_ROOT / "03_reviews/new_model_branch_summary.csv"
SOURCE_STAGE59J_JSON = SOURCE_STAGE59J_ROOT / "03_reviews/new_model_branch_summary.json"
...
COMMON_ROOT = f"Project_Obsidian_Prime_v2/s59k/{RUN_NUMBER}"

REPORT_PATH = REVIEWS_ROOT / "bounded_followup_from_stage59j_report.md"
...
DECISION_PATH = REVIEWS_ROOT / "stage59k_decision.md"
AUDIT_CSV_PATH = REVIEWS_ROOT / "stage59k_trade_audit.csv"
...
STAGE59K_VARIANTS = (
    repair.RepairVariant(
        adapter_id="s59k_v62_th60_sd5",
        label="run50BQ_v62_transition_h4_threshold60_risk3pct_wide_atr_sd5",
        atr_enabled=True,
        ...
        max_hold_bars=4,
        notes="Stage59K bounded follow-up: keep Stage59J v62 transition hold4 source and raise threshold to 0.60 to test validation cost/PF repair.",
    ),
    repair.RepairVariant(
        adapter_id="s59k_v62_th62_sd5",
        label="run50BQ_v62_transition_h4_threshold62_risk3pct_wide_atr_sd5",
        atr_enabled=True,
        model_risk_enabled=True,
        fixed_lot=0.25,
        atr_stop_multiplier=2.5,
        ...
        close_only_on_opposite_signal=True,
        max_hold_bars=4,
        notes="Stage59K bounded follow-up: keep Stage59J v62 transition hold4 source and raise threshold to 0.62 to test stronger confidence pressure.",
    ),
    repair.RepairVariant(
        adapter_id="s59k_v62_th60_rearm02_sd5",
        label="run50BQ_v62_transition_h4_threshold60_rearm02_risk3pct_wide_atr_sd5",
        atr_enabled=True,
        model_risk_enabled=True,
        fixed_lot=0.25,
        atr_stop_multiplier=2.5,
        ...
        close_only_on_opposite_signal=True,
        max_hold_bars=4,
        notes="Stage59K bounded follow-up: keep Stage59J v62 transition hold4 source, threshold 0.60, and require 0.02 rearm confidence delta.",
    ),
)
...
def source_specs() -> dict[str, dict[str, Any]]:
    ...
    return {variant.adapter_id: dict(v62) for variant in STAGE59K_VARIANTS}


def configure_reused_engine() -> None:
    engine.STAGE59_ID = STAGE59K_ID
    ...
    engine.STAGE59_VARIANTS = STAGE59K_VARIANTS
    engine.SOURCE_SPECS = source_specs()
    engine.MODEL_RISK_MIN_PCT = {variant.adapter_id: 0.005 for variant in STAGE59K_VARIANTS}

    repair.STAGE_ID = STAGE59K_ID
    repair.RUN_NUMBER = RUN_NUMBER
    repair.RUN_ID = RUN_ID
    repair.RUN_ROOT = RUN_ROOT
    repair.REPAIR_VARIANTS = STAGE59K_VARIANTS
    s58.STAGE58_ID = STAGE59K_ID
    ...
    s58.STAGE58_VARIANTS = STAGE59K_VARIANTS
...
def build_attempts(inputs: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    for variant_index, variant in enumerate(STAGE59K_VARIANTS, start=1):
            ...
            for role_index, (tier, attempt_role, prefix, attempt_token) in enumerate(
                ...
                attempts.append(
                    attempt_payload(
                        ...
                        run_id=RUN_ID,
                        stage_number=59,
                        exploration_label="stage59K_BaselineAdapter__BoundedFollowupFromStage59J",
...
def execute_prepared_run_checkpointed(prepared: Mapping[str, Any], args: argparse.Namespace) -> dict[str, Any]:
    ...
    return {
        **dict(prepared),
        ...
        "judgment": "inconclusive_stage59k_runtime_probe_completed" if completed and report_completed else "blocked_stage59k_runtime_probe",
    }
...
def next_stage_for_decision(decision: str) -> str:
    ...
    if decision == "demote_current_adapter_and_select_backup":
        return "59L_adapter_repair__backup_or_demote_from_stage59k"
    if decision == "open_bounded_followup":
        return "59L_adapter_repair__bounded_followup_from_stage59k"
...
def next_packet_for_decision(decision: str) -> tuple[str, str]:
    ...
    return "stage59l_bounded_followup_from_stage59k_v1", "run59G_stage59l_bounded_followup_from_stage59k_v1"
            ...
            rows.append(
                {
                    ...
                    "stage_id": STAGE59K_ID,
                    "run_id": RUN_ID,
                    "artifact_type": "stage59k_bounded_followup_evidence",
                    "path": rel(path),
                    "sha256": sha256_file_lf_normalized(path),
                    "created_at_utc": created,
                    "notes": "Stage59K bounded follow-up from Stage59J artifact.",
                }
            )
            ...
            rows.append(
                {
                    ...
                    "created_at_utc": created,
                    "notes": "Actual Stage59K MT5 Strategy Tester HTML report.",
                }
            )
    return rows
...
def write_run_identity(result: Mapping[str, Any]) -> None:
    engine.write_json(
        RUN_ROOT / "run_manifest.json",
        {
            ...
            "bounded_question": "Can a bounded follow-up on the Stage59J v62 candidate repair validation PF and cost-stressed expectancy without starting ONNX?",
            "source_stage59j_decision": rel(SOURCE_STAGE59J_DECISION),
            "source_stage59j_pushed_commit": SOURCE_STAGE59J_PUSHED_COMMIT,
            "variants": [
                {
                    **variant.__dict__,
                ...
                for variant in STAGE59K_VARIANTS
            ],
...
def write_ledgers(
        ...
        [
            {
                "run_id": RUN_ID,
                "stage_id": STAGE59K_ID,
                "lane": "baseline_adapter_bounded_followup_from_stage59j",
                "status": status,
                "judgment": decision,
    ...
    ledger_rows.append(
        {
            "ledger_row_id": f"{RUN_ID}__aggregate_bounded_followup_from_stage59j",
            "stage_id": STAGE59K_ID,
            "run_id": RUN_ID,
            "subrun_id": "aggregate_bounded_followup_from_stage59j",
            "parent_run_id": PARENT_RUN_ID,
            "record_view": "bounded_followup_from_stage59j",
            "tier_scope": "Tier A+B",
            ...
            "external_verification_status": external,
            "notes": "Stage59K bounded follow-up from Stage59J transition evidence; not final package completion.",
        }
    )
...
def report_markdown(summary_rows: Sequence[Mapping[str, Any]], decision: str, reasons: Sequence[str], external: str) -> str:
    ...
    best = best_repaired_variant(summary_rows)
    return f"""# Stage59K Bounded Followup From Stage59J Report(59K단계 59J단계 기반 경계 후속 보고서)

- stage(단계): `{STAGE59K_ID}`
- run(실행): `{RUN_ID}`
...
def equity_audit_markdown(summary_rows: Sequence[Mapping[str, Any]], segment_rows: Sequence[Mapping[str, Any]]) -> str:
    ...
    return f"""# Stage59K Bounded Followup Equity Curve Audit(59K단계 경계 후속 자금 곡선 감사)
...
def decision_markdown(decision: str, reasons: Sequence[str], best: Mapping[str, Any], external: str) -> str:
    return f"""# Stage59K Decision(59K단계 판정)

decision(판정): `{decision}`

Stage59K(59K단계)는 Stage59J v62 evidence(59J단계 v62 근거)를 사용한 bounded follow-up(경계 후속)으로 기록한다. Effect(효과): threshold/rearm(문턱값/재무장) 압박의 성공/실패를 다음 bounded stage(경계 다음 단계)의 입력 근거로 넘긴다.
...
Stage59K closeout(59K단계 종료)는 overall goal completion(전체 목표 완료)이 아니다. Effect(효과): Stage60 ONNX hardening(60단계 ONNX 경화)은 adapter quality(어댑터 품질)가 강할 때만 열린다.
    ...
    engine.write_json(
        SUMMARY_JSON_PATH,
        {
            ...
            "required_outputs": {
                ...
                "stage59k_decision": rel(DECISION_PATH),
            },
...
def write_packet_files(
    ...
    files = {
        ...
        "experiment_design_receipt.json": {
            "hypothesis": "A bounded threshold/rearm follow-up on the Stage59J v62 candidate may improve validation PF and cost-stressed expectancy while preserving OOS behavior.",
            ...
            "comparison_baseline": "Stage59J best candidate s59j_v62_trn_h4_sd5.",
            ...
            "changed_variables": ["short_threshold", "long_threshold", "entry_transition_rearm_min_confidence_delta"],
            ...
            "stop_conditions": "three bounded Stage59K threshold/rearm variants only",
        ...
        "artifact_lineage_audit.json": {
            "source_inputs": [rel(SOURCE_STAGE59J_DECISION), rel(SOURCE_STAGE59J_REPORT), rel(SOURCE_STAGE59J_SUMMARY), rel(SOURCE_STAGE59J_JSON)],
...
- stage_id(단계 ID): `{STAGE59K_ID}`
- source_stage(원천 단계): `{SOURCE_STAGE59J_ID}`
- source_decision(원천 판정): `continue_repair_in_new_bounded_stage`
- bounded_question(경계 질문): `Can a bounded follow-up on the Stage59J v62 candidate repair validation PF and cost-stressed expectancy without starting ONNX?`
- boundary(경계): `{BOUNDARY}`

Stage59K(59K단계)는 Stage59J(59J단계) v62 source(v62 원천)를 유지하고 threshold/rearm(문턱값/재무장) 세 변형만 측정한다. Effect(효과): Stage60 ONNX(60단계 ONNX)나 deployment(배포)로 이어가지 않고 검증 약점 수리 여부만 본다.
""",
    )
    engine.write_md(
...
- stage59j_decision(59J단계 판정): `{rel(SOURCE_STAGE59J_DECISION)}`
- stage59j_report(59J단계 보고서): `{rel(SOURCE_STAGE59J_REPORT)}`
- stage59j_summary(59J단계 요약): `{rel(SOURCE_STAGE59J_SUMMARY)}`
- stage59j_pushed_commit(59J단계 푸시 커밋): `{SOURCE_STAGE59J_PUSHED_COMMIT}`
...
Effect(효과): Stage59K(59K단계)는 Stage59J(59J단계) 최선 후보와 run50BQ(run50BQ) v62 전환 원천을 명시적으로 연결한다.
""",
    )
    engine.write_md(
...
- stage_status(단계 상태): `closed_bounded_followup_from_stage59j`
- source_stage(원천 단계): `{SOURCE_STAGE59J_ID}`
- source_decision(원천 판정): `continue_repair_in_new_bounded_stage`
- stage59k_decision(59K단계 판정): `{decision}`
- next_stage_or_branch(다음 단계/분기): `{next_stage}`
- selected_research_baseline(선택 연구 기준선): `none`
- claim_boundary(주장 경계): `{BOUNDARY}`

Effect(효과): Stage59K(59K단계)는 측정 결과를 보존하지만 final package(최종 패키지)나 operating claim(운영 주장)을 만들지 않는다.
""",
    )
    engine.write_md(
        REVIEWS_ROOT / "review_index.md",
        f"""# Stage59K Review Index(59K단계 검토 색인)
...
- bounded_followup_risk_atr_telemetry(경계 후속 위험/ATR 텔레메트리): `{rel(RISK_ATR_TELEMETRY_PATH)}`
- stage59k_decision(59K단계 판정): `{rel(DECISION_PATH)}`
""",
    )
    if not next_stage.startswith("60_"):
        ...
        engine.write_md(
            ...
            f"""# Stage59L Brief(59L단계 개요)

- stage_id(단계 ID): `{next_stage}`
- source_stage(원천 단계): `{STAGE59K_ID}`
- source_decision(원천 판정): `{decision}`
- bounded_question(경계 질문): `What bounded follow-up should repair, demote, or branch from the Stage59K evidence without starting ONNX prematurely?`
- boundary(경계): `{BOUNDARY}`

Stage59L(59L단계)는 Stage59K(59K단계) 결과가 Stage60 ONNX(60단계 ONNX)에 충분하지 않을 때 이어지는 bounded follow-up(경계 후속)이다. Effect(효과): repair(수리), demotion(강등), branch(분기)를 다음 작은 질문으로 제한한다.
""",
        )
        engine.write_md(
            next_root / "01_inputs/input_refs.md",
            f"""# Stage59L Input References(59L단계 입력 참조)

- stage59k_decision(59K단계 판정): `{rel(DECISION_PATH)}`
- stage59k_report(59K단계 보고서): `{rel(REPORT_PATH)}`
- stage59k_summary(59K단계 요약): `{rel(SUMMARY_CSV_PATH)}`
- stage59k_segment_kpi(59K단계 구간 KPI): `{rel(SEGMENT_KPI_PATH)}`
- stage59k_risk_atr_telemetry(59K단계 위험/ATR 텔레메트리): `{rel(RISK_ATR_TELEMETRY_PATH)}`
""",
        )
        engine.write_md(
            next_root / "03_reviews/review_index.md",
            """# Stage59L Review Index(59L단계 검토 색인)

Stage59L(59L단계)는 planned(계획) 상태다. Effect(효과): Stage59K(59K단계) 약점을 다음 bounded repair(경계 수리) 근거로 넘긴다.
""",
        )
        engine.write_md(
            next_root / "04_selected/selection_status.md",
            f"""# Stage59L Selection Status(59L단계 선택 상태)

- stage_status(단계 상태): `active_planned_from_stage59k`
- source_stage(원천 단계): `{STAGE59K_ID}`
- source_decision(원천 판정): `{decision}`
- selected_research_baseline(선택 연구 기준선): `none`
- claim_boundary(주장 경계): `{BOUNDARY}`

Effect(효과): Stage59L(59L단계)는 Stage59K(59K단계) 이후 필요한 다음 작은 질문만 다룬다.
""",
        )
...
- status(상태): `stage59k_closed_{decision}`
...
""",
    ...
    focus = (
        "current_focus:\n"
        f"- >-\n"
        f"  Stage59K(59K단계) `{STAGE59K_ID}` closed(종료) as bounded follow-up from Stage59J(59J단계 기반 경계 후속); decision(판정)=`{decision}`. "
        ...
        f"- >-\n"
        f"  Next stage_or_branch(다음 단계/분기) `{next_stage}` is active/planned(활성/계획). Effect(효과): Stage59K(59K단계) 결과를 다음 bounded step(경계 다음 단계)으로 넘긴다.\n"
    )
    text = re.sub(
...
stage59k_bounded_followup_from_stage59j:
  packet_id: {PACKET_ID}
  stage_id: {STAGE59K_ID}
  status: closed_bounded_followup_from_stage59j
  ...
  boundary: {BOUNDARY}
"""
    if "stage59k_bounded_followup_from_stage59j:" in text:
        text = re.sub(r"\nstage59k_bounded_followup_from_stage59j:\n(?:  .*\n)*", block, text, count=1)
    else:
        text += block
...
def parse_args(argv: Sequence[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run Stage59K bounded follow-up from Stage59J.")
...
def load_existing_result() -> dict[str, Any]:
    ...
    if not path_exists(manifest) or not path_exists(kpi):
        raise FileNotFoundError("Stage59K existing run_manifest.json or kpi_record.json is missing")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.finalize_existing:
        ...
        prepared = {
            ...
            "model_family": "baseline_adapter_stage59k_bounded_followup_ebm_table",
            "feature_set_id": "stage59k_v62_threshold_rearm_discrete_signal",
