...
from stage_pipelines.stage339 import execute_shorter_hold_side_balance_trade_count_expansion_mt5_probe_without_db as ebase
from stage_pipelines.stage339 import materialize_quality_balance_blend_mt5_probe_package_without_db as pkg


TODAY = "2026-06-01"
RUN_NUMBER = "run339G"
RUN_ID = "run339G_execute_quality_balance_blend_mt5_probe_without_db_v1"
PARENT_RUN_ID = pkg.RUN_ID
NEXT_RUN_ID = "run339H_review_quality_balance_blend_mt5_probe_without_db_v1"
STATUS_COMPLETED = "completed_stage339G_quality_balance_blend_mt5_probe_executed_review_required_no_selection"
STATUS_BLOCKED = "blocked_stage339G_quality_balance_blend_mt5_probe_attempt_recorded_repair_required_no_selection"
JUDGMENT_COMPLETED = "mt5_quality_balance_blend_probe_outputs_available_review_required_no_selection"
JUDGMENT_BLOCKED = "mt5_quality_balance_blend_probe_attempt_recorded_but_outputs_missing_or_failed_repair_required"
DECISION_COMPLETED = "stage339G_open_run339H_review_quality_balance_blend_probe"
DECISION_BLOCKED = "stage339G_open_run339H_review_or_repair_quality_balance_blend_probe"
CLAIM_BOUNDARY = (
    "research_development_quality_balance_blend_mt5_runtime_probe_attempt_only_no_candidate_selection_"
...
REPORT_PATH = REVIEW_DIR / "run339G_mt5_probe.md"
DECISION_DOC = ROOT / "docs" / "decisions" / f"{TODAY}_stage339G_mt5_probe.md"
...
EXECUTION_SUMMARY = RUN_DIR / "quality_balance_blend_mt5_probe_summary.csv"
...
def write_docs(final: Mapping[str, Any]) -> None:
    report = f"""# run339G Quality Balance Blend MT5 Probe(품질-균형 혼합 MT5 탐침)
...
"""
    decision = f"""# {TODAY} Stage339G Decision(339G 결정)
...
Action(행동): quality-balance blend(품질-균형 혼합) 변형을 MT5 runtime probe(MT5 런타임 탐침)로 실행했다.

Effect(효과): run339H(339H 실행)가 KPI(핵심 성과 지표), trade count(거래수), side balance(방향 균형), proxy-MT5 diff(프록시-MT5 차이)를 검토할 수 있다.
...
run339G(339G 실행)는 MT5 runtime probe(MT5 런타임 탐침)를 실행했다. run339H(339H 실행)는 결과를 검토해서 quality-balance blend(품질-균형 혼합)가 c01/c07(씨01/씨07) 분기를 닫았는지 판단한다.
...
"""
    ...
    marker = f"run339G {RUN_ID}"
    ebase.append_text_once_direct(STAGE_BRIEF, marker, f"""## run339G Quality Balance Blend MT5 Probe(품질-균형 혼합 MT5 탐침)
...
""")
    ebase.append_text_once_direct(STAGE_README, marker, f"""## run339G Quality Balance Blend MT5 Probe(품질-균형 혼합 MT5 탐침)
...
""")
    changelog = f"""## {TODAY} run339G Quality Balance Blend MT5 Probe(품질-균형 혼합 MT5 탐침)

- action(행동): quality-balance blend(품질-균형 혼합) `{final['attempt_rows']}`개를 MT5 runtime probe(MT5 런타임 탐침)로 실행했다.
...
def make_gates(final: Mapping[str, Any]):
    ...
    return ebase.pd.DataFrame(
        [
            gate_row("parent_339F_gates_passed", "passed" if final["parent_gate_passed"] else "failed", ebase.rel(pkg.GATE_AUDIT), "run339F(339F 실행) package gate(패키지 게이트)를 이어받는다."),
            ...
            gate_row("exact_runtime_parity_reached", "passed" if final["matched_rows"] == final["expected_rows"] and final["mismatch_rows"] == 0 else "failed", ebase.rel(PROXY_MT5_DIFF), "expected tape(예상 테이프)와 MT5 telemetry(MT5 기록)의 동등성(parity, 동등성)을 확인한다."),
