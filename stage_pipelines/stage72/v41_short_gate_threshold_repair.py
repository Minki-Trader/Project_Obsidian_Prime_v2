...
STAGE_ID = "72_adapter_research__v41_source_repair_review"
RUN_NUMBER = "run72A"
RUN_ID = "run72A_stage72_v41_source_repair_review_v1"
PACKET_ID = "stage72_v41_source_repair_review_v1"
PARENT_RUN_ID = "run71A_stage71_new_model_branch_review_v1"
SOURCE_STAGE71_ID = "71_adapter_research__new_model_branch_review"
SOURCE_STAGE71_COMMIT = "c6e46abaf901b672289160ba0e5a9d7008fcf2f1"
NEXT_STAGE_ID = "73_adapter_research__v41_gate_repair_followup"
NEXT_RUN_ID = "run73A_stage73_v41_gate_repair_followup_v1"
NEXT_PACKET_ID = "stage73_v41_gate_repair_followup_v1"
...
COMMON_ROOT = f"OPV2/s72a/{RUN_NUMBER}"

SUMMARY_JSON_PATH = REVIEWS_ROOT / "stage72_v41_gate_repair_summary.json"
SUMMARY_CSV_PATH = REVIEWS_ROOT / "stage72_v41_gate_repair_summary.csv"
REPORT_PATH = REVIEWS_ROOT / "stage72_v41_gate_repair_report.md"
SEGMENT_KPI_PATH = REVIEWS_ROOT / "stage72_segment_kpi_summary.csv"
RISK_ATR_TELEMETRY_PATH = REVIEWS_ROOT / "stage72_risk_atr_telemetry.csv"
GATE_FEATURE_SUMMARY_PATH = REVIEWS_ROOT / "stage72_gate_feature_summary.csv"
TIER_B_DIAGNOSTIC_PATH = REVIEWS_ROOT / "stage72_tier_b_diagnostic_summary.csv"
DECISION_PATH = REVIEWS_ROOT / "stage72_decision.md"
AUDIT_CSV_PATH = REVIEWS_ROOT / "stage72_trade_audit.csv"
...
VARIANTS = (
    repair.RepairVariant(
        adapter_id="s72_v41_h3_risk45_gate04",
        label="run50BN_v41_h3_risk45_short_margin_gate04",
        atr_enabled=True,
        ...
        max_hold_bars=3,
        notes="Stage72 v41 short-gate repair: margin threshold 0.04, risk cap 4.5%, hold3.",
    ),
    repair.RepairVariant(
        adapter_id="s72_v41_h3_risk45_gate06",
        label="run50BN_v41_h3_risk45_short_margin_gate06",
        atr_enabled=True,
        model_risk_enabled=True,
        fixed_lot=0.25,
        atr_stop_multiplier=2.5,
        ...
        close_only_on_opposite_signal=True,
        max_hold_bars=3,
        notes="Stage72 v41 short-gate repair: margin threshold 0.06, risk cap 4.5%, hold3.",
    ),
    repair.RepairVariant(
        adapter_id="s72_v41_h3_risk5_gate06",
        label="run50BN_v41_h3_risk5_short_margin_gate06",
        atr_enabled=True,
        model_risk_enabled=True,
        fixed_lot=0.25,
        atr_stop_multiplier=2.5,
        ...
        close_only_on_opposite_signal=True,
        max_hold_bars=3,
        notes="Stage72 v41 short-gate repair: margin threshold 0.06, risk cap 5%, hold3.",
    ),
)
...
CONTEXT_GATE_SPECS = {
    "s72_v41_h3_risk45_gate04": {
        "gate_column": "stage72_gate_margin_lt_004_short",
        "gate_type": "margin",
        "threshold": 0.04,
        "block_mode": "short",
        "description": "v41 source; block shorts if et40_decision_margin < 0.04; risk cap 4.5%, hold 3",
    },
    "s72_v41_h3_risk45_gate06": {
        "gate_column": "stage72_gate_margin_lt_006_short",
        "gate_type": "margin",
        "threshold": 0.06,
        "block_mode": "short",
        "description": "v41 source; block shorts if et40_decision_margin < 0.06; risk cap 4.5%, hold 3",
    },
    "s72_v41_h3_risk5_gate06": {
        "gate_column": "stage72_gate_margin_lt_006_short",
        "gate_type": "margin",
        "threshold": 0.06,
        "block_mode": "short",
        "description": "v41 source; block shorts if et40_decision_margin < 0.06; risk cap 5%, hold 3",
    },
}
...
def decide(summary_rows: Sequence[Mapping[str, Any]], segment_rows: Sequence[Mapping[str, Any]], external: str) -> str:
    if external != "completed":
        return "continue_stage72_due_to_incomplete_runtime"
    ...
    if val_net >= 900 and oos_net >= 520 and val_dd <= 18.0 and oos_dd <= 19.0 and val_pf >= 1.50 and oos_pf >= 1.30 and not reasons:
        return "proceed_to_stage73_candidate_review"
    if val_net >= 650 and oos_net >= 420 and val_dd <= 24.0 and oos_dd <= 24.0 and val_pf >= 1.35 and oos_pf >= 1.25:
        return "continue_v41_gate_repair_in_stage73"
    if val_pf < 1.0 or val_net <= 0:
        return "open_another_model_source_branch_in_stage73"
    return "continue_v41_gate_repair_in_stage73"
...
def report_markdown(
    ...
    return f"""# Stage72 V41 Gate Repair Report(72단계 V41 게이트 수리 보고서)

- run(실행): `{RUN_ID}`
- target_surface(목표 표면): `{TARGET_SURFACE}`
- source_stage(원천 단계): `{SOURCE_STAGE71_ID}`
- source_stage71_commit(원천 71단계 커밋): `{SOURCE_STAGE71_COMMIT}`
...
def decision_markdown(decision: str, external: str) -> str:
    return f"""# Stage72 Decision(72단계 판정)
            ...
            rows.append(
                {
                    ...
                    "split": split_name,
                    "tier_b_policy": "diagnostic_missing_required_but_disabled_for_this_v41_gate_repair",
                    ...
                    "reason": "Stage72 isolates v41 gate repair first; Tier B fallback remains diagnostic and disabled.",
                }
            )
    return rows
            ...
            rows.append(
                {
                    ...
                    "artifact_type": "stage72_v41_gate_repair_evidence",
                    "path": rel(path),
                    "sha256": sha256_file_lf_normalized(path),
                    "stage_id": STAGE_ID,
                    "run_id": RUN_ID,
                    "created_at_utc": created,
                    "notes": "Stage72 v2-native v41 gate repair artifact.",
                }
            )
            ...
            rows.append(
                {
                    ...
                    "run_id": RUN_ID,
                    "created_at_utc": created,
                    "notes": "Actual Stage72 MT5 Strategy Tester HTML report.",
                }
            )
    return rows
...
def write_run_identity(result: Mapping[str, Any]) -> None:
    write_json(
        RUN_ROOT / "run_manifest.json",
        {
            ...
            "source_stage71_pushed_commit": SOURCE_STAGE71_COMMIT,
        ...
        [
            {
                "run_id": RUN_ID,
                "stage_id": STAGE_ID,
                "lane": "baseline_adapter_v2_native_v41_gate_repair",
                "status": status,
                "judgment": decision,
                ...
                "notes": ledger_pairs(
                    (
                        ...
                        ("source_stage71_commit", SOURCE_STAGE71_COMMIT),
                        ("target_surface", TARGET_SURFACE),
        ...
        alpha_rows = [
            {
                ...
                "tier_scope": "Tier A+B",
                "kpi_scope": "stage72_v41_gate_repair",
                ...
                "external_verification_status": external,
                "notes": "Stage72 run materialized or blocked before KPI records were available.",
            }
        ]
...
Stage73(73단계)는 Stage72(72단계)의 v41 gate repair(브이41 게이트 수리) 결과를 받아 candidate review(후보 검토), continued repair(계속 수리), 또는 another model source(다른 모델 원천)를 고르는 follow-up(후속) 단계다.

## Bounded Question(경계 질문)

Stage72(72단계)의 softer short gate(더 약한 숏 게이트)가 34D KPI(34D 핵심 성과 지표) 격차를 줄일 만큼 강한가?

Effect(효과): Stage73(73단계)는 다음 판단을 하나의 경계 질문으로 좁힌다.
...
""",
    ...
    write_md(
        ...
        f"""# Stage73 Input References(73단계 입력 참조)

- source_stage(원천 단계): `{STAGE_ID}`
- source_run(원천 실행): `{RUN_ID}`
- source_decision(원천 판정): `{decision}`
- source_external_verification_status(원천 외부 검증 상태): `{external}`
- stage72_report(72단계 보고서): `{rel(REPORT_PATH)}`
- stage72_decision(72단계 판정): `{rel(DECISION_PATH)}`
- summary(요약): `{rel(SUMMARY_CSV_PATH)}`
- target_surface(목표 표면): `{TARGET_SURFACE}`

Effect(효과): Stage73(73단계)는 v2 고유 근거만 이어받아 34D KPI(34D 핵심 성과 지표) 격차 축소를 계속한다.
""",
    )
    write_md(NEXT_STAGE_ROOT / "03_reviews" / "review_index.md", f"""# Stage73 Review Index(73단계 검토 색인)
...
Effect(효과): Stage73(73단계)는 Stage72(72단계) closeout(종료 기록)을 이어받아 다음 bounded batch(경계 묶음 실행)만 검토한다.
""")
    write_md(NEXT_STAGE_ROOT / "04_selected" / "selection_status.md", f"""# Stage73 Selection Status(73단계 선택 상태)
...
Effect(효과): Stage73(73단계)는 34D KPI(34D 핵심 성과 지표) 격차 축소를 계속하지만, 운영 의미 없이 연구개발로만 이어진다.
""")
...
- >-
  Stage72(72단계) closed(종료) as `{decision}` and Stage73(73단계) `{NEXT_STAGE_ID}` is open_planned(열림 계획). Effect(효과): v41 gate repair(브이41 게이트 수리) 근거를 보존하고 다음 경계 연구 질문으로만 넘긴다.
- >-
  Stage72 result(72단계 결과): validation/OOS(검증/표본외), PF/net/DD(수익 팩터/순손익/손실률), risk/ATR telemetry(위험/ATR 텔레메트리)는 `{rel(SUMMARY_CSV_PATH)}`와 `{rel(RISK_ATR_TELEMETRY_PATH)}`에 기록된다. Effect(효과): 34D target surface(34D 목표 표면) 대비 KPI(핵심 성과 지표) 차이를 다음 단계 입력으로 보존한다.
- >-
...
stage72_v41_gate_repair:
  ...
  boundary: {BOUNDARY}
"""
    if "stage72_v41_gate_repair:" in text:
        text = re.sub(r"\nstage72_v41_gate_repair:\n(?:  .*\n)+", block + "\n", text, count=1)
    else:
        text = text.rstrip() + block
    io_path(WORKSPACE_STATE_PATH).write_text(text + ("\n" if not text.endswith("\n") else ""), encoding="utf-8-sig")
    write_md(SELECTED_ROOT / "selection_status.md", f"""# Stage72 Selection Status(72단계 선택 상태)

- stage_status(단계 상태): `closed_{decision}`
- source_stage(원천 단계): `{SOURCE_STAGE71_ID}`
- source_decision(원천 판정): `continue_v41_source_repair_in_stage72`
- source_stage71_pushed_commit(원천 71단계 푸시 커밋): `{SOURCE_STAGE71_COMMIT}`
- current_run(현재 실행): `{RUN_ID}`
- adapter_under_review(검토 중 어댑터): `{SOURCE_ADAPTER_ID}`
- target_surface(목표 표면): `{TARGET_SURFACE}`
- stage72_decision(72단계 판정): `{decision}`
- next_stage_or_branch(다음 단계/분기): `{NEXT_STAGE_ID}`
- claim_boundary(주장 경계): `{BOUNDARY}`

Effect(효과): Stage72(72단계)는 v41 gate repair(브이41 게이트 수리)를 측정하고, 운영 의미 없이 Stage73(73단계)로 넘긴다.
""")
...
- target_surface(목표 표면): `{TARGET_SURFACE}`
- adapter_under_review(검토 중 어댑터): `{SOURCE_ADAPTER_ID}`
- status(상태): `stage72_closed_{decision}`
- claim_boundary(주장 경계): `{BOUNDARY}`

Stage72(72단계) closed(종료) as v2-native v41 gate repair batch(브이투 고유 브이41 게이트 수리 묶음). Effect(효과): legacy 34D(레거시 34D)는 target surface(목표 표면)로만 쓰였고, 다음 연구는 Stage73(73단계)로 이어진다.

## Latest Stage72 Evidence(최신 72단계 근거)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    prepared = {
        ...
        "model_family": "baseline_adapter_stage72_v2_native_v41_gate_repair_ebm_table",
        "feature_set_id": "stage72_v41_short_gate_signal",
