...
STAGE_ID = "83_adapter_research__v41_hybrid_sl_cooldown_repair"
RUN_NUMBER = "run83A"
RUN_ID = "run83A_stage83_v41_hybrid_sl_cooldown_repair_v1"
PACKET_ID = "stage83_v41_hybrid_sl_cooldown_repair_v1"
PARENT_RUN_ID = "run82A_stage82_v41_early_oos_followup_review_v1"
SOURCE_STAGE82_ID = "82_adapter_research__v41_early_oos_followup_review"
SOURCE_STAGE82_CLOSEOUT_COMMIT = "1bb8a999aa8aeb0409e68c6672ea4985b47444e5"
SOURCE_STAGE82_LATEST_COMMIT = "85664137db84f34240c48ce1795aa4f5214bbb69"
SOURCE_STAGE81_PUSHED_COMMIT = "642b154b71bccd28bfcc2ec5b532e0c00fa680da"
...
NEXT_STAGE_ID = "84_adapter_research__v41_hybrid_sl_cooldown_followup_review"
NEXT_RUN_ID = "run84A_stage84_v41_hybrid_sl_cooldown_followup_review_v1"
NEXT_PACKET_ID = "stage84_v41_hybrid_sl_cooldown_followup_review_v1"
...
COMMON_ROOT = f"OPV2/s83a/{RUN_NUMBER}"

SUMMARY_JSON_PATH = REVIEWS_ROOT / "stage83_v41_hybrid_sl_cooldown_summary.json"
SUMMARY_CSV_PATH = REVIEWS_ROOT / "stage83_v41_hybrid_sl_cooldown_summary.csv"
REPORT_PATH = REVIEWS_ROOT / "stage83_v41_hybrid_sl_cooldown_report.md"
SEGMENT_KPI_PATH = REVIEWS_ROOT / "stage83_segment_kpi_summary.csv"
RISK_ATR_TELEMETRY_PATH = REVIEWS_ROOT / "stage83_risk_atr_telemetry.csv"
GATE_FEATURE_SUMMARY_PATH = REVIEWS_ROOT / "stage83_gate_feature_summary.csv"
TIER_B_DIAGNOSTIC_PATH = REVIEWS_ROOT / "stage83_tier_b_diagnostic_summary.csv"
DECISION_PATH = REVIEWS_ROOT / "stage83_decision.md"
AUDIT_CSV_PATH = REVIEWS_ROOT / "stage83_trade_audit.csv"
...
VARIANTS = (
    repair.RepairVariant(
        adapter_id="s83_v41_h3_risk5_gate08_sl225_tp40_cd12",
        label="run50BN_v41_h3_risk5_gate08_sl225_tp40_cd12",
        atr_enabled=True,
        ...
        max_hold_bars=3,
        notes="Stage83 hybrid SL/cooldown repair: combine SL2.25 with 12-bar cooldown to test early OOS and DD repair.",
    ),
    repair.RepairVariant(
        adapter_id="s83_v41_h3_risk5_gate08_sl225_tp40_cd10",
        label="run50BN_v41_h3_risk5_gate08_sl225_tp40_cd10",
        atr_enabled=True,
        model_risk_enabled=True,
        fixed_lot=0.25,
        atr_stop_multiplier=2.25,
        ...
        close_only_on_opposite_signal=True,
        max_hold_bars=3,
        notes="Stage83 hybrid SL/cooldown repair: combine SL2.25 with 10-bar cooldown to recover net while testing early OOS repair.",
    ),
    repair.RepairVariant(
        adapter_id="s83_v41_h3_risk475_gate08_sl225_tp40_cd12",
        label="run50BN_v41_h3_risk475_gate08_sl225_tp40_cd12",
        atr_enabled=True,
        model_risk_enabled=True,
        fixed_lot=0.25,
        atr_stop_multiplier=2.25,
        ...
        close_only_on_opposite_signal=True,
        max_hold_bars=3,
        notes="Stage83 hybrid SL/cooldown repair: combine SL2.25, 12-bar cooldown, and 4.75% risk cap to test DD compression.",
    ),
)
...
CONTEXT_GATE_SPECS = {
    variant.adapter_id: {
        "gate_column": "stage83_gate_margin_lt_008_short",
        "gate_type": "margin",
        "threshold": 0.08,
        "block_mode": "short",
...
def decide(summary_rows: Sequence[Mapping[str, Any]], segment_rows: Sequence[Mapping[str, Any]], external: str) -> str:
    if external != "completed":
        return "continue_stage83_due_to_incomplete_runtime"
    ...
    if val_net >= 900 and oos_net >= 520 and val_dd <= 22.5 and oos_dd <= 20.8 and val_pf >= 1.45 and oos_pf >= 1.45 and not reasons:
        return "proceed_to_stage84_candidate_review"
    if val_net >= 800 and oos_net >= 430 and val_dd <= 26.0 and oos_dd <= 22.8 and val_pf >= 1.35 and oos_pf >= 1.35:
        return "continue_hybrid_sl_cooldown_review_in_stage84"
    if val_pf < 1.0 or val_net <= 0:
        return "open_new_model_source_branch_in_stage84"
    return "continue_hybrid_sl_cooldown_review_in_stage84"
...
def report_markdown(
    ...
    return f"""# Stage83 V41 Hybrid SL/Cooldown Repair Report(83단계 V41 손절/재진입 냉각 혼합 수리 보고서)

- run(실행): `{RUN_ID}`
- target_surface(목표 표면): `{TARGET_SURFACE}`
- source_stage(원천 단계): `{SOURCE_STAGE82_ID}`
- source_stage82_closeout_commit(원천 82단계 종료 커밋): `{SOURCE_STAGE82_CLOSEOUT_COMMIT}`
- source_stage82_latest_commit(원천 82단계 최신 커밋): `{SOURCE_STAGE82_LATEST_COMMIT}`
- source_stage81_pushed_commit(원천 81단계 푸시 커밋): `{SOURCE_STAGE81_PUSHED_COMMIT}`
...
def decision_markdown(decision: str, external: str) -> str:
    return f"""# Stage83 Decision(83단계 판정)

decision(판정): `{decision}`

Stage83(83단계)는 Stage82(82단계) 판정에 따라 SL2.25(손절 2.25), cooldown(재진입 냉각), 4.75% risk cap(위험 상한)을 좁게 결합해 OOS early(표본외 초반)와 DD(손실률) 수리 가능성을 측정했다.
            ...
            rows.append(
                {
                    ...
                    "split": split_name,
                    "tier_b_policy": "diagnostic_missing_required_but_disabled_for_this_v41_hybrid_sl_cooldown_repair",
                    ...
                    "reason": "Stage83 isolates v41 hybrid SL/cooldown repair first; Tier B fallback remains diagnostic and disabled.",
                }
            )
    return rows
            ...
            rows.append(
                {
                    ...
                    "artifact_type": "stage83_v41_hybrid_sl_cooldown_repair_evidence",
                    "path": rel(path),
                    "sha256": sha256_file_lf_normalized(path),
                    "stage_id": STAGE_ID,
                    "run_id": RUN_ID,
                    "created_at_utc": created,
                    "notes": "Stage83 v2-native v41 hybrid SL/cooldown repair artifact.",
                }
            )
            ...
            rows.append(
                {
                    ...
                    "run_id": RUN_ID,
                    "created_at_utc": created,
                    "notes": "Actual Stage83 MT5 Strategy Tester HTML report.",
                }
            )
    return rows
...
def write_run_identity(result: Mapping[str, Any]) -> None:
    write_json(
        RUN_ROOT / "run_manifest.json",
        {
            ...
            "source_stage82_closeout_commit": SOURCE_STAGE82_CLOSEOUT_COMMIT,
            "source_stage82_latest_commit": SOURCE_STAGE82_LATEST_COMMIT,
            "source_stage81_pushed_commit": SOURCE_STAGE81_PUSHED_COMMIT,
        ...
        [
            {
                "run_id": RUN_ID,
                "stage_id": STAGE_ID,
                "lane": "baseline_adapter_v2_native_v41_hybrid_sl_cooldown_repair",
                "status": status,
                "judgment": decision,
        ...
        alpha_rows = [
            {
                ...
                "tier_scope": "Tier A+B",
                "kpi_scope": "stage83_v41_hybrid_sl_cooldown_repair",
                ...
                "external_verification_status": external,
                "notes": "Stage83 run materialized or blocked before KPI records were available.",
            }
        ]
...
def write_packet_files(result: Mapping[str, Any], decision: str, ledger_payload: Mapping[str, Any]) -> None:
    ...
    write_json(PACKET_ROOT / "aggregate_summary.json", {"packet_id": PACKET_ID, "stage_id": STAGE_ID, "run_id": RUN_ID, "decision": decision, "source_stage82_closeout_commit": SOURCE_STAGE82_CLOSEOUT_COMMIT, "source_stage82_latest_commit": SOURCE_STAGE82_LATEST_COMMIT, "source_stage81_pushed_commit": SOURCE_STAGE81_PUSHED_COMMIT, "source_stage79_latest_commit": SOURCE_STAGE79_LATEST_COMMIT, "source_stage73_latest_commit": SOURCE_STAGE73_LATEST_COMMIT, "gate_feature_summary_path": rel(GATE_FEATURE_SUMMARY_PATH), "ledger_payload": ledger_payload, "overall_goal_complete": False})
...
Stage84(84단계)는 Stage83(83단계)의 hybrid SL/cooldown repair(손절/재진입 냉각 혼합 수리) 결과를 받아 candidate review(후보 검토), continued repair(계속 수리), 또는 another model source(다른 모델 원천)를 고르는 follow-up(후속) 단계다.

## Bounded Question(경계 질문)

Stage83(83단계)의 SL2.25/cooldown hybrid(손절 2.25/재진입 냉각 혼합)가 Stage81(81단계) `cd12` OOS PF/net(표본외 수익 팩터/순손익)을 보존하면서 OOS early(표본외 초반)와 DD(손실률)를 충분히 낮췄는가?

Effect(효과): Stage84(84단계)는 Stage83(83단계)의 결과를 과장하지 않고 다음 판단으로만 좁힌다.
...
""",
    ...
    write_md(
        ...
        f"""# Stage84 Input References(84단계 입력 참조)

- source_stage(원천 단계): `{STAGE_ID}`
- source_run(원천 실행): `{RUN_ID}`
- source_decision(원천 판정): `{decision}`
- source_external_verification_status(원천 외부 검증 상태): `{external}`
- stage83_report(83단계 보고서): `{rel(REPORT_PATH)}`
- stage83_decision(83단계 판정): `{rel(DECISION_PATH)}`
- summary(요약): `{rel(SUMMARY_CSV_PATH)}`
- target_surface(목표 표면): `{TARGET_SURFACE}`

Effect(효과): Stage84(84단계)는 v2 고유 근거만 이어받아 34D KPI(34D 핵심 성과 지표) 격차 축소를 계속한다.
""",
    )
    write_md(NEXT_STAGE_ROOT / "03_reviews" / "review_index.md", f"""# Stage84 Review Index(84단계 검토 색인)
...
Effect(효과): Stage84(84단계)는 Stage83(83단계) closeout(종료 기록)을 이어받아 다음 bounded batch(경계 묶음 실행)만 검토한다.
""")
    write_md(NEXT_STAGE_ROOT / "04_selected" / "selection_status.md", f"""# Stage84 Selection Status(84단계 선택 상태)
...
Effect(효과): Stage84(84단계)는 34D KPI(34D 핵심 성과 지표) 격차 축소를 계속하지만, 운영 의미 없이 연구개발로만 이어진다.
""")
...
- >-
  Stage83(83단계) closed(종료) as `{decision}` and Stage84(84단계) `{NEXT_STAGE_ID}` is open_planned(열림 계획). Effect(효과): v41 hybrid SL/cooldown repair(V41 손절/재진입 냉각 혼합 수리) 근거를 보존하고 다음 경계 연구 질문으로만 넘긴다.
- >-
  Stage83 result(83단계 결과): validation/OOS(검증/표본외), PF/net/DD(수익 팩터/순손익/손실률), risk/ATR telemetry(위험/ATR 텔레메트리)는 `{rel(SUMMARY_CSV_PATH)}`와 `{rel(RISK_ATR_TELEMETRY_PATH)}`에 기록된다. Effect(효과): 34D target surface(34D 목표 표면) 대비 KPI(핵심 성과 지표) 차이를 다음 단계 입력으로 보존한다.
- >-
...
stage83_v41_hybrid_sl_cooldown_repair:
  packet_id: {PACKET_ID}
  stage_id: {STAGE_ID}
  ...
  boundary: {BOUNDARY}
"""
    if "stage83_v41_hybrid_sl_cooldown_repair:" in text:
        text = re.sub(r"\nstage83_v41_hybrid_sl_cooldown_repair:\n(?:  .*\n)+", block + "\n", text, count=1)
    else:
        text = text.rstrip() + block
    io_path(WORKSPACE_STATE_PATH).write_text(text + ("\n" if not text.endswith("\n") else ""), encoding="utf-8-sig")
    write_md(SELECTED_ROOT / "selection_status.md", f"""# Stage83 Selection Status(83단계 선택 상태)

- stage_status(단계 상태): `closed_{decision}`
- source_stage(원천 단계): `{SOURCE_STAGE82_ID}`
- source_decision(원천 판정): `continue_hybrid_sl_cooldown_repair_in_stage83`
- source_stage82_closeout_commit(원천 82단계 종료 커밋): `{SOURCE_STAGE82_CLOSEOUT_COMMIT}`
- source_stage82_latest_commit(원천 82단계 최신 커밋): `{SOURCE_STAGE82_LATEST_COMMIT}`
- source_stage81_pushed_commit(원천 81단계 푸시 커밋): `{SOURCE_STAGE81_PUSHED_COMMIT}`
- source_stage79_latest_commit(원천 79단계 최신 커밋): `{SOURCE_STAGE79_LATEST_COMMIT}`
...
- target_surface(목표 표면): `{TARGET_SURFACE}`
- stage83_decision(83단계 판정): `{decision}`
- next_stage_or_branch(다음 단계/분기): `{NEXT_STAGE_ID}`
- claim_boundary(주장 경계): `{BOUNDARY}`

Effect(효과): Stage83(83단계)는 v41 hybrid SL/cooldown repair(V41 손절/재진입 냉각 혼합 수리)를 측정하고, 운영 의미 없이 Stage84(84단계)로 넘긴다.
""")
...
- target_surface(목표 표면): `{TARGET_SURFACE}`
- adapter_under_review(검토 중 어댑터): `{SOURCE_ADAPTER_ID}`
- status(상태): `stage83_closed_{decision}`
- claim_boundary(주장 경계): `{BOUNDARY}`

Stage83(83단계) closed(종료) as v2-native v41 hybrid SL/cooldown repair batch(브이투 고유 브이41 손절/재진입 냉각 혼합 수리 묶음). Effect(효과): legacy 34D(레거시 34D)는 target surface(목표 표면)로만 쓰였고, 다음 연구는 Stage84(84단계)로 이어진다.

## Latest Stage83 Evidence(최신 83단계 근거)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    prepared = {
        ...
        "model_family": "baseline_adapter_stage83_v2_native_v41_hybrid_sl_cooldown_repair_ebm_table",
        "feature_set_id": "stage83_v41_hybrid_sl_cooldown_signal",
