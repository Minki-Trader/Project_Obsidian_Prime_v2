...
from stage_pipelines.stage186 import tp45_midwide_bracket_shape_repair as base  # noqa: E402
...
STAGE_ID = "188_adapter_research__v2_native_context_feature_branch_after_midwide_tradeoff"
RUN_NUMBER = "run188A"
RUN_ID = "run188A_stage188_v2_native_context_feature_branch_after_midwide_tradeoff_v1"
PACKET_ID = "stage188_v2_native_context_feature_branch_after_midwide_tradeoff_v1"
PARENT_RUN_ID = "run187A_stage187_stage186_bracket_shape_followup_review_v1"
SOURCE_STAGE_ID = "187_adapter_research__stage186_bracket_shape_followup_review"
SOURCE_RUN_ID = "run187A_stage187_stage186_bracket_shape_followup_review_v1"
SOURCE_STAGE187_CLOSEOUT_COMMIT = "eeaed81f257810cf1058f22f0b311ca303e6e7a7"
SOURCE_STAGE187_HASH_RECORD_COMMIT = "bc01c6e42e24d09a431019338ad5f22f1a21258a"
SOURCE_ADAPTER_ID = "s186_bctl"
NEXT_STAGE_ID = "189_adapter_research__stage188_context_feature_followup_review"
NEXT_RUN_ID = "run189A_stage189_stage188_context_feature_followup_review_v1"
NEXT_PACKET_ID = "stage189_stage188_context_feature_followup_review_v1"
TARGET_SURFACE = "legacy_34d_kpi_lesson_only_v2_native_context_feature_branch"
BOUNDARY = (
...
COMMON_ROOT = f"OPV2/s188a/{RUN_NUMBER}"

SUMMARY_JSON_PATH = REVIEWS_ROOT / "stage188_context_feature_summary.json"
SUMMARY_CSV_PATH = REVIEWS_ROOT / "stage188_context_feature_summary.csv"
REPORT_PATH = REVIEWS_ROOT / "stage188_context_feature_report.md"
SEGMENT_KPI_PATH = REVIEWS_ROOT / "stage188_segment_kpi_summary.csv"
BALANCE_CURVE_AUDIT_PATH = REVIEWS_ROOT / "stage188_balance_curve_audit.csv"
MONTHLY_KPI_PATH = REVIEWS_ROOT / "stage188_monthly_kpi_summary.csv"
CONCENTRATION_PATH = REVIEWS_ROOT / "stage188_concentration_risk_summary.csv"
DRAWDOWN_PATH = REVIEWS_ROOT / "stage188_drawdown_recovery_summary.csv"
QUALITY_MATRIX_PATH = REVIEWS_ROOT / "stage188_quality_matrix.csv"
RISK_ATR_TELEMETRY_PATH = REVIEWS_ROOT / "stage188_risk_atr_telemetry.csv"
GATE_FEATURE_SUMMARY_PATH = REVIEWS_ROOT / "stage188_gate_feature_summary.csv"
PROBABILITY_BINDING_PATH = REVIEWS_ROOT / "stage188_probability_binding_summary.csv"
MODEL_SCORE_AUDIT_PATH = REVIEWS_ROOT / "stage188_model_score_audit.csv"
TIER_B_DIAGNOSTIC_PATH = REVIEWS_ROOT / "stage188_tier_b_diagnostic_summary.csv"
DECISION_PATH = REVIEWS_ROOT / "stage188_decision.md"
AUDIT_CSV_PATH = REVIEWS_ROOT / "stage188_trade_audit.csv"
...
PRODUCER_PATH = Path("stage_pipelines/stage188/v2_native_context_feature_branch.py")
ARTIFACT_COLUMNS = s172.ARTIFACT_COLUMNS

VARIANTS = (
    repair.RepairVariant(
        adapter_id="s188_bctl",
        label="stage188_bctl",
        atr_enabled=True,
        ...
        max_hold_bars=3,
        notes="Stage188 control: Stage186 control context gate, TP45 bracket, and risk0325.",
    ),
    repair.RepairVariant(
        adapter_id="s188_short_relief",
        label="stage188_short_relief",
        atr_enabled=True,
        model_risk_enabled=True,
        fixed_lot=0.25,
        atr_stop_multiplier=2.075,
        ...
        close_only_on_opposite_signal=False,
        max_hold_bars=3,
        notes="Stage188: relieve short-side context blocking while preserving long low-edge block.",
    ),
    repair.RepairVariant(
        adapter_id="s188_long_strict",
        label="stage188_long_strict",
        atr_enabled=True,
        model_risk_enabled=True,
        fixed_lot=0.25,
        atr_stop_multiplier=2.075,
        ...
        close_only_on_opposite_signal=False,
        max_hold_bars=3,
        notes="Stage188: stricter long-side wide low-edge context block with baseline short block.",
    ),
    repair.RepairVariant(
        adapter_id="s188_gate_off",
        label="stage188_gate_off",
        atr_enabled=True,
        model_risk_enabled=True,
        fixed_lot=0.25,
        atr_stop_multiplier=2.075,
        ...
        close_only_on_opposite_signal=False,
        max_hold_bars=3,
        notes="Stage188: remove encoded context gate to measure whether the feature branch is over-blocking.",
    ),
)
    ...
    for variant_index, variant in enumerate(VARIANTS, start=1):
        ...
        for split in ("validation_is", "oos"):
            ...
            for role_index, (tier, attempt_role, prefix, attempt_token) in enumerate(
                ...
                magic = 18810000 + variant_index * 100 + (1 if split == "validation_is" else 50) + role_index
                attempts.append(
                    s161.base.attempt_payload(
                        run_root=variant_root,
                        run_id=RUN_ID,
                        stage_number=188,
                        exploration_label="stage188_BaselineAdapter__V2NativeContextFeatureBranch",
...
def configure_runner() -> None:
    ...
    for variant in VARIANTS:
        ...
        context_specs[variant.adapter_id] = {
            "gate_column": f"stage188_gate_{extra['axis']}",
            "gate_type": "encoded_stage188_v2_native_context_feature_branch",
            ...
            "description": "Stage188 encoded gate: tests context relief/strictness without changing ATR bracket or risk.",
        }
    values = stage_values(context_specs)
    for name, value in values.items():
        for module in (base, s184, s180, s178, s176, s174, s172, s161):
...
def decide(rows: Sequence[Mapping[str, Any]], external: str) -> str:
    if external != "completed":
        return "continue_stage188_runtime_completion_due_to_incomplete_runtime_candidate_not_final"
    if any(row.get("hard_quality_pass") for row in rows):
        return "open_stage189_context_feature_followup_review_candidate_not_final"
    return "open_stage189_bounded_followup_due_to_context_feature_tradeoff_candidate_not_final"
...
def kpi_table(rows: Sequence[Mapping[str, Any]]) -> str:
    ...
    for row in rows:
        ...
        lines.append(
            "| {adapter_id} | {axis} | {validation_pf:.6f} | {validation_net:.2f} | {validation_balance_dd_percent:.4f} | {validation_mid_pf:.6f} | {oos_pf:.6f} | {oos_net:.2f} | {oos_balance_dd_percent:.4f} | {quality_flags} |".format(
...
def report_markdown(rows: Sequence[Mapping[str, Any]], decision: str, external: str) -> str:
    best = best_row(rows)
    return f"""# Stage188 V2-Native Context Feature Branch Report(188단계 v2 고유 문맥 피처 분기 보고서)
...
- source_stage187_closeout_commit(원천 187단계 종료 커밋): `{SOURCE_STAGE187_CLOSEOUT_COMMIT}`
- source_stage187_hash_record_commit(원천 187단계 해시 기록 커밋): `{SOURCE_STAGE187_HASH_RECORD_COMMIT}`
...
def decision_markdown(decision: str, external: str) -> str:
    return f"""# Stage188 Decision(188단계 판정)
...
Stage188(188단계) closeout(종료)는 overall goal complete(전체 목표 완료)가 아니다. Effect(효과): Stage189(189단계)에서 context/feature branch(문맥/피처 분기)의 KPI(핵심 성과 지표) 효용을 follow-up review(후속 검토)로 다시 판독한다.
"""
            ...
            rows.append(
                {
                    ...
                    "artifact_type": "stage188_evidence",
                    "path": rel(path),
                    "sha256": s172.sha256_file_lf_normalized(path),
                    "stage_id": STAGE_ID,
                    "run_id": RUN_ID,
                    "created_at_utc": now,
                    "notes": "Stage188 v2-native context feature branch evidence.",
                }
            )
            ...
            rows.append(
                {
                    ...
                    "run_id": RUN_ID,
                    "created_at_utc": now,
                    "notes": "Stage188 MT5 Strategy Tester report.",
                }
            )
    return rows
        ...
        [
            {
                "run_id": RUN_ID,
                "stage_id": STAGE_ID,
                "lane": "baseline_adapter_stage188_v2_native_context_feature_branch",
                "status": status,
                "judgment": decision,
                "path": rel(DECISION_PATH),
                "notes": s172.ledger_pairs(
                    (
                        ("source_stage187_closeout_commit", SOURCE_STAGE187_CLOSEOUT_COMMIT),
                        ("source_stage187_hash_record_commit", SOURCE_STAGE187_HASH_RECORD_COMMIT),
        ...
        alpha_rows = [
            {
                ...
                "tier_scope": "Tier A+B",
                "kpi_scope": "stage188_v2_native_context_feature_branch",
                ...
                "external_verification_status": external,
                "notes": "Stage188 materialized or blocked before KPI records were available.",
            }
        ]
...
def write_packet_files(result: Mapping[str, Any], decision: str, ledger_payload: Mapping[str, Any], quality: Sequence[Mapping[str, Any]]) -> None:
    ...
    s172.write_md(
        ...
        f"""# Stage188 Closeout Packet(188단계 종료 작업 묶음)
...
Stage189(189단계)는 Stage188(188단계) v2-native context/feature branch(v2 고유 문맥/피처 분기) 결과를 follow-up review(후속 검토)한다.

## Bounded Question(경계 질문)

Did Stage188(188단계) improve validation DD(검증 낙폭), validation mid PF(검증 중반 수익요인), MFE capture(최대유리이동 포착), validation/OOS consistency(검증/표본외 일관성), and 34D(34D) KPI(핵심 성과 지표) proximity enough to justify the next bounded repair(경계 수정), or should this branch be demoted(강등) or replaced(교체)?

Effect(효과): Stage188(188단계) 안에서 끝없이 수정하지 않고 context/feature branch(문맥/피처 분기)의 실제 효용만 판독한다.
...
- status(상태): `open_planned_from_stage188`
- source_stage(원천 단계): `{STAGE_ID}`
- source_decision(원천 판정): `{decision}`
...
- stage_status(단계 상태): `open_planned_from_stage188`
...
- >-
  Stage188(188단계) closed(종료) as `{decision}` and Stage189(189단계) `{NEXT_STAGE_ID}` is open_planned(개방 계획). Effect(효과): context/feature branch(문맥/피처 분기) 결과를 별도 follow-up review(후속 검토)로 넘긴다.
- >-
  Stage188 evidence(188단계 근거)는 `{rel(REPORT_PATH)}`, `{rel(SUMMARY_CSV_PATH)}`, `{rel(QUALITY_MATRIX_PATH)}`, `{rel(BALANCE_CURVE_AUDIT_PATH)}`, `{rel(RISK_ATR_TELEMETRY_PATH)}`에 있다. Effect(효과): 34D(34D) KPI(핵심 성과 지표) 근접도와 DD/mid PF(낙폭/중반 수익요인)를 함께 본다.
- >-
...
"""
        ...
        focus = f"""current_focus:
- >-
  Stage188(188단계) runtime evidence(런타임 근거)가 incomplete(불완전)하여 `{decision}`로 기록했다. Effect(효과): 완료 주장을 낮추고 runtime completion(런타임 완료) 조건을 보존한다.

"""
    state = re.sub(r"(?ms)^current_focus:\r?\n.*?(?=\r?\nstage\d+_)", focus, state, count=1)
    state = re.sub(r"(?ms)^stage188_v2_native_context_feature_branch:\r?\n.*?(?=^stage\d+_|\Z)", "", state)
    block = f"""
stage188_v2_native_context_feature_branch:
  packet_id: {PACKET_ID}
  stage_id: {STAGE_ID}
...
- adapter_under_review(검토 중 어댑터): `stage188_v2_native_context_feature_branch`
- status(상태): `stage188_{decision}`
- claim_boundary(주장 경계): `{BOUNDARY}`

Stage188(188단계)는 Stage186(186단계) control(대조군)을 기준으로 ATR bracket(ATR 브래킷)과 model-controlled risk(모델 제어 위험)를 고정하고 encoded context gate(인코딩 문맥 게이트)만 바꿔 측정했다. Effect(효과): 브래킷 미세조정이 아니라 v2-native context/feature branch(v2 고유 문맥/피처 분기)의 KPI(핵심 성과 지표) 효용을 분리한다.

## Latest Stage188 Evidence(최신 188단계 근거)
...
def append_changelog(decision: str) -> None:
    ...
    if RUN_ID in existing:
        return
    entry = (
        f"\n## {s172.utc_now()} Stage188 v2-native context feature branch closeout(188단계 v2 고유 문맥 피처 분기 종료)\n\n"
