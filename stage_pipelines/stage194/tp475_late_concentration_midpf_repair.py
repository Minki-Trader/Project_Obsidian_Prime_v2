...
STAGE_ID = "194_adapter_research__tp475_late_concentration_midpf_repair"
RUN_NUMBER = "run194A"
RUN_ID = "run194A_stage194_tp475_late_concentration_midpf_repair_v1"
PACKET_ID = "stage194_tp475_late_concentration_midpf_repair_v1"
PARENT_RUN_ID = "run193A_stage193_stage192_tp475_midsegment_followup_review_v1"
SOURCE_STAGE_ID = "193_adapter_research__stage192_tp475_midsegment_followup_review"
SOURCE_RUN_ID = "run193A_stage193_stage192_tp475_midsegment_followup_review_v1"
SOURCE_STAGE193_CLOSEOUT_COMMIT = "fd4eaaf7ee8d2cf4fecd3d6b114f944630916530"
SOURCE_STAGE193_HASH_RECORD_COMMIT = "e0b46a66b4034be48a8e21a66a4d951005cfb5f7"
SOURCE_ADAPTER_ID = "s192_tp475_r0330_net_dd_pass_midpf_late_fail"
NEXT_STAGE_ID = "195_adapter_research__stage194_late_midpf_followup_review"
NEXT_RUN_ID = "run195A_stage195_stage194_late_midpf_followup_review_v1"
NEXT_PACKET_ID = "stage195_stage194_late_midpf_followup_review_v1"
...
COMMON_ROOT = f"OPV2/s194a/{RUN_NUMBER}"

SUMMARY_JSON_PATH = REVIEWS_ROOT / "stage194_tp475_late_midpf_summary.json"
SUMMARY_CSV_PATH = REVIEWS_ROOT / "stage194_tp475_late_midpf_summary.csv"
REPORT_PATH = REVIEWS_ROOT / "stage194_tp475_late_midpf_report.md"
SEGMENT_KPI_PATH = REVIEWS_ROOT / "stage194_segment_kpi_summary.csv"
BALANCE_CURVE_AUDIT_PATH = REVIEWS_ROOT / "stage194_balance_curve_audit.csv"
MONTHLY_KPI_PATH = REVIEWS_ROOT / "stage194_monthly_kpi_summary.csv"
CONCENTRATION_PATH = REVIEWS_ROOT / "stage194_concentration_risk_summary.csv"
DRAWDOWN_PATH = REVIEWS_ROOT / "stage194_drawdown_recovery_summary.csv"
QUALITY_MATRIX_PATH = REVIEWS_ROOT / "stage194_quality_matrix.csv"
RISK_ATR_TELEMETRY_PATH = REVIEWS_ROOT / "stage194_risk_atr_telemetry.csv"
GATE_FEATURE_SUMMARY_PATH = REVIEWS_ROOT / "stage194_gate_feature_summary.csv"
PROBABILITY_BINDING_PATH = REVIEWS_ROOT / "stage194_probability_binding_summary.csv"
MODEL_SCORE_AUDIT_PATH = REVIEWS_ROOT / "stage194_model_score_audit.csv"
TIER_B_DIAGNOSTIC_PATH = REVIEWS_ROOT / "stage194_tier_b_diagnostic_summary.csv"
DECISION_PATH = REVIEWS_ROOT / "stage194_decision.md"
AUDIT_CSV_PATH = REVIEWS_ROOT / "stage194_trade_audit.csv"
...
PRODUCER_PATH = Path("stage_pipelines/stage194/tp475_late_concentration_midpf_repair.py")
ARTIFACT_COLUMNS = s172.ARTIFACT_COLUMNS

VARIANTS = (
    repair.RepairVariant(
        adapter_id="s194_ref_r0330",
        label="stage194_ref_r0330",
        atr_enabled=True,
        ...
        max_hold_bars=3,
        notes="Stage194 reference: Stage192 TP4.75 r0330 net/DD pass with mid PF and late concentration weakness.",
    ),
    repair.RepairVariant(
        adapter_id="s194_bctl_tp475_r0330",
        label="stage194_bctl_tp475_r0330",
        atr_enabled=True,
        model_risk_enabled=True,
        fixed_lot=0.25,
        atr_stop_multiplier=2.075,
        ...
        close_only_on_opposite_signal=False,
        max_hold_bars=3,
        notes="Stage194 context rebalance: restore bctl long gate clue while keeping TP4.75 and r0330.",
    ),
    repair.RepairVariant(
        adapter_id="s194_hold2_r0330",
        label="stage194_hold2_r0330",
        atr_enabled=True,
        model_risk_enabled=True,
        fixed_lot=0.25,
        atr_stop_multiplier=2.075,
        ...
        close_only_on_opposite_signal=False,
        max_hold_bars=2,
        notes="Stage194 lifecycle repair: shorten max hold to test stale-trade and late concentration sensitivity.",
    ),
    repair.RepairVariant(
        adapter_id="s194_cd8_r0330",
        label="stage194_cd8_r0330",
        atr_enabled=True,
        model_risk_enabled=True,
        fixed_lot=0.25,
        atr_stop_multiplier=2.075,
        ...
        close_only_on_opposite_signal=False,
        max_hold_bars=3,
        notes="Stage194 re-entry repair: widen same-direction cooldown without risk-only increase.",
    ),
)
...
VARIANT_EXTRAS: dict[str, dict[str, Any]] = {
    ...
    "s194_bctl_tp475_r0330": {
        **BASE_EXTRA,
        "short_block_rule": "midwide_lowedge",
        "long_block_rule": "lowedge_gate",
        "axis": "bctl_tp475_r0330",
    },
    "s194_hold2_r0330": {
        **BASE_EXTRA,
        "short_block_rule": "midwide_lowedge",
    ...
    for variant_index, variant in enumerate(VARIANTS, start=1):
        ...
        for split in ("validation_is", "oos"):
            ...
            for role_index, (tier, attempt_role, prefix, attempt_token) in enumerate(
                ...
                magic = 19410000 + variant_index * 100 + (1 if split == "validation_is" else 50) + role_index
                attempts.append(
                    s161.base.attempt_payload(
                        run_root=variant_root,
                        run_id=RUN_ID,
                        stage_number=194,
                        exploration_label="stage194_BaselineAdapter__TP475LateConcentrationMidPFRepair",
...
def configure_runner() -> None:
    ...
    for variant in VARIANTS:
        ...
        context_specs[variant.adapter_id] = {
            "gate_column": f"stage194_gate_{extra['axis']}",
            "gate_type": "encoded_stage194_tp475_late_midpf_repair",
            ...
            "description": "Stage194 encoded gate: TP4.75 r0330 late concentration and mid PF bounded repair.",
        }
...
def decide(rows: Sequence[Mapping[str, Any]], external: str) -> str:
    if external != "completed":
        return "continue_stage194_runtime_completion_due_to_incomplete_runtime_candidate_not_final"
    if any(row.get("hard_quality_pass") for row in rows):
        return "open_stage195_stage194_late_midpf_followup_review_candidate_not_final"
    return "open_stage195_bounded_followup_due_to_late_midpf_tradeoff_candidate_not_final"
...
def report_markdown(rows: Sequence[Mapping[str, Any]], decision: str, external: str) -> str:
    best = best_row(rows)
    ref = next((row for row in rows if row.get("adapter_id") == "s194_ref_r0330"), {})
    return f"""# Stage194 TP4.75 Late Concentration Mid PF Repair Report(194단계 TP4.75 후반 집중 중반 수익요인 수정 보고서)
...
- source_stage193_closeout_commit(원천 193단계 종료 커밋): `{SOURCE_STAGE193_CLOSEOUT_COMMIT}`
- source_stage193_hash_record_commit(원천 193단계 해시 기록 커밋): `{SOURCE_STAGE193_HASH_RECORD_COMMIT}`
...
def decision_markdown(decision: str, external: str) -> str:
    return f"""# Stage194 Decision(194단계 판정)
...
Stage194(194단계) closeout(종료)은 overall goal complete(전체 목표 완료)가 아니다. Effect(효과): Stage195(195단계) follow-up review(후속 검토)에서 KPI(핵심 성과 지표) 상충을 따로 판정한다.
"""
...
def artifact_rows(result: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    for row in rows:
        if row.get("artifact_type") != "mt5_strategy_tester_report":
            row["artifact_type"] = "stage194_evidence"
            row["notes"] = "Stage194 TP4.75 late concentration and mid PF repair evidence."
        else:
            row["notes"] = "Stage194 MT5 Strategy Tester report."
    return rows
        ...
        [
            {
                "run_id": RUN_ID,
                "stage_id": STAGE_ID,
                "lane": "baseline_adapter_stage194_tp475_late_midpf_repair",
                "status": status,
                "judgment": decision,
                "path": rel(DECISION_PATH),
                "notes": s172.ledger_pairs(
                    (
                        ("source_stage193_closeout_commit", SOURCE_STAGE193_CLOSEOUT_COMMIT),
                        ("source_stage193_hash_record_commit", SOURCE_STAGE193_HASH_RECORD_COMMIT),
        ...
        alpha_rows = [
            {
                ...
                "tier_scope": "Tier A+B",
                "kpi_scope": "stage194_tp475_late_midpf_repair",
                ...
                "external_verification_status": external,
                "notes": "Stage194 materialized or blocked before KPI records were available.",
            }
        ]
...
def write_packet_files(result: Mapping[str, Any], decision: str, ledger_payload: Mapping[str, Any], quality: Sequence[Mapping[str, Any]]) -> None:
    ...
    s172.write_md(
        ...
        f"""# Stage194 Closeout Packet(194단계 종료 작업 묶음)
...
Stage195(195단계)는 Stage194(194단계) TP4.75 late concentration/mid PF repair(후반 집중/중반 수익요인 수정) 결과를 follow-up review(후속 검토)한다.

## Bounded Question(경계 질문)

Did Stage194(194단계) reduce late concentration(후반 집중) and improve mid PF(중반 수익요인) while preserving validation net/PF/DD(검증 순손익/수익요인/낙폭) and OOS(표본외) credibility?

Effect(효과): Stage194(194단계) 안에서 추가 수리를 하지 않고, 측정 결과를 별도 판독 단계에서 과장 없이 판단한다.
...
- status(상태): `open_planned_from_stage194`
- source_stage(원천 단계): `{STAGE_ID}`
- source_decision(원천 판정): `{decision}`
...
- stage_status(단계 상태): `open_planned_from_stage194`
...
- >-
  Stage194(194단계) closed(종료) as `{decision}` and Stage195(195단계) `{NEXT_STAGE_ID}` is open_planned(개방 계획). Effect(효과): late concentration/mid PF(후반 집중/중반 수익요인) 수리 결과를 별도 follow-up review(후속 검토)로 넘긴다.
- >-
  Stage194 evidence(194단계 근거)는 `{rel(REPORT_PATH)}`, `{rel(SUMMARY_CSV_PATH)}`, `{rel(QUALITY_MATRIX_PATH)}`, `{rel(BALANCE_CURVE_AUDIT_PATH)}`, `{rel(RISK_ATR_TELEMETRY_PATH)}`에 있다. Effect(효과): net/PF/DD(순손익/수익요인/낙폭), segment(구간), concentration(집중도), ATR/risk telemetry(ATR/위험 기록)를 같이 본다.
- >-
...
"""
        ...
        focus = f"""current_focus:
- >-
  Stage194(194단계) runtime evidence(런타임 근거)가 incomplete(불완전)하여 `{decision}`로 기록했다. Effect(효과): 완료 주장을 낮추고 runtime completion(런타임 완료) 조건을 보존한다.

"""
    state = re.sub(r"(?ms)^current_focus:\r?\n.*?(?=\r?\nstage\d+_)", focus, state, count=1)
    state = re.sub(r"(?ms)^stage194_tp475_late_concentration_midpf_repair:\r?\n.*?(?=^stage\d+_|\Z)", "", state)
    block = f"""
stage194_tp475_late_concentration_midpf_repair:
  packet_id: {PACKET_ID}
  stage_id: {STAGE_ID}
...
- status(상태): `stage194_{decision}`
- claim_boundary(주장 경계): `{BOUNDARY}`

Stage194(194단계)는 TP4.75/r0330(TP4.75/위험 0.0330) 표면에서 context rebalance(문맥 재균형), hold compression(보유 압축), cooldown widening(대기 확대)을 MT5 Strategy Tester(메타트레이더5 전략 테스터)로 측정했다. Effect(효과): Stage195(195단계)가 KPI(핵심 성과 지표) 상충을 별도 review(검토)로 판정한다.

## Latest Stage194 Evidence(최신 194단계 근거)
...
def append_changelog(decision: str) -> None:
    ...
    if RUN_ID in existing:
        return
    entry = (
        f"\n## {s172.utc_now()} Stage194 TP4.75 late concentration mid PF repair closeout(194단계 TP4.75 후반 집중 중반 수익요인 수정 종료)\n\n"
