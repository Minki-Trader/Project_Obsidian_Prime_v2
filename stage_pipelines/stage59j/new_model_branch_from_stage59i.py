...
SOURCE_STAGE59I_ID = "59I_adapter_repair__bounded_followup_from_stage59h"
STAGE59J_ID = "59J_adapter_repair__new_model_branch_from_stage59i"
NEXT_REPAIR_STAGE_ID = "59K_adapter_repair__bounded_followup_from_stage59j"
RUN_NUMBER = "run59E"
RUN_ID = "run59E_stage59j_new_model_branch_from_stage59i_v1"
PACKET_ID = "stage59j_new_model_branch_from_stage59i_v1"
PARENT_RUN_ID = "run59D_stage59i_bounded_followup_from_stage59h_v1"
...
SOURCE_STAGE59I_PUSHED_COMMIT = "02fbd87e337756fbe5f7c3f480e1e37a49868e6d"
BOUNDARY = (
    "research_development_only_no_live_readiness_no_runtime_authority_"
    "no_operating_promotion_no_operating_reference_no_production_baseline_no_deployment"
)

STAGE_ROOT = Path("stages") / STAGE59J_ID
...
SOURCE_STAGE59I_ROOT = Path("stages") / SOURCE_STAGE59I_ID
SOURCE_STAGE59I_DECISION = SOURCE_STAGE59I_ROOT / "03_reviews/stage59i_decision.md"
SOURCE_STAGE59I_REPORT = SOURCE_STAGE59I_ROOT / "03_reviews/demotion_or_branch_from_stage59h_report.md"
SOURCE_STAGE59I_SUMMARY = SOURCE_STAGE59I_ROOT / "03_reviews/demotion_or_branch_summary.csv"
SOURCE_STAGE59I_JSON = SOURCE_STAGE59I_ROOT / "03_reviews/demotion_or_branch_summary.json"
...
COMMON_ROOT = f"Project_Obsidian_Prime_v2/s59j/{RUN_NUMBER}"

REPORT_PATH = REVIEWS_ROOT / "new_model_branch_from_stage59i_report.md"
...
DECISION_PATH = REVIEWS_ROOT / "stage59j_decision.md"
AUDIT_CSV_PATH = REVIEWS_ROOT / "stage59j_trade_audit.csv"
...
STAGE59J_VARIANTS = (
    repair.RepairVariant(
        adapter_id="s59j_v61_trn_h2_sd5",
        label="run50BQ_v61_transition_h2_threshold57_risk3pct_wide_atr_sd5",
        atr_enabled=True,
        ...
        max_hold_bars=2,
        notes="Stage59J bounded new branch: run50BQ v61 entry-transition source after Stage59I v54 demotion, 3% model risk cap, and wide ATR bracket.",
    ),
    repair.RepairVariant(
        adapter_id="s59j_v62_trn_h4_sd5",
        label="run50BQ_v62_transition_h4_threshold57_risk3pct_wide_atr_sd5",
        atr_enabled=True,
        model_risk_enabled=True,
        fixed_lot=0.25,
        atr_stop_multiplier=2.5,
        ...
        close_only_on_opposite_signal=True,
        max_hold_bars=4,
        notes="Stage59J bounded new branch: run50BQ v62 transition hold4 source after Stage59I v54 demotion, 3% model risk cap, and wide ATR bracket.",
    ),
    repair.RepairVariant(
        adapter_id="s59j_v63_trn_h6_sd5",
        label="run50BQ_v63_transition_h6_threshold57_risk3pct_wide_atr_sd5",
        atr_enabled=True,
        model_risk_enabled=True,
        fixed_lot=0.25,
        atr_stop_multiplier=2.5,
        ...
        close_only_on_opposite_signal=True,
        max_hold_bars=6,
        notes="Stage59J bounded new branch: run50BQ v63 transition hold6 source after Stage59I v54 demotion, 3% model risk cap, and wide ATR bracket.",
    ),
)
...
def source_specs() -> dict[str, dict[str, Any]]:
    return {
        "s59j_v61_trn_h2_sd5": {
            ...
            "source_note": "Stage56 run50BQ v61 context ExtraTrees firewall transition hold2 source",
        },
        "s59j_v62_trn_h4_sd5": {
            "label": "run50bq_v62_transition_h4",
            "run_root": RUN50BQ_ROOT,
            "variant_root": RUN50BQ_ROOT / RUN50BQ_V62_ANCHOR,
            "anchor": RUN50BQ_V62_ANCHOR,
            "model": RUN50BQ_ROOT / "models/stage56_context_timed_event_signal_discrete_score_table.csv",
            "signal_column": RUN50BQ_SIGNAL,
            "validation_ini": RUN50BQ_ROOT / RUN50BQ_V62_ANCHOR / "mt5/x03_ta_val.ini",
            "oos_ini": RUN50BQ_ROOT / RUN50BQ_V62_ANCHOR / "mt5/x03_ta_oos.ini",
            "source_note": "Stage56 run50BQ v62 context ExtraTrees firewall transition hold4 source",
        },
        "s59j_v63_trn_h6_sd5": {
            "label": "run50bq_v63_transition_h6",
            "run_root": RUN50BQ_ROOT,
            "variant_root": RUN50BQ_ROOT / RUN50BQ_V63_ANCHOR,
            "anchor": RUN50BQ_V63_ANCHOR,
            "model": RUN50BQ_ROOT / "models/stage56_context_timed_event_signal_discrete_score_table.csv",
            "signal_column": RUN50BQ_SIGNAL,
            "validation_ini": RUN50BQ_ROOT / RUN50BQ_V63_ANCHOR / "mt5/x04_ta_val.ini",
            "oos_ini": RUN50BQ_ROOT / RUN50BQ_V63_ANCHOR / "mt5/x04_ta_oos.ini",
            "source_note": "Stage56 run50BQ v63 context ExtraTrees firewall transition hold6 source",
        },
    }


def configure_reused_engine() -> None:
    engine.STAGE59_ID = STAGE59J_ID
    ...
    engine.STAGE59_VARIANTS = STAGE59J_VARIANTS
    engine.SOURCE_SPECS = source_specs()
    engine.MODEL_RISK_MIN_PCT = {variant.adapter_id: 0.005 for variant in STAGE59J_VARIANTS}

    repair.STAGE_ID = STAGE59J_ID
    repair.RUN_NUMBER = RUN_NUMBER
    repair.RUN_ID = RUN_ID
    repair.RUN_ROOT = RUN_ROOT
    repair.REPAIR_VARIANTS = STAGE59J_VARIANTS
    s58.STAGE58_ID = STAGE59J_ID
    ...
    s58.STAGE58_VARIANTS = STAGE59J_VARIANTS
...
def build_attempts(inputs: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    for variant_index, variant in enumerate(STAGE59J_VARIANTS, start=1):
            ...
            for role_index, (tier, attempt_role, prefix, attempt_token) in enumerate(
                ...
                attempts.append(
                    attempt_payload(
                        ...
                        run_id=RUN_ID,
                        stage_number=59,
                        exploration_label="stage59J_BaselineAdapter__NewModelBranchFromFailureMemory",
...
def execute_prepared_run_checkpointed(prepared: Mapping[str, Any], args: argparse.Namespace) -> dict[str, Any]:
    ...
    return {
        **dict(prepared),
        ...
        "judgment": "inconclusive_stage59j_runtime_probe_completed" if completed and report_completed else "blocked_stage59j_runtime_probe",
    }
...
def next_stage_for_decision(decision: str) -> str:
    ...
    if decision == "demote_current_adapter_and_select_backup":
        return "59K_adapter_repair__backup_or_demote_from_stage59j"
    if decision == "open_new_model_branch":
        return "59K_adapter_repair__new_model_branch_from_stage59j"
...
def next_packet_for_decision(decision: str) -> tuple[str, str]:
    ...
    return "stage59k_bounded_followup_from_stage59j_v1", "run59F_stage59k_bounded_followup_from_stage59j_v1"
            ...
            rows.append(
                {
                    ...
                    "stage_id": STAGE59J_ID,
                    "run_id": RUN_ID,
                    "artifact_type": "stage59j_new_model_branch_evidence",
                    "path": rel(path),
                    "sha256": sha256_file_lf_normalized(path),
                    "created_at_utc": created,
                    "notes": "Stage59J bounded new model branch from Stage59I artifact.",
                }
            )
            ...
            rows.append(
                {
                    ...
                    "created_at_utc": created,
                    "notes": "Actual Stage59J MT5 Strategy Tester HTML report.",
                }
            )
    return rows
...
def write_run_identity(result: Mapping[str, Any]) -> None:
    engine.write_json(
        RUN_ROOT / "run_manifest.json",
        {
            ...
            "bounded_question": "Can a new bounded model branch, informed by the Stage59I demotion decision, produce a post-ATR/risk adapter candidate without starting ONNX?",
            "source_stage59i_decision": rel(SOURCE_STAGE59I_DECISION),
            "source_stage59i_pushed_commit": SOURCE_STAGE59I_PUSHED_COMMIT,
            "variants": [
                {
                    **variant.__dict__,
                ...
                for variant in STAGE59J_VARIANTS
            ],
...
def write_ledgers(
        ...
        [
            {
                "run_id": RUN_ID,
                "stage_id": STAGE59J_ID,
                "lane": "baseline_adapter_new_model_branch_from_stage59i",
                "status": status,
                "judgment": decision,
    ...
    ledger_rows.append(
        {
            "ledger_row_id": f"{RUN_ID}__aggregate_new_model_branch_from_stage59i",
            "stage_id": STAGE59J_ID,
            "run_id": RUN_ID,
            "subrun_id": "aggregate_new_model_branch_from_stage59i",
            "parent_run_id": PARENT_RUN_ID,
            "record_view": "new_model_branch_from_stage59i",
            "tier_scope": "Tier A+B",
            ...
            "external_verification_status": external,
            "notes": "Stage59J bounded new model branch from Stage59I demotion evidence; not final package completion.",
        }
    )
...
def report_markdown(summary_rows: Sequence[Mapping[str, Any]], decision: str, reasons: Sequence[str], external: str) -> str:
    ...
    best = best_repaired_variant(summary_rows)
    return f"""# Stage59J New Model Branch From Stage59I Report(59J단계 59I단계 기반 새 모델 분기 보고서)

- stage(단계): `{STAGE59J_ID}`
- run(실행): `{RUN_ID}`
...
def equity_audit_markdown(summary_rows: Sequence[Mapping[str, Any]], segment_rows: Sequence[Mapping[str, Any]]) -> str:
    ...
    return f"""# Stage59J New Model Branch Equity Curve Audit(59J단계 새 모델 분기 자금 곡선 감사)
...
def decision_markdown(decision: str, reasons: Sequence[str], best: Mapping[str, Any], external: str) -> str:
    return f"""# Stage59J Decision(59J단계 판정)

decision(판정): `{decision}`

Stage59J(59J단계)는 Stage59I demotion evidence(59I단계 강등 근거)를 사용한 bounded new model branch(경계 새 모델 분기)로 기록한다. Effect(효과): run50BQ transition source family(run50BQ 전환 원천 계열)의 성공/실패를 다음 bounded stage(경계 다음 단계)의 입력 근거로 넘긴다.
...
Stage59J closeout(59J단계 종료)는 overall goal completion(전체 목표 완료)이 아니다. Effect(효과): Stage60 ONNX hardening(60단계 ONNX 경화)은 adapter quality(어댑터 품질)가 강할 때만 열린다.
    ...
    engine.write_json(
        SUMMARY_JSON_PATH,
        {
            ...
            "required_outputs": {
                ...
                "stage59j_decision": rel(DECISION_PATH),
            },
...
def write_packet_files(
    ...
    files = {
        ...
        "experiment_design_receipt.json": {
            "hypothesis": "A bounded new model branch using run50BQ transition sources may improve post-ATR/risk validation and OOS behavior versus the demoted v54 repair line.",
            ...
            "comparison_baseline": "Stage59I demoted v54 repair line and Stage56 run50BQ transition evidence.",
            ...
            "stop_conditions": "three bounded Stage59J source-family variants only",
        ...
        "artifact_lineage_audit.json": {
            "source_inputs": [rel(SOURCE_STAGE59I_DECISION), rel(SOURCE_STAGE59I_REPORT), rel(SOURCE_STAGE59I_SUMMARY), rel(SOURCE_STAGE59I_JSON)],
...
- stage_id(단계 ID): `{STAGE59J_ID}`
- source_stage(원천 단계): `{SOURCE_STAGE59I_ID}`
- source_decision(원천 판정): `open_new_model_branch`
...
- stage59i_decision(59I단계 판정): `{rel(SOURCE_STAGE59I_DECISION)}`
- stage59i_report(59I단계 보고서): `{rel(SOURCE_STAGE59I_REPORT)}`
- stage59i_summary(59I단계 요약): `{rel(SOURCE_STAGE59I_SUMMARY)}`
- stage59i_pushed_commit(59I단계 푸시 커밋): `{SOURCE_STAGE59I_PUSHED_COMMIT}`
...
- run50bq_v62_source(run50BQ v62 원천): `{rel(RUN50BQ_ROOT / RUN50BQ_V62_ANCHOR)}`
- run50bq_v63_source(run50BQ v63 원천): `{rel(RUN50BQ_ROOT / RUN50BQ_V63_ANCHOR)}`

Effect(효과): Stage59J(59J단계)는 Stage59I(59I단계) 강등 근거와 run50BQ(run50BQ) 전환 원천을 명시적으로 연결한다.
""",
    )
    engine.write_md(
...
- stage_status(단계 상태): `closed_bounded_new_model_branch_from_stage59i`
- source_stage(원천 단계): `{SOURCE_STAGE59I_ID}`
- source_decision(원천 판정): `open_new_model_branch`
- stage59j_decision(59J단계 판정): `{decision}`
- next_stage_or_branch(다음 단계/분기): `{next_stage}`
- selected_research_baseline(선택 연구 기준선): `none`
- claim_boundary(주장 경계): `{BOUNDARY}`

Effect(효과): Stage59J(59J단계)는 측정 결과를 보존하지만 final package(최종 패키지)나 operating claim(운영 주장)을 만들지 않는다.
""",
    )
    engine.write_md(
        REVIEWS_ROOT / "review_index.md",
        f"""# Stage59J Review Index(59J단계 검토 색인)
...
- new_model_branch_risk_atr_telemetry(새 모델 분기 위험/ATR 텔레메트리): `{rel(RISK_ATR_TELEMETRY_PATH)}`
- stage59j_decision(59J단계 판정): `{rel(DECISION_PATH)}`
""",
    )
    if not next_stage.startswith("60_"):
        ...
        engine.write_md(
            ...
            f"""# Stage59K Brief(59K단계 개요)

- stage_id(단계 ID): `{next_stage}`
- source_stage(원천 단계): `{STAGE59J_ID}`
- source_decision(원천 판정): `{decision}`
- bounded_question(경계 질문): `What bounded follow-up should repair, demote, or branch from the Stage59J new-model evidence without starting ONNX prematurely?`
- boundary(경계): `{BOUNDARY}`

Stage59K(59K단계)는 Stage59J(59J단계) 결과가 Stage60 ONNX(60단계 ONNX)에 충분하지 않을 때 이어지는 bounded follow-up(경계 후속)이다. Effect(효과): repair(수리), demotion(강등), branch(분기)를 다음 작은 질문으로 제한한다.
""",
        )
        engine.write_md(
            next_root / "01_inputs/input_refs.md",
            f"""# Stage59K Input References(59K단계 입력 참조)

- stage59j_decision(59J단계 판정): `{rel(DECISION_PATH)}`
- stage59j_report(59J단계 보고서): `{rel(REPORT_PATH)}`
- stage59j_summary(59J단계 요약): `{rel(SUMMARY_CSV_PATH)}`
- stage59j_segment_kpi(59J단계 구간 KPI): `{rel(SEGMENT_KPI_PATH)}`
- stage59j_risk_atr_telemetry(59J단계 위험/ATR 텔레메트리): `{rel(RISK_ATR_TELEMETRY_PATH)}`
""",
        )
        engine.write_md(
            next_root / "03_reviews/review_index.md",
            """# Stage59K Review Index(59K단계 검토 색인)

Stage59K(59K단계)는 planned(계획) 상태다. Effect(효과): Stage59J(59J단계) 약점을 다음 bounded repair(경계 수리) 근거로 넘긴다.
""",
        )
        engine.write_md(
            next_root / "04_selected/selection_status.md",
            f"""# Stage59K Selection Status(59K단계 선택 상태)

- stage_status(단계 상태): `active_planned_from_stage59j`
- source_stage(원천 단계): `{STAGE59J_ID}`
- source_decision(원천 판정): `{decision}`
- selected_research_baseline(선택 연구 기준선): `none`
- claim_boundary(주장 경계): `{BOUNDARY}`

Effect(효과): Stage59K(59K단계)는 Stage59J(59J단계) 이후 필요한 다음 작은 질문만 다룬다.
""",
        )
...
- status(상태): `stage59j_closed_{decision}`
...
""",
    ...
    focus = (
        "current_focus:\n"
        f"- >-\n"
        f"  Stage59J(59J단계) `{STAGE59J_ID}` closed(종료) as bounded new model branch from Stage59I(59I단계 기반 경계 새 모델 분기); decision(판정)=`{decision}`. "
        ...
        f"- >-\n"
        f"  Next stage_or_branch(다음 단계/분기) `{next_stage}` is active/planned(활성/계획). Effect(효과): Stage59J(59J단계) 결과를 다음 bounded step(경계 다음 단계)으로 넘긴다.\n"
    )
    text = re.sub(
...
stage59j_new_model_branch_from_stage59i:
  packet_id: {PACKET_ID}
  stage_id: {STAGE59J_ID}
  status: closed_bounded_new_model_branch_from_stage59i
  ...
  boundary: {BOUNDARY}
"""
    if "stage59j_new_model_branch_from_stage59i:" in text:
        text = re.sub(r"\nstage59j_new_model_branch_from_stage59i:\n(?:  .*\n)*", block, text, count=1)
    else:
        text += block
...
def parse_args(argv: Sequence[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run Stage59J bounded new model branch from Stage59I.")
...
def load_existing_result() -> dict[str, Any]:
    ...
    if not path_exists(manifest) or not path_exists(kpi):
        raise FileNotFoundError("Stage59J existing run_manifest.json or kpi_record.json is missing")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.finalize_existing:
        ...
        prepared = {
            ...
            "model_family": "baseline_adapter_stage59j_new_model_branch_ebm_table",
            "feature_set_id": "stage59j_source_family_discrete_signal",
