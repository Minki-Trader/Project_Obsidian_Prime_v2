...
STAGE_ID = "235_adapter_research__side_specific_validation_net_recovery_after_session_context_tradeoff"
RUN_NUMBER = "run235A"
RUN_ID = "run235A_stage235_side_specific_validation_net_recovery_after_session_context_tradeoff_v1"
PACKET_ID = "stage235_side_specific_validation_net_recovery_after_session_context_tradeoff_v1"
PARENT_RUN_ID = "run234A_stage234_stage233_side_session_context_followup_review_v1"
SOURCE_STAGE_ID = "234_adapter_research__stage233_side_session_context_followup_review"
SOURCE_RUN_ID = PARENT_RUN_ID
SOURCE_STAGE234_EVIDENCE_COMMIT = "d27cac4723c76a9aa05c4d4f43fefd0ead359753"
SOURCE_STAGE234_HASH_RECORD_COMMIT = "fa2626fa61ccbcd868cc9dce1b81f2da6a43515b"
SOURCE_STAGE233_RUN_ID = "run233A_stage233_side_session_context_repair_after_lifecycle_failure_v1"
NEXT_STAGE_ID = "236_adapter_research__stage235_side_specific_followup_review"
NEXT_RUN_ID = "run236A_stage236_stage235_side_specific_followup_review_v1"
NEXT_PACKET_ID = "stage236_stage235_side_specific_followup_review_v1"
...
VALIDATION_RECOVERY_CLUE = {
    "adapter_id": "s233_cashopen_long_h3_cd8",
    "validation_net": 731.84,
    "validation_early_pf": 1.531640832,
    "validation_mid_pf": 1.678066129,
    "validation_dd": 9.6957,
    "oos_net": 602.79,
    "oos_pf": 1.85,
    "oos_dd": 12.8696,
}
...
COMMON_ROOT = f"OPV2/s235a/{RUN_NUMBER}"

SUMMARY_JSON_PATH = REVIEWS_ROOT / "stage235_summary.json"
SUMMARY_CSV_PATH = REVIEWS_ROOT / "stage235_side_specific_kpi_summary.csv"
REPORT_PATH = REVIEWS_ROOT / "stage235_side_specific_recovery_report.md"
SEGMENT_KPI_PATH = REVIEWS_ROOT / "stage235_segment_kpi_summary.csv"
BALANCE_CURVE_AUDIT_PATH = REVIEWS_ROOT / "stage235_balance_curve_audit.csv"
MONTHLY_KPI_PATH = REVIEWS_ROOT / "stage235_monthly_kpi_summary.csv"
CONCENTRATION_PATH = REVIEWS_ROOT / "stage235_concentration_risk_summary.csv"
DRAWDOWN_PATH = REVIEWS_ROOT / "stage235_drawdown_recovery_summary.csv"
QUALITY_MATRIX_PATH = REVIEWS_ROOT / "stage235_quality_matrix.csv"
RISK_ATR_TELEMETRY_PATH = REVIEWS_ROOT / "stage235_risk_atr_telemetry.csv"
GATE_FEATURE_SUMMARY_PATH = REVIEWS_ROOT / "stage235_gate_feature_summary.csv"
PROBABILITY_BINDING_PATH = REVIEWS_ROOT / "stage235_probability_telemetry_summary.csv"
MODEL_SCORE_AUDIT_PATH = REVIEWS_ROOT / "stage235_model_score_audit.csv"
TIER_B_DIAGNOSTIC_PATH = REVIEWS_ROOT / "stage235_tier_b_diagnostic_summary.csv"
DECISION_PATH = REVIEWS_ROOT / "stage235_decision.md"
AUDIT_CSV_PATH = REVIEWS_ROOT / "stage235_trade_audit.csv"
...
PRODUCER_PATH = Path("stage_pipelines/stage235/side_specific_validation_net_recovery_after_session_context_tradeoff.py")
ARTIFACT_COLUMNS = prior.ARTIFACT_COLUMNS

VARIANTS = (
    repair.RepairVariant(
        adapter_id="s235_session_ref_h3_cd8",
        label="stage235_session_ref_h3_cd8",
        atr_enabled=True,
        ...
        max_hold_bars=3,
        notes="Stage235 control: Stage233 OOS reference bound.",
    ),
    repair.RepairVariant(
        adapter_id="s235_cashopen45_h3_cd8",
        label="stage235_cashopen45_h3_cd8",
        atr_enabled=True,
        model_risk_enabled=True,
        fixed_lot=0.25,
        atr_stop_multiplier=2.0325,
        ...
        close_only_on_opposite_signal=False,
        max_hold_bars=3,
        notes="Stage235 repair: half cash-open long guard, short block unchanged.",
    ),
    repair.RepairVariant(
        adapter_id="s235_session_ref_short_open_h3_cd8",
        label="stage235_session_ref_short_open_h3_cd8",
        atr_enabled=True,
        model_risk_enabled=True,
        fixed_lot=0.25,
        atr_stop_multiplier=2.0325,
        ...
        close_only_on_opposite_signal=False,
        max_hold_bars=3,
        notes="Stage235 repair: short-side block relief with OOS reference long rule.",
    ),
    repair.RepairVariant(
        adapter_id="s235_cashopen45_short_open_h3_cd8",
        label="stage235_cashopen45_short_open_h3_cd8",
        atr_enabled=True,
        model_risk_enabled=True,
        fixed_lot=0.25,
        atr_stop_multiplier=2.0325,
        ...
        close_only_on_opposite_signal=False,
        max_hold_bars=3,
        notes="Stage235 repair: cash-open clue plus short-side block relief.",
    ),
)

BASE_EXTRA = {**prior.BASE_EXTRA, "block_mode": "encoded_side_context"}
VARIANT_EXTRAS: dict[str, dict[str, Any]] = {
    "s235_session_ref_h3_cd8": {
        **BASE_EXTRA,
        "short_block_rule": "midwide_lowedge",
        "long_block_rule": "session_only",
        "axis": "session_ref_h3_cd8",
    },
    "s235_cashopen45_h3_cd8": {
        **BASE_EXTRA,
        "short_block_rule": "midwide_lowedge",
        "long_block_rule": "cashopen_guard",
        "axis": "cashopen45_h3_cd8",
        "cashopen_min": 0.0,
        "cashopen_max": 45.0,
    },
    "s235_session_ref_short_open_h3_cd8": {
        **BASE_EXTRA,
        "short_block_rule": "none",
        "long_block_rule": "session_only",
        "axis": "session_ref_short_open_h3_cd8",
    },
    "s235_cashopen45_short_open_h3_cd8": {
        **BASE_EXTRA,
        "short_block_rule": "none",
        "long_block_rule": "cashopen_guard",
        "axis": "cashopen45_short_open_h3_cd8",
        "cashopen_min": 0.0,
        "cashopen_max": 45.0,
    },
}
...
def cashopen_hit(row: Mapping[str, str], spec: Mapping[str, Any]) -> bool:
    ...
    if minutes is None:
        return False
    return float(spec["cashopen_min"]) <= float(minutes) <= float(spec["cashopen_max"])
...
def gate_value(row: Mapping[str, str], variant: repair.RepairVariant) -> float:
    ...
    if signal > 0:
        ...
        if rule == "session_only" and s210.bounded_lowedge_hit(row, spec, "", use_margin=False):
            return 2.0
        if rule == "session_only_p5" and s210.bounded_lowedge_hit(row, spec, "wide", use_margin=False):
            return 2.0
        if rule == "session_only_p10" and s210.bounded_lowedge_hit(row, spec, "tight", use_margin=False):
            return 2.0
...
def apply_stage_values(context_specs: Mapping[str, Any]) -> None:
    values = {
        ...
        "SOURCE_ADAPTER_ID": "stage235_side_specific_validation_net_recovery",
    ...
    for variant_index, variant in enumerate(VARIANTS, start=1):
        ...
        for split in ("validation_is", "oos"):
            ...
            for role_index, (tier, attempt_role, prefix, attempt_token) in enumerate(
                ...
                magic = 23510000 + variant_index * 100 + (1 if split == "validation_is" else 50) + role_index
                attempts.append(
                    s161.base.attempt_payload(
                        run_root=variant_root,
                        run_id=RUN_ID,
                        stage_number=235,
                        exploration_label="stage235_BaselineAdapter__SideSpecificValidationNetRecoveryAfterSessionContextTradeoff",
...
def configure_runner() -> None:
    ...
    for variant in VARIANTS:
        ...
        context_specs[variant.adapter_id] = {
            "gate_column": f"stage235_gate_{extra['axis']}",
            "gate_type": "encoded_stage235_side_specific_validation_net_recovery_after_session_context_tradeoff",
            ...
            "cashopen_min": float(extra.get("cashopen_min", 0.0)),
            "cashopen_max": float(extra.get("cashopen_max", 90.0)),
            "description": "Stage235 encoded gate: side-specific validation net recovery after session/context tradeoff.",
        }
...
def pass_stage233(row: Mapping[str, Any]) -> bool:
    return (
...
def decide(rows: Sequence[Mapping[str, Any]], external: str) -> str:
    if external != "completed":
        return "continue_stage235_runtime_completion_due_to_incomplete_runtime_candidate_not_final"
    if any(pass_stage233(row) for row in rows):
        return "open_stage236_stage235_side_specific_followup_review_candidate_not_final"
    return "open_stage236_bounded_followup_due_to_side_specific_tradeoff_candidate_not_final"
...
def report_markdown(rows: Sequence[Mapping[str, Any]], decision: str, external: str) -> str:
    best = best_row(rows)
    return f"""# Stage235 Side-Specific Validation Net Recovery Report(235단계 방향별 검증 순손익 회복 보고서)
...
- full_stage_pass(전체 통과): `{any(pass_stage233(row) for row in rows)}`
...
def decision_markdown(decision: str, external: str) -> str:
    return f"""# Stage235 Decision(235단계 판정)
...
- source_stage(원천 단계): `{SOURCE_STAGE_ID}`
- source_run(원천 실행): `{SOURCE_RUN_ID}`
- source_stage234_evidence_commit(원천 234단계 근거 커밋): `{SOURCE_STAGE234_EVIDENCE_COMMIT}`
- source_stage234_hash_record_commit(원천 234단계 해시 기록 커밋): `{SOURCE_STAGE234_HASH_RECORD_COMMIT}`
- external_verification_status(외부 검증 상태): `{external}`
...
Stage235(235단계) closeout(종료)는 overall goal complete(전체 목표 완료)가 아니다.

Effect(효과): Stage236(236단계) follow-up review(후속 검토)에서 side-specific validation net recovery(방향별 검증 순손익 회복)의 KPI(핵심 성과 지표) 상충과 다음 bounded repair(경계 수리) 축을 분리 판정한다.
"""
...
def artifact_rows(result: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    for row in rows:
        if row.get("artifact_type") != "mt5_strategy_tester_report":
            row["artifact_type"] = "stage235_evidence"
            row["notes"] = "Stage235 side-specific validation net recovery evidence."
        else:
            row["notes"] = "Stage235 MT5 Strategy Tester report."
    return rows
        ...
        [
            {
                "run_id": RUN_ID,
                "stage_id": STAGE_ID,
                "lane": "baseline_adapter_stage235_side_specific_validation_net_recovery",
                "status": status,
                "judgment": decision,
                "path": rel(DECISION_PATH),
                "notes": s172.ledger_pairs(
                    (
                        ("source_stage234_evidence_commit", SOURCE_STAGE234_EVIDENCE_COMMIT),
                        ("source_stage234_hash_record_commit", SOURCE_STAGE234_HASH_RECORD_COMMIT),
...
def write_packet_files(result: Mapping[str, Any], decision: str, ledger_payload: Mapping[str, Any], quality: Sequence[Mapping[str, Any]]) -> None:
    ...
    s172.write_md(
        ...
        f"""# Stage235 Closeout Packet(235단계 종료 작업 묶음)
...
Stage236(236단계)는 Stage235(235단계) side-specific validation net recovery(방향별 검증 순손익 회복) 결과를 follow-up review(후속 검토)하는 bounded review(경계 검토) 단계다.

## Bounded Question(경계 질문)

Did Stage235(235단계) recover validation net/early PF(검증 순손익/초반 수익요인) without losing the OOS reference bound(표본외 기준 경계), model-controlled risk%(모델 제어 위험 비율), and ATR SL/TP(ATR 손절/익절)?

Effect(효과): Stage235(235단계) 안에서 다음 수리를 흡수하지 않고, 방향별 수리의 KPI(핵심 성과 지표) 상충을 별도 검토로 닫는다.
...
- status(상태): `open_planned_from_stage235`
- source_stage(원천 단계): `{STAGE_ID}`
- source_decision(원천 판정): `{decision}`
...
- stage_status(단계 상태): `open_planned_from_stage235`
...
- >-
  Stage235(235단계) closed(종료) as `{decision}` and Stage236(236단계) `{NEXT_STAGE_ID}` is open_planned(개방 계획). Effect(효과): side-specific validation net recovery(방향별 검증 순손익 회복)의 KPI(핵심 성과 지표) 상충을 별도 review(검토)로 판정한다.
- >-
  Stage235 evidence(235단계 근거)는 `{rel(REPORT_PATH)}`, `{rel(SUMMARY_CSV_PATH)}`, `{rel(QUALITY_MATRIX_PATH)}`, `{rel(MONTHLY_KPI_PATH)}`, `{rel(CONCENTRATION_PATH)}`, `{rel(RISK_ATR_TELEMETRY_PATH)}`에 있다. Effect(효과): cashopen 45분(현금장 초반 45분)과 short block off(숏 차단 해제)가 validation/OOS(검증/표본외)를 함께 끌어올렸는지 확인한다.
- >-
...
stage236_stage235_side_specific_followup_review:
  ...
  status: open_planned_from_stage235
...
- adapter_under_review(검토 중 어댑터): `stage235_side_specific_validation_net_recovery_after_session_context_tradeoff`
- status(상태): `stage235_{decision}`
- claim_boundary(주장 경계): `{BOUNDARY}`

Stage235(235단계)는 side-specific validation net recovery(방향별 검증 순손익 회복)를 측정했다. Effect(효과): Stage236(236단계)가 결과 상충과 다음 bounded repair(경계 수리)를 별도 review(검토)로 판정한다.

## Latest Stage235 Evidence(최신 235단계 근거)
...
def append_changelog(decision: str) -> None:
    ...
    if RUN_ID in existing:
        return
    entry = (
        f"\n## {s172.utc_now()} Stage235 side-specific validation net recovery closeout(235단계 방향별 검증 순손익 회복 종료)\n\n"
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    prepared = {
        ...
        "model_family": "baseline_adapter_stage235_v2_native_side_specific_validation_net_recovery",
        "feature_set_id": "stage235_signal_plus_side_specific_validation_net_recovery",
