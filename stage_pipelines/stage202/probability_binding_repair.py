...
STAGE_ID = "202_adapter_research__stage200_probability_binding_repair"
RUN_NUMBER = "run202A"
RUN_ID = "run202A_stage202_stage200_probability_binding_repair_v1"
PACKET_ID = "stage202_stage200_probability_binding_repair_v1"
PARENT_RUN_ID = "run201A_stage201_stage200_mid_drawdown_entry_quality_followup_review_v1"
SOURCE_STAGE_ID = "201_adapter_research__stage200_mid_drawdown_entry_quality_followup_review"
SOURCE_RUN_ID = "run201A_stage201_stage200_mid_drawdown_entry_quality_followup_review_v1"
SOURCE_STAGE201_EVIDENCE_COMMIT = "9d6c0635315bf0ded42a287499ef5c634b0be8ca"
SOURCE_STAGE201_HASH_RECORD_COMMIT = "22a6eab97daff93e781b350cab670073c1a406f2"
SOURCE_ADAPTER_ID = "s200_cd8_ref_r0325"
NEXT_STAGE_ID = "203_adapter_research__stage202_probability_binding_followup_review"
NEXT_RUN_ID = "run203A_stage203_stage202_probability_binding_followup_review_v1"
NEXT_PACKET_ID = "stage203_stage202_probability_binding_followup_review_v1"
...
COMMON_ROOT = f"OPV2/s202a/{RUN_NUMBER}"

SUMMARY_JSON_PATH = REVIEWS_ROOT / "stage202_probability_binding_summary.json"
SUMMARY_CSV_PATH = REVIEWS_ROOT / "stage202_probability_binding_kpi_summary.csv"
REPORT_PATH = REVIEWS_ROOT / "stage202_probability_binding_report.md"
SEGMENT_KPI_PATH = REVIEWS_ROOT / "stage202_segment_kpi_summary.csv"
BALANCE_CURVE_AUDIT_PATH = REVIEWS_ROOT / "stage202_balance_curve_audit.csv"
MONTHLY_KPI_PATH = REVIEWS_ROOT / "stage202_monthly_kpi_summary.csv"
CONCENTRATION_PATH = REVIEWS_ROOT / "stage202_concentration_risk_summary.csv"
DRAWDOWN_PATH = REVIEWS_ROOT / "stage202_drawdown_recovery_summary.csv"
QUALITY_MATRIX_PATH = REVIEWS_ROOT / "stage202_quality_matrix.csv"
RISK_ATR_TELEMETRY_PATH = REVIEWS_ROOT / "stage202_risk_atr_telemetry.csv"
GATE_FEATURE_SUMMARY_PATH = REVIEWS_ROOT / "stage202_gate_feature_summary.csv"
PROBABILITY_BINDING_PATH = REVIEWS_ROOT / "stage202_probability_binding_telemetry_summary.csv"
MODEL_SCORE_AUDIT_PATH = REVIEWS_ROOT / "stage202_model_score_audit.csv"
TIER_B_DIAGNOSTIC_PATH = REVIEWS_ROOT / "stage202_tier_b_diagnostic_summary.csv"
DECISION_PATH = REVIEWS_ROOT / "stage202_decision.md"
AUDIT_CSV_PATH = REVIEWS_ROOT / "stage202_trade_audit.csv"
...
PRODUCER_PATH = Path("stage_pipelines/stage202/probability_binding_repair.py")
ARTIFACT_COLUMNS = s172.ARTIFACT_COLUMNS

VARIANTS = (
    repair.RepairVariant(
        adapter_id="s202_cd8_ref_r0325",
        label="stage202_cd8_ref_r0325",
        atr_enabled=True,
        ...
        max_hold_bars=3,
        notes="Stage202 reference: Stage200 best shape with cd8/r0325 and fixed risk/ATR settings.",
    ),
    repair.RepairVariant(
        adapter_id="s202_cd8_shortcut_r0325",
        label="stage202_cd8_shortcut_r0325",
        atr_enabled=True,
        model_risk_enabled=True,
        fixed_lot=0.25,
        atr_stop_multiplier=2.075,
        ...
        close_only_on_opposite_signal=False,
        max_hold_bars=3,
        notes="Stage202 probability binding repair: raise short threshold above the observed probability plateau.",
    ),
    repair.RepairVariant(
        adapter_id="s202_cd8_longcut_r0325",
        label="stage202_cd8_longcut_r0325",
        atr_enabled=True,
        model_risk_enabled=True,
        fixed_lot=0.25,
        atr_stop_multiplier=2.075,
        ...
        close_only_on_opposite_signal=False,
        max_hold_bars=3,
        notes="Stage202 probability binding repair: raise long threshold above the observed probability plateau.",
    ),
    repair.RepairVariant(
        adapter_id="s202_cd8_bothcut_r0325",
        label="stage202_cd8_bothcut_r0325",
        atr_enabled=True,
        model_risk_enabled=True,
        fixed_lot=0.25,
        atr_stop_multiplier=2.075,
        ...
        close_only_on_opposite_signal=False,
        max_hold_bars=3,
        notes="Stage202 probability binding repair: raise both thresholds as a binding proof control.",
    ),
)
...
VARIANT_EXTRAS: dict[str, dict[str, Any]] = {
    ...
    "s202_cd8_shortcut_r0325": {
        **BASE_EXTRA,
        "short_block_rule": "midwide_lowedge",
        "long_block_rule": "lowedge_gate",
        "axis": "shortcut58",
    },
    "s202_cd8_longcut_r0325": {
        **BASE_EXTRA,
        "short_block_rule": "midwide_lowedge",
        "long_block_rule": "lowedge_gate",
        "axis": "longcut58",
    },
    "s202_cd8_bothcut_r0325": {
        **BASE_EXTRA,
        "short_block_rule": "midwide_lowedge",
        "long_block_rule": "lowedge_gate",
        "axis": "bothcut58",
    ...
    for variant_index, variant in enumerate(VARIANTS, start=1):
        ...
        for split in ("validation_is", "oos"):
            ...
            for role_index, (tier, attempt_role, prefix, attempt_token) in enumerate(
                ...
                magic = 20210000 + variant_index * 100 + (1 if split == "validation_is" else 50) + role_index
                attempts.append(
                    s161.base.attempt_payload(
                        run_root=variant_root,
                        run_id=RUN_ID,
                        stage_number=202,
                        exploration_label="stage202_BaselineAdapter__ProbabilityBindingRepair",
...
def configure_runner() -> None:
    ...
    for variant in VARIANTS:
        ...
        context_specs[variant.adapter_id] = {
            "gate_column": f"stage202_gate_{extra['axis']}",
            "gate_type": "encoded_stage202_probability_binding_repair",
            ...
            "description": "Stage202 encoded gate: probability binding repair without broad qwide filter.",
        }
...
def decide(rows: Sequence[Mapping[str, Any]], external: str) -> str:
    if external != "completed":
        return "continue_stage202_runtime_completion_due_to_incomplete_runtime_candidate_not_final"
    if any(row.get("hard_quality_pass") for row in rows):
        return "open_stage203_stage202_probability_binding_followup_review_candidate_not_final"
    return "open_stage203_bounded_followup_due_to_probability_binding_tradeoff_candidate_not_final"
...
def report_markdown(rows: Sequence[Mapping[str, Any]], decision: str, external: str) -> str:
    best = best_row(rows)
    ref = next((row for row in rows if row.get("adapter_id") == "s202_cd8_ref_r0325"), {})
    return f"""# Stage202 Probability Binding Repair Report(202단계 확률 구속 수리 보고서)
...
- source_stage201_evidence_commit(원천 201단계 근거 커밋): `{SOURCE_STAGE201_EVIDENCE_COMMIT}`
- source_stage201_hash_record_commit(원천 201단계 해시 기록 커밋): `{SOURCE_STAGE201_HASH_RECORD_COMMIT}`
...
def decision_markdown(decision: str, external: str) -> str:
    return f"""# Stage202 Decision(202단계 판정)
...
Stage202(202단계) closeout(종료)은 overall goal complete(전체 목표 완료)가 아니다. Effect(효과): Stage203(203단계) follow-up review(후속 검토)에서 probability binding(확률 구속) 상충을 따로 판정한다.
"""
...
def artifact_rows(result: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    for row in rows:
        if row.get("artifact_type") != "mt5_strategy_tester_report":
            row["artifact_type"] = "stage202_evidence"
            row["notes"] = "Stage202 probability binding repair evidence."
        else:
            row["notes"] = "Stage202 MT5 Strategy Tester report."
    return rows
        ...
        [
            {
                "run_id": RUN_ID,
                "stage_id": STAGE_ID,
                "lane": "baseline_adapter_stage202_probability_binding_repair",
                "status": status,
                "judgment": decision,
                "path": rel(DECISION_PATH),
                "notes": s172.ledger_pairs(
                    (
                        ("source_stage201_evidence_commit", SOURCE_STAGE201_EVIDENCE_COMMIT),
                        ("source_stage201_hash_record_commit", SOURCE_STAGE201_HASH_RECORD_COMMIT),
        ...
        alpha_rows = [
            {
                ...
                "tier_scope": "Tier A+B",
                "kpi_scope": "stage202_probability_binding_repair",
                ...
                "external_verification_status": external,
                "notes": "Stage202 materialized or blocked before KPI records were available.",
            }
        ]
...
def write_packet_files(result: Mapping[str, Any], decision: str, ledger_payload: Mapping[str, Any], quality: Sequence[Mapping[str, Any]]) -> None:
    ...
    s172.write_md(
        ...
        f"""# Stage202 Closeout Packet(202단계 종료 작업 묶음)
...
Stage203(203단계)는 Stage202(202단계) probability binding repair(확률 구속 수리) 결과를 follow-up review(후속 검토)한다.

## Bounded Question(경계 질문)

Did Stage202(202단계) side-specific binding thresholds(방향별 구속 문턱값) improve validation DD/mid PF(검증 낙폭/중반 수익요인) without damaging validation net/PF(검증 순손익/수익요인), trade supply(거래 공급), OOS(표본외), and risk/ATR telemetry(위험/ATR 기록)?

Effect(효과): Stage202(202단계) 안에서 추가 수리를 하지 않고, probability binding(확률 구속) 측정 결과를 별도 판독 단계에서 과장 없이 판단한다.
...
- status(상태): `open_planned_from_stage202`
- source_stage(원천 단계): `{STAGE_ID}`
- source_decision(원천 판정): `{decision}`
...
- stage_status(단계 상태): `open_planned_from_stage202`
...
- >-
  Stage202(202단계) closed(종료) as `{decision}` and Stage203(203단계) `{NEXT_STAGE_ID}` is open_planned(개방 계획). Effect(효과): probability binding(확률 구속) 결과를 별도 follow-up review(후속 검토)로 넘긴다.
- >-
  Stage202 evidence(202단계 근거)는 `{rel(REPORT_PATH)}`, `{rel(SUMMARY_CSV_PATH)}`, `{rel(QUALITY_MATRIX_PATH)}`, `{rel(BALANCE_CURVE_AUDIT_PATH)}`, `{rel(RISK_ATR_TELEMETRY_PATH)}`에 있다. Effect(효과): 실제 구속된 threshold(문턱값)가 DD/PF/OOS(낙폭/수익요인/표본외)에 준 영향을 같이 본다.
- >-
...
"""
    if re.search(r"(?ms)^current_focus:\r?\n.*?(?=\r?\nstage\d+_)", state):
        ...
        state = state.rstrip() + "\n" + focus
    state = re.sub(r"(?ms)^stage202_probability_binding_repair:\r?\n.*?(?=^stage\d+_|\Z)", "", state)
    block = f"""
stage202_probability_binding_repair:
  packet_id: {PACKET_ID}
  stage_id: {STAGE_ID}
...
- adapter_under_review(검토 중 어댑터): `stage202_probability_binding_repair`
- status(상태): `stage202_{decision}`
- claim_boundary(주장 경계): `{BOUNDARY}`

Stage202(202단계)는 Stage201(201단계) 판정에 따라 probability binding(확률 구속) 수리를 MT5 Strategy Tester(메타트레이더5 전략 테스터)로 측정했다. Effect(효과): Stage203(203단계)가 KPI(핵심 성과 지표) 상충을 별도 review(검토)로 판정한다.

## Latest Stage202 Evidence(최신 202단계 근거)
...
def append_changelog(decision: str) -> None:
    ...
    if RUN_ID in existing:
        return
    entry = (
        f"\n## {s172.utc_now()} Stage202 probability binding repair closeout(202단계 확률 구속 수리 종료)\n\n"
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    prepared = {
        ...
        "model_family": "baseline_adapter_stage202_v2_native_probability_binding_repair",
        "feature_set_id": "stage202_signal_plus_encoded_probability_binding_gate",
