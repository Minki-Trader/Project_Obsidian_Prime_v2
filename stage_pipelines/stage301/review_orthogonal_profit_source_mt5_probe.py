...
NEXT_STAGE_ID = "302_onnx_candidate_campaign__payoff_convexity_profit_scale_rebuild"
RUN_ID = "run301C_review_orthogonal_profit_source_mt5_probe_v1"
RUN_NUMBER = "run301C"
SOURCE_RUN_ID = "run301B_orthogonal_profit_source_mt5_probe_v1"
PARENT_RUN_ID = "run301A_design_orthogonal_profit_source_rebuild_v1"
...
RUN301A = STAGE_ROOT / "02_runs" / "run301A"
RUN301B = STAGE_ROOT / "02_runs" / "run301B"
...
SOURCE_EXECUTION = RUN301B / "execution_result.json"
SOURCE_KPI = RUN301B / "mt5_kpi_summary.csv"
SOURCE_SCOUT = RUN301A / "model_scout_scoreboard.csv"
PRODUCER = Path("stage_pipelines/stage301/review_orthogonal_profit_source_mt5_probe.py")

SCOREBOARD = RUN_ROOT / "orthogonal_profit_source_review_scoreboard.csv"
...
NEXT_STAGE_QUEUE = RUN_ROOT / "stage302_seed_queue.csv"
RESULT_JUDGMENT = RUN_ROOT / "result_judgment.csv"
GATE_AUDIT = RUN_ROOT / "required_gate_coverage_audit.csv"
RUN_MANIFEST = RUN_ROOT / "run_manifest.json"
LINEAGE = RUN_ROOT / "artifact_lineage_receipt.json"
REPORT = REVIEWS / "run301C_orthogonal_profit_source_review_stage302_open_report.md"
DECISION = ROOT / "docs" / "decisions" / "2026-05-24_stage301_orthogonal_profit_source_review_stage302_open.md"
...
def feature_trading_days(attempt: Mapping[str, Any]) -> int:
    ...
    local = RUN301B / "features" / Path(feature_path).name
...
def load_actual_rows() -> tuple[list[dict[str, Any]], dict[str, dict[str, Any]]]:
    ...
    with ledger.io_path(SOURCE_KPI).open("r", encoding="utf-8-sig", newline="") as handle:
        for source_row in csv.DictReader(handle):
            ...
            materialized_id = str(attempt.get("stage301_branch_id") or attempt.get("materialized_branch_id") or "")
...
def build_scoreboard(rows: Sequence[Mapping[str, Any]], scout_rows: Mapping[str, Mapping[str, Any]]) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    ...
    for candidate_id, split_rows in sorted(by_candidate.items()):
        ...
        if not selected:
            ...
            failure_rows.append(
                {
                    "failure_id": f"{candidate_id}_stage301_negative_or_watch",
                    "materialized_branch_id": candidate_id,
                    ...
                    "salvage_value": "cp301E/cp301F처럼 positive-but-small(양수지만 작은) 표본은 payoff convexity(보상 볼록성)와 risk/reward asymmetry(위험/보상 비대칭) 새 논제의 입력 단서로만 쓴다.",
                    "reopen_condition": "validation/OOS(검증/표본외) 모두 충분한 순수익, 4-10 trades/day(일 4-10거래), PF(수익요인), recovery(회복), 확대 곡선 품질을 동시에 보여야 한다.",
                    "do_not_repeat": "HGB inverse-return threshold(HGB 역방향 수익률 임계값)만 좁게 조정하는 repair loop(수정 반복)는 하지 않는다.",
                    "claim_boundary": BOUNDARY,
                }
            )
...
def stage302_queue_rows(scoreboard: Sequence[Mapping[str, Any]]) -> list[dict[str, Any]]:
    ...
    for index, row in enumerate(best, start=1):
        rows.append(
            {
                "seed_id": f"stage302_seed_{index:02d}",
                "source_stage_id": STAGE_ID,
                "source_run_id": RUN_ID,
                "source_package_id": row.get("package_id", ""),
                "seed_role": "positive_but_not_worthy(양수지만 가치 부족)",
                "hypothesis": "payoff convexity(보상 볼록성)와 risk/reward asymmetry(위험/보상 비대칭)를 새로 설계하면 작은 양수 edge(우위)를 수익 규모(profit scale, 수익 규모)로 키울 수 있는지 본다.",
                "broad_sweep": "feature construction(피처 구성), model variants(모델 변형), hold/exit logic(보유/청산 로직), risk gating(위험 게이팅)을 제한 없이 탐색한다.",
                "aggressive_sweep": "6-10 trades/day(일 6-10거래)에서 net profit(순수익)과 PF(수익요인)를 먼저 키운다.",
                "defensive_sweep": "local drawdown pockets(국소 손실 함몰), weak month(약한 월), weak session(약한 세션)을 동시에 누른다.",
                "discard_condition": "actual MT5(실제 메타트레이더5)에서 OOS(표본외) 수익 규모나 곡선 품질이 다시 작게 남으면 폐기한다.",
                "prior_stage_refs": "stage300_negative;stage301_positive_small_no_scale",
                "claim_boundary": BOUNDARY,
            }
        )
    if not rows:
        rows.append(
            {
                "seed_id": "stage302_seed_fresh_payoff_convexity",
                "source_stage_id": STAGE_ID,
                "source_run_id": RUN_ID,
                "source_package_id": "none",
                "seed_role": "fresh_thesis(새 논제)",
                "hypothesis": "orthogonal profit source(직교 수익 원천)가 실패하면 payoff surface(보상 표면)를 새로 만든다.",
...
def scaffold_stage302() -> None:
    ...
    write_text(
        ...
        "\n".join(
            [
                "# Stage302 Brief(302단계 개요)",
                "",
                f"- stage_id(단계 ID): `{NEXT_STAGE_ID}`",
                f"- source_stage(원천 단계): `{STAGE_ID}`",
                f"- source_run(원천 실행): `{RUN_ID}`",
                "- question(질문): Can payoff convexity(보상 볼록성) and risk/reward asymmetry(위험/보상 비대칭) turn the small positive MT5(메타트레이더5) edge(우위) from Stage301(301단계) into enough profit scale(수익 규모), 4-10 trades/day(일 4-10거래), and smooth equity curve(매끄러운 평가금 곡선)?",
                f"- boundary(경계): `{BOUNDARY}`",
                "",
                "Effect(효과): HGB inverse-return(HGB 역방향 수익률) threshold repair(임계값 수정)를 반복하지 않고, 보상/청산/위험 표면을 새로 설계한다.",
            ]
        ),
    )
    write_text(
        ...
        "\n".join(
            [
                "# Stage302 Selection Status(302단계 선택 상태)",
                "",
                "- stage_status(단계 상태): `opened_payoff_convexity_profit_scale_rebuild`",
                "- current_packet(현재 작업 묶음): `302_onnx_candidate_campaign__payoff_convexity_profit_scale_rebuild_v1`",
                ...
                f"- stage301_review(301단계 검토): `{rel(REPORT)}`",
            ]
        ),
    )
    write_text(
    ...
    write_csv(
        ...
        [
            {
                "row_id": "stage302_opened_from_run301C",
                "stage_id": NEXT_STAGE_ID,
                "run_id": RUN_ID,
                "view": "stage_open",
                "tier_scope": "not_applicable",
                "scoreboard": "stage301_review",
                "status": "opened_payoff_convexity_profit_scale_rebuild",
                "judgment": "opened_from_stage301_actual_mt5_positive_small_no_onnx_worthy_candidate",
                "evidence_boundary": "planning_from_stage301_actual_mt5_evidence",
                "report_path": rel(REPORT),
                "notes": "next_action=run302A_design_payoff_convexity_profit_scale_rebuild_packet",
            }
        ],
    )
...
def update_docs(status: str, judgment: str, next_action: str) -> None:
    ...
    selected += f"- run301C_report(301C 보고): `{rel(REPORT)}`\n"
    selected += f"- stage302_opened(302단계 열림): `{NEXT_STAGE_ID}`\n"
    write_text(SELECTED, selected)

    review_index = read_text(REVIEW_INDEX)
    review_index += f"- run301C_report(301C 보고): `{rel(REPORT)}`\n"
    review_index += f"- run301C_scoreboard(301C 점수판): `{rel(SCOREBOARD)}`\n"
    review_index += f"- stage302_seed_queue(302단계 씨앗 대기열): `{rel(NEXT_STAGE_QUEUE)}`\n"
    ...
    current += "- run301C_summary(301C 요약): Stage301(301단계)은 actual MT5(실제 메타트레이더5) 양수 단서를 만들었지만 ONNX-worthy(온엑스 가치 있음) 조건에는 부족했다. Effect(효과): Adapter(어댑터)와 ONNX(온엑스)를 시작하지 않고 Stage302(302단계) payoff convexity(보상 볼록성) 질문으로 전환했다.\n"
    ...
    focus = (
        f"- >-\n"
        f"  Stage301(301단계) run301C(301C 실행) orthogonal profit source review(직교 수익 원천 검토) `{RUN_ID}` closed Stage301 and opened Stage302(302단계). "
        f"Effect(효과): selected candidate(선택 후보), Adapter package(어댑터 패키지), ONNX readiness(온엑스 준비)는 없고 next_action(다음 행동)은 `run302A_design_payoff_convexity_profit_scale_rebuild_packet`이다.\n"
    )
    ...
    changelog += (
        f"\n## {UPDATED_ON} run301C orthogonal profit source review(301C 직교 수익 원천 검토)\n\n"
        f"- status(상태): `{status}`\n"
...
def update_registers(status: str, judgment: str, next_action: str) -> None:
    safe_upsert(
        ...
        [{"run_id": RUN_ID, "stage_id": STAGE_ID, "lane": "orthogonal_profit_source_review", "status": status, "judgment": judgment, "path": rel(REPORT), "notes": f"selected_candidate=none;adapter_package=none;onnx_readiness=not_started;next_action={next_action}."}],
        "run_id",
    )
    safe_upsert(
        ALPHA_LEDGER,
        ...
        [
            {
                ...
                "record_view": "orthogonal_profit_source_review",
    ...
    safe_upsert(
        STAGE_LEDGER,
        STAGE_LEDGER_COLUMNS,
        [{"row_id": f"{RUN_ID}__review", "stage_id": STAGE_ID, "run_id": RUN_ID, "view": "orthogonal_profit_source_review", "tier_scope": "Tier A used/Tier B fallback/actual routed total", "scoreboard": "orthogonal_profit_source_review_scoreboard", "status": status, "judgment": judgment, "evidence_boundary": "runtime_probe_review_no_candidate_no_onnx", "report_path": rel(REPORT), "notes": "Stage302 opened; no Adapter(어댑터); no ONNX(온엑스)."}],
        "row_id",
    )
...
def update_memory_registers(failures: Sequence[Mapping[str, Any]]) -> None:
    ...
    if RUN_ID not in idea:
        idea += (
            f"\n## {RUN_ID} payoff convexity handoff(보상 볼록성 인계)\n\n"
    ...
    if RUN_ID not in negative:
        negative += (
            f"\n## {RUN_ID} Stage301 positive-small failure memory(301단계 양수-소규모 실패 기억)\n\n"
        ...
        rows.append(
            {
                ...
                "artifact_type": "stage301_orthogonal_profit_source_review_artifact",
                "path": rel(path),
                "sha256": ledger.sha256_file_lf_normalized(path),
                "stage_id": STAGE_ID,
                "run_id": RUN_ID,
                "created_at_utc": "2026-05-24T20:50:00Z",
                "notes": "Stage301 review and Stage302 open handoff",
            }
        )
...
def main() -> None:
    ...
    stage302_rows = stage302_queue_rows(scoreboard)
    status = "completed_orthogonal_profit_source_review_no_candidate_stage302_opened"
    judgment = "actual_mt5_positive_small_but_not_onnx_worthy_no_adapter_no_onnx"
    next_action = "run302A_design_payoff_convexity_profit_scale_rebuild_packet"
    ...
    write_csv(NEXT_STAGE_QUEUE, list(stage302_rows[0].keys()), stage302_rows)
    write_csv(
        RESULT_JUDGMENT,
    ...
    write_text(
        ...
        "\n".join(
            [
                ...
                "",
                "Effect(효과): 작은 양수 edge(우위)는 보존하지만 ONNX-worthy(온엑스 가치 있음) 후보로 선언하지 않고, payoff convexity(보상 볼록성) 새 단계로 넘긴다.",
            ]
        ),
    )
    scaffold_stage302()
