...
RUN_ID = "run301B_orthogonal_profit_source_mt5_probe_v1"
RUN_NUMBER = "run301B"
SOURCE_RUN_ID = "run301A_design_orthogonal_profit_source_rebuild_v1"
PARENT_RUN_ID = "run300C_review_split_forward_trade_shape_generalization_mt5_probe_v1"
STATUS_PREPARED = "prepared_orthogonal_profit_source_mt5_probe_no_runtime_kpi"
...
EXPLORATION_LABEL = "stage301_Model__OrthogonalProfitSourceReplay"
SIGNAL_COLUMN = "run301b_route_signal"
FEATURE_ORDER = (SIGNAL_COLUMN,)
COMMON_ROOT = "Project_Obsidian_Prime_v2/stage301/run301B_orthogonal_profit_source"
...
MT5_QUEUE = RUN301A / "mt5_probe_queue.csv"
SOURCE_MANIFEST = RUN301A / "candidate_payload_manifest.csv"
SOURCE_RUN_MANIFEST = RUN301A / "run_manifest.json"
SOURCE_REPORT = REVIEWS / "run301A_orthogonal_profit_source_materialization_report.md"
PRODUCER = Path("stage_pipelines/stage301/execute_orthogonal_profit_source_mt5_probe.py")
...
REPORT = REVIEWS / "run301B_orthogonal_profit_source_mt5_probe_report.md"
...
def attach_identity(attempt: dict[str, Any], queue_row: Mapping[str, str], max_hold: int, close_on_flat: bool, same_reentry: int) -> None:
    ...
    attempt["stage301_branch_id"] = queue_row.get("materialized_branch_id", "")
    ...
    for queue_row in queue_rows:
        ...
        for runtime_split, split_token in (("validation_is", "val"), ("oos", "oos")):
            for tier_key, tier_label, tier_token in (
                ...
                attempt["signal_policy"] = "stage301 orthogonal profit source route_signal_value -1/0/+1 through single-feature table"
            ...
            if include_routed:
                ...
                attach_identity(attempt, queue_row, max_hold, close_on_flat, same_reentry)
                attempt["signal_policy"] = "Tier A primary + Tier B fallback stage301 orthogonal profit source route_signal_value"
                attempts.append(attempt)
    return attempts
...
def classify_status(result: Mapping[str, Any], materialize_only: bool) -> tuple[str, str, str, str]:
    ...
    if materialize_only:
        return (
            STATUS_PREPARED,
            "orthogonal_profit_source_runtime_probe_prepared_no_external_execution",
            "out_of_scope_by_claim_materialize_only",
            "run301B_execute_orthogonal_profit_source_mt5_probe_external_check",
        )
    ...
    if planned and completed_exec >= planned and len(kpis) >= planned:
        return (
            "completed_orthogonal_profit_source_mt5_probe_no_selection",
            "runtime_probe_completed_requires_profit_scale_curve_review_no_selection",
            "completed",
            "run301C_review_orthogonal_profit_source_mt5_probe",
        )
    if kpis:
        return (
            "partial_orthogonal_profit_source_mt5_probe_no_selection",
            "runtime_probe_partial_requires_continuation_or_review_no_selection",
            "partial_or_blocked",
            "run301B_continue_orthogonal_profit_source_mt5_probe" if limited else "run301C_review_with_runtime_gaps",
        )
    return (
        "blocked_orthogonal_profit_source_mt5_probe_no_kpi",
        "runtime_probe_blocked_no_kpi_no_selection",
        "blocked_or_invalid",
        "run301B_repair_or_block_orthogonal_profit_source_mt5_probe",
    )
...
def report_markdown(result: Mapping[str, Any], status: str, judgment: str, external_status: str, next_action: str) -> str:
    ...
    return "\n".join(
        [
            "# run301B Orthogonal Profit Source MT5 Probe(301B 직교 수익 원천 MT5 탐침)",
            "",
            ...
            "",
            "Effect(효과): run301A(301A 실행)의 orthogonal profit source(직교 수익 원천) 신호가 실제 MT5 tester(MT5 테스터)에서 순수익, 거래 수, 곡선 품질로 재현되는지 확인했다.",
            "",
            "## Boundary(경계)",
            "",
            f"`{BOUNDARY}`",
        ...
        [
            {
                "run_id": RUN_ID,
                "stage_id": STAGE_ID,
                "lane": "orthogonal_profit_source_mt5_probe",
                "status": status,
                "judgment": judgment,
        ...
        [
            {
                ...
                "record_view": "orthogonal_profit_source_mt5_probe",
        ...
        [
            {
                ...
                "stage_id": STAGE_ID,
                "run_id": RUN_ID,
                "view": "orthogonal_profit_source_mt5_probe",
                "tier_scope": "Tier A used/Tier B fallback stress/actual routed total",
...
def update_docs(status: str, judgment: str, next_action: str, kpi_count: int, attempt_count: int) -> None:
    ...
    selected = prev.prev.e295.e294.base.base.append_once(selected, "run301B_report", f"- run301B_report(301B 보고): `{prev.prev.e295.e294.base.base.rel(REPORT)}`")
    selected = prev.prev.e295.e294.base.base.append_once(selected, "run301B_execution_result", f"- run301B_execution_result(301B 실행 결과): `{prev.prev.e295.e294.base.base.rel(EXECUTION_RESULT)}`")
    prev.prev.e295.e294.base.base.write_md(SELECTED, selected)

    review_index = prev.prev.e295.e294.base.base.io_path(REVIEW_INDEX).read_text(encoding="utf-8-sig") if prev.prev.e295.e294.base.base.path_exists(REVIEW_INDEX) else "# Stage301 Review Index(301단계 검토 색인)\n"
    review_index = prev.prev.e295.e294.base.base.append_once(
        review_index,
        "run301B_report",
        f"- run301B_report(301B 보고): `{prev.prev.e295.e294.base.base.rel(REPORT)}`\n- run301B_execution_result(301B 실행 결과): `{prev.prev.e295.e294.base.base.rel(EXECUTION_RESULT)}`\n- run301B_mt5_kpi_summary(301B MT5 KPI 요약): `{prev.prev.e295.e294.base.base.rel(MT5_KPI_SUMMARY)}`",
    )
    ...
    current = prev.prev.e295.e294.base.base.append_once(
        current,
        "run301B_summary",
        f"- run301B_summary(301B 요약): orthogonal profit source MT5 probe(직교 수익 원천 MT5 탐침)를 실행했다. Effect(효과): attempts(시도) `{attempt_count}`개와 MT5 KPI records(MT5 KPI 기록) `{kpi_count}`개를 엮었고 후보/어댑터/ONNX(온엑스) 주장은 하지 않는다.",
    )
    ...
    focus = (
        f"- >-\n"
        f"  Stage301(301단계) run301B(301B 실행) orthogonal profit source MT5 probe(직교 수익 원천 MT5 탐침) `{RUN_ID}`. "
    ...
    changelog = prev.prev.e295.e294.base.base.append_once(
        changelog,
        RUN_ID,
        f"## {UPDATED_ON} run301B orthogonal profit source MT5 probe(301B 직교 수익 원천 MT5 탐침)\n\n"
        f"- status(상태): `{status}`\n"
        f"- judgment(판정): `{judgment}`\n"
