...
SOURCE_STAGE59P_ID = "59P_adapter_repair__bounded_followup_from_stage59o"
STAGE59Q_ID = "59Q_adapter_repair__bounded_followup_from_stage59p"
NEXT_REPAIR_STAGE_ID = "59R_adapter_repair__bounded_followup_from_stage59q"
RUN_NUMBER = "run59L"
RUN_ID = "run59L_stage59q_bounded_followup_from_stage59p_v1"
PACKET_ID = "stage59q_bounded_followup_from_stage59p_v1"
PARENT_RUN_ID = "run59K_stage59p_bounded_followup_from_stage59o_v1"
SOURCE_ADAPTER_ID = "s59p_v62_sl20_tp30_sd12_h5"
DEVELOPMENT_ANCHOR = "v64_v47_ctxgap14_refill_etfw_h2_no_b"
BACKUP_ANCHOR = "v60_v47_et_stable_damage_firewall_h2c0_no_b"
SOURCE_STAGE59P_PUSHED_COMMIT = "aaf46e630c2c6bd8e3894744a981ede6bf60e2d4"
BOUNDARY = (
    "research_development_only_no_live_readiness_no_runtime_authority_"
    "no_operating_promotion_no_operating_reference_no_production_baseline_no_deployment"
)

STAGE_ROOT = Path("stages") / STAGE59Q_ID
...
SOURCE_STAGE59P_ROOT = Path("stages") / SOURCE_STAGE59P_ID
SOURCE_STAGE59P_DECISION = SOURCE_STAGE59P_ROOT / "03_reviews/stage59p_decision.md"
SOURCE_STAGE59P_REPORT = SOURCE_STAGE59P_ROOT / "03_reviews/bounded_followup_from_stage59o_report.md"
SOURCE_STAGE59P_SUMMARY = SOURCE_STAGE59P_ROOT / "03_reviews/bounded_followup_summary.csv"
SOURCE_STAGE59P_JSON = SOURCE_STAGE59P_ROOT / "03_reviews/bounded_followup_summary.json"
...
COMMON_ROOT = f"Project_Obsidian_Prime_v2/s59q/{RUN_NUMBER}"

REPORT_PATH = REVIEWS_ROOT / "bounded_followup_from_stage59p_report.md"
...
DECISION_PATH = REVIEWS_ROOT / "stage59q_decision.md"
AUDIT_CSV_PATH = REVIEWS_ROOT / "stage59q_trade_audit.csv"
...
STAGE59Q_VARIANTS = (
    repair.RepairVariant(
        adapter_id="s59q_v62_sl20_tp30_sd12_h5_rearm002",
        label="run50BQ_v62_transition_h5_sl20_tp30_sd12_t52_rearm002_risk3pct_atr",
        atr_enabled=True,
        ...
        max_hold_bars=5,
        notes="Stage59Q bounded follow-up: keep Stage59P h5 and allow same-signal rearm only when confidence delta is at least 0.02.",
    ),
    repair.RepairVariant(
        adapter_id="s59q_v62_sl20_tp30_sd12_h5_rearm004",
        label="run50BQ_v62_transition_h5_sl20_tp30_sd12_t52_rearm004_risk3pct_atr",
        atr_enabled=True,
        model_risk_enabled=True,
        fixed_lot=0.25,
        atr_stop_multiplier=2.0,
        ...
        same_direction_reentry_cooldown_bars=12,
        entry_transition_only=True,
        entry_transition_rearm_min_confidence_delta=0.04,
        short_threshold=0.52,
        long_threshold=0.52,
        reverse_on_opposite_signal=False,
        close_only_on_opposite_signal=True,
        max_hold_bars=5,
        notes="Stage59Q bounded follow-up: keep Stage59P h5 and allow same-signal rearm only when confidence delta is at least 0.04.",
    ),
    repair.RepairVariant(
        adapter_id="s59q_v62_sl20_tp30_sd12_h5_rearm006",
        label="run50BQ_v62_transition_h5_sl20_tp30_sd12_t52_rearm006_risk3pct_atr",
        atr_enabled=True,
        model_risk_enabled=True,
        fixed_lot=0.25,
        atr_stop_multiplier=2.0,
        ...
        same_direction_reentry_cooldown_bars=12,
        entry_transition_only=True,
        entry_transition_rearm_min_confidence_delta=0.06,
        short_threshold=0.52,
        long_threshold=0.52,
        reverse_on_opposite_signal=False,
        close_only_on_opposite_signal=True,
        max_hold_bars=5,
        notes="Stage59Q bounded follow-up: keep Stage59P h5 and allow same-signal rearm only when confidence delta is at least 0.06.",
    ),
)
...
def source_specs() -> dict[str, dict[str, Any]]:
    ...
    return {variant.adapter_id: dict(v62) for variant in STAGE59Q_VARIANTS}


def configure_reused_engine() -> None:
    engine.STAGE59_ID = STAGE59Q_ID
    ...
    engine.STAGE59_VARIANTS = STAGE59Q_VARIANTS
    engine.SOURCE_SPECS = source_specs()
    engine.MODEL_RISK_MIN_PCT = {variant.adapter_id: 0.005 for variant in STAGE59Q_VARIANTS}

    repair.STAGE_ID = STAGE59Q_ID
    repair.RUN_NUMBER = RUN_NUMBER
    repair.RUN_ID = RUN_ID
    repair.RUN_ROOT = RUN_ROOT
    repair.REPAIR_VARIANTS = STAGE59Q_VARIANTS
    s58.STAGE58_ID = STAGE59Q_ID
    ...
    s58.STAGE58_VARIANTS = STAGE59Q_VARIANTS
...
def build_attempts(inputs: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    for variant_index, variant in enumerate(STAGE59Q_VARIANTS, start=1):
            ...
            for role_index, (tier, attempt_role, prefix, attempt_token) in enumerate(
                ...
                attempts.append(
                    attempt_payload(
                        ...
                        run_id=RUN_ID,
                        stage_number=59,
                        exploration_label="stage59P_BaselineAdapter__MaxHoldPressureFromStage59P",
...
def execute_prepared_run_checkpointed(prepared: Mapping[str, Any], args: argparse.Namespace) -> dict[str, Any]:
    ...
    return {
        **dict(prepared),
        ...
        "judgment": "inconclusive_stage59q_runtime_probe_completed" if completed and report_completed else "blocked_stage59q_runtime_probe",
    }
...
def next_stage_for_decision(decision: str) -> str:
    ...
    if decision == "demote_current_adapter_and_select_backup":
        return "59R_adapter_repair__backup_or_demote_from_stage59q"
    if decision == "open_bounded_followup":
        return "59R_adapter_repair__bounded_followup_from_stage59q"
...
def next_packet_for_decision(decision: str) -> tuple[str, str]:
    ...
    return "stage59r_bounded_followup_from_stage59q_v1", "run59M_stage59r_bounded_followup_from_stage59q_v1"
            ...
            rows.append(
                {
                    ...
                    "stage_id": STAGE59Q_ID,
                    "run_id": RUN_ID,
                    "artifact_type": "stage59q_bounded_followup_evidence",
                    "path": rel(path),
                    "sha256": sha256_file_lf_normalized(path),
                    "created_at_utc": created,
                    "notes": "Stage59Q bounded follow-up from Stage59P artifact.",
                }
            )
            ...
            rows.append(
                {
                    ...
                    "created_at_utc": created,
                    "notes": "Actual Stage59Q MT5 Strategy Tester HTML report.",
                }
            )
    return rows
...
def write_run_identity(result: Mapping[str, Any]) -> None:
    engine.write_json(
        RUN_ROOT / "run_manifest.json",
        {
            ...
            "bounded_question": "Can bounded entry-transition confidence rearm on the Stage59P h5 candidate recover OOS density and early/mid segment weakness without damaging PF, drawdown, cost stress, same-move concentration, ATR/risk telemetry, or starting ONNX?",
            "source_stage59p_decision": rel(SOURCE_STAGE59P_DECISION),
            "source_stage59p_pushed_commit": SOURCE_STAGE59P_PUSHED_COMMIT,
            "variants": [
                {
                    **variant.__dict__,
                ...
                for variant in STAGE59Q_VARIANTS
            ],
...
def write_ledgers(
        ...
        [
            {
                "run_id": RUN_ID,
                "stage_id": STAGE59Q_ID,
                "lane": "baseline_adapter_bounded_followup_from_stage59p",
                "status": status,
                "judgment": decision,
    ...
    ledger_rows.append(
        {
            "ledger_row_id": f"{RUN_ID}__aggregate_bounded_followup_from_stage59p",
            "stage_id": STAGE59Q_ID,
            "run_id": RUN_ID,
            "subrun_id": "aggregate_bounded_followup_from_stage59p",
            "parent_run_id": PARENT_RUN_ID,
            "record_view": "bounded_followup_from_stage59p",
            "tier_scope": "Tier A+B",
            ...
            "external_verification_status": external,
            "notes": "Stage59Q bounded confidence-rearm follow-up from Stage59P evidence; not final package completion.",
        }
    )
...
def report_markdown(summary_rows: Sequence[Mapping[str, Any]], decision: str, reasons: Sequence[str], external: str) -> str:
    ...
    best = best_repaired_variant(summary_rows)
    return f"""# Stage59Q Bounded Followup From Stage59P Report(59Q단계 59P단계 기반 경계 후속 보고서)

- stage(단계): `{STAGE59Q_ID}`
- run(실행): `{RUN_ID}`
...
def equity_audit_markdown(summary_rows: Sequence[Mapping[str, Any]], segment_rows: Sequence[Mapping[str, Any]]) -> str:
    ...
    return f"""# Stage59Q Bounded Followup Equity Curve Audit(59Q단계 경계 후속 자금 곡선 감사)
...
def decision_markdown(decision: str, reasons: Sequence[str], best: Mapping[str, Any], external: str) -> str:
    return f"""# Stage59Q Decision(59Q단계 판정)

decision(판정): `{decision}`

Stage59Q(59Q단계)는 Stage59P evidence(59P단계 근거)를 사용한 bounded follow-up(경계 후속)으로 기록한다. Effect(효과): entry_transition_rearm_min_confidence_delta(진입 전환 재무장 최소 신뢰도 차이)의 성공/실패를 다음 bounded stage(경계 다음 단계)의 입력 근거로 넘긴다.
...
Stage59Q closeout(59Q단계 종료)는 overall goal completion(전체 목표 완료)이 아니다. Effect(효과): Stage60 ONNX hardening(60단계 ONNX 경화)은 adapter quality(어댑터 품질)가 강할 때만 열린다.
    ...
    engine.write_json(
        SUMMARY_JSON_PATH,
        {
            ...
            "required_outputs": {
                ...
                "stage59q_decision": rel(DECISION_PATH),
            },
...
def write_packet_files(
    ...
    files = {
        ...
        "experiment_design_receipt.json": {
            "hypothesis": "A bounded entry-transition confidence rearm follow-up on the Stage59P h5 candidate may recover OOS density and early/mid segment weakness while preserving validation/OOS PF, cost stress, drawdown, same-move concentration, and ATR/risk telemetry.",
            ...
            "comparison_baseline": "Stage59P best candidate s59p_v62_sl20_tp30_sd12_h5.",
            "control_variables": ["US100", "M5", "split_v1", "Tier B disabled", "close_only_on_opposite", "same-direction cooldown 12", "confidence thresholds 0.52/0.52", "ATR SL 2.0", "ATR TP 3.0", "3% model-controlled risk cap", "ONNX deferred"],
            ...
            "stop_conditions": "three bounded Stage59Q confidence-rearm variants only",
        ...
        "artifact_lineage_audit.json": {
            "source_inputs": [rel(SOURCE_STAGE59P_DECISION), rel(SOURCE_STAGE59P_REPORT), rel(SOURCE_STAGE59P_SUMMARY), rel(SOURCE_STAGE59P_JSON)],
...
def write_stage_docs(decision: str) -> None:
    ...
    engine.write_md(
        ...
        f"""# Stage59Q Brief(59Q단계 개요)

- stage_id(단계 ID): `{STAGE59Q_ID}`
- source_stage(원천 단계): `{SOURCE_STAGE59P_ID}`
- source_decision(원천 판정): `continue_repair_in_new_bounded_stage`
- bounded_question(경계 질문): `Can bounded entry-transition confidence rearm on the Stage59P h5 candidate recover OOS density and early/mid segment weakness without damaging PF, drawdown, cost stress, same-move concentration, ATR/risk telemetry, or starting ONNX?`
- boundary(경계): `{BOUNDARY}`

Stage59Q(59Q단계)는 Stage59P(59P단계) h5 candidate(h5 후보)를 유지하고 entry_transition_rearm_min_confidence_delta(진입 전환 재무장 최소 신뢰도 차이) 세 변형만 측정한다. Effect(효과): Stage60 ONNX(60단계 ONNX)나 deployment(배포)로 이어가지 않고 OOS 약점 수리 여부만 본다.
""",
    )
    engine.write_md(
        INPUT_ROOT / "input_refs.md",
        f"""# Stage59Q Input References(59Q단계 입력 참조)

- stage59p_decision(59P단계 판정): `{rel(SOURCE_STAGE59P_DECISION)}`
- stage59p_report(59P단계 보고서): `{rel(SOURCE_STAGE59P_REPORT)}`
- stage59p_summary(59P단계 요약): `{rel(SOURCE_STAGE59P_SUMMARY)}`
- stage59p_pushed_commit(59P단계 푸시 커밋): `{SOURCE_STAGE59P_PUSHED_COMMIT}`
...
Effect(효과): Stage59Q(59Q단계)는 Stage59P(59P단계) 실패 기억과 run50BQ(run50BQ) v62 전환 원천을 명시적으로 연결한다.
""",
    )
    engine.write_md(
        SELECTED_ROOT / "selection_status.md",
        f"""# Stage59Q Selection Status(59Q단계 선택 상태)

- stage_status(단계 상태): `closed_bounded_followup_from_stage59p`
- source_stage(원천 단계): `{SOURCE_STAGE59P_ID}`
- source_decision(원천 판정): `continue_repair_in_new_bounded_stage`
- stage59q_decision(59Q단계 판정): `{decision}`
- next_stage_or_branch(다음 단계/분기): `{next_stage}`
- selected_research_baseline(선택 연구 기준선): `none`
- claim_boundary(주장 경계): `{BOUNDARY}`

Effect(효과): Stage59Q(59Q단계)는 측정 결과를 보존하지만 final package(최종 패키지)나 operating claim(운영 주장)을 만들지 않는다.
""",
    )
    engine.write_md(
        REVIEWS_ROOT / "review_index.md",
        f"""# Stage59Q Review Index(59Q단계 검토 색인)
...
- bounded_followup_risk_atr_telemetry(경계 후속 위험/ATR 텔레메트리): `{rel(RISK_ATR_TELEMETRY_PATH)}`
- stage59q_decision(59Q단계 판정): `{rel(DECISION_PATH)}`
""",
    )
    if not next_stage.startswith("60_"):
        ...
        engine.write_md(
            ...
            f"""# Stage59R Brief(59R단계 개요)

- stage_id(단계 ID): `{next_stage}`
- source_stage(원천 단계): `{STAGE59Q_ID}`
- source_decision(원천 판정): `{decision}`
- bounded_question(경계 질문): `What bounded follow-up should repair, demote, or branch from the Stage59Q evidence without starting ONNX prematurely?`
- boundary(경계): `{BOUNDARY}`

Stage59R(59R단계)는 Stage59Q(59Q단계) 결과가 Stage60 ONNX(60단계 ONNX)에 충분하지 않을 때 이어지는 bounded follow-up(경계 후속)이다. Effect(효과): repair(수리), demotion(강등), branch(분기)를 다음 작은 질문으로 제한한다.
""",
        )
        engine.write_md(
            next_root / "01_inputs/input_refs.md",
            f"""# Stage59R Input References(59R단계 입력 참조)

- stage59q_decision(59Q단계 판정): `{rel(DECISION_PATH)}`
- stage59q_report(59Q단계 보고서): `{rel(REPORT_PATH)}`
- stage59q_summary(59Q단계 요약): `{rel(SUMMARY_CSV_PATH)}`
- stage59q_segment_kpi(59Q단계 구간 KPI): `{rel(SEGMENT_KPI_PATH)}`
- stage59q_risk_atr_telemetry(59Q단계 위험/ATR 텔레메트리): `{rel(RISK_ATR_TELEMETRY_PATH)}`
""",
        )
        engine.write_md(
...
""",
        ...
        engine.write_md(
            ...
            f"""# Stage59R Selection Status(59R단계 선택 상태)

- stage_status(단계 상태): `active_planned_from_stage59q`
- source_stage(원천 단계): `{STAGE59Q_ID}`
- source_decision(원천 판정): `{decision}`
- selected_research_baseline(선택 연구 기준선): `none`
- claim_boundary(주장 경계): `{BOUNDARY}`

Effect(효과): Stage59R(59R단계)는 Stage59Q(59Q단계) 이후 필요한 다음 작은 질문만 다룬다.
""",
        )
...
- status(상태): `stage59q_closed_{decision}`
- claim_boundary(주장 경계): research/development only(연구/개발 전용)

Stage59Q(59Q단계) closed(종료) as bounded confidence-rearm follow-up from Stage59P(59P단계 기반 경계 신뢰도 재무장 후속). Effect(효과): Stage59P(59P단계) segment/density weakness(구간/밀도 약점)는 보존하고, Stage60 ONNX(60단계 ONNX)는 품질 근거가 강할 때만 열린다.

## Latest Stage59Q Evidence(최신 59Q단계 근거)
...
""",
    ...
    focus = (
        "current_focus:\n"
        f"- >-\n"
        f"  Stage59Q(59Q단계) `{STAGE59Q_ID}` closed(종료) as bounded confidence-rearm follow-up from Stage59P(59P단계 기반 경계 신뢰도 재무장 후속); decision(판정)=`{decision}`. "
        ...
        f"- >-\n"
        f"  Next stage_or_branch(다음 단계/분기) `{next_stage}` is active/planned(활성/계획). Effect(효과): Stage59Q(59Q단계) 결과를 다음 bounded step(경계 다음 단계)으로 넘긴다.\n"
    )
    text = re.sub(
...
stage59q_bounded_followup_from_stage59p:
  packet_id: {PACKET_ID}
  stage_id: {STAGE59Q_ID}
  status: closed_bounded_followup_from_stage59p
  ...
  boundary: {BOUNDARY}
"""
    if "stage59q_bounded_followup_from_stage59p:" in text:
        text = re.sub(r"\nstage59q_bounded_followup_from_stage59p:\n(?:  .*\n)*", block, text, count=1)
    else:
        text += block
...
def parse_args(argv: Sequence[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run Stage59Q bounded follow-up from Stage59P.")
...
def load_existing_result() -> dict[str, Any]:
    ...
    if not path_exists(manifest) or not path_exists(kpi):
        raise FileNotFoundError("Stage59Q existing run_manifest.json or kpi_record.json is missing")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.finalize_existing:
        ...
        prepared = {
            ...
            "model_family": "baseline_adapter_stage59q_confidence_rearm_ebm_table",
            "feature_set_id": "stage59q_v62_confidence_rearm_discrete_signal",
