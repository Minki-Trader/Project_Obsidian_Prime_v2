...
SOURCE_STAGE59Z_ID = "59Z_adapter_repair__bounded_followup_from_stage59y"
STAGE59AA_ID = "59AA_adapter_repair__bounded_followup_from_stage59z"
NEXT_REPAIR_STAGE_ID = "59AB_adapter_repair__bounded_followup_from_stage59aa"
RUN_NUMBER = "run59V"
RUN_ID = "run59V_stage59aa_bounded_followup_from_stage59z_v1"
PACKET_ID = "stage59aa_bounded_followup_from_stage59z_v1"
PARENT_RUN_ID = "run59U_stage59z_bounded_followup_from_stage59y_v1"
SOURCE_ADAPTER_ID = "s59z_v64_gap14_t59_h2_sd5"
DEVELOPMENT_ANCHOR = "v64_v47_ctxgap14_refill_etfw_h2_no_b"
BACKUP_ANCHOR = "v60_v47_et_stable_damage_firewall_h2c0_no_b"
SOURCE_STAGE59Z_PUSHED_COMMIT = "6711d330dda7e106bf328f3184548b4fb4a88677"
BOUNDARY = (
    "research_development_only_no_live_readiness_no_runtime_authority_"
    "no_operating_promotion_no_operating_reference_no_production_baseline_no_deployment"
)

STAGE_ROOT = Path("stages") / STAGE59AA_ID
...
SOURCE_STAGE59Z_ROOT = Path("stages") / SOURCE_STAGE59Z_ID
SOURCE_STAGE59Z_DECISION = SOURCE_STAGE59Z_ROOT / "03_reviews/stage59z_decision.md"
SOURCE_STAGE59Z_REPORT = SOURCE_STAGE59Z_ROOT / "03_reviews/bounded_followup_from_stage59y_report.md"
SOURCE_STAGE59Z_SUMMARY = SOURCE_STAGE59Z_ROOT / "03_reviews/bounded_followup_summary.csv"
SOURCE_STAGE59Z_JSON = SOURCE_STAGE59Z_ROOT / "03_reviews/bounded_followup_summary.json"
...
COMMON_ROOT = f"Project_Obsidian_Prime_v2/s59aa/{RUN_NUMBER}"

REPORT_PATH = REVIEWS_ROOT / "bounded_followup_from_stage59z_report.md"
...
DECISION_PATH = REVIEWS_ROOT / "stage59aa_decision.md"
AUDIT_CSV_PATH = REVIEWS_ROOT / "stage59aa_trade_audit.csv"
...
STAGE59AA_VARIANTS = (
    repair.RepairVariant(
        adapter_id="s59aa_v64_gap14_t59_h2_globalcool3_sd5",
        label="run50BR_v64_context_gap14_h2_t59_globalcool3_risk3pct_wide_atr_sd5",
        atr_enabled=True,
        ...
        max_hold_bars=2,
        notes="Stage59AA bounded follow-up: add a three-bar global re-entry cooldown to the Stage59Z t59 adapter to test whether validation cost-stress weakness is density-driven.",
    ),
    repair.RepairVariant(
        adapter_id="s59aa_v64_gap14_t59_h2_sd10",
        label="run50BR_v64_context_gap14_h2_t59_same_direction_cool10_risk3pct_wide_atr",
        atr_enabled=True,
        model_risk_enabled=True,
        fixed_lot=0.25,
        atr_stop_multiplier=2.5,
        ...
        close_only_on_opposite_signal=True,
        max_hold_bars=2,
        notes="Stage59AA bounded follow-up: widen same-direction cooldown to ten bars on the Stage59Z t59 adapter to test same-move concentration repair.",
    ),
    repair.RepairVariant(
        adapter_id="s59aa_v64_gap14_t59_h2_entrytrans_sd5",
        label="run50BR_v64_context_gap14_h2_t59_entry_transition_risk3pct_wide_atr_sd5",
        atr_enabled=True,
        model_risk_enabled=True,
        fixed_lot=0.25,
        atr_stop_multiplier=2.5,
        ...
        close_only_on_opposite_signal=True,
        max_hold_bars=2,
        notes="Stage59AA bounded follow-up: require entry transitions on the Stage59Z t59 adapter to test whether repeat entries drive validation flatline and cost stress.",
    ),
)
...
def source_specs() -> dict[str, dict[str, Any]]:
    return {
        "s59aa_v64_gap14_t59_h2_globalcool3_sd5": {
        ...
        "s59aa_v64_gap14_t59_h2_entrytrans_sd5": {
            "label": "run50br_v64_context_gap14_h2",
            "run_root": RUN50BR_ROOT,
            "variant_root": RUN50BR_ROOT / RUN50BR_V64_ANCHOR,
            "anchor": RUN50BR_V64_ANCHOR,
...
def configure_reused_engine() -> None:
    engine.STAGE59_ID = STAGE59AA_ID
    ...
    engine.STAGE59_VARIANTS = STAGE59AA_VARIANTS
    engine.SOURCE_SPECS = source_specs()
    engine.MODEL_RISK_MIN_PCT = {variant.adapter_id: 0.005 for variant in STAGE59AA_VARIANTS}

    repair.STAGE_ID = STAGE59AA_ID
    repair.RUN_NUMBER = RUN_NUMBER
    repair.RUN_ID = RUN_ID
    repair.RUN_ROOT = RUN_ROOT
    repair.REPAIR_VARIANTS = STAGE59AA_VARIANTS
    s58.STAGE58_ID = STAGE59AA_ID
    ...
    s58.STAGE58_VARIANTS = STAGE59AA_VARIANTS
...
def build_attempts(inputs: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    for variant_index, variant in enumerate(STAGE59AA_VARIANTS, start=1):
            ...
            for role_index, (tier, attempt_role, prefix, attempt_token) in enumerate(
                ...
                attempts.append(
                    attempt_payload(
                        ...
                        run_id=RUN_ID,
                        stage_number=59,
                        exploration_label="stage59AA_BaselineAdapter__V64DensityThrottle",
...
def execute_prepared_run_checkpointed(prepared: Mapping[str, Any], args: argparse.Namespace) -> dict[str, Any]:
    ...
    return {
        **dict(prepared),
        ...
        "judgment": "inconclusive_stage59z_runtime_probe_completed" if completed and report_completed else "blocked_stage59z_runtime_probe",
    }
...
def next_stage_for_decision(decision: str) -> str:
    ...
    if decision == "demote_current_adapter_and_select_backup":
        return "59AB_adapter_repair__backup_or_demote_from_stage59aa"
    if decision == "open_new_model_branch":
        return "59AB_adapter_repair__new_model_branch_from_stage59aa"
...
def next_packet_for_decision(decision: str) -> tuple[str, str]:
    ...
    return "stage59ab_bounded_followup_from_stage59aa_v1", "run59W_stage59ab_bounded_followup_from_stage59aa_v1"
            ...
            rows.append(
                {
                    ...
                    "stage_id": STAGE59AA_ID,
                    "run_id": RUN_ID,
                    "artifact_type": "stage59aa_bounded_followup_evidence",
                    "path": rel(path),
                    "sha256": sha256_file_lf_normalized(path),
                    "created_at_utc": created,
                    "notes": "Stage59AA bounded density-throttle follow-up from Stage59Z artifact.",
                }
            )
            ...
            rows.append(
                {
                    ...
                    "created_at_utc": created,
                    "notes": "Actual Stage59AA MT5 Strategy Tester HTML report.",
                }
            )
    return rows
...
def write_run_identity(result: Mapping[str, Any]) -> None:
    engine.write_json(
        RUN_ROOT / "run_manifest.json",
        {
            ...
            "bounded_question": "Can bounded density and re-entry throttle on the Stage59Z v64 gap14 adapter repair validation PF and cost-stressed expectancy without damaging OOS behavior?",
            "source_stage59z_decision": rel(SOURCE_STAGE59Z_DECISION),
            "source_stage59z_pushed_commit": SOURCE_STAGE59Z_PUSHED_COMMIT,
            "variants": [
                {
                    **variant.__dict__,
                ...
                for variant in STAGE59AA_VARIANTS
            ],
...
def write_ledgers(
        ...
        [
            {
                "run_id": RUN_ID,
                "stage_id": STAGE59AA_ID,
                "lane": "baseline_adapter_bounded_followup_from_stage59z",
                "status": status,
                "judgment": decision,
    ...
    ledger_rows.append(
        {
            "ledger_row_id": f"{RUN_ID}__aggregate_bounded_followup_from_stage59z",
            "stage_id": STAGE59AA_ID,
            "run_id": RUN_ID,
            "subrun_id": "aggregate_bounded_followup_from_stage59z",
            "parent_run_id": PARENT_RUN_ID,
            "record_view": "bounded_followup_from_stage59z",
            "tier_scope": "Tier A+B",
            ...
            "external_verification_status": external,
            "notes": "Stage59AA bounded density-throttle follow-up from Stage59Z evidence; not final package completion.",
        }
    )
    ...
    engine.write_json(
        SUMMARY_JSON_PATH,
        {
            ...
            "required_outputs": {
                ...
                "stage59aa_decision": rel(DECISION_PATH),
            },
...
def report_markdown(summary_rows: Sequence[Mapping[str, Any]], decision: str, reasons: Sequence[str], external: str) -> str:
    ...
    best = best_repaired_variant(summary_rows)
    return f"""# Stage59AA Bounded Follow-Up From Stage59Z Report(59AA단계 Stage59Z 기반 경계 후속 보고서)

- stage(단계): `{STAGE59AA_ID}`
- run(실행): `{RUN_ID}`
...
def equity_audit_markdown(summary_rows: Sequence[Mapping[str, Any]], segment_rows: Sequence[Mapping[str, Any]]) -> str:
    ...
    return f"""# Stage59AA Bounded Follow-Up Equity Curve Audit(59AA단계 경계 후속 자금 곡선 감사)
...
def decision_markdown(decision: str, reasons: Sequence[str], best: Mapping[str, Any], external: str) -> str:
    return f"""# Stage59AA Decision(59AA단계 판정)

decision(판정): `{decision}`

Stage59AA(59AA단계)는 Stage59Z v64 gap14 evidence(Stage59Z v64 공백14 근거)를 사용한 bounded density-throttle follow-up(경계 거래밀도 제한 후속)이다. Effect(효과): Stage60 ONNX(60단계 ONNX)로 넘어가기 전 validation PF(검증 수익 팩터)와 cost-stressed expectancy(비용 압박 기대값) 약점을 먼저 확인한다.
...
Stage59AA closeout(59AA단계 종료)는 overall goal completion(전체 목표 완료)이 아니다. Effect(효과): strong full adapter evidence(강한 전체 어댑터 근거)가 없으면 다음 bounded stage(경계 다음 단계)로 넘긴다.
...
def write_packet_files(
    ...
    files = {
        ...
        "experiment_design_receipt.json": {
            "hypothesis": "A bounded density and re-entry throttle on the Stage59Z v64 gap14 adapter may repair validation PF and cost-stressed expectancy while preserving OOS net, PF, ATR/bracket telemetry, and model-risk telemetry.",
            ...
            "comparison_baseline": "Stage59Z best adapter s59z_v64_gap14_t59_h2_sd5 with validation PF 1.07 and validation cost-stressed expectancy below zero.",
            ...
            "changed_variables": ["reentry_cooldown_bars", "same_direction_reentry_cooldown_bars", "entry_transition_only"],
            ...
            "stop_conditions": "three bounded Stage59AA density-throttle variants only",
        ...
        "artifact_lineage_audit.json": {
            "source_inputs": [
                rel(SOURCE_STAGE59Z_DECISION),
                rel(SOURCE_STAGE59Z_REPORT),
                rel(SOURCE_STAGE59Z_SUMMARY),
                rel(SOURCE_STAGE59Z_JSON),
                rel(RUN50BR_ROOT / "models/stage56_context_timed_event_signal_discrete_score_table.csv"),
                rel(RUN50BR_ROOT / RUN50BR_V64_ANCHOR),
            ],
            "producer": rel(Path("stage_pipelines/stage59aa/bounded_followup_from_stage59z.py")),
...
def write_stage_docs(decision: str) -> None:
    ...
    engine.write_md(
        ...
        f"""# Stage59AA Brief(59AA단계 개요)

- stage_id(단계 ID): `{STAGE59AA_ID}`
- source_stage(원천 단계): `{SOURCE_STAGE59Z_ID}`
- source_decision(원천 판정): `continue_repair_in_new_bounded_stage`
- bounded_question(경계 질문): `Can density and re-entry throttle on the Stage59Z v64 gap14 adapter repair validation PF and cost-stressed expectancy without damaging OOS behavior?`
- boundary(경계): `{BOUNDARY}`

Stage59AA(59AA단계)는 Stage59Z(59Z단계)의 best adapter(최선 어댑터) `s59z_v64_gap14_t59_h2_sd5`에서 density/re-entry throttle(거래 밀도/재진입 제한) 세 변형만 측정한다. Effect(효과): Stage60 ONNX(60단계 ONNX)로 조기 이동하지 않고 검증 품질 약점을 먼저 좁게 확인한다.
""",
    )
    engine.write_md(
        INPUT_ROOT / "input_refs.md",
        f"""# Stage59AA Input References(59AA단계 입력 참조)

- stage59z_decision(59Z단계 판정): `{rel(SOURCE_STAGE59Z_DECISION)}`
- stage59z_report(59Z단계 보고서): `{rel(SOURCE_STAGE59Z_REPORT)}`
- stage59z_summary(59Z단계 요약): `{rel(SOURCE_STAGE59Z_SUMMARY)}`
- stage59z_pushed_commit(59Z단계 푸시 커밋): `{SOURCE_STAGE59Z_PUSHED_COMMIT}`
...
Effect(효과): Stage59AA(59AA단계)는 Stage59Z(59Z단계) 약점과 run50BR v64(run50BR 변형64) 원천을 명시적으로 연결한다.
""",
    )
    engine.write_md(
        SELECTED_ROOT / "selection_status.md",
        f"""# Stage59AA Selection Status(59AA단계 선택 상태)

- stage_status(단계 상태): `closed_bounded_followup_from_stage59z`
- source_stage(원천 단계): `{SOURCE_STAGE59Z_ID}`
- source_decision(원천 판정): `continue_repair_in_new_bounded_stage`
- stage59aa_decision(59AA단계 판정): `{decision}`
- next_stage_or_branch(다음 단계/분기): `{next_stage}`
- selected_research_baseline(선택 연구 기준선): `none`
- claim_boundary(주장 경계): `{BOUNDARY}`

Effect(효과): Stage59AA(59AA단계)는 측정 결과를 보존하지만 final package(최종 패키지)나 operating claim(운영 주장)을 만들지 않는다.
""",
    )
    engine.write_md(
        REVIEWS_ROOT / "review_index.md",
        f"""# Stage59AA Review Index(59AA단계 검토 색인)
...
- bounded_followup_risk_atr_telemetry(경계 후속 위험/ATR 기록): `{rel(RISK_ATR_TELEMETRY_PATH)}`
- stage59aa_decision(59AA단계 판정): `{rel(DECISION_PATH)}`
""",
    )
...
- source_stage(원천 단계): `{STAGE59AA_ID}`
- source_decision(원천 판정): `{decision}`
- bounded_question(경계 질문): `What bounded follow-up should repair, demote, or branch from the Stage59AA density-throttle evidence without starting ONNX prematurely?`
- boundary(경계): `{BOUNDARY}`

{next_stage_label}({next_stage_label}단계)는 Stage59AA(59AA단계) 결과가 Stage60 ONNX(60단계 ONNX)에 충분하지 않을 때 이어지는 bounded follow-up(경계 후속)이다. Effect(효과): repair(수리), demotion(강등), branch(분기)를 다음 작은 질문으로 제한한다.
""",
        )
        engine.write_md(
...
- stage59aa_decision(59AA단계 판정): `{rel(DECISION_PATH)}`
- stage59aa_report(59AA단계 보고서): `{rel(REPORT_PATH)}`
- stage59aa_summary(59AA단계 요약): `{rel(SUMMARY_CSV_PATH)}`
- stage59aa_segment_kpi(59AA단계 구간 KPI): `{rel(SEGMENT_KPI_PATH)}`
- stage59aa_risk_atr_telemetry(59AA단계 위험/ATR 기록): `{rel(RISK_ATR_TELEMETRY_PATH)}`
""",
        )
        engine.write_md(
...
{next_stage_label}({next_stage_label}단계)는 planned(계획) 상태다. Effect(효과): Stage59AA(59AA단계) 약점을 다음 bounded repair(경계 수리) 근거로 넘긴다.
""",
        )
        engine.write_md(
...
- stage_status(단계 상태): `active_planned_from_stage59aa`
- source_stage(원천 단계): `{STAGE59AA_ID}`
- source_decision(원천 판정): `{decision}`
- selected_research_baseline(선택 연구 기준선): `none`
- claim_boundary(주장 경계): `{BOUNDARY}`

Effect(효과): {next_stage_label}({next_stage_label}단계)는 Stage59AA(59AA단계) 이후 필요한 다음 작은 질문만 다룬다.
""",
        )
...
- status(상태): `stage59aa_closed_{decision}`
- claim_boundary(주장 경계): research/development only(연구/개발 전용)

Stage59AA(59AA단계) closed(종료) as bounded density-throttle follow-up from Stage59Z(Stage59Z 기반 경계 거래밀도 제한 후속). Effect(효과): Stage59Z(59Z단계)의 validation PF/cost weakness(검증 수익 팩터/비용 약점)를 보존하고, Stage60 ONNX(60단계 ONNX)는 품질 근거가 강할 때만 열린다.

## Latest Stage59AA Evidence(최신 59AA단계 근거)
...
""",
    ...
    focus = (
        "current_focus:\n"
        f"- >-\n"
        f"  Stage59AA(59AA단계) `{STAGE59AA_ID}` closed(종료) as bounded density-throttle follow-up from Stage59Z(Stage59Z 기반 경계 거래밀도 제한 후속); decision(판정)=`{decision}`. "
        ...
        f"- >-\n"
        f"  Next stage_or_branch(다음 단계/분기) `{next_stage}` is active/planned(활성/계획). Effect(효과): Stage59AA(59AA단계) 결과를 다음 bounded step(경계 다음 단계)으로 넘긴다.\n"
    )
    text = re.sub(r"current_focus:\n(?:- >-\n(?:  .*\n)+)+", focus, text, count=1)
    block = f"""

stage59aa_bounded_followup_from_stage59z:
  packet_id: {PACKET_ID}
  stage_id: {STAGE59AA_ID}
  status: closed_bounded_followup_from_stage59z
  ...
  boundary: {BOUNDARY}
"""
    if "stage59aa_bounded_followup_from_stage59z:" in text:
        text = re.sub(r"\nstage59aa_bounded_followup_from_stage59z:\n(?:  .*\n)*", block, text, count=1)
    else:
        text += block
...
def parse_args(argv: Sequence[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run Stage59AA bounded follow-up from Stage59Z.")
...
def load_existing_result() -> dict[str, Any]:
    ...
    if not path_exists(manifest) or not path_exists(kpi):
        raise FileNotFoundError("Stage59AA existing run_manifest.json or kpi_record.json is missing")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.finalize_existing:
        ...
        prepared = {
            ...
            "model_family": "baseline_adapter_stage59aa_density_throttle_ebm_table",
            "feature_set_id": "stage59aa_v64_density_throttle_signal",
