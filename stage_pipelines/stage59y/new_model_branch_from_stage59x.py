...
SOURCE_STAGE59X_ID = "59X_adapter_repair__bounded_followup_from_stage59w"
STAGE59Y_ID = "59Y_adapter_repair__new_model_branch_from_stage59x"
NEXT_REPAIR_STAGE_ID = "59Z_adapter_repair__bounded_followup_from_stage59y"
RUN_NUMBER = "run59T"
RUN_ID = "run59T_stage59y_new_model_branch_from_stage59x_v1"
PACKET_ID = "stage59y_new_model_branch_from_stage59x_v1"
PARENT_RUN_ID = "run59S_stage59x_bounded_followup_from_stage59w_v1"
...
SOURCE_STAGE59X_PUSHED_COMMIT = "0043c8972511c6b4e073e25f651cb4b863523d38"
BOUNDARY = (
    "research_development_only_no_live_readiness_no_runtime_authority_"
    "no_operating_promotion_no_operating_reference_no_production_baseline_no_deployment"
)

STAGE_ROOT = Path("stages") / STAGE59Y_ID
...
SOURCE_STAGE59X_ROOT = Path("stages") / SOURCE_STAGE59X_ID
SOURCE_STAGE59X_DECISION = SOURCE_STAGE59X_ROOT / "03_reviews/stage59x_decision.md"
SOURCE_STAGE59X_REPORT = SOURCE_STAGE59X_ROOT / "03_reviews/demotion_or_branch_from_stage59w_report.md"
SOURCE_STAGE59X_SUMMARY = SOURCE_STAGE59X_ROOT / "03_reviews/demotion_or_branch_summary.csv"
SOURCE_STAGE59X_JSON = SOURCE_STAGE59X_ROOT / "03_reviews/demotion_or_branch_summary.json"

RUN50BR_ROOT = SOURCE_STAGE_ROOT / "02_runs/run50BR"
RUN50BR_V64_ANCHOR = "v64_v47_ctxgap14_refill_etfw_h2_no_b"
RUN50BR_V65_ANCHOR = "v65_v47_ctxgap24_refill_etfw_h2_no_b"
RUN50BR_V67_ANCHOR = "v67_v47_ctxgap24_refill_etfw_h4_no_b"
RUN50BR_SIGNAL = "stage56_context_gap_refill_signal"

COMMON_ROOT = f"Project_Obsidian_Prime_v2/s59y/{RUN_NUMBER}"

REPORT_PATH = REVIEWS_ROOT / "new_model_branch_from_stage59x_report.md"
...
DECISION_PATH = REVIEWS_ROOT / "stage59y_decision.md"
AUDIT_CSV_PATH = REVIEWS_ROOT / "stage59y_trade_audit.csv"
...
STAGE59Y_VARIANTS = (
    repair.RepairVariant(
        adapter_id="s59y_v64_gap14_h2_sd5",
        label="run50BR_v64_context_gap14_h2_threshold57_risk3pct_wide_atr_sd5",
        atr_enabled=True,
        ...
        max_hold_bars=2,
        notes="Stage59Y bounded new branch: run50BR v64 context-gap14 refill source after Stage59X demotion, 3% model risk cap, and wide ATR bracket.",
    ),
    repair.RepairVariant(
        adapter_id="s59y_v65_gap24_h2_sd5",
        label="run50BR_v65_context_gap24_h2_threshold57_risk3pct_wide_atr_sd5",
        atr_enabled=True,
        model_risk_enabled=True,
        fixed_lot=0.25,
        atr_stop_multiplier=2.5,
        ...
        close_only_on_opposite_signal=True,
        max_hold_bars=2,
        notes="Stage59Y bounded new branch: run50BR v65 context-gap24 refill source after Stage59X demotion, 3% model risk cap, and wide ATR bracket.",
    ),
    repair.RepairVariant(
        adapter_id="s59y_v67_gap24_h4_sd5",
        label="run50BR_v67_context_gap24_h4_threshold57_risk3pct_wide_atr_sd5",
        atr_enabled=True,
        model_risk_enabled=True,
        fixed_lot=0.25,
        atr_stop_multiplier=2.5,
        ...
        close_only_on_opposite_signal=True,
        max_hold_bars=4,
        notes="Stage59Y bounded new branch: run50BR v67 context-gap24 hold4 refill source after Stage59X demotion, 3% model risk cap, and wide ATR bracket.",
    ),
)
...
def source_specs() -> dict[str, dict[str, Any]]:
    return {
        ...
        "s59y_v65_gap24_h2_sd5": {
            "label": "run50br_v65_context_gap24_h2",
            "run_root": RUN50BR_ROOT,
            "variant_root": RUN50BR_ROOT / RUN50BR_V65_ANCHOR,
            "anchor": RUN50BR_V65_ANCHOR,
            "model": RUN50BR_ROOT / "models/stage56_context_timed_event_signal_discrete_score_table.csv",
            "signal_column": RUN50BR_SIGNAL,
            "validation_ini": RUN50BR_ROOT / RUN50BR_V65_ANCHOR / "mt5/x02_ta_val.ini",
            "oos_ini": RUN50BR_ROOT / RUN50BR_V65_ANCHOR / "mt5/x02_ta_oos.ini",
            "source_note": "Stage56 run50BR v65 context-gap24 refill ET firewall hold2 source",
        },
        "s59y_v67_gap24_h4_sd5": {
            "label": "run50br_v67_context_gap24_h4",
            "run_root": RUN50BR_ROOT,
            "variant_root": RUN50BR_ROOT / RUN50BR_V67_ANCHOR,
            "anchor": RUN50BR_V67_ANCHOR,
            "model": RUN50BR_ROOT / "models/stage56_context_timed_event_signal_discrete_score_table.csv",
            "signal_column": RUN50BR_SIGNAL,
            "validation_ini": RUN50BR_ROOT / RUN50BR_V67_ANCHOR / "mt5/x04_ta_val.ini",
            "oos_ini": RUN50BR_ROOT / RUN50BR_V67_ANCHOR / "mt5/x04_ta_oos.ini",
            "source_note": "Stage56 run50BR v67 context-gap24 refill ET firewall hold4 source",
        },
    }


def configure_reused_engine() -> None:
    engine.STAGE59_ID = STAGE59Y_ID
    ...
    engine.STAGE59_VARIANTS = STAGE59Y_VARIANTS
    engine.SOURCE_SPECS = source_specs()
    engine.MODEL_RISK_MIN_PCT = {variant.adapter_id: 0.005 for variant in STAGE59Y_VARIANTS}

    repair.STAGE_ID = STAGE59Y_ID
    repair.RUN_NUMBER = RUN_NUMBER
    repair.RUN_ID = RUN_ID
    repair.RUN_ROOT = RUN_ROOT
    repair.REPAIR_VARIANTS = STAGE59Y_VARIANTS
    s58.STAGE58_ID = STAGE59Y_ID
    ...
    s58.STAGE58_VARIANTS = STAGE59Y_VARIANTS
...
def build_attempts(inputs: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    for variant_index, variant in enumerate(STAGE59Y_VARIANTS, start=1):
            ...
            for role_index, (tier, attempt_role, prefix, attempt_token) in enumerate(
                ...
                attempts.append(
                    attempt_payload(
                        ...
                        run_id=RUN_ID,
                        stage_number=59,
                        exploration_label="stage59Y_BaselineAdapter__ContextGapRefillBranchFromStage59X",
...
def execute_prepared_run_checkpointed(prepared: Mapping[str, Any], args: argparse.Namespace) -> dict[str, Any]:
    ...
    return {
        **dict(prepared),
        ...
        "judgment": "inconclusive_stage59y_runtime_probe_completed" if completed and report_completed else "blocked_stage59y_runtime_probe",
    }
...
def next_stage_for_decision(decision: str) -> str:
    ...
    if decision == "demote_current_adapter_and_select_backup":
        return "59Z_adapter_repair__backup_or_demote_from_stage59y"
    if decision == "open_new_model_branch":
        return "59Z_adapter_repair__new_model_branch_from_stage59y"
...
def next_packet_for_decision(decision: str) -> tuple[str, str]:
    ...
    return "stage59z_bounded_followup_from_stage59y_v1", "run59U_stage59z_bounded_followup_from_stage59y_v1"
            ...
            rows.append(
                {
                    ...
                    "stage_id": STAGE59Y_ID,
                    "run_id": RUN_ID,
                    "artifact_type": "stage59y_new_model_branch_evidence",
                    "path": rel(path),
                    "sha256": sha256_file_lf_normalized(path),
                    "created_at_utc": created,
                    "notes": "Stage59Y bounded new model branch from Stage59X artifact.",
                }
            )
            ...
            rows.append(
                {
                    ...
                    "created_at_utc": created,
                    "notes": "Actual Stage59Y MT5 Strategy Tester HTML report.",
                }
            )
    return rows
...
def write_run_identity(result: Mapping[str, Any]) -> None:
    engine.write_json(
        RUN_ROOT / "run_manifest.json",
        {
            ...
            "bounded_question": "Can a new bounded model branch, informed by the Stage59X demotion decision, produce a post-ATR/risk adapter candidate without starting ONNX?",
            "source_stage59x_decision": rel(SOURCE_STAGE59X_DECISION),
            "source_stage59x_pushed_commit": SOURCE_STAGE59X_PUSHED_COMMIT,
            "variants": [
                {
                    **variant.__dict__,
                ...
                for variant in STAGE59Y_VARIANTS
            ],
...
def write_ledgers(
        ...
        [
            {
                "run_id": RUN_ID,
                "stage_id": STAGE59Y_ID,
                "lane": "baseline_adapter_new_model_branch_from_stage59x",
                "status": status,
                "judgment": decision,
    ...
    ledger_rows.append(
        {
            "ledger_row_id": f"{RUN_ID}__aggregate_new_model_branch_from_stage59x",
            "stage_id": STAGE59Y_ID,
            "run_id": RUN_ID,
            "subrun_id": "aggregate_new_model_branch_from_stage59x",
            "parent_run_id": PARENT_RUN_ID,
            "record_view": "new_model_branch_from_stage59x",
            "tier_scope": "Tier A+B",
            ...
            "external_verification_status": external,
            "notes": "Stage59Y bounded new model branch from Stage59X demotion evidence; not final package completion.",
        }
    )
    ...
    engine.write_json(
        SUMMARY_JSON_PATH,
        {
            ...
            "required_outputs": {
                ...
                "stage59y_decision": rel(DECISION_PATH),
            },
...
def report_markdown(summary_rows: Sequence[Mapping[str, Any]], decision: str, reasons: Sequence[str], external: str) -> str:
    ...
    best = best_repaired_variant(summary_rows)
    return f"""# Stage59Y New Model Branch From Stage59X Report(59Y단계 Stage59X 기반 새 모델 분기 보고서)

- stage(단계): `{STAGE59Y_ID}`
- run(실행): `{RUN_ID}`
...
- new_model_branch_risk_atr_telemetry(새 모델 분기 위험/ATR 기록): `{rel(RISK_ATR_TELEMETRY_PATH)}`
...
def equity_audit_markdown(summary_rows: Sequence[Mapping[str, Any]], segment_rows: Sequence[Mapping[str, Any]]) -> str:
    ...
    return f"""# Stage59Y New Model Branch Equity Curve Audit(59Y단계 새 모델 분기 자금 곡선 감사)
...
def decision_markdown(decision: str, reasons: Sequence[str], best: Mapping[str, Any], external: str) -> str:
    return f"""# Stage59Y Decision(59Y단계 판정)

decision(판정): `{decision}`

Stage59Y(59Y단계)는 Stage59X demotion evidence(Stage59X 강등 근거)를 사용한 bounded new model branch(경계 새 모델 분기)로 기록한다. Effect(효과): run50BR context-gap refill source family(run50BR 문맥 공백 보충 원천 계열)의 성공/실패를 다음 bounded stage(경계 다음 단계)의 입력 근거로 넘긴다.
...
Stage59Y closeout(59Y단계 종료)는 overall goal completion(전체 목표 완료)이 아니다. Effect(효과): Stage60 ONNX hardening(60단계 ONNX 경화)은 adapter quality(어댑터 품질)가 강할 때만 열린다.
...
def write_packet_files(
    ...
    files = {
        ...
        "experiment_design_receipt.json": {
            "hypothesis": "A bounded new model branch using run50BR context-gap refill sources may improve post-ATR/risk validation and OOS behavior after Stage59X demoted the Stage59S/V/W repair line.",
            ...
            "comparison_baseline": "Stage59X demoted the Stage59S/V/W repair line and Stage56 run50BR context-gap refill evidence.",
            ...
            "changed_variables": ["context_gap_refill_source", "max_hold_bars", "source_feature_csv"],
            ...
            "stop_conditions": "three bounded Stage59Y run50BR context-gap refill variants only",
        ...
        "artifact_lineage_audit.json": {
            "source_inputs": [
                rel(SOURCE_STAGE59X_DECISION),
                rel(SOURCE_STAGE59X_REPORT),
                rel(SOURCE_STAGE59X_SUMMARY),
                rel(SOURCE_STAGE59X_JSON),
                rel(RUN50BR_ROOT / "models/stage56_context_timed_event_signal_discrete_score_table.csv"),
                rel(RUN50BR_ROOT / RUN50BR_V64_ANCHOR),
                rel(RUN50BR_ROOT / RUN50BR_V65_ANCHOR),
                rel(RUN50BR_ROOT / RUN50BR_V67_ANCHOR),
            ],
            "producer": rel(Path("stage_pipelines/stage59y/new_model_branch_from_stage59x.py")),
...
- stage_id(단계 ID): `{STAGE59Y_ID}`
- source_stage(원천 단계): `{SOURCE_STAGE59X_ID}`
- source_decision(원천 판정): `open_new_model_branch`
- bounded_question(경계 질문): `Can a run50BR context-gap refill source branch produce a post-ATR/risk adapter candidate without starting ONNX?`
- boundary(경계): `{BOUNDARY}`

Stage59Y(59Y단계)는 run50BR context-gap refill source family(run50BR 문맥 공백 보충 원천 계열) 세 변형만 측정한다. Effect(효과): Stage59X(59X단계) 이후 새 분기를 작게 열되 Stage60 ONNX(60단계 ONNX)나 deployment(배포)로 이어가지 않는다.
""",
    )
    engine.write_md(
...
- stage59x_decision(59X단계 판정): `{rel(SOURCE_STAGE59X_DECISION)}`
- stage59x_report(59X단계 보고서): `{rel(SOURCE_STAGE59X_REPORT)}`
- stage59x_summary(59X단계 요약): `{rel(SOURCE_STAGE59X_SUMMARY)}`
- stage59x_pushed_commit(59X단계 푸시 커밋): `{SOURCE_STAGE59X_PUSHED_COMMIT}`
...
- run50br_v65_source(run50BR v65 원천): `{rel(RUN50BR_ROOT / RUN50BR_V65_ANCHOR)}`
- run50br_v67_source(run50BR v67 원천): `{rel(RUN50BR_ROOT / RUN50BR_V67_ANCHOR)}`

Effect(효과): Stage59Y(59Y단계)는 Stage59X(59X단계) 강등 근거와 run50BR context-gap refill(run50BR 문맥 공백 보충) 원천을 명시적으로 연결한다.
""",
    )
    engine.write_md(
...
- stage_status(단계 상태): `closed_bounded_new_model_branch_from_stage59x`
- source_stage(원천 단계): `{SOURCE_STAGE59X_ID}`
- source_decision(원천 판정): `open_new_model_branch`
- stage59y_decision(59Y단계 판정): `{decision}`
- next_stage_or_branch(다음 단계/분기): `{next_stage}`
- selected_research_baseline(선택 연구 기준선): `none`
- claim_boundary(주장 경계): `{BOUNDARY}`

Effect(효과): Stage59Y(59Y단계)는 측정 결과를 보존하지만 final package(최종 패키지)나 operating claim(운영 주장)을 만들지 않는다.
""",
    )
    engine.write_md(
        REVIEWS_ROOT / "review_index.md",
        f"""# Stage59Y Review Index(59Y단계 검토 색인)
...
- new_model_branch_risk_atr_telemetry(새 모델 분기 위험/ATR 기록): `{rel(RISK_ATR_TELEMETRY_PATH)}`
- stage59y_decision(59Y단계 판정): `{rel(DECISION_PATH)}`
""",
    )
    if not next_stage.startswith("60_"):
        ...
        engine.write_md(
            ...
            f"""# Stage59Z Brief(59Z단계 개요)

- stage_id(단계 ID): `{next_stage}`
- source_stage(원천 단계): `{STAGE59Y_ID}`
- source_decision(원천 판정): `{decision}`
- bounded_question(경계 질문): `What bounded follow-up should repair, demote, or branch from the Stage59Y new-model evidence without starting ONNX prematurely?`
- boundary(경계): `{BOUNDARY}`

Stage59Z(59Z단계)는 Stage59Y(59Y단계) 결과가 Stage60 ONNX(60단계 ONNX)에 충분하지 않을 때 이어지는 bounded follow-up(경계 후속)이다. Effect(효과): repair(수리), demotion(강등), branch(분기)를 다음 작은 질문으로 제한한다.
""",
        )
        engine.write_md(
            next_root / "01_inputs/input_refs.md",
            f"""# Stage59Z Input References(59Z단계 입력 참조)

- stage59y_decision(59Y단계 판정): `{rel(DECISION_PATH)}`
- stage59y_report(59Y단계 보고서): `{rel(REPORT_PATH)}`
- stage59y_summary(59Y단계 요약): `{rel(SUMMARY_CSV_PATH)}`
- stage59y_segment_kpi(59Y단계 구간 KPI): `{rel(SEGMENT_KPI_PATH)}`
- stage59y_risk_atr_telemetry(59Y단계 위험/ATR 기록): `{rel(RISK_ATR_TELEMETRY_PATH)}`
""",
        )
        engine.write_md(
...
""",
        ...
        engine.write_md(
            ...
            f"""# Stage59Z Selection Status(59Z단계 선택 상태)

- stage_status(단계 상태): `active_planned_from_stage59y`
- source_stage(원천 단계): `{STAGE59Y_ID}`
- source_decision(원천 판정): `{decision}`
- selected_research_baseline(선택 연구 기준선): `none`
- claim_boundary(주장 경계): `{BOUNDARY}`

Effect(효과): Stage59Z(59Z단계)는 Stage59Y(59Y단계) 이후 필요한 다음 작은 질문만 다룬다.
""",
        )
...
- status(상태): `stage59y_closed_{decision}`
...
""",
    ...
    focus = (
        "current_focus:\n"
        f"- >-\n"
        f"  Stage59Y(59Y단계) `{STAGE59Y_ID}` closed(종료) as bounded new model branch from Stage59X(Stage59X 기반 경계 새 모델 분기); decision(판정)=`{decision}`. "
        ...
        f"- >-\n"
        f"  Next stage_or_branch(다음 단계/분기) `{next_stage}` is active/planned(활성/계획). Effect(효과): Stage59Y(59Y단계) 결과를 다음 bounded step(경계 다음 단계)으로 넘긴다.\n"
    )
    text = re.sub(r"current_focus:\n", focus, text, count=1)
    block = f"""

stage59y_new_model_branch_from_stage59x:
  packet_id: {PACKET_ID}
  stage_id: {STAGE59Y_ID}
  status: closed_bounded_new_model_branch_from_stage59x
  ...
  boundary: {BOUNDARY}
"""
    if "stage59y_new_model_branch_from_stage59x:" in text:
        text = re.sub(r"\nstage59y_new_model_branch_from_stage59x:\n(?:  .*\n)*", block, text, count=1)
    else:
        text += block
...
def parse_args(argv: Sequence[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run Stage59Y bounded new model branch from Stage59X.")
...
def load_existing_result() -> dict[str, Any]:
    ...
    if not path_exists(manifest) or not path_exists(kpi):
        raise FileNotFoundError("Stage59Y existing run_manifest.json or kpi_record.json is missing")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.finalize_existing:
        ...
        prepared = {
            ...
            "model_family": "baseline_adapter_stage59y_new_model_branch_ebm_table",
            "feature_set_id": "stage59y_source_family_discrete_signal",
