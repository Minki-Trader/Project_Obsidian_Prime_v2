...
SOURCE_STAGE59B_ID = "59C_adapter_repair__new_model_source_branch"
STAGE59_ID = "59D_adapter_repair__source_lifecycle_or_demote"
NEXT_REPAIR_STAGE_ID = "59E_adapter_repair__demotion_or_new_branch"
RUN_NUMBER = "run57A"
RUN_ID = "run57A_stage59d_source_lifecycle_or_demote_v1"
PACKET_ID = "stage59d_source_lifecycle_or_demote_v1"
PARENT_RUN_ID = "run56A_stage59c_new_model_source_branch_v1"
...
SOURCE_STAGE59B_ROOT = Path("stages") / SOURCE_STAGE59B_ID
SOURCE_STAGE59B_DECISION = SOURCE_STAGE59B_ROOT / "03_reviews/stage59c_decision.md"
SOURCE_STAGE59B_REPORT = SOURCE_STAGE59B_ROOT / "03_reviews/new_model_source_branch_report.md"
SOURCE_STAGE59B_SUMMARY = SOURCE_STAGE59B_ROOT / "03_reviews/new_model_source_branch_summary.csv"
SOURCE_STAGE59B_SEGMENTS = SOURCE_STAGE59B_ROOT / "03_reviews/new_model_source_segment_kpi_summary.csv"
SOURCE_STAGE59B_RISK = SOURCE_STAGE59B_ROOT / "03_reviews/new_model_source_risk_atr_telemetry.csv"

DEVELOPMENT_SIGNAL_COLUMN = "stage56_context_gap_refill_signal"
BACKUP_SIGNAL_COLUMN = "stage56_context_et_firewall_signal"
COMMON_ROOT = f"Project_Obsidian_Prime_v2/stage59d/{RUN_NUMBER}_source_lifecycle_or_demote"

REPORT_PATH = REVIEWS_ROOT / "source_lifecycle_or_demote_report.md"
SUMMARY_JSON_PATH = REVIEWS_ROOT / "source_lifecycle_or_demote_summary.json"
SUMMARY_CSV_PATH = REVIEWS_ROOT / "source_lifecycle_or_demote_summary.csv"
SEGMENT_KPI_PATH = REVIEWS_ROOT / "source_lifecycle_or_demote_segment_kpi_summary.csv"
EQUITY_AUDIT_PATH = REVIEWS_ROOT / "source_lifecycle_or_demote_equity_curve_audit.md"
RISK_ATR_TELEMETRY_PATH = REVIEWS_ROOT / "source_lifecycle_or_demote_risk_atr_telemetry.csv"
DECISION_PATH = REVIEWS_ROOT / "stage59d_decision.md"
AUDIT_CSV_PATH = REVIEWS_ROOT / "stage59d_trade_audit.csv"
...
STAGE59B_PUSHED_COMMIT = "3887de776c0bc84bf06229e78e0d65aa414941e4"

STAGE59_VARIANTS = (
    repair.RepairVariant(
        adapter_id="s59d_v64_control_thr57_mr03_wideatr_sd5",
        ...
        long_threshold=0.57,
        notes="Stage59D bounded control: v64 source with Stage59C best threshold, 3% model-controlled risk cap, wide ATR, and native lifecycle.",
    ),
    repair.RepairVariant(
        adapter_id="s59d_v64_closeflat_thr57_mr03_wideatr_sd5",
        label="v64_close_on_flat_threshold57_risk3pct_wide_atr_sd5",
        atr_enabled=True,
        model_risk_enabled=True,
        fixed_lot=0.25,
        atr_stop_multiplier=2.5,
        ...
        notes="Stage59D source lifecycle test: v64 source closes on flat signal to reduce stale exposure before demotion.",
    ),
    repair.RepairVariant(
        adapter_id="s59d_v64_closeonlyopp_thr57_mr03_wideatr_sd5",
        label="v64_close_only_on_opposite_threshold57_risk3pct_wide_atr_sd5",
        atr_enabled=True,
        model_risk_enabled=True,
        fixed_lot=0.25,
        atr_stop_multiplier=2.5,
        ...
        notes="Stage59D source lifecycle test: v64 source closes on opposite signal without immediate reversal to reduce churn and same-move concentration.",
    ),
    repair.RepairVariant(
        adapter_id="s59d_v64_hold3_thr57_mr03_wideatr_sd5",
        label="v64_hold3_threshold57_risk3pct_wide_atr_sd5",
        atr_enabled=True,
        model_risk_enabled=True,
        fixed_lot=0.25,
        atr_stop_multiplier=2.5,
        ...
        long_threshold=0.57,
        max_hold_bars=3,
        notes="Stage59D source lifecycle test: v64 source allows 3-bar hold to test MFE capture versus drawdown and cost stress before demotion.",
    ),
)
...
SOURCE_SPECS: dict[str, dict[str, Any]] = {
    variant.adapter_id: {
        ...
        "anchor": DEVELOPMENT_ANCHOR,
        "model": SOURCE_MODEL,
        "signal_column": DEVELOPMENT_SIGNAL_COLUMN,
        "validation_ini": SOURCE_VARIANT_ROOT / "mt5/x01_ta_val.ini",
        "oos_ini": SOURCE_VARIANT_ROOT / "mt5/x01_ta_oos.ini",
        "source_note": "Stage56 v64 development anchor control",
    }
...
def source_spec_for_variant(variant: repair.RepairVariant | str) -> dict[str, Any]:
    adapter_id = variant.adapter_id if isinstance(variant, repair.RepairVariant) else str(variant)
    try:
        return SOURCE_SPECS[adapter_id]
    except KeyError as exc:
        raise KeyError(f"Missing Stage59D source spec for {adapter_id}") from exc


def source_root_for_variant(variant: repair.RepairVariant | str) -> Path:
    return Path(source_spec_for_variant(variant)["variant_root"])


def source_anchor_for_variant(variant: repair.RepairVariant | str) -> str:
    return str(source_spec_for_variant(variant)["anchor"])


def source_model_for_variant(variant: repair.RepairVariant | str) -> Path:
    return Path(source_spec_for_variant(variant)["model"])


def signal_column_for_variant(variant: repair.RepairVariant | str) -> str:
    return str(source_spec_for_variant(variant)["signal_column"])


def source_label_for_variant(variant: repair.RepairVariant | str) -> str:
    return str(source_spec_for_variant(variant)["label"])
...
def source_attempt_ini(split: str, variant: repair.RepairVariant | str) -> Path:
    spec = source_spec_for_variant(variant)
    return Path(spec["validation_ini"] if split == "validation_is" else spec["oos_ini"])
...
def prepare_inputs(common_files_root: Path) -> dict[str, Any]:
    ...
    for variant in STAGE59_VARIANTS:
        source_label = source_label_for_variant(variant)
        ...
        for split in ("validation_is", "oos"):
            ...
            feature_local = RUN_ROOT / variant.adapter_id / "features" / f"{variant.adapter_id}_stage59d_adapter_a_{token}.csv"
    ...
    for variant_index, variant in enumerate(STAGE59_VARIANTS, start=1):
        ...
        for split in ("validation_is", "oos"):
            ...
            for role_index, (tier, attempt_role, prefix, attempt_token) in enumerate(
                ...
                magic = 5905700 + variant_index * 100 + (1 if split == "validation_is" else 50) + role_index
                attempts.append(
                    attempt_payload(
                        run_root=variant_root,
                        run_id=RUN_ID,
                        stage_number=59,
                        exploration_label="stage59D_BaselineAdapter__SourceLifecycleOrDemote",
...
def load_existing_result() -> dict[str, Any]:
    ...
    if not path_exists(manifest) or not path_exists(kpi):
        raise FileNotFoundError("Stage59D existing run_manifest.json or kpi_record.json is missing")
...
def repair_failure_reasons(summary_rows: Sequence[Mapping[str, Any]], segment_rows: Sequence[Mapping[str, Any]]) -> list[str]:
    ...
    blocked_or_missing = [
        ...
        if row.get("view") == "actual_routed_total" and str(row.get("status") or "") != "completed"
    ]
    if blocked_or_missing:
        reasons.append("stage59d_lifecycle_variant_missing_or_blocked")
            ...
            rows.append(
                {
                    ...
                    "stage_id": STAGE59_ID,
                    "run_id": RUN_ID,
                    "artifact_type": "stage59d_source_lifecycle_or_demote_evidence",
                    "path": rel(path),
                    "sha256": sha256_file_lf_normalized(path),
                    "hash_policy": "lf_normalized_text" if path.suffix.lower() in {".csv", ".json", ".md"} else "raw_file",
                    "created_at_utc": created,
                    "notes": "Stage59D bounded source lifecycle or demote artifact.",
                }
            )
            ...
            rows.append(
                {
                    ...
                    "created_at_utc": created,
                    "notes": "Actual Stage59D MT5 Strategy Tester HTML report.",
                }
            )
    return rows
...
def write_run_identity(result: Mapping[str, Any]) -> None:
    write_json(
        RUN_ROOT / "run_manifest.json",
        {
            ...
            "bounded_question": "Can source lifecycle or demotion routing repair the remaining post-Stage59C weakness without starting ONNX?",
            "source_stage59c_decision": rel(SOURCE_STAGE59B_DECISION),
            "source_stage59c_pushed_commit": STAGE59B_PUSHED_COMMIT,
            "variants": [
                {
                    **variant.__dict__,
                    "source_anchor": source_anchor_for_variant(variant),
...
def write_ledgers(
        ...
        [
            {
                "run_id": RUN_ID,
                "stage_id": STAGE59_ID,
                "lane": "baseline_adapter_source_lifecycle_or_demote",
                "status": status,
                "judgment": decision,
    ...
    ledger_rows.append(
        {
            "ledger_row_id": f"{RUN_ID}__aggregate_source_lifecycle_or_demote",
            "stage_id": STAGE59_ID,
            "run_id": RUN_ID,
            "subrun_id": "aggregate_source_lifecycle_or_demote",
            "parent_run_id": PARENT_RUN_ID,
            "record_view": "source_lifecycle_or_demote",
            "tier_scope": "Tier A+B",
            ...
            "external_verification_status": external,
            "notes": "Stage59D bounded source lifecycle or demote; not final package completion.",
        }
    )
...
def report_markdown(summary_rows: Sequence[Mapping[str, Any]], decision: str, reasons: Sequence[str], external: str) -> str:
    ...
    best = best_repaired_variant(summary_rows)
    return f"""# Stage59D Source Lifecycle Or Demote Report(59D단계 원천 생명주기 또는 강등 보고서)

- stage(단계): `{STAGE59_ID}`
- run(실행): `{RUN_ID}`
- source_adapter(원천 어댑터): `{SOURCE_ADAPTER_ID}`
- source_stage59c_commit(원천 59C단계 커밋): `{STAGE59B_PUSHED_COMMIT}`
...
def equity_audit_markdown(summary_rows: Sequence[Mapping[str, Any]], segment_rows: Sequence[Mapping[str, Any]]) -> str:
    ...
    return f"""# Stage59D Source Lifecycle Equity Curve Audit(59D단계 원천 생명주기 자금 곡선 감사)
...
def decision_markdown(decision: str, reasons: Sequence[str], best: Mapping[str, Any], external: str) -> str:
    return f"""# Stage59D Decision(59D단계 판정)
    ...
    write_json(
        SUMMARY_JSON_PATH,
        {
            ...
            "required_outputs": {
                ...
                "stage59d_decision": rel(DECISION_PATH),
            },
...
def write_packet_files(
    ...
    files = {
        ...
        "experiment_design_receipt.json": {
            "hypothesis": "Changing source lifecycle controls may repair the remaining Stage59C validation cost-stressed weakness while preserving ATR bracket and model-controlled risk behavior",
            ...
            "comparison_baseline": "Stage59C best adapter s59c_v64_control_thr57_mr03_wideatr_sd5",
            "control_variables": ["US100", "M5", "split_v1", "Tier B disabled", "wide ATR bracket", "3% model-controlled risk cap", "ONNX deferred"],
            "changed_variables": ["close_on_flat_signal", "reverse_on_opposite_signal", "close_only_on_opposite_signal", "max_hold_bars"],
            ...
            "stop_condition": "four bounded Stage59D lifecycle variants only",
            "status": "completed",
        },
        ...
        "artifact_lineage_audit.json": {
            "source_inputs": [rel(SOURCE_STAGE59B_DECISION), rel(SOURCE_STAGE59B_REPORT), rel(SOURCE_STAGE59B_SUMMARY), rel(SOURCE_MODEL)],
...
- source_stage(원천 단계): `{SOURCE_STAGE59B_ID}`
...
- stage59b_decision(59B단계 판정): `{rel(SOURCE_STAGE59B_DECISION)}`
- stage59b_report(59B단계 보고서): `{rel(SOURCE_STAGE59B_REPORT)}`
- stage59b_summary(59B단계 요약): `{rel(SOURCE_STAGE59B_SUMMARY)}`
- stage59b_segment_kpi(59B단계 구간 KPI): `{rel(SOURCE_STAGE59B_SEGMENTS)}`
- stage59b_risk_telemetry(59B단계 위험 텔레메트리): `{rel(SOURCE_STAGE59B_RISK)}`
- stage59b_pushed_commit(59B단계 푸시 커밋): `{STAGE59B_PUSHED_COMMIT}`
- control_source_model(대조군 원천 모델): `{rel(SOURCE_MODEL)}`
- new_source_model(새 원천 모델): `{rel(S43_MODEL)}`
""",
    )
    write_md(
...
- stage_status(단계 상태): `closed_bounded_new_model_source_branch`
- source_stage(원천 단계): `{SOURCE_STAGE59B_ID}`
- source_decision(원천 판정): `continue_repair_in_new_bounded_stage`
- stage59c_decision(59C단계 판정): `{decision}`
- next_stage_or_branch(다음 단계/분기): `{next_stage}`
- selected_research_baseline(선택 연구 기준선): `none`
- claim_boundary(주장 경계): `{BOUNDARY}`

Effect(효과): Stage59C(59C단계)는 new model source branch(새 모델 원천 분기) 결과를 보존하지만 final package(최종 패키지)나 operating claim(운영 주장)을 만들지 않는다.
""",
    )
...
""",
        ...
        write_md(
            ...
            f"""# Stage59D Input References(59D단계 입력 참조)

- stage59c_decision(59C단계 판정): `{rel(DECISION_PATH)}`
- new_model_source_report(새 모델 원천 보고서): `{rel(REPORT_PATH)}`
- new_model_source_summary(새 모델 원천 요약): `{rel(SUMMARY_CSV_PATH)}`
- new_model_source_segment_kpi(새 모델 원천 구간 KPI): `{rel(SEGMENT_KPI_PATH)}`
- new_model_source_risk_atr_telemetry(새 모델 원천 위험/ATR 텔레메트리): `{rel(RISK_ATR_TELEMETRY_PATH)}`
""",
        )
        write_md(
            next_root / "03_reviews/review_index.md",
            """# Stage59D Review Index(59D단계 검토 색인)

Stage59D(59D단계)는 아직 planned(계획) 상태다. Effect(효과): Stage59C(59C단계) 약점 기록을 다음 bounded repair(경계 수리)에 연결한다.
""",
        )
        write_md(
            next_root / "04_selected/selection_status.md",
            f"""# Stage59D Selection Status(59D단계 선택 상태)

- stage_status(단계 상태): `active_planned_from_stage59c`
- source_stage(원천 단계): `{STAGE59_ID}`
- source_decision(원천 판정): `{decision}`
- selected_research_baseline(선택 연구 기준선): `none`
- claim_boundary(주장 경계): `{BOUNDARY}`

Effect(효과): Stage59D(59D단계)는 Stage59C(59C단계)의 남은 약점을 source lifecycle(원천 생명주기) 또는 demotion routing(강등 라우팅)으로만 다룬다.
""",
        )
    write_md(
        REVIEWS_ROOT / "review_index.md",
        f"""# Stage59C Review Index(59C단계 검토 색인)

- new_model_source_report(새 모델 원천 보고서): `{rel(REPORT_PATH)}`
- new_model_source_summary(새 모델 원천 요약): `{rel(SUMMARY_CSV_PATH)}`
- new_model_source_segment_kpi(새 모델 원천 구간 KPI): `{rel(SEGMENT_KPI_PATH)}`
- new_model_source_equity_curve_audit(새 모델 원천 자금 곡선 감사): `{rel(EQUITY_AUDIT_PATH)}`
- new_model_source_risk_atr_telemetry(새 모델 원천 위험/ATR 텔레메트리): `{rel(RISK_ATR_TELEMETRY_PATH)}`
- stage59c_decision(59C단계 판정): `{rel(DECISION_PATH)}`
""",
    )
...
def update_current_truth(decision: str, summary_rows: Sequence[Mapping[str, Any]], external: str) -> None:
    best = best_repaired_variant(summary_rows)
    next_stage = next_stage_for_decision(decision)
    next_packet = "stage59d_source_lifecycle_or_demote_v1" if decision == "continue_repair_in_new_bounded_stage" else "stage60_onnx_hardening_v1"
    next_run = "run57A_stage59d_source_lifecycle_or_demote_v1" if decision == "continue_repair_in_new_bounded_stage" else "run57A_stage60_onnx_hardening_v1"
    write_md(
        CURRENT_WORKING_STATE_PATH,
        f"""# Current Working State(현재 작업 상태)
...
- backup_anchor(예비 기준점): `{BACKUP_ANCHOR}`
- adapter_under_review(검토 중 어댑터): `{SOURCE_ADAPTER_ID}`
- status(상태): `stage59c_closed_{decision}`
- claim_boundary(주장 경계): research/development only(연구/개발 전용)
...
""",
    ...
    focus = (
        "current_focus:\n"
        f"- >-\n"
        f"  Stage59C(59C단계) `{STAGE59_ID}` closed(종료) as bounded new model source branch(경계 새 모델 원천 분기); decision(판정)=`{decision}`. "
        f"Effect(효과): full adapter repair(전체 어댑터 수리)는 계속 진행 중이며 final(최종) 또는 operating(운영) 주장은 없다.\n"
        f"- >-\n"
        f"  Next stage_or_branch(다음 단계/분기) `{next_stage}` is active/planned(활성/계획). Effect(효과): Stage59C(59C단계) 결과를 다음 bounded step(경계 다음 단계)으로 넘긴다.\n"
    )
    text = re.sub(r"current_focus:\n", focus, text, count=1)
    block = f"""

stage59c_new_model_source_branch:
  packet_id: {PACKET_ID}
  stage_id: {STAGE59_ID}
  status: closed_bounded_new_model_source_branch
  current_run_id: {RUN_ID}
  source_adapter: {SOURCE_ADAPTER_ID}
  source_stage59b_pushed_commit: {STAGE59B_PUSHED_COMMIT}
  best_repaired_adapter: {best.get('adapter_id', 'none')}
  decision: {decision}
  next_stage_or_branch: {next_stage}
  report_path: {rel(DECISION_PATH)}
  ...
  boundary: {BOUNDARY}
"""
    if "stage59c_new_model_source_branch:" in text:
        text = re.sub(r"\nstage59c_new_model_source_branch:\n(?:  .*\n)*", block, text, count=1)
    else:
        text += block
    io_path(WORKSPACE_STATE_PATH).write_text(text, encoding="utf-8-sig")
...
def append_changelog(decision: str) -> None:
    entry = (
        ...
        f"- run(실행): `{RUN_ID}`\n"
        f"- decision(판정): `{decision}`\n"
        f"- effect(효과): Stage59B(59B단계) 이후 남은 cost-stressed weakness(비용 압박 약점)를 v64 control(v64 대조군)과 Stage43 new source(Stage43 새 원천)로 측정하고 다음 단계/분기 조건을 남겼다.\n"
    )
    existing = io_path(CHANGELOG_PATH).read_text(encoding="utf-8-sig") if path_exists(CHANGELOG_PATH) else ""
    if f"- run(실행): `{RUN_ID}`" not in existing:
        io_path(CHANGELOG_PATH).write_text(existing.rstrip() + entry, encoding="utf-8-sig")
...
- stage59c_decision(59C단계 판정): `{rel(SOURCE_STAGE59B_DECISION)}`
- stage59c_report(59C단계 보고서): `{rel(SOURCE_STAGE59B_REPORT)}`
- stage59c_summary(59C단계 요약): `{rel(SOURCE_STAGE59B_SUMMARY)}`
- stage59c_segment_kpi(59C단계 구간 KPI): `{rel(SOURCE_STAGE59B_SEGMENTS)}`
- stage59c_risk_telemetry(59C단계 위험 텔레메트리): `{rel(SOURCE_STAGE59B_RISK)}`
- stage59c_pushed_commit(59C단계 푸시 커밋): `{STAGE59B_PUSHED_COMMIT}`
- control_source_model(대조군 원천 모델): `{rel(SOURCE_MODEL)}`
""",
    )
    write_md(
...
- source_stage(원천 단계): `{SOURCE_STAGE59B_ID}`
- source_decision(원천 판정): `continue_repair_in_new_bounded_stage`
- stage59d_decision(59D단계 판정): `{decision}`
- next_stage_or_branch(다음 단계/분기): `{next_stage}`
- selected_research_baseline(선택 연구 기준선): `none`
- claim_boundary(주장 경계): `{BOUNDARY}`
...
Stage59E(59E단계)는 Stage59D(59D단계) 후에도 남은 약점을 demotion(강등) 또는 new bounded model branch(새 경계 모델 분기)로 다루는 계획 단계다. Effect(효과): Stage60 ONNX(60단계 ONNX)는 adapter quality(어댑터 품질)가 강해질 때까지 열지 않는다.
""",
        )
        write_md(
            next_root / "01_inputs/input_refs.md",
            f"""# Stage59E Input References(59E단계 입력 참조)

- stage59d_decision(59D단계 판정): `{rel(DECISION_PATH)}`
- source_lifecycle_report(원천 생명주기 보고서): `{rel(REPORT_PATH)}`
- source_lifecycle_summary(원천 생명주기 요약): `{rel(SUMMARY_CSV_PATH)}`
- source_lifecycle_segment_kpi(원천 생명주기 구간 KPI): `{rel(SEGMENT_KPI_PATH)}`
- source_lifecycle_risk_atr_telemetry(원천 생명주기 위험/ATR 텔레메트리): `{rel(RISK_ATR_TELEMETRY_PATH)}`
""",
        )
        write_md(
            next_root / "03_reviews/review_index.md",
            """# Stage59E Review Index(59E단계 검토 색인)

Stage59E(59E단계)는 아직 planned(계획) 상태다. Effect(효과): Stage59D(59D단계) 약점 기록을 다음 bounded repair(경계 수리)에 연결한다.
""",
        )
        write_md(
            next_root / "04_selected/selection_status.md",
...
- stage_status(단계 상태): `active_planned_from_stage59d`
- source_stage(원천 단계): `{STAGE59_ID}`
- source_decision(원천 판정): `{decision}`
- selected_research_baseline(선택 연구 기준선): `none`
- claim_boundary(주장 경계): `{BOUNDARY}`

Effect(효과): Stage59E(59E단계)는 Stage59D(59D단계)의 남은 약점을 demotion(강등) 또는 new branch(새 분기)로만 다룬다.
""",
        )
    write_md(
        REVIEWS_ROOT / "review_index.md",
        f"""# Stage59D Review Index(59D단계 검토 색인)

- source_lifecycle_report(원천 생명주기 보고서): `{rel(REPORT_PATH)}`
- source_lifecycle_summary(원천 생명주기 요약): `{rel(SUMMARY_CSV_PATH)}`
- source_lifecycle_segment_kpi(원천 생명주기 구간 KPI): `{rel(SEGMENT_KPI_PATH)}`
- source_lifecycle_equity_curve_audit(원천 생명주기 자금 곡선 감사): `{rel(EQUITY_AUDIT_PATH)}`
- source_lifecycle_risk_atr_telemetry(원천 생명주기 위험/ATR 텔레메트리): `{rel(RISK_ATR_TELEMETRY_PATH)}`
- stage59d_decision(59D단계 판정): `{rel(DECISION_PATH)}`
""",
    )
...
def update_current_truth(decision: str, summary_rows: Sequence[Mapping[str, Any]], external: str) -> None:
    best = best_repaired_variant(summary_rows)
    next_stage = next_stage_for_decision(decision)
    next_packet = "stage59e_demotion_or_new_branch_v1" if decision == "continue_repair_in_new_bounded_stage" else "stage60_onnx_hardening_v1"
    next_run = "run58A_stage59e_demotion_or_new_branch_v1" if decision == "continue_repair_in_new_bounded_stage" else "run58A_stage60_onnx_hardening_v1"
    write_md(
        CURRENT_WORKING_STATE_PATH,
        f"""# Current Working State(현재 작업 상태)
...
- backup_anchor(예비 기준점): `{BACKUP_ANCHOR}`
- adapter_under_review(검토 중 어댑터): `{SOURCE_ADAPTER_ID}`
- status(상태): `stage59d_closed_{decision}`
- claim_boundary(주장 경계): research/development only(연구/개발 전용)

Stage59D(59D단계) closed(종료) as bounded source lifecycle or demote(경계 원천 생명주기 또는 강등). Effect(효과): lifecycle controls(생명주기 제어)는 measured(측정됨)됐지만 final adapter(최종 어댑터) 또는 overall goal complete(전체 목표 완료)는 아니다.

## Latest Stage59D Evidence(최신 59D단계 근거)
...
""",
    ...
    focus = (
        "current_focus:\n"
        f"- >-\n"
        f"  Stage59D(59D단계) `{STAGE59_ID}` closed(종료) as bounded source lifecycle or demote(경계 원천 생명주기 또는 강등); decision(판정)=`{decision}`. "
        f"Effect(효과): full adapter repair(전체 어댑터 수리)는 계속 진행 중이며 final(최종) 또는 operating(운영) 주장은 없다.\n"
        f"- >-\n"
        f"  Next stage_or_branch(다음 단계/분기) `{next_stage}` is active/planned(활성/계획). Effect(효과): Stage59D(59D단계) 결과를 다음 bounded step(경계 다음 단계)으로 넘긴다.\n"
    )
    text = re.sub(r"current_focus:\n", focus, text, count=1)
    block = f"""

stage59d_source_lifecycle_or_demote:
  packet_id: {PACKET_ID}
  stage_id: {STAGE59_ID}
  status: closed_bounded_source_lifecycle_or_demote
  current_run_id: {RUN_ID}
  source_adapter: {SOURCE_ADAPTER_ID}
  source_stage59c_pushed_commit: {STAGE59B_PUSHED_COMMIT}
  best_repaired_adapter: {best.get('adapter_id', 'none')}
  ...
  boundary: {BOUNDARY}
"""
    if "stage59d_source_lifecycle_or_demote:" in text:
        text = re.sub(r"\nstage59d_source_lifecycle_or_demote:\n(?:  .*\n)*", block, text, count=1)
    else:
        text += block
    io_path(WORKSPACE_STATE_PATH).write_text(text, encoding="utf-8-sig")
...
def append_changelog(decision: str) -> None:
    entry = (
        ...
        f"- run(실행): `{RUN_ID}`\n"
        f"- decision(판정): `{decision}`\n"
        f"- effect(효과): Stage59C(59C단계) 이후 남은 validation weakness(검증 약점)를 v64 lifecycle controls(v64 생명주기 제어)로 측정하고 다음 단계/분기 조건을 남겼다.\n"
    )
    existing = io_path(CHANGELOG_PATH).read_text(encoding="utf-8-sig") if path_exists(CHANGELOG_PATH) else ""
    if f"- run(실행): `{RUN_ID}`" not in existing:
        io_path(CHANGELOG_PATH).write_text(existing.rstrip() + entry, encoding="utf-8-sig")
...
def parse_args(argv: Sequence[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run Stage59D bounded source lifecycle or demote.")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.finalize_existing:
        ...
        prepared = {
            ...
            "model_family": "baseline_adapter_stage59d_source_lifecycle_ebm_table",
            "feature_set_id": "stage59d_v64_lifecycle_signal",
