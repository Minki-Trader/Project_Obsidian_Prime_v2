...
RUN_ID = "run319B_execute_curve_pocket_risk_asymmetry_mt5_probe_v1"
RUN_NUMBER = "run319B"
SOURCE_RUN_ID = "run319A_design_curve_pocket_risk_asymmetry_rebuild_packet_v1"
PARENT_RUN_ID = "run318C_review_post_non_time_curve_stability_mt5_probe_v1"
STATUS_PREPARED = "prepared_curve_pocket_risk_asymmetry_mt5_probe_no_runtime_kpi"
UPDATED_ON = "2026-05-25"
EXPLORATION_LABEL = "stage319_Model__CurvePocketRiskAsymmetryReplay"
SIGNAL_COLUMN = "run319b_route_signal"
FEATURE_ORDER = (SIGNAL_COLUMN,)
COMMON_ROOT = "Project_Obsidian_Prime_v2/stage319/run319B_curve_pocket_risk_asymmetry"
...
MT5_QUEUE = RUN319A / "mt5_probe_queue.csv"
SOURCE_MANIFEST = RUN319A / "candidate_payload_manifest.csv"
SOURCE_RUN_MANIFEST = RUN319A / "run_manifest.json"
SOURCE_REPORT = REVIEWS / "run319A_materialization.md"
PRODUCER = Path("stage_pipelines/stage319/execute_curve_pocket_risk_asymmetry_mt5_probe.py")
...
REPORT = REVIEWS / "run319B_mt5_probe.md"
...
def classify_status(result: Mapping[str, Any], materialize_only: bool) -> tuple[str, str, str, str]:
    ...
    if materialize_only:
        return (
            STATUS_PREPARED,
            "curve_pocket_risk_asymmetry_runtime_probe_prepared_no_external_execution",
            "out_of_scope_by_claim_materialize_only",
            "run319B_execute_curve_pocket_risk_asymmetry_mt5_probe_external_check",
        )
    ...
    if planned and completed_exec >= planned and len(kpis) >= planned:
        return (
            "completed_curve_pocket_risk_asymmetry_mt5_probe_no_selection",
            "runtime_probe_completed_requires_profit_scale_curve_review_no_selection",
            "completed",
            "run319C_review_curve_pocket_risk_asymmetry_mt5_probe",
        )
    if kpis:
        return (
            "partial_curve_pocket_risk_asymmetry_mt5_probe_no_selection",
            "runtime_probe_partial_requires_continuation_or_review_no_selection",
            "partial_or_blocked",
            "run319B_continue_curve_pocket_risk_asymmetry_mt5_probe" if limited else "run319C_review_with_runtime_gaps",
        )
    return (
        "blocked_curve_pocket_risk_asymmetry_mt5_probe_no_kpi",
        "runtime_probe_blocked_no_kpi_no_selection",
        "blocked_or_invalid",
        "run319B_repair_or_block_curve_pocket_risk_asymmetry_mt5_probe",
    )
...
def report_markdown(result: Mapping[str, Any], status: str, judgment: str, external_status: str, next_action: str) -> str:
    ...
    return "\n".join(
        [
            "# run319B Curve-Pocket Risk Asymmetry MT5 Probe(319B 곡선 포켓 위험 비대칭 MT5 탐침)",
            "",
            ...
            "",
            "Effect(효과): run319A(319A 실행)의 curve-pocket risk asymmetry route_signal_value(곡선 포켓 위험 비대칭 경로 신호)를 MT5 tester(MT5 테스터)에 넣어 실제 trade count(거래 수), net profit(순수익), PF(수익 팩터), DD(drawdown, 드로다운), recovery(회복)를 확인한다.",
            "",
            f"`{e310.BOUNDARY}`",
        ]
    )
...
def upsert_ledgers(result: Mapping[str, Any], status: str, judgment: str, external_status: str, next_action: str) -> None:
    ...
    ledger.upsert_csv_rows(
        RUN_REGISTRY,
        base.RUN_REGISTRY_COLUMNS,
        [{"run_id": RUN_ID, "stage_id": STAGE_ID, "lane": "curve_pocket_risk_asymmetry_mt5_probe", "status": status, "judgment": judgment, "path": base.rel(REPORT), "notes": f"attempts={attempt_count};mt5_kpi_records={kpi_count};selected_candidate=none;onnx_readiness=not_claimed;next_action={next_action}."}],
        key="run_id",
    )
    ledger.upsert_csv_rows(
        ALPHA_LEDGER,
        ...
        [
            {
                ...
                "record_view": "curve_pocket_risk_asymmetry_mt5_probe",
                "tier_scope": "Tier A used/Tier B fallback stress/actual routed total",
                "kpi_scope": "runtime_probe",
                "scoreboard_lane": "curve_pocket_risk_asymmetry",
                "status": status,
                "judgment": judgment,
    ...
    ledger.upsert_csv_rows(
        ...
        [{"row_id": f"{RUN_ID}__mt5_probe", "stage_id": STAGE_ID, "run_id": RUN_ID, "view": "curve_pocket_risk_asymmetry_mt5_probe", "tier_scope": "Tier A used/Tier B fallback/actual routed total", "scoreboard": "runtime_probe", "status": status, "judgment": judgment, "evidence_boundary": "runtime_probe_no_candidate_no_onnx", "report_path": base.rel(REPORT), "notes": f"attempts={attempt_count};mt5_kpi_records={kpi_count}."}],
        key="row_id",
    )
...
def update_docs(status: str, judgment: str, next_action: str, kpi_count: int, attempt_count: int) -> None:
    ...
    selected = base.append_once(selected, "run319B_report", f"- run319B_report(319B 보고서): `{base.rel(REPORT)}`")
    selected = base.append_once(selected, "run319B_execution_result", f"- run319B_execution_result(319B 실행 결과): `{base.rel(EXECUTION_RESULT)}`")
    save_md(SELECTED, selected)

    review_index = base.io_path(REVIEW_INDEX).read_text(encoding="utf-8-sig") if base.path_exists(REVIEW_INDEX) else "# Stage319 Review Index(319단계 검토 색인)\n"
    review_index = base.append_once(review_index, "run319B_report", f"- run319B_report(319B 보고서): `{base.rel(REPORT)}`\n- run319B_execution_result(319B 실행 결과): `{base.rel(EXECUTION_RESULT)}`\n- run319B_mt5_kpi_summary(319B MT5 핵심 성과 지표 요약): `{base.rel(MT5_KPI_SUMMARY)}`")
    ...
    current = base.append_once(current, "run319B_summary", f"- run319B_summary(319B 요약): curve-pocket risk asymmetry MT5 probe(곡선 포켓 위험 비대칭 MT5 탐침)를 실행했다. Effect(효과): attempts(시도) `{attempt_count}`개와 MT5 KPI records(MT5 핵심 성과 지표 기록) `{kpi_count}`개를 만들었고 후보/Adapter(어댑터)/ONNX(온엑스)는 주장하지 않는다.")
    ...
    focus = f"- >-\n  Stage319(319단계) run319B(319B 실행) curve-pocket risk asymmetry MT5 probe(곡선 포켓 위험 비대칭 MT5 탐침) `{RUN_ID}`. Effect(효과): attempts(시도) `{attempt_count}`개와 MT5 KPI records(MT5 핵심 성과 지표 기록) `{kpi_count}`개를 기록했고 selected candidate(선택 후보), Adapter package(어댑터 패키지), ONNX readiness(온엑스 준비)는 주장하지 않는다.\n"
