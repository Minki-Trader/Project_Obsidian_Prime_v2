...
RUN_ID = "frontier77D_mt5_lifecycle_negative_control_runtime_probe_v1"
PARENT_RUN_ID = "frontier77C_pre_mt5_grok_lifecycle_negative_control_runtime_probe_v1"
NEXT_RUN_ID = "frontier77E_proxy_runtime_gap_analysis_and_repair_decision_v1"
COMMON_RUN_ROOT = "Project_Obsidian_Prime_v2/frontier77D_mt5_lifecycle_negative_control_runtime_probe"
THRESHOLD_EPSILON = 1e-7
RUNTIME_CANDIDATE_PREFIX = "f77d_runtime"
SLTP_POINT_SCALE = 1.0
TRADE_SHAPE_LABEL = "short_only_max_hold_12_fixed_tp18_sl12_points"
RUN_SHORT_LABEL = "F77D"
ATTEMPT_PREFIX = "f77d_lifecycle_negative_control"
EXPLORATION_LABEL = "frontier77D_lifecycle_negative_control_runtime_probe"
ATTEMPT_ROLE = "lifecycle_negative_control_runtime_probe"
RECORD_VIEW_PREFIX = "mt5_f77d_lifecycle_negative_control"
CLAIM_BOUNDARY = (
...
REPORT_PATH = REVIEW_DIR / "frontier77D_mt5_lifecycle_negative_control_runtime_probe_report.md"
GATE_AUDIT_PATH = REVIEW_DIR / "required_gate_coverage_audit_f77d.md"
SUMMARY_PATH = REVIEW_DIR / "f77d_mt5_lifecycle_runtime_probe_summary.json"
RUN_MANIFEST_PATH = RUN_DIR / "run_manifest.json"
CONTEXT_ANCHOR_PATH = f"stages/{STAGE_ID}/03_reviews/context_anchor.md"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="F77D MT5 lifecycle negative-control runtime probe.")
...
def target_row() -> dict[str, Any]:
    ...
    if target.get("candidate_id") != "f77b_07979":
        raise RuntimeError(f"target_lock_failed:{target.get('candidate_id')}")
    if target.get("model") != "extra_trees_d7_l80":
...
def cleaned_full_frame(frame: pd.DataFrame, train_valid: np.ndarray, columns: Sequence[str]) -> pd.DataFrame:
    output = frame.copy()
...
def build_context(target: Mapping[str, Any]) -> dict[str, Any]:
    frame, raw, features = f77b.load_inputs()
    indices = f77b.entry_indices(frame, raw)
    spec = next(item for item in f77b.lifecycle_specs() if item.name == str(target["label_name"]))
    outcome = f77b.compute_path_outcome(raw, indices, spec)
    label = f77b.make_label(frame, outcome, spec)
    train_valid = (frame["split"] == "train").to_numpy() & outcome["valid"]
    feature_columns = f77b.feature_sets(features)[str(target["feature_set"])]
    ...
    model = f77b.model_builders()[str(target["model"])]()
    model.fit(train_matrix, y_train)
    train_probs = f77b.probability(model, train_matrix)
    ...
    thresholds = f77b.risk_thresholds(frame)
    ...
    for split, prefix in (("validation", "val"), ("oos", "oos")):
        ...
        event_filter = valid & f77b.session_mask(split_df, str(target["session"])) & f77b.risk_mask(split_df, str(target["risk_filter"]), spec.side, thresholds)
        raw_signal = (score[split_mask] >= threshold) & event_filter
        selected = f77b.lifecycle_select(raw_signal, np.asarray(split_outcome["exit_offset"], dtype=int))
        metrics = f77b.lifecycle_kpi(split_df, selected, split_outcome)
        ...
        reproduction_rows.append(
            {
                ...
                "source_trades_day": target.get(f"{prefix}_trades_day"),
                "source_trade_count": target.get(f"{prefix}_trade_count"),
                "source_raw_signal_count": target.get(f"{split}_raw_signal_count"),
                "source_lifecycle_trade_count": target.get(f"{split}_lifecycle_trade_count"),
                ...
                "raw_signal_count_diff": int(raw_signal.sum()) - as_int(target.get(f"{split}_raw_signal_count")),
                ...
                "passed": bool(
                    ...
                    and int(raw_signal.sum()) == as_int(target.get(f"{split}_raw_signal_count"))
...
def parity_rows(context: Mapping[str, Any], raw_onnx_path: Path, patched_onnx_path: Path) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    ...
    for split in ("train", "validation", "oos"):
        ...
        onnx_raw_signal = context["event_filter"][idx] & (all_patched[:, 0] >= runtime_threshold)
...
def materialize(context: Mapping[str, Any], common_files_root: Path) -> tuple[dict[str, Any], list[dict[str, Any]], list[dict[str, Any]], list[dict[str, Any]]]:
    candidate_id = f"{RUNTIME_CANDIDATE_PREFIX}_{context['target']['candidate_id']}"
    ...
    artifact = {
        "candidate_id": candidate_id,
        "source_candidate_id": context["target"].get("candidate_id"),
        "blocked_best_candidate_id": "f77b_08051",
        "materialization_mode": "short_binary_onnx_three_column_lifecycle_negative_control",
        "surrogate_boundary": "exportable_surrogate_probe_not_best_hist_gbm_proxy",
        ...
        "threshold_epsilon": THRESHOLD_EPSILON,
        "short_threshold": context["runtime_threshold"],
        "long_threshold": 1.1,
        "min_margin": -1.0,
        "decision_mode": "threshold_margin",
        "trade_shape": TRADE_SHAPE_LABEL,
        "sltp_point_scale": SLTP_POINT_SCALE,
...
def build_attempts(context: Mapping[str, Any], artifact: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    for split in ("validation", "oos"):
        ...
        extra = {
            ...
            "InpAtrMinStopPoints": float(spec.sl_points) * float(SLTP_POINT_SCALE),
            "InpAtrMaxStopPoints": float(spec.sl_points) * float(SLTP_POINT_SCALE),
            "InpAtrMinTakeProfitPoints": float(spec.tp_points) * float(SLTP_POINT_SCALE),
            "InpAtrMaxTakeProfitPoints": float(spec.tp_points) * float(SLTP_POINT_SCALE),
        ...
        attempt.update(
            {
                ...
                "blocked_best_candidate_id": artifact["blocked_best_candidate_id"],
                "axis_id": "short_h12_tp18_sl12_price_action_core_extra_trees_cash_mid_trend_aligned_q80",
                ...
                "surrogate_boundary": artifact["surrogate_boundary"],
            }
        )
...
def build_summary(payload: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "candidate_id": artifact.get("candidate_id", f"{RUNTIME_CANDIDATE_PREFIX}_f77b_07979"),
        "source_candidate_id": "f77b_07979",
        "blocked_best_candidate_id": "f77b_08051",
...
def gate_audit_text(payload: Mapping[str, Any], created_at: str) -> str:
    summary = build_summary(payload)
    return f"""# Required Gate Coverage Audit F77D(F77D 필수 게이트 커버리지 감사)

Updated(갱신): {created_at}

| gate(게이트) | status(상태) | evidence/effect(근거/효과) |
|---|---|---|
| F77C Grok accepted with conditions(F77C Grok 조건부 수용) | `passed(통과)` | `stages/{STAGE_ID}/03_reviews/grok_pre_mt5_lifecycle_negative_control_runtime_probe_receipt.md` |
...
def update_ledgers(payload: Mapping[str, Any], created_at: str) -> None:
    ...
    row = {
        ...
        "guardrail_kpi": f"signal_diff={best.get('signal_count_diff', '')};feature_diff={best.get('feature_ready_diff', '')};surrogate_boundary=f77b_07979_not_f77b_08051",
        ...
        "notes": f"attempts={summary['attempt_count']};completed={summary['completed_attempt_count']};candidate=f77d_runtime_f77b_07979",
        "lane": "mt5_runtime_probe(MT5 런타임 탐침)",
        "family": "runtime_backtest(런타임 백테스트)",
        "primary_report": rel(REPORT_PATH),
        "run_number": "frontier77D",
        "date": created_at[:10],
        ...
        "question": "Does lifecycle-native proxy mechanics survive MT5 runtime?(생명주기 기본 프록시 메커니즘이 MT5 런타임에서 유지되는가?)",
        "artifact_count": "12",
        ...
        "run_type": "mt5_lifecycle_negative_control_runtime_probe",
    ...
    upsert_csv(REVIEW_DIR / "stage_run_ledger.csv", "ledger_row_id", row, source_header=ROOT / "docs/registers/alpha_run_ledger.csv")


def update_registers_and_state(payload: Mapping[str, Any], created_at: str) -> None:
    summary = build_summary(payload)
    best = summary["best_runtime"]
    marker = "<!-- frontier77D_mt5_lifecycle_negative_control_runtime_probe_v1 -->"
...
next_run_id: {NEXT_RUN_ID}
runtime_probe_status: f77_mandatory_runtime_probe_attempted
runtime_authority: not_claimed
operating_promotion: not_claimed
...
notes:
  - "Action(행동): F77D MT5 Runtime Probe(MT5 런타임 탐침)를 실행/시도했다."
  - "Effect(효과): proxy/runtime gap analysis(프록시/런타임 간극 분석)를 다음 실행으로 넘길 근거를 만들었다."
...
Effect(효과): F77B lifecycle proxy(생명주기 프록시)를 Strategy Tester(전략 테스터) 근거로 물질화하고, 다음 F77E gap analysis(간극 분석)의 입력을 만들었다.
...
Effect(효과): 다음 실행은 F77E proxy/runtime gap analysis(프록시/런타임 간극 분석)와 repair decision(수리 결정)이다.
...
def main() -> int:
    ...
    if artifact.get("export_status") != "negative_control_parity_passed":
        ...
        status = "completed_mt5_lifecycle_negative_control_runtime_probe_observation_no_authority"
        judgment = "runtime_probe_completed_gap_analysis_required_no_authority"
    elif args.execute:
        status = "blocked_mt5_lifecycle_runtime_probe_attempted_no_authority"
    ...
    write_csv(RUN_DIR / "f77d_probability_parity.csv", probability)
    write_csv(RUN_DIR / "f77d_signal_parity.csv", signal)
    write_csv(RUN_DIR / "f77d_feature_readiness_parity.csv", feature_parity)
    write_csv(RUN_DIR / "f77d_source_reproduction.csv", context["reproduction_rows"])
    write_csv(RUN_DIR / "f77d_runtime_receipt.csv", runtime_receipt, f71d.RUNTIME_RECEIPT_COLUMNS)
    write_json(RUN_DIR / "f77d_execution_results.json", execution_results)
