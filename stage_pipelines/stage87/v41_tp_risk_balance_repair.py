...
STAGE_ID = "87_adapter_research__v41_tp_risk_balance_repair"
RUN_NUMBER = "run87A"
RUN_ID = "run87A_stage87_v41_tp_risk_balance_repair_v1"
PACKET_ID = "stage87_v41_tp_risk_balance_repair_v1"
PARENT_RUN_ID = "run86A_stage86_v41_validation_dd_followup_review_v1"
SOURCE_STAGE86_ID = "86_adapter_research__v41_validation_dd_followup_review"
SOURCE_STAGE86_CLOSEOUT_COMMIT = "b487be5cc16858e44d57ce50e4b213d602a29fc1"
SOURCE_STAGE86_LATEST_COMMIT = "9db00a72501752ada76997e261145b21e8bd40cc"
SOURCE_STAGE85_PUSHED_COMMIT = "886e07afe1421a38b53c4c8ca5c629d574b3bbac"
SOURCE_STAGE85_LATEST_COMMIT = "55efc21f7f9f100a78f078049fcf10f7949f1ea3"
SOURCE_STAGE73_LATEST_COMMIT = "76db6f199ff917da2f8311544f68dc6f24612e0e"
NEXT_STAGE_ID = "88_adapter_research__v41_tp_risk_balance_followup_review"
NEXT_RUN_ID = "run88A_stage88_v41_tp_risk_balance_followup_review_v1"
NEXT_PACKET_ID = "stage88_v41_tp_risk_balance_followup_review_v1"
...
COMMON_ROOT = f"OPV2/s87a/{RUN_NUMBER}"

SUMMARY_JSON_PATH = REVIEWS_ROOT / "stage87_v41_tp_risk_balance_summary.json"
SUMMARY_CSV_PATH = REVIEWS_ROOT / "stage87_v41_tp_risk_balance_summary.csv"
REPORT_PATH = REVIEWS_ROOT / "stage87_v41_tp_risk_balance_report.md"
SEGMENT_KPI_PATH = REVIEWS_ROOT / "stage87_segment_kpi_summary.csv"
RISK_ATR_TELEMETRY_PATH = REVIEWS_ROOT / "stage87_risk_atr_telemetry.csv"
GATE_FEATURE_SUMMARY_PATH = REVIEWS_ROOT / "stage87_gate_feature_summary.csv"
TIER_B_DIAGNOSTIC_PATH = REVIEWS_ROOT / "stage87_tier_b_diagnostic_summary.csv"
DECISION_PATH = REVIEWS_ROOT / "stage87_decision.md"
AUDIT_CSV_PATH = REVIEWS_ROOT / "stage87_trade_audit.csv"
...
VARIANTS = (
    repair.RepairVariant(
        adapter_id="s87_v41_h3_risk475_gate08_sl225_tp38_cd10",
        label="run50BN_v41_h3_risk475_gate08_sl225_tp38_cd10",
        atr_enabled=True,
        ...
        max_hold_bars=3,
        notes="Stage87 TP/risk balance repair: combine 4.75% risk cap with TP3.8 and CD10.",
    ),
    repair.RepairVariant(
        adapter_id="s87_v41_h3_risk45_gate08_sl225_tp38_cd10",
        label="run50BN_v41_h3_risk45_gate08_sl225_tp38_cd10",
        atr_enabled=True,
        model_risk_enabled=True,
        fixed_lot=0.25,
        atr_stop_multiplier=2.25,
        ...
        close_only_on_opposite_signal=True,
        max_hold_bars=3,
        notes="Stage87 TP/risk balance repair: combine 4.5% risk cap with TP3.8 and CD10.",
    ),
    repair.RepairVariant(
        adapter_id="s87_v41_h3_risk475_gate08_sl215_tp38_cd10",
        label="run50BN_v41_h3_risk475_gate08_sl215_tp38_cd10",
        atr_enabled=True,
        model_risk_enabled=True,
        fixed_lot=0.25,
        ...
        close_only_on_opposite_signal=True,
        max_hold_bars=3,
        notes="Stage87 TP/risk balance repair: combine 4.75% risk cap, tighter SL2.15, TP3.8, and CD10.",
    ),
)
...
CONTEXT_GATE_SPECS = {
    variant.adapter_id: {
        "gate_column": "stage87_gate_margin_lt_008_short",
        "gate_type": "margin",
        "threshold": 0.08,
        "block_mode": "short",
...
def decide(summary_rows: Sequence[Mapping[str, Any]], segment_rows: Sequence[Mapping[str, Any]], external: str) -> str:
    if external != "completed":
        return "continue_stage87_due_to_incomplete_runtime"
    ...
    if val_net >= 900 and oos_net >= 520 and val_dd <= 22.5 and oos_dd <= 20.8 and val_pf >= 1.45 and oos_pf >= 1.45 and not reasons:
        return "proceed_to_stage88_followup_review"
    if val_net >= 800 and oos_net >= 430 and val_dd <= 26.0 and oos_dd <= 22.8 and val_pf >= 1.35 and oos_pf >= 1.35:
        return "continue_tp_risk_balance_followup_review_in_stage88"
    if val_pf < 1.0 or val_net <= 0:
        return "open_new_model_source_branch_in_stage88"
    return "continue_tp_risk_balance_followup_review_in_stage88"
...
def report_markdown(
    ...
    return f"""# Stage87 V41 TP/Risk Balance Repair Report(87단계 V41 익절/위험 균형 수리 보고서)

- run(실행): `{RUN_ID}`
- target_surface(목표 표면): `{TARGET_SURFACE}`
- source_stage(원천 단계): `{SOURCE_STAGE86_ID}`
- source_stage86_closeout_commit(원천 86단계 종료 커밋): `{SOURCE_STAGE86_CLOSEOUT_COMMIT}`
- source_stage86_latest_commit(원천 86단계 최신 커밋): `{SOURCE_STAGE86_LATEST_COMMIT}`
- source_stage85_pushed_commit(원천 85단계 푸시 커밋): `{SOURCE_STAGE85_PUSHED_COMMIT}`
- source_stage85_latest_commit(원천 85단계 최신 커밋): `{SOURCE_STAGE85_LATEST_COMMIT}`
...
def decision_markdown(decision: str, external: str) -> str:
    return f"""# Stage87 Decision(87단계 판정)

decision(판정): `{decision}`

Stage87(87단계)는 Stage86(86단계) 판정에 따라 Stage85(85단계)에서 분리된 risk cap(위험 상한)과 TP trim(익절 축소)을 결합해 validation DD(검증 손실률)와 OOS(표본외) 강점의 균형을 좁게 측정했다.
            ...
            rows.append(
                {
                    ...
                    "split": split_name,
                    "tier_b_policy": "diagnostic_missing_required_but_disabled_for_this_v41_tp_risk_balance_repair",
                    ...
                    "reason": "Stage87 isolates v41 TP/risk balance first; Tier B fallback remains diagnostic and disabled.",
                }
            )
    return rows
            ...
            rows.append(
                {
                    ...
                    "artifact_type": "stage87_v41_tp_risk_balance_repair_evidence",
                    "path": rel(path),
                    "sha256": sha256_file_lf_normalized(path),
                    "stage_id": STAGE_ID,
                    "run_id": RUN_ID,
                    "created_at_utc": created,
                    "notes": "Stage87 v2-native v41 TP/risk balance repair artifact.",
                }
            )
            ...
            rows.append(
                {
                    ...
                    "run_id": RUN_ID,
                    "created_at_utc": created,
                    "notes": "Actual Stage87 MT5 Strategy Tester HTML report.",
                }
            )
    return rows
...
def write_run_identity(result: Mapping[str, Any]) -> None:
    write_json(
        RUN_ROOT / "run_manifest.json",
        {
            ...
            "source_stage86_closeout_commit": SOURCE_STAGE86_CLOSEOUT_COMMIT,
            "source_stage86_latest_commit": SOURCE_STAGE86_LATEST_COMMIT,
            "source_stage85_pushed_commit": SOURCE_STAGE85_PUSHED_COMMIT,
            "source_stage85_latest_commit": SOURCE_STAGE85_LATEST_COMMIT,
        ...
        [
            {
                "run_id": RUN_ID,
                "stage_id": STAGE_ID,
                "lane": "baseline_adapter_v2_native_v41_tp_risk_balance_repair",
                "status": status,
                "judgment": decision,
        ...
        alpha_rows = [
            {
                ...
                "tier_scope": "Tier A+B",
                "kpi_scope": "stage87_v41_tp_risk_balance_repair",
                ...
                "external_verification_status": external,
                "notes": "Stage87 run materialized or blocked before KPI records were available.",
            }
        ]
...
def write_packet_files(result: Mapping[str, Any], decision: str, ledger_payload: Mapping[str, Any]) -> None:
    ...
    write_json(PACKET_ROOT / "aggregate_summary.json", {"packet_id": PACKET_ID, "stage_id": STAGE_ID, "run_id": RUN_ID, "decision": decision, "source_stage86_closeout_commit": SOURCE_STAGE86_CLOSEOUT_COMMIT, "source_stage86_latest_commit": SOURCE_STAGE86_LATEST_COMMIT, "source_stage85_pushed_commit": SOURCE_STAGE85_PUSHED_COMMIT, "source_stage85_latest_commit": SOURCE_STAGE85_LATEST_COMMIT, "source_stage73_latest_commit": SOURCE_STAGE73_LATEST_COMMIT, "gate_feature_summary_path": rel(GATE_FEATURE_SUMMARY_PATH), "ledger_payload": ledger_payload, "overall_goal_complete": False})
...
Stage88(88단계)는 Stage87(87단계)의 TP/risk balance repair(익절/위험 균형 수리) 결과를 review gate(검토 게이트)로만 판독하는 follow-up(후속) 단계다.

## Bounded Question(경계 질문)

Stage87(87단계)의 risk cap + TP trim(위험 상한 + 익절 축소) 조합이 Stage83 CD10(83단계 CD10)의 OOS(표본외) 강점을 보존하면서 validation DD(검증 손실률)를 충분히 낮췄는가?

Effect(효과): Stage88(88단계)는 Stage87(87단계)의 결과를 과장하지 않고 다음 판단으로만 좁힌다.
...
""",
    ...
    write_md(
        ...
        f"""# Stage88 Input References(88단계 입력 참조)

- source_stage(원천 단계): `{STAGE_ID}`
- source_run(원천 실행): `{RUN_ID}`
- source_decision(원천 판정): `{decision}`
- source_external_verification_status(원천 외부 검증 상태): `{external}`
- stage87_report(87단계 보고서): `{rel(REPORT_PATH)}`
- stage87_decision(87단계 판정): `{rel(DECISION_PATH)}`
- summary(요약): `{rel(SUMMARY_CSV_PATH)}`
- target_surface(목표 표면): `{TARGET_SURFACE}`

Effect(효과): Stage88(88단계)는 v2 고유 근거만 이어받아 34D KPI(34D 핵심 성과 지표) 격차 축소를 계속한다.
""",
    )
    write_md(NEXT_STAGE_ROOT / "03_reviews" / "review_index.md", f"""# Stage88 Review Index(88단계 검토 색인)
...
Effect(효과): Stage88(88단계)는 Stage87(87단계) closeout(종료 기록)을 이어받아 다음 bounded batch(경계 묶음 실행)만 검토한다.
""")
    write_md(NEXT_STAGE_ROOT / "04_selected" / "selection_status.md", f"""# Stage88 Selection Status(88단계 선택 상태)
...
Effect(효과): Stage88(88단계)는 34D KPI(34D 핵심 성과 지표) 격차 축소를 계속하지만, 운영 의미 없이 연구개발로만 이어진다.
""")
...
- >-
  Stage87(87단계) closed(종료) as `{decision}` and Stage88(88단계) `{NEXT_STAGE_ID}` is open_planned(열림 계획). Effect(효과): v41 TP/risk balance repair(V41 익절/위험 균형 수리) 근거를 보존하고 다음 경계 연구 질문으로만 넘긴다.
- >-
  Stage87 result(87단계 결과): validation/OOS(검증/표본외), PF/net/DD(수익 팩터/순손익/손실률), risk/ATR telemetry(위험/ATR 텔레메트리)는 `{rel(SUMMARY_CSV_PATH)}`와 `{rel(RISK_ATR_TELEMETRY_PATH)}`에 기록된다. Effect(효과): 34D target surface(34D 목표 표면) 대비 KPI(핵심 성과 지표) 차이를 다음 단계 입력으로 보존한다.
- >-
...
stage87_v41_tp_risk_balance_repair:
  packet_id: {PACKET_ID}
  stage_id: {STAGE_ID}
  ...
  boundary: {BOUNDARY}
"""
    if "stage87_v41_tp_risk_balance_repair:" in text:
        text = re.sub(r"\nstage87_v41_tp_risk_balance_repair:\n(?:  .*\n)+", block + "\n", text, count=1)
    else:
        text = text.rstrip() + block
    io_path(WORKSPACE_STATE_PATH).write_text(text + ("\n" if not text.endswith("\n") else ""), encoding="utf-8-sig")
    write_md(SELECTED_ROOT / "selection_status.md", f"""# Stage87 Selection Status(87단계 선택 상태)
...
- target_surface(목표 표면): `{TARGET_SURFACE}`
- stage87_decision(87단계 판정): `{decision}`
- next_stage_or_branch(다음 단계/분기): `{NEXT_STAGE_ID}`
- claim_boundary(주장 경계): `{BOUNDARY}`

Effect(효과): Stage87(87단계)는 v41 TP/risk balance repair(V41 익절/위험 균형 수리)를 측정하고, 운영 의미 없이 Stage88(88단계)로 넘긴다.
""")
...
- target_surface(목표 표면): `{TARGET_SURFACE}`
- adapter_under_review(검토 중 어댑터): `{SOURCE_ADAPTER_ID}`
- status(상태): `stage87_closed_{decision}`
- claim_boundary(주장 경계): `{BOUNDARY}`
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    prepared = {
        ...
        "model_family": "baseline_adapter_stage87_v2_native_v41_tp_risk_balance_repair_ebm_table",
        "feature_set_id": "stage87_v41_tp_risk_balance_signal",
