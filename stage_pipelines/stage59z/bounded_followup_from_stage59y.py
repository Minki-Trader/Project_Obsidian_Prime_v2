...
SOURCE_STAGE59Y_ID = "59Y_adapter_repair__new_model_branch_from_stage59x"
STAGE59Z_ID = "59Z_adapter_repair__bounded_followup_from_stage59y"
NEXT_REPAIR_STAGE_ID = "59AA_adapter_repair__bounded_followup_from_stage59z"
RUN_NUMBER = "run59U"
RUN_ID = "run59U_stage59z_bounded_followup_from_stage59y_v1"
PACKET_ID = "stage59z_bounded_followup_from_stage59y_v1"
PARENT_RUN_ID = "run59T_stage59y_new_model_branch_from_stage59x_v1"
SOURCE_ADAPTER_ID = "s59y_v64_gap14_h2_sd5"
DEVELOPMENT_ANCHOR = "v64_v47_ctxgap14_refill_etfw_h2_no_b"
BACKUP_ANCHOR = "v60_v47_et_stable_damage_firewall_h2c0_no_b"
SOURCE_STAGE59Y_PUSHED_COMMIT = "ac951f48758a3774b23874f643c6468574e0665e"
BOUNDARY = (
    "research_development_only_no_live_readiness_no_runtime_authority_"
    "no_operating_promotion_no_operating_reference_no_production_baseline_no_deployment"
)

STAGE_ROOT = Path("stages") / STAGE59Z_ID
...
SOURCE_STAGE59Y_ROOT = Path("stages") / SOURCE_STAGE59Y_ID
SOURCE_STAGE59Y_DECISION = SOURCE_STAGE59Y_ROOT / "03_reviews/stage59y_decision.md"
SOURCE_STAGE59Y_REPORT = SOURCE_STAGE59Y_ROOT / "03_reviews/new_model_branch_from_stage59x_report.md"
SOURCE_STAGE59Y_SUMMARY = SOURCE_STAGE59Y_ROOT / "03_reviews/new_model_branch_summary.csv"
SOURCE_STAGE59Y_JSON = SOURCE_STAGE59Y_ROOT / "03_reviews/new_model_branch_summary.json"
...
COMMON_ROOT = f"Project_Obsidian_Prime_v2/s59z/{RUN_NUMBER}"

REPORT_PATH = REVIEWS_ROOT / "bounded_followup_from_stage59y_report.md"
...
DECISION_PATH = REVIEWS_ROOT / "stage59z_decision.md"
AUDIT_CSV_PATH = REVIEWS_ROOT / "stage59z_trade_audit.csv"
...
STAGE59Z_VARIANTS = (
    repair.RepairVariant(
        adapter_id="s59z_v64_gap14_t59_h2_sd5",
        label="run50BR_v64_context_gap14_h2_threshold59_risk3pct_wide_atr_sd5",
        atr_enabled=True,
        ...
        max_hold_bars=2,
        notes="Stage59Z bounded follow-up: run50BR v64 context-gap14 refill source with threshold 0.59 after Stage59Y validation cost-stress weakness.",
    ),
    repair.RepairVariant(
        adapter_id="s59z_v64_gap14_t61_h2_sd5",
        label="run50BR_v64_context_gap14_h2_threshold61_risk3pct_wide_atr_sd5",
        atr_enabled=True,
        model_risk_enabled=True,
        fixed_lot=0.25,
        atr_stop_multiplier=2.5,
        ...
        close_only_on_opposite_signal=True,
        max_hold_bars=2,
        notes="Stage59Z bounded follow-up: run50BR v64 context-gap14 refill source with threshold 0.61 after Stage59Y validation cost-stress weakness.",
    ),
    repair.RepairVariant(
        adapter_id="s59z_v64_gap14_t63_h2_sd5",
        label="run50BR_v64_context_gap14_h2_threshold63_risk3pct_wide_atr_sd5",
        atr_enabled=True,
        model_risk_enabled=True,
        fixed_lot=0.25,
        atr_stop_multiplier=2.5,
        ...
        close_only_on_opposite_signal=True,
        max_hold_bars=2,
        notes="Stage59Z bounded follow-up: run50BR v64 context-gap14 refill source with threshold 0.63 after Stage59Y validation cost-stress weakness.",
    ),
)
...
def configure_reused_engine() -> None:
    ...
    engine.STAGE59_VARIANTS = STAGE59Z_VARIANTS
    engine.SOURCE_SPECS = source_specs()
    engine.MODEL_RISK_MIN_PCT = {variant.adapter_id: 0.005 for variant in STAGE59Z_VARIANTS}

    repair.STAGE_ID = STAGE59Z_ID
    repair.RUN_NUMBER = RUN_NUMBER
    repair.RUN_ID = RUN_ID
    repair.RUN_ROOT = RUN_ROOT
    repair.REPAIR_VARIANTS = STAGE59Z_VARIANTS
    s58.STAGE58_ID = STAGE59Z_ID
    ...
    s58.STAGE58_VARIANTS = STAGE59Z_VARIANTS
...
def build_attempts(inputs: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    for variant_index, variant in enumerate(STAGE59Z_VARIANTS, start=1):
            ...
            for role_index, (tier, attempt_role, prefix, attempt_token) in enumerate(
                ...
                attempts.append(
                    attempt_payload(
                        ...
                        run_id=RUN_ID,
                        stage_number=59,
                        exploration_label="stage59Z_BaselineAdapter__V64ThresholdQualityCompression",
...
def next_stage_for_decision(decision: str) -> str:
    ...
    if decision == "demote_current_adapter_and_select_backup":
        return "59AA_adapter_repair__backup_or_demote_from_stage59z"
    if decision == "open_new_model_branch":
        return "59AA_adapter_repair__new_model_branch_from_stage59z"
...
def next_packet_for_decision(decision: str) -> tuple[str, str]:
    ...
    return "stage59aa_bounded_followup_from_stage59z_v1", "run59V_stage59aa_bounded_followup_from_stage59z_v1"
            ...
            rows.append(
                {
                    ...
                    "stage_id": STAGE59Z_ID,
                    "run_id": RUN_ID,
                    "artifact_type": "stage59z_bounded_followup_evidence",
                    "path": rel(path),
                    "sha256": sha256_file_lf_normalized(path),
                    "created_at_utc": created,
                    "notes": "Stage59Z bounded threshold-quality follow-up from Stage59Y artifact.",
                }
            )
            ...
            rows.append(
                {
                    ...
                    "created_at_utc": created,
                    "notes": "Actual Stage59Z MT5 Strategy Tester HTML report.",
                }
            )
    return rows
...
def write_run_identity(result: Mapping[str, Any]) -> None:
    engine.write_json(
        RUN_ROOT / "run_manifest.json",
        {
            ...
            "bounded_question": "Can bounded threshold-quality compression on the Stage59Y v64 gap14 adapter repair validation PF and cost-stressed expectancy without damaging OOS behavior?",
            "source_stage59y_decision": rel(SOURCE_STAGE59Y_DECISION),
            "source_stage59y_pushed_commit": SOURCE_STAGE59Y_PUSHED_COMMIT,
            "variants": [
                {
                    **variant.__dict__,
                ...
                for variant in STAGE59Z_VARIANTS
            ],
...
def write_ledgers(
        ...
        [
            {
                "run_id": RUN_ID,
                "stage_id": STAGE59Z_ID,
                "lane": "baseline_adapter_bounded_followup_from_stage59y",
                "status": status,
                "judgment": decision,
    ...
    ledger_rows.append(
        {
            "ledger_row_id": f"{RUN_ID}__aggregate_bounded_followup_from_stage59y",
            "stage_id": STAGE59Z_ID,
            "run_id": RUN_ID,
            "subrun_id": "aggregate_bounded_followup_from_stage59y",
            "parent_run_id": PARENT_RUN_ID,
            "record_view": "bounded_followup_from_stage59y",
            "tier_scope": "Tier A+B",
            ...
            "external_verification_status": external,
            "notes": "Stage59Z bounded threshold-quality follow-up from Stage59Y evidence; not final package completion.",
        }
    )
    ...
    engine.write_json(
        SUMMARY_JSON_PATH,
        {
            ...
            "required_outputs": {
                ...
                "stage59z_decision": rel(DECISION_PATH),
            },
...
def report_markdown(summary_rows: Sequence[Mapping[str, Any]], decision: str, reasons: Sequence[str], external: str) -> str:
    ...
    best = best_repaired_variant(summary_rows)
    return f"""# Stage59Z Bounded Follow-Up From Stage59Y Report(59Z단계 Stage59Y 기반 경계 후속 보고서)

- stage(단계): `{STAGE59Z_ID}`
- run(실행): `{RUN_ID}`
...
def equity_audit_markdown(summary_rows: Sequence[Mapping[str, Any]], segment_rows: Sequence[Mapping[str, Any]]) -> str:
    ...
    return f"""# Stage59Z Bounded Follow-Up Equity Curve Audit(59Z단계 경계 후속 자금 곡선 감사)
...
def decision_markdown(decision: str, reasons: Sequence[str], best: Mapping[str, Any], external: str) -> str:
    return f"""# Stage59Z Decision(59Z단계 판정)
...
def write_packet_files(
    ...
    files = {
        ...
        "experiment_design_receipt.json": {
            "hypothesis": "A bounded threshold-quality compression on the Stage59Y v64 gap14 adapter may repair validation PF and cost-stressed expectancy while preserving OOS net, PF, density, ATR/bracket telemetry, and model-risk telemetry.",
            ...
            "comparison_baseline": "Stage59Y best adapter s59y_v64_gap14_h2_sd5 with validation PF 1.07 and validation cost-stressed expectancy below zero.",
            ...
            "stop_conditions": "three bounded Stage59Z threshold variants only",
        ...
        "artifact_lineage_audit.json": {
            "source_inputs": [
                rel(SOURCE_STAGE59Y_DECISION),
                rel(SOURCE_STAGE59Y_REPORT),
                rel(SOURCE_STAGE59Y_SUMMARY),
                rel(SOURCE_STAGE59Y_JSON),
                rel(RUN50BR_ROOT / "models/stage56_context_timed_event_signal_discrete_score_table.csv"),
                rel(RUN50BR_ROOT / RUN50BR_V64_ANCHOR),
            ],
            "producer": rel(Path("stage_pipelines/stage59z/bounded_followup_from_stage59y.py")),
...
- stage_id(단계 ID): `{STAGE59Z_ID}`
- source_stage(원천 단계): `{SOURCE_STAGE59Y_ID}`
- source_decision(원천 판정): `continue_repair_in_new_bounded_stage`
- bounded_question(경계 질문): `Can threshold-quality compression on the Stage59Y v64 gap14 adapter repair validation PF and cost-stressed expectancy without damaging OOS behavior?`
- boundary(경계): `{BOUNDARY}`

Stage59Z(59Z단계)는 Stage59Y(59Y단계)의 best adapter(최선 어댑터) `s59y_v64_gap14_h2_sd5`에서 threshold(문턱값) 세 변형만 측정한다. Effect(효과): Stage60 ONNX(60단계 ONNX)로 조기 이동하지 않고 검증 품질 약점을 먼저 좁게 확인한다.
""",
    )
    engine.write_md(
...
- stage59y_decision(59Y단계 판정): `{rel(SOURCE_STAGE59Y_DECISION)}`
- stage59y_report(59Y단계 보고서): `{rel(SOURCE_STAGE59Y_REPORT)}`
- stage59y_summary(59Y단계 요약): `{rel(SOURCE_STAGE59Y_SUMMARY)}`
- stage59y_pushed_commit(59Y단계 푸시 커밋): `{SOURCE_STAGE59Y_PUSHED_COMMIT}`
...
Effect(효과): Stage59Z(59Z단계)는 Stage59Y(59Y단계) 약점과 run50BR v64(run50BR 변형64) 원천을 명시적으로 연결한다.
""",
    )
    engine.write_md(
...
- stage_status(단계 상태): `closed_bounded_followup_from_stage59y`
- source_stage(원천 단계): `{SOURCE_STAGE59Y_ID}`
- source_decision(원천 판정): `continue_repair_in_new_bounded_stage`
- stage59z_decision(59Z단계 판정): `{decision}`
- next_stage_or_branch(다음 단계/분기): `{next_stage}`
- selected_research_baseline(선택 연구 기준선): `none`
- claim_boundary(주장 경계): `{BOUNDARY}`

Effect(효과): Stage59Z(59Z단계)는 측정 결과를 보존하지만 final package(최종 패키지)나 operating claim(운영 주장)을 만들지 않는다.
""",
    )
    engine.write_md(
        REVIEWS_ROOT / "review_index.md",
        f"""# Stage59Z Review Index(59Z단계 검토 색인)
...
- bounded_followup_risk_atr_telemetry(경계 후속 위험/ATR 기록): `{rel(RISK_ATR_TELEMETRY_PATH)}`
- stage59z_decision(59Z단계 판정): `{rel(DECISION_PATH)}`
""",
    )
...
- source_stage(원천 단계): `{STAGE59Z_ID}`
- source_decision(원천 판정): `{decision}`
- bounded_question(경계 질문): `What bounded follow-up should repair, demote, or branch from the Stage59Z threshold-quality evidence without starting ONNX prematurely?`
- boundary(경계): `{BOUNDARY}`

Stage59AA(59AA단계)는 Stage59Z(59Z단계) 결과가 Stage60 ONNX(60단계 ONNX)에 충분하지 않을 때 이어지는 bounded follow-up(경계 후속)이다. Effect(효과): repair(수리), demotion(강등), branch(분기)를 다음 작은 질문으로 제한한다.
""",
        )
        engine.write_md(
...
- stage59z_decision(59Z단계 판정): `{rel(DECISION_PATH)}`
- stage59z_report(59Z단계 보고서): `{rel(REPORT_PATH)}`
- stage59z_summary(59Z단계 요약): `{rel(SUMMARY_CSV_PATH)}`
- stage59z_segment_kpi(59Z단계 구간 KPI): `{rel(SEGMENT_KPI_PATH)}`
- stage59z_risk_atr_telemetry(59Z단계 위험/ATR 기록): `{rel(RISK_ATR_TELEMETRY_PATH)}`
""",
        )
        engine.write_md(
            next_root / "03_reviews/review_index.md",
            """# Stage59AA Review Index(59AA단계 검토 색인)

Stage59AA(59AA단계)는 planned(계획) 상태다. Effect(효과): Stage59Z(59Z단계) 약점을 다음 bounded repair(경계 수리) 근거로 넘긴다.
""",
        )
        engine.write_md(
...
- stage_status(단계 상태): `active_planned_from_stage59z`
- source_stage(원천 단계): `{STAGE59Z_ID}`
- source_decision(원천 판정): `{decision}`
- selected_research_baseline(선택 연구 기준선): `none`
- claim_boundary(주장 경계): `{BOUNDARY}`

Effect(효과): Stage59AA(59AA단계)는 Stage59Z(59Z단계) 이후 필요한 다음 작은 질문만 다룬다.
""",
        )
...
- status(상태): `stage59z_closed_{decision}`
- claim_boundary(주장 경계): research/development only(연구/개발 전용)

Stage59Z(59Z단계) closed(종료) as bounded threshold-quality follow-up from Stage59Y(Stage59Y 기반 경계 문턱값 품질 후속). Effect(효과): Stage59Y(59Y단계)의 validation PF/cost weakness(검증 수익 팩터/비용 약점)를 보존하고, Stage60 ONNX(60단계 ONNX)는 품질 근거가 강할 때만 열린다.

## Latest Stage59Z Evidence(최신 59Z단계 근거)
...
""",
    ...
    focus = (
        "current_focus:\n"
        f"- >-\n"
        f"  Stage59Z(59Z단계) `{STAGE59Z_ID}` closed(종료) as bounded threshold-quality follow-up from Stage59Y(Stage59Y 기반 경계 문턱값 품질 후속); decision(판정)=`{decision}`. "
        ...
        f"- >-\n"
        f"  Next stage_or_branch(다음 단계/분기) `{next_stage}` is active/planned(활성/계획). Effect(효과): Stage59Z(59Z단계) 결과를 다음 bounded step(경계 다음 단계)으로 넘긴다.\n"
    )
    text = re.sub(r"current_focus:\n(?:- >-\n(?:  .*\n)+)+", focus, text, count=1)
    block = f"""

stage59z_bounded_followup_from_stage59y:
  packet_id: {PACKET_ID}
  stage_id: {STAGE59Z_ID}
  status: closed_bounded_followup_from_stage59y
  ...
  boundary: {BOUNDARY}
"""
    if "stage59z_bounded_followup_from_stage59y:" in text:
        text = re.sub(r"\nstage59z_bounded_followup_from_stage59y:\n(?:  .*\n)*", block, text, count=1)
    else:
        text += block
...
def parse_args(argv: Sequence[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run Stage59Z bounded follow-up from Stage59Y.")
...
def load_existing_result() -> dict[str, Any]:
    ...
    if not path_exists(manifest) or not path_exists(kpi):
        raise FileNotFoundError("Stage59Z existing run_manifest.json or kpi_record.json is missing")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.finalize_existing:
        ...
        prepared = {
            ...
            "model_family": "baseline_adapter_stage59z_threshold_quality_ebm_table",
            "feature_set_id": "stage59z_v64_threshold_quality_signal",
