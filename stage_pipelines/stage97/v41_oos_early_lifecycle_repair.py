...
STAGE_ID = "97_adapter_research__v41_oos_early_lifecycle_repair"
RUN_NUMBER = "run97A"
RUN_ID = "run97A_stage97_v41_oos_early_lifecycle_repair_v1"
PACKET_ID = "stage97_v41_oos_early_lifecycle_repair_v1"
PARENT_RUN_ID = "run96A_stage96_v41_oos_early_entry_gate_followup_review_v1"
SOURCE_STAGE96_ID = "96_adapter_research__v41_oos_early_entry_gate_followup_review"
SOURCE_STAGE96_CLOSEOUT_COMMIT = "6c843b8b201da5e8aff17188d406a39c6c8c34f8"
SOURCE_STAGE96_LATEST_COMMIT = "b8ac64ba491004e8029eba318cd4f1fc2c94c6b2"
...
NEXT_STAGE_ID = "98_adapter_research__v41_oos_early_lifecycle_followup_review"
NEXT_RUN_ID = "run98A_stage98_v41_oos_early_lifecycle_followup_review_v1"
NEXT_PACKET_ID = "stage98_v41_oos_early_lifecycle_followup_review_v1"
...
COMMON_ROOT = f"OPV2/s97a/{RUN_NUMBER}"

SUMMARY_JSON_PATH = REVIEWS_ROOT / "stage97_v41_oos_early_lifecycle_repair_summary.json"
SUMMARY_CSV_PATH = REVIEWS_ROOT / "stage97_v41_oos_early_lifecycle_repair_summary.csv"
REPORT_PATH = REVIEWS_ROOT / "stage97_v41_oos_early_lifecycle_repair_report.md"
SEGMENT_KPI_PATH = REVIEWS_ROOT / "stage97_segment_kpi_summary.csv"
RISK_ATR_TELEMETRY_PATH = REVIEWS_ROOT / "stage97_risk_atr_telemetry.csv"
GATE_FEATURE_SUMMARY_PATH = REVIEWS_ROOT / "stage97_gate_feature_summary.csv"
TIER_B_DIAGNOSTIC_PATH = REVIEWS_ROOT / "stage97_tier_b_diagnostic_summary.csv"
DECISION_PATH = REVIEWS_ROOT / "stage97_decision.md"
AUDIT_CSV_PATH = REVIEWS_ROOT / "stage97_trade_audit.csv"
...
VARIANTS = (
    repair.RepairVariant(
        adapter_id="s97_v41_h2_risk475_gate08_sl2075_tp40_cd10",
        label="run50BN_v41_h2_risk475_gate08_sl2075_tp40_cd10",
        atr_enabled=True,
        ...
        max_hold_bars=2,
        notes="Stage97 OOS early lifecycle repair: keep gate08 SL2.075 TP4.0 CD10 and shorten max hold to 2 bars.",
    ),
    repair.RepairVariant(
        adapter_id="s97_v41_h4_risk475_gate08_sl2075_tp40_cd10",
        label="run50BN_v41_h4_risk475_gate08_sl2075_tp40_cd10",
        atr_enabled=True,
        model_risk_enabled=True,
        fixed_lot=0.25,
        atr_stop_multiplier=2.075,
        ...
        close_only_on_opposite_signal=True,
        max_hold_bars=4,
        notes="Stage97 OOS early lifecycle repair: keep gate08 SL2.075 TP4.0 CD10 and extend max hold to 4 bars.",
    ),
    repair.RepairVariant(
        adapter_id="s97_v41_h3_risk475_gate08_sl2075_tp40_cd8",
        label="run50BN_v41_h3_risk475_gate08_sl2075_tp40_cd8",
        atr_enabled=True,
        model_risk_enabled=True,
        fixed_lot=0.25,
        atr_stop_multiplier=2.075,
        ...
        close_only_on_opposite_signal=True,
        max_hold_bars=3,
        notes="Stage97 OOS early lifecycle repair: keep gate08 SL2.075 TP4.0 H3 and loosen same-direction cooldown to 8 bars.",
    ),
)
...
CONTEXT_GATE_THRESHOLDS = {variant.adapter_id: 0.08 for variant in VARIANTS}

CONTEXT_GATE_SPECS = {
    variant.adapter_id: {
        "gate_column": f"stage97_gate_margin_lt_{int(CONTEXT_GATE_THRESHOLDS[variant.adapter_id] * 100):03d}_short",
        "gate_type": "margin",
...
def decide(summary_rows: Sequence[Mapping[str, Any]], segment_rows: Sequence[Mapping[str, Any]], external: str) -> str:
    if external != "completed":
        return "continue_stage97_due_to_incomplete_runtime"
    ...
    if val_net >= 930 and oos_net >= 600 and val_dd <= 22.5 and oos_dd <= 20.5 and val_pf >= 1.52 and oos_pf >= 1.58 and not reasons:
        return "proceed_to_stage98_lifecycle_followup_review"
    if val_net >= 850 and oos_net >= 540 and val_dd <= 23.5 and oos_dd <= 21.0 and val_pf >= 1.45 and oos_pf >= 1.48:
        return "continue_oos_early_lifecycle_followup_review_in_stage98"
    if val_pf < 1.0 or val_net <= 0:
        return "open_new_model_source_branch_in_stage98"
    return "continue_oos_early_lifecycle_followup_review_in_stage98"
...
def report_markdown(
    ...
    return f"""# Stage97 V41 OOS Early Lifecycle Repair Report(97단계 V41 표본외 초반 생명주기 수리 보고서)

- run(실행): `{RUN_ID}`
- target_surface(목표 표면): `{TARGET_SURFACE}`
- source_stage(원천 단계): `{SOURCE_STAGE96_ID}`
- source_stage96_closeout_commit(원천 96단계 종료 커밋): `{SOURCE_STAGE96_CLOSEOUT_COMMIT}`
- source_stage96_latest_commit(원천 96단계 최신 커밋): `{SOURCE_STAGE96_LATEST_COMMIT}`
...
def decision_markdown(decision: str, external: str) -> str:
    return f"""# Stage97 Decision(97단계 판정)
            ...
            rows.append(
                {
                    ...
                    "split": split_name,
                    "tier_b_policy": "diagnostic_missing_required_but_disabled_for_this_v41_oos_early_lifecycle_repair",
                    ...
                    "reason": "Stage97 isolates v41 OOS early lifecycle repair first; Tier B fallback remains diagnostic and disabled.",
                }
            )
    return rows
            ...
            rows.append(
                {
                    ...
                    "artifact_type": "stage97_v41_oos_early_lifecycle_repair_evidence",
                    "path": rel(path),
                    "sha256": sha256_file_lf_normalized(path),
                    "stage_id": STAGE_ID,
                    "run_id": RUN_ID,
                    "created_at_utc": created,
                    "notes": "Stage97 v2-native v41 OOS early lifecycle repair artifact.",
                }
            )
            ...
            rows.append(
                {
                    ...
                    "run_id": RUN_ID,
                    "created_at_utc": created,
                    "notes": "Actual Stage97 MT5 Strategy Tester HTML report.",
                }
            )
    return rows
...
def write_run_identity(result: Mapping[str, Any]) -> None:
    write_json(
        RUN_ROOT / "run_manifest.json",
        {
            ...
            "source_stage96_closeout_commit": SOURCE_STAGE96_CLOSEOUT_COMMIT,
            "source_stage96_latest_commit": SOURCE_STAGE96_LATEST_COMMIT,
        ...
        [
            {
                "run_id": RUN_ID,
                "stage_id": STAGE_ID,
                "lane": "baseline_adapter_v2_native_v41_oos_early_lifecycle_repair",
                "status": status,
                "judgment": decision,
        ...
        alpha_rows = [
            {
                ...
                "tier_scope": "Tier A+B",
                "kpi_scope": "stage97_v41_oos_early_lifecycle_repair",
                ...
                "external_verification_status": external,
                "notes": "Stage97 run materialized or blocked before KPI records were available.",
            }
        ]
...
Stage98(98단계)는 Stage97(97단계)의 OOS early lifecycle repair(표본외 초반 생명주기 수리) 결과를 review gate(검토 게이트)로만 판독하는 follow-up(후속) 단계다.
...
Effect(효과): Stage98(98단계)는 Stage97(97단계)의 결과를 과장하지 않고 다음 판단으로만 좁힌다.
...
""",
    ...
    write_md(
        ...
        f"""# Stage98 Input References(98단계 입력 참조)

- source_stage(원천 단계): `{STAGE_ID}`
- source_run(원천 실행): `{RUN_ID}`
- source_decision(원천 판정): `{decision}`
- source_external_verification_status(원천 외부 검증 상태): `{external}`
- stage97_report(97단계 보고서): `{rel(REPORT_PATH)}`
- stage97_decision(97단계 판정): `{rel(DECISION_PATH)}`
- summary(요약): `{rel(SUMMARY_CSV_PATH)}`
- target_surface(목표 표면): `{TARGET_SURFACE}`

Effect(효과): Stage98(98단계)는 v2 고유 근거만 이어받아 34D KPI(34D 핵심 성과 지표) 격차 축소를 계속한다.
""",
    )
    write_md(NEXT_STAGE_ROOT / "03_reviews" / "review_index.md", f"""# Stage98 Review Index(98단계 검토 색인)
...
Effect(효과): Stage98(98단계)는 Stage97(97단계) closeout(종료 기록)을 이어받아 다음 bounded review(경계 검토)만 수행한다.
""")
    write_md(NEXT_STAGE_ROOT / "04_selected" / "selection_status.md", f"""# Stage98 Selection Status(98단계 선택 상태)
...
Effect(효과): Stage98(98단계)는 34D KPI(34D 핵심 성과 지표) 격차 축소를 계속하지만, 운영 의미 없이 연구개발로만 이어진다.
""")
...
- >-
  Stage97(97단계) closed(종료) as `{decision}` and Stage98(98단계) `{NEXT_STAGE_ID}` is open_planned(열림 계획). Effect(효과): v41 OOS early lifecycle repair(V41 표본외 초반 생명주기 수리) 근거를 보존하고 다음 경계 연구 질문으로만 넘긴다.
- >-
  Stage97 result(97단계 결과): validation/OOS(검증/표본외), PF/net/DD(수익 팩터/순손익/손실률), risk/ATR telemetry(위험/ATR 텔레메트리)는 `{rel(SUMMARY_CSV_PATH)}`와 `{rel(RISK_ATR_TELEMETRY_PATH)}`에 기록된다. Effect(효과): 34D target surface(34D 목표 표면) 대비 KPI(핵심 성과 지표) 차이를 다음 단계 입력으로 보존한다.
- >-
...
stage97_v41_oos_early_lifecycle_repair:
  packet_id: {PACKET_ID}
  stage_id: {STAGE_ID}
  ...
  boundary: {BOUNDARY}
"""
    if "stage97_v41_oos_early_lifecycle_repair:" in text:
        text = re.sub(r"\nstage97_v41_oos_early_lifecycle_repair:\n(?:  .*\n)+", block + "\n", text, count=1)
    else:
        text = text.rstrip() + block
    io_path(WORKSPACE_STATE_PATH).write_text(text + ("\n" if not text.endswith("\n") else ""), encoding="utf-8-sig")
    write_md(SELECTED_ROOT / "selection_status.md", f"""# Stage97 Selection Status(97단계 선택 상태)

- stage_status(단계 상태): `closed_{decision}`
- source_stage(원천 단계): `{SOURCE_STAGE96_ID}`
- source_decision(원천 판정): `continue_oos_early_lifecycle_repair_in_stage97`
- source_stage96_closeout_commit(원천 96단계 종료 커밋): `{SOURCE_STAGE96_CLOSEOUT_COMMIT}`
- source_stage96_latest_commit(원천 96단계 최신 커밋): `{SOURCE_STAGE96_LATEST_COMMIT}`
- source_stage93_closeout_commit(원천 93단계 종료 커밋): `{SOURCE_STAGE93_CLOSEOUT_COMMIT}`
- source_stage93_latest_commit(원천 93단계 최신 커밋): `{SOURCE_STAGE93_LATEST_COMMIT}`
...
- target_surface(목표 표면): `{TARGET_SURFACE}`
- stage97_decision(97단계 판정): `{decision}`
- next_stage_or_branch(다음 단계/분기): `{NEXT_STAGE_ID}`
- claim_boundary(주장 경계): `{BOUNDARY}`
...
- target_surface(목표 표면): `{TARGET_SURFACE}`
- adapter_under_review(검토 중 어댑터): `{SOURCE_ADAPTER_ID}`
- status(상태): `stage97_closed_{decision}`
- claim_boundary(주장 경계): `{BOUNDARY}`

Stage97(97단계) closed(종료) as v2-native v41 OOS early lifecycle repair batch(브이투 고유 브이41 표본외 초반 생명주기 수리 묶음). Effect(효과): legacy 34D(레거시 34D)는 target surface(목표 표면)로만 쓰였고, 다음 연구는 Stage98(98단계)로 이어진다.

## Latest Stage97 Evidence(최신 97단계 근거)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    prepared = {
        ...
        "model_family": "baseline_adapter_stage97_v2_native_v41_oos_early_lifecycle_repair_ebm_table",
        "feature_set_id": "stage97_v41_oos_early_lifecycle_signal",
