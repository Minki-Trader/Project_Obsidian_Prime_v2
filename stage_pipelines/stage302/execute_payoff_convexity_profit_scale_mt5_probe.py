...
RUN_ID = "run302B_payoff_convexity_profit_scale_mt5_probe_v1"
RUN_NUMBER = "run302B"
SOURCE_RUN_ID = "run302A_design_payoff_convexity_profit_scale_rebuild_v1"
PARENT_RUN_ID = "run301C_review_orthogonal_profit_source_mt5_probe_v1"
STATUS_PREPARED = "prepared_payoff_convexity_profit_scale_mt5_probe_no_runtime_kpi"
...
EXPLORATION_LABEL = "stage302_Model__PayoffConvexityProfitScaleReplay"
SIGNAL_COLUMN = "run302b_route_signal"
FEATURE_ORDER = (SIGNAL_COLUMN,)
COMMON_ROOT = "Project_Obsidian_Prime_v2/stage302/run302B_payoff_convexity_profit_scale"

STAGE_ROOT = ROOT / "stages" / STAGE_ID
RUN302A = STAGE_ROOT / "02_runs" / "run302A"
...
MT5_QUEUE = RUN302A / "mt5_probe_queue.csv"
SOURCE_MANIFEST = RUN302A / "candidate_payload_manifest.csv"
SOURCE_RUN_MANIFEST = RUN302A / "run_manifest.json"
SOURCE_REPORT = REVIEWS / "run302A_payoff_convexity_profit_scale_materialization_report.md"
PRODUCER = Path("stage_pipelines/stage302/execute_payoff_convexity_profit_scale_mt5_probe.py")
...
REPORT = REVIEWS / "run302B_payoff_convexity_profit_scale_mt5_probe_report.md"
...
def attach_identity(attempt: dict[str, Any], queue_row: Mapping[str, str], max_hold: int, close_on_flat: bool, same_reentry: int) -> None:
    ...
    attempt["stage302_branch_id"] = queue_row.get("materialized_branch_id", "")
    ...
    for queue_row in queue_rows:
        ...
        for runtime_split, split_token in (("validation_is", "val"), ("oos", "oos")):
            for tier_key, tier_label, tier_token in (
                ...
                attempt["signal_policy"] = "stage302 payoff convexity profit scale route_signal_value -1/0/+1 through single-feature table"
            ...
            if include_routed:
                ...
                attach_identity(attempt, queue_row, max_hold, close_on_flat, same_reentry)
                attempt["signal_policy"] = "Tier A primary + Tier B fallback stage302 payoff convexity profit scale route_signal_value"
                attempts.append(attempt)
    return attempts
...
def classify_status(result: Mapping[str, Any], materialize_only: bool) -> tuple[str, str, str, str]:
    ...
    if materialize_only:
        return (
            STATUS_PREPARED,
            "payoff_convexity_profit_scale_runtime_probe_prepared_no_external_execution",
            "out_of_scope_by_claim_materialize_only",
            "run302B_execute_payoff_convexity_profit_scale_mt5_probe_external_check",
        )
    ...
    if planned and completed_exec >= planned and len(kpis) >= planned:
        return (
            "completed_payoff_convexity_profit_scale_mt5_probe_no_selection",
            "runtime_probe_completed_requires_profit_scale_curve_review_no_selection",
            "completed",
            "run302C_review_payoff_convexity_profit_scale_mt5_probe",
        )
    if kpis:
        return (
            "partial_payoff_convexity_profit_scale_mt5_probe_no_selection",
            "runtime_probe_partial_requires_continuation_or_review_no_selection",
            "partial_or_blocked",
            "run302B_continue_payoff_convexity_profit_scale_mt5_probe" if limited else "run302C_review_with_runtime_gaps",
        )
    return (
        "blocked_payoff_convexity_profit_scale_mt5_probe_no_kpi",
        "runtime_probe_blocked_no_kpi_no_selection",
        "blocked_or_invalid",
        "run302B_repair_or_block_payoff_convexity_profit_scale_mt5_probe",
    )
...
def report_markdown(result: Mapping[str, Any], status: str, judgment: str, external_status: str, next_action: str) -> str:
    ...
    return "\n".join(
        [
            "# run302B Payoff Convexity Profit Scale MT5 Probe(302B 보상 볼록성 수익 규모 MT5 탐침)",
            "",
            ...
            "",
            "Effect(효과): run302A(302A 실행)의 payoff convexity profit scale(보상 볼록성 수익 규모) 신호와 risk logic(위험 로직)이 실제 MT5 tester(MT5 테스터)에서 순수익, 거래 수, 곡선 품질로 재현되는지 확인했다.",
            "",
            "## Boundary(경계)",
            "",
            f"`{BOUNDARY}`",
        ...
        [
            {
                "run_id": RUN_ID,
                "stage_id": STAGE_ID,
                "lane": "payoff_convexity_profit_scale_mt5_probe",
                "status": status,
                "judgment": judgment,
        ...
        [
            {
                ...
                "record_view": "payoff_convexity_profit_scale_mt5_probe",
        ...
        [
            {
                ...
                "stage_id": STAGE_ID,
                "run_id": RUN_ID,
                "view": "payoff_convexity_profit_scale_mt5_probe",
                "tier_scope": "Tier A used/Tier B fallback stress/actual routed total",
    ...
    rows = [
        {
            ...
            "artifact_type": "stage302_payoff_convexity_profit_scale_mt5_artifact",
            "path": prev.prev.e295.e294.base.base.rel(path),
            "sha256": prev.prev.e295.e294.base.base.sha256_file_lf_normalized(path),
            "stage_id": STAGE_ID,
            "run_id": RUN_ID,
            "created_at_utc": created_at,
            "notes": "run302B payoff convexity profit scale MT5 probe",
        }
        for path in paths
...
def update_docs(status: str, judgment: str, next_action: str, kpi_count: int, attempt_count: int) -> None:
    ...
    selected = prev.prev.e295.e294.base.base.append_once(selected, "run302B_report", f"- run302B_report(302B 보고): `{prev.prev.e295.e294.base.base.rel(REPORT)}`")
    selected = prev.prev.e295.e294.base.base.append_once(selected, "run302B_execution_result", f"- run302B_execution_result(302B 실행 결과): `{prev.prev.e295.e294.base.base.rel(EXECUTION_RESULT)}`")
    prev.prev.e295.e294.base.base.write_md(SELECTED, selected)

    review_index = prev.prev.e295.e294.base.base.io_path(REVIEW_INDEX).read_text(encoding="utf-8-sig") if prev.prev.e295.e294.base.base.path_exists(REVIEW_INDEX) else "# Stage302 Review Index(302단계 검토 색인)\n"
    review_index = prev.prev.e295.e294.base.base.append_once(
        review_index,
        "run302B_report",
        f"- run302B_report(302B 보고): `{prev.prev.e295.e294.base.base.rel(REPORT)}`\n- run302B_execution_result(302B 실행 결과): `{prev.prev.e295.e294.base.base.rel(EXECUTION_RESULT)}`\n- run302B_mt5_kpi_summary(302B MT5 KPI 요약): `{prev.prev.e295.e294.base.base.rel(MT5_KPI_SUMMARY)}`",
    )
    ...
    current = prev.prev.e295.e294.base.base.append_once(
        current,
        "run302B_summary",
        f"- run302B_summary(302B 요약): payoff convexity profit scale MT5 probe(보상 볼록성 수익 규모 MT5 탐침)를 실행했다. Effect(효과): attempts(시도) `{attempt_count}`개와 MT5 KPI records(MT5 KPI 기록) `{kpi_count}`개를 엮었고 후보/어댑터/ONNX(온엑스) 주장은 하지 않는다.",
    )
    ...
    focus = (
        f"- >-\n"
        f"  Stage302(302단계) run302B(302B 실행) payoff convexity profit scale MT5 probe(보상 볼록성 수익 규모 MT5 탐침) `{RUN_ID}`. "
    ...
    changelog = prev.prev.e295.e294.base.base.append_once(
        changelog,
        RUN_ID,
        f"## {UPDATED_ON} run302B payoff convexity profit scale MT5 probe(302B 보상 볼록성 수익 규모 MT5 탐침)\n\n"
        f"- status(상태): `{status}`\n"
        f"- judgment(판정): `{judgment}`\n"
