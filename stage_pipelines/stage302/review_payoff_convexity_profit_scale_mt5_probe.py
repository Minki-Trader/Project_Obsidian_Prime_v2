...
NEXT_STAGE_ID = "303_onnx_candidate_campaign__regime_balanced_profit_scale_router"
RUN_ID = "run302C_review_payoff_convexity_profit_scale_mt5_probe_v1"
RUN_NUMBER = "run302C"
SOURCE_RUN_ID = "run302B_payoff_convexity_profit_scale_mt5_probe_v1"
PARENT_RUN_ID = "run302A_design_payoff_convexity_profit_scale_rebuild_v1"
...
RUN302B = STAGE_ROOT / "02_runs" / "run302B"
...
SOURCE_EXECUTION = RUN302B / "execution_result.json"
SOURCE_KPI = RUN302B / "mt5_kpi_summary.csv"
SOURCE_SCOUT = RUN302A / "model_scout_scoreboard.csv"
PRODUCER = Path("stage_pipelines/stage302/review_payoff_convexity_profit_scale_mt5_probe.py")

SCOREBOARD = RUN_ROOT / "payoff_convexity_profit_scale_review_scoreboard.csv"
...
NEXT_STAGE_QUEUE = RUN_ROOT / "stage303_seed_queue.csv"
RESULT_JUDGMENT = RUN_ROOT / "result_judgment.csv"
GATE_AUDIT = RUN_ROOT / "required_gate_coverage_audit.csv"
RUN_MANIFEST = RUN_ROOT / "run_manifest.json"
LINEAGE = RUN_ROOT / "artifact_lineage_receipt.json"
REPORT = REVIEWS / "run302C_payoff_convexity_profit_scale_review_stage303_open_report.md"
DECISION = ROOT / "docs" / "decisions" / "2026-05-24_stage302_payoff_convexity_profit_scale_review_stage303_open.md"
...
def feature_trading_days(attempt: Mapping[str, Any]) -> int:
    ...
    local = RUN302B / "features" / Path(feature_path).name
...
def load_actual_rows() -> tuple[list[dict[str, Any]], dict[str, dict[str, Any]]]:
    ...
    with ledger.io_path(SOURCE_KPI).open("r", encoding="utf-8-sig", newline="") as handle:
        for source_row in csv.DictReader(handle):
            ...
            materialized_id = str(attempt.get("stage302_branch_id") or attempt.get("materialized_branch_id") or "")
...
def build_scoreboard(rows: Sequence[Mapping[str, Any]], scout_rows: Mapping[str, Mapping[str, Any]]) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    ...
    for candidate_id, split_rows in sorted(by_candidate.items()):
        ...
        if not selected:
            ...
            failure_rows.append(
                {
                    "failure_id": f"{candidate_id}_stage302_negative_or_watch",
                    "materialized_branch_id": candidate_id,
...
def stage303_queue_rows(scoreboard: Sequence[Mapping[str, Any]]) -> list[dict[str, Any]]:
    ...
    for index, row in enumerate(best, start=1):
        rows.append(
            {
                "seed_id": f"stage303_seed_{index:02d}",
                ...
                "prior_stage_refs": "stage301_positive_small;stage302_oos_scale_validation_damage",
                "claim_boundary": BOUNDARY,
            }
        )
    if not rows:
        rows.append(
            {
                "seed_id": "stage303_seed_fresh_regime_balance",
                "source_stage_id": STAGE_ID,
                "source_run_id": RUN_ID,
...
def scaffold_stage303() -> None:
    ...
    write_text(
        ...
        "\n".join(
            [
                ...
                "- question(질문): Can a regime-balanced profit scale router(레짐 균형 수익 규모 라우터) preserve the OOS scale(표본외 규모) seen in Stage302(302단계) while removing validation damage(검증 손상), keeping 4-10 trades/day(일 4-10거래), and smoothing the equity curve(평가금 곡선)?",
    ...
    write_text(
        ...
        "\n".join(
            [
                ...
                f"- stage302_review(302단계 검토): `{rel(REPORT)}`",
            ]
        ),
    )
    write_text(
    ...
    write_csv(
        ...
        [
            {
                "row_id": "stage303_opened_from_run302C",
                "stage_id": NEXT_STAGE_ID,
                "run_id": RUN_ID,
                "view": "stage_open",
                "tier_scope": "not_applicable",
                "scoreboard": "stage302_review",
                "status": "opened_regime_balanced_profit_scale_router",
                "judgment": "opened_from_stage302_oos_scale_validation_damage_no_onnx_worthy_candidate",
                "evidence_boundary": "planning_from_stage302_actual_mt5_evidence",
                "report_path": rel(REPORT),
                "notes": "next_action=run303A_design_regime_balanced_profit_scale_router_packet",
            }
        ],
    )
...
def update_docs(status: str, judgment: str, next_action: str) -> None:
    ...
    selected += f"- run302C_report(302C 보고): `{rel(REPORT)}`\n"
    selected += f"- stage303_opened(303단계 열림): `{NEXT_STAGE_ID}`\n"
    write_text(SELECTED, selected)

    review_index = read_text(REVIEW_INDEX)
    review_index += f"- run302C_report(302C 보고): `{rel(REPORT)}`\n"
    review_index += f"- run302C_scoreboard(302C 점수판): `{rel(SCOREBOARD)}`\n"
    review_index += f"- stage303_seed_queue(303단계 씨앗 대기열): `{rel(NEXT_STAGE_QUEUE)}`\n"
    ...
    current += "- run302C_summary(302C 요약): Stage302(302단계)는 actual MT5(실제 메타트레이더5) OOS scale(표본외 규모)을 만들었지만 validation damage(검증 손상)가 커서 ONNX-worthy(온엑스 가치 있음) 조건에는 부족했다. Effect(효과): Adapter(어댑터)와 ONNX(온엑스)를 시작하지 않고 Stage303(303단계) regime-balanced router(레짐 균형 라우터) 질문으로 전환했다.\n"
    ...
    focus = (
        f"- >-\n"
        f"  Stage302(302단계) run302C(302C 실행) payoff convexity profit scale review(보상 볼록성 수익 규모 검토) `{RUN_ID}` closed Stage302 and opened Stage303(303단계). "
        f"Effect(효과): selected candidate(선택 후보), Adapter package(어댑터 패키지), ONNX readiness(온엑스 준비)는 없고 next_action(다음 행동)은 `run303A_design_regime_balanced_profit_scale_router_packet`이다.\n"
    )
    ...
    changelog += (
        f"\n## {UPDATED_ON} run302C payoff convexity profit scale review(302C 보상 볼록성 수익 규모 검토)\n\n"
        f"- status(상태): `{status}`\n"
...
def update_registers(status: str, judgment: str, next_action: str) -> None:
    safe_upsert(
        ...
        [{"run_id": RUN_ID, "stage_id": STAGE_ID, "lane": "payoff_convexity_profit_scale_review", "status": status, "judgment": judgment, "path": rel(REPORT), "notes": f"selected_candidate=none;adapter_package=none;onnx_readiness=not_started;next_action={next_action}."}],
        "run_id",
    )
    safe_upsert(
        ALPHA_LEDGER,
        ...
        [
            {
                ...
                "record_view": "payoff_convexity_profit_scale_review",
    ...
    safe_upsert(
        STAGE_LEDGER,
        STAGE_LEDGER_COLUMNS,
        [{"row_id": f"{RUN_ID}__review", "stage_id": STAGE_ID, "run_id": RUN_ID, "view": "payoff_convexity_profit_scale_review", "tier_scope": "Tier A used/Tier B fallback/actual routed total", "scoreboard": "payoff_convexity_profit_scale_review_scoreboard", "status": status, "judgment": judgment, "evidence_boundary": "runtime_probe_review_no_candidate_no_onnx", "report_path": rel(REPORT), "notes": "Stage303 opened; no Adapter(어댑터); no ONNX(온엑스)."}],
        "row_id",
    )
...
def update_memory_registers(failures: Sequence[Mapping[str, Any]]) -> None:
    ...
    if RUN_ID not in negative:
        negative += (
            f"\n## {RUN_ID} Stage302 OOS-scale validation-damage failure memory(302단계 표본외 규모/검증 손상 실패 기억)\n\n"
        ...
        rows.append(
            {
                ...
                "artifact_type": "stage302_payoff_convexity_profit_scale_review_artifact",
                "path": rel(path),
                "sha256": ledger.sha256_file_lf_normalized(path),
                "stage_id": STAGE_ID,
                "run_id": RUN_ID,
                "created_at_utc": "2026-05-24T20:50:00Z",
                "notes": "Stage302 review and Stage303 open handoff",
            }
        )
...
def main() -> None:
    ...
    stage303_rows = stage303_queue_rows(scoreboard)
    status = "completed_payoff_convexity_profit_scale_review_no_candidate_stage303_opened"
    judgment = "actual_mt5_oos_scale_validation_damage_not_onnx_worthy_no_adapter_no_onnx"
    next_action = "run303A_design_regime_balanced_profit_scale_router_packet"
    ...
    write_csv(NEXT_STAGE_QUEUE, list(stage303_rows[0].keys()), stage303_rows)
    write_csv(
        RESULT_JUDGMENT,
