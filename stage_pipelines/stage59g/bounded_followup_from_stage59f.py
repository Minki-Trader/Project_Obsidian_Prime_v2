...
NEXT_REPAIR_STAGE_ID = "59H_adapter_repair__bounded_followup_from_stage59g"
RUN_NUMBER = "run59B"
RUN_ID = "run59B_stage59g_bounded_followup_from_stage59f_v1"
PACKET_ID = "stage59g_bounded_followup_from_stage59f_v1"
PARENT_RUN_ID = "run59A_stage59f_new_model_branch_from_failure_memory_v1"
SOURCE_ADAPTER_ID = "s59f_v54_coo"
...
STAGE_ROOT = Path("stages") / STAGE59G_ID
...
COMMON_ROOT = f"Project_Obsidian_Prime_v2/s59g/{RUN_NUMBER}"

REPORT_PATH = REVIEWS_ROOT / "bounded_followup_from_stage59f_report.md"
...
DECISION_PATH = REVIEWS_ROOT / "stage59g_decision.md"
AUDIT_CSV_PATH = REVIEWS_ROOT / "stage59g_trade_audit.csv"
...
STAGE59G_VARIANTS = (
    repair.RepairVariant(
        adapter_id="s59g_v54_sd10",
        label="run50BP_v54_stage59f_source_same_direction_cooldown10",
        atr_enabled=True,
        ...
        max_hold_bars=2,
        notes="Stage59G bounded follow-up: keep Stage59F v54 source and widen same-direction re-entry cooldown to test validation damage relief.",
    ),
    repair.RepairVariant(
        adapter_id="s59g_v54_th60_sd8",
        label="run50BP_v54_stage59f_source_threshold60_cooldown8",
        atr_enabled=True,
        model_risk_enabled=True,
        fixed_lot=0.25,
        atr_stop_multiplier=2.5,
        ...
        close_only_on_opposite_signal=True,
        max_hold_bars=2,
        notes="Stage59G bounded follow-up: test modest confidence threshold pressure plus re-entry cooldown against Stage59F validation weakness.",
    ),
    repair.RepairVariant(
        adapter_id="s59g_v54_trn02_sd8",
        label="run50BP_v54_stage59f_source_transition_rearm02_cooldown8",
        atr_enabled=True,
        model_risk_enabled=True,
        fixed_lot=0.25,
        atr_stop_multiplier=2.5,
        ...
        close_only_on_opposite_signal=True,
        max_hold_bars=2,
        notes="Stage59G bounded follow-up: require transition/rearm confirmation to test same-move density and validation segment repair.",
    ),
)
...
def source_specs() -> dict[str, dict[str, Any]]:
    ...
    return {variant.adapter_id: dict(v54) for variant in STAGE59G_VARIANTS}


def configure_reused_engine() -> None:
    engine.STAGE59_ID = STAGE59G_ID
    ...
    engine.STAGE59_VARIANTS = STAGE59G_VARIANTS
    engine.SOURCE_SPECS = source_specs()
    engine.MODEL_RISK_MIN_PCT = {variant.adapter_id: 0.005 for variant in STAGE59G_VARIANTS}

    repair.STAGE_ID = STAGE59G_ID
    repair.RUN_NUMBER = RUN_NUMBER
    repair.RUN_ID = RUN_ID
    repair.RUN_ROOT = RUN_ROOT
    repair.REPAIR_VARIANTS = STAGE59G_VARIANTS
    s58.STAGE58_ID = STAGE59G_ID
    ...
    s58.STAGE58_VARIANTS = STAGE59G_VARIANTS
...
def build_attempts(inputs: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    for variant_index, variant in enumerate(STAGE59G_VARIANTS, start=1):
            ...
            for role_index, (tier, attempt_role, prefix, attempt_token) in enumerate(
                ...
                attempts.append(
                    attempt_payload(
                        ...
                        run_id=RUN_ID,
                        stage_number=59,
                        exploration_label="stage59G_BaselineAdapter__BoundedFollowupFromStage59F",
...
def execute_prepared_run_checkpointed(prepared: Mapping[str, Any], args: argparse.Namespace) -> dict[str, Any]:
    ...
    return {
        **dict(prepared),
        ...
        "judgment": "inconclusive_stage59g_runtime_probe_completed" if completed and report_completed else "blocked_stage59g_runtime_probe",
    }
...
def next_stage_for_decision(decision: str) -> str:
    ...
    if decision == "demote_current_adapter_and_select_backup":
        return "59H_adapter_repair__backup_or_demote_from_stage59g"
    if decision == "open_new_model_branch":
        return "59H_adapter_repair__new_model_branch_from_stage59g"
...
def next_packet_for_decision(decision: str) -> tuple[str, str]:
    ...
    return "stage59h_bounded_followup_from_stage59g_v1", "run59C_stage59h_bounded_followup_from_stage59g_v1"
            ...
            rows.append(
                {
                    ...
                    "run_id": RUN_ID,
                    "artifact_type": "stage59g_bounded_followup_evidence",
                    "path": rel(path),
                    "sha256": sha256_file_lf_normalized(path),
                    "created_at_utc": created,
                    "notes": "Stage59G bounded follow-up from Stage59F artifact.",
                }
            )
            ...
            rows.append(
                {
                    ...
                    "created_at_utc": created,
                    "notes": "Actual Stage59G MT5 Strategy Tester HTML report.",
                }
            )
    return rows
...
def write_run_identity(result: Mapping[str, Any]) -> None:
    engine.write_json(
        RUN_ROOT / "run_manifest.json",
        {
            ...
            "bounded_question": "Can a narrow same-direction re-entry and threshold follow-up repair Stage59F v54 validation weakness without starting ONNX?",
            "source_stage59f_decision": rel(SOURCE_STAGE59F_DECISION),
            ...
            "variants": [
                ...
                for variant in STAGE59G_VARIANTS
            ],
...
def write_ledgers(
        ...
        [
            {
                "run_id": RUN_ID,
                "stage_id": STAGE59G_ID,
                "lane": "baseline_adapter_bounded_followup_from_stage59f",
                "status": status,
                "judgment": decision,
    ...
    ledger_rows.append(
        {
            "ledger_row_id": f"{RUN_ID}__aggregate_bounded_followup_from_stage59f",
            "stage_id": STAGE59G_ID,
            "run_id": RUN_ID,
            "subrun_id": "aggregate_bounded_followup_from_stage59f",
            "parent_run_id": PARENT_RUN_ID,
            "record_view": "bounded_followup_from_stage59f",
            "tier_scope": "Tier A+B",
            ...
            "external_verification_status": external,
            "notes": "Stage59G bounded follow-up from Stage59F v54 validation weakness; not final package completion.",
        }
    )
...
def parse_args(argv: Sequence[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run Stage59G bounded follow-up from Stage59F.")
...
def load_existing_result() -> dict[str, Any]:
    ...
    if not path_exists(manifest) or not path_exists(kpi):
        raise FileNotFoundError("Stage59G existing run_manifest.json or kpi_record.json is missing")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.finalize_existing:
        ...
        prepared = {
            ...
            "model_family": "baseline_adapter_stage59g_bounded_followup_ebm_table",
            "feature_set_id": "stage59g_v54_reentry_threshold_followup",
