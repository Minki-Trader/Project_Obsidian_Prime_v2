...
RUN_ID = "run303B_regime_balanced_profit_scale_router_mt5_probe_v1"
RUN_NUMBER = "run303B"
SOURCE_RUN_ID = "run303A_design_regime_balanced_profit_scale_router_v1"
PARENT_RUN_ID = "run302C_review_payoff_convexity_profit_scale_mt5_probe_v1"
STATUS_PREPARED = "prepared_regime_balanced_profit_scale_router_mt5_probe_no_runtime_kpi"
...
EXPLORATION_LABEL = "stage303_Model__RegimeBalancedProfitScaleRouterReplay"
SIGNAL_COLUMN = "run303b_route_signal"
FEATURE_ORDER = (SIGNAL_COLUMN,)
COMMON_ROOT = "Project_Obsidian_Prime_v2/stage303/run303B_regime_balanced_profit_scale_router"
...
MT5_QUEUE = RUN303A / "mt5_probe_queue.csv"
SOURCE_MANIFEST = RUN303A / "candidate_payload_manifest.csv"
SOURCE_RUN_MANIFEST = RUN303A / "run_manifest.json"
SOURCE_REPORT = REVIEWS / "run303A_regime_balanced_profit_scale_router_materialization_report.md"
PRODUCER = Path("stage_pipelines/stage303/execute_regime_balanced_profit_scale_router_mt5_probe.py")
...
REPORT = REVIEWS / "run303B_regime_balanced_profit_scale_router_mt5_probe_report.md"
...
def safe_upsert_rows(path: Path, columns: Sequence[str], rows: Sequence[Mapping[str, Any]], key: str) -> None:
    try:
        prev.prev.e295.e294.base.base.upsert_rows(path, columns, rows, key=key)
        return
    except OSError:
        pass
...
def extra_set_values(queue_row: Mapping[str, str]) -> dict[str, Any]:
    return {
        ...
        "InpSameDirectionReentryCooldownBars": prev.prev.e295.e294.int_value(queue_row.get("same_direction_reentry_cooldown_bars"), 0),
        "InpAtrSltpEnabled": prev.prev.e295.e294.bool_value(queue_row.get("atr_sltp_enabled")),
        "InpAtrPeriod": prev.prev.e295.e294.int_value(queue_row.get("atr_period"), 14),
        ...
        "InpExitRiskOverlayEnabled": prev.prev.e295.e294.bool_value(queue_row.get("exit_risk_overlay_enabled")),
        "InpExitRiskCloseLongFeatureIndex": prev.prev.e295.e294.int_value(queue_row.get("exit_risk_close_long_feature_index"), -1),
        "InpExitRiskCloseShortFeatureIndex": prev.prev.e295.e294.int_value(queue_row.get("exit_risk_close_short_feature_index"), -1),
        "InpExitRiskCloseThreshold": float_value(queue_row.get("exit_risk_close_threshold"), 0.5),
        "InpExitRiskMinHoldBars": prev.prev.e295.e294.int_value(queue_row.get("exit_risk_min_hold_bars"), 0),
        "InpExitRiskMaxHoldFeatureIndex": prev.prev.e295.e294.int_value(queue_row.get("exit_risk_max_hold_feature_index"), -1),
        "InpModelRiskSizingEnabled": prev.prev.e295.e294.bool_value(queue_row.get("model_risk_sizing_enabled")),
...
def attach_identity(attempt: dict[str, Any], queue_row: Mapping[str, str], max_hold: int, close_on_flat: bool, same_reentry: int) -> None:
    ...
    attempt["stage303_branch_id"] = queue_row.get("materialized_branch_id", "")
    attempt["stage302_branch_id"] = queue_row.get("stage302_branch_id", "")
    attempt["stage301_branch_id"] = queue_row.get("stage301_branch_id", "")
    attempt["stage300_branch_id"] = queue_row.get("stage300_branch_id", "")
    ...
    for queue_row in queue_rows:
        ...
        for runtime_split, split_token in (("validation_is", "val"), ("oos", "oos")):
            for tier_key, tier_label, tier_token in (
                ...
                attempt["signal_policy"] = "stage303 regime balanced profit scale router route_signal_value -1/0/+1 through single-feature table"
            ...
            if include_routed:
                ...
                attach_identity(attempt, queue_row, max_hold, close_on_flat, same_reentry)
                attempt["signal_policy"] = "Tier A primary + Tier B fallback stage303 regime balanced profit scale router route_signal_value"
                attempts.append(attempt)
    return attempts
...
def classify_status(result: Mapping[str, Any], materialize_only: bool) -> tuple[str, str, str, str]:
    ...
    if materialize_only:
        return (
            STATUS_PREPARED,
            "regime_balanced_profit_scale_router_runtime_probe_prepared_no_external_execution",
            "out_of_scope_by_claim_materialize_only",
            "run303B_execute_regime_balanced_profit_scale_router_mt5_probe_external_check",
        )
    ...
    if planned and completed_exec >= planned and len(kpis) >= planned:
        return (
            "completed_regime_balanced_profit_scale_router_mt5_probe_no_selection",
            "runtime_probe_completed_requires_profit_scale_curve_review_no_selection",
            "completed",
            "run303C_review_regime_balanced_profit_scale_router_mt5_probe",
        )
    if kpis:
        return (
            "partial_regime_balanced_profit_scale_router_mt5_probe_no_selection",
            "runtime_probe_partial_requires_continuation_or_review_no_selection",
            "partial_or_blocked",
            "run303B_continue_regime_balanced_profit_scale_router_mt5_probe" if limited else "run303C_review_with_runtime_gaps",
        )
    return (
        "blocked_regime_balanced_profit_scale_router_mt5_probe_no_kpi",
        "runtime_probe_blocked_no_kpi_no_selection",
        "blocked_or_invalid",
        "run303B_repair_or_block_regime_balanced_profit_scale_router_mt5_probe",
    )
...
def report_markdown(result: Mapping[str, Any], status: str, judgment: str, external_status: str, next_action: str) -> str:
    ...
    return "\n".join(
        [
            "# run303B Regime Balanced Profit Scale Router MT5 Probe(302B 보상 볼록성 수익 규모 MT5 탐침)",
            "",
            ...
            "",
            "Effect(효과): run303A(302A 실행)의 regime balanced profit scale router(보상 볼록성 수익 규모) 신호와 risk logic(위험 로직)이 실제 MT5 tester(MT5 테스터)에서 순수익, 거래 수, 곡선 품질로 재현되는지 확인했다.",
            "",
            "## Boundary(경계)",
            "",
            f"`{BOUNDARY}`",
        ...
        [
            {
                "run_id": RUN_ID,
                "stage_id": STAGE_ID,
                "lane": "regime_balanced_profit_scale_router_mt5_probe",
                "status": status,
                "judgment": judgment,
        ...
        [
            {
                ...
                "record_view": "regime_balanced_profit_scale_router_mt5_probe",
        ...
        [
            {
                ...
                "stage_id": STAGE_ID,
                "run_id": RUN_ID,
                "view": "regime_balanced_profit_scale_router_mt5_probe",
                "tier_scope": "Tier A used/Tier B fallback stress/actual routed total",
    ...
    rows = [
        {
            ...
            "artifact_type": "stage303_regime_balanced_profit_scale_router_mt5_artifact",
            "path": prev.prev.e295.e294.base.base.rel(path),
            "sha256": prev.prev.e295.e294.base.base.sha256_file_lf_normalized(path),
            "stage_id": STAGE_ID,
            "run_id": RUN_ID,
            "created_at_utc": created_at,
            "notes": "run303B regime balanced profit scale router MT5 probe",
        }
        for path in paths
...
def update_docs(status: str, judgment: str, next_action: str, kpi_count: int, attempt_count: int) -> None:
    ...
    selected = prev.prev.e295.e294.base.base.append_once(selected, "run303B_report", f"- run303B_report(302B 보고): `{prev.prev.e295.e294.base.base.rel(REPORT)}`")
    selected = prev.prev.e295.e294.base.base.append_once(selected, "run303B_execution_result", f"- run303B_execution_result(302B 실행 결과): `{prev.prev.e295.e294.base.base.rel(EXECUTION_RESULT)}`")
    prev.prev.e295.e294.base.base.write_md(SELECTED, selected)

    review_index = prev.prev.e295.e294.base.base.io_path(REVIEW_INDEX).read_text(encoding="utf-8-sig") if prev.prev.e295.e294.base.base.path_exists(REVIEW_INDEX) else "# Stage303 Review Index(302단계 검토 색인)\n"
    review_index = prev.prev.e295.e294.base.base.append_once(
        review_index,
        "run303B_report",
        f"- run303B_report(302B 보고): `{prev.prev.e295.e294.base.base.rel(REPORT)}`\n- run303B_execution_result(302B 실행 결과): `{prev.prev.e295.e294.base.base.rel(EXECUTION_RESULT)}`\n- run303B_mt5_kpi_summary(302B MT5 KPI 요약): `{prev.prev.e295.e294.base.base.rel(MT5_KPI_SUMMARY)}`",
    )
    ...
    current = prev.prev.e295.e294.base.base.append_once(
        current,
        "run303B_summary",
        f"- run303B_summary(302B 요약): regime balanced profit scale router MT5 probe(보상 볼록성 수익 규모 MT5 탐침)를 실행했다. Effect(효과): attempts(시도) `{attempt_count}`개와 MT5 KPI records(MT5 KPI 기록) `{kpi_count}`개를 엮었고 후보/어댑터/ONNX(온엑스) 주장은 하지 않는다.",
    )
    ...
    focus = (
        f"- >-\n"
        f"  Stage303(302단계) run303B(302B 실행) regime balanced profit scale router MT5 probe(보상 볼록성 수익 규모 MT5 탐침) `{RUN_ID}`. "
    ...
    changelog = prev.prev.e295.e294.base.base.append_once(
        changelog,
        RUN_ID,
        f"## {UPDATED_ON} run303B regime balanced profit scale router MT5 probe(302B 보상 볼록성 수익 규모 MT5 탐침)\n\n"
        f"- status(상태): `{status}`\n"
        f"- judgment(판정): `{judgment}`\n"
