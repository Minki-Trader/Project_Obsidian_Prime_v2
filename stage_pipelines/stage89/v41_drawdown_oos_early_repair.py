...
STAGE_ID = "89_adapter_research__v41_drawdown_and_oos_early_repair"
RUN_NUMBER = "run89A"
RUN_ID = "run89A_stage89_v41_drawdown_oos_early_repair_v1"
PACKET_ID = "stage89_v41_drawdown_oos_early_repair_v1"
PARENT_RUN_ID = "run88A_stage88_v41_tp_risk_balance_followup_review_v1"
SOURCE_STAGE88_ID = "88_adapter_research__v41_tp_risk_balance_followup_review"
SOURCE_STAGE88_CLOSEOUT_COMMIT = "65ef18b96c7d643339129104df722bbc6bc66c12"
SOURCE_STAGE88_LATEST_COMMIT = "cc33c88091eabf1e0f127ea08d9f7fbe7d99c065"
SOURCE_STAGE87_CLOSEOUT_COMMIT = "025fbbdb0f1cc03bd0afb5705ca4e6f4db720a57"
SOURCE_STAGE87_LATEST_COMMIT = "8d4ae045c08abdbfa6742d945a22f706dc9890a6"
SOURCE_STAGE73_LATEST_COMMIT = "76db6f199ff917da2f8311544f68dc6f24612e0e"
NEXT_STAGE_ID = "90_adapter_research__v41_drawdown_oos_early_followup_review"
NEXT_RUN_ID = "run90A_stage90_v41_drawdown_oos_early_followup_review_v1"
NEXT_PACKET_ID = "stage90_v41_drawdown_oos_early_followup_review_v1"
SOURCE_ADAPTER_ID = "s87_v41_h3_risk475_gate08_sl215_tp38_cd10"
...
COMMON_ROOT = f"OPV2/s89a/{RUN_NUMBER}"

SUMMARY_JSON_PATH = REVIEWS_ROOT / "stage89_v41_drawdown_oos_early_repair_summary.json"
SUMMARY_CSV_PATH = REVIEWS_ROOT / "stage89_v41_drawdown_oos_early_repair_summary.csv"
REPORT_PATH = REVIEWS_ROOT / "stage89_v41_drawdown_oos_early_repair_report.md"
SEGMENT_KPI_PATH = REVIEWS_ROOT / "stage89_segment_kpi_summary.csv"
RISK_ATR_TELEMETRY_PATH = REVIEWS_ROOT / "stage89_risk_atr_telemetry.csv"
GATE_FEATURE_SUMMARY_PATH = REVIEWS_ROOT / "stage89_gate_feature_summary.csv"
TIER_B_DIAGNOSTIC_PATH = REVIEWS_ROOT / "stage89_tier_b_diagnostic_summary.csv"
DECISION_PATH = REVIEWS_ROOT / "stage89_decision.md"
AUDIT_CSV_PATH = REVIEWS_ROOT / "stage89_trade_audit.csv"
...
VARIANTS = (
    repair.RepairVariant(
        adapter_id="s89_v41_h3_risk475_gate08_sl205_tp38_cd10",
        label="run50BN_v41_h3_risk475_gate08_sl205_tp38_cd10",
        atr_enabled=True,
        ...
        max_hold_bars=3,
        notes="Stage89 drawdown/OOS early repair: compress SL to 2.05 while preserving risk4.75 TP3.8 CD10.",
    ),
    repair.RepairVariant(
        adapter_id="s89_v41_h3_risk475_gate08_sl215_tp38_cd12",
        label="run50BN_v41_h3_risk475_gate08_sl215_tp38_cd12",
        atr_enabled=True,
        model_risk_enabled=True,
        fixed_lot=0.25,
        atr_stop_multiplier=2.15,
        atr_take_profit_multiplier=3.8,
        model_risk_max_pct=0.0475,
        ...
        close_only_on_opposite_signal=True,
        max_hold_bars=3,
        notes="Stage89 drawdown/OOS early repair: preserve SL2.15 TP3.8 risk4.75 and test CD12 guard.",
    ),
    repair.RepairVariant(
        adapter_id="s89_v41_h3_risk45_gate08_sl215_tp38_cd10",
        label="run50BN_v41_h3_risk45_gate08_sl215_tp38_cd10",
        atr_enabled=True,
        model_risk_enabled=True,
        fixed_lot=0.25,
        atr_stop_multiplier=2.15,
        ...
        close_only_on_opposite_signal=True,
        max_hold_bars=3,
        notes="Stage89 drawdown/OOS early repair: lower risk cap to 4.5 while preserving SL2.15 TP3.8 CD10.",
    ),
)
...
CONTEXT_GATE_SPECS = {
    variant.adapter_id: {
        "gate_column": "stage89_gate_margin_lt_008_short",
        "gate_type": "margin",
        "threshold": 0.08,
        "block_mode": "short",
...
def decide(summary_rows: Sequence[Mapping[str, Any]], segment_rows: Sequence[Mapping[str, Any]], external: str) -> str:
    if external != "completed":
        return "continue_stage89_due_to_incomplete_runtime"
    ...
    if val_net >= 900 and oos_net >= 520 and val_dd <= 22.5 and oos_dd <= 20.8 and val_pf >= 1.45 and oos_pf >= 1.45 and not reasons:
        return "proceed_to_stage90_followup_review"
    if val_net >= 800 and oos_net >= 430 and val_dd <= 26.0 and oos_dd <= 22.8 and val_pf >= 1.35 and oos_pf >= 1.35:
        return "continue_drawdown_oos_early_followup_review_in_stage90"
    if val_pf < 1.0 or val_net <= 0:
        return "open_new_model_source_branch_in_stage90"
    return "continue_drawdown_oos_early_followup_review_in_stage90"
...
def report_markdown(
    ...
    return f"""# Stage89 V41 Drawdown/OOS Early Repair Report(89단계 V41 손실률/표본외 초반 수리 보고서)

- run(실행): `{RUN_ID}`
- target_surface(목표 표면): `{TARGET_SURFACE}`
- source_stage(원천 단계): `{SOURCE_STAGE88_ID}`
- source_stage88_closeout_commit(원천 88단계 종료 커밋): `{SOURCE_STAGE88_CLOSEOUT_COMMIT}`
- source_stage88_latest_commit(원천 88단계 최신 커밋): `{SOURCE_STAGE88_LATEST_COMMIT}`
- source_stage87_closeout_commit(원천 87단계 종료 커밋): `{SOURCE_STAGE87_CLOSEOUT_COMMIT}`
- source_stage87_latest_commit(원천 87단계 최신 커밋): `{SOURCE_STAGE87_LATEST_COMMIT}`
...
def decision_markdown(decision: str, external: str) -> str:
    return f"""# Stage89 Decision(89단계 판정)

decision(판정): `{decision}`

Stage89(89단계)는 Stage88(88단계) 판정에 따라 Stage87 best(87단계 최선안)의 validation DD(검증 손실률)와 OOS early(표본외 초반)를 좁게 수리했다.
            ...
            rows.append(
                {
                    ...
                    "split": split_name,
                    "tier_b_policy": "diagnostic_missing_required_but_disabled_for_this_v41_drawdown_oos_early_repair",
                    ...
                    "reason": "Stage89 isolates v41 drawdown/OOS early first; Tier B fallback remains diagnostic and disabled.",
                }
            )
    return rows
            ...
            rows.append(
                {
                    ...
                    "artifact_type": "stage89_v41_drawdown_oos_early_repair_evidence",
                    "path": rel(path),
                    "sha256": sha256_file_lf_normalized(path),
                    "stage_id": STAGE_ID,
                    "run_id": RUN_ID,
                    "created_at_utc": created,
                    "notes": "Stage89 v2-native v41 drawdown/OOS early repair artifact.",
                }
            )
            ...
            rows.append(
                {
                    ...
                    "run_id": RUN_ID,
                    "created_at_utc": created,
                    "notes": "Actual Stage89 MT5 Strategy Tester HTML report.",
                }
            )
    return rows
...
def write_run_identity(result: Mapping[str, Any]) -> None:
    write_json(
        RUN_ROOT / "run_manifest.json",
        {
            ...
            "source_stage88_closeout_commit": SOURCE_STAGE88_CLOSEOUT_COMMIT,
            "source_stage88_latest_commit": SOURCE_STAGE88_LATEST_COMMIT,
            "source_stage87_closeout_commit": SOURCE_STAGE87_CLOSEOUT_COMMIT,
            "source_stage87_latest_commit": SOURCE_STAGE87_LATEST_COMMIT,
        ...
        [
            {
                "run_id": RUN_ID,
                "stage_id": STAGE_ID,
                "lane": "baseline_adapter_v2_native_v41_drawdown_oos_early_repair",
                "status": status,
                "judgment": decision,
        ...
        alpha_rows = [
            {
                ...
                "tier_scope": "Tier A+B",
                "kpi_scope": "stage89_v41_drawdown_oos_early_repair",
                ...
                "external_verification_status": external,
                "notes": "Stage89 run materialized or blocked before KPI records were available.",
            }
        ]
...
def write_packet_files(result: Mapping[str, Any], decision: str, ledger_payload: Mapping[str, Any]) -> None:
    ...
    write_json(PACKET_ROOT / "aggregate_summary.json", {"packet_id": PACKET_ID, "stage_id": STAGE_ID, "run_id": RUN_ID, "decision": decision, "source_stage88_closeout_commit": SOURCE_STAGE88_CLOSEOUT_COMMIT, "source_stage88_latest_commit": SOURCE_STAGE88_LATEST_COMMIT, "source_stage87_closeout_commit": SOURCE_STAGE87_CLOSEOUT_COMMIT, "source_stage87_latest_commit": SOURCE_STAGE87_LATEST_COMMIT, "source_stage73_latest_commit": SOURCE_STAGE73_LATEST_COMMIT, "gate_feature_summary_path": rel(GATE_FEATURE_SUMMARY_PATH), "ledger_payload": ledger_payload, "overall_goal_complete": False})
...
Stage90(90단계)는 Stage89(89단계)의 drawdown/OOS early repair(손실률/표본외 초반 수리) 결과를 review gate(검토 게이트)로만 판독하는 follow-up(후속) 단계다.

## Bounded Question(경계 질문)

Stage89(89단계)의 SL compression/cooldown/risk cap(손절 압축/재진입 냉각/위험 상한) 조합이 Stage87 best(87단계 최선안)의 PF/net(수익 팩터/순손익)을 보존하면서 validation DD(검증 손실률)와 OOS early(표본외 초반)를 충분히 수리했는가?

Effect(효과): Stage90(90단계)는 Stage89(89단계)의 결과를 과장하지 않고 다음 판단으로만 좁힌다.
...
""",
    ...
    write_md(
        ...
        f"""# Stage90 Input References(90단계 입력 참조)

- source_stage(원천 단계): `{STAGE_ID}`
- source_run(원천 실행): `{RUN_ID}`
- source_decision(원천 판정): `{decision}`
- source_external_verification_status(원천 외부 검증 상태): `{external}`
- stage89_report(89단계 보고서): `{rel(REPORT_PATH)}`
- stage89_decision(89단계 판정): `{rel(DECISION_PATH)}`
- summary(요약): `{rel(SUMMARY_CSV_PATH)}`
- target_surface(목표 표면): `{TARGET_SURFACE}`

Effect(효과): Stage90(90단계)는 v2 고유 근거만 이어받아 34D KPI(34D 핵심 성과 지표) 격차 축소를 계속한다.
""",
    )
    write_md(NEXT_STAGE_ROOT / "03_reviews" / "review_index.md", f"""# Stage90 Review Index(90단계 검토 색인)
...
Effect(효과): Stage90(90단계)는 Stage89(89단계) closeout(종료 기록)을 이어받아 다음 bounded batch(경계 묶음 실행)만 검토한다.
""")
    write_md(NEXT_STAGE_ROOT / "04_selected" / "selection_status.md", f"""# Stage90 Selection Status(90단계 선택 상태)
...
Effect(효과): Stage90(90단계)는 34D KPI(34D 핵심 성과 지표) 격차 축소를 계속하지만, 운영 의미 없이 연구개발로만 이어진다.
""")
...
- >-
  Stage89(89단계) closed(종료) as `{decision}` and Stage90(90단계) `{NEXT_STAGE_ID}` is open_planned(열림 계획). Effect(효과): v41 drawdown/OOS early repair(V41 손실률/표본외 초반 수리) 근거를 보존하고 다음 경계 연구 질문으로만 넘긴다.
- >-
  Stage89 result(89단계 결과): validation/OOS(검증/표본외), PF/net/DD(수익 팩터/순손익/손실률), risk/ATR telemetry(위험/ATR 텔레메트리)는 `{rel(SUMMARY_CSV_PATH)}`와 `{rel(RISK_ATR_TELEMETRY_PATH)}`에 기록된다. Effect(효과): 34D target surface(34D 목표 표면) 대비 KPI(핵심 성과 지표) 차이를 다음 단계 입력으로 보존한다.
- >-
...
stage89_v41_drawdown_oos_early_repair:
  packet_id: {PACKET_ID}
  stage_id: {STAGE_ID}
  ...
  boundary: {BOUNDARY}
"""
    if "stage89_v41_drawdown_oos_early_repair:" in text:
        text = re.sub(r"\nstage89_v41_drawdown_oos_early_repair:\n(?:  .*\n)+", block + "\n", text, count=1)
    else:
        text = text.rstrip() + block
    io_path(WORKSPACE_STATE_PATH).write_text(text + ("\n" if not text.endswith("\n") else ""), encoding="utf-8-sig")
    write_md(SELECTED_ROOT / "selection_status.md", f"""# Stage89 Selection Status(89단계 선택 상태)

- stage_status(단계 상태): `closed_{decision}`
- source_stage(원천 단계): `{SOURCE_STAGE88_ID}`
- source_decision(원천 판정): `continue_drawdown_oos_early_repair_in_stage89`
- source_stage88_closeout_commit(원천 88단계 종료 커밋): `{SOURCE_STAGE88_CLOSEOUT_COMMIT}`
- source_stage88_latest_commit(원천 88단계 최신 커밋): `{SOURCE_STAGE88_LATEST_COMMIT}`
- source_stage87_closeout_commit(원천 87단계 종료 커밋): `{SOURCE_STAGE87_CLOSEOUT_COMMIT}`
- source_stage87_latest_commit(원천 87단계 최신 커밋): `{SOURCE_STAGE87_LATEST_COMMIT}`
...
- target_surface(목표 표면): `{TARGET_SURFACE}`
- stage89_decision(89단계 판정): `{decision}`
- next_stage_or_branch(다음 단계/분기): `{NEXT_STAGE_ID}`
- claim_boundary(주장 경계): `{BOUNDARY}`

Effect(효과): Stage89(89단계)는 v41 drawdown/OOS early repair(V41 손실률/표본외 초반 수리)를 측정하고, 운영 의미 없이 Stage90(90단계)로 넘긴다.
""")
...
- target_surface(목표 표면): `{TARGET_SURFACE}`
- adapter_under_review(검토 중 어댑터): `{SOURCE_ADAPTER_ID}`
- status(상태): `stage89_closed_{decision}`
- claim_boundary(주장 경계): `{BOUNDARY}`

Stage89(89단계) closed(종료) as v2-native v41 drawdown/OOS early repair batch(브이투 고유 브이41 손실률/표본외 초반 수리 묶음). Effect(효과): legacy 34D(레거시 34D)는 target surface(목표 표면)로만 쓰였고, 다음 연구는 Stage90(90단계)로 이어진다.

## Latest Stage89 Evidence(최신 89단계 근거)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    prepared = {
        ...
        "model_family": "baseline_adapter_stage89_v2_native_v41_drawdown_oos_early_repair_ebm_table",
        "feature_set_id": "stage89_v41_drawdown_oos_early_signal",
