...
RUN_ID = "run316C_review_post_interaction_profit_scale_curve_mt5_probe_v1"
RUN_NUMBER = "run316C"
SOURCE_RUN_ID = "run316B_execute_post_interaction_profit_scale_curve_mt5_probe_v1"
PARENT_RUN_ID = "run316A_design_post_interaction_profit_scale_curve_rebuild_packet_v1"
UPDATED_ON = "2026-05-24"
BOUNDARY = r310.BOUNDARY
NEXT_REBUILD_STAGE_ID = "317_onnx_candidate_campaign__fresh_non_time_profit_source_rebuild"
NEXT_ADAPTER_STAGE_ID = "317_onnx_candidate_campaign__adapter_package_for_post_interaction_profit_scale_curve"

STAGE_ROOT = ROOT / "stages" / STAGE_ID
RUN316B = STAGE_ROOT / "02_runs" / "run316B"
RUN_ROOT = STAGE_ROOT / "02_runs" / RUN_NUMBER
REVIEWS = STAGE_ROOT / "03_reviews"
SELECTED = STAGE_ROOT / "04_selected" / "selection_status.md"
REVIEW_INDEX = REVIEWS / "review_index.md"
STAGE_LEDGER = REVIEWS / "stage_run_ledger.csv"
SOURCE_KPI = RUN316B / "mt5_kpi_summary.csv"
SOURCE_ATTEMPT_SUMMARY = RUN316B / "attempt_summary.csv"
PRODUCER = Path("stage_pipelines/stage316/review_post_interaction_profit_scale_curve_mt5_probe.py")
SCOREBOARD = RUN_ROOT / "post_interaction_profit_scale_curve_review_scoreboard.csv"
...
NEXT_STAGE_QUEUE = RUN_ROOT / "stage317_seed_queue.csv"
RESULT_JUDGMENT = RUN_ROOT / "result_judgment.csv"
GATE_AUDIT = RUN_ROOT / "required_gate_coverage_audit.csv"
RUN_MANIFEST = RUN_ROOT / "run_manifest.json"
LINEAGE = RUN_ROOT / "artifact_lineage_receipt.json"
REPORT = REVIEWS / "run316C_review_stage317_open.md"
DECISION = ROOT / "docs" / "decisions" / "2026-05-24_stage316_post_interaction_profit_scale_curve_review_stage317_open.md"
...
def next_stage_rows(scoreboard: Sequence[Mapping[str, Any]]) -> list[dict[str, Any]]:
    ...
    return [{"seed_id": "stage316_post_interaction_profit_scale_curve_review_seed", "source_candidates": ";".join(row["package_id"] for row in positive[:3]) or "none", "fresh_thesis": "fresh_non_time_profit_source_rebuild", "use_as": "failure_memory_or_candidate_seed", "upside": "20/22시 sell-only(매도 전용) 샘플링이 실패하면 시간 축을 버리고 non-time profit source(비시간 수익 원천)로 이동한다.", "failure_mode": "scale, efficiency, curve, density(규모/효율/곡선/밀도)가 함께 통과하지 못하면 실패다.", "discard_condition": "hour-only repair(시간만 고치는 수리)를 더 반복하지 않는다.", "next_action": "run317A_design_fresh_non_time_profit_source_rebuild_packet"}]


def scaffold_next_stage(selected_rows: Sequence[Mapping[str, Any]]) -> tuple[str, str, str, str]:
    if selected_rows:
        next_stage_id = NEXT_ADAPTER_STAGE_ID
        question = "selected post-interaction profit scale/curve(선택된 상호작용 이후 수익 규모/곡선)를 Adapter package(어댑터 패키지)로 정리할 수 있는가?"
        status = "opened_adapter_package_stage_after_stage316_candidate_selection"
        judgment = "candidate_selected_adapter_package_stage_opened_no_onnx_yet"
        next_action = "run317A_build_post_interaction_profit_scale_curve_adapter_package"
    else:
        next_stage_id = NEXT_REBUILD_STAGE_ID
        question = "시간 기반 수익 원천을 버리고 non-time profit source(비시간 수익 원천)로 ONNX-worthy(온엑스 가치 있음) 후보를 만들 수 있는가?"
        status = "opened_fresh_non_time_profit_source_after_stage316_no_selection"
        judgment = "no_candidate_selected_fresh_non_time_profit_source_stage_opened"
        next_action = "run317A_design_fresh_non_time_profit_source_rebuild_packet"
    next_root = ROOT / "stages" / next_stage_id
    write_text(next_root / "00_spec" / "stage_brief.md", f"# Stage317 Brief(317단계 개요)\n\n- stage_id(단계 ID): `{next_stage_id}`\n- source_stage(원천 단계): `{STAGE_ID}`\n- source_run(원천 실행): `{RUN_ID}`\n- question(질문): {question}\n- boundary(경계): `{BOUNDARY}`\n\nEffect(효과): Stage316(316단계)의 결과를 후보 확정이 아니라 다음 연구 질문의 근거로 넘긴다.")
    write_text(next_root / "04_selected" / "selection_status.md", f"# Stage317 Selection Status(317단계 선택 상태)\n\n- stage_status(단계 상태): `{status}`\n- current_packet(현재 작업 묶음): `{next_stage_id}_v1`\n- current_run(현재 실행): `{RUN_ID}`\n- source_stage(원천 단계): `{STAGE_ID}`\n- selected_candidate(선택 후보): `{selected_rows[0]['package_id'] if selected_rows else 'none'}`\n- Adapter package(어댑터 패키지): `{'pending_adapter_build' if selected_rows else 'none'}`\n- ONNX readiness(온엑스 준비): `not_started`\n- Goal Achieve(목표 달성): `not_claimed`\n- next_action(다음 행동): `{next_action}`\n- stage316_review(316단계 검토): `{rel(REPORT)}`")
    write_text(next_root / "03_reviews" / "review_index.md", f"# Stage317 Review Index(317단계 검토 색인)\n\n- stage316_review(316단계 검토): `{rel(REPORT)}`\n")
    write_csv(next_root / "03_reviews" / "stage_run_ledger.csv", r310.STAGE_LEDGER_COLUMNS, [{"row_id": f"{RUN_ID}__stage317_open", "stage_id": next_stage_id, "run_id": RUN_ID, "view": "stage_open", "tier_scope": "not_applicable", "scoreboard": "handoff", "status": status, "judgment": judgment, "evidence_boundary": "research_development_only", "report_path": rel(REPORT), "notes": f"next_action={next_action}."}])
...
def update_docs(status: str, judgment: str, next_stage_id: str, next_action: str, selected_rows: Sequence[Mapping[str, Any]]) -> None:
    ...
    selected = r310.replace_line(selected, "- Adapter package(", f"- Adapter package(어댑터 패키지): `{'deferred_to_stage317' if selected_rows else 'none'}`")
    selected = r310.replace_line(selected, "- next_action(", f"- next_action(다음 행동): `{next_action}`")
    selected = r310.drop_prefixed_lines(selected, ("- run316C_report(", "- stage317_opened(")) + f"- run316C_report(316C 보고서): `{rel(REPORT)}`\n- stage317_opened(317단계 열림): `{next_stage_id}`\n"
    write_text(SELECTED, selected)
    review_index = r310.drop_prefixed_lines(read_text(REVIEW_INDEX), ("- run316C_report(", "- run316C_scoreboard(", "- stage317_seed_queue(")) + f"- run316C_report(316C 보고서): `{rel(REPORT)}`\n- run316C_scoreboard(316C 점수판): `{rel(SCOREBOARD)}`\n- stage317_seed_queue(317단계 씨앗 대기열): `{rel(NEXT_STAGE_QUEUE)}`\n"
    ...
    current = r310.drop_prefixed_lines(current, ("- run316C_summary(",)).rstrip() + f"\n- run316C_summary(316C 요약): Stage316(316단계) actual MT5(실제 메타트레이더5) 검토를 완료했다. Effect(효과): selected_candidate(선택 후보)는 `{selected_rows[0]['package_id'] if selected_rows else 'none'}`이고 next_stage(다음 단계)는 `{next_stage_id}`다.\n"
    ...
    workspace = r310.prepend_focus(workspace, f"- >-\n  Stage316(316단계) run316C(316C 실행) post interaction profit scale/curve review(상호작용 이후 수익 규모/곡선 검토) `{RUN_ID}` closed Stage316 and opened `{next_stage_id}`. Effect(효과): selected candidate(선택 후보)는 `{selected_rows[0]['package_id'] if selected_rows else 'none'}`이고 Adapter package(어댑터 패키지)는 `{'deferred_to_stage317' if selected_rows else 'none'}`, ONNX readiness(온엑스 준비)는 `not_started`다.\n", RUN_ID)
    write_text(WORKSPACE_STATE, workspace)
    changelog = read_text(CHANGELOG) or "# Changelog(변경 기록)\n"
    if RUN_ID not in changelog:
        changelog += f"\n## {UPDATED_ON} run316C Post interaction profit scale/curve review(316C 상호작용 이후 수익 규모/곡선 검토)\n\n- status(상태): `{status}`\n- judgment(판정): `{judgment}`\n- effect(효과): Stage316(316단계)를 닫고 `{next_stage_id}`를 열었다.\n- boundary(경계): 운영 승격이나 런타임 권위를 주장하지 않는다.\n"
    write_text(CHANGELOG, changelog)


def update_registers(status: str, judgment: str, next_action: str) -> None:
    safe_upsert(RUN_REGISTRY, r310.RUN_REGISTRY_COLUMNS, [{"run_id": RUN_ID, "stage_id": STAGE_ID, "lane": "post_interaction_profit_scale_curve_review", "status": status, "judgment": judgment, "path": rel(REPORT), "notes": f"selected_candidate_reviewed;next_action={next_action}."}], "run_id")
    safe_upsert(ALPHA_LEDGER, ledger.ALPHA_LEDGER_COLUMNS, [{"ledger_row_id": f"{RUN_ID}__review", "stage_id": STAGE_ID, "run_id": RUN_ID, "subrun_id": RUN_NUMBER, "parent_run_id": SOURCE_RUN_ID, "record_view": "post_interaction_profit_scale_curve_review", "tier_scope": "Tier A used/Tier B fallback/actual routed total", "kpi_scope": "trade_quality_curve_profit_scale", "scoreboard_lane": "onnx_candidate_campaign", "status": status, "judgment": judgment, "path": rel(REPORT), "primary_kpi": "actual_mt5_review_completed", "guardrail_kpi": "ONNX=not_started", "external_verification_status": "completed", "notes": f"next_action={next_action}."}], "ledger_row_id")
    safe_upsert(STAGE_LEDGER, r310.STAGE_LEDGER_COLUMNS, [{"row_id": f"{RUN_ID}__review", "stage_id": STAGE_ID, "run_id": RUN_ID, "view": "post_interaction_profit_scale_curve_review", "tier_scope": "Tier A used/Tier B fallback/actual routed total", "scoreboard": "post_interaction_profit_scale_curve_review_scoreboard", "status": status, "judgment": judgment, "evidence_boundary": "runtime_probe_review_no_onnx", "report_path": rel(REPORT), "notes": "Stage317 opened if no selected candidate; ONNX not started."}], "row_id")
...
def update_artifact_registry(paths: Sequence[Path]) -> None:
    ...
    for path in paths:
        ...
        rows.append({"artifact_id": f"{RUN_ID}__{artifact_id}", "artifact_type": "stage316_post_interaction_profit_scale_curve_review_artifact", "path": rel(path), "sha256": sha256_file(path), "stage_id": STAGE_ID, "run_id": RUN_ID, "created_at_utc": "2026-05-24T23:59:00Z", "notes": "Stage316 review and Stage317 open handoff"})
...
def main() -> None:
    ...
    stage317_rows = next_stage_rows(scoreboard)
    next_stage_id, _next_status, _next_judgment, next_action = scaffold_next_stage(selected_rows)
    status = "completed_post_interaction_profit_scale_curve_review_stage317_opened"
    judgment = "actual_mt5_candidate_gate_passed_adapter_stage_opened" if selected_rows else "actual_mt5_no_onnx_worthy_candidate_post_interaction_profit_scale_curve_opened"
    ...
    write_csv(NEXT_STAGE_QUEUE, list(stage317_rows[0].keys()), stage317_rows)
    write_csv(RESULT_JUDGMENT, ("run_id", "status", "judgment", "selected_candidate", "adapter_package", "onnx_readiness", "next_action", "claim_boundary"), [{"run_id": RUN_ID, "status": status, "judgment": judgment, "selected_candidate": selected_rows[0]["package_id"] if selected_rows else "none", "adapter_package": "deferred_to_stage317" if selected_rows else "none", "onnx_readiness": "not_started", "next_action": next_action, "claim_boundary": BOUNDARY}])
    ...
    manifest = {"run_id": RUN_ID, "stage_id": STAGE_ID, "source_run_id": SOURCE_RUN_ID, "status": status, "judgment": judgment, "selected_candidate": selected_rows[0]["package_id"] if selected_rows else "none", "adapter_package": "deferred_to_stage317" if selected_rows else "none", "onnx_readiness": "not_started", "goal_achieve": "not_claimed", "next_stage_id": next_stage_id, "next_action": next_action, "artifacts": [rel(path) for path in (SCOREBOARD, TRADE_QUALITY, REPORT_SOURCE_RECEIPT, CURVE, FAILURE_MEMORY, SELECTED_QUEUE, NEXT_STAGE_QUEUE, RESULT_JUDGMENT, GATE_AUDIT, REPORT, DECISION)], "claim_boundary": BOUNDARY}
    ...
    print(json.dumps({"status": status, "judgment": judgment, "scoreboard_rows": len(scoreboard), "failure_rows": len(failure_rows), "selected_candidate": selected_rows[0]["package_id"] if selected_rows else "none", "adapter_package": "deferred_to_stage317" if selected_rows else "none", "onnx_readiness": "not_started", "goal_achieve": "not_claimed", "next_stage_id": next_stage_id, "next_action": next_action}, ensure_ascii=False, indent=2, sort_keys=True))
