...
STAGE_ID = "81_adapter_research__v41_early_oos_segment_repair"
RUN_NUMBER = "run81A"
RUN_ID = "run81A_stage81_v41_early_oos_segment_repair_v1"
PACKET_ID = "stage81_v41_early_oos_segment_repair_v1"
PARENT_RUN_ID = "run80A_stage80_v41_atr_stop_followup_review_v1"
SOURCE_STAGE80_ID = "80_adapter_research__v41_atr_stop_followup_review"
SOURCE_STAGE80_CLOSEOUT_COMMIT = "0006a61af9ce3a343f5a6be318310f09a85440a6"
SOURCE_STAGE80_LATEST_COMMIT = "eeba12d2075fe29028d1ee746fc71fb886ca7168"
...
NEXT_STAGE_ID = "82_adapter_research__v41_early_oos_followup_review"
NEXT_RUN_ID = "run82A_stage82_v41_early_oos_followup_review_v1"
NEXT_PACKET_ID = "stage82_v41_early_oos_followup_review_v1"
...
COMMON_ROOT = f"OPV2/s81a/{RUN_NUMBER}"

SUMMARY_JSON_PATH = REVIEWS_ROOT / "stage81_v41_early_oos_segment_summary.json"
SUMMARY_CSV_PATH = REVIEWS_ROOT / "stage81_v41_early_oos_segment_summary.csv"
REPORT_PATH = REVIEWS_ROOT / "stage81_v41_early_oos_segment_report.md"
SEGMENT_KPI_PATH = REVIEWS_ROOT / "stage81_segment_kpi_summary.csv"
RISK_ATR_TELEMETRY_PATH = REVIEWS_ROOT / "stage81_risk_atr_telemetry.csv"
GATE_FEATURE_SUMMARY_PATH = REVIEWS_ROOT / "stage81_gate_feature_summary.csv"
TIER_B_DIAGNOSTIC_PATH = REVIEWS_ROOT / "stage81_tier_b_diagnostic_summary.csv"
DECISION_PATH = REVIEWS_ROOT / "stage81_decision.md"
AUDIT_CSV_PATH = REVIEWS_ROOT / "stage81_trade_audit.csv"
...
VARIANTS = (
    repair.RepairVariant(
        adapter_id="s81_v41_h3_risk5_gate08_sl20_tp40_cd12",
        label="run50BN_v41_h3_risk5_gate08_sl20_tp40_cd12",
        atr_enabled=True,
        ...
        max_hold_bars=3,
        notes="Stage81 early OOS repair: Stage79 TP4.0/SL2.0 surface with cooldown extended to 12 bars.",
    ),
    repair.RepairVariant(
        adapter_id="s81_v41_h3_risk5_gate08_sl20_tp40_h2",
        label="run50BN_v41_h3_risk5_gate08_sl20_tp40_h2",
        atr_enabled=True,
        model_risk_enabled=True,
        fixed_lot=0.25,
        atr_stop_multiplier=2.0,
        ...
        close_only_on_opposite_signal=True,
        max_hold_bars=2,
        notes="Stage81 early OOS repair: Stage79 TP4.0/SL2.0 surface with max hold shortened to 2 bars.",
    ),
    repair.RepairVariant(
        adapter_id="s81_v41_h3_risk5_gate08_sl225_tp40",
        label="run50BN_v41_h3_risk5_gate08_sl225_tp40",
        atr_enabled=True,
        model_risk_enabled=True,
        fixed_lot=0.25,
        ...
        close_only_on_opposite_signal=True,
        max_hold_bars=3,
        notes="Stage81 early OOS repair: Stage79 TP4.0 surface with SL widened to 2.25 to test early OOS stopout sensitivity.",
    ),
)
...
CONTEXT_GATE_SPECS = {
    variant.adapter_id: {
        "gate_column": "stage81_gate_margin_lt_008_short",
        "gate_type": "margin",
        "threshold": 0.08,
        "block_mode": "short",
...
def decide(summary_rows: Sequence[Mapping[str, Any]], segment_rows: Sequence[Mapping[str, Any]], external: str) -> str:
    if external != "completed":
        return "continue_stage81_due_to_incomplete_runtime"
    ...
    if val_net >= 1000 and oos_net >= 520 and val_dd <= 21.0 and oos_dd <= 20.5 and val_pf >= 1.45 and oos_pf >= 1.38 and not reasons:
        return "proceed_to_stage82_candidate_review"
    if val_net >= 900 and oos_net >= 450 and val_dd <= 24.0 and oos_dd <= 22.0 and val_pf >= 1.35 and oos_pf >= 1.25:
        return "continue_early_oos_segment_review_in_stage82"
    if val_pf < 1.0 or val_net <= 0:
        return "open_new_model_source_branch_in_stage82"
    return "continue_early_oos_segment_review_in_stage82"
...
def report_markdown(
    ...
    return f"""# Stage81 V41 Early OOS Segment Repair Report(81단계 V41 표본외 초반 구간 수리 보고서)

- run(실행): `{RUN_ID}`
- target_surface(목표 표면): `{TARGET_SURFACE}`
- source_stage(원천 단계): `{SOURCE_STAGE80_ID}`
- source_stage80_closeout_commit(원천 80단계 종료 커밋): `{SOURCE_STAGE80_CLOSEOUT_COMMIT}`
- source_stage80_latest_commit(원천 80단계 최신 커밋): `{SOURCE_STAGE80_LATEST_COMMIT}`
...
def decision_markdown(decision: str, external: str) -> str:
    return f"""# Stage81 Decision(81단계 판정)

decision(판정): `{decision}`

Stage81(81단계)는 Stage79(79단계)의 강한 TP4/SL2(익절 4, 손절 2) 표면에서 cooldown(재진입 냉각), max hold(최대 보유), SL sensitivity(손절 민감도)를 좁게 바꿔 OOS early(표본외 초반) 약점 수리 가능성을 측정했다.
            ...
            rows.append(
                {
                    ...
                    "split": split_name,
                    "tier_b_policy": "diagnostic_missing_required_but_disabled_for_this_v41_early_oos_segment_repair",
                    ...
                    "reason": "Stage81 isolates v41 early OOS segment repair first; Tier B fallback remains diagnostic and disabled.",
                }
            )
    return rows
            ...
            rows.append(
                {
                    ...
                    "artifact_type": "stage81_v41_early_oos_segment_repair_evidence",
                    "path": rel(path),
                    "sha256": sha256_file_lf_normalized(path),
                    "stage_id": STAGE_ID,
                    "run_id": RUN_ID,
                    "created_at_utc": created,
                    "notes": "Stage81 v2-native v41 early OOS segment repair artifact.",
                }
            )
            ...
            rows.append(
                {
                    ...
                    "run_id": RUN_ID,
                    "created_at_utc": created,
                    "notes": "Actual Stage81 MT5 Strategy Tester HTML report.",
                }
            )
    return rows
...
def write_run_identity(result: Mapping[str, Any]) -> None:
    write_json(
        RUN_ROOT / "run_manifest.json",
        {
            ...
            "source_stage80_closeout_commit": SOURCE_STAGE80_CLOSEOUT_COMMIT,
            "source_stage80_latest_commit": SOURCE_STAGE80_LATEST_COMMIT,
        ...
        [
            {
                "run_id": RUN_ID,
                "stage_id": STAGE_ID,
                "lane": "baseline_adapter_v2_native_v41_early_oos_segment_repair",
                "status": status,
                "judgment": decision,
        ...
        alpha_rows = [
            {
                ...
                "tier_scope": "Tier A+B",
                "kpi_scope": "stage81_v41_early_oos_segment_repair",
                ...
                "external_verification_status": external,
                "notes": "Stage81 run materialized or blocked before KPI records were available.",
            }
        ]
...
Stage82(82단계)는 Stage81(81단계)의 v41 early OOS segment repair(V41 표본외 초반 구간 수리) 결과를 받아 candidate review(후보 검토), continued repair(계속 수리), 또는 another model source(다른 모델 원천)를 고르는 follow-up(후속) 단계다.

## Bounded Question(경계 질문)

Stage81(81단계)의 cooldown/max-hold/SL sensitivity(재진입 냉각/최대 보유/손절 민감도) 수리가 Stage79(79단계)의 net(순손익)을 보존하면서 OOS early(표본외 초반) 음수 구간과 DD(손실률)를 낮출 만큼 강한가?

Effect(효과): Stage82(82단계)는 다음 판단을 하나의 경계 질문으로 좁힌다.
...
""",
    ...
    write_md(
        ...
        f"""# Stage82 Input References(82단계 입력 참조)

- source_stage(원천 단계): `{STAGE_ID}`
- source_run(원천 실행): `{RUN_ID}`
- source_decision(원천 판정): `{decision}`
- source_external_verification_status(원천 외부 검증 상태): `{external}`
- stage81_report(81단계 보고서): `{rel(REPORT_PATH)}`
- stage81_decision(81단계 판정): `{rel(DECISION_PATH)}`
- summary(요약): `{rel(SUMMARY_CSV_PATH)}`
- target_surface(목표 표면): `{TARGET_SURFACE}`

Effect(효과): Stage82(82단계)는 v2 고유 근거만 이어받아 34D KPI(34D 핵심 성과 지표) 격차 축소를 계속한다.
""",
    )
    write_md(NEXT_STAGE_ROOT / "03_reviews" / "review_index.md", f"""# Stage82 Review Index(82단계 검토 색인)
...
Effect(효과): Stage82(82단계)는 Stage81(81단계) closeout(종료 기록)을 이어받아 다음 bounded batch(경계 묶음 실행)만 검토한다.
""")
    write_md(NEXT_STAGE_ROOT / "04_selected" / "selection_status.md", f"""# Stage82 Selection Status(82단계 선택 상태)
...
Effect(효과): Stage82(82단계)는 34D KPI(34D 핵심 성과 지표) 격차 축소를 계속하지만, 운영 의미 없이 연구개발로만 이어진다.
""")
...
- >-
  Stage81(81단계) closed(종료) as `{decision}` and Stage82(82단계) `{NEXT_STAGE_ID}` is open_planned(열림 계획). Effect(효과): v41 early OOS segment repair(V41 표본외 초반 구간 수리) 근거를 보존하고 다음 경계 연구 질문으로만 넘긴다.
- >-
  Stage81 result(81단계 결과): validation/OOS(검증/표본외), PF/net/DD(수익 팩터/순손익/손실률), risk/ATR telemetry(위험/ATR 텔레메트리)는 `{rel(SUMMARY_CSV_PATH)}`와 `{rel(RISK_ATR_TELEMETRY_PATH)}`에 기록된다. Effect(효과): 34D target surface(34D 목표 표면) 대비 KPI(핵심 성과 지표) 차이를 다음 단계 입력으로 보존한다.
- >-
...
stage81_v41_early_oos_segment_repair:
  packet_id: {PACKET_ID}
  stage_id: {STAGE_ID}
  ...
  boundary: {BOUNDARY}
"""
    if "stage81_v41_early_oos_segment_repair:" in text:
        text = re.sub(r"\nstage81_v41_early_oos_segment_repair:\n(?:  .*\n)+", block + "\n", text, count=1)
    else:
        text = text.rstrip() + block
    io_path(WORKSPACE_STATE_PATH).write_text(text + ("\n" if not text.endswith("\n") else ""), encoding="utf-8-sig")
    write_md(SELECTED_ROOT / "selection_status.md", f"""# Stage81 Selection Status(81단계 선택 상태)

- stage_status(단계 상태): `closed_{decision}`
- source_stage(원천 단계): `{SOURCE_STAGE80_ID}`
- source_decision(원천 판정): `continue_early_oos_segment_repair_in_stage81`
- source_stage80_closeout_commit(원천 80단계 종료 커밋): `{SOURCE_STAGE80_CLOSEOUT_COMMIT}`
- source_stage80_latest_commit(원천 80단계 최신 커밋): `{SOURCE_STAGE80_LATEST_COMMIT}`
- source_stage79_latest_commit(원천 79단계 최신 커밋): `{SOURCE_STAGE79_LATEST_COMMIT}`
- source_stage73_latest_commit(원천 73단계 최신 커밋): `{SOURCE_STAGE73_LATEST_COMMIT}`
...
- target_surface(목표 표면): `{TARGET_SURFACE}`
- stage81_decision(81단계 판정): `{decision}`
- next_stage_or_branch(다음 단계/분기): `{NEXT_STAGE_ID}`
- claim_boundary(주장 경계): `{BOUNDARY}`

Effect(효과): Stage81(81단계)는 v41 early OOS segment repair(V41 표본외 초반 구간 수리)를 측정하고, 운영 의미 없이 Stage82(82단계)로 넘긴다.
""")
...
- target_surface(목표 표면): `{TARGET_SURFACE}`
- adapter_under_review(검토 중 어댑터): `{SOURCE_ADAPTER_ID}`
- status(상태): `stage81_closed_{decision}`
- claim_boundary(주장 경계): `{BOUNDARY}`

Stage81(81단계) closed(종료) as v2-native v41 early OOS segment repair batch(브이투 고유 브이41 표본외 초반 구간 수리 묶음). Effect(효과): legacy 34D(레거시 34D)는 target surface(목표 표면)로만 쓰였고, 다음 연구는 Stage82(82단계)로 이어진다.

## Latest Stage81 Evidence(최신 81단계 근거)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    prepared = {
        ...
        "model_family": "baseline_adapter_stage81_v2_native_v41_early_oos_segment_repair_ebm_table",
        "feature_set_id": "stage81_v41_early_oos_segment_signal",
