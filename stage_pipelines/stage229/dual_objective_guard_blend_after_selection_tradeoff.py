...
from stage_pipelines.stage227 import selection_structure_repair_after_threshold_axis_no_effect as prior  # noqa: E402
...
STAGE_ID = "229_adapter_research__dual_objective_guard_blend_after_selection_tradeoff"
RUN_NUMBER = "run229A"
RUN_ID = "run229A_stage229_dual_objective_guard_blend_after_selection_tradeoff_v1"
PACKET_ID = "stage229_dual_objective_guard_blend_after_selection_tradeoff_v1"
PARENT_RUN_ID = "run228A_stage228_stage227_selection_structure_followup_review_v1"
SOURCE_STAGE_ID = "228_adapter_research__stage227_selection_structure_followup_review"
SOURCE_RUN_ID = PARENT_RUN_ID
SOURCE_STAGE228_EVIDENCE_COMMIT = "cdb3022287a76f8130a580d389f9dcce11dfef7e"
SOURCE_STAGE228_HASH_RECORD_COMMIT = "9057f767ae2ea8b8a59fd7f7110d87ff3cf1cb68"
SOURCE_STAGE227_RUN_ID = "run227A_stage227_selection_structure_repair_after_threshold_axis_no_effect_v1"
NEXT_STAGE_ID = "230_adapter_research__stage229_guard_blend_followup_review"
NEXT_RUN_ID = "run230A_stage230_stage229_guard_blend_followup_review_v1"
NEXT_PACKET_ID = "stage230_stage229_guard_blend_followup_review_v1"
...
COMMON_ROOT = f"OPV2/s229a/{RUN_NUMBER}"

SUMMARY_JSON_PATH = REVIEWS_ROOT / "stage229_summary.json"
SUMMARY_CSV_PATH = REVIEWS_ROOT / "stage229_guard_blend_kpi_summary.csv"
REPORT_PATH = REVIEWS_ROOT / "stage229_guard_blend_report.md"
SEGMENT_KPI_PATH = REVIEWS_ROOT / "stage229_segment_kpi_summary.csv"
BALANCE_CURVE_AUDIT_PATH = REVIEWS_ROOT / "stage229_balance_curve_audit.csv"
MONTHLY_KPI_PATH = REVIEWS_ROOT / "stage229_monthly_kpi_summary.csv"
CONCENTRATION_PATH = REVIEWS_ROOT / "stage229_concentration_risk_summary.csv"
DRAWDOWN_PATH = REVIEWS_ROOT / "stage229_drawdown_recovery_summary.csv"
QUALITY_MATRIX_PATH = REVIEWS_ROOT / "stage229_quality_matrix.csv"
RISK_ATR_TELEMETRY_PATH = REVIEWS_ROOT / "stage229_risk_atr_telemetry.csv"
GATE_FEATURE_SUMMARY_PATH = REVIEWS_ROOT / "stage229_gate_feature_summary.csv"
PROBABILITY_BINDING_PATH = REVIEWS_ROOT / "stage229_probability_telemetry_summary.csv"
MODEL_SCORE_AUDIT_PATH = REVIEWS_ROOT / "stage229_model_score_audit.csv"
TIER_B_DIAGNOSTIC_PATH = REVIEWS_ROOT / "stage229_tier_b_diagnostic_summary.csv"
DECISION_PATH = REVIEWS_ROOT / "stage229_decision.md"
AUDIT_CSV_PATH = REVIEWS_ROOT / "stage229_trade_audit.csv"
...
PRODUCER_PATH = Path("stage_pipelines/stage229/dual_objective_guard_blend_after_selection_tradeoff.py")
ARTIFACT_COLUMNS = prior.ARTIFACT_COLUMNS

VARIANTS = (
    repair.RepairVariant(
        adapter_id="s229_blend_session_only_ref",
        label="stage229_blend_session_only_ref",
        atr_enabled=True,
        ...
        max_hold_bars=3,
        notes="Stage229 control: session-only OOS-preserving guard.",
    ),
    repair.RepairVariant(
        adapter_id="s229_blend_wide_margin_band",
        label="stage229_blend_wide_margin_band",
        atr_enabled=True,
        model_risk_enabled=True,
        fixed_lot=0.25,
        atr_stop_multiplier=2.0325,
        ...
        close_only_on_opposite_signal=False,
        max_hold_bars=3,
        notes="Stage229 blend: release only session longs outside the wide margin band.",
    ),
    repair.RepairVariant(
        adapter_id="s229_blend_base_margin_band",
        label="stage229_blend_base_margin_band",
        atr_enabled=True,
        model_risk_enabled=True,
        fixed_lot=0.25,
        atr_stop_multiplier=2.0325,
        ...
        close_only_on_opposite_signal=False,
        max_hold_bars=3,
        notes="Stage229 comparison: Stage227 session-and-margin base band.",
    ),
    repair.RepairVariant(
        adapter_id="s229_blend_tight_margin_band",
        label="stage229_blend_tight_margin_band",
        atr_enabled=True,
        model_risk_enabled=True,
        fixed_lot=0.25,
        atr_stop_multiplier=2.0325,
        ...
        close_only_on_opposite_signal=False,
        max_hold_bars=3,
        notes="Stage229 blend: release more session longs by blocking only the tight margin band.",
    ),
)

BASE_EXTRA = {**prior.BASE_EXTRA, "block_mode": "encoded_side_context"}
VARIANT_EXTRAS: dict[str, dict[str, Any]] = {
    "s229_blend_session_only_ref": {
        **BASE_EXTRA,
        "short_block_rule": "midwide_lowedge",
        "long_block_rule": "session_only",
        "axis": "session_only_ref",
    },
    "s229_blend_wide_margin_band": {
        **BASE_EXTRA,
        "short_block_rule": "midwide_lowedge",
        "long_block_rule": "session_and_wide_margin",
        "axis": "wide_margin_band",
    },
    "s229_blend_base_margin_band": {
        **BASE_EXTRA,
        "short_block_rule": "midwide_lowedge",
        "long_block_rule": "session_and_base_margin",
        "axis": "base_margin_band",
    },
    "s229_blend_tight_margin_band": {
        **BASE_EXTRA,
        "short_block_rule": "midwide_lowedge",
        "long_block_rule": "session_and_tight_margin",
        "axis": "tight_margin_band",
    },
}
...
def margin_hit(row: Mapping[str, str], spec: Mapping[str, Any], prefix: str) -> bool:
    ...
    return float(spec[f"{key_prefix}margin_min"]) <= margin <= float(spec[f"{key_prefix}margin_max"])


def session_hit(row: Mapping[str, str], spec: Mapping[str, Any]) -> bool:
    minutes = s174.s167_minutes_for(row)
    return minutes is not None and float(spec["session_min"]) <= minutes <= float(spec["session_max"])
...
def gate_value(row: Mapping[str, str], variant: repair.RepairVariant) -> float:
    ...
    if signal > 0:
        ...
        if rule == "session_and_wide_margin" and session and margin_hit(row, spec, "wide"):
            return 2.0
        if rule == "session_and_base_margin" and session and margin_hit(row, spec, ""):
            return 2.0
        if rule == "session_and_tight_margin" and session and margin_hit(row, spec, "tight"):
            return 2.0
    if signal < 0:
...
def apply_stage_values(context_specs: Mapping[str, Any]) -> None:
    values = {
        ...
        "SOURCE_ADAPTER_ID": "stage229_guard_blend",
    ...
    for variant_index, variant in enumerate(VARIANTS, start=1):
        ...
        for split in ("validation_is", "oos"):
            ...
            for role_index, (tier, attempt_role, prefix, attempt_token) in enumerate(
                ...
                magic = 22910000 + variant_index * 100 + (1 if split == "validation_is" else 50) + role_index
                attempts.append(
                    s161.base.attempt_payload(
                        run_root=variant_root,
                        run_id=RUN_ID,
                        stage_number=229,
                        exploration_label="stage229_BaselineAdapter__DualObjectiveGuardBlendAfterSelectionTradeoff",
...
def configure_runner() -> None:
    ...
    for variant in VARIANTS:
        ...
        context_specs[variant.adapter_id] = {
            "gate_column": f"stage229_gate_{extra['axis']}",
            "gate_type": "encoded_stage229_dual_objective_guard_blend",
            ...
            "description": "Stage229 encoded gate: dual-objective session/margin guard blend.",
        }
...
def pass_stage229(row: Mapping[str, Any]) -> bool:
    return (
...
def decide(rows: Sequence[Mapping[str, Any]], external: str) -> str:
    if external != "completed":
        return "continue_stage229_runtime_completion_due_to_incomplete_runtime_candidate_not_final"
    if any(pass_stage229(row) for row in rows):
        return "open_stage230_stage229_guard_blend_followup_review_candidate_not_final"
    return "open_stage230_bounded_followup_due_to_guard_blend_tradeoff_candidate_not_final"
...
def report_markdown(rows: Sequence[Mapping[str, Any]], decision: str, external: str) -> str:
    best = best_row(rows)
    return f"""# Stage229 Dual-Objective Guard Blend Report(229단계 이중목표 보호 혼합 보고서)

- stage(단계): `{STAGE_ID}`
- run(실행): `{RUN_ID}`
- source_stage(원천 단계): `{SOURCE_STAGE_ID}`
- source_run(원천 실행): `{SOURCE_RUN_ID}`
- source_stage228_evidence_commit(원천 228단계 근거 커밋): `{SOURCE_STAGE228_EVIDENCE_COMMIT}`
- source_stage228_hash_record_commit(원천 228단계 해시 기록 커밋): `{SOURCE_STAGE228_HASH_RECORD_COMMIT}`
...
- comparison_baseline(비교 기준): Stage227 session-only(세션 전용) OOS net(표본외 순손익) `{SESSION_ONLY_REFERENCE["oos_net"]}`, Stage227 session-and-margin(세션+마진) validation net(검증 순손익) `{SESSION_AND_MARGIN_CLUE["validation_net"]}`.
...
def decision_markdown(decision: str, external: str) -> str:
    return f"""# Stage229 Decision(229단계 판정)
...
Stage229(229단계) closeout(종료)은 overall goal complete(전체 목표 완료)가 아니다.

Effect(효과): Stage230(230단계) follow-up review(후속 검토)에서 guard blend(보호 혼합)의 KPI(핵심 성과 지표) 상충과 다음 bounded repair(경계 수리) 축을 분리 판정한다.
"""
...
def artifact_rows(result: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    for row in rows:
        if row.get("artifact_type") != "mt5_strategy_tester_report":
            row["artifact_type"] = "stage229_evidence"
            row["notes"] = "Stage229 dual-objective guard blend evidence."
        else:
            row["notes"] = "Stage229 MT5 Strategy Tester report."
    return rows
        ...
        [
            {
                "run_id": RUN_ID,
                "stage_id": STAGE_ID,
                "lane": "baseline_adapter_stage229_dual_objective_guard_blend",
                "status": status,
                "judgment": decision,
                "path": rel(DECISION_PATH),
                "notes": s172.ledger_pairs(
                    (
                        ("source_stage228_evidence_commit", SOURCE_STAGE228_EVIDENCE_COMMIT),
                        ("source_stage228_hash_record_commit", SOURCE_STAGE228_HASH_RECORD_COMMIT),
                        ("session_only_reference", SESSION_ONLY_REFERENCE["adapter_id"]),
                        ("session_and_margin_clue", SESSION_AND_MARGIN_CLUE["adapter_id"]),
                        ("target_surface", TARGET_SURFACE),
...
def write_packet_files(result: Mapping[str, Any], decision: str, ledger_payload: Mapping[str, Any], quality: Sequence[Mapping[str, Any]]) -> None:
    ...
    s172.write_md(
        ...
        f"""# Stage229 Closeout Packet(229단계 종료 작업 묶음)
...
Stage230(230단계)는 Stage229(229단계) guard blend(보호 혼합) 결과를 follow-up review(후속 검토)하는 bounded review(경계 검토) 단계다.

## Bounded Question(경계 질문)

Did Stage229(229단계) recover validation net(검증 순손익), early PF(초반 수익요인), mid PF(중반 수익요인), and drawdown(낙폭) while preserving session-only(세션 전용) OOS net(표본외 순손익), OOS PF(표본외 수익요인), model-controlled risk%(모델 제어 위험 비율), and ATR/bracket behavior(ATR/브래킷 동작)?

Effect(효과): Stage229(229단계) 안에서 다음 수리를 흡수하지 않고 KPI(핵심 성과 지표) 상충을 별도 검토로 닫는다.
...
- status(상태): `open_planned_from_stage229`
- source_stage(원천 단계): `{STAGE_ID}`
- source_decision(원천 판정): `{decision}`
...
- stage_status(단계 상태): `open_planned_from_stage229`
...
- >-
  Stage229(229단계) closed(종료) as `{decision}` and Stage230(230단계) `{NEXT_STAGE_ID}` is open_planned(개방 계획). Effect(효과): dual-objective guard blend(이중목표 보호 혼합)의 KPI(핵심 성과 지표) 상충을 별도 review(검토)로 판정한다.
- >-
  Stage229 evidence(229단계 근거)는 `{rel(REPORT_PATH)}`, `{rel(SUMMARY_CSV_PATH)}`, `{rel(QUALITY_MATRIX_PATH)}`, `{rel(MONTHLY_KPI_PATH)}`, `{rel(CONCENTRATION_PATH)}`, `{rel(RISK_ATR_TELEMETRY_PATH)}`에 있다. Effect(효과): margin band(마진 구간) 폭이 validation/OOS(검증/표본외)를 동시에 살리는지 확인한다.
- >-
...
"""
    if re.search(r"(?ms)^current_focus:\r?\n.*?(?=\r?\nstage\d+_)", state):
        ...
        state = state.rstrip() + "\n" + focus
    state = re.sub(r"(?ms)^stage229_dual_objective_guard_blend_after_selection_tradeoff:\r?\n.*?(?=^stage\d+_|\Z)", "", state)
    block = f"""
stage229_dual_objective_guard_blend_after_selection_tradeoff:
  packet_id: {PACKET_ID}
  stage_id: {STAGE_ID}
...
- status(상태): `stage229_{decision}`
- claim_boundary(주장 경계): `{BOUNDARY}`

Stage229(229단계)는 dual-objective guard blend(이중목표 보호 혼합)를 MT5 Strategy Tester(MetaTrader 5 전략 테스터)로 측정했다. Effect(효과): Stage230(230단계)가 KPI(핵심 성과 지표) 상충을 별도 review(검토)로 판정한다.

## Latest Stage229 Evidence(최신 229단계 근거)
...
def append_changelog(decision: str) -> None:
    ...
    if RUN_ID in existing:
        return
    entry = (
        f"\n## {s172.utc_now()} Stage229 dual-objective guard blend closeout(229단계 이중목표 보호 혼합 종료)\n\n"
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    prepared = {
        ...
        "model_family": "baseline_adapter_stage229_v2_native_dual_objective_guard_blend",
        "feature_set_id": "stage229_signal_plus_session_margin_guard_blend",
    ...
    s172.write_json(
        SUMMARY_JSON_PATH,
        {
            ...
            "session_and_margin_clue": SESSION_AND_MARGIN_CLUE,
            "claim_boundary": BOUNDARY,
            "overall_goal_complete": False,
            "source_stage228_evidence_commit": SOURCE_STAGE228_EVIDENCE_COMMIT,
            "source_stage228_hash_record_commit": SOURCE_STAGE228_HASH_RECORD_COMMIT,
