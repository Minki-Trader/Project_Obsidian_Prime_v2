...
STAGE_ID = "95_adapter_research__v41_oos_early_entry_gate_repair"
RUN_NUMBER = "run95A"
RUN_ID = "run95A_stage95_v41_oos_early_entry_gate_repair_v1"
PACKET_ID = "stage95_v41_oos_early_entry_gate_repair_v1"
PARENT_RUN_ID = "run94A_stage94_v41_sl210_oos_early_followup_review_v1"
SOURCE_STAGE94_ID = "94_adapter_research__v41_sl210_oos_early_followup_review"
SOURCE_STAGE94_CLOSEOUT_COMMIT = "26a803de31c2b993f29617b7eabb0ff200192c59"
SOURCE_STAGE94_LATEST_COMMIT = "a1e776b2d2575df89b7ed2e707418d39cf191eb0"
...
NEXT_STAGE_ID = "96_adapter_research__v41_oos_early_entry_gate_followup_review"
NEXT_RUN_ID = "run96A_stage96_v41_oos_early_entry_gate_followup_review_v1"
NEXT_PACKET_ID = "stage96_v41_oos_early_entry_gate_followup_review_v1"
...
COMMON_ROOT = f"OPV2/s95a/{RUN_NUMBER}"

SUMMARY_JSON_PATH = REVIEWS_ROOT / "stage95_v41_oos_early_entry_gate_repair_summary.json"
SUMMARY_CSV_PATH = REVIEWS_ROOT / "stage95_v41_oos_early_entry_gate_repair_summary.csv"
REPORT_PATH = REVIEWS_ROOT / "stage95_v41_oos_early_entry_gate_repair_report.md"
SEGMENT_KPI_PATH = REVIEWS_ROOT / "stage95_segment_kpi_summary.csv"
RISK_ATR_TELEMETRY_PATH = REVIEWS_ROOT / "stage95_risk_atr_telemetry.csv"
GATE_FEATURE_SUMMARY_PATH = REVIEWS_ROOT / "stage95_gate_feature_summary.csv"
TIER_B_DIAGNOSTIC_PATH = REVIEWS_ROOT / "stage95_tier_b_diagnostic_summary.csv"
DECISION_PATH = REVIEWS_ROOT / "stage95_decision.md"
AUDIT_CSV_PATH = REVIEWS_ROOT / "stage95_trade_audit.csv"
...
VARIANTS = (
    repair.RepairVariant(
        adapter_id="s95_v41_h3_risk475_gate09_sl2075_tp40_cd10",
        label="run50BN_v41_h3_risk475_gate09_sl2075_tp40_cd10",
        atr_enabled=True,
        ...
        max_hold_bars=3,
        notes="Stage95 OOS early entry gate repair: keep SL2.075 TP4.0 and tighten short margin gate to 0.09.",
    ),
    repair.RepairVariant(
        adapter_id="s95_v41_h3_risk475_gate10_sl2075_tp40_cd10",
        label="run50BN_v41_h3_risk475_gate10_sl2075_tp40_cd10",
        atr_enabled=True,
        model_risk_enabled=True,
        fixed_lot=0.25,
        atr_stop_multiplier=2.075,
        ...
        close_only_on_opposite_signal=True,
        max_hold_bars=3,
        notes="Stage95 OOS early entry gate repair: keep SL2.075 TP4.0 and tighten short margin gate to 0.10.",
    ),
    repair.RepairVariant(
        adapter_id="s95_v41_h3_risk475_gate08_sl2075_tp40_thr056_cd10",
        label="run50BN_v41_h3_risk475_gate08_sl2075_tp40_thr056_cd10",
        atr_enabled=True,
        model_risk_enabled=True,
        fixed_lot=0.25,
        atr_stop_multiplier=2.075,
        ...
        close_only_on_opposite_signal=True,
        max_hold_bars=3,
        notes="Stage95 OOS early entry gate repair: keep gate08 SL2.075 TP4.0 and raise direction threshold to 0.56.",
    ),
)
...
CONTEXT_GATE_THRESHOLDS = {
    "s95_v41_h3_risk475_gate09_sl2075_tp40_cd10": 0.09,
    "s95_v41_h3_risk475_gate10_sl2075_tp40_cd10": 0.10,
    "s95_v41_h3_risk475_gate08_sl2075_tp40_thr056_cd10": 0.08,
}

CONTEXT_GATE_SPECS = {
    variant.adapter_id: {
        "gate_column": f"stage95_gate_margin_lt_{int(CONTEXT_GATE_THRESHOLDS[variant.adapter_id] * 100):03d}_short",
        "gate_type": "margin",
        "threshold": CONTEXT_GATE_THRESHOLDS[variant.adapter_id],
        "block_mode": "short",
        "description": f"v41 source; block shorts if et40_decision_margin < {CONTEXT_GATE_THRESHOLDS[variant.adapter_id]:.2f}; {variant.label}",
    }
    for variant in VARIANTS
}
...
def decide(summary_rows: Sequence[Mapping[str, Any]], segment_rows: Sequence[Mapping[str, Any]], external: str) -> str:
    if external != "completed":
        return "continue_stage95_due_to_incomplete_runtime"
    ...
    if val_net >= 920 and oos_net >= 580 and val_dd <= 22.5 and oos_dd <= 20.5 and val_pf >= 1.50 and oos_pf >= 1.54 and not reasons:
        return "proceed_to_stage96_oos_early_entry_gate_followup_review"
    if val_net >= 850 and oos_net >= 540 and val_dd <= 23.5 and oos_dd <= 21.0 and val_pf >= 1.45 and oos_pf >= 1.48:
        return "continue_oos_early_entry_gate_followup_review_in_stage96"
    if val_pf < 1.0 or val_net <= 0:
        return "open_new_model_source_branch_in_stage96"
    return "continue_oos_early_entry_gate_followup_review_in_stage96"
...
def report_markdown(
    ...
    return f"""# Stage95 V41 OOS Early Entry Gate Repair Report(95단계 V41 표본외 초반 진입 게이트 수리 보고서)

- run(실행): `{RUN_ID}`
- target_surface(목표 표면): `{TARGET_SURFACE}`
- source_stage(원천 단계): `{SOURCE_STAGE94_ID}`
- source_stage94_closeout_commit(원천 94단계 종료 커밋): `{SOURCE_STAGE94_CLOSEOUT_COMMIT}`
- source_stage94_latest_commit(원천 94단계 최신 커밋): `{SOURCE_STAGE94_LATEST_COMMIT}`
- source_stage93_closeout_commit(원천 93단계 종료 커밋): `{SOURCE_STAGE93_CLOSEOUT_COMMIT}`
- source_stage93_latest_commit(원천 93단계 최신 커밋): `{SOURCE_STAGE93_LATEST_COMMIT}`
...
def decision_markdown(decision: str, external: str) -> str:
    return f"""# Stage95 Decision(95단계 판정)

decision(판정): `{decision}`

Stage95(95단계)는 Stage94(94단계) 판정에 따라 Stage93 best(93단계 최선안)의 entry gate/confidence threshold(진입 게이트/신뢰도 문턱)를 좁게 수리했다.
            ...
            rows.append(
                {
                    ...
                    "split": split_name,
                    "tier_b_policy": "diagnostic_missing_required_but_disabled_for_this_v41_oos_early_entry_gate_repair",
                    ...
                    "reason": "Stage95 isolates v41 OOS early entry gate repair first; Tier B fallback remains diagnostic and disabled.",
                }
            )
    return rows
            ...
            rows.append(
                {
                    ...
                    "artifact_type": "stage95_v41_oos_early_entry_gate_repair_evidence",
                    "path": rel(path),
                    "sha256": sha256_file_lf_normalized(path),
                    "stage_id": STAGE_ID,
                    "run_id": RUN_ID,
                    "created_at_utc": created,
                    "notes": "Stage95 v2-native v41 OOS early entry gate repair artifact.",
                }
            )
            ...
            rows.append(
                {
                    ...
                    "run_id": RUN_ID,
                    "created_at_utc": created,
                    "notes": "Actual Stage95 MT5 Strategy Tester HTML report.",
                }
            )
    return rows
...
def write_run_identity(result: Mapping[str, Any]) -> None:
    write_json(
        RUN_ROOT / "run_manifest.json",
        {
            ...
            "source_stage94_closeout_commit": SOURCE_STAGE94_CLOSEOUT_COMMIT,
            "source_stage94_latest_commit": SOURCE_STAGE94_LATEST_COMMIT,
        ...
        [
            {
                "run_id": RUN_ID,
                "stage_id": STAGE_ID,
                "lane": "baseline_adapter_v2_native_v41_oos_early_entry_gate_repair",
                "status": status,
                "judgment": decision,
        ...
        alpha_rows = [
            {
                ...
                "tier_scope": "Tier A+B",
                "kpi_scope": "stage95_v41_oos_early_entry_gate_repair",
                ...
                "external_verification_status": external,
                "notes": "Stage95 run materialized or blocked before KPI records were available.",
            }
        ]
...
def write_packet_files(result: Mapping[str, Any], decision: str, ledger_payload: Mapping[str, Any]) -> None:
    ...
    write_json(PACKET_ROOT / "aggregate_summary.json", {"packet_id": PACKET_ID, "stage_id": STAGE_ID, "run_id": RUN_ID, "decision": decision, "source_stage94_closeout_commit": SOURCE_STAGE94_CLOSEOUT_COMMIT, "source_stage94_latest_commit": SOURCE_STAGE94_LATEST_COMMIT, "source_stage93_closeout_commit": SOURCE_STAGE93_CLOSEOUT_COMMIT, "source_stage93_latest_commit": SOURCE_STAGE93_LATEST_COMMIT, "gate_feature_summary_path": rel(GATE_FEATURE_SUMMARY_PATH), "ledger_payload": ledger_payload, "overall_goal_complete": False})
...
Stage96(96단계)는 Stage95(95단계)의 OOS early entry gate repair(표본외 초반 진입 게이트 수리) 결과를 review gate(검토 게이트)로만 판독하는 follow-up(후속) 단계다.
...
Effect(효과): Stage96(96단계)는 Stage95(95단계)의 결과를 과장하지 않고 다음 판단으로만 좁힌다.
...
""",
    ...
    write_md(
        ...
        f"""# Stage96 Input References(96단계 입력 참조)

- source_stage(원천 단계): `{STAGE_ID}`
- source_run(원천 실행): `{RUN_ID}`
- source_decision(원천 판정): `{decision}`
- source_external_verification_status(원천 외부 검증 상태): `{external}`
- stage95_report(95단계 보고서): `{rel(REPORT_PATH)}`
- stage95_decision(95단계 판정): `{rel(DECISION_PATH)}`
- summary(요약): `{rel(SUMMARY_CSV_PATH)}`
- target_surface(목표 표면): `{TARGET_SURFACE}`

Effect(효과): Stage96(96단계)는 v2 고유 근거만 이어받아 34D KPI(34D 핵심 성과 지표) 격차 축소를 계속한다.
""",
    )
    write_md(NEXT_STAGE_ROOT / "03_reviews" / "review_index.md", f"""# Stage96 Review Index(96단계 검토 색인)
...
Effect(효과): Stage96(96단계)는 Stage95(95단계) closeout(종료 기록)을 이어받아 다음 bounded review(경계 검토)만 수행한다.
""")
    write_md(NEXT_STAGE_ROOT / "04_selected" / "selection_status.md", f"""# Stage96 Selection Status(96단계 선택 상태)
...
Effect(효과): Stage96(96단계)는 34D KPI(34D 핵심 성과 지표) 격차 축소를 계속하지만, 운영 의미 없이 연구개발로만 이어진다.
""")
...
- >-
  Stage95(95단계) closed(종료) as `{decision}` and Stage96(96단계) `{NEXT_STAGE_ID}` is open_planned(열림 계획). Effect(효과): v41 OOS early entry gate repair(V41 표본외 초반 진입 게이트 수리) 근거를 보존하고 다음 경계 연구 질문으로만 넘긴다.
- >-
  Stage95 result(95단계 결과): validation/OOS(검증/표본외), PF/net/DD(수익 팩터/순손익/손실률), risk/ATR telemetry(위험/ATR 텔레메트리)는 `{rel(SUMMARY_CSV_PATH)}`와 `{rel(RISK_ATR_TELEMETRY_PATH)}`에 기록된다. Effect(효과): 34D target surface(34D 목표 표면) 대비 KPI(핵심 성과 지표) 차이를 다음 단계 입력으로 보존한다.
- >-
...
stage95_v41_oos_early_entry_gate_repair:
  packet_id: {PACKET_ID}
  stage_id: {STAGE_ID}
  ...
  boundary: {BOUNDARY}
"""
    if "stage95_v41_oos_early_entry_gate_repair:" in text:
        text = re.sub(r"\nstage95_v41_oos_early_entry_gate_repair:\n(?:  .*\n)+", block + "\n", text, count=1)
    else:
        text = text.rstrip() + block
    io_path(WORKSPACE_STATE_PATH).write_text(text + ("\n" if not text.endswith("\n") else ""), encoding="utf-8-sig")
    write_md(SELECTED_ROOT / "selection_status.md", f"""# Stage95 Selection Status(95단계 선택 상태)

- stage_status(단계 상태): `closed_{decision}`
- source_stage(원천 단계): `{SOURCE_STAGE94_ID}`
- source_decision(원천 판정): `continue_oos_early_entry_gate_repair_in_stage95`
- source_stage94_closeout_commit(원천 94단계 종료 커밋): `{SOURCE_STAGE94_CLOSEOUT_COMMIT}`
- source_stage94_latest_commit(원천 94단계 최신 커밋): `{SOURCE_STAGE94_LATEST_COMMIT}`
- source_stage93_closeout_commit(원천 93단계 종료 커밋): `{SOURCE_STAGE93_CLOSEOUT_COMMIT}`
- source_stage93_latest_commit(원천 93단계 최신 커밋): `{SOURCE_STAGE93_LATEST_COMMIT}`
...
- target_surface(목표 표면): `{TARGET_SURFACE}`
- stage95_decision(95단계 판정): `{decision}`
- next_stage_or_branch(다음 단계/분기): `{NEXT_STAGE_ID}`
- claim_boundary(주장 경계): `{BOUNDARY}`

Effect(효과): Stage95(95단계)는 v41 OOS early entry gate repair(V41 표본외 초반 진입 게이트 수리)를 측정하고, 운영 의미 없이 Stage96(96단계)로 넘긴다.
""")
...
- target_surface(목표 표면): `{TARGET_SURFACE}`
- adapter_under_review(검토 중 어댑터): `{SOURCE_ADAPTER_ID}`
- status(상태): `stage95_closed_{decision}`
- claim_boundary(주장 경계): `{BOUNDARY}`

Stage95(95단계) closed(종료) as v2-native v41 OOS early entry gate repair batch(브이투 고유 브이41 표본외 초반 진입 게이트 수리 묶음). Effect(효과): legacy 34D(레거시 34D)는 target surface(목표 표면)로만 쓰였고, 다음 연구는 Stage96(96단계)로 이어진다.

## Latest Stage95 Evidence(최신 95단계 근거)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    prepared = {
        ...
        "model_family": "baseline_adapter_stage95_v2_native_v41_oos_early_entry_gate_repair_ebm_table",
        "feature_set_id": "stage95_v41_oos_early_entry_gate_signal",
