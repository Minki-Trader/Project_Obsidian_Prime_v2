...
STAGE_ID = "stage_frontier_48__short_pf_edge_event_rarity_risk_sizing_after_f47_state_budget_memory"
PREV_STAGE_ID = "stage_frontier_47__short_pf_edge_sequence_state_risk_budget_after_f46_sequence_context_memory"
RUN_A = "frontier48A_stage_open_short_pf_edge_event_rarity_risk_sizing_hypothesis_design_v1"
RUN_B = "frontier48B_train_only_nonpercentile_event_rarity_risk_sizing_proxy_v1"
RUN_C = "frontier48C_capped_nonpercentile_state_gate_repair_v1"
RUN_D = "frontier48D_stage_closeout_event_rarity_risk_sizing_v1"
NEXT_STAGE_ID = "stage_frontier_49__short_pf_edge_forward_floor_state_machine_after_f48_event_risk_memory"
NEXT_RUN_ID = "frontier49A_stage_open_short_pf_edge_forward_floor_state_machine_hypothesis_design_v1"
RUNTIME_REVIEW_RUN_ID = "frontier48E_pre_expensive_runtime_validation_review_v1"
...
GROK_OPEN_ROOT = Path("docs") / "agent_control" / "grok_reviews" / "2026-06-15_frontier48_stage_open" / "small_review"
GROK_CLOSE_ROOT = Path("docs") / "agent_control" / "grok_reviews" / "2026-06-15_frontier48_stage_closeout" / "small_review"
...
def main() -> None:
    ...
    write_json(INPUT_ROOT / "event_rarity_risk_sizing_manifest.json", manifest)
    ...
    write_json(
        RUN_D_ROOT / "run_manifest.json",
        {
            ...
            "profile": "short_event_rarity_risk_sizing_proxy",
            "open_review": open_review,
            "closeout_review": closeout_review,
            "repair_decision": repair_decision,
            "closeout": closeout,
...
def load_open_grok_review() -> dict[str, Any]:
    ...
    return {
        ...
        "classification": "accepted_stage_open_train_split_only_nonpercentile_state_gate_lock" if accepted and guardrail_seen and claim_ok and not rejected else "needs_local_verification",
...
def load_closeout_grok_review() -> dict[str, Any]:
    ...
    return {
        ...
        "metadata": metadata,
        "clean_output_path": clean_path.as_posix(),
        "classification": "accepted_closeout_event_rarity_risk_sizing_boundary" if accepted and boundary_ok and not rejected else "needs_local_verification",
...
def context_checks(
    ...
    checks = {
        "workspace_points_to_f48": f"next_stage_id: {STAGE_ID}" in workspace or f"current_stage_id: {STAGE_ID}" in workspace,
    ...
    if not all(checks.values()):
        raise RuntimeError(f"Frontier48 context check failed: {json.dumps(checks, ensure_ascii=False)}")
    return {
        "checks": checks,
...
def build_input_manifest(
    ...
    return {
        "stage_id": STAGE_ID,
        "hypothesis": "train_only_nonpercentile_state_gate_and_risk_sizing_on_f47_event_risk_memory",
        "idea_id": "IDEA-FR48-SHORT-EVENT-RARITY-RISK-SIZING",
        "decision_use": "scout_seed_runtime_candidate_screening_only",
        "comparison_baseline": "F47 best and preserved nonwinner rows are reference-only clues, not inherited baseline",
        "control_variables": {
            "symbol": "US100",
            "timeframe": "M5",
            "side": SIDE_LABEL,
        ...
        "changed_variables": {
            "sequence_context": "same lagged frozen event score rolls and horizon-embargoed past outcome tape as reference scaffold",
            "state_gate": "fixed non-percentile squeeze, cooldown, bad-event-rate, volatility, and high-count gates",
            "risk_sizing": "train-only first-hit SLTP caps with fixed score/event scaffold and non-percentile state admission",
            "novelty_line": "state gates use fixed entry-known thresholds instead of another percentile risk-budget sweep",
            "model_family": "class_weighted_sequence_context_classifier_with_state_budget_overlay",
            "threshold_policy": "train_split_event_score_threshold_only; state gates are fixed absolute thresholds; validation_oos_read_only",
        },
        "sample_scope": {
            "rows": int(len(frame)),
        ...
        "evidence_plan": [
            ...
            "event_rarity_risk_sizing_model_audit",
            "candidate_ledger",
            "split_metrics",
            "candidate_summary",
            "stage_run_ledger",
    ...
    rows: list[dict[str, Any]] = [
        {
            ...
            "gate_variant": "f47_reference_event_model_context_score_sltp",
            "threshold_name": "archived_f47_scaffold",
...
def risk_budget_specs(profile: str) -> list[dict[str, Any]]:
    specs = [
        {
            "risk_budget_variant": "state_gate_squeeze_off_bad_fast_le1_vol5_le1p5",
            "risk_budget_definition": "fixed non-percentile gate: no squeeze, fast bad-event rate <= 1.0, realized vol expansion <= 1.5",
            "equals_features": {
                "bb_squeeze": 0.0,
            },
            "max_fixed_features": {
                "seq_past_bad_event_rate_fast": 1.0,
                "historical_vol_5_over_20": 1.5,
            },
            "min_fixed_features": {},
        },
        {
            "risk_budget_variant": "state_gate_squeeze_off_cooldown2_atr_le1p5",
            "risk_budget_definition": "fixed non-percentile gate: no squeeze, two-bar cooldown after high score, ATR ratio <= 1.5",
            "equals_features": {
                "bb_squeeze": 0.0,
            },
            "max_fixed_features": {
                "atr_14_over_atr_50": 1.5,
            },
            "min_fixed_features": {
                "seq_bars_since_high_score": 2.0,
            },
        },
        {
            "risk_budget_variant": "state_gate_cooldown3_bad_slow_le0p875_vol5_le1p5",
            "risk_budget_definition": "fixed non-percentile gate: three-bar cooldown, slow bad-event rate <= 0.875, realized vol expansion <= 1.5",
            "equals_features": {},
            "max_fixed_features": {
                "seq_past_bad_event_rate_slow": 0.875,
                "historical_vol_5_over_20": 1.5,
            },
            "min_fixed_features": {
        ...
        {
            "risk_budget_variant": "state_gate_high_count_le12_bad_fast_le0p875",
            "risk_budget_definition": "fixed non-percentile gate: high-score crowding <= 12 and fast bad-event rate <= 0.875",
            "equals_features": {},
            "max_fixed_features": {
                "seq_high_count_fast": 12.0,
                "seq_past_bad_event_rate_fast": 0.875,
            },
            "min_fixed_features": {},
        },
    ]
    if profile == "repair":
        specs.extend(
            [
                {
                    "risk_budget_variant": "repair_state_gate_squeeze_off_high_count_le12_cooldown2",
                    "risk_budget_definition": "fixed repair gate: no squeeze, high-score crowding <= 12, two-bar cooldown",
                    "equals_features": {
                        "bb_squeeze": 0.0,
                    },
                    "max_fixed_features": {
                ...
                {
                    "risk_budget_variant": "repair_state_gate_vol5_le1p75_bad_fast_le1",
                    "risk_budget_definition": "fixed repair gate: realized vol expansion <= 1.75 and fast bad-event rate <= 1.0",
                    "equals_features": {},
                    "max_fixed_features": {
                ...
                {
                    "risk_budget_variant": "repair_state_gate_cooldown5_bad_slow_le0p75_atr_le1p25",
                    "risk_budget_definition": "fixed repair gate: five-bar cooldown, slow bad-event rate <= 0.75, ATR ratio <= 1.25",
                    "equals_features": {},
                    "max_fixed_features": {
                        "seq_past_bad_event_rate_slow": 0.75,
                        "atr_14_over_atr_50": 1.25,
                    },
                    "min_fixed_features": {
                        "seq_bars_since_high_score": 5.0,
                    },
                },
                {
                    "risk_budget_variant": "repair_state_gate_squeeze_off_vol_atr_le1p75",
                    "risk_budget_definition": "fixed repair gate: no squeeze, realized vol expansion <= 1.75, ATR ratio <= 1.75",
                    "equals_features": {
                        "bb_squeeze": 0.0,
                    },
                    "max_fixed_features": {
...
def build_event_surface(
    ...
    *,
    ...
    summary = summarize_f48_candidates(split_metrics, selected)
    return {
...
def build_sequence_context(
    *,
    ...
    data = {
        ...
        "seq_past_event_rate_fast": known_event.rolling(fast, min_periods=max(2, fast // 2)).mean(),
        "seq_past_event_rate_slow": known_event.rolling(slow, min_periods=max(3, slow // 2)).mean(),
        "seq_past_bad_event_rate_fast": known_bad_event.rolling(fast, min_periods=max(2, fast // 2)).mean(),
        "seq_past_bad_event_rate_slow": known_bad_event.rolling(slow, min_periods=max(3, slow // 2)).mean(),
...
def candidates_for_mask(
    ...
    condition = {
        ...
        "feature_family": "train_only_event_rarity_risk_sizing_score",
    ...
    for row in threshold_rows(
        ...
        take_quantiles,
        rr_floors,
        f"f48_{profile}_train_only_event_rarity_risk_sizing_sl_tp_caps",
    ):
        ...
        candidate["f48_train_selection_score"] = score_value
        candidate["selection_rank_basis"] = "train_only_event_event_rarity_risk_sizing_no_validation_oos_feedback"
        variants.append(candidate)
    variants.sort(key=lambda item: float(item["f48_train_selection_score"]), reverse=True)
    return variants[:2]
...
def rank_candidates(candidates: list[dict[str, Any]], prefix: str, limit: int) -> list[dict[str, Any]]:
    ...
    for candidate in sorted(candidates, key=lambda item: float(item["f48_train_selection_score"]), reverse=True):
        key = (
...
def summarize_f48_candidates(split_metrics: pd.DataFrame, candidates: list[dict[str, Any]]) -> pd.DataFrame:
    ...
    metadata = {
        str(item["candidate_id"]): {
            ...
            "f48_train_selection_score": item.get("f48_train_selection_score", ""),
    ...
    summary["f48_scout_clue_flag"] = dual & density_scout & (f_min_pf >= SCOUT_MIN_PF) & (f_max_dd <= SCOUT_MAX_DD)
    summary["f48_seed_surface_flag"] = dual & density_seed & (f_min_pf >= SEED_MIN_PF) & (f_max_dd <= SEED_MAX_DD)
    summary["runtime_probe_candidate_flag"] = (
        summary["f48_seed_surface_flag"].astype(bool) & (f_min_pf >= RUNTIME_MIN_PF) & (f_max_dd <= RUNTIME_MAX_DD)
    )
    summary["f48_axis_gap_to_seed"] = (
...
def surface_counts(summary: pd.DataFrame) -> dict[str, Any]:
    ...
    return {
        "scout_clue_rows": int(summary["f48_scout_clue_flag"].sum()),
        "seed_surface_rows": int(summary["f48_seed_surface_flag"].sum()),
...
def closest_nonwinner_forward_observation(combined: pd.DataFrame, best: dict[str, Any]) -> dict[str, Any]:
    ...
    frame["__seed_gap"] = numeric_column("f48_axis_gap_to_seed", 999999.0)
...
def build_repair_decision(initial_summary: pd.DataFrame) -> dict[str, Any]:
    if initial_summary.empty:
        return {
            ...
            "repair_reason": "Initial event rarity risk sizing surface produced no candidate summary.",
        }
    runtime = int(initial_summary["runtime_probe_candidate_flag"].sum())
    seed = int(initial_summary["f48_seed_surface_flag"].sum())
    scout_rows = int(initial_summary["f48_scout_clue_flag"].sum())
    if runtime:
        return {
    ...
    return {
        "run_repair_grid": True,
        "repair_action": "run_capped_nonpercentile_state_gate_repair",
        "repair_reason": f"Initial event rarity risk sizing surface scout={scout_rows}, seed=0, runtime=0; run bounded fixed state-gate repair.",
...
def classify_closeout(initial_summary: pd.DataFrame, repair_summary: pd.DataFrame) -> dict[str, Any]:
    ...
    if combined.empty:
        ...
        scout_rows = int(combined["f48_scout_clue_flag"].sum())
        seed_rows = int(combined["f48_seed_surface_flag"].sum())
    ...
    if runtime_rows:
        ...
        runtime_status = "runtime_probe_out_of_scope_by_claim_seed_only_no_runtime_candidate_after_f48_event_risk_sizing_proxy"
        next_stage = NEXT_STAGE_ID
        next_run = NEXT_RUN_ID
    elif scout_rows:
        closeout_class = "preserved_clue_negative_memory"
        runtime_status = "runtime_probe_ineligible_no_seed_or_runtime_candidate_after_f48_event_risk_sizing_proxy"
        next_stage = NEXT_STAGE_ID
        next_run = NEXT_RUN_ID
    else:
        closeout_class = "negative_memory"
        runtime_status = "runtime_probe_ineligible_no_scout_seed_or_runtime_candidate_after_f48_event_risk_sizing_proxy"
        next_stage = NEXT_STAGE_ID
        next_run = NEXT_RUN_ID
    return {
...
def build_closeout_prompt(closeout: dict[str, Any], best_rows: list[dict[str, Any]], repair_decision: dict[str, Any]) -> str:
    ...
    compact_rows = "\n".join(
        (
            ...
            f"fwd_dd={row.get('forward_max_dd')}; scout={row.get('f48_scout_clue_flag')}; "
            f"seed={row.get('f48_seed_surface_flag')}; runtime={row.get('runtime_probe_candidate_flag')}"
        )
        for row in best_rows[:6]
    )
    return f"""# Frontier48 closeout Grok review(그록 마감 검토)
...
- state_gate_variant(상태 게이트 변형): {best.get("risk_budget_variant")}
- state_gate_keep_rate(상태 게이트 유지율): {best.get("risk_budget_train_keep_rate")}
- state_gate_block_rate(상태 게이트 차단율): {best.get("risk_budget_train_block_rate")}
...
- scout/seed/runtime(탐색/씨앗/런타임): {best.get("f48_scout_clue_flag")}/{best.get("f48_seed_surface_flag")}/{best.get("runtime_probe_candidate_flag")}
...
F48 opened train-only event rarity risk sizing(학습 전용 이벤트 희소성/위험 크기 조절) hypothesis(가설). F47 is reference only(참조 전용), not baseline/winner(기준선/승자 아님).
"""
...
- scope_completion_gate(범위 완료 게이트): pass(통과), F48 hypothesis/proxy/repair/closeout(가설/프록시/수리/마감) materialized.
...
- data_integrity(데이터 무결성): pass(통과), closed-bar feature order(닫힌 봉 피처 순서), train-only frozen base scorer(학습 전용 고정 기본 채점기), lagged score context(지연 점수 문맥), horizon+1 embargo(예측수평+1 유예), and fixed non-percentile state gates(고정 비분위수 상태 게이트) verified.
...
- final_codex_direction(최종 코덱스 방향): run F48 event rarity risk sizing proxy with validation/OOS read-only(검증/표본외 읽기 전용)
"""
...
- final_codex_direction(최종 코덱스 방향): close F48 as `{closeout.get("closeout_class")}` with no authority claim(권위 주장 없음)
"""
    return {
...
F48 negative memory(부정 기억)는 train-only event rarity risk sizing(학습 전용 이벤트 희소성/위험 크기 조절)이 seed/runtime(씨앗/런타임) 후보를 만들었는지 여부와 반복 금지 경계를 기록한다.
    ...
    rows = [
        {
            ...
            "notes": "F48 opened with train-only event rarity risk sizing hypothesis and Grok guardrails.",
        },
        {
            "stage_id": STAGE_ID,
            "run_id": RUN_B,
            ...
            "notes": "Initial train-only event rarity risk sizing proxy surface.",
        },
        {
            "stage_id": STAGE_ID,
            "run_id": RUN_C,
            ...
            "runtime_probe_status": "evaluated_for_runtime_candidate",
            "notes": "Capped event rarity risk sizing repair diagnostic.",
        },
        {
            "stage_id": STAGE_ID,
            "run_id": RUN_D,
        ...
        },
        {
            ...
            "closeout_class": closeout.get("closeout_class"),
            "runtime_probe_status": "out_of_scope_by_claim_tier_a_event_risk_sizing_proxy_only",
            "notes": "F48 used Tier A event rarity risk sizing proxy only; Tier B not claimed.",
        },
        {
            "stage_id": STAGE_ID,
            "run_id": RUN_D,
...
def project_ledger_row(row: dict[str, Any], fields: list[str]) -> dict[str, Any]:
    ...
    values = {
        ...
        "kpi_scope": "short_event_rarity_risk_sizing_proxy",
        ...
        "run_family": "frontier_short_event_rarity_risk_sizing_proxy",
...
current_judgment: {closeout.get("closeout_class")}(F48 train-only event rarity risk sizing proxy no operating authority)
...
Frontier48(F48, 전선 48단계)가 `{closeout.get("closeout_class")}`로 닫혔다.
...
F48 carry-forward(이월) 기록은 train-only event rarity risk sizing(학습 전용 이벤트 희소성/위험 크기 조절)이 PF/DD/density(수익 팩터/손실폭/밀도)를 얼마나 바꾸었는지와 seed/runtime(씨앗/런타임) 후보가 생겼는지 여부다.
"""
...
def artifact_map() -> dict[str, str]:
    return {
        "input_manifest": (INPUT_ROOT / "event_rarity_risk_sizing_manifest.json").as_posix(),
