...
from stage_pipelines.stage309 import execute_split_coherent_profit_curve_source_mt5_probe as e309  # noqa: E402


STAGE_ID = "310_onnx_candidate_campaign__runtime_positive_fragment_allocation_rebuild"
RUN_ID = "run310B_execute_runtime_positive_fragment_allocation_mt5_probe_v1"
RUN_NUMBER = "run310B"
SOURCE_RUN_ID = "run310A_design_runtime_positive_fragment_allocation_rebuild_packet_v1"
PARENT_RUN_ID = "run309C_review_split_coherent_profit_curve_source_mt5_probe_v1"
STATUS_PREPARED = "prepared_runtime_positive_fragment_allocation_mt5_probe_no_runtime_kpi"
...
EXPLORATION_LABEL = "stage310_Model__RuntimePositiveFragmentAllocationReplay"
SIGNAL_COLUMN = "run310b_route_signal"
FEATURE_ORDER = (SIGNAL_COLUMN,)
COMMON_ROOT = "Project_Obsidian_Prime_v2/stage310/run310B_runtime_positive_fragment_allocation"

STAGE_ROOT = ROOT / "stages" / STAGE_ID
RUN310A = STAGE_ROOT / "02_runs" / "run310A"
...
MT5_QUEUE = RUN310A / "mt5_probe_queue.csv"
SOURCE_MANIFEST = RUN310A / "candidate_payload_manifest.csv"
SOURCE_RUN_MANIFEST = RUN310A / "run_manifest.json"
SOURCE_REPORT = REVIEWS / "run310A_materialization.md"
PRODUCER = Path("stage_pipelines/stage310/execute_runtime_positive_fragment_allocation_mt5_probe.py")
...
REPORT = REVIEWS / "run310B_mt5_probe.md"
...
def safe_upsert_rows(path: Path, columns: Sequence[str], rows: Sequence[Mapping[str, Any]], key: str) -> None:
    e309.safe_upsert_rows(path, columns, rows, key)


def extra_set_values(queue_row: Mapping[str, str]) -> dict[str, Any]:
    return e309.extra_set_values(queue_row)
...
def attach_identity(attempt: dict[str, Any], queue_row: Mapping[str, str], max_hold: int, close_on_flat: bool, same_reentry: int) -> None:
    ...
    attempt["stage310_branch_id"] = queue_row.get("materialized_branch_id", "")
    attempt["stage309_branch_id"] = queue_row.get("stage309_branch_id", "")
    attempt["stage308_branch_id"] = queue_row.get("stage308_branch_id", "")
    ...
    for queue_row in queue_rows:
        ...
        for runtime_split, split_token in (("validation_is", "val"), ("oos", "oos")):
            for tier_key, tier_label, tier_token in (
                ...
                attempt["signal_policy"] = "stage310 runtime positive fragment allocation route_signal_value(경로 신호) -1/0/+1 single-feature table(단일 피처 표)"
            ...
            if include_routed:
                ...
                attach_identity(attempt, queue_row, max_hold, close_on_flat, same_reentry)
                attempt["signal_policy"] = "Tier A primary + Tier B fallback(티어 A 우선 + 티어 B 대체) stage310 runtime allocation route_signal_value(경로 신호)"
                attempts.append(attempt)
    return attempts
...
def classify_status(result: Mapping[str, Any], materialize_only: bool) -> tuple[str, str, str, str]:
    ...
    if materialize_only:
        return (
            STATUS_PREPARED,
            "runtime_positive_fragment_allocation_runtime_probe_prepared_no_external_execution",
            "out_of_scope_by_claim_materialize_only",
            "run310B_execute_runtime_positive_fragment_allocation_mt5_probe_external_check",
        )
    ...
    if planned and completed_exec >= planned and len(kpis) >= planned:
        return (
            "completed_runtime_positive_fragment_allocation_mt5_probe_no_selection",
            "runtime_probe_completed_requires_profit_scale_curve_review_no_selection",
            "completed",
            "run310C_review_runtime_positive_fragment_allocation_mt5_probe",
        )
    if kpis:
        return (
            "partial_runtime_positive_fragment_allocation_mt5_probe_no_selection",
            "runtime_probe_partial_requires_continuation_or_review_no_selection",
            "partial_or_blocked",
            "run310B_continue_runtime_positive_fragment_allocation_mt5_probe" if limited else "run310C_review_with_runtime_gaps",
        )
    return (
        "blocked_runtime_positive_fragment_allocation_mt5_probe_no_kpi",
        "runtime_probe_blocked_no_kpi_no_selection",
        "blocked_or_invalid",
        "run310B_repair_or_block_runtime_positive_fragment_allocation_mt5_probe",
    )
...
def report_markdown(result: Mapping[str, Any], status: str, judgment: str, external_status: str, next_action: str) -> str:
    ...
    return "\n".join(
        [
            "# run310B Runtime Positive Fragment Allocation MT5 Probe(310B 런타임 양수 조각 배분 MT5 탐침)",
            "",
            ...
            "",
            "Effect(효과): run310A(310A 실행)의 allocation route_signal_value(배분 경로 신호)를 MT5 tester(MT5 테스터)에 넣어 trade count(거래수), net profit(순수익), PF(수익 팩터), DD(drawdown, 손실폭), recovery(회복)를 확인했다.",
            "",
            f"`{BOUNDARY}`",
        ]
    )
...
def upsert_ledgers(result: Mapping[str, Any], status: str, judgment: str, external_status: str, next_action: str) -> None:
    ...
    safe_upsert_rows(
        ...
        [{"run_id": RUN_ID, "stage_id": STAGE_ID, "lane": "runtime_positive_fragment_allocation_mt5_probe", "status": status, "judgment": judgment, "path": runner.prev.e295.e294.base.base.rel(REPORT), "notes": f"attempts={attempt_count};mt5_kpi_records={kpi_count};selected_candidate=none;onnx_readiness=not_claimed;next_action={next_action}."}],
        key="run_id",
    )
    safe_upsert_rows(
        ALPHA_LEDGER,
        ...
        [
            {
                ...
                "record_view": "runtime_positive_fragment_allocation_mt5_probe",
    ...
    rows = [
        {
            ...
            "artifact_type": "stage310_runtime_positive_fragment_allocation_mt5_artifact",
            "path": runner.prev.e295.e294.base.base.rel(path),
            "sha256": runner.prev.e295.e294.base.base.sha256_file_lf_normalized(path),
            "stage_id": STAGE_ID,
            "run_id": RUN_ID,
            "created_at_utc": created_at,
            "notes": "run310B runtime positive fragment allocation MT5 probe",
        }
        for path in paths
...
def update_docs(status: str, judgment: str, next_action: str, kpi_count: int, attempt_count: int) -> None:
    ...
    selected = base.append_once(selected, "run310B_report", f"- run310B_report(310B 보고서): `{base.rel(REPORT)}`")
    selected = base.append_once(selected, "run310B_execution_result", f"- run310B_execution_result(310B 실행 결과): `{base.rel(EXECUTION_RESULT)}`")
    e309.e308.safe_write_md(SELECTED, selected)

    review_index = base.io_path(REVIEW_INDEX).read_text(encoding="utf-8-sig") if base.path_exists(REVIEW_INDEX) else "# Stage310 Review Index(310단계 검토 색인)\n"
    review_index = base.append_once(
        review_index,
        "run310B_report",
        f"- run310B_report(310B 보고서): `{base.rel(REPORT)}`\n- run310B_execution_result(310B 실행 결과): `{base.rel(EXECUTION_RESULT)}`\n- run310B_mt5_kpi_summary(310B MT5 KPI 요약): `{base.rel(MT5_KPI_SUMMARY)}`",
    )
    e309.e308.safe_write_md(REVIEW_INDEX, review_index)
    ...
    current = base.append_once(
        current,
        "run310B_summary",
        f"- run310B_summary(310B 요약): runtime positive fragment allocation MT5 probe(런타임 양수 조각 배분 MT5 탐침)를 실행했다. Effect(효과): attempts(시도) `{attempt_count}`개와 MT5 KPI records(MT5 KPI 기록) `{kpi_count}`개를 만들었고 선택 후보/Adapter(어댑터)/ONNX(온엑스)는 주장하지 않는다.",
    )
    e309.e308.safe_write_md(CURRENT_STATE, current)
    ...
    focus = (
        f"- >-\n"
        f"  Stage310(310단계) run310B(310B 실행) runtime positive fragment allocation MT5 probe(런타임 양수 조각 배분 MT5 탐침) `{RUN_ID}`. "
    ...
    e309.e308.safe_write_md(WORKSPACE_STATE, workspace)
    ...
    changelog = base.append_once(
        changelog,
        RUN_ID,
        f"## {UPDATED_ON} run310B Runtime positive fragment allocation MT5 probe(310B 런타임 양수 조각 배분 MT5 탐침)\n\n"
        f"- status(상태): `{status}`\n"
        f"- judgment(판정): `{judgment}`\n"
    ...
    e309.e308.safe_write_md(CHANGELOG, changelog)
