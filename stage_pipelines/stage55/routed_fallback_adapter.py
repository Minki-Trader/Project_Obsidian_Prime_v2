...
STAGE_NUMBER = 55
STAGE_ID = "55_adapter_routing__tier_b_fallback_side_filter_router"
IDEA_ID = "IDEA-ST55-TIER-B-FALLBACK-SIDE-FILTER-ROUTER"
RUN_ID = "run49A_tier_b_fallback_side_filter_router_v1"
RUN_DIR_NAME = "run49A"
PACKET_ID = "stage55_run49A_tier_b_fallback_side_filter_router_v1"
BOUNDARY = (
    "stage55_tier_b_fallback_router_runtime_probe_only_no_baseline_no_promotion_"
...
SOURCE_STAGE_ID = "54_adapter_signal__cost_aware_side_permission_filter"
SOURCE_RUN_ID = "run48A_cost_aware_side_permission_filter_v1"
...
COMMON_RUN_ROOT = "Project_Obsidian_Prime_v2/stage55/run49A"
...
MT5_SUMMARY_PATH = RESULTS_ROOT / "routed_fallback_mt5_summary.csv"
...
SIDE_FILTER_CANDIDATES: tuple[dict[str, str], ...] = (
    {
        "adapter_id": "csp00_stage51_control",
        "description": "no additional tier-b fallback side-filter routing filter",
    },
    {
        "adapter_id": "csp01_stage53_candidate_retest",
        "description": "retest the Stage53 user-review candidate by blocking long entries during early session or ADX greater than 25",
    },
    {
        "adapter_id": "csp02_no_early_longs_no_lowadx_shorts",
        "description": "block early-session long entries and block short entries when ADX is below 20",
    },
    {
        "adapter_id": "csp03_midlate_longs_strong_shorts",
        "description": "block early/trend long entries and allow shorts only in late, trend, or DI-short-strong context",
    },
    {
        "adapter_id": "csp04_late_longs_no_volmid_shorts",
        "description": "allow longs only during late session and block shorts in vol-mid or low-ADX context",
    },
    {
        "adapter_id": "csp05_short_only_trend_or_di",
        "description": "block all longs and allow shorts only when ADX is greater than 25 or DI-short-strong context is present",
    },
)

ROUTING_CANDIDATES: tuple[dict[str, str], ...] = (
    {
        "adapter_id": "rfp00_csp03_primary_control_fallback",
        "primary_adapter_id": "csp03_midlate_longs_strong_shorts",
        "fallback_adapter_id": "csp00_stage51_control",
        "description": "use the Stage54 user-review primary filter on Tier A and an unfiltered Tier B fallback",
    },
    {
        "adapter_id": "rfp01_csp03_primary_csp03_fallback",
        "primary_adapter_id": "csp03_midlate_longs_strong_shorts",
        "fallback_adapter_id": "csp03_midlate_longs_strong_shorts",
        "description": "use the Stage54 user-review filter on both Tier A primary and Tier B fallback",
    },
    {
        "adapter_id": "rfp02_csp03_primary_csp05_fallback",
        "primary_adapter_id": "csp03_midlate_longs_strong_shorts",
        "fallback_adapter_id": "csp05_short_only_trend_or_di",
        "description": "use the Stage54 user-review primary filter and allow Tier B fallback only in strong short contexts",
    },
    {
        "adapter_id": "rfp03_csp02_primary_csp03_fallback",
        "primary_adapter_id": "csp02_no_early_longs_no_lowadx_shorts",
        "fallback_adapter_id": "csp03_midlate_longs_strong_shorts",
        "description": "use a broader Tier A permission filter while keeping the Stage54 filter on Tier B fallback",
    },
)
...
def routing_candidate_by_id(adapter_id: str) -> Mapping[str, str]:
    return next(candidate for candidate in ROUTING_CANDIDATES if candidate["adapter_id"] == adapter_id)
...
def should_block_signal(row: Mapping[str, Any], adapter_id: str) -> bool:
    ...
    if side == "flat" or adapter_id == "csp00_stage51_control":
        return False
    ...
    if adapter_id == "csp01_stage53_candidate_retest":
        return side == "long" and (session == "early" or adx == "adx_gt25")
    if adapter_id == "csp02_no_early_longs_no_lowadx_shorts":
        return (side == "long" and session == "early") or (side == "short" and adx == "adx_lt20")
    if adapter_id == "csp03_midlate_longs_strong_shorts":
        return (side == "long" and (session == "early" or adx == "adx_gt25")) or (
            side == "short" and not (session == "late" or adx == "adx_gt25" or di_spread == "di_short_strong")
        )
    if adapter_id == "csp04_late_longs_no_volmid_shorts":
        return (side == "long" and session != "late") or (side == "short" and (volatility == "vol_mid" or adx == "adx_lt20"))
    if adapter_id == "csp05_short_only_trend_or_di":
        return side == "long" or not (adx == "adx_gt25" or di_spread == "di_short_strong")
    ...
    for candidate in SIDE_FILTER_CANDIDATES:
        ...
        for tier_scope, source, columns, source_files, token in payloads:
            for window in WFO_WINDOWS:
                ...
                output_name = f"{RUN_DIR_NAME}_{token}_{adapter_id}_{window['window_id']}_s55.csv"
...
def make_attempts(model: Mapping[str, Any], features: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    for candidate in ROUTING_CANDIDATES:
        ...
        primary_adapter_id = str(candidate["primary_adapter_id"])
        fallback_adapter_id = str(candidate["fallback_adapter_id"])
        for window in WFO_WINDOWS:
            primary_key = f"{primary_adapter_id}_tier_a_{window['window_id']}"
            fallback_key = f"{fallback_adapter_id}_tier_b_{window['window_id']}"
            tier_a = attempt_payload(
                run_root=RUN_ROOT,
                run_id=RUN_ID,
                stage_number=STAGE_NUMBER,
                exploration_label="stage55_AdapterRouting__TierBFallbackSideFilterRouter",
                attempt_name=f"tier_a_{adapter_id}_{window['window_id']}",
                tier=mt5.TIER_A,
                split=window["window_id"],
                model_path=str(model["common_path"]),
                model_id=f"{RUN_ID}_{SOURCE_CANDIDATE_ID}_{adapter_id}_tier_a_signal_table",
                model_backend="ebm_table",
                feature_path=str(features["exports"][primary_key]["common_path"]),
                feature_count=1,
                ...
                record_view_prefix=f"mt5_tier_a_{adapter_id}",
                ...
                extra_set_values=candidate_set_values(candidate, 1005500 + index),
            )
            tier_a.update(
                {
                    "adapter_id": adapter_id,
                    ...
                    "route_mode": "tier_a_primary_side_filter_separate",
                    "route_view": "tier_a_primary_side_filter_separate",
                    "window_id": window["window_id"],
                    "window_label": window["label"],
                    "partition": stage52.window_partition(window["window_id"]),
                }
            )
            attempts.append(tier_a)
            index += 1
            ...
            tier_b = attempt_payload(
                ...
                stage_number=STAGE_NUMBER,
                exploration_label="stage55_AdapterRouting__TierBFallbackSideFilterRouter",
                attempt_name=f"tier_b_{adapter_id}_{window['window_id']}",
                tier=mt5.TIER_B,
                split=window["window_id"],
                model_path=str(model["common_path"]),
                model_id=f"{RUN_ID}_{SOURCE_CANDIDATE_ID}_{adapter_id}_tier_b_signal_table",
                model_backend="ebm_table",
                feature_path=str(features["exports"][fallback_key]["common_path"]),
                feature_count=1,
                ...
                attempt_role="tier_only_total",
                record_view_prefix=f"mt5_tier_b_{adapter_id}",
                max_hold_bars=int(rules["max_hold_bars"]),
                common_root=COMMON_RUN_ROOT,
                fallback_enabled=False,
                close_on_flat_signal=bool(rules["close_on_flat_signal"]),
                reverse_on_opposite_signal=bool(rules["reverse_on_opposite_signal"]),
                close_only_on_opposite_signal=bool(rules["close_only_on_opposite_signal"]),
                extra_set_values=candidate_set_values(candidate, 1005600 + index),
            )
            tier_b.update(
                {
                    "adapter_id": adapter_id,
                    ...
                    "primary_adapter_id": primary_adapter_id,
                    "fallback_adapter_id": fallback_adapter_id,
                    "route_mode": "tier_b_fallback_side_filter_separate",
                    "route_view": "tier_b_fallback_side_filter_separate",
                    "window_id": window["window_id"],
                    "window_label": window["label"],
                    "partition": stage52.window_partition(window["window_id"]),
                }
            ...
            routed = attempt_payload(
                ...
                stage_number=STAGE_NUMBER,
                exploration_label="stage55_AdapterRouting__TierBFallbackSideFilterRouter",
                attempt_name=f"routed_{adapter_id}_{window['window_id']}",
                tier=mt5.TIER_AB,
                split=window["window_id"],
                model_path=str(model["common_path"]),
                model_id=f"{RUN_ID}_{SOURCE_CANDIDATE_ID}_{adapter_id}_tier_a_signal_table",
                ...
                record_view_prefix=f"mt5_routed_{adapter_id}",
                max_hold_bars=int(rules["max_hold_bars"]),
                common_root=COMMON_RUN_ROOT,
                fallback_enabled=True,
                fallback_model_path=str(model["common_path"]),
                fallback_model_id=f"{RUN_ID}_{SOURCE_CANDIDATE_ID}_{adapter_id}_tier_b_signal_table",
                fallback_model_backend="ebm_table",
                fallback_feature_path=str(features["exports"][fallback_key]["common_path"]),
                ...
                reverse_on_opposite_signal=bool(rules["reverse_on_opposite_signal"]),
                close_only_on_opposite_signal=bool(rules["close_only_on_opposite_signal"]),
                extra_set_values=candidate_set_values(candidate, 1005700 + index),
            )
            routed.update(
                {
                    "adapter_id": adapter_id,
                    ...
                    "primary_adapter_id": primary_adapter_id,
                    "fallback_adapter_id": fallback_adapter_id,
                    "route_mode": "tier_a_primary_tier_b_side_filter_fallback",
                    "route_view": "tier_a_primary_tier_b_side_filter_routed_total",
                    "window_id": window["window_id"],
                    "window_label": window["label"],
                    "partition": stage52.window_partition(window["window_id"]),
                }
...
def route_coverage(features: Mapping[str, Any]) -> dict[str, Any]:
    ...
    for window in WFO_WINDOWS:
        ...
        subtype_by_split[window["window_id"]] = {"Stage55_fallback_router_Tier_B_accounting": tier_b}
...
def execute_mt5(attempts: Sequence[Mapping[str, Any]], coverage: Mapping[str, Any], args: argparse.Namespace) -> dict[str, Any]:
    ...
    for attempt in attempts:
        ...
        result = mt5.run_mt5_tester(
            ...
            tester_profile_ini_path=Path(args.tester_profile_root) / f"opv2_s55_{safe_name(attempt['attempt_name'], 80)}.ini",
        ...
        result.update(
            {
                ...
                "primary_adapter_id": attempt.get("primary_adapter_id"),
                "fallback_adapter_id": attempt.get("fallback_adapter_id"),
                "candidate_id": SOURCE_CANDIDATE_ID,
            ...
            row.update(
                {
                    ...
                    "source_label": "stage55_routed_fallback_adapter_filter",
                    "adapter_id": execution.get("adapter_id", ""),
...
def evaluate_candidates(summary_rows: Sequence[Mapping[str, Any]], trade_rows: Sequence[Mapping[str, Any]], mt5_result: Mapping[str, Any]) -> dict[str, Any]:
    ...
    best: dict[str, Any] = {}
    for adapter_id in sorted({str(row.get("adapter_id") or "") for row in summary_rows if str(row.get("adapter_id") or "").startswith("rfp")}):
        route_view = "tier_a_primary_tier_b_side_filter_routed_total"
        ...
        candidate_conc = next((row for row in concentrations if row["adapter_id"] == adapter_id and row["route_view"] == route_view), {})
        candidate_cost = next((row for row in costs if row["adapter_id"] == adapter_id and row["route_view"] == route_view), {})
        candidate_cov = next((row for row in coverage if row["adapter_id"] == adapter_id and row["route_view"] == route_view), {})
        candidate_gate_pass = (
            float(validation.get("net_profit") or 0.0) > 0.0
            and float(oos.get("net_profit") or 0.0) > 0.0
            and float(validation.get("profit_factor") or 0.0) >= 1.10
            and float(oos.get("profit_factor") or 0.0) >= 1.10
            and candidate_cov.get("status") == "passed"
            and candidate_conc.get("status") == "passed"
            and candidate_cost.get("cost_0.5_status") == "passed"
            and candidate_cost.get("cost_1.0_status") == "passed"
            and max(float(validation.get("max_drawdown_amount") or 0.0), float(oos.get("max_drawdown_amount") or 0.0)) <= 160.0
        )
        if candidate_gate_pass:
            score += 1_000_000.0
        if not best or score > float(best.get("score") or -999999.0):
            best = {"adapter_id": adapter_id, "route_view": route_view, "validation": validation, "oos": oos, "score": score, "mechanical_gate_preferred": candidate_gate_pass}
    ...
    if max(float(validation.get("max_drawdown_amount") or 0.0), float(oos.get("max_drawdown_amount") or 0.0)) > 160.0:
        adapter_failures.append("max_drawdown_over_stage55_limit_160")
    tier_a_validation = next((row for row in partitions if row["adapter_id"] == best_adapter and row["route_view"] == "tier_a_primary_side_filter_separate" and row["partition"] == "validation"), {})
    tier_a_oos = next((row for row in partitions if row["adapter_id"] == best_adapter and row["route_view"] == "tier_a_primary_side_filter_separate" and row["partition"] == "oos"), {})
    tier_b_validation = next((row for row in partitions if row["adapter_id"] == best_adapter and row["route_view"] == "tier_b_fallback_side_filter_separate" and row["partition"] == "validation"), {})
    tier_b = next((row for row in partitions if row["adapter_id"] == best_adapter and row["route_view"] == "tier_b_fallback_side_filter_separate" and row["partition"] == "oos"), {})
    if not tier_a_validation or not tier_a_oos:
        adapter_failures.append("tier_a_primary_accounting_missing")
    if not tier_b:
        adapter_failures.append("tier_b_accounting_missing")
    tier_b_net = float(tier_b_validation.get("net_profit") or 0.0) + float(tier_b.get("net_profit") or 0.0)
    routed_net = float(validation.get("net_profit") or 0.0) + float(oos.get("net_profit") or 0.0)
    tier_b_accounting_status = "tier_b_not_carrying_result"
    if tier_b_net > routed_net and routed_net > 0.0:
        tier_b_accounting_status = "tier_b_fallback_net_exceeds_routed_net_user_review_required"

    adapter_mechanical_status = "passed" if not adapter_failures else "failed"
    routing_description = routing_candidate_by_id(best_adapter).get("description", "") if best_adapter else ""
    routing_candidate = routing_candidate_by_id(best_adapter) if best_adapter else {}
    adapter_gate = {
        ...
        "tier_b_accounting_status": tier_b_accounting_status,
        "primary_adapter_id": routing_candidate.get("primary_adapter_id", ""),
        "fallback_adapter_id": routing_candidate.get("fallback_adapter_id", ""),
    ...
    practical_gate = {
        ...
        "adapter_id": best_adapter,
        "explicit_trading_behavior": routing_description,
        "deterministic_mapping": "adapter_id maps to a Tier A primary filter, Tier B fallback filter, routed MT5 .set files, and a candidate grid row",
        ...
        "risk_parameters": {
            ...
            "routing_rule": routing_description,
        },
        "tier_a_b_accounting": {
            "tier_a_validation": tier_a_validation,
            "tier_a_oos": tier_a_oos,
            "tier_b_validation": tier_b_validation,
            "tier_b_oos": tier_b,
            "tier_b_accounting_status": tier_b_accounting_status,
        },
        "side_specific": True,
...
def write_stage_docs(results: Mapping[str, Any], judgment: str) -> None:
    ...
    coverage_gate = next((row for row in results["evaluation"]["trade_count_coverage"] if row["adapter_id"] == best_adapter and row["route_view"] == "tier_a_primary_tier_b_side_filter_routed_total"), {})
    concentration_gate = next((row for row in results["evaluation"]["concentration"] if row["adapter_id"] == best_adapter and row["route_view"] == "tier_a_primary_tier_b_side_filter_routed_total"), {})
    write_md(
        STAGE_ROOT / "00_spec" / "stage_brief.md",
        f"""# Stage55 Tier-B Fallback Side-Filter Routing Filter(53?④퀎 諛⑺뼢 ?덉슜 ?꾪꽣)

- idea_id(?꾩씠?붿뼱 ID): `{IDEA_ID}`
- run_id(?ㅽ뻾 ID): `{RUN_ID}`
- packet_id(?⑦궥 ID): `{PACKET_ID}`
- adapter_hypothesis(?대뙌??媛??: Stage52(52?④퀎) control(?議곌뎔)?먯꽌 sell side(留ㅻ룄 諛⑺뼢)媛 ???덉젙?곸씤 ?먮쫫??蹂댁??쇰?濡? long side(濡?諛⑺뼢)瑜?紐낆떆?곸쑝濡??쒗븳?섎뒗 deterministic signal adapter(寃곗젙???좏샇 ?대뙌??瑜??ㅼ젣 MT5濡??뺤씤?쒕떎.
- core_question(?듭떖 吏덈Ц): side-specific permission(諛⑺뼢蹂??덉슜)??validation/OOS(寃利??쒕낯?? ?섏씡, profit factor(?섏씡 ?⑺꽣), trade-count coverage(嫄곕옒??而ㅻ쾭由ъ?), concentration(吏묒쨷??, cost sensitivity(鍮꾩슜 誘쇨컧??瑜??숈떆???듦낵?섎뒗媛?
- expected_mt5_evidence(?덉긽 MT5 洹쇨굅): `.set`, `.ini`, Strategy Tester HTML(?꾨왂 ?뚯뒪??HTML), imported KPI(媛?몄삩 ?듭떖?깃낵吏??, trade-level attribution(嫄곕옒 ?⑥쐞 洹??.
- boundary(二쇱옣 寃쎄퀎): `{BOUNDARY}`
""",
    )
    write_md(
        STAGE_ROOT / "01_inputs" / "input_refs.md",
        f"""# Stage55 Input References(53?④퀎 ?낅젰 李몄“)

- source_stage(?먯쿇 ?④퀎): `{SOURCE_STAGE_ID}`
- source_run(?먯쿇 ?ㅽ뻾): `{SOURCE_RUN_ID}`
- source_model(?먯쿇 紐⑤뜽): `{rel(SOURCE_MODEL_PATH)}`
- source_firewall_variant(?먯쿇 諛⑺솕踰?蹂??: `{SOURCE_FIREWALL_VARIANT}`
- Stage52 clue(52?④퀎 ?⑥꽌): `csp01_stage53_candidate_retest` and related side filters(諛⑺뼢 ?꾪꽣)??Stage52 control(?議곌뎔)??validation/OOS(寃利??쒕낯?? sell-side(留ㅻ룄 諛⑺뼢) attribution(洹???먯꽌 異쒕컻?덈떎.
""",
    )
    write_md(
        REVIEW_ROOT / f"{RUN_ID}_packet.md",
        f"""# Stage55 Run Packet(53?④퀎 ?ㅽ뻾 ?⑦궥)

- judgment(?먯젙): `{judgment}`
- best_validation_candidate(理쒖긽 寃利??꾨낫): `{best_adapter}` net(?쒖넀??=`{best_validation.get('net_profit', '')}` pf(?섏씡 ?⑺꽣)=`{best_validation.get('profit_factor', '')}` trades(嫄곕옒??=`{best_validation.get('closed_trades', '')}`
- best_oos_candidate(理쒖긽 ?쒕낯???꾨낫): `{best_adapter}` net(?쒖넀??=`{best_oos.get('net_profit', '')}` pf(?섏씡 ?⑺꽣)=`{best_oos.get('profit_factor', '')}` trades(嫄곕옒??=`{best_oos.get('closed_trades', '')}`
- adapter_candidate_gate(?대뙌???꾨낫 寃뚯씠??: `{adapter_gate['status']}` reasons(?댁쑀)=`{adapter_gate.get('failed_reasons', [])}`
- practical_tradability_gate(?ㅼ쟾 嫄곕옒 媛?μ꽦 寃뚯씠??: `{practical_gate['status']}`
- trade_count_coverage_gate(嫄곕옒??而ㅻ쾭由ъ? 寃뚯씠??: `{coverage_gate.get('status', '')}` reasons(?댁쑀)=`{coverage_gate.get('failed_reasons', [])}`
- concentration_audit(吏묒쨷??媛먯궗): `{concentration_gate.get('status', '')}`
- MT5 attempts(MT5 ?쒕룄): `{len(results['attempts'])}`
- MT5 status(MT5 ?곹깭): `{results['mt5'].get('external_verification_status')}`
- boundary(二쇱옣 寃쎄퀎): `{BOUNDARY}`
""",
    )
    write_md(
        REVIEW_ROOT / "review_index.md",
        f"""# Stage55 Review Index(53?④퀎 寃???됱씤)

- run packet(?ㅽ뻾 ?⑦궥): `{RUN_ID}_packet.md`
- stage ledger(?④퀎 ?λ?): `stage_run_ledger.csv`
- MT5 summary(MT5 ?붿빟): `{rel(MT5_SUMMARY_PATH)}`
- partition summary(遺꾪븷 ?붿빟): `{rel(PARTITION_SUMMARY_PATH)}`
- trade coverage(嫄곕옒 而ㅻ쾭由ъ?): `{rel(TRADE_COVERAGE_PATH)}`
- concentration(吏묒쨷??: `{rel(CONCENTRATION_PATH)}`
- packet root(?⑦궥 猷⑦듃): `{rel(PACKET_ROOT)}`
""",
    )
    write_md(
        STAGE_ROOT / "04_selected" / "selection_status.md",
        f"""# Stage55 Selection Status(53?④퀎 ?좏깮 ?곹깭)

- final_judgment(理쒖쥌 ?먯젙): `{judgment}`
- selected_adapter_candidate(?좏깮 ?대뙌???꾨낫): `{best_adapter}`
- adapter_candidate_status(?대뙌???꾨낫 ?곹깭): `{results['evaluation']['adapter_candidate_review_gate']['status']}`
- selected_baseline(?좏깮 湲곗???: `none`
- selected_promotion(?좏깮 ?밴꺽): `none`
- runtime_authority(?고???沅뚯쐞): `none`
- live_readiness(?ㅺ굅??以鍮?: `none`
- operating_reference(?댁쁺 李몄“): `none`
- latest_run_id(理쒖떊 ?ㅽ뻾 ID): `{RUN_ID}`
- boundary(二쇱옣 寃쎄퀎): `{BOUNDARY}`
""",
    )
...
def ledger_rows(judgment: str, evaluation: Mapping[str, Any]) -> tuple[list[dict[str, Any]], list[dict[str, Any]], list[dict[str, Any]]]:
        ...
        alpha_rows.append(
            {
                ...
                "kpi_scope": "stage55_side_permission_validation_oos",
    ...
    alpha_rows.append(
        {
            "ledger_row_id": f"{RUN_ID}_{adapter_id}_stage55_closeout",
            "stage_id": STAGE_ID,
            "run_id": RUN_ID,
            "subrun_id": "stage55_closeout",
            "parent_run_id": RUN_ID,
            "record_view": "stage55_closeout",
            "tier_scope": "Tier A and Tier B separate",
            "kpi_scope": "stage55_side_permission_stage_closeout",
    ...
    artifact_rows = [
        {"artifact_id": "stage55_run48A_manifest", "type": "manifest", "path": rel(MANIFEST_PATH), "status": "generated", "notes": "Stage55 fallback routing run manifest."},
        {"artifact_id": "stage55_run48A_mt5_summary", "type": "result_table", "path": rel(MT5_SUMMARY_PATH), "status": "generated", "notes": "Imported MT5 KPI summary."},
        {"artifact_id": "stage55_run48A_trade_coverage", "type": "result_table", "path": rel(TRADE_COVERAGE_PATH), "status": "generated", "notes": "Period-adjusted trade-count gate evidence."},
        {"artifact_id": "stage55_run48A_concentration", "type": "result_table", "path": rel(CONCENTRATION_PATH), "status": "generated", "notes": "Trade/day/week/month concentration audit."},
    ]
...
def update_workspace_state(judgment: str, evaluation: Mapping[str, Any]) -> None:
    ...
    focus = (
        f"- Stage55(53?④퀎) `{STAGE_ID}`: side-specific permission filter(諛⑺뼢蹂??덉슜 ?꾪꽣)瑜?`{judgment}`濡?湲곕줉?덈떎; "
        f"selected_adapter(?좏깮 ?대뙌??=`{adapter_id}`; baseline(湲곗???, promotion(?밴꺽), runtime authority(?고???沅뚯쐞), "
        "live readiness(?ㅺ굅??以鍮?, operating reference(?댁쁺 李몄“)??留뚮뱾吏 ?딆븯??"
    )
    if focus not in text:
        text = text.replace("current_focus:\n", "current_focus:\n" + focus + "\n", 1)
    block = f"""
stage55_routed_fallback_adapter_filter:
  packet_id: {PACKET_ID}
  stage_id: {STAGE_ID}
  idea_id: {IDEA_ID}
  status: {stage_status}
  ...
  boundary: {BOUNDARY}
"""
    if "stage55_routed_fallback_adapter_filter:" not in text:
        text = text.rstrip() + "\n\n" + block
    else:
        text = re.sub(r"\nstage55_routed_fallback_adapter_filter:\n(?:  .+\n)+", "\n" + block, text)
    io_path(WORKSPACE_STATE_PATH).write_text(text.rstrip() + "\n", encoding="utf-8")

    current_block = f"""## Latest Stage55 Tier-B Fallback Side-Filter Routing Filter(理쒖떊 53?④퀎 諛⑺뼢 ?덉슜 ?꾪꽣)

- current run(?꾩옱 ?ㅽ뻾): `{RUN_ID}`

Stage55(53?④퀎) `{STAGE_ID}`??side-specific permission filter(諛⑺뼢蹂??덉슜 ?꾪꽣)瑜?`{judgment}`濡?湲곕줉?덈떎. selected candidate(?좏깮 ?꾨낫)??`{adapter_id}`?닿퀬, boundary(寃쎄퀎)??runtime_probe_only(?고????먯묠 ?꾩슜)?대떎. baseline(湲곗???, promotion(?밴꺽), runtime authority(?고???沅뚯쐞), live readiness(?ㅺ굅??以鍮?, operating reference(?댁쁺 李몄“)???녿떎.

"""
    current = read_text(CURRENT_WORKING_STATE_PATH)
    if "## Latest Stage55 Tier-B Fallback Side-Filter Routing Filter(理쒖떊 53?④퀎 諛⑺뼢 ?덉슜 ?꾪꽣)" not in current:
        io_path(CURRENT_WORKING_STATE_PATH).write_text(current_block + current, encoding="utf-8-sig")
    append_once(
        CHANGELOG_PATH,
        f"- 2026-05-11T00:00:00Z `{STAGE_ID}` recorded(湲곕줉) tier-b fallback side-filter routing filter(諛⑺뼢 ?덉슜 ?꾪꽣) as `{judgment}`; no baseline(湲곗???, promotion(?밴꺽), runtime authority(?고???沅뚯쐞), live readiness(?ㅺ굅??以鍮?, or operating reference(?댁쁺 李몄“) created(?앹꽦).",
        bom=True,
    )
...
def write_packet_files(results: Mapping[str, Any], judgment: str, ledger_sync: Mapping[str, Any]) -> None:
    ...
    validation_commands = [
        run_command(["python", "-m", "py_compile", "stage_pipelines/stage55/routed_fallback_adapter.py", "foundation/pipelines/run_stage55_routed_fallback_adapter.py"]),
        run_command(["python", "-m", "pytest", "tests/test_stage55_routed_fallback_adapter.py", "tests/test_stage52_atr_sltp_adapter.py", "-q"]),
    ]
    write_json(PACKET_ROOT / "work_packet.yaml", {"packet_id": PACKET_ID, "stage_id": STAGE_ID, "primary_family": "adapter_routing", "primary_skill": "obsidian-runtime-parity", "support_skills": ["obsidian-experiment-design", "obsidian-backtest-forensics", "obsidian-artifact-lineage", "obsidian-result-judgment"], "required_gates": ["runtime_evidence_gate", "adapter_candidate_gate", "practical_tradability_gate", "trade_count_coverage_gate", "concentration_audit", "artifact_lineage_audit"], "claim_boundary": BOUNDARY})
    ...
    write_json(PACKET_ROOT / "runtime_evidence_gate.json", {"status": runtime_status, "mt5_attempts": len(results["attempts"]), "compile": mt5_result.get("compile"), "compile_returncode_note": "If MetaEditor returns a non-zero process code while the compile log reports zero errors and Strategy Tester reports are produced, runtime evidence is judged from the compile log plus actual tester outputs, not the process code alone.", "report_count": len(mt5_result.get("strategy_tester_reports", [])), "tester_identity": {"symbol": "US100", "timeframe": "M5", "model": "Every tick based on real ticks", "deposit": 500, "leverage": "1:100"}, "spread_execution_assumptions": "MT5 Strategy Tester terminal defaults for FPMarkets US100 M5 plus explicit per-trade cost sensitivity at 0.25, 0.5, 1.0, and 2.0 result-currency units.", "backtest_judgment": "usable_with_boundary" if runtime_status == "passed" else "blocked"})
    ...
    write_json(PACKET_ROOT / "artifact_lineage_audit.json", {"status": "passed", "source_inputs": [rel(SOURCE_MODEL_PATH), rel(FEATURE_AUDIT_PATH)], "producer": rel(Path("stage_pipelines/stage55/routed_fallback_adapter.py")), "runtime_path": rel(mt5.EA_SOURCE_PATH), "lineage_judgment": "connected_with_boundary"})
    ...
    write_json(PACKET_ROOT / "git_sync_record.json", {"status": "pending_main_push", "branch": "codex/overnight-autonomous-adapter-campaign-continue", "stage_id": STAGE_ID})
...
def write_adapter_candidate_review_packet(results: Mapping[str, Any], judgment: str) -> str | None:
    ...
    packet_path = common.ROOT / "docs" / "agent_control" / "packets" / f"adapter_candidate_review_stage55_{adapter_id}_v1"
...
def run_pipeline(args: argparse.Namespace) -> dict[str, Any]:
    ...
    write_json(MANIFEST_PATH, {"run_id": RUN_ID, "stage_id": STAGE_ID, "adapter_candidates": list(ROUTING_CANDIDATES), "side_filter_primitives": list(SIDE_FILTER_CANDIDATES), "model": model, "features": features, "attempts": attempts, "mt5": mt5_result, "summary_rows": summary_rows, "evaluation": evaluation, "judgment": judgment, "boundary": BOUNDARY, "created_at_utc": utc_now()})
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run Stage55 tier-b fallback side-filter routing filter MT5 probe.")
