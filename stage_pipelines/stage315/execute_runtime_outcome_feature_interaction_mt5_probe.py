...
RUN_ID = "run315B_execute_runtime_outcome_feature_interaction_mt5_probe_v1"
RUN_NUMBER = "run315B"
SOURCE_RUN_ID = "run315A_design_runtime_outcome_feature_interaction_rebuild_packet_v1"
PARENT_RUN_ID = "run314C_review_runtime_outcome_feature_source_mt5_probe_v1"
STATUS_PREPARED = "prepared_runtime_outcome_feature_interaction_mt5_probe_no_runtime_kpi"
UPDATED_ON = "2026-05-24"
EXPLORATION_LABEL = "stage315_Model__RuntimeOutcomeFeatureInteractionReplay"
SIGNAL_COLUMN = "run315b_route_signal"
FEATURE_ORDER = (SIGNAL_COLUMN,)
COMMON_ROOT = "Project_Obsidian_Prime_v2/stage315/run315B_runtime_outcome_feature_interaction"

STAGE_ROOT = ROOT / "stages" / STAGE_ID
RUN315A = STAGE_ROOT / "02_runs" / "run315A"
...
MT5_QUEUE = RUN315A / "mt5_probe_queue.csv"
SOURCE_MANIFEST = RUN315A / "candidate_payload_manifest.csv"
SOURCE_RUN_MANIFEST = RUN315A / "run_manifest.json"
SOURCE_REPORT = REVIEWS / "run315A_materialization.md"
PRODUCER = Path("stage_pipelines/stage315/execute_runtime_outcome_feature_interaction_mt5_probe.py")
...
REPORT = REVIEWS / "run315B_mt5_probe.md"
...
def classify_status(result: Mapping[str, Any], materialize_only: bool) -> tuple[str, str, str, str]:
    ...
    if materialize_only:
        return (
            STATUS_PREPARED,
            "runtime_outcome_feature_interaction_runtime_probe_prepared_no_external_execution",
            "out_of_scope_by_claim_materialize_only",
            "run315B_execute_runtime_outcome_feature_interaction_mt5_probe_external_check",
        )
    ...
    if planned and completed_exec >= planned and len(kpis) >= planned:
        return (
            "completed_runtime_outcome_feature_interaction_mt5_probe_no_selection",
            "runtime_probe_completed_requires_profit_scale_curve_review_no_selection",
            "completed",
            "run315C_review_runtime_outcome_feature_interaction_mt5_probe",
        )
    if kpis:
        return (
            "partial_runtime_outcome_feature_interaction_mt5_probe_no_selection",
            "runtime_probe_partial_requires_continuation_or_review_no_selection",
            "partial_or_blocked",
            "run315B_continue_runtime_outcome_feature_interaction_mt5_probe" if limited else "run315C_review_with_runtime_gaps",
        )
    return (
        "blocked_runtime_outcome_feature_interaction_mt5_probe_no_kpi",
        "runtime_probe_blocked_no_kpi_no_selection",
        "blocked_or_invalid",
        "run315B_repair_or_block_runtime_outcome_feature_interaction_mt5_probe",
    )
...
def report_markdown(result: Mapping[str, Any], status: str, judgment: str, external_status: str, next_action: str) -> str:
    ...
    return "\n".join(
        [
            "# run315B Runtime Outcome Feature Interaction MT5 Probe(315B 런타임 결과 피처 상호작용 MT5 탐침)",
            "",
            ...
            "",
            "Effect(효과): run315A(315A 실행)의 feature interaction route_signal_value(피처 상호작용 경로 신호)를 MT5 tester(MT5 테스터)에 넣어 실제 거래 수, 순수익, PF(수익 팩터), DD(손실폭), 곡선 포켓을 확인한다.",
            "",
            f"`{e310.BOUNDARY}`",
        ]
    )
...
def upsert_ledgers(result: Mapping[str, Any], status: str, judgment: str, external_status: str, next_action: str) -> None:
    ...
    e310.safe_upsert_rows(
        RUN_REGISTRY,
        base.RUN_REGISTRY_COLUMNS,
        [{"run_id": RUN_ID, "stage_id": STAGE_ID, "lane": "runtime_outcome_feature_interaction_mt5_probe", "status": status, "judgment": judgment, "path": base.rel(REPORT), "notes": f"attempts={attempt_count};mt5_kpi_records={kpi_count};selected_candidate=none;onnx_readiness=not_claimed;next_action={next_action}."}],
        key="run_id",
    )
    e310.safe_upsert_rows(
        ALPHA_LEDGER,
        ...
        [
            {
                ...
                "record_view": "runtime_outcome_feature_interaction_mt5_probe",
                "tier_scope": "Tier A used/Tier B fallback stress/actual routed total",
                "kpi_scope": "runtime_probe",
                "scoreboard_lane": "runtime_outcome_feature_interaction",
                "status": status,
                "judgment": judgment,
    ...
    e310.safe_upsert_rows(
        STAGE_LEDGER,
        e310.runner.prev.e295.e294.base.STAGE_LEDGER_COLUMNS,
        [{"row_id": f"{RUN_ID}__mt5_probe", "stage_id": STAGE_ID, "run_id": RUN_ID, "view": "runtime_outcome_feature_interaction_mt5_probe", "tier_scope": "Tier A used/Tier B fallback/actual routed total", "scoreboard": "runtime_probe", "status": status, "judgment": judgment, "evidence_boundary": "runtime_probe_no_candidate_no_onnx", "report_path": base.rel(REPORT), "notes": f"attempts={attempt_count};mt5_kpi_records={kpi_count}."}],
        key="row_id",
    )
...
def update_docs(status: str, judgment: str, next_action: str, kpi_count: int, attempt_count: int) -> None:
    ...
    selected = base.append_once(selected, "run315B_report", f"- run315B_report(315B 보고서): `{base.rel(REPORT)}`")
    selected = base.append_once(selected, "run315B_execution_result", f"- run315B_execution_result(315B 실행 결과): `{base.rel(EXECUTION_RESULT)}`")
    save_md(SELECTED, selected)

    review_index = base.io_path(REVIEW_INDEX).read_text(encoding="utf-8-sig") if base.path_exists(REVIEW_INDEX) else "# Stage315 Review Index(315단계 검토 색인)\n"
    review_index = base.append_once(
        review_index,
        "run315B_report",
        f"- run315B_report(315B 보고서): `{base.rel(REPORT)}`\n- run315B_execution_result(315B 실행 결과): `{base.rel(EXECUTION_RESULT)}`\n- run315B_mt5_kpi_summary(315B MT5 핵심 성과 지표 요약): `{base.rel(MT5_KPI_SUMMARY)}`",
    )
    ...
    current = base.append_once(current, "run315B_summary", f"- run315B_summary(315B 요약): runtime outcome feature interaction MT5 probe(런타임 결과 피처 상호작용 MT5 탐침)를 실행했다. Effect(효과): attempts(시도) `{attempt_count}`개와 MT5 KPI records(MT5 핵심 성과 지표 기록) `{kpi_count}`개를 만들었고 후보/Adapter(어댑터)/ONNX(온엑스)는 주장하지 않는다.")
    ...
    focus = (
        f"- >-\n"
        f"  Stage315(315단계) run315B(315B 실행) runtime outcome feature interaction MT5 probe(런타임 결과 피처 상호작용 MT5 탐침) `{RUN_ID}`. "
        f"Effect(효과): attempts(시도) `{attempt_count}`개와 MT5 KPI records(MT5 핵심 성과 지표 기록) `{kpi_count}`개를 기록했고 selected candidate(선택 후보), Adapter package(어댑터 패키지), ONNX readiness(온엑스 준비)는 주장하지 않는다.\n"
    )
    ...
    changelog = base.append_once(
        changelog,
        RUN_ID,
        f"## {UPDATED_ON} run315B Runtime outcome feature interaction MT5 probe(315B 런타임 결과 피처 상호작용 MT5 탐침)\n\n"
        f"- status(상태): `{status}`\n"
        f"- judgment(판정): `{judgment}`\n"
        f"- effect(효과): attempts(시도) `{attempt_count}`개와 MT5 KPI records(MT5 핵심 성과 지표 기록) `{kpi_count}`개를 기록했다.\n"
