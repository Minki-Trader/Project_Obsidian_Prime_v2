...
RUN_ID = "run315C_review_runtime_outcome_feature_interaction_mt5_probe_v1"
RUN_NUMBER = "run315C"
SOURCE_RUN_ID = "run315B_execute_runtime_outcome_feature_interaction_mt5_probe_v1"
PARENT_RUN_ID = "run315A_design_runtime_outcome_feature_interaction_rebuild_packet_v1"
UPDATED_ON = "2026-05-24"
BOUNDARY = r310.BOUNDARY

NEXT_REBUILD_STAGE_ID = "316_onnx_candidate_campaign__post_interaction_profit_scale_curve_rebuild"
NEXT_ADAPTER_STAGE_ID = "316_onnx_candidate_campaign__adapter_package_for_runtime_outcome_feature_interaction"

STAGE_ROOT = ROOT / "stages" / STAGE_ID
RUN315B = STAGE_ROOT / "02_runs" / "run315B"
...
SOURCE_KPI = RUN315B / "mt5_kpi_summary.csv"
SOURCE_ATTEMPT_SUMMARY = RUN315B / "attempt_summary.csv"
PRODUCER = Path("stage_pipelines/stage315/review_runtime_outcome_feature_interaction_mt5_probe.py")

SCOREBOARD = RUN_ROOT / "runtime_outcome_feature_interaction_review_scoreboard.csv"
...
NEXT_STAGE_QUEUE = RUN_ROOT / "stage316_seed_queue.csv"
RESULT_JUDGMENT = RUN_ROOT / "result_judgment.csv"
GATE_AUDIT = RUN_ROOT / "required_gate_coverage_audit.csv"
RUN_MANIFEST = RUN_ROOT / "run_manifest.json"
LINEAGE = RUN_ROOT / "artifact_lineage_receipt.json"
REPORT = REVIEWS / "run315C_review_stage316_open.md"
DECISION = ROOT / "docs" / "decisions" / "2026-05-24_stage315_runtime_outcome_feature_interaction_review_stage316_open.md"
...
def next_stage_rows(scoreboard: Sequence[Mapping[str, Any]]) -> list[dict[str, Any]]:
    ...
    return [
        {
            "seed_id": "stage315_runtime_outcome_feature_interaction_review_seed",
            "source_candidates": ";".join(row["package_id"] for row in positive[:3]) or "none",
            "fresh_thesis": "post_interaction_profit_scale_curve_rebuild",
            "use_as": "failure_memory_or_candidate_seed",
            "upside": "feature interaction(피처 상호작용)이 일부 수익 단서를 만들면 next stage(다음 단계)에서 수익 규모와 곡선 포켓을 더 넓은 risk/reward surface(위험/보상 표면)로 압박한다.",
            "failure_mode": "validation/OOS(검증/표본외), density(밀도), efficiency(효율), curve(곡선)가 함께 통과하지 못하면 실패다.",
            "discard_condition": "hour-only repair(시간만 고치는 수리)로 좁히지 않고, 실제 수익 규모/곡선 조건이 없으면 폐기한다.",
            "next_action": "run316A_design_post_interaction_profit_scale_curve_rebuild_packet",
        }
    ]
...
def scaffold_next_stage(selected_rows: Sequence[Mapping[str, Any]]) -> tuple[str, str, str, str]:
    if selected_rows:
        ...
        question = "selected runtime outcome feature interaction(선택된 런타임 결과 피처 상호작용)을 Adapter package(어댑터 패키지)로 정리할 수 있는가?"
        status = "opened_adapter_package_stage_after_stage315_candidate_selection"
        judgment = "candidate_selected_adapter_package_stage_opened_no_onnx_yet"
        next_action = "run316A_build_runtime_outcome_feature_interaction_adapter_package"
    else:
        next_stage_id = NEXT_REBUILD_STAGE_ID
        question = "Stage315 feature interaction(315단계 피처 상호작용) 실패 기억을 넘어 profit scale and curve(수익 규모와 곡선)를 같이 만족하는 새 risk/reward surface(위험/보상 표면)를 만들 수 있는가?"
        status = "opened_post_interaction_profit_scale_curve_after_stage315_no_selection"
        judgment = "no_candidate_selected_post_interaction_profit_scale_curve_stage_opened"
        next_action = "run316A_design_post_interaction_profit_scale_curve_rebuild_packet"
    ...
    write_text(
        ...
        "\n".join(
            [
                ...
                "",
                "Effect(효과): Stage315(315단계)의 결과를 후보 확정이 아니라 다음 연구 질문의 근거로 넘긴다.",
            ]
        ),
    )
    write_text(
        ...
        "\n".join(
            [
                "# Stage316 Selection Status(316단계 선택 상태)",
                "",
                ...
                f"- stage315_review(315단계 검토): `{rel(REPORT)}`",
            ]
        ),
    )
    write_text(next_root / "03_reviews" / "review_index.md", f"# Stage316 Review Index(316단계 검토 색인)\n\n- stage315_review(315단계 검토): `{rel(REPORT)}`\n")
    write_csv(
        next_root / "03_reviews" / "stage_run_ledger.csv",
        STAGE_LEDGER_COLUMNS,
        [
            {
                "row_id": f"{RUN_ID}__stage316_open",
...
def report_markdown(scoreboard: Sequence[Mapping[str, Any]], selected_rows: Sequence[Mapping[str, Any]], next_stage_id: str, next_action: str) -> str:
    ...
    lines = [
        ...
        f"- selected_candidate(선택 후보): `{selected_rows[0]['package_id'] if selected_rows else 'none'}`",
        f"- Adapter package(어댑터 패키지): `{'deferred_to_stage316' if selected_rows else 'none'}`",
        "- ONNX readiness(온엑스 준비): `not_started`",
        "- Goal Achieve(목표 달성): `not_claimed`",
    ...
    for row in scoreboard:
        ...
        lines.append(
            "| {pkg} | {vn:.2f} | {vpf:.2f} | {on:.2f} | {opf:.2f} | {vtd:.2f}/{otd:.2f} | {combined:.2f} | {gates} |".format(
                pkg=row["package_id"],
                ...
                combined=number(row["combined_net_profit"]),
...
def update_docs(status: str, judgment: str, next_stage_id: str, next_action: str, selected_rows: Sequence[Mapping[str, Any]]) -> None:
    ...
    selected = r310.replace_line(selected, "- Adapter package(", f"- Adapter package(어댑터 패키지): `{'deferred_to_stage316' if selected_rows else 'none'}`")
    selected = r310.replace_line(selected, "- next_action(", f"- next_action(다음 행동): `{next_action}`")
    selected = r310.drop_prefixed_lines(selected, ("- run315C_report(", "- stage316_opened("))
    selected += f"- run315C_report(315C 보고서): `{rel(REPORT)}`\n"
    selected += f"- stage316_opened(316단계 열림): `{next_stage_id}`\n"
    write_text(SELECTED, selected)

    review_index = read_text(REVIEW_INDEX)
    review_index = r310.drop_prefixed_lines(review_index, ("- run315C_report(", "- run315C_scoreboard(", "- stage316_seed_queue("))
    review_index += f"- run315C_report(315C 보고서): `{rel(REPORT)}`\n"
    review_index += f"- run315C_scoreboard(315C 점수판): `{rel(SCOREBOARD)}`\n"
    review_index += f"- stage316_seed_queue(316단계 씨앗 대기열): `{rel(NEXT_STAGE_QUEUE)}`\n"
    ...
    current = r310.drop_prefixed_lines(current, ("- run315C_summary(",))
    current = current.rstrip() + f"\n- run315C_summary(315C 요약): Stage315(315단계) actual MT5(실제 메타트레이더5) 검토를 완료했다. Effect(효과): selected_candidate(선택 후보)는 `{selected_rows[0]['package_id'] if selected_rows else 'none'}`이고 next_stage(다음 단계)는 `{next_stage_id}`다.\n"
    ...
    focus = (
        f"- >-\n"
        f"  Stage315(315단계) run315C(315C 실행) runtime outcome feature interaction review(런타임 결과 피처 상호작용 검토) `{RUN_ID}` closed Stage315 and opened `{next_stage_id}`. "
        f"Effect(효과): selected candidate(선택 후보)는 `{selected_rows[0]['package_id'] if selected_rows else 'none'}`이고 Adapter package(어댑터 패키지)는 `{'deferred_to_stage316' if selected_rows else 'none'}`, ONNX readiness(온엑스 준비)는 `not_started`다.\n"
    )
    ...
    if RUN_ID not in changelog:
        changelog += (
            f"\n## {UPDATED_ON} run315C Runtime outcome feature interaction review(315C 런타임 결과 피처 상호작용 검토)\n\n"
            f"- status(상태): `{status}`\n"
            f"- judgment(판정): `{judgment}`\n"
            f"- effect(효과): Stage315(315단계)를 닫고 `{next_stage_id}`를 열었다.\n"
...
def update_memory_registers(failures: Sequence[Mapping[str, Any]], selected_rows: Sequence[Mapping[str, Any]]) -> None:
    ...
    if RUN_ID not in idea:
        idea += (
            f"\n## {RUN_ID} runtime_outcome_feature_interaction_review(런타임 결과 피처 상호작용 검토)\n\n"
    ...
    if failures:
        ...
        if RUN_ID not in negative:
            negative += (
                f"\n## {RUN_ID} Stage315 runtime outcome feature interaction failure memory(315단계 런타임 결과 피처 상호작용 실패 기억)\n\n"
...
def update_registers(status: str, judgment: str, next_action: str) -> None:
    safe_upsert(RUN_REGISTRY, RUN_REGISTRY_COLUMNS, [{"run_id": RUN_ID, "stage_id": STAGE_ID, "lane": "runtime_outcome_feature_interaction_review", "status": status, "judgment": judgment, "path": rel(REPORT), "notes": f"selected_candidate_reviewed;next_action={next_action}."}], "run_id")
    safe_upsert(
        ALPHA_LEDGER,
        ...
        [
            {
                ...
                "record_view": "runtime_outcome_feature_interaction_review",
    ...
    safe_upsert(STAGE_LEDGER, STAGE_LEDGER_COLUMNS, [{"row_id": f"{RUN_ID}__review", "stage_id": STAGE_ID, "run_id": RUN_ID, "view": "runtime_outcome_feature_interaction_review", "tier_scope": "Tier A used/Tier B fallback/actual routed total", "scoreboard": "runtime_outcome_feature_interaction_review_scoreboard", "status": status, "judgment": judgment, "evidence_boundary": "runtime_probe_review_no_onnx", "report_path": rel(REPORT), "notes": "Stage316 opened if no selected candidate; ONNX not started."}], "row_id")
        ...
        rows.append(
            {
                ...
                "artifact_type": "stage315_runtime_outcome_feature_interaction_review_artifact",
                "path": rel(path),
                "sha256": sha256_file(path),
                "stage_id": STAGE_ID,
                "run_id": RUN_ID,
                "created_at_utc": "2026-05-24T23:59:00Z",
                "notes": "Stage315 review and Stage316 open handoff",
            }
        )
...
def main() -> None:
    ...
    stage316_rows = next_stage_rows(scoreboard)
    next_stage_id, _next_status, _next_judgment, next_action = scaffold_next_stage(selected_rows)
    status = "completed_runtime_outcome_feature_interaction_review_stage316_opened"
    judgment = "actual_mt5_candidate_gate_passed_adapter_stage_opened" if selected_rows else "actual_mt5_no_onnx_worthy_candidate_runtime_outcome_feature_interaction_opened"
    ...
    write_csv(NEXT_STAGE_QUEUE, list(stage316_rows[0].keys()), stage316_rows)
    write_csv(
        RESULT_JUDGMENT,
        ("run_id", "status", "judgment", "selected_candidate", "adapter_package", "onnx_readiness", "next_action", "claim_boundary"),
        [{"run_id": RUN_ID, "status": status, "judgment": judgment, "selected_candidate": selected_rows[0]["package_id"] if selected_rows else "none", "adapter_package": "deferred_to_stage316" if selected_rows else "none", "onnx_readiness": "not_started", "next_action": next_action, "claim_boundary": BOUNDARY}],
    )
    gate_rows = [
    ...
    manifest = {
        ...
        "adapter_package": "deferred_to_stage316" if selected_rows else "none",
    ...
    print(json.dumps({"status": status, "judgment": judgment, "scoreboard_rows": len(scoreboard), "failure_rows": len(failure_rows), "selected_candidate": selected_rows[0]["package_id"] if selected_rows else "none", "adapter_package": "deferred_to_stage316" if selected_rows else "none", "onnx_readiness": "not_started", "goal_achieve": "not_claimed", "next_stage_id": next_stage_id, "next_action": next_action}, ensure_ascii=False, indent=2, sort_keys=True))
