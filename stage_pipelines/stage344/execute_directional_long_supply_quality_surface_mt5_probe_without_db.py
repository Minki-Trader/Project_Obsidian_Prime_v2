...
RUN_NUMBER = "run344D"
RUN_ID = "run344D_execute_directional_long_supply_quality_surface_mt5_probe_without_db_v1"
PARENT_RUN_ID = pkg.RUN_ID
SOURCE_PACKAGE_RUN_ID = pkg.RUN_ID
NEXT_RUN_ID = "run344E_review_directional_long_quality_surface_mt5_probe_without_db_v1"
STATUS_COMPLETED = "completed_stage344D_directional_long_quality_surface_mt5_probe_executed_review_required_no_selection"
STATUS_BLOCKED = "blocked_stage344D_directional_long_quality_surface_mt5_probe_attempt_recorded_repair_required_no_selection"
JUDGMENT_COMPLETED = "mt5_directional_long_quality_surface_probe_outputs_available_review_required_no_selection"
JUDGMENT_BLOCKED = "mt5_directional_long_quality_surface_probe_attempt_recorded_but_outputs_missing_or_failed_repair_required"
DECISION_COMPLETED = "stage344D_open_run344E_review_directional_long_quality_surface_probe"
DECISION_BLOCKED = "stage344D_open_run344E_review_or_repair_directional_long_quality_surface_probe"
CLAIM_BOUNDARY = (
    "research_development_directional_long_quality_surface_mt5_runtime_probe_attempt_only_"
...
REPORT_PATH = REVIEW_DIR / "run344D_directional_long_quality_surface_mt5_probe.md"
DECISION_DOC = ROOT / "docs" / "decisions" / f"{TODAY}_stage344D_directional_long_quality_surface_mt5_probe.md"
...
EXECUTION_SUMMARY = RUN_DIR / "directional_long_quality_surface_mt5_probe_summary.csv"
...
def make_gates(final: Mapping[str, Any]) -> pd.DataFrame:
    ...
    return pd.DataFrame(
        [
            gate_row("parent_344C_package_gates_passed", "passed" if final["parent_gate_passed"] else "failed", HELPER.rel(pkg.GATE_AUDIT), "run344C(344C 실행) package gate(패키지 게이트)를 이어받는다."),
            gate_row("mt5_attempts_recorded", "passed" if final["execution_result_rows"] == final["attempt_rows"] and final["attempt_rows"] > 0 else "failed", HELPER.rel(MT5_EXECUTION_RESULT), "각 attempt(시도)의 MT5 실행 결과를 기록한다."),
            gate_row("runtime_outputs_completed", "passed" if final["runtime_completed_rows"] == final["attempt_rows"] else "failed", HELPER.rel(RUNTIME_OUTPUT_COPY), "모든 telemetry(런타임 기록)를 복사한다."),
            gate_row("strategy_tester_reports_collected", "passed" if final["report_completed_rows"] == final["attempt_rows"] else "failed", HELPER.rel(STRATEGY_TESTER_REPORTS), "각 Strategy Tester report(전략 테스터 보고서)를 수집한다."),
            ...
            gate_row("no_forbidden_operating_claim", "passed" if no_forbidden else "failed", HELPER.rel(FINAL_DECISION), "runtime probe(런타임 탐침)를 selection(선정), operating promotion(운영 승격), Goal Achieve(목표 달성)로 주장하지 않는다."),
...
def build_summary(
    ...
    if HELPER.exists(RUNTIME_IDENTITY):
        ...
        if "identity_id" in identity.columns:
            identity["identity_id"] = "stage344D_runtime_identity"
...
def write_receipts(final: Mapping[str, Any]) -> None:
    ...
    lineage_payload["lineage_judgment"] = "connected_with_directional_long_quality_runtime_probe_boundary(방향성 롱 품질 런타임 탐침 경계로 연결)"
    HELPER.write_json_direct(LINEAGE_RECEIPT, lineage_payload)


def write_docs(final: Mapping[str, Any]) -> None:
    report = f"""# run344D Directional Long Quality Surface MT5 Probe(344D 방향성 롱 품질 표면 MT5 탐침)
...
run344C(344C 실행)의 directional long quality surface(방향성 롱 품질 표면) package(패키지)를 MT5 Strategy Tester(MT5 전략 테스터)로 실행하고, expected tape(예상 테이프)와 telemetry(런타임 기록)를 비교했다.

## Effect(효과)

run344E(344E 실행)가 net profit(순수익), profit factor(수익 팩터), expectancy(기대값), drawdown(낙폭), recovery factor(회복 계수), trade count(거래수), long/short balance(롱/숏 균형), proxy-MT5 diff(프록시-MT5 차이)를 실제 runtime evidence(런타임 근거)로 검토할 수 있다.

## Boundary(경계)

run344D(344D 실행)는 runtime probe attempt(런타임 탐침 시도)다. selected model(선정 모델), runtime authority(런타임 권위), operating promotion(운영 승격), Goal Achieve(목표 달성)는 없다.
"""
    decision = f"""# {TODAY} Stage344D MT5 Probe Decision(344D MT5 탐침 결정)
...
Action(행동): directional long quality surface(방향성 롱 품질 표면) package(패키지)를 MT5 runtime probe(MT5 런타임 탐침)로 실행했다.
Effect(효과): run344E(344E 실행)가 KPI(핵심 성과 지표), runtime mapping effect(런타임 매핑 효과), proxy-MT5 diff(프록시-MT5 차이)를 판정할 수 있다.
...
run344D(344D 실행)는 directional long quality surface(방향성 롱 품질 표면)를 MT5 runtime probe(MT5 런타임 탐침)로 실행했다. run344E(344E 실행)는 결과를 검토해 positive clue(긍정 단서), failure memory(실패 기억), next offensive seed(다음 공격 탐색 씨앗)를 분리한다.
...
"""
    ...
    marker = f"run344D {RUN_ID}"
    HELPER.append_text_once_direct(STAGE_BRIEF, marker, f"""## run344D Directional Long Quality Surface MT5 Probe(344D 방향성 롱 품질 표면 MT5 탐침)
...
""")
    HELPER.append_text_once_direct(STAGE_README, marker, f"""## run344D Directional Long Quality Surface MT5 Probe(344D 방향성 롱 품질 표면 MT5 탐침)
...
""")
    changelog = f"""## {TODAY} run344D Directional Long Quality Surface MT5 Probe(방향성 롱 품질 표면 MT5 탐침)

- action(행동): directional long quality surface(방향성 롱 품질 표면) `{final['attempt_rows']}`개를 MT5 runtime probe(MT5 런타임 탐침)로 실행했다.
    ...
    HELPER.append_or_replace_csv_direct(
        ...
        ["run_id"],
        {
            **base_row,
            ...
            "notes": "Directional long quality surface(방향성 롱 품질 표면) MT5 runtime probe(런타임 탐침), review required(검토 필요).",
    ...
    rows = [
        ...
        },
        {
            **base_row,
            ...
            "guardrail_kpi": f"drawdown={final['best_max_drawdown_amount']};long_short={final['best_long_trade_count']}/{final['best_short_trade_count']}",
            "external_verification_status": final["external_verification_status"],
            "notes": "Tier B(티어 B)가 없으므로 combined(합산)은 Tier A(티어 A)와 같은 경계로 기록한다.",
