...
RUN_ID = "run306B_anti_surface_trade_shape_mt5_probe_v1"
RUN_NUMBER = "run306B"
SOURCE_RUN_ID = "run306A_design_anti_surface_trade_shape_rebuild_v1"
PARENT_RUN_ID = "run305C_review_runtime_realized_curve_attribution_mt5_probe_v1"
STATUS_PREPARED = "prepared_anti_surface_trade_shape_mt5_probe_no_runtime_kpi"
...
EXPLORATION_LABEL = "stage306_Model__AntiSurfaceTradeShapeReplay"
SIGNAL_COLUMN = "run306b_route_signal"
FEATURE_ORDER = (SIGNAL_COLUMN,)
COMMON_ROOT = "Project_Obsidian_Prime_v2/stage306/run306B_anti_surface_trade_shape"

STAGE_ROOT = ROOT / "stages" / STAGE_ID
RUN306A = STAGE_ROOT / "02_runs" / "run306A"
...
MT5_QUEUE = RUN306A / "mt5_probe_queue.csv"
SOURCE_MANIFEST = RUN306A / "candidate_payload_manifest.csv"
SOURCE_RUN_MANIFEST = RUN306A / "run_manifest.json"
SOURCE_REPORT = REVIEWS / "run306A_anti_surface_trade_shape_materialization_report.md"
PRODUCER = Path("stage_pipelines/stage306/execute_anti_surface_trade_shape_mt5_probe.py")
...
REPORT = REVIEWS / "run306B_anti_surface_trade_shape_mt5_probe_report.md"
...
def attach_identity(attempt: dict[str, Any], queue_row: Mapping[str, str], max_hold: int, close_on_flat: bool, same_reentry: int) -> None:
    ...
    attempt["stage306_branch_id"] = queue_row.get("materialized_branch_id", "")
    ...
    for queue_row in queue_rows:
        ...
        for runtime_split, split_token in (("validation_is", "val"), ("oos", "oos")):
            for tier_key, tier_label, tier_token in (
                ...
                attempt["signal_policy"] = "stage306 anti-surface trade-shape route_signal_value -1/0/+1 through single-feature table"
            ...
            if include_routed:
                ...
                attach_identity(attempt, queue_row, max_hold, close_on_flat, same_reentry)
                attempt["signal_policy"] = "Tier A primary + Tier B fallback stage306 anti-surface trade-shape route_signal_value"
                attempts.append(attempt)
    return attempts
...
def classify_status(result: Mapping[str, Any], materialize_only: bool) -> tuple[str, str, str, str]:
    ...
    if materialize_only:
        return (
            STATUS_PREPARED,
            "anti_surface_trade_shape_runtime_probe_prepared_no_external_execution",
            "out_of_scope_by_claim_materialize_only",
            "run306B_execute_anti_surface_trade_shape_mt5_probe_external_check",
        )
    ...
    if planned and completed_exec >= planned and len(kpis) >= planned:
        return (
            "completed_anti_surface_trade_shape_mt5_probe_no_selection",
            "runtime_probe_completed_requires_profit_scale_curve_review_no_selection",
            "completed",
            "run306C_review_anti_surface_trade_shape_mt5_probe",
        )
    if kpis:
        return (
            "partial_anti_surface_trade_shape_mt5_probe_no_selection",
            "runtime_probe_partial_requires_continuation_or_review_no_selection",
            "partial_or_blocked",
            "run306B_continue_anti_surface_trade_shape_mt5_probe" if limited else "run306C_review_with_runtime_gaps",
        )
    return (
        "blocked_anti_surface_trade_shape_mt5_probe_no_kpi",
        "runtime_probe_blocked_no_kpi_no_selection",
        "blocked_or_invalid",
        "run306B_repair_or_block_anti_surface_trade_shape_mt5_probe",
    )
...
def report_markdown(result: Mapping[str, Any], status: str, judgment: str, external_status: str, next_action: str) -> str:
    ...
    return "\n".join(
        [
            "# run306B Anti-Surface Trade Shape MT5 Probe(306B 반표면 거래 형태 MT5 탐침)",
            "",
            ...
            "",
            "Effect(효과): run306A(306A 실행)의 route_signal_value(경로 신호)를 실제 MT5 tester(MT5 테스터)에 넣어 trade count(거래 수), net profit(순수익), PF(수익 팩터), DD(drawdown, 손실폭), recovery(회복)를 확인했다.",
            "",
            f"`{BOUNDARY}`",
        ]
    )
...
def upsert_ledgers(result: Mapping[str, Any], status: str, judgment: str, external_status: str, next_action: str) -> None:
    ...
    safe_upsert_rows(
        ...
        [{"run_id": RUN_ID, "stage_id": STAGE_ID, "lane": "anti_surface_trade_shape_mt5_probe", "status": status, "judgment": judgment, "path": runner.prev.e295.e294.base.base.rel(REPORT), "notes": f"attempts={attempt_count};mt5_kpi_records={kpi_count};selected_candidate=none;onnx_readiness=not_claimed;next_action={next_action}."}],
        key="run_id",
    )
    safe_upsert_rows(
        ALPHA_LEDGER,
        ...
        [
            {
                ...
                "record_view": "anti_surface_trade_shape_mt5_probe",
...
def update_docs(status: str, judgment: str, next_action: str, kpi_count: int, attempt_count: int) -> None:
    ...
    selected = runner.prev.e295.e294.base.base.append_once(selected, "run306B_report", f"- run306B_report(306B 보고서): `{runner.prev.e295.e294.base.base.rel(REPORT)}`")
    selected = runner.prev.e295.e294.base.base.append_once(selected, "run306B_execution_result", f"- run306B_execution_result(306B 실행 결과): `{runner.prev.e295.e294.base.base.rel(EXECUTION_RESULT)}`")
    runner.prev.e295.e294.base.base.write_md(SELECTED, selected)

    review_index = runner.prev.e295.e294.base.base.io_path(REVIEW_INDEX).read_text(encoding="utf-8-sig") if runner.prev.e295.e294.base.base.path_exists(REVIEW_INDEX) else "# Stage306 Review Index(306단계 검토 색인)\n"
    review_index = runner.prev.e295.e294.base.base.append_once(
        review_index,
        "run306B_report",
        f"- run306B_report(306B 보고서): `{runner.prev.e295.e294.base.base.rel(REPORT)}`\n- run306B_execution_result(306B 실행 결과): `{runner.prev.e295.e294.base.base.rel(EXECUTION_RESULT)}`\n- run306B_mt5_kpi_summary(306B MT5 KPI 요약): `{runner.prev.e295.e294.base.base.rel(MT5_KPI_SUMMARY)}`",
    )
    ...
    current = runner.prev.e295.e294.base.base.append_once(
        current,
        "run306B_summary",
        f"- run306B_summary(306B 요약): anti-surface trade-shape MT5 probe(반표면 거래 형태 MT5 탐침)를 실행했다. Effect(효과): attempts(시도) `{attempt_count}`개와 MT5 KPI records(MT5 KPI 기록) `{kpi_count}`개를 만들었고 선택 후보/Adapter(어댑터)/ONNX(온엑스)는 주장하지 않는다.",
    )
    ...
    focus = (
        f"- >-\n"
        f"  Stage306(306단계) run306B(306B 실행) anti-surface trade-shape MT5 probe(반표면 거래 형태 MT5 탐침) `{RUN_ID}`. "
    ...
    changelog = runner.prev.e295.e294.base.base.append_once(
        changelog,
        RUN_ID,
        f"## {UPDATED_ON} run306B Anti-surface trade-shape MT5 probe(306B 반표면 거래 형태 MT5 탐침)\n\n"
        f"- status(상태): `{status}`\n"
        f"- judgment(판정): `{judgment}`\n"
