...
RUN_ID = "run306C_review_anti_surface_trade_shape_mt5_probe_v1"
RUN_NUMBER = "run306C"
SOURCE_RUN_ID = "run306B_anti_surface_trade_shape_mt5_probe_v1"
PARENT_RUN_ID = "run306A_design_anti_surface_trade_shape_rebuild_v1"
...
NEXT_REBUILD_STAGE_ID = "307_onnx_candidate_campaign__post_trade_shape_scale_rebuild"
NEXT_ADAPTER_STAGE_ID = "307_onnx_candidate_campaign__adapter_package_for_anti_surface_trade_shape"

STAGE_ROOT = ROOT / "stages" / STAGE_ID
RUN306A = STAGE_ROOT / "02_runs" / "run306A"
RUN306B = STAGE_ROOT / "02_runs" / "run306B"
...
SOURCE_EXECUTION = RUN306B / "execution_result.json"
SOURCE_KPI = RUN306B / "mt5_kpi_summary.csv"
SOURCE_SCOUT = RUN306A / "model_scout_scoreboard.csv"
SOURCE_MANIFEST = RUN306A / "candidate_payload_manifest.csv"
PRODUCER = Path("stage_pipelines/stage306/review_anti_surface_trade_shape_mt5_probe.py")

SCOREBOARD = RUN_ROOT / "anti_surface_trade_shape_review_scoreboard.csv"
...
NEXT_STAGE_QUEUE = RUN_ROOT / "stage307_seed_queue.csv"
RESULT_JUDGMENT = RUN_ROOT / "result_judgment.csv"
GATE_AUDIT = RUN_ROOT / "required_gate_coverage_audit.csv"
RUN_MANIFEST = RUN_ROOT / "run_manifest.json"
LINEAGE = RUN_ROOT / "artifact_lineage_receipt.json"
REPORT = REVIEWS / "run306C_anti_surface_trade_shape_review_stage307_open_report.md"
DECISION = ROOT / "docs" / "decisions" / "2026-05-24_stage306_anti_surface_trade_shape_review_stage307_open.md"
...
def feature_trading_days(attempt: Mapping[str, Any]) -> int:
    ...
    local = RUN306B / "features" / Path(feature_path).name
...
def build_scoreboard(rows: Sequence[Mapping[str, Any]], scout_rows: Mapping[str, Mapping[str, Any]]) -> tuple[list[dict[str, Any]], list[dict[str, Any]], list[dict[str, Any]]]:
    ...
    for candidate_id, split_rows in sorted(by_candidate.items()):
        ...
        row = {
            ...
            "adapter_package": "deferred_to_stage307" if selected else "none",
...
def selected_or_stage306_rows(scoreboard: Sequence[Mapping[str, Any]], selected_rows: Sequence[Mapping[str, Any]]) -> list[dict[str, Any]]:
    if selected_rows:
        return [
            {
                "queue_id": f"stage307_adapter_{index:02d}",
                "materialized_branch_id": row["materialized_branch_id"],
                "package_id": row["package_id"],
                ...
                "next_action": "run307A_build_adapter_package_for_anti_surface_trade_shape",
                "claim_boundary": BOUNDARY,
            }
    ...
    return [
        {
            "queue_id": f"stage307_rebuild_seed_{index:02d}",
            "materialized_branch_id": row["materialized_branch_id"],
            "package_id": row["package_id"],
            "queue_role": "anti_surface_trade_shape_seed",
            "validation_net_profit": row["validation_net_profit"],
            "oos_net_profit": row["oos_net_profit"],
            "combined_net_profit": row["combined_net_profit"],
            ...
            "next_action": "run307A_design_post_trade_shape_scale_rebuild_packet",
            "claim_boundary": BOUNDARY,
        }
...
def scaffold_stage306(selected_rows: Sequence[Mapping[str, Any]]) -> tuple[str, str, str, str]:
    if selected_rows:
        ...
        next_status = "opened_adapter_package_for_anti_surface_trade_shape"
        ...
        effect = "Stage306 candidate gate passed; Stage307 prepares Adapter package before any ONNX export."
    else:
        next_stage_id = NEXT_REBUILD_STAGE_ID
        next_status = "opened_post_trade_shape_scale_rebuild"
        next_judgment = "opened_from_stage306_no_onnx_worthy_candidate"
        next_action = "run307A_design_post_trade_shape_scale_rebuild_packet"
        question = "Can a new profit-scale source overcome Stage306 trade-shape failure without repeating narrow repair?"
        effect = "Stage306 evidence becomes failure memory and seed data for a fresh post-trade-shape rebuild."
    ...
    write_text(
        ...
        "\n".join(
            [
                "# Stage307 Selection Status(307단계 선택 상태)",
                "",
                ...
                f"- stage306_review(306단계 검토): `{rel(REPORT)}`",
            ]
        ),
    )
    write_text(
        ...
        "\n".join(
            [
                "# Stage307 Review Index(307단계 검토 색인)",
                "",
    ...
    write_csv(
        ...
        [
            {
                "row_id": "stage307_opened_from_run306C",
...
def report_markdown(scoreboard: Sequence[Mapping[str, Any]], failures: Sequence[Mapping[str, Any]], selected_rows: Sequence[Mapping[str, Any]], next_stage_id: str, next_action: str) -> str:
    ...
    lines = [
        ...
        f"- selected_candidate(선택 후보): `{selected_rows[0]['package_id'] if selected_rows else 'none'}`",
        f"- Adapter package(어댑터 패키지): `{'deferred_to_stage307' if selected_rows else 'none'}`",
        "- ONNX readiness(온엑스 준비): `not_started`",
        "- Goal Achieve(목표 달성): `not_claimed`",
...
def update_docs(status: str, judgment: str, next_stage_id: str, next_action: str, selected_rows: Sequence[Mapping[str, Any]]) -> None:
    ...
    selected = replace_line(selected, "- Adapter package(", f"- Adapter package(어댑터 패키지): `{'deferred_to_stage307' if selected_rows else 'none'}`")
    selected = replace_line(selected, "- next_action(", f"- next_action(다음 행동): `{next_action}`")
    selected += f"- run306C_report(306C 보고): `{rel(REPORT)}`\n"
    selected += f"- stage307_opened(307단계 열림): `{next_stage_id}`\n"
    write_text(SELECTED, selected)

    review_index = read_text(REVIEW_INDEX)
    review_index += f"- run306C_report(306C 보고): `{rel(REPORT)}`\n"
    review_index += f"- run306C_scoreboard(306C 점수표): `{rel(SCOREBOARD)}`\n"
    review_index += f"- stage307_seed_queue(307단계 씨앗 대기열): `{rel(NEXT_STAGE_QUEUE)}`\n"
    ...
    current = current.rstrip() + f"\n- run306C_summary(306C 요약): Stage306(306단계) actual MT5(실제 메타트레이더5) 검토를 완료했다. Effect(효과): selected_candidate(선택 후보)는 `{selected_rows[0]['package_id'] if selected_rows else 'none'}`이고 next_stage(다음 단계)는 `{next_stage_id}`다.\n"
    ...
    focus = (
        f"- >-\n"
        f"  Stage306(306단계) run306C(306C 실행) anti-surface trade-shape review(반표면 거래 형태 검토) `{RUN_ID}` closed Stage306 and opened `{next_stage_id}`. "
        f"Effect(효과): selected candidate(선택 후보)는 `{selected_rows[0]['package_id'] if selected_rows else 'none'}`이고 Adapter package(어댑터 패키지)는 `{'deferred_to_stage307' if selected_rows else 'none'}`, ONNX readiness(온엑스 준비)는 `not_started`다.\n"
    )
    ...
    changelog += (
        f"\n## {UPDATED_ON} run306C Anti-surface trade-shape review(306C 반표면 거래 형태 검토)\n\n"
        f"- status(상태): `{status}`\n"
        f"- judgment(판정): `{judgment}`\n"
        f"- effect(효과): Stage306(306단계)를 닫고 `{next_stage_id}`를 열었다.\n"
...
def update_registers(status: str, judgment: str, next_action: str) -> None:
    safe_upsert(
        ...
        [{"run_id": RUN_ID, "stage_id": STAGE_ID, "lane": "anti_surface_trade_shape_review", "status": status, "judgment": judgment, "path": rel(REPORT), "notes": f"selected_candidate_reviewed;next_action={next_action}."}],
        "run_id",
    )
    safe_upsert(
        ALPHA_LEDGER,
        ...
        [
            {
                ...
                "record_view": "anti_surface_trade_shape_review",
    ...
    safe_upsert(
        STAGE_LEDGER,
        STAGE_LEDGER_COLUMNS,
        [{"row_id": f"{RUN_ID}__review", "stage_id": STAGE_ID, "run_id": RUN_ID, "view": "anti_surface_trade_shape_review", "tier_scope": "Tier A used/Tier B fallback/actual routed total", "scoreboard": "anti_surface_trade_shape_review_scoreboard", "status": status, "judgment": judgment, "evidence_boundary": "runtime_probe_review_no_onnx", "report_path": rel(REPORT), "notes": "Stage307 opened if no selected candidate; ONNX not started."}],
        "row_id",
    )
...
def update_memory_registers(failures: Sequence[Mapping[str, Any]], selected_rows: Sequence[Mapping[str, Any]]) -> None:
    ...
    if failures:
        ...
        if RUN_ID not in negative:
            negative += (
                f"\n## {RUN_ID} Stage306 anti-surface trade-shape failure memory(306단계 반표면 거래 형태 실패 기억)\n\n"
        ...
        rows.append(
            {
                ...
                "artifact_type": "stage306_anti_surface_trade_shape_review_artifact",
                "path": rel(path),
                "sha256": ledger.sha256_file_lf_normalized(path),
                "stage_id": STAGE_ID,
                "run_id": RUN_ID,
                "created_at_utc": "2026-05-24T22:00:00Z",
                "notes": "Stage306 review and Stage307 open handoff",
            }
        )
...
def main() -> None:
    ...
    status = "completed_anti_surface_trade_shape_review_stage307_opened"
    judgment = (
        "actual_mt5_candidate_gate_passed_adapter_stage_opened"
        if selected_rows
        else "actual_mt5_no_onnx_worthy_candidate_post_trade_shape_rebuild_opened"
    )
    ...
    write_csv(
        ...
        [{"run_id": RUN_ID, "status": status, "judgment": judgment, "selected_candidate": selected_rows[0]["package_id"] if selected_rows else "none", "adapter_package": "deferred_to_stage307" if selected_rows else "none", "onnx_readiness": "not_started", "next_action": next_action, "claim_boundary": BOUNDARY}],
    )
    gate_rows = [
    ...
    manifest = {
        ...
        "adapter_package": "deferred_to_stage307" if selected_rows else "none",
        "onnx_readiness": "not_started",
    ...
    write_text(
        ...
        "\n".join(
            [
                ...
                "",
                "Effect(효과): 실제 MT5(메타트레이더5) report source_path(보고서 원천 경로)로 곡선 판정을 복구했고, 후보가 없으면 다음 단계에서 새 수익 규모 원천으로 방향 전환한다.",
            ]
        ),
    )
