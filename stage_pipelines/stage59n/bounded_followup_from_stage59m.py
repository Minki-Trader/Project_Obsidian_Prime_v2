...
SOURCE_STAGE59M_ID = "59M_adapter_repair__bounded_followup_from_stage59l"
STAGE59N_ID = "59N_adapter_repair__bounded_followup_from_stage59m"
NEXT_REPAIR_STAGE_ID = "59O_adapter_repair__bounded_followup_from_stage59n"
RUN_NUMBER = "run59I"
RUN_ID = "run59I_stage59n_bounded_followup_from_stage59m_v1"
PACKET_ID = "stage59n_bounded_followup_from_stage59m_v1"
PARENT_RUN_ID = "run59H_stage59m_bounded_followup_from_stage59l_v1"
SOURCE_ADAPTER_ID = "s59m_v62_sl20_tp30_sd5"
DEVELOPMENT_ANCHOR = "v64_v47_ctxgap14_refill_etfw_h2_no_b"
BACKUP_ANCHOR = "v60_v47_et_stable_damage_firewall_h2c0_no_b"
SOURCE_STAGE59M_PUSHED_COMMIT = "c170060f33d0bc3c06f60ca39d6fc59d90cba8a4"
BOUNDARY = (
    "research_development_only_no_live_readiness_no_runtime_authority_"
    "no_operating_promotion_no_operating_reference_no_production_baseline_no_deployment"
)

STAGE_ROOT = Path("stages") / STAGE59N_ID
...
SOURCE_STAGE59M_ROOT = Path("stages") / SOURCE_STAGE59M_ID
SOURCE_STAGE59M_DECISION = SOURCE_STAGE59M_ROOT / "03_reviews/stage59m_decision.md"
SOURCE_STAGE59M_REPORT = SOURCE_STAGE59M_ROOT / "03_reviews/bounded_followup_from_stage59l_report.md"
SOURCE_STAGE59M_SUMMARY = SOURCE_STAGE59M_ROOT / "03_reviews/bounded_followup_summary.csv"
SOURCE_STAGE59M_JSON = SOURCE_STAGE59M_ROOT / "03_reviews/bounded_followup_summary.json"
...
COMMON_ROOT = f"Project_Obsidian_Prime_v2/s59n/{RUN_NUMBER}"

REPORT_PATH = REVIEWS_ROOT / "bounded_followup_from_stage59m_report.md"
...
DECISION_PATH = REVIEWS_ROOT / "stage59n_decision.md"
AUDIT_CSV_PATH = REVIEWS_ROOT / "stage59n_trade_audit.csv"
...
STAGE59N_VARIANTS = (
    repair.RepairVariant(
        adapter_id="s59n_v62_sl20_tp30_sd8",
        label="run50BQ_v62_transition_h4_sl20_tp30_sd8_risk3pct_atr",
        atr_enabled=True,
        ...
        max_hold_bars=4,
        notes="Stage59N bounded follow-up: keep Stage59M bracket and raise same-direction cooldown to 8 bars.",
    ),
    repair.RepairVariant(
        adapter_id="s59n_v62_sl20_tp30_sd10",
        label="run50BQ_v62_transition_h4_sl20_tp30_sd10_risk3pct_atr",
        atr_enabled=True,
        model_risk_enabled=True,
        fixed_lot=0.25,
        atr_stop_multiplier=2.0,
        ...
        close_only_on_opposite_signal=True,
        max_hold_bars=4,
        notes="Stage59N bounded follow-up: keep Stage59M bracket and raise same-direction cooldown to 10 bars.",
    ),
    repair.RepairVariant(
        adapter_id="s59n_v62_sl20_tp30_sd12",
        label="run50BQ_v62_transition_h4_sl20_tp30_sd12_risk3pct_atr",
        atr_enabled=True,
        model_risk_enabled=True,
        fixed_lot=0.25,
        atr_stop_multiplier=2.0,
        ...
        close_only_on_opposite_signal=True,
        max_hold_bars=4,
        notes="Stage59N bounded follow-up: keep Stage59M bracket and raise same-direction cooldown to 12 bars.",
    ),
)
...
def source_specs() -> dict[str, dict[str, Any]]:
    ...
    return {variant.adapter_id: dict(v62) for variant in STAGE59N_VARIANTS}


def configure_reused_engine() -> None:
    engine.STAGE59_ID = STAGE59N_ID
    ...
    engine.STAGE59_VARIANTS = STAGE59N_VARIANTS
    engine.SOURCE_SPECS = source_specs()
    engine.MODEL_RISK_MIN_PCT = {variant.adapter_id: 0.005 for variant in STAGE59N_VARIANTS}

    repair.STAGE_ID = STAGE59N_ID
    repair.RUN_NUMBER = RUN_NUMBER
    repair.RUN_ID = RUN_ID
    repair.RUN_ROOT = RUN_ROOT
    repair.REPAIR_VARIANTS = STAGE59N_VARIANTS
    s58.STAGE58_ID = STAGE59N_ID
    ...
    s58.STAGE58_VARIANTS = STAGE59N_VARIANTS
...
def build_attempts(inputs: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    for variant_index, variant in enumerate(STAGE59N_VARIANTS, start=1):
            ...
            for role_index, (tier, attempt_role, prefix, attempt_token) in enumerate(
                ...
                attempts.append(
                    attempt_payload(
                        ...
                        run_id=RUN_ID,
                        stage_number=59,
                        exploration_label="stage59N_BaselineAdapter__BoundedFollowupFromStage59M",
...
def execute_prepared_run_checkpointed(prepared: Mapping[str, Any], args: argparse.Namespace) -> dict[str, Any]:
    ...
    return {
        **dict(prepared),
        ...
        "judgment": "inconclusive_stage59n_runtime_probe_completed" if completed and report_completed else "blocked_stage59n_runtime_probe",
    }
...
def next_stage_for_decision(decision: str) -> str:
    ...
    if decision == "demote_current_adapter_and_select_backup":
        return "59O_adapter_repair__backup_or_demote_from_stage59n"
    if decision == "open_bounded_followup":
        return "59O_adapter_repair__bounded_followup_from_stage59n"
...
def next_packet_for_decision(decision: str) -> tuple[str, str]:
    ...
    return "stage59o_bounded_followup_from_stage59n_v1", "run59J_stage59o_bounded_followup_from_stage59n_v1"
            ...
            rows.append(
                {
                    ...
                    "stage_id": STAGE59N_ID,
                    "run_id": RUN_ID,
                    "artifact_type": "stage59n_bounded_followup_evidence",
                    "path": rel(path),
                    "sha256": sha256_file_lf_normalized(path),
                    "created_at_utc": created,
                    "notes": "Stage59N bounded follow-up from Stage59M artifact.",
                }
            )
            ...
            rows.append(
                {
                    ...
                    "created_at_utc": created,
                    "notes": "Actual Stage59N MT5 Strategy Tester HTML report.",
                }
            )
    return rows
...
def write_run_identity(result: Mapping[str, Any]) -> None:
    engine.write_json(
        RUN_ROOT / "run_manifest.json",
        {
            ...
            "bounded_question": "Can bounded same-direction cooldown pressure on the Stage59M bracket candidate repair OOS PF and cost-stressed expectancy without starting ONNX?",
            "source_stage59m_decision": rel(SOURCE_STAGE59M_DECISION),
            "source_stage59m_pushed_commit": SOURCE_STAGE59M_PUSHED_COMMIT,
            "variants": [
                {
                    **variant.__dict__,
                ...
                for variant in STAGE59N_VARIANTS
            ],
...
def write_ledgers(
        ...
        [
            {
                "run_id": RUN_ID,
                "stage_id": STAGE59N_ID,
                "lane": "baseline_adapter_bounded_followup_from_stage59m",
                "status": status,
                "judgment": decision,
    ...
    ledger_rows.append(
        {
            "ledger_row_id": f"{RUN_ID}__aggregate_bounded_followup_from_stage59m",
            "stage_id": STAGE59N_ID,
            "run_id": RUN_ID,
            "subrun_id": "aggregate_bounded_followup_from_stage59m",
            "parent_run_id": PARENT_RUN_ID,
            "record_view": "bounded_followup_from_stage59m",
            "tier_scope": "Tier A+B",
            ...
            "external_verification_status": external,
            "notes": "Stage59N bounded same-direction cooldown follow-up from Stage59M evidence; not final package completion.",
        }
    )
...
def report_markdown(summary_rows: Sequence[Mapping[str, Any]], decision: str, reasons: Sequence[str], external: str) -> str:
    ...
    best = best_repaired_variant(summary_rows)
    return f"""# Stage59N Bounded Followup From Stage59M Report(59N단계 59M단계 기반 경계 후속 보고서)

- stage(단계): `{STAGE59N_ID}`
- run(실행): `{RUN_ID}`
...
def equity_audit_markdown(summary_rows: Sequence[Mapping[str, Any]], segment_rows: Sequence[Mapping[str, Any]]) -> str:
    ...
    return f"""# Stage59N Bounded Followup Equity Curve Audit(59N단계 경계 후속 자금 곡선 감사)
...
def decision_markdown(decision: str, reasons: Sequence[str], best: Mapping[str, Any], external: str) -> str:
    return f"""# Stage59N Decision(59N단계 판정)

decision(판정): `{decision}`

Stage59N(59N단계)는 Stage59M evidence(59M단계 근거)를 사용한 bounded follow-up(경계 후속)으로 기록한다. Effect(효과): same-direction cooldown(동일 방향 쿨다운)의 성공/실패를 다음 bounded stage(경계 다음 단계)의 입력 근거로 넘긴다.
...
Stage59N closeout(59N단계 종료)는 overall goal completion(전체 목표 완료)이 아니다. Effect(효과): Stage60 ONNX hardening(60단계 ONNX 경화)은 adapter quality(어댑터 품질)가 강할 때만 열린다.
    ...
    engine.write_json(
        SUMMARY_JSON_PATH,
        {
            ...
            "required_outputs": {
                ...
                "stage59n_decision": rel(DECISION_PATH),
            },
...
def write_packet_files(
    ...
    files = {
        ...
        "experiment_design_receipt.json": {
            "hypothesis": "A bounded same-direction cooldown follow-up on the Stage59M bracket candidate may improve OOS PF and cost-stressed expectancy while preserving validation behavior.",
            ...
            "comparison_baseline": "Stage59M best candidate s59m_v62_sl20_tp30_sd5.",
            ...
            "changed_variables": ["same_direction_reentry_cooldown_bars"],
            ...
            "stop_conditions": "three bounded Stage59N same-direction cooldown variants only",
        ...
        "artifact_lineage_audit.json": {
            "source_inputs": [rel(SOURCE_STAGE59M_DECISION), rel(SOURCE_STAGE59M_REPORT), rel(SOURCE_STAGE59M_SUMMARY), rel(SOURCE_STAGE59M_JSON)],
...
- stage_id(단계 ID): `{STAGE59N_ID}`
- source_stage(원천 단계): `{SOURCE_STAGE59M_ID}`
- source_decision(원천 판정): `continue_repair_in_new_bounded_stage`
- bounded_question(경계 질문): `Can bounded same-direction cooldown pressure on the Stage59M bracket candidate repair OOS PF and cost-stressed expectancy without starting ONNX?`
- boundary(경계): `{BOUNDARY}`

Stage59N(59N단계)는 Stage59M(59M단계) bracket candidate(브래킷 후보)를 유지하고 same-direction cooldown(동일 방향 쿨다운) 세 변형만 측정한다. Effect(효과): Stage60 ONNX(60단계 ONNX)나 deployment(배포)로 이어가지 않고 OOS 약점 수리 여부만 본다.
""",
    )
    engine.write_md(
...
- stage59m_decision(59M단계 판정): `{rel(SOURCE_STAGE59M_DECISION)}`
- stage59m_report(59M단계 보고서): `{rel(SOURCE_STAGE59M_REPORT)}`
- stage59m_summary(59M단계 요약): `{rel(SOURCE_STAGE59M_SUMMARY)}`
- stage59m_pushed_commit(59M단계 푸시 커밋): `{SOURCE_STAGE59M_PUSHED_COMMIT}`
...
Effect(효과): Stage59N(59N단계)는 Stage59M(59M단계) 실패 기억과 run50BQ(run50BQ) v62 전환 원천을 명시적으로 연결한다.
""",
    )
    engine.write_md(
...
- stage_status(단계 상태): `closed_bounded_followup_from_stage59m`
- source_stage(원천 단계): `{SOURCE_STAGE59M_ID}`
- source_decision(원천 판정): `continue_repair_in_new_bounded_stage`
- stage59n_decision(59N단계 판정): `{decision}`
- next_stage_or_branch(다음 단계/분기): `{next_stage}`
- selected_research_baseline(선택 연구 기준선): `none`
- claim_boundary(주장 경계): `{BOUNDARY}`

Effect(효과): Stage59N(59N단계)는 측정 결과를 보존하지만 final package(최종 패키지)나 operating claim(운영 주장)을 만들지 않는다.
""",
    )
    engine.write_md(
        REVIEWS_ROOT / "review_index.md",
        f"""# Stage59N Review Index(59N단계 검토 색인)
...
- bounded_followup_risk_atr_telemetry(경계 후속 위험/ATR 텔레메트리): `{rel(RISK_ATR_TELEMETRY_PATH)}`
- stage59n_decision(59N단계 판정): `{rel(DECISION_PATH)}`
""",
    )
...
- source_stage(원천 단계): `{STAGE59N_ID}`
- source_decision(원천 판정): `{decision}`
- bounded_question(경계 질문): `What bounded follow-up should repair, demote, or branch from the Stage59N evidence without starting ONNX prematurely?`
- boundary(경계): `{BOUNDARY}`

Stage59O(59O단계)는 Stage59N(59N단계) 결과가 Stage60 ONNX(60단계 ONNX)에 충분하지 않을 때 이어지는 bounded follow-up(경계 후속)이다. Effect(효과): repair(수리), demotion(강등), branch(분기)를 다음 작은 질문으로 제한한다.
""",
        )
        engine.write_md(
...
- stage59n_decision(59N단계 판정): `{rel(DECISION_PATH)}`
- stage59n_report(59N단계 보고서): `{rel(REPORT_PATH)}`
- stage59n_summary(59N단계 요약): `{rel(SUMMARY_CSV_PATH)}`
- stage59n_segment_kpi(59N단계 구간 KPI): `{rel(SEGMENT_KPI_PATH)}`
- stage59n_risk_atr_telemetry(59N단계 위험/ATR 텔레메트리): `{rel(RISK_ATR_TELEMETRY_PATH)}`
""",
        )
        engine.write_md(
            next_root / "03_reviews/review_index.md",
            """# Stage59O Review Index(59O단계 검토 색인)

Stage59O(59O단계)는 planned(계획) 상태다. Effect(효과): Stage59N(59N단계) 약점을 다음 bounded repair(경계 수리) 근거로 넘긴다.
""",
        )
        engine.write_md(
...
- stage_status(단계 상태): `active_planned_from_stage59n`
- source_stage(원천 단계): `{STAGE59N_ID}`
- source_decision(원천 판정): `{decision}`
- selected_research_baseline(선택 연구 기준선): `none`
- claim_boundary(주장 경계): `{BOUNDARY}`

Effect(효과): Stage59O(59O단계)는 Stage59N(59N단계) 이후 필요한 다음 작은 질문만 다룬다.
""",
        )
...
- status(상태): `stage59n_closed_{decision}`
- claim_boundary(주장 경계): research/development only(연구/개발 전용)

Stage59N(59N단계) closed(종료) as bounded same-direction cooldown follow-up from Stage59M(59M단계 기반 경계 동일 방향 쿨다운 후속). Effect(효과): Stage59M(59M단계) OOS weakness(OOS 약점)는 보존하고, Stage60 ONNX(60단계 ONNX)는 품질 근거가 강할 때만 열린다.

## Latest Stage59N Evidence(최신 59N단계 근거)
...
""",
    ...
    focus = (
        "current_focus:\n"
        f"- >-\n"
        f"  Stage59N(59N단계) `{STAGE59N_ID}` closed(종료) as bounded same-direction cooldown follow-up from Stage59M(59M단계 기반 경계 동일 방향 쿨다운 후속); decision(판정)=`{decision}`. "
        ...
        f"- >-\n"
        f"  Next stage_or_branch(다음 단계/분기) `{next_stage}` is active/planned(활성/계획). Effect(효과): Stage59N(59N단계) 결과를 다음 bounded step(경계 다음 단계)으로 넘긴다.\n"
    )
    text = re.sub(
...
stage59n_bounded_followup_from_stage59m:
  packet_id: {PACKET_ID}
  stage_id: {STAGE59N_ID}
  status: closed_bounded_followup_from_stage59m
  ...
  boundary: {BOUNDARY}
"""
    if "stage59n_bounded_followup_from_stage59m:" in text:
        text = re.sub(r"\nstage59n_bounded_followup_from_stage59m:\n(?:  .*\n)*", block, text, count=1)
    else:
        text += block
...
def parse_args(argv: Sequence[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run Stage59N bounded follow-up from Stage59M.")
...
def load_existing_result() -> dict[str, Any]:
    ...
    if not path_exists(manifest) or not path_exists(kpi):
        raise FileNotFoundError("Stage59N existing run_manifest.json or kpi_record.json is missing")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.finalize_existing:
        ...
        prepared = {
            ...
            "model_family": "baseline_adapter_stage59n_bounded_followup_ebm_table",
            "feature_set_id": "stage59n_v62_same_direction_cooldown_discrete_signal",
