...
RUN_ID = "run312B_execute_fresh_model_asymmetry_mt5_probe_v1"
RUN_NUMBER = "run312B"
SOURCE_RUN_ID = "run312A_design_fresh_model_asymmetry_rebuild_packet_v1"
PARENT_RUN_ID = "run311C_review_post_allocation_fresh_edge_mt5_probe_v1"
STATUS_PREPARED = "prepared_fresh_model_asymmetry_mt5_probe_no_runtime_kpi"
UPDATED_ON = "2026-05-24"
EXPLORATION_LABEL = "stage312_Model__FreshModelAsymmetryReplay"
SIGNAL_COLUMN = "run312b_route_signal"
FEATURE_ORDER = (SIGNAL_COLUMN,)
COMMON_ROOT = "Project_Obsidian_Prime_v2/stage312/run312B_fresh_model_asymmetry"

STAGE_ROOT = ROOT / "stages" / STAGE_ID
RUN312A = STAGE_ROOT / "02_runs" / "run312A"
...
MT5_QUEUE = RUN312A / "mt5_probe_queue.csv"
SOURCE_MANIFEST = RUN312A / "candidate_payload_manifest.csv"
SOURCE_RUN_MANIFEST = RUN312A / "run_manifest.json"
SOURCE_REPORT = REVIEWS / "run312A_materialization.md"
PRODUCER = Path("stage_pipelines/stage312/execute_fresh_model_asymmetry_mt5_probe.py")
...
REPORT = REVIEWS / "run312B_mt5_probe.md"
...
def classify_status(result: Mapping[str, Any], materialize_only: bool) -> tuple[str, str, str, str]:
    ...
    if materialize_only:
        return (
            STATUS_PREPARED,
            "fresh_model_asymmetry_runtime_probe_prepared_no_external_execution",
            "out_of_scope_by_claim_materialize_only",
            "run312B_execute_fresh_model_asymmetry_mt5_probe_external_check",
        )
    ...
    if planned and completed_exec >= planned and len(kpis) >= planned:
        return (
            "completed_fresh_model_asymmetry_mt5_probe_no_selection",
            "runtime_probe_completed_requires_profit_scale_curve_review_no_selection",
            "completed",
            "run312C_review_fresh_model_asymmetry_mt5_probe",
        )
    if kpis:
        return (
            "partial_fresh_model_asymmetry_mt5_probe_no_selection",
            "runtime_probe_partial_requires_continuation_or_review_no_selection",
            "partial_or_blocked",
            "run312B_continue_fresh_model_asymmetry_mt5_probe" if limited else "run312C_review_with_runtime_gaps",
        )
    return (
        "blocked_fresh_model_asymmetry_mt5_probe_no_kpi",
        "runtime_probe_blocked_no_kpi_no_selection",
        "blocked_or_invalid",
        "run312B_repair_or_block_fresh_model_asymmetry_mt5_probe",
    )
...
def report_markdown(result: Mapping[str, Any], status: str, judgment: str, external_status: str, next_action: str) -> str:
    ...
    return "\n".join(
        [
            "# run312B Fresh Model Asymmetry MT5 Probe(312B 새 모델 비대칭 MT5 탐침)",
            "",
            ...
            "",
            "Effect(효과): run312A(312A 실행)의 fresh model asymmetry route_signal_value(새 모델 비대칭 경로 신호)를 MT5 tester(MT5 테스터)에 넣어 실제 거래 수, 순수익, 수익 곡선을 확인했다.",
            "",
            f"`{e310.BOUNDARY}`",
        ]
    )
...
def upsert_ledgers(result: Mapping[str, Any], status: str, judgment: str, external_status: str, next_action: str) -> None:
    ...
    e310.safe_upsert_rows(
        ...
        [{"run_id": RUN_ID, "stage_id": STAGE_ID, "lane": "fresh_model_asymmetry_mt5_probe", "status": status, "judgment": judgment, "path": base.rel(REPORT), "notes": f"attempts={attempt_count};mt5_kpi_records={kpi_count};selected_candidate=none;onnx_readiness=not_claimed;next_action={next_action}."}],
        key="run_id",
    )
    e310.safe_upsert_rows(
        ALPHA_LEDGER,
    ...
    e310.safe_upsert_rows(
        ...
        [{"row_id": f"{RUN_ID}__mt5_probe", "stage_id": STAGE_ID, "run_id": RUN_ID, "view": "fresh_model_asymmetry_mt5_probe", "tier_scope": "Tier A used/Tier B fallback/actual routed total", "scoreboard": "runtime_probe", "status": status, "judgment": judgment, "evidence_boundary": "runtime_probe_no_candidate_no_onnx", "report_path": base.rel(REPORT), "notes": f"attempts={attempt_count};mt5_kpi_records={kpi_count}."}],
        key="row_id",
    )
...
def update_docs(status: str, judgment: str, next_action: str, kpi_count: int, attempt_count: int) -> None:
    ...
    selected = base.append_once(selected, "run312B_report", f"- run312B_report(312B 보고서): `{base.rel(REPORT)}`")
    selected = base.append_once(selected, "run312B_execution_result", f"- run312B_execution_result(312B 실행 결과): `{base.rel(EXECUTION_RESULT)}`")
    e310.e309.e308.safe_write_md(SELECTED, selected)

    review_index = base.io_path(REVIEW_INDEX).read_text(encoding="utf-8-sig") if base.path_exists(REVIEW_INDEX) else "# Stage312 Review Index(312단계 검토 색인)\n"
    review_index = base.append_once(
        review_index,
        "run312B_report",
        f"- run312B_report(312B 보고서): `{base.rel(REPORT)}`\n- run312B_execution_result(312B 실행 결과): `{base.rel(EXECUTION_RESULT)}`\n- run312B_mt5_kpi_summary(312B MT5 핵심 성과 지표 요약): `{base.rel(MT5_KPI_SUMMARY)}`",
    )
    e310.e309.e308.safe_write_md(REVIEW_INDEX, review_index)
    ...
    current = base.append_once(current, "run312B_summary", f"- run312B_summary(312B 요약): fresh model asymmetry MT5 probe(새 모델 비대칭 MT5 탐침)를 실행했다. Effect(효과): attempts(시도) `{attempt_count}`개와 MT5 KPI records(MT5 핵심 성과 지표 기록) `{kpi_count}`개를 만들었고 선택 후보/Adapter(어댑터)/ONNX(온엑스)는 주장하지 않는다.")
    e310.e309.e308.safe_write_md(CURRENT_STATE, current)
    ...
    focus = (
        f"- >-\n"
        f"  Stage312(312단계) run312B(312B 실행) fresh model asymmetry MT5 probe(새 모델 비대칭 MT5 탐침) `{RUN_ID}`. "
    ...
    e310.e309.e308.safe_write_md(WORKSPACE_STATE, workspace)
    ...
    changelog = base.append_once(
        changelog,
        RUN_ID,
        f"## {UPDATED_ON} run312B Fresh model asymmetry MT5 probe(312B 새 모델 비대칭 MT5 탐침)\n\n"
        f"- status(상태): `{status}`\n"
        f"- judgment(판정): `{judgment}`\n"
    ...
    e310.e309.e308.safe_write_md(CHANGELOG, changelog)
