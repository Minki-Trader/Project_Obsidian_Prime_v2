...
RUN_ID = "run312C_review_fresh_model_asymmetry_mt5_probe_v1"
RUN_NUMBER = "run312C"
SOURCE_RUN_ID = "run312B_execute_fresh_model_asymmetry_mt5_probe_v1"
PARENT_RUN_ID = "run312A_design_fresh_model_asymmetry_rebuild_packet_v1"
UPDATED_ON = "2026-05-24"
BOUNDARY = r310.BOUNDARY

NEXT_REBUILD_STAGE_ID = "313_onnx_candidate_campaign__runtime_outcome_source_pivot_rebuild"
NEXT_ADAPTER_STAGE_ID = "313_onnx_candidate_campaign__adapter_package_for_fresh_model_asymmetry"

STAGE_ROOT = ROOT / "stages" / STAGE_ID
RUN312B = STAGE_ROOT / "02_runs" / "run312B"
...
SOURCE_KPI = RUN312B / "mt5_kpi_summary.csv"
SOURCE_ATTEMPT_SUMMARY = RUN312B / "attempt_summary.csv"
PRODUCER = Path("stage_pipelines/stage312/review_fresh_model_asymmetry_mt5_probe.py")

SCOREBOARD = RUN_ROOT / "fresh_model_asymmetry_review_scoreboard.csv"
...
NEXT_STAGE_QUEUE = RUN_ROOT / "stage313_seed_queue.csv"
RESULT_JUDGMENT = RUN_ROOT / "result_judgment.csv"
GATE_AUDIT = RUN_ROOT / "required_gate_coverage_audit.csv"
RUN_MANIFEST = RUN_ROOT / "run_manifest.json"
LINEAGE = RUN_ROOT / "artifact_lineage_receipt.json"
REPORT = REVIEWS / "run312C_review_stage313_open.md"
DECISION = ROOT / "docs" / "decisions" / "2026-05-24_stage312_fresh_model_asymmetry_review_stage313_open.md"
...
def next_stage_rows(scoreboard: Sequence[Mapping[str, Any]]) -> list[dict[str, Any]]:
    ...
    return [
        {
            "seed_id": "stage312_fresh_model_asymmetry_review_seed",
            "source_candidates": ";".join(row["package_id"] for row in positive[:3]) or "none",
            "fresh_thesis": "runtime_outcome_source_pivot_rebuild",
            "use_as": "failure_memory_or_candidate_seed",
            "upside": "fresh model asymmetry may reveal a profitable direction source, but only if actual MT5 scale and curve survive.",
            "failure_mode": "fails if validation/OOS, density, efficiency, or curve cannot pass together.",
            "discard_condition": "discard narrow direction-table repair; require a new runtime outcome source pivot if no gate pass.",
            "next_action": "run313A_design_runtime_outcome_source_pivot_rebuild_packet",
        }
    ]
...
def scaffold_next_stage(selected_rows: Sequence[Mapping[str, Any]]) -> tuple[str, str, str, str]:
    if selected_rows:
        ...
        question = "selected fresh model asymmetry(선택된 새 모델 비대칭)를 Adapter package(어댑터 패키지)로 정리할 수 있는가?"
        status = "opened_adapter_package_stage_after_stage312_candidate_selection"
        judgment = "candidate_selected_adapter_package_stage_opened_no_onnx_yet"
        next_action = "run313A_build_fresh_model_asymmetry_adapter_package"
    else:
        next_stage_id = NEXT_REBUILD_STAGE_ID
        question = "Stage312 fresh model asymmetry(312단계 새 모델 비대칭) 실패 기억을 넘어 runtime outcome source pivot(런타임 결과 원천 전환)을 만들 수 있는가?"
        status = "opened_runtime_outcome_source_pivot_after_stage312_no_selection"
        judgment = "no_candidate_selected_runtime_outcome_source_pivot_stage_opened"
        next_action = "run313A_design_runtime_outcome_source_pivot_rebuild_packet"
    ...
    write_text(
        ...
        "\n".join(
            [
                ...
                "",
                "Effect(효과): Stage312(312단계)의 결과를 후보 확정이 아니라 다음 연구 질문의 근거로 넘긴다.",
            ]
        ),
    )
    write_text(
        ...
        "\n".join(
            [
                "# Stage313 Selection Status(313단계 선택 상태)",
                "",
                ...
                f"- stage312_review(312단계 검토): `{rel(REPORT)}`",
            ]
        ),
    )
    write_text(next_root / "03_reviews" / "review_index.md", f"# Stage313 Review Index(313단계 검토 색인)\n\n- stage312_review(312단계 검토): `{rel(REPORT)}`\n")
    write_csv(
        next_root / "03_reviews" / "stage_run_ledger.csv",
        STAGE_LEDGER_COLUMNS,
        [
            {
                "row_id": f"{RUN_ID}__stage313_open",
...
def report_markdown(scoreboard: Sequence[Mapping[str, Any]], selected_rows: Sequence[Mapping[str, Any]], next_stage_id: str, next_action: str) -> str:
    ...
    lines = [
        ...
        f"- selected_candidate(선택 후보): `{selected_rows[0]['package_id'] if selected_rows else 'none'}`",
        f"- Adapter package(어댑터 패키지): `{'deferred_to_stage313' if selected_rows else 'none'}`",
        "- ONNX readiness(온엑스 준비): `not_started`",
        "- Goal Achieve(목표 달성): `not_claimed`",
    ...
    lines.extend(
        [
            ...
            "## Decision(결정)",
            "",
            "Stage312(312단계)는 selected candidate(선택 후보) 없이 닫는다." if not selected_rows else "Stage312(312단계)는 Adapter(어댑터) 패키지 단계로 넘긴다.",
...
def update_docs(status: str, judgment: str, next_stage_id: str, next_action: str, selected_rows: Sequence[Mapping[str, Any]]) -> None:
    ...
    selected = r310.replace_line(selected, "- Adapter package(", f"- Adapter package(어댑터 패키지): `{'deferred_to_stage313' if selected_rows else 'none'}`")
    selected = r310.replace_line(selected, "- next_action(", f"- next_action(다음 행동): `{next_action}`")
    selected = r310.drop_prefixed_lines(selected, ("- run312C_report(", "- stage313_opened("))
    selected += f"- run312C_report(312C 보고서): `{rel(REPORT)}`\n"
    selected += f"- stage313_opened(313단계 열림): `{next_stage_id}`\n"
    write_text(SELECTED, selected)

    review_index = read_text(REVIEW_INDEX)
    review_index = r310.drop_prefixed_lines(review_index, ("- run312C_report(", "- run312C_scoreboard(", "- stage313_seed_queue("))
    review_index += f"- run312C_report(312C 보고서): `{rel(REPORT)}`\n"
    review_index += f"- run312C_scoreboard(312C 점수판): `{rel(SCOREBOARD)}`\n"
    review_index += f"- stage313_seed_queue(313단계 씨앗 대기열): `{rel(NEXT_STAGE_QUEUE)}`\n"
    ...
    current = r310.drop_prefixed_lines(current, ("- run312C_summary(",))
    current = current.rstrip() + f"\n- run312C_summary(312C 요약): Stage312(312단계) actual MT5(실제 메타트레이더5) 검토를 완료했다. Effect(효과): selected_candidate(선택 후보)는 `{selected_rows[0]['package_id'] if selected_rows else 'none'}`이고 next_stage(다음 단계)는 `{next_stage_id}`다.\n"
    ...
    focus = (
        f"- >-\n"
        f"  Stage312(312단계) run312C(312C 실행) fresh model asymmetry review(새 모델 비대칭 검토) `{RUN_ID}` closed Stage312 and opened `{next_stage_id}`. "
        f"Effect(효과): selected candidate(선택 후보)는 `{selected_rows[0]['package_id'] if selected_rows else 'none'}`이고 Adapter package(어댑터 패키지)는 `{'deferred_to_stage313' if selected_rows else 'none'}`, ONNX readiness(온엑스 준비)는 `not_started`다.\n"
    )
    ...
    if RUN_ID not in changelog:
        changelog += (
            f"\n## {UPDATED_ON} run312C Fresh model asymmetry review(312C 새 모델 비대칭 검토)\n\n"
            f"- status(상태): `{status}`\n"
            f"- judgment(판정): `{judgment}`\n"
            f"- effect(효과): Stage312(312단계)를 닫고 `{next_stage_id}`를 열었다.\n"
...
def update_memory_registers(failures: Sequence[Mapping[str, Any]], selected_rows: Sequence[Mapping[str, Any]]) -> None:
    ...
    if RUN_ID not in idea:
        idea += (
            f"\n## {RUN_ID} fresh_model_asymmetry_review(새 모델 비대칭 검토)\n\n"
    ...
    if failures:
        ...
        if RUN_ID not in negative:
            negative += (
                f"\n## {RUN_ID} Stage312 fresh model asymmetry failure memory(312단계 새 모델 비대칭 실패 기억)\n\n"
...
def update_registers(status: str, judgment: str, next_action: str) -> None:
    safe_upsert(RUN_REGISTRY, RUN_REGISTRY_COLUMNS, [{"run_id": RUN_ID, "stage_id": STAGE_ID, "lane": "fresh_model_asymmetry_review", "status": status, "judgment": judgment, "path": rel(REPORT), "notes": f"selected_candidate_reviewed;next_action={next_action}."}], "run_id")
    safe_upsert(
        ALPHA_LEDGER,
        ...
        [
            {
                ...
                "record_view": "fresh_model_asymmetry_review",
    ...
    safe_upsert(STAGE_LEDGER, STAGE_LEDGER_COLUMNS, [{"row_id": f"{RUN_ID}__review", "stage_id": STAGE_ID, "run_id": RUN_ID, "view": "fresh_model_asymmetry_review", "tier_scope": "Tier A used/Tier B fallback/actual routed total", "scoreboard": "fresh_model_asymmetry_review_scoreboard", "status": status, "judgment": judgment, "evidence_boundary": "runtime_probe_review_no_onnx", "report_path": rel(REPORT), "notes": "Stage313 opened if no selected candidate; ONNX not started."}], "row_id")
        ...
        rows.append(
            {
                ...
                "artifact_type": "stage312_fresh_model_asymmetry_review_artifact",
                "path": rel(path),
                "sha256": sha256_file(path),
                "stage_id": STAGE_ID,
                "run_id": RUN_ID,
                "created_at_utc": "2026-05-24T23:59:00Z",
                "notes": "Stage312 review and Stage313 open handoff",
            }
        )
...
def main() -> None:
    ...
    status = "completed_fresh_model_asymmetry_review_stage313_opened"
    ...
    write_csv(
        ...
        [{"run_id": RUN_ID, "status": status, "judgment": judgment, "selected_candidate": selected_rows[0]["package_id"] if selected_rows else "none", "adapter_package": "deferred_to_stage313" if selected_rows else "none", "onnx_readiness": "not_started", "next_action": next_action, "claim_boundary": BOUNDARY}],
    )
    gate_rows = [
    ...
    manifest = {
        ...
        "adapter_package": "deferred_to_stage313" if selected_rows else "none",
    ...
    print(json.dumps({"status": status, "judgment": judgment, "scoreboard_rows": len(scoreboard), "failure_rows": len(failure_rows), "selected_candidate": selected_rows[0]["package_id"] if selected_rows else "none", "adapter_package": "deferred_to_stage313" if selected_rows else "none", "onnx_readiness": "not_started", "goal_achieve": "not_claimed", "next_stage_id": next_stage_id, "next_action": next_action}, ensure_ascii=False, indent=2, sort_keys=True))
