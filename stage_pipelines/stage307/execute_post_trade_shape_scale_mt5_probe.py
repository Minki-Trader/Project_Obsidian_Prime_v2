...
RUN_ID = "run307B_post_trade_shape_scale_mt5_probe_v1"
RUN_NUMBER = "run307B"
SOURCE_RUN_ID = "run307A_design_post_trade_shape_scale_rebuild_v1"
PARENT_RUN_ID = "run306C_review_anti_surface_trade_shape_mt5_probe_v1"
STATUS_PREPARED = "prepared_post_trade_shape_scale_mt5_probe_no_runtime_kpi"
...
EXPLORATION_LABEL = "stage307_Model__PostTradeShapeScaleReplay"
SIGNAL_COLUMN = "run307b_route_signal"
FEATURE_ORDER = (SIGNAL_COLUMN,)
COMMON_ROOT = "Project_Obsidian_Prime_v2/stage307/run307B_post_trade_shape_scale"
...
MT5_QUEUE = RUN307A / "mt5_probe_queue.csv"
SOURCE_MANIFEST = RUN307A / "candidate_payload_manifest.csv"
SOURCE_RUN_MANIFEST = RUN307A / "run_manifest.json"
SOURCE_REPORT = REVIEWS / "run307A_post_trade_shape_scale_materialization_report.md"
PRODUCER = Path("stage_pipelines/stage307/execute_post_trade_shape_scale_mt5_probe.py")
...
REPORT = REVIEWS / "run307B_post_trade_shape_scale_mt5_probe_report.md"
...
def attach_identity(attempt: dict[str, Any], queue_row: Mapping[str, str], max_hold: int, close_on_flat: bool, same_reentry: int) -> None:
    ...
    attempt["stage307_branch_id"] = queue_row.get("materialized_branch_id", "")
    ...
    for queue_row in queue_rows:
        ...
        for runtime_split, split_token in (("validation_is", "val"), ("oos", "oos")):
            for tier_key, tier_label, tier_token in (
                ...
                attempt["signal_policy"] = "stage307 post-trade-shape scale route_signal_value -1/0/+1 through single-feature table"
            ...
            if include_routed:
                ...
                attach_identity(attempt, queue_row, max_hold, close_on_flat, same_reentry)
                attempt["signal_policy"] = "Tier A primary + Tier B fallback stage307 post-trade-shape scale route_signal_value"
                attempts.append(attempt)
    return attempts
...
def classify_status(result: Mapping[str, Any], materialize_only: bool) -> tuple[str, str, str, str]:
    ...
    if materialize_only:
        return (
            STATUS_PREPARED,
            "post_trade_shape_scale_runtime_probe_prepared_no_external_execution",
            "out_of_scope_by_claim_materialize_only",
            "run307B_execute_post_trade_shape_scale_mt5_probe_external_check",
        )
    ...
    if planned and completed_exec >= planned and len(kpis) >= planned:
        return (
            "completed_post_trade_shape_scale_mt5_probe_no_selection",
            "runtime_probe_completed_requires_profit_scale_curve_review_no_selection",
            "completed",
            "run307C_review_post_trade_shape_scale_mt5_probe",
        )
    if kpis:
        return (
            "partial_post_trade_shape_scale_mt5_probe_no_selection",
            "runtime_probe_partial_requires_continuation_or_review_no_selection",
            "partial_or_blocked",
            "run307B_continue_post_trade_shape_scale_mt5_probe" if limited else "run307C_review_with_runtime_gaps",
        )
    return (
        "blocked_post_trade_shape_scale_mt5_probe_no_kpi",
        "runtime_probe_blocked_no_kpi_no_selection",
        "blocked_or_invalid",
        "run307B_repair_or_block_post_trade_shape_scale_mt5_probe",
    )
...
def report_markdown(result: Mapping[str, Any], status: str, judgment: str, external_status: str, next_action: str) -> str:
    ...
    return "\n".join(
        [
            "# run307B Post-Trade-Shape Scale MT5 Probe(307B 거래 형태 이후 수익 규모 MT5 탐침)",
            "",
            ...
            "",
            "Effect(효과): run307A(307A 실행)의 route_signal_value(경로 신호)를 실제 MT5 tester(MT5 테스터)에 넣어 trade count(거래 수), net profit(순수익), PF(수익 팩터), DD(drawdown, 손실폭), recovery(회복)를 확인했다.",
            "",
            f"`{BOUNDARY}`",
        ]
    )
...
def upsert_ledgers(result: Mapping[str, Any], status: str, judgment: str, external_status: str, next_action: str) -> None:
    ...
    safe_upsert_rows(
        ...
        [{"run_id": RUN_ID, "stage_id": STAGE_ID, "lane": "post_trade_shape_scale_mt5_probe", "status": status, "judgment": judgment, "path": runner.prev.e295.e294.base.base.rel(REPORT), "notes": f"attempts={attempt_count};mt5_kpi_records={kpi_count};selected_candidate=none;onnx_readiness=not_claimed;next_action={next_action}."}],
        key="run_id",
    )
    safe_upsert_rows(
        ALPHA_LEDGER,
...
def update_docs(status: str, judgment: str, next_action: str, kpi_count: int, attempt_count: int) -> None:
    ...
    selected = runner.prev.e295.e294.base.base.append_once(selected, "run307B_report", f"- run307B_report(307B 보고서): `{runner.prev.e295.e294.base.base.rel(REPORT)}`")
    selected = runner.prev.e295.e294.base.base.append_once(selected, "run307B_execution_result", f"- run307B_execution_result(307B 실행 결과): `{runner.prev.e295.e294.base.base.rel(EXECUTION_RESULT)}`")
    runner.prev.e295.e294.base.base.write_md(SELECTED, selected)

    review_index = runner.prev.e295.e294.base.base.io_path(REVIEW_INDEX).read_text(encoding="utf-8-sig") if runner.prev.e295.e294.base.base.path_exists(REVIEW_INDEX) else "# Stage307 Review Index(307단계 검토 색인)\n"
    review_index = runner.prev.e295.e294.base.base.append_once(
        review_index,
        "run307B_report",
        f"- run307B_report(307B 보고서): `{runner.prev.e295.e294.base.base.rel(REPORT)}`\n- run307B_execution_result(307B 실행 결과): `{runner.prev.e295.e294.base.base.rel(EXECUTION_RESULT)}`\n- run307B_mt5_kpi_summary(307B MT5 KPI 요약): `{runner.prev.e295.e294.base.base.rel(MT5_KPI_SUMMARY)}`",
    )
    ...
    current = runner.prev.e295.e294.base.base.append_once(
        current,
        "run307B_summary",
        f"- run307B_summary(307B 요약): post-trade-shape scale MT5 probe(거래 형태 이후 수익 규모 MT5 탐침)를 실행했다. Effect(효과): attempts(시도) `{attempt_count}`개와 MT5 KPI records(MT5 KPI 기록) `{kpi_count}`개를 만들었고 선택 후보/Adapter(어댑터)/ONNX(온엑스)는 주장하지 않는다.",
    )
    ...
    focus = (
        f"- >-\n"
        f"  Stage307(307단계) run307B(307B 실행) post-trade-shape scale MT5 probe(거래 형태 이후 수익 규모 MT5 탐침) `{RUN_ID}`. "
    ...
    changelog = runner.prev.e295.e294.base.base.append_once(
        changelog,
        RUN_ID,
        f"## {UPDATED_ON} run307B Post-trade-shape scale MT5 probe(307B 거래 형태 이후 수익 규모 MT5 탐침)\n\n"
        f"- status(상태): `{status}`\n"
        f"- judgment(판정): `{judgment}`\n"
