...
RUN_ID = "run313B_execute_runtime_outcome_source_pivot_mt5_probe_v1"
RUN_NUMBER = "run313B"
SOURCE_RUN_ID = "run313A_design_runtime_outcome_source_pivot_rebuild_packet_v1"
PARENT_RUN_ID = "run312C_review_fresh_model_asymmetry_mt5_probe_v1"
STATUS_PREPARED = "prepared_runtime_outcome_source_pivot_mt5_probe_no_runtime_kpi"
UPDATED_ON = "2026-05-24"
EXPLORATION_LABEL = "stage313_Model__RuntimeOutcomeSourcePivotReplay"
SIGNAL_COLUMN = "run313b_route_signal"
FEATURE_ORDER = (SIGNAL_COLUMN,)
COMMON_ROOT = "Project_Obsidian_Prime_v2/stage313/run313B_runtime_outcome_source_pivot"

STAGE_ROOT = ROOT / "stages" / STAGE_ID
RUN313A = STAGE_ROOT / "02_runs" / "run313A"
...
MT5_QUEUE = RUN313A / "mt5_probe_queue.csv"
SOURCE_MANIFEST = RUN313A / "candidate_payload_manifest.csv"
SOURCE_RUN_MANIFEST = RUN313A / "run_manifest.json"
SOURCE_REPORT = REVIEWS / "run313A_materialization.md"
PRODUCER = Path("stage_pipelines/stage313/execute_runtime_outcome_source_pivot_mt5_probe.py")
...
REPORT = REVIEWS / "run313B_mt5_probe.md"
...
def classify_status(result: Mapping[str, Any], materialize_only: bool) -> tuple[str, str, str, str]:
    ...
    if materialize_only:
        return (
            STATUS_PREPARED,
            "runtime_outcome_source_pivot_runtime_probe_prepared_no_external_execution",
            "out_of_scope_by_claim_materialize_only",
            "run313B_execute_runtime_outcome_source_pivot_mt5_probe_external_check",
        )
    ...
    if planned and completed_exec >= planned and len(kpis) >= planned:
        return (
            "completed_runtime_outcome_source_pivot_mt5_probe_no_selection",
            "runtime_probe_completed_requires_profit_scale_curve_review_no_selection",
            "completed",
            "run313C_review_runtime_outcome_source_pivot_mt5_probe",
        )
    if kpis:
        return (
            "partial_runtime_outcome_source_pivot_mt5_probe_no_selection",
            "runtime_probe_partial_requires_continuation_or_review_no_selection",
            "partial_or_blocked",
            "run313B_continue_runtime_outcome_source_pivot_mt5_probe" if limited else "run313C_review_with_runtime_gaps",
        )
    return (
        "blocked_runtime_outcome_source_pivot_mt5_probe_no_kpi",
        "runtime_probe_blocked_no_kpi_no_selection",
        "blocked_or_invalid",
        "run313B_repair_or_block_runtime_outcome_source_pivot_mt5_probe",
    )
...
def report_markdown(result: Mapping[str, Any], status: str, judgment: str, external_status: str, next_action: str) -> str:
    ...
    return "\n".join(
        [
            "# run313B Runtime Outcome Source Pivot MT5 Probe(313B 런타임 결과 원천 전환 MT5 탐침)",
            "",
            ...
            "",
            "Effect(효과): run313A(313A 실행)의 runtime outcome source pivot route_signal_value(런타임 결과 원천 전환 경로 신호)를 MT5 tester(MT5 테스터)에 넣어 실제 거래 수, 순수익, 수익 곡선을 확인했다.",
            "",
            f"`{e310.BOUNDARY}`",
        ]
    )
...
def upsert_ledgers(result: Mapping[str, Any], status: str, judgment: str, external_status: str, next_action: str) -> None:
    ...
    e310.safe_upsert_rows(
        ...
        [{"run_id": RUN_ID, "stage_id": STAGE_ID, "lane": "runtime_outcome_source_pivot_mt5_probe", "status": status, "judgment": judgment, "path": base.rel(REPORT), "notes": f"attempts={attempt_count};mt5_kpi_records={kpi_count};selected_candidate=none;onnx_readiness=not_claimed;next_action={next_action}."}],
        key="run_id",
    )
    e310.safe_upsert_rows(
        ALPHA_LEDGER,
        ...
        [
            {
                ...
                "record_view": "runtime_outcome_source_pivot_mt5_probe",
                "tier_scope": "Tier A used/Tier B fallback stress/actual routed total",
                "kpi_scope": "runtime_probe",
                "scoreboard_lane": "runtime_outcome_source_pivot",
                "status": status,
                "judgment": judgment,
    ...
    e310.safe_upsert_rows(
        ...
        [{"row_id": f"{RUN_ID}__mt5_probe", "stage_id": STAGE_ID, "run_id": RUN_ID, "view": "runtime_outcome_source_pivot_mt5_probe", "tier_scope": "Tier A used/Tier B fallback/actual routed total", "scoreboard": "runtime_probe", "status": status, "judgment": judgment, "evidence_boundary": "runtime_probe_no_candidate_no_onnx", "report_path": base.rel(REPORT), "notes": f"attempts={attempt_count};mt5_kpi_records={kpi_count}."}],
        key="row_id",
    )
...
def update_docs(status: str, judgment: str, next_action: str, kpi_count: int, attempt_count: int) -> None:
    ...
    selected = base.append_once(selected, "run313B_report", f"- run313B_report(313B 보고서): `{base.rel(REPORT)}`")
    selected = base.append_once(selected, "run313B_execution_result", f"- run313B_execution_result(313B 실행 결과): `{base.rel(EXECUTION_RESULT)}`")
    save_md(SELECTED, selected)

    review_index = base.io_path(REVIEW_INDEX).read_text(encoding="utf-8-sig") if base.path_exists(REVIEW_INDEX) else "# Stage313 Review Index(313단계 검토 색인)\n"
    review_index = base.append_once(
        review_index,
        "run313B_report",
        f"- run313B_report(313B 보고서): `{base.rel(REPORT)}`\n- run313B_execution_result(313B 실행 결과): `{base.rel(EXECUTION_RESULT)}`\n- run313B_mt5_kpi_summary(313B MT5 핵심 성과 지표 요약): `{base.rel(MT5_KPI_SUMMARY)}`",
    )
    ...
    current = base.append_once(current, "run313B_summary", f"- run313B_summary(313B 요약): runtime outcome source pivot MT5 probe(런타임 결과 원천 전환 MT5 탐침)를 실행했다. Effect(효과): attempts(시도) `{attempt_count}`개와 MT5 KPI records(MT5 핵심 성과 지표 기록) `{kpi_count}`개를 만들었고 선택 후보/Adapter(어댑터)/ONNX(온엑스)는 주장하지 않는다.")
    ...
    focus = (
        f"- >-\n"
        f"  Stage313(313단계) run313B(313B 실행) runtime outcome source pivot MT5 probe(런타임 결과 원천 전환 MT5 탐침) `{RUN_ID}`. "
    ...
    changelog = base.append_once(
        changelog,
        RUN_ID,
        f"## {UPDATED_ON} run313B Runtime outcome source pivot MT5 probe(313B 런타임 결과 원천 전환 MT5 탐침)\n\n"
        f"- status(상태): `{status}`\n"
        f"- judgment(판정): `{judgment}`\n"
