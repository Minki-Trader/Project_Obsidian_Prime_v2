...
from stage_pipelines.stage215 import validation_mid_pf_recovery_preserve_oos_gain as s215  # noqa: E402
...
STAGE_ID = "217_adapter_research__oos_preserving_mid_pf_micro_interpolation"
RUN_NUMBER = "run217A"
RUN_ID = "run217A_stage217_oos_preserving_mid_pf_micro_interpolation_v1"
PACKET_ID = "stage217_oos_preserving_mid_pf_micro_interpolation_v1"
PARENT_RUN_ID = "run216A_stage216_stage215_mid_pf_recovery_followup_review_v1"
SOURCE_STAGE_ID = "216_adapter_research__stage215_mid_pf_recovery_followup_review"
SOURCE_RUN_ID = "run216A_stage216_stage215_mid_pf_recovery_followup_review_v1"
SOURCE_STAGE216_EVIDENCE_COMMIT = "abb5fd5bef288e496cee6cf590715b83f33a22ae"
SOURCE_STAGE216_HASH_RECORD_COMMIT = "4c1ec028a8c504bb8f1d0a823470c2bf20289b73"
SOURCE_OOS_PRESERVER_ID = "s215_r031375_s2025_t460"
SOURCE_MID_RECOVERY_ID = "s215_r031375_s2050_t465"
NEXT_STAGE_ID = "218_adapter_research__stage217_micro_interpolation_followup_review"
NEXT_RUN_ID = "run218A_stage218_stage217_micro_interpolation_followup_review_v1"
NEXT_PACKET_ID = "stage218_stage217_micro_interpolation_followup_review_v1"
TARGET_SURFACE = "legacy_34d_kpi_lesson_only_v2_native_oos_preserving_mid_pf_micro_interpolation"
...
STAGE215_OOS_PRESERVER = {
    "adapter_id": SOURCE_OOS_PRESERVER_ID,
    "validation_net": 956.51,
    "validation_mid_pf": 1.531012848,
    "validation_dd": 12.7149,
    "oos_net": 726.04,
}
STAGE215_MID_RECOVERY = {
    "adapter_id": SOURCE_MID_RECOVERY_ID,
...
COMMON_ROOT = f"OPV2/s217a/{RUN_NUMBER}"

SUMMARY_JSON_PATH = REVIEWS_ROOT / "stage217_summary.json"
SUMMARY_CSV_PATH = REVIEWS_ROOT / "stage217_micro_interpolation_kpi_summary.csv"
REPORT_PATH = REVIEWS_ROOT / "stage217_micro_interpolation_report.md"
SEGMENT_KPI_PATH = REVIEWS_ROOT / "stage217_segment_kpi_summary.csv"
BALANCE_CURVE_AUDIT_PATH = REVIEWS_ROOT / "stage217_balance_curve_audit.csv"
MONTHLY_KPI_PATH = REVIEWS_ROOT / "stage217_monthly_kpi_summary.csv"
CONCENTRATION_PATH = REVIEWS_ROOT / "stage217_concentration_risk_summary.csv"
DRAWDOWN_PATH = REVIEWS_ROOT / "stage217_drawdown_recovery_summary.csv"
QUALITY_MATRIX_PATH = REVIEWS_ROOT / "stage217_quality_matrix.csv"
RISK_ATR_TELEMETRY_PATH = REVIEWS_ROOT / "stage217_risk_atr_telemetry.csv"
GATE_FEATURE_SUMMARY_PATH = REVIEWS_ROOT / "stage217_gate_feature_summary.csv"
PROBABILITY_BINDING_PATH = REVIEWS_ROOT / "stage217_probability_telemetry_summary.csv"
MODEL_SCORE_AUDIT_PATH = REVIEWS_ROOT / "stage217_model_score_audit.csv"
TIER_B_DIAGNOSTIC_PATH = REVIEWS_ROOT / "stage217_tier_b_diagnostic_summary.csv"
DECISION_PATH = REVIEWS_ROOT / "stage217_decision.md"
AUDIT_CSV_PATH = REVIEWS_ROOT / "stage217_trade_audit.csv"
...
PRODUCER_PATH = Path("stage_pipelines/stage217/oos_preserving_mid_pf_micro_interpolation.py")
ARTIFACT_COLUMNS = s172.ARTIFACT_COLUMNS

VARIANTS = (
    repair.RepairVariant(
        adapter_id="s217_r031375_s20325_t4615",
        label="stage217_r031375_s20325_t4615",
        atr_enabled=True,
        ...
        max_hold_bars=3,
        notes="Stage217 micro interpolation: lower midpoint near OOS preserver.",
    ),
    repair.RepairVariant(
        adapter_id="s217_r031375_s20375_t4625",
        label="stage217_r031375_s20375_t4625",
        atr_enabled=True,
        model_risk_enabled=True,
        fixed_lot=0.25,
        atr_stop_multiplier=2.0375,
        atr_take_profit_multiplier=4.625,
        model_risk_max_pct=0.031375,
        same_direction_reentry_cooldown_bars=8,
        short_threshold=0.54,
        long_threshold=0.52,
        ...
        close_only_on_opposite_signal=False,
        max_hold_bars=3,
        notes="Stage217 micro interpolation: center midpoint.",
    ),
    repair.RepairVariant(
        adapter_id="s217_r031375_s20425_t4635",
        label="stage217_r031375_s20425_t4635",
        atr_enabled=True,
        model_risk_enabled=True,
        fixed_lot=0.25,
        atr_stop_multiplier=2.0425,
        atr_take_profit_multiplier=4.635,
        model_risk_max_pct=0.031375,
        same_direction_reentry_cooldown_bars=8,
        short_threshold=0.54,
        long_threshold=0.52,
        ...
        close_only_on_opposite_signal=False,
        max_hold_bars=3,
        notes="Stage217 micro interpolation: upper midpoint near mid PF recovery.",
    ),
    repair.RepairVariant(
        adapter_id="s217_r03125_s20375_t4625",
        label="stage217_r03125_s20375_t4625",
        atr_enabled=True,
        model_risk_enabled=True,
        fixed_lot=0.25,
        atr_stop_multiplier=2.0375,
        ...
        close_only_on_opposite_signal=False,
        max_hold_bars=3,
        notes="Stage217 micro interpolation: center midpoint with lower risk cap blend.",
    ),
)

BASE_EXTRA = {**s213.BASE_EXTRA, "block_mode": "encoded_side_context"}
VARIANT_EXTRAS: dict[str, dict[str, Any]] = {
    "s217_r031375_s20325_t4615": {**BASE_EXTRA, "short_block_rule": "midwide_lowedge", "long_block_rule": "session_only", "axis": "r031375_s20325_t4615"},
    "s217_r031375_s20375_t4625": {**BASE_EXTRA, "short_block_rule": "midwide_lowedge", "long_block_rule": "session_only", "axis": "r031375_s20375_t4625"},
    "s217_r031375_s20425_t4635": {**BASE_EXTRA, "short_block_rule": "midwide_lowedge", "long_block_rule": "session_only", "axis": "r031375_s20425_t4635"},
    "s217_r03125_s20375_t4625": {**BASE_EXTRA, "short_block_rule": "midwide_lowedge", "long_block_rule": "session_only", "axis": "r03125_s20375_t4625"},
}
...
def install_stage_values(context_specs: Mapping[str, Any]) -> None:
    values = {
        ...
        "SOURCE_ADAPTER_ID": SOURCE_MID_RECOVERY_ID,
    ...
    for variant_index, variant in enumerate(VARIANTS, start=1):
        ...
        for split in ("validation_is", "oos"):
            ...
            for role_index, (tier, attempt_role, prefix, attempt_token) in enumerate(
                ...
                magic = 21710000 + variant_index * 100 + (1 if split == "validation_is" else 50) + role_index
                attempts.append(
                    s161.base.attempt_payload(
                        run_root=variant_root,
                        run_id=RUN_ID,
                        stage_number=217,
                        exploration_label="stage217_BaselineAdapter__OOSPreservingMidPFMicroInterpolation",
...
def configure_runner() -> None:
    ...
    for variant in VARIANTS:
        ...
        context_specs[variant.adapter_id] = {
            "gate_column": f"stage217_gate_{extra['axis']}",
            "gate_type": "encoded_stage217_oos_preserving_mid_pf_micro_gate",
            ...
            "description": "Stage217 encoded gate: narrow SL/TP interpolation for OOS preservation and mid PF recovery.",
        }
...
def pass_micro_interpolation(row: Mapping[str, Any]) -> bool:
    return (
...
def decide(rows: Sequence[Mapping[str, Any]], external: str) -> str:
    if external != "completed":
        return "continue_stage217_runtime_completion_due_to_incomplete_runtime_candidate_not_final"
    if any(pass_micro_interpolation(row) for row in rows):
        return "open_stage218_stage217_micro_interpolation_followup_review_candidate_not_final"
    return "open_stage218_bounded_followup_due_to_micro_interpolation_tradeoff_candidate_not_final"
...
def best_row(rows: Sequence[Mapping[str, Any]]) -> Mapping[str, Any]:
    ...
    return max(
        rows,
        key=lambda row: (
            pass_micro_interpolation(row),
            min(as_float(row, "validation_mid_pf") - LEGACY_34D["profit_factor"], as_float(row, "oos_net") - STAGE210_ANCHOR["oos_net"]),
...
def kpi_table(rows: Sequence[Mapping[str, Any]]) -> str:
    ...
    for row in rows:
        ...
        if variant is not None:
            sltp = f"{variant.atr_stop_multiplier:.4f}/{variant.atr_take_profit_multiplier:.3f}"
            risk_cap = f"{variant.model_risk_max_pct:.6f}"
        lines.append(
            "| {adapter} | {sltp} | {risk} | {mid:.6f} | {oos:.2f} | {val:.2f} | {dd:.4f} | {early:.6f} | {flags} |".format(
                adapter=adapter_id,
                sltp=sltp,
                risk=risk_cap,
...
def report_markdown(rows: Sequence[Mapping[str, Any]], decision: str, external: str) -> str:
    best = best_row(rows)
    return f"""# Stage217 Micro Interpolation Report(217단계 미세 보간 보고서)

- stage(단계): `{STAGE_ID}`
- run(실행): `{RUN_ID}`
- source_stage(원천 단계): `{SOURCE_STAGE_ID}`
- source_run(원천 실행): `{SOURCE_RUN_ID}`
- source_stage216_evidence_commit(원천 216단계 근거 커밋): `{SOURCE_STAGE216_EVIDENCE_COMMIT}`
- source_stage216_hash_record_commit(원천 216단계 해시 기록 커밋): `{SOURCE_STAGE216_HASH_RECORD_COMMIT}`
- source_oos_preserver(원천 표본외 보존): `{SOURCE_OOS_PRESERVER_ID}`
- source_mid_recovery(원천 중반 회복): `{SOURCE_MID_RECOVERY_ID}`
...
- comparison_baseline(비교 기준): Stage210 anchor(210단계 기준 후보), Stage215 OOS preserver(215단계 표본외 보존) `{SOURCE_OOS_PRESERVER_ID}`, Stage215 mid recovery(215단계 중반 회복) `{SOURCE_MID_RECOVERY_ID}`다.
...
- comparison_baseline(비교 기준): Stage215 OOS preserver(215단계 표본외 보존)는 OOS net(표본외 순손익) `{STAGE215_OOS_PRESERVER["oos_net"]}`였고, Stage215 mid recovery(215단계 중반 회복)는 validation mid PF(검증 중반 수익요인) `{STAGE215_MID_RECOVERY["validation_mid_pf"]}`였다.
...
def decision_markdown(decision: str, external: str) -> str:
    return f"""# Stage217 Decision(217단계 판정)
...
Stage217(217단계) closeout(종료)은 overall goal complete(전체 목표 완료)가 아니다.

Effect(효과): Stage218(218단계) follow-up review(후속 검토)에서 OOS preservation(표본외 보존), mid PF recovery(중반 수익요인 회복), early PF(초반 수익요인) 상충을 판정한다.
"""
...
def artifact_rows(result: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    for row in rows:
        if row.get("artifact_type") != "mt5_strategy_tester_report":
            row["artifact_type"] = "stage217_evidence"
            row["notes"] = "Stage217 OOS-preserving mid PF micro interpolation evidence."
        else:
            row["notes"] = "Stage217 MT5 Strategy Tester report."
    return rows
        ...
        [
            {
                "run_id": RUN_ID,
                "stage_id": STAGE_ID,
                "lane": "baseline_adapter_stage217_oos_preserving_mid_pf_micro_interpolation",
                "status": status,
                "judgment": decision,
                "path": rel(DECISION_PATH),
                "notes": s172.ledger_pairs(
                    (
                        ("source_stage216_evidence_commit", SOURCE_STAGE216_EVIDENCE_COMMIT),
                        ("source_stage216_hash_record_commit", SOURCE_STAGE216_HASH_RECORD_COMMIT),
                        ("source_oos_preserver", SOURCE_OOS_PRESERVER_ID),
                        ("source_mid_recovery", SOURCE_MID_RECOVERY_ID),
                        ("target_surface", TARGET_SURFACE),
        ...
        alpha_rows = [
            {
                ...
                "tier_scope": "Tier A+B",
                "kpi_scope": "stage217_oos_preserving_mid_pf_micro_interpolation",
                ...
                "external_verification_status": external,
                "notes": "Stage217 materialized or blocked before KPI records were available.",
            }
        ]
...
def write_packet_files(result: Mapping[str, Any], decision: str, ledger_payload: Mapping[str, Any], quality: Sequence[Mapping[str, Any]]) -> None:
    ...
    s172.write_md(
        ...
        f"""# Stage217 Closeout Packet(217단계 종료 작업 묶음)
...
Stage218(218단계)은 Stage217(217단계) micro interpolation(미세 보간) 결과를 follow-up review(후속 검토)한다.

## Bounded Question(경계 질문)

Did Stage217(217단계) preserve OOS net(표본외 순손익) while recovering validation mid PF(검증 중반 수익요인), validation net/DD(검증 순손익/낙폭), early PF(초반 수익요인), monthly behavior(월별 행동), concentration risk(집중 위험), and risk/ATR telemetry(위험/ATR 기록)?

Effect(효과): Stage217(217단계) 안에서 추가 수리하지 않고, KPI(핵심 성과 지표) 상충을 별도 review(검토)로 판정한다.
...
""",
    ...
    s172.write_md(NEXT_STAGE_ROOT / "03_reviews" / "review_index.md", f"# Stage218 Review Index(218단계 검토 색인)\n\n- status(상태): `open_planned_from_stage217`\n- source_stage(원천 단계): `{STAGE_ID}`\n- source_decision(원천 판정): `{decision}`\n- next_action(다음 행동): `{NEXT_RUN_ID}`\n")
    s172.write_md(NEXT_STAGE_ROOT / "04_selected" / "selection_status.md", f"# Stage218 Selection Status(218단계 선택 상태)\n\n- stage_status(단계 상태): `open_planned_from_stage217`\n- current_packet(현재 작업 묶음): `{NEXT_PACKET_ID}`\n- current_run(현재 실행): `{NEXT_RUN_ID}`\n- source_stage(원천 단계): `{STAGE_ID}`\n- source_run(원천 실행): `{RUN_ID}`\n- source_decision(원천 판정): `{decision}`\n- claim_boundary(주장 경계): `{BOUNDARY}`\n")
...
- >-
  Stage217(217단계) closed(종료) as `{decision}` and Stage218(218단계) `{NEXT_STAGE_ID}` is open_planned(개방 계획). Effect(효과): micro interpolation(미세 보간)이 OOS preservation(표본외 보존)과 mid PF recovery(중반 수익요인 회복)를 동시에 만족했는지 별도 follow-up review(후속 검토)로 판정한다.
- >-
  Stage217 evidence(217단계 근거)는 `{rel(REPORT_PATH)}`, `{rel(SUMMARY_CSV_PATH)}`, `{rel(QUALITY_MATRIX_PATH)}`, `{rel(MONTHLY_KPI_PATH)}`, `{rel(CONCENTRATION_PATH)}`, `{rel(RISK_ATR_TELEMETRY_PATH)}`에 있다. Effect(효과): SL/TP(손절/익절) 미세 보간이 KPI(핵심 성과 지표)에 준 영향을 같이 본다.
- >-
...
"""
    if re.search(r"(?ms)^current_focus:\r?\n.*?(?=\r?\nstage\d+_)", state):
        ...
        state = state.rstrip() + "\n" + focus
    state = re.sub(r"(?ms)^stage217_oos_preserving_mid_pf_micro_interpolation:\r?\n.*?(?=^stage\d+_|\Z)", "", state)
    block = f"""
stage217_oos_preserving_mid_pf_micro_interpolation:
  packet_id: {PACKET_ID}
  stage_id: {STAGE_ID}
...
- adapter_under_review(검토 중 어댑터): `stage217_micro_interpolation`
- status(상태): `stage217_{decision}`
- claim_boundary(주장 경계): `{BOUNDARY}`

Stage217(217단계)는 Stage216(216단계) 판정에 따라 OOS preservation(표본외 보존)과 validation mid PF recovery(검증 중반 수익요인 회복)를 MT5 Strategy Tester(메타트레이더5 전략 테스터)로 측정했다. Effect(효과): Stage218(218단계)가 KPI(핵심 성과 지표) 상충을 별도 review(검토)로 판정한다.

## Latest Stage217 Evidence(최신 217단계 근거)
...
def append_changelog(decision: str) -> None:
    ...
    if RUN_ID in existing:
        return
    entry = (
        f"\n## {s172.utc_now()} Stage217 OOS preserving mid PF micro interpolation closeout(217단계 표본외 보존 중반 수익요인 미세 보간 종료)\n\n"
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    prepared = {
        ...
        "model_family": "baseline_adapter_stage217_v2_native_oos_preserving_mid_pf_micro_interpolation",
        "feature_set_id": "stage217_signal_plus_encoded_long_session_micro_interpolation",
    ...
    s172.write_json(
        SUMMARY_JSON_PATH,
        {
            ...
            "stage215_oos_preserver": STAGE215_OOS_PRESERVER,
            "stage215_mid_recovery": STAGE215_MID_RECOVERY,
            "claim_boundary": BOUNDARY,
            "overall_goal_complete": False,
            "source_stage216_evidence_commit": SOURCE_STAGE216_EVIDENCE_COMMIT,
            "source_stage216_hash_record_commit": SOURCE_STAGE216_HASH_RECORD_COMMIT,
