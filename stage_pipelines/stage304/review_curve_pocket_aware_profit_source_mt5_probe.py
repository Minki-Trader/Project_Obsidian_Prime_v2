...
RUN_ID = "run304C_review_curve_pocket_aware_profit_source_mt5_probe_v1"
RUN_NUMBER = "run304C"
SOURCE_RUN_ID = "run304B_curve_pocket_aware_profit_source_mt5_probe_v1"
PARENT_RUN_ID = "run304A_design_curve_pocket_aware_profit_source_rebuild_v1"
...
NEXT_REBUILD_STAGE_ID = "305_onnx_candidate_campaign__runtime_realized_curve_attribution_rebuild"
NEXT_ADAPTER_STAGE_ID = "305_onnx_candidate_campaign__adapter_package_for_curve_pocket_source"

STAGE_ROOT = ROOT / "stages" / STAGE_ID
RUN304A = STAGE_ROOT / "02_runs" / "run304A"
RUN304B = STAGE_ROOT / "02_runs" / "run304B"
...
SOURCE_EXECUTION = RUN304B / "execution_result.json"
SOURCE_KPI = RUN304B / "mt5_kpi_summary.csv"
SOURCE_SCOUT = RUN304A / "model_scout_scoreboard.csv"
SOURCE_MANIFEST = RUN304A / "candidate_payload_manifest.csv"
PRODUCER = Path("stage_pipelines/stage304/review_curve_pocket_aware_profit_source_mt5_probe.py")

SCOREBOARD = RUN_ROOT / "curve_pocket_aware_profit_source_review_scoreboard.csv"
...
NEXT_STAGE_QUEUE = RUN_ROOT / "stage305_seed_queue.csv"
RESULT_JUDGMENT = RUN_ROOT / "result_judgment.csv"
GATE_AUDIT = RUN_ROOT / "required_gate_coverage_audit.csv"
RUN_MANIFEST = RUN_ROOT / "run_manifest.json"
LINEAGE = RUN_ROOT / "artifact_lineage_receipt.json"
REPORT = REVIEWS / "run304C_curve_pocket_aware_profit_source_review_stage305_open_report.md"
DECISION = ROOT / "docs" / "decisions" / "2026-05-24_stage304_curve_pocket_aware_profit_source_review_stage305_open.md"
...
def feature_trading_days(attempt: Mapping[str, Any]) -> int:
    ...
    local = RUN304B / "features" / Path(feature_path).name
...
def load_actual_rows() -> tuple[list[dict[str, Any]], dict[str, dict[str, Any]], list[dict[str, Any]]]:
    ...
    with ledger.io_path(SOURCE_KPI).open("r", encoding="utf-8-sig", newline="") as handle:
        for source_row in csv.DictReader(handle):
            ...
            materialized_id = str(attempt.get("stage304_branch_id") or attempt.get("materialized_branch_id") or "")
...
def build_scoreboard(rows: Sequence[Mapping[str, Any]], scout_rows: Mapping[str, Mapping[str, Any]]) -> tuple[list[dict[str, Any]], list[dict[str, Any]], list[dict[str, Any]]]:
    ...
    for candidate_id, split_rows in sorted(by_candidate.items()):
        ...
        row = {
            ...
            "adapter_package": "deferred_to_stage305" if selected else "none",
        ...
        failure_rows.append(
            {
                "failure_id": f"{candidate_id}_stage304_negative_or_watch",
                "materialized_branch_id": candidate_id,
                "package_id": package_id,
                ...
                "do_not_repeat": "Do not repeat only lot, ATR, or density micro repair on the same Stage304 surface.",
                "claim_boundary": BOUNDARY,
            }
        )
...
def selected_or_stage305_rows(scoreboard: Sequence[Mapping[str, Any]], selected_rows: Sequence[Mapping[str, Any]]) -> list[dict[str, Any]]:
    if selected_rows:
        return [
            {
                "queue_id": f"stage305_adapter_{index:02d}",
                "materialized_branch_id": row["materialized_branch_id"],
                "package_id": row["package_id"],
                ...
                "next_action": "run305A_build_adapter_package_for_curve_pocket_source",
                "claim_boundary": BOUNDARY,
            }
    ...
    return [
        {
            "queue_id": f"stage305_rebuild_seed_{index:02d}",
            "materialized_branch_id": row["materialized_branch_id"],
            "package_id": row["package_id"],
            ...
            "next_action": "run305A_design_runtime_realized_curve_attribution_rebuild_packet",
            "claim_boundary": BOUNDARY,
        }
...
def scaffold_stage305(selected_rows: Sequence[Mapping[str, Any]]) -> tuple[str, str, str, str]:
    if selected_rows:
        next_stage_id = NEXT_ADAPTER_STAGE_ID
        next_status = "opened_adapter_package_for_curve_pocket_source"
        next_judgment = "opened_from_stage304_selected_candidate_for_adapter_packaging"
        next_action = "run305A_build_adapter_package_for_curve_pocket_source"
        question = "Can the selected Stage304 candidate be packaged as an Adapter-ready candidate with feature order and runtime handoff receipts?"
        effect = "Stage304 candidate gate passed; Stage305 prepares Adapter package before any ONNX export."
    else:
        next_stage_id = NEXT_REBUILD_STAGE_ID
        next_status = "opened_runtime_realized_curve_attribution_rebuild"
        next_judgment = "opened_from_stage304_no_onnx_worthy_candidate"
        next_action = "run305A_design_runtime_realized_curve_attribution_rebuild_packet"
        question = "Can actual MT5 trade attribution and parsed curve pockets rebuild a stronger profit source rather than repairing Stage304 thresholds?"
        effect = "Stage304 evidence becomes runtime-realized failure memory and seed data for a fresh rebuild."
    ...
    write_text(
        ...
        "\n".join(
            [
                "# Stage305 Selection Status(305단계 선택 상태)",
                "",
                ...
                f"- stage304_review(304단계 검토): `{rel(REPORT)}`",
            ]
        ),
    )
    write_text(
    ...
    write_csv(
        ...
        [
            {
                "row_id": "stage305_opened_from_run304C",
                "stage_id": next_stage_id,
                "run_id": RUN_ID,
                "view": "stage_open",
                "tier_scope": "not_applicable",
                "scoreboard": "stage304_review",
                "status": next_status,
                "judgment": next_judgment,
                "evidence_boundary": "planning_from_stage304_actual_mt5_evidence",
...
def report_markdown(scoreboard: Sequence[Mapping[str, Any]], failures: Sequence[Mapping[str, Any]], selected_rows: Sequence[Mapping[str, Any]], next_stage_id: str, next_action: str) -> str:
    ...
    lines = [
        ...
        f"- selected_candidate(선택 후보): `{selected_rows[0]['package_id'] if selected_rows else 'none'}`",
        f"- Adapter package(어댑터 패키지): `{'deferred_to_stage305' if selected_rows else 'none'}`",
        "- ONNX readiness(온엑스 준비): `not_started`",
        "- Goal Achieve(목표 달성): `not_claimed`",
...
def update_docs(status: str, judgment: str, next_stage_id: str, next_action: str, selected_rows: Sequence[Mapping[str, Any]]) -> None:
    ...
    selected = replace_line(selected, "- Adapter package(", f"- Adapter package(어댑터 패키지): `{'deferred_to_stage305' if selected_rows else 'none'}`")
    selected = replace_line(selected, "- next_action(", f"- next_action(다음 행동): `{next_action}`")
    selected += f"- run304C_report(304C 보고): `{rel(REPORT)}`\n"
    selected += f"- stage305_opened(305단계 열림): `{next_stage_id}`\n"
    write_text(SELECTED, selected)

    review_index = read_text(REVIEW_INDEX)
    review_index += f"- run304C_report(304C 보고): `{rel(REPORT)}`\n"
    review_index += f"- run304C_scoreboard(304C 점수표): `{rel(SCOREBOARD)}`\n"
    review_index += f"- stage305_seed_queue(305단계 씨앗 대기열): `{rel(NEXT_STAGE_QUEUE)}`\n"
    ...
    current = current.rstrip() + f"\n- run304C_summary(304C 요약): Stage304(304단계) actual MT5(실제 메타트레이더5) 검토를 완료했다. Effect(효과): selected_candidate(선택 후보)는 `{selected_rows[0]['package_id'] if selected_rows else 'none'}`이고 next_stage(다음 단계)는 `{next_stage_id}`다.\n"
    ...
    focus = (
        f"- >-\n"
        f"  Stage304(304단계) run304C(304C 실행) curve-pocket-aware profit source review(곡선 포켓 인식 수익 원천 검토) `{RUN_ID}` closed Stage304 and opened Stage305(305단계). "
        f"Effect(효과): selected candidate(선택 후보)는 `{selected_rows[0]['package_id'] if selected_rows else 'none'}`이고 Adapter package(어댑터 패키지)는 `{'deferred_to_stage305' if selected_rows else 'none'}`, ONNX readiness(온엑스 준비)는 `not_started`다.\n"
    )
    ...
    changelog += (
        f"\n## {UPDATED_ON} run304C Curve-pocket-aware profit source review(304C 곡선 포켓 인식 수익 원천 검토)\n\n"
        f"- status(상태): `{status}`\n"
        f"- judgment(판정): `{judgment}`\n"
        f"- effect(효과): Stage304(304단계)를 닫고 `{next_stage_id}`를 열었다.\n"
...
def update_registers(status: str, judgment: str, next_action: str) -> None:
    safe_upsert(
        ...
        [{"run_id": RUN_ID, "stage_id": STAGE_ID, "lane": "curve_pocket_aware_profit_source_review", "status": status, "judgment": judgment, "path": rel(REPORT), "notes": f"selected_candidate_reviewed;next_action={next_action}."}],
        "run_id",
    )
    safe_upsert(
        ALPHA_LEDGER,
        ...
        [
            {
                ...
                "record_view": "curve_pocket_aware_profit_source_review",
    ...
    safe_upsert(
        STAGE_LEDGER,
        STAGE_LEDGER_COLUMNS,
        [{"row_id": f"{RUN_ID}__review", "stage_id": STAGE_ID, "run_id": RUN_ID, "view": "curve_pocket_aware_profit_source_review", "tier_scope": "Tier A used/Tier B fallback/actual routed total", "scoreboard": "curve_pocket_aware_profit_source_review_scoreboard", "status": status, "judgment": judgment, "evidence_boundary": "runtime_probe_review_no_onnx", "report_path": rel(REPORT), "notes": "Stage305 opened; ONNX not started."}],
        "row_id",
    )
...
def update_memory_registers(failures: Sequence[Mapping[str, Any]], selected_rows: Sequence[Mapping[str, Any]]) -> None:
    ...
    if RUN_ID not in idea:
        idea += (
            f"\n## {RUN_ID} curve-pocket-aware profit source(곡선 포켓 인식 수익 원천)\n\n"
    ...
    if failures:
        ...
        if RUN_ID not in negative:
            negative += (
                f"\n## {RUN_ID} Stage304 curve-pocket-aware failure memory(304단계 곡선 포켓 인식 실패 기억)\n\n"
        ...
        rows.append(
            {
                ...
                "artifact_type": "stage304_curve_pocket_aware_profit_source_review_artifact",
                "path": rel(path),
                "sha256": ledger.sha256_file_lf_normalized(path),
                "stage_id": STAGE_ID,
                "run_id": RUN_ID,
                "created_at_utc": "2026-05-24T22:00:00Z",
                "notes": "Stage304 review and Stage305 open handoff",
            }
        )
...
def main() -> None:
    ...
    stage305_rows = selected_or_stage305_rows(scoreboard, selected_rows)
    next_stage_id, _next_status, _next_judgment, next_action = scaffold_stage305(selected_rows)
    status = "completed_curve_pocket_aware_profit_source_review_stage305_opened"
    judgment = (
    ...
    write_csv(NEXT_STAGE_QUEUE, list(stage305_rows[0].keys()), stage305_rows)
    write_csv(
        RESULT_JUDGMENT,
        ("run_id", "status", "judgment", "selected_candidate", "adapter_package", "onnx_readiness", "next_action", "claim_boundary"),
        [{"run_id": RUN_ID, "status": status, "judgment": judgment, "selected_candidate": selected_rows[0]["package_id"] if selected_rows else "none", "adapter_package": "deferred_to_stage305" if selected_rows else "none", "onnx_readiness": "not_started", "next_action": next_action, "claim_boundary": BOUNDARY}],
    )
    gate_rows = [
    ...
    manifest = {
        ...
        "adapter_package": "deferred_to_stage305" if selected_rows else "none",
        "onnx_readiness": "not_started",
    ...
    write_text(
        ...
        "\n".join(
            [
                ...
                "",
                "Effect(효과): 실제 MT5(메타트레이더5) report source_path(보고서 원천 경로)를 이용해 곡선 판정을 복구하고, 후보가 없으면 새 런타임 기여도 재구성으로 넘긴다.",
            ]
        ),
    )
