...
RUN_ID = "run304B_curve_pocket_aware_profit_source_mt5_probe_v1"
RUN_NUMBER = "run304B"
SOURCE_RUN_ID = "run304A_design_curve_pocket_aware_profit_source_rebuild_v1"
PARENT_RUN_ID = "run303C_review_regime_balanced_profit_scale_router_mt5_probe_v1"
STATUS_PREPARED = "prepared_curve_pocket_aware_profit_source_mt5_probe_no_runtime_kpi"
...
EXPLORATION_LABEL = "stage304_Model__CurvePocketAwareProfitSourceReplay"
SIGNAL_COLUMN = "run304b_route_signal"
FEATURE_ORDER = (SIGNAL_COLUMN,)
COMMON_ROOT = "Project_Obsidian_Prime_v2/stage304/run304B_curve_pocket_source"
...
MT5_QUEUE = RUN304A / "mt5_probe_queue.csv"
SOURCE_MANIFEST = RUN304A / "candidate_payload_manifest.csv"
SOURCE_RUN_MANIFEST = RUN304A / "run_manifest.json"
SOURCE_REPORT = REVIEWS / "run304A_curve_pocket_aware_profit_source_materialization_report.md"
PRODUCER = Path("stage_pipelines/stage304/execute_curve_pocket_aware_profit_source_mt5_probe.py")
...
REPORT = REVIEWS / "run304B_curve_pocket_aware_profit_source_mt5_probe_report.md"
...
def attach_identity(attempt: dict[str, Any], queue_row: Mapping[str, str], max_hold: int, close_on_flat: bool, same_reentry: int) -> None:
    ...
    attempt["stage304_branch_id"] = queue_row.get("materialized_branch_id", "")
    ...
    for queue_row in queue_rows:
        ...
        for runtime_split, split_token in (("validation_is", "val"), ("oos", "oos")):
            for tier_key, tier_label, tier_token in (
                ...
                attempt["signal_policy"] = "stage304 curve-pocket-aware profit source route_signal_value -1/0/+1 through single-feature table"
            ...
            if include_routed:
                ...
                attach_identity(attempt, queue_row, max_hold, close_on_flat, same_reentry)
                attempt["signal_policy"] = "Tier A primary + Tier B fallback stage304 curve-pocket-aware profit source route_signal_value"
                attempts.append(attempt)
    return attempts
...
def classify_status(result: Mapping[str, Any], materialize_only: bool) -> tuple[str, str, str, str]:
    ...
    if materialize_only:
        return (
            STATUS_PREPARED,
            "curve_pocket_aware_profit_source_runtime_probe_prepared_no_external_execution",
            "out_of_scope_by_claim_materialize_only",
            "run304B_execute_curve_pocket_aware_profit_source_mt5_probe_external_check",
        )
    ...
    if planned and completed_exec >= planned and len(kpis) >= planned:
        return (
            "completed_curve_pocket_aware_profit_source_mt5_probe_no_selection",
            "runtime_probe_completed_requires_profit_scale_curve_review_no_selection",
            "completed",
            "run304C_review_curve_pocket_aware_profit_source_mt5_probe",
        )
    if kpis:
        return (
            "partial_curve_pocket_aware_profit_source_mt5_probe_no_selection",
            "runtime_probe_partial_requires_continuation_or_review_no_selection",
            "partial_or_blocked",
            "run304B_continue_curve_pocket_aware_profit_source_mt5_probe" if limited else "run304C_review_with_runtime_gaps",
        )
    return (
        "blocked_curve_pocket_aware_profit_source_mt5_probe_no_kpi",
        "runtime_probe_blocked_no_kpi_no_selection",
        "blocked_or_invalid",
        "run304B_repair_or_block_curve_pocket_aware_profit_source_mt5_probe",
    )
...
def report_markdown(result: Mapping[str, Any], status: str, judgment: str, external_status: str, next_action: str) -> str:
    ...
    return "\n".join(
        [
            "# run304B Curve-Pocket-Aware Profit Source MT5 Probe(304B 곡선 포켓 인식 수익 원천 MT5 탐침)",
            "",
            ...
            "",
            "Effect(효과): run304A(304A 실행)의 route_signal_value(경로 신호)를 실제 MT5 tester(MT5 테스터)에 넣어 trade count(거래 수), net profit(순수익), PF(수익 팩터), DD(drawdown, 손실폭), recovery(회복)를 확인한다.",
            "",
            f"`{BOUNDARY}`",
        ]
    )
...
def upsert_ledgers(result: Mapping[str, Any], status: str, judgment: str, external_status: str, next_action: str) -> None:
    ...
    safe_upsert_rows(
        ...
        [{"run_id": RUN_ID, "stage_id": STAGE_ID, "lane": "curve_pocket_aware_profit_source_mt5_probe", "status": status, "judgment": judgment, "path": runner.prev.e295.e294.base.base.rel(REPORT), "notes": f"attempts={attempt_count};mt5_kpi_records={kpi_count};selected_candidate=none;onnx_readiness=not_claimed;next_action={next_action}."}],
        key="run_id",
    )
    safe_upsert_rows(
        ALPHA_LEDGER,
        ...
        [
            {
                ...
                "record_view": "curve_pocket_aware_profit_source_mt5_probe",
    ...
    rows = [
        {
            ...
            "artifact_type": "stage304_curve_pocket_aware_profit_source_mt5_artifact",
            "path": runner.prev.e295.e294.base.base.rel(path),
            "sha256": runner.prev.e295.e294.base.base.sha256_file_lf_normalized(path),
            "stage_id": STAGE_ID,
            "run_id": RUN_ID,
            "created_at_utc": created_at,
            "notes": "run304B curve-pocket-aware profit source MT5 probe",
        }
        for path in paths
...
def update_docs(status: str, judgment: str, next_action: str, kpi_count: int, attempt_count: int) -> None:
    ...
    selected = runner.prev.e295.e294.base.base.append_once(selected, "run304B_report", f"- run304B_report(304B 보고): `{runner.prev.e295.e294.base.base.rel(REPORT)}`")
    selected = runner.prev.e295.e294.base.base.append_once(selected, "run304B_execution_result", f"- run304B_execution_result(304B 실행 결과): `{runner.prev.e295.e294.base.base.rel(EXECUTION_RESULT)}`")
    runner.prev.e295.e294.base.base.write_md(SELECTED, selected)

    review_index = runner.prev.e295.e294.base.base.io_path(REVIEW_INDEX).read_text(encoding="utf-8-sig") if runner.prev.e295.e294.base.base.path_exists(REVIEW_INDEX) else "# Stage304 Review Index(304단계 검토 색인)\n"
    review_index = runner.prev.e295.e294.base.base.append_once(
        review_index,
        "run304B_report",
        f"- run304B_report(304B 보고): `{runner.prev.e295.e294.base.base.rel(REPORT)}`\n- run304B_execution_result(304B 실행 결과): `{runner.prev.e295.e294.base.base.rel(EXECUTION_RESULT)}`\n- run304B_mt5_kpi_summary(304B MT5 KPI 요약): `{runner.prev.e295.e294.base.base.rel(MT5_KPI_SUMMARY)}`",
    )
    ...
    current = runner.prev.e295.e294.base.base.append_once(
        current,
        "run304B_summary",
        f"- run304B_summary(304B 요약): curve-pocket-aware profit source MT5 probe(곡선 포켓 인식 수익 원천 MT5 탐침)를 실행했다. Effect(효과): attempts(시도) `{attempt_count}`개와 MT5 KPI records(MT5 KPI 기록) `{kpi_count}`개를 엮었고 후보/어댑터/ONNX(온엑스) 주장은 하지 않는다.",
    )
    ...
    focus = (
        f"- >-\n"
        f"  Stage304(304단계) run304B(304B 실행) curve-pocket-aware profit source MT5 probe(곡선 포켓 인식 수익 원천 MT5 탐침) `{RUN_ID}`. "
    ...
    changelog = runner.prev.e295.e294.base.base.append_once(
        changelog,
        RUN_ID,
        f"## {UPDATED_ON} run304B Curve-pocket-aware profit source MT5 probe(304B 곡선 포켓 인식 수익 원천 MT5 탐침)\n\n"
        f"- status(상태): `{status}`\n"
        f"- judgment(판정): `{judgment}`\n"
