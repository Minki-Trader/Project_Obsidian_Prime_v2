...
from stage_pipelines.stage72 import v41_short_gate_threshold_repair as stage72  # noqa: E402


base = stage72.base

STAGE_ID = "73_adapter_research__v41_gate_repair_followup"
RUN_NUMBER = "run73A"
RUN_ID = "run73A_stage73_v41_gate_repair_followup_v1"
PACKET_ID = "stage73_v41_gate_repair_followup_v1"
PARENT_RUN_ID = "run72A_stage72_v41_source_repair_review_v1"
SOURCE_STAGE72_ID = "72_adapter_research__v41_source_repair_review"
SOURCE_STAGE72_COMMIT = "51bdf7f69e92f596610fff6c626ac48e33f22e9f"
NEXT_STAGE_ID = "74_adapter_research__v41_tp_risk_followup_review"
NEXT_RUN_ID = "run74A_stage74_v41_tp_risk_followup_review_v1"
NEXT_PACKET_ID = "stage74_v41_tp_risk_followup_review_v1"
SOURCE_ADAPTER_ID = "s71_v41_h3_risk45_short_gate"
...
COMMON_ROOT = f"OPV2/s73a/{RUN_NUMBER}"

SUMMARY_JSON_PATH = REVIEWS_ROOT / "stage73_v41_tp_risk_summary.json"
SUMMARY_CSV_PATH = REVIEWS_ROOT / "stage73_v41_tp_risk_summary.csv"
REPORT_PATH = REVIEWS_ROOT / "stage73_v41_tp_risk_report.md"
SEGMENT_KPI_PATH = REVIEWS_ROOT / "stage73_segment_kpi_summary.csv"
RISK_ATR_TELEMETRY_PATH = REVIEWS_ROOT / "stage73_risk_atr_telemetry.csv"
GATE_FEATURE_SUMMARY_PATH = REVIEWS_ROOT / "stage73_gate_feature_summary.csv"
TIER_B_DIAGNOSTIC_PATH = REVIEWS_ROOT / "stage73_tier_b_diagnostic_summary.csv"
DECISION_PATH = REVIEWS_ROOT / "stage73_decision.md"
AUDIT_CSV_PATH = REVIEWS_ROOT / "stage73_trade_audit.csv"
...
VARIANTS = (
    repair.RepairVariant(
        adapter_id="s73_v41_h3_risk5_gate08_tp35",
        label="run50BN_v41_h3_risk5_gate08_tp35",
        atr_enabled=True,
        ...
        max_hold_bars=3,
        notes="Stage73 v41 follow-up: restore 0.08 short gate, risk cap 5%, TP 3.5, hold3.",
    ),
    repair.RepairVariant(
        adapter_id="s73_v41_h3_risk45_gate08_tp40",
        label="run50BN_v41_h3_risk45_gate08_tp40",
        atr_enabled=True,
        model_risk_enabled=True,
        fixed_lot=0.25,
        atr_stop_multiplier=2.5,
        ...
        close_only_on_opposite_signal=True,
        max_hold_bars=3,
        notes="Stage73 v41 follow-up: restore 0.08 short gate, risk cap 4.5%, TP 4.0, hold3.",
    ),
    repair.RepairVariant(
        adapter_id="s73_v41_h3_risk5_gate08_tp40",
        label="run50BN_v41_h3_risk5_gate08_tp40",
        atr_enabled=True,
        model_risk_enabled=True,
        fixed_lot=0.25,
        atr_stop_multiplier=2.5,
        ...
        close_only_on_opposite_signal=True,
        max_hold_bars=3,
        notes="Stage73 v41 follow-up: restore 0.08 short gate, risk cap 5%, TP 4.0, hold3.",
    ),
)
...
CONTEXT_GATE_SPECS = {
    variant.adapter_id: {
        "gate_column": "stage73_gate_margin_lt_008_short",
        "gate_type": "margin",
        "threshold": 0.08,
        "block_mode": "short",
...
def configure_base() -> None:
    ...
    base.BACKUP_ANCHOR = "s71_v41_h3_risk45_short_gate"
...
def decide(summary_rows: Sequence[Mapping[str, Any]], segment_rows: Sequence[Mapping[str, Any]], external: str) -> str:
    if external != "completed":
        return "continue_stage73_due_to_incomplete_runtime"
    ...
    if val_net >= 900 and oos_net >= 520 and val_dd <= 20.0 and oos_dd <= 20.0 and val_pf >= 1.50 and oos_pf >= 1.30 and not reasons:
        return "proceed_to_stage74_candidate_review"
    if val_net >= 650 and oos_net >= 420 and val_dd <= 28.0 and oos_dd <= 24.0 and val_pf >= 1.35 and oos_pf >= 1.25:
        return "continue_v41_tp_risk_repair_in_stage74"
    if val_pf < 1.0 or val_net <= 0:
        return "open_another_model_source_branch_in_stage74"
    return "continue_v41_tp_risk_repair_in_stage74"
...
def report_markdown(
    ...
    return f"""# Stage73 V41 TP/Risk Follow-up Report(73단계 V41 TP/위험 후속 보고서)

- run(실행): `{RUN_ID}`
- target_surface(목표 표면): `{TARGET_SURFACE}`
- source_stage(원천 단계): `{SOURCE_STAGE72_ID}`
- source_stage72_commit(원천 72단계 커밋): `{SOURCE_STAGE72_COMMIT}`
...
Stage72(72단계)의 softer gate(더 약한 게이트)는 validation(검증)을 훼손했다. Effect(효과): Stage73(73단계)는 Stage71(71단계)의 0.08 short gate(숏 게이트)를 되살리고 risk cap(위험 상한)과 TP(익절 폭)만 좁게 바꿔 순손익 확장 가능성을 측정한다.
...
def decision_markdown(decision: str, external: str) -> str:
    return f"""# Stage73 Decision(73단계 판정)

decision(판정): `{decision}`

Stage73(73단계)는 Stage71(71단계)의 0.08 short gate(숏 게이트)를 복원하고 risk cap(위험 상한)과 TP(익절 폭)를 좁게 바꿔 PF/net/DD(수익 팩터/순손익/손실률) 균형을 측정했다.
            ...
            rows.append(
                {
                    ...
                    "split": split_name,
                    "tier_b_policy": "diagnostic_missing_required_but_disabled_for_this_v41_tp_risk_followup",
                    ...
                    "reason": "Stage73 isolates v41 TP/risk repair first; Tier B fallback remains diagnostic and disabled.",
                }
            )
    return rows
            ...
            rows.append(
                {
                    ...
                    "artifact_type": "stage73_v41_tp_risk_followup_evidence",
                    "path": rel(path),
                    "sha256": sha256_file_lf_normalized(path),
                    "stage_id": STAGE_ID,
                    "run_id": RUN_ID,
                    "created_at_utc": created,
                    "notes": "Stage73 v2-native v41 TP/risk follow-up artifact.",
                }
            )
            ...
            rows.append(
                {
                    ...
                    "run_id": RUN_ID,
                    "created_at_utc": created,
                    "notes": "Actual Stage73 MT5 Strategy Tester HTML report.",
                }
            )
    return rows
...
def write_run_identity(result: Mapping[str, Any]) -> None:
    stage72.write_json(
        RUN_ROOT / "run_manifest.json",
        {
            ...
            "source_stage72_pushed_commit": SOURCE_STAGE72_COMMIT,
        ...
        [
            {
                "run_id": RUN_ID,
                "stage_id": STAGE_ID,
                "lane": "baseline_adapter_v2_native_v41_tp_risk_followup",
                "status": status,
                "judgment": decision,
                ...
                "notes": ledger_pairs(
                    (
                        ...
                        ("source_stage72_commit", SOURCE_STAGE72_COMMIT),
                        ("target_surface", TARGET_SURFACE),
        ...
        alpha_rows = [
            {
                ...
                "tier_scope": "Tier A+B",
                "kpi_scope": "stage73_v41_tp_risk_followup",
                ...
                "external_verification_status": external,
                "notes": "Stage73 run materialized or blocked before KPI records were available.",
            }
        ]
...
def write_packet_files(result: Mapping[str, Any], decision: str, ledger_payload: Mapping[str, Any]) -> None:
    ...
    stage72.write_json(PACKET_ROOT / "routing_receipt.json", {"packet_id": PACKET_ID, "stage_id": STAGE_ID, "primary_family": "experiment_design", "primary_skill": "obsidian-experiment-design", "support_skills": ["obsidian-exploration-mandate", "obsidian-performance-attribution", "obsidian-model-validation"], "required_gates": ["runtime_evidence_gate", "kpi_contract_audit", "result_judgment_gate"], "status": status})
    stage72.write_json(PACKET_ROOT / "runtime_evidence_gate.json", {"packet_id": PACKET_ID, "run_id": RUN_ID, "external_verification_status": result.get("external_verification_status"), "completed_attempt_count": result.get("completed_attempt_count"), "expected_attempt_count": result.get("expected_attempt_count"), "gate_feature_summary_path": rel(GATE_FEATURE_SUMMARY_PATH), "claim_boundary": BOUNDARY})
    stage72.write_json(PACKET_ROOT / "result_judgment_gate.json", {"packet_id": PACKET_ID, "run_id": RUN_ID, "decision": decision, "legacy_relation": "lesson_only_target_surface_no_code_copy", "forbidden_claims": ["deployment", "live_readiness", "production_baseline", "operating_promotion", "operating_reference", "runtime_authority", "legacy_inheritance"]})
    stage72.write_json(PACKET_ROOT / "aggregate_summary.json", {"packet_id": PACKET_ID, "stage_id": STAGE_ID, "run_id": RUN_ID, "decision": decision, "gate_feature_summary_path": rel(GATE_FEATURE_SUMMARY_PATH), "ledger_payload": ledger_payload, "overall_goal_complete": False})


def create_next_stage(decision: str, external: str) -> None:
    stage72.write_md(NEXT_STAGE_ROOT / "00_spec" / "stage_brief.md", f"""# {NEXT_STAGE_ID}

Stage74(74단계)는 Stage73(73단계)의 v41 TP/risk(브이41 익절/위험) 결과를 받아 candidate review(후보 검토), continued repair(계속 수리), 또는 another model source(다른 모델 원천)를 고르는 follow-up(후속) 단계다.

## Bounded Question(경계 질문)

Stage73(73단계)의 TP/risk(익절/위험) 조합이 34D KPI(34D 핵심 성과 지표) 격차를 줄일 만큼 강한가?

Effect(효과): Stage74(74단계)는 다음 판단을 하나의 경계 질문으로 좁힌다.

## Boundary(경계)

`{BOUNDARY}`
""")
    stage72.write_md(NEXT_STAGE_ROOT / "01_inputs" / "input_refs.md", f"""# Stage74 Input References(74단계 입력 참조)

- source_stage(원천 단계): `{STAGE_ID}`
- source_run(원천 실행): `{RUN_ID}`
- source_decision(원천 판정): `{decision}`
- source_external_verification_status(원천 외부 검증 상태): `{external}`
- stage73_report(73단계 보고서): `{rel(REPORT_PATH)}`
- stage73_decision(73단계 판정): `{rel(DECISION_PATH)}`
- summary(요약): `{rel(SUMMARY_CSV_PATH)}`
- target_surface(목표 표면): `{TARGET_SURFACE}`

Effect(효과): Stage74(74단계)는 v2 고유 근거만 이어받아 34D KPI(34D 핵심 성과 지표) 격차 축소를 계속한다.
""")
    stage72.write_md(NEXT_STAGE_ROOT / "03_reviews" / "review_index.md", f"""# Stage74 Review Index(74단계 검토 색인)
...
Effect(효과): Stage74(74단계)는 Stage73(73단계) closeout(종료 기록)을 이어받아 다음 bounded batch(경계 묶음 실행)만 검토한다.
""")
    stage72.write_md(NEXT_STAGE_ROOT / "04_selected" / "selection_status.md", f"""# Stage74 Selection Status(74단계 선택 상태)
...
Effect(효과): Stage74(74단계)는 34D KPI(34D 핵심 성과 지표) 격차 축소를 계속하지만, 운영 의미 없이 연구개발로만 이어진다.
""")
...
- >-
  Stage73(73단계) closed(종료) as `{decision}` and Stage74(74단계) `{NEXT_STAGE_ID}` is open_planned(열림 계획). Effect(효과): v41 TP/risk(브이41 익절/위험) 근거를 보존하고 다음 경계 연구 질문으로만 넘긴다.
- >-
  Stage73 result(73단계 결과): validation/OOS(검증/표본외), PF/net/DD(수익 팩터/순손익/손실률), risk/ATR telemetry(위험/ATR 텔레메트리)는 `{rel(SUMMARY_CSV_PATH)}`와 `{rel(RISK_ATR_TELEMETRY_PATH)}`에 기록된다. Effect(효과): 34D target surface(34D 목표 표면) 대비 KPI(핵심 성과 지표) 차이를 다음 단계 입력으로 보존한다.
- >-
...
stage73_v41_tp_risk_followup:
  ...
  boundary: {BOUNDARY}
"""
    if "stage73_v41_tp_risk_followup:" in text:
        text = re.sub(r"\nstage73_v41_tp_risk_followup:\n(?:  .*\n)+", block + "\n", text, count=1)
    else:
        text = text.rstrip() + block
    io_path(WORKSPACE_STATE_PATH).write_text(text + ("\n" if not text.endswith("\n") else ""), encoding="utf-8-sig")
    stage72.write_md(SELECTED_ROOT / "selection_status.md", f"""# Stage73 Selection Status(73단계 선택 상태)

- stage_status(단계 상태): `closed_{decision}`
- source_stage(원천 단계): `{SOURCE_STAGE72_ID}`
- source_decision(원천 판정): `continue_v41_gate_repair_in_stage73`
- source_stage72_pushed_commit(원천 72단계 푸시 커밋): `{SOURCE_STAGE72_COMMIT}`
- current_run(현재 실행): `{RUN_ID}`
- adapter_under_review(검토 중 어댑터): `{SOURCE_ADAPTER_ID}`
- target_surface(목표 표면): `{TARGET_SURFACE}`
- stage73_decision(73단계 판정): `{decision}`
- next_stage_or_branch(다음 단계/분기): `{NEXT_STAGE_ID}`
- claim_boundary(주장 경계): `{BOUNDARY}`

Effect(효과): Stage73(73단계)는 v41 TP/risk(브이41 익절/위험) 조합을 측정하고, 운영 의미 없이 Stage74(74단계)로 넘긴다.
""")
    stage72.write_md(CURRENT_WORKING_STATE_PATH, f"""# Current Working State(현재 작업 상태)
...
- target_surface(목표 표면): `{TARGET_SURFACE}`
- adapter_under_review(검토 중 어댑터): `{SOURCE_ADAPTER_ID}`
- status(상태): `stage73_closed_{decision}`
- claim_boundary(주장 경계): `{BOUNDARY}`

Stage73(73단계) closed(종료) as v2-native v41 TP/risk follow-up batch(브이투 고유 브이41 익절/위험 후속 묶음). Effect(효과): legacy 34D(레거시 34D)는 target surface(목표 표면)로만 쓰였고, 다음 연구는 Stage74(74단계)로 이어진다.

## Latest Stage73 Evidence(최신 73단계 근거)
...
def append_changelog(decision: str) -> None:
    entry = (
        "\n## 2026-05-17 - Stage73 v41 TP/risk follow-up closeout(73단계 v41 익절/위험 후속 종료)\n\n"
        f"- run(실행): `{RUN_ID}`\n"
        f"- decision(판정): `{decision}`\n"
        f"- target_surface(목표 표면): `{TARGET_SURFACE}`\n"
        "- effect(효과): Stage71(71단계)의 0.08 short gate(숏 게이트)를 복원하고 risk cap(위험 상한)과 TP(익절 폭)를 좁게 바꿔 PF/net/DD(수익 팩터/순손익/손실률) 균형을 측정했다.\n"
    )
...
def clean_decision_markdown(decision: str, external: str) -> str:
    return f"""# Stage73 Decision(73단계 판정)

decision(판정): `{decision}`

Stage73(73단계)는 Stage71(71단계)의 0.08 short gate(숏 게이트)를 복원하고 risk cap(위험 상한)과 TP(take profit, 익절 폭)를 좁게 바꿔 PF/net/DD(수익 팩터/순손익/손실률) 균형을 측정했다.

Effect(효과): 이번 단계 결과는 operating claim(운영 주장)이 아니라, 다음 bounded research(경계 연구)의 근거만 만든다.
...
def clean_create_next_stage(decision: str, external: str) -> None:
    stage72.write_md(NEXT_STAGE_ROOT / "00_spec" / "stage_brief.md", f"""# {NEXT_STAGE_ID}

Stage74(74단계)는 Stage73(73단계)의 v41 TP/risk(V41 익절 폭/위험) 결과를 받아 candidate review(후보 검토), continued repair(계속 수리), 또는 another model source(다른 모델 원천)를 고르는 follow-up(후속) 단계다.

## Bounded Question(경계 질문)

Stage73(73단계)의 TP/risk(익절 폭/위험) 조합이 34D KPI(34D 핵심 성과 지표) 격차를 줄일 만큼 강한가?
...
Effect(효과): Stage74(74단계)는 v2-native evidence(v2 고유 근거)만 이어받아 34D KPI(34D 핵심 성과 지표) 격차 축소를 계속한다.
""")
    stage72.write_md(NEXT_STAGE_ROOT / "03_reviews" / "review_index.md", f"""# Stage74 Review Index(74단계 검토 색인)
...
def clean_update_current_truth(decision: str, external: str) -> None:
    stage72.write_md(SPEC_ROOT / "stage_brief.md", f"""# {STAGE_ID}

Stage73(73단계)는 Stage72(72단계)의 약한 softer gate(더 약한 게이트) 결과를 받아, Stage71(71단계)의 0.08 short gate(숏 게이트)를 복원한 뒤 risk cap(위험 상한)과 TP(take profit, 익절 폭)만 좁게 바꿔 본 follow-up(후속) 단계다.

## Bounded Question(경계 질문)

Stage71(71단계)의 0.08 short gate(숏 게이트)를 복원한 v41 source(v41 원천)에서 risk cap(위험 상한)과 TP(익절 폭) 변경이 34D KPI(34D 핵심 성과 지표) 격차를 줄일 만큼 강한가?

Effect(효과): Stage73(73단계)는 adapter(어댑터)를 넓게 다시 찾지 않고, TP/risk(익절 폭/위험) 영향만 측정한다.
...
""")
    ...
    current_focus = f"""current_focus:
- >-
  Stage73(73단계) closed(종료) as `{decision}` and Stage74(74단계) `{NEXT_STAGE_ID}` is open_planned(열림 계획). Effect(효과): v41 TP/risk(V41 익절 폭/위험) 근거를 보존하고 다음 경계 연구 질문으로만 넘긴다.
- >-
  Stage73 result(73단계 결과): validation/OOS(검증/표본외), PF/net/DD(수익 팩터/순손익/손실률), risk/ATR telemetry(위험/ATR 텔레메트리)는 `{rel(SUMMARY_CSV_PATH)}`와 `{rel(RISK_ATR_TELEMETRY_PATH)}`에 기록된다. Effect(효과): 34D target surface(34D 목표 표면) 대비 KPI(핵심 성과 지표) 차이를 다음 단계 입력으로 보존한다.
- >-
...
Effect(효과): Stage73(73단계)는 v41 TP/risk(V41 익절 폭/위험) 조합을 측정하고, 운영 의미 없이 Stage74(74단계)로 넘긴다.
""")
    stage72.write_md(CURRENT_WORKING_STATE_PATH, f"""# Current Working State(현재 작업 상태)
...
Stage73(73단계) closed(종료) as v2-native v41 TP/risk follow-up batch(v2 고유 v41 익절 폭/위험 후속 묶음). Effect(효과): legacy 34D(레거시 34D)는 target surface(목표 표면)로만 쓰였고, 다음 연구는 Stage74(74단계)로 이어진다.
...
Forbidden claims(금지 주장): deployment(배포), live_readiness(실거래 준비), runtime_authority(런타임 권위), operating_promotion(운영 승격), operating_reference(운영 기준), production_baseline(생산 기준선), legacy_inheritance(레거시 상속).
""")
    clean_create_next_stage(decision, external)


def clean_append_changelog(decision: str) -> None:
    entry = (
...
def install_patches() -> None:
    stage72.configure_base = configure_base
    stage72.decide = decide
    stage72.report_markdown = clean_report_markdown
    stage72.decision_markdown = clean_decision_markdown
    stage72.tier_b_rows = tier_b_rows
    stage72.artifact_rows = artifact_rows
    stage72.write_run_identity = write_run_identity
    stage72.write_ledgers = write_ledgers
    stage72.write_packet_files = write_packet_files
    stage72.create_next_stage = clean_create_next_stage
    stage72.update_current_truth = clean_update_current_truth
    stage72.append_changelog = clean_append_changelog
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    prepared = {
        ...
        "model_family": "baseline_adapter_stage73_v2_native_v41_tp_risk_followup_ebm_table",
        "feature_set_id": "stage73_v41_tp_risk_signal",
    ...
    stage72.write_csv(AUDIT_CSV_PATH, audit_rows)
    stage72.write_csv(SUMMARY_CSV_PATH, summary_rows)
    stage72.write_csv(SEGMENT_KPI_PATH, segment_rows)
    stage72.write_csv(RISK_ATR_TELEMETRY_PATH, risk_rows)
    stage72.write_csv(GATE_FEATURE_SUMMARY_PATH, inputs["gate_rows"])
    stage72.write_csv(TIER_B_DIAGNOSTIC_PATH, tier_b_rows())
    stage72.write_md(REPORT_PATH, clean_report_markdown(summary_rows, segment_rows, target_rows, decision, external))
    stage72.write_md(DECISION_PATH, clean_decision_markdown(decision, external))
    stage72.write_json(
        SUMMARY_JSON_PATH,
        {
            "run_id": RUN_ID,
    ...
    if not args.materialize_only:
        clean_update_current_truth(decision, external)
        clean_append_changelog(decision)
    print(
        json.dumps(
            json_ready(
                {
