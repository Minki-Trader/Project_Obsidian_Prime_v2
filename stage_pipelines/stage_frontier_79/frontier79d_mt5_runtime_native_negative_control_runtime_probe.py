...
RUN_ID = "frontier79D_mt5_runtime_native_negative_control_runtime_probe_v1"
PARENT_RUN_ID = "frontier79C_pre_mt5_grok_runtime_native_negative_control_runtime_probe_v1"
NEXT_RUN_ID = "frontier79E_proxy_runtime_gap_analysis_and_repair_decision_v1"
COMMON_RUN_ROOT = "Project_Obsidian_Prime_v2/frontier79D_mt5_runtime_native_negative_control_runtime_probe"
THRESHOLD_EPSILON = 1e-7
RUNTIME_CANDIDATE_PREFIX = "f79d_runtime"
RUN_SHORT_LABEL = "F79D"
ATTEMPT_PREFIX = "f79d_runtime_native_negative_control"
EXPLORATION_LABEL = "frontier79D_runtime_native_negative_control_runtime_probe"
ATTEMPT_ROLE = "runtime_native_negative_control_runtime_probe"
RECORD_VIEW_PREFIX = "mt5_f79d_runtime_native_negative_control"
CLAIM_BOUNDARY = (
...
TARGET_SELECTION_PATH = REVIEW_DIR / "f79c_runtime_materialization_target_selection.json"
REPORT_PATH = REVIEW_DIR / "frontier79D_mt5_runtime_native_negative_control_runtime_probe_report.md"
GATE_AUDIT_PATH = REVIEW_DIR / "required_gate_coverage_audit_f79d.md"
SUMMARY_PATH = REVIEW_DIR / "f79d_mt5_runtime_native_runtime_probe_summary.json"
RUNTIME_PARITY_PATH = REVIEW_DIR / "f79d_runtime_parity_receipt.json"
BACKTEST_FORENSICS_PATH = REVIEW_DIR / "f79d_backtest_forensics_receipt.json"
...
SCRIPT_REL = "stage_pipelines/stage_frontier_79/frontier79d_mt5_runtime_native_negative_control_runtime_probe.py"
...
def configure_runtime_base() -> None:
    ...
    runtime_base.SLTP_POINT_SCALE = f79b.SLTP_POINT_SCALE
    runtime_base.TRADE_SHAPE_LABEL = "runtime_native_trade_shape_label_from_fill_path"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="F79D MT5 runtime-native negative-control runtime probe.")
...
def ensure_dirs() -> None:
    ...
    for path in (MODEL_DIR, FEATURE_DIR, VETO_DIR, MT5_DIR, REVIEW_DIR, SELECTED_DIR):
...
def target_row() -> dict[str, Any]:
    ...
    if target.get("candidate_id") != "f79b_02371":
...
def build_context(target: Mapping[str, Any]) -> dict[str, Any]:
    ...
    f78b.INITIAL_BALANCE = f79b.INITIAL_BALANCE
    try:
        frame, raw, features = f78b.load_inputs()
        indices = f79b.entry_indices(frame, raw, str(target["entry_mode"]))
        spec = next(item for item in f79b.runtime_specs() if item.name == str(target["label_name"]))
        outcome = f79b.compute_outcome(raw, indices, spec)
        label = f79b.make_label(frame, outcome, spec)
        train_valid = (frame["split"] == "train").to_numpy() & np.asarray(outcome["valid"], dtype=bool)
        feature_columns = f79b.feature_sets(features)[str(target["feature_set"])]
        ...
        model = f79b.model_builders()[str(target["model"])]()
        ...
        for split, prefix in (("validation", "val"), ("oos", "oos")):
            ...
            raw_key = "validation_raw_signal_count" if split == "validation" else "oos_raw_signal_count"
            lifecycle_key = "validation_lifecycle_trade_count" if split == "validation" else "oos_lifecycle_trade_count"
        ...
        return {
            ...
            "known_differences": [
                "Proxy labels use same-bar open fill-path simulation(프록시는 동일 봉 시가 체결 경로 시뮬레이션을 쓴다).",
                "MT5 uses broker tester fill semantics(MT5는 브로커 테스터 체결 의미를 쓴다).",
                "Runtime veto tape forces selected-entry timestamps(런타임 거부 테이프가 선택 진입 시각을 강제한다).",
            ],
        }
    finally:
...
def patch_binary_onnx_to_side_three_columns(binary_path: Path, patched_path: Path, side: str) -> dict[str, Any]:
    ...
    initializers = [
        numpy_helper.from_array(np.asarray([1], dtype=np.int64), name="f79_slice_axes_col"),
        numpy_helper.from_array(np.asarray([1], dtype=np.int64), name="f79_slice_steps_col"),
        numpy_helper.from_array(np.asarray([0], dtype=np.int64), name="f79_slice_start_col0"),
        numpy_helper.from_array(np.asarray([1], dtype=np.int64), name="f79_slice_end_col1"),
        numpy_helper.from_array(np.asarray([1], dtype=np.int64), name="f79_slice_start_col1"),
        numpy_helper.from_array(np.asarray([2], dtype=np.int64), name="f79_slice_end_col2"),
        numpy_helper.from_array(np.asarray([0.0], dtype=np.float32), name="f79_zero_scalar"),
    ]
    model.graph.initializer.extend(initializers)
    model.graph.node.extend(
        [
            helper.make_node("Slice", [prob_name, "f79_slice_start_col0", "f79_slice_end_col1", "f79_slice_axes_col", "f79_slice_steps_col"], ["f79_flat_col"], name="F79SliceFlatProbability"),
            helper.make_node("Slice", [prob_name, "f79_slice_start_col1", "f79_slice_end_col2", "f79_slice_axes_col", "f79_slice_steps_col"], ["f79_event_col"], name="F79SliceEventProbability"),
            helper.make_node("Mul", ["f79_flat_col", "f79_zero_scalar"], ["f79_zero_col"], name="F79BuildZeroProbability"),
        ]
    )
    concat_inputs = ["f79_zero_col", "f79_flat_col", "f79_event_col"] if side == "long" else ["f79_event_col", "f79_flat_col", "f79_zero_col"]
    model.graph.node.extend([helper.make_node("Concat", concat_inputs, ["probabilities_3"], name="F79BuildThreeColumnProbability", axis=1)])
    ...
    schema = "[p_short=0,p_flat,p_long=event]" if side == "long" else "[p_short=event,p_flat,p_long=0]"
    return {
        "binary_probability_output": prob_name,
        "patched_probability_output": "probabilities_3",
        "path": rel(patched_path),
        "sha256": f74e.sha256_file(patched_path),
        "schema": schema,
        "positive_side": side,
    }
...
def materialize(context: Mapping[str, Any], common_files_root: Path) -> tuple[dict[str, Any], list[dict[str, Any]], list[dict[str, Any]], list[dict[str, Any]]]:
    ...
    f74e.joblib.dump(context["model"], io_path(model_path))
    ...
    patch_meta = patch_binary_onnx_to_side_three_columns(raw_onnx_path, patched_onnx_path, str(target["side"]))
    ...
    artifact = {
        "candidate_id": candidate_id,
        "source_candidate_id": target.get("candidate_id"),
        "materialization_mode": f"{side}_binary_onnx_three_column_runtime_native_negative_control",
        "surrogate_boundary": "best_exportable_weak_nonzero_target_no_surrogate_substitution",
        ...
        "cooldown_bars_proxy": target.get("cooldown_bars"),
    ...
    artifact["export_status"] = "negative_control_parity_passed" if probability_ok and signal_ok and feature_ok and reproduction_ok else "negative_control_parity_failed"
...
def build_attempts(context: Mapping[str, Any], artifact: Mapping[str, Any], *, include_oos: bool) -> list[dict[str, Any]]:
    ...
    for split in splits:
        ...
        extra = {
            ...
            "InpAtrMinStopPoints": float(target.get("sl_broker_points", spec.sl_price_units * f79b.SLTP_POINT_SCALE)),
            "InpAtrMaxStopPoints": float(target.get("sl_broker_points", spec.sl_price_units * f79b.SLTP_POINT_SCALE)),
            "InpAtrMinTakeProfitPoints": float(target.get("tp_broker_points", spec.tp_price_units * f79b.SLTP_POINT_SCALE)),
            "InpAtrMaxTakeProfitPoints": float(target.get("tp_broker_points", spec.tp_price_units * f79b.SLTP_POINT_SCALE)),
        ...
        attempt.update(
            {
                ...
                "axis_id": f"{side}_h{target.get('hold_bars')}_tp{target.get('tp_price_units')}_sl{target.get('sl_price_units')}_{target.get('feature_set')}_{target.get('model')}_{target.get('session')}_{target.get('risk_filter')}_q{target.get('prob_quantile')}",
...
def build_summary(payload: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "candidate_id": artifact.get("candidate_id", "f79d_runtime_f79b_02371"),
        "source_candidate_id": artifact.get("source_candidate_id", "f79b_02371"),
...
def backtest_forensics_receipt(payload: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "report_identity": {
            "runtime_receipt_path": rel(RUN_DIR / "f79d_runtime_receipt.csv"),
            "execution_results_path": rel(RUN_DIR / "f79d_execution_results.json"),
            "summary_path": rel(SUMMARY_PATH),
        },
        "trade_evidence": receipt,
        "cost_assumptions": {
            "spread": "proxy includes raw spread cost(프록시는 원시 스프레드 비용을 포함); tester uses broker execution(테스터는 브로커 체결을 사용)",
...
def report_text(payload: Mapping[str, Any], created_at: str) -> str:
    ...
    lines = [
        ...
        "",
        f"- validation proxy(검증 프록시): net/PF/DD/calendar_tpd/trades(순수익/수익 팩터/손실폭/달력 일 거래/거래) `{target.get('val_net')}/{target.get('val_pf')}/{target.get('val_dd_pct')}/{target.get('val_calendar_trades_day')}/{target.get('val_trade_count')}`",
        f"- OOS proxy(표본외 프록시): net/PF/DD/calendar_tpd/trades(순수익/수익 팩터/손실폭/달력 일 거래/거래) `{target.get('oos_net')}/{target.get('oos_pf')}/{target.get('oos_dd_pct')}/{target.get('oos_calendar_trades_day')}/{target.get('oos_trade_count')}`",
        f"- signal count proxy(신호 수 프록시): validation raw/selected(검증 원신호/선택) `{target.get('validation_raw_signal_count')}/{target.get('validation_lifecycle_trade_count')}`, OOS raw/selected(표본외 원신호/선택) `{target.get('oos_raw_signal_count')}/{target.get('oos_lifecycle_trade_count')}`",
        "",
...
def gate_audit_text(payload: Mapping[str, Any], created_at: str) -> str:
    summary = build_summary(payload)
    return f"""# Required Gate Coverage Audit F79D(F79D 필수 게이트 커버리지 감사)

Updated(갱신): {created_at}

| gate(게이트) | status(상태) | evidence/effect(근거/효과) |
|---|---|---|
| F79C Grok accepted with conditions(F79C Grok 조건부 수용) | `passed(통과)` | `stages/{STAGE_ID}/03_reviews/grok_pre_mt5_runtime_native_negative_control_runtime_probe_receipt.md` |
...
| probability parity(확률 동등성) | `{summary['probability_parity_pass_rows']}/3` | ONNX long schema(ONNX 롱 스키마) |
| signal count parity(신호 수 동등성) | `{summary['signal_parity_pass_rows']}/3` | selected-entry veto tape(선택 진입 거부 테이프) |
| feature readiness parity(피처 준비 동등성) | `{summary['feature_readiness_pass_rows']}/1` | 37 feature CSV(37개 피처 CSV) |
...
def ledger_row(payload: Mapping[str, Any], created_at: str) -> dict[str, Any]:
    ...
    return {
        ...
        "guardrail_kpi": f"signal_diff={best.get('signal_count_diff', '')};feature_diff={best.get('feature_ready_diff', '')};candidate=f79b_02371",
        ...
        "notes": f"attempts={summary['attempt_count']};completed={summary['completed_attempt_count']};candidate=f79d_runtime_f79b_02371",
        "lane": "mt5_runtime_probe(MT5 런타임 탐침)",
        "family": "runtime_backtest(런타임 백테스트)",
        "primary_report": rel(REPORT_PATH),
        "run_number": "frontier79D",
        "date": created_at[:10],
        ...
        "question": "Does runtime-native fill-path labeling survive MT5 runtime?(런타임 네이티브 체결 경로 라벨링이 MT5 런타임에서 살아남는가?)",
        "artifact_count": "14",
        ...
        "run_type": "mt5_runtime_native_negative_control_runtime_probe",
...
def update_registers_and_state(payload: Mapping[str, Any], created_at: str) -> None:
    summary = build_summary(payload)
    best = summary["best_runtime"]
    marker = "<!-- frontier79D_mt5_runtime_native_negative_control_runtime_probe_v1 -->"
...
next_run_id: {NEXT_RUN_ID}
runtime_probe_status: f79_mandatory_runtime_probe_attempted
runtime_authority: not_claimed
operating_promotion: not_claimed
...
notes:
  - "Action(행동): F79D MT5 Runtime Probe(MT5 런타임 탐침)를 실행/시도했다."
  - "Effect(효과): F79E proxy/runtime gap analysis(프록시/런타임 간극 분석)와 repair decision(수리 결정)에 필요한 근거를 만들었다."
...
Effect(효과): F79B runtime-native proxy(런타임 네이티브 프록시)를 Strategy Tester(전략 테스터) 근거로 물질화했고, 다음 F79E gap analysis(간극 분석)의 입력을 만들었다.
...
Effect(효과): 다음 실행은 F79E proxy/runtime gap analysis(프록시/런타임 간극 분석)와 repair decision(수리 결정)이다.
...
def build_payload(
    *,
    ...
    return {
        ...
        "source_reproduction": list(context["reproduction_rows"]),
...
def main() -> int:
    ...
    if artifact.get("export_status") != "negative_control_parity_passed":
        ...
        status = "completed_mt5_runtime_native_negative_control_runtime_probe_observation_no_authority"
        judgment = "runtime_probe_completed_gap_analysis_required_no_authority"
    elif args.execute:
        status = "blocked_mt5_runtime_native_runtime_probe_attempted_no_authority"
    ...
    write_csv(RUN_DIR / "f79d_probability_parity.csv", probability)
    write_csv(RUN_DIR / "f79d_signal_parity.csv", signal)
    write_csv(RUN_DIR / "f79d_feature_readiness_parity.csv", feature_parity)
    write_csv(RUN_DIR / "f79d_source_reproduction.csv", context["reproduction_rows"])
    write_csv(RUN_DIR / "f79d_runtime_receipt.csv", runtime_receipt, f71d.RUNTIME_RECEIPT_COLUMNS)
    write_json(RUN_DIR / "f79d_execution_results.json", execution_results)
