...
RUN_ID = "run299B_runtime_realized_trade_shape_mt5_probe_v1"
RUN_NUMBER = "run299B"
SOURCE_RUN_ID = "run299A_design_runtime_realized_trade_shape_rebuild_v1"
PARENT_RUN_ID = "run298C_review_profit_scale_edge_amplification_mt5_probe_v1"
STATUS_PREPARED = "prepared_runtime_realized_trade_shape_mt5_probe_no_runtime_kpi"
...
EXPLORATION_LABEL = "stage299_Model__RuntimeRealizedTradeShapeReplay"
SIGNAL_COLUMN = "run299b_route_signal"
FEATURE_ORDER = (SIGNAL_COLUMN,)
COMMON_ROOT = "Project_Obsidian_Prime_v2/stage299/run299B_runtime_realized_trade_shape"
...
MT5_QUEUE = RUN299A / "mt5_probe_queue.csv"
SOURCE_MANIFEST = RUN299A / "candidate_payload_manifest.csv"
SOURCE_RUN_MANIFEST = RUN299A / "run_manifest.json"
SOURCE_REPORT = REVIEWS / "run299A_runtime_realized_trade_shape_materialization_report.md"
PRODUCER = Path("stage_pipelines/stage299/execute_runtime_realized_trade_shape_mt5_probe.py")
...
REPORT = REVIEWS / "run299B_runtime_realized_trade_shape_mt5_probe_report.md"
...
def attach_identity(attempt: dict[str, Any], queue_row: Mapping[str, str], max_hold: int, close_on_flat: bool, same_reentry: int) -> None:
    ...
    attempt["stage299_branch_id"] = queue_row.get("materialized_branch_id", "")
    ...
    for queue_row in queue_rows:
        ...
        for runtime_split, split_token in (("validation_is", "val"), ("oos", "oos")):
            for tier_key, tier_label, tier_token in (
                ...
                attempt["signal_policy"] = "stage299 runtime-realized trade shape route_signal_value -1/0/+1 through single-feature table"
            ...
            if include_routed:
                ...
                attach_identity(attempt, queue_row, max_hold, close_on_flat, same_reentry)
                attempt["signal_policy"] = "Tier A primary + Tier B fallback stage299 runtime-realized trade shape route_signal_value"
                attempts.append(attempt)
    return attempts
...
def classify_status(result: Mapping[str, Any], materialize_only: bool) -> tuple[str, str, str, str]:
    ...
    if materialize_only:
        return (
            STATUS_PREPARED,
            "runtime_realized_trade_shape_runtime_probe_prepared_no_external_execution",
            "out_of_scope_by_claim_materialize_only",
            "run299B_execute_runtime_realized_trade_shape_mt5_probe_external_check",
        )
    ...
    if planned and completed_exec >= planned and len(kpis) >= planned:
        return (
            "completed_runtime_realized_trade_shape_mt5_probe_no_selection",
            "runtime_probe_completed_requires_profit_scale_curve_review_no_selection",
            "completed",
            "run299C_review_runtime_realized_trade_shape_mt5_probe",
        )
    if kpis:
        return (
            "partial_runtime_realized_trade_shape_mt5_probe_no_selection",
            "runtime_probe_partial_requires_continuation_or_review_no_selection",
            "partial_or_blocked",
            "run299B_continue_runtime_realized_trade_shape_mt5_probe" if limited else "run299C_review_with_runtime_gaps",
        )
    return (
        "blocked_runtime_realized_trade_shape_mt5_probe_no_kpi",
        "runtime_probe_blocked_no_kpi_no_selection",
        "blocked_or_invalid",
        "run299B_repair_or_block_runtime_realized_trade_shape_mt5_probe",
    )
...
def report_markdown(result: Mapping[str, Any], status: str, judgment: str, external_status: str, next_action: str) -> str:
    ...
    return "\n".join(
        [
            "# run299B Runtime-Realized Trade Shape MT5 Probe(299B 런타임 실제 거래 형태 MT5 탐침)",
            "",
            ...
            "",
            "Effect(효과): run299A(299A 실행)의 runtime-realized trade shape(런타임 실제 거래 형태) 신호가 실제 MT5 tester(MT5 테스터)에서 순수익, 거래 수, 곡선 품질로 재현되는지 확인했다.",
            "",
            "## Boundary(경계)",
            "",
            f"`{BOUNDARY}`",
        ...
        [
            {
                "run_id": RUN_ID,
                "stage_id": STAGE_ID,
                "lane": "runtime_realized_trade_shape_mt5_probe",
                "status": status,
                "judgment": judgment,
        ...
        [
            {
                ...
                "record_view": "runtime_realized_trade_shape_mt5_probe",
        ...
        [
            {
                ...
                "stage_id": STAGE_ID,
                "run_id": RUN_ID,
                "view": "runtime_realized_trade_shape_mt5_probe",
                "tier_scope": "Tier A used/Tier B fallback stress/actual routed total",
    ...
    rows = [
        {
            ...
            "artifact_type": "stage299_runtime_realized_trade_shape_mt5_artifact",
            "path": prev.prev.e295.e294.base.base.rel(path),
            "sha256": prev.prev.e295.e294.base.base.sha256_file_lf_normalized(path),
            "stage_id": STAGE_ID,
            "run_id": RUN_ID,
            "created_at_utc": created_at,
            "notes": "run299B runtime-realized trade shape MT5 probe",
        }
        for path in paths
...
def update_docs(status: str, judgment: str, next_action: str, kpi_count: int, attempt_count: int) -> None:
    ...
    selected = prev.prev.e295.e294.base.base.append_once(selected, "run299B_report", f"- run299B_report(299B 보고): `{prev.prev.e295.e294.base.base.rel(REPORT)}`")
    selected = prev.prev.e295.e294.base.base.append_once(selected, "run299B_execution_result", f"- run299B_execution_result(299B 실행 결과): `{prev.prev.e295.e294.base.base.rel(EXECUTION_RESULT)}`")
    prev.prev.e295.e294.base.base.write_md(SELECTED, selected)

    review_index = prev.prev.e295.e294.base.base.io_path(REVIEW_INDEX).read_text(encoding="utf-8-sig") if prev.prev.e295.e294.base.base.path_exists(REVIEW_INDEX) else "# Stage299 Review Index(299단계 검토 색인)\n"
    review_index = prev.prev.e295.e294.base.base.append_once(
        review_index,
        "run299B_report",
        f"- run299B_report(299B 보고): `{prev.prev.e295.e294.base.base.rel(REPORT)}`\n- run299B_execution_result(299B 실행 결과): `{prev.prev.e295.e294.base.base.rel(EXECUTION_RESULT)}`\n- run299B_mt5_kpi_summary(299B MT5 KPI 요약): `{prev.prev.e295.e294.base.base.rel(MT5_KPI_SUMMARY)}`",
    )
    ...
    current = prev.prev.e295.e294.base.base.append_once(
        current,
        "run299B_summary",
        f"- run299B_summary(299B 요약): runtime-realized trade shape MT5 probe(런타임 실제 거래 형태 MT5 탐침)를 실행했다. Effect(효과): attempts(시도) `{attempt_count}`개와 MT5 KPI records(MT5 KPI 기록) `{kpi_count}`개를 엮었고 후보/어댑터/ONNX(온엑스) 주장은 하지 않는다.",
    )
    ...
    focus = (
        f"- >-\n"
        f"  Stage299(299단계) run299B(299B 실행) runtime-realized trade shape MT5 probe(런타임 실제 거래 형태 MT5 탐침) `{RUN_ID}`. "
    ...
    changelog = prev.prev.e295.e294.base.base.append_once(
        changelog,
        RUN_ID,
        f"## {UPDATED_ON} run299B runtime-realized trade shape MT5 probe(299B 런타임 실제 거래 형태 MT5 탐침)\n\n"
        f"- status(상태): `{status}`\n"
        f"- judgment(판정): `{judgment}`\n"
