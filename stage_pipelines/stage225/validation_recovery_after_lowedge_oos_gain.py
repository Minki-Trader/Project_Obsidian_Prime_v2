...
from stage_pipelines.stage223 import oos_recovery_after_no_long_block_validation_gain as prior  # noqa: E402
...
STAGE_ID = "225_adapter_research__validation_recovery_after_lowedge_oos_gain"
RUN_NUMBER = "run225A"
RUN_ID = "run225A_stage225_validation_recovery_after_lowedge_oos_gain_v1"
PACKET_ID = "stage225_validation_recovery_after_lowedge_oos_gain_v1"
PARENT_RUN_ID = "run224A_stage224_stage223_oos_recovery_followup_review_v1"
SOURCE_STAGE_ID = "224_adapter_research__stage223_oos_recovery_followup_review"
SOURCE_RUN_ID = PARENT_RUN_ID
SOURCE_STAGE224_EVIDENCE_COMMIT = "98093eae8fd0d033bff8560b3aa8a8304c491885"
SOURCE_STAGE224_HASH_RECORD_COMMIT = "6abc125d59657ed6b192d63f1773a7dba6cb952c"
SOURCE_OOS_GAIN_ID = "s223_oos_lowedge_long_guard"
NEXT_STAGE_ID = "226_adapter_research__stage225_validation_recovery_followup_review"
NEXT_RUN_ID = "run226A_stage226_stage225_validation_recovery_followup_review_v1"
NEXT_PACKET_ID = "stage226_stage225_validation_recovery_followup_review_v1"
...
STAGE210_ANCHOR = prior.STAGE210_ANCHOR
STAGE219_CONTROL = prior.STAGE219_CONTROL
STAGE223_OOS_GAIN = {
    "adapter_id": SOURCE_OOS_GAIN_ID,
...
COMMON_ROOT = f"OPV2/s225a/{RUN_NUMBER}"

SUMMARY_JSON_PATH = REVIEWS_ROOT / "stage225_summary.json"
SUMMARY_CSV_PATH = REVIEWS_ROOT / "stage225_validation_recovery_kpi_summary.csv"
REPORT_PATH = REVIEWS_ROOT / "stage225_validation_recovery_report.md"
SEGMENT_KPI_PATH = REVIEWS_ROOT / "stage225_segment_kpi_summary.csv"
BALANCE_CURVE_AUDIT_PATH = REVIEWS_ROOT / "stage225_balance_curve_audit.csv"
MONTHLY_KPI_PATH = REVIEWS_ROOT / "stage225_monthly_kpi_summary.csv"
CONCENTRATION_PATH = REVIEWS_ROOT / "stage225_concentration_risk_summary.csv"
DRAWDOWN_PATH = REVIEWS_ROOT / "stage225_drawdown_recovery_summary.csv"
QUALITY_MATRIX_PATH = REVIEWS_ROOT / "stage225_quality_matrix.csv"
RISK_ATR_TELEMETRY_PATH = REVIEWS_ROOT / "stage225_risk_atr_telemetry.csv"
GATE_FEATURE_SUMMARY_PATH = REVIEWS_ROOT / "stage225_gate_feature_summary.csv"
PROBABILITY_BINDING_PATH = REVIEWS_ROOT / "stage225_probability_telemetry_summary.csv"
MODEL_SCORE_AUDIT_PATH = REVIEWS_ROOT / "stage225_model_score_audit.csv"
TIER_B_DIAGNOSTIC_PATH = REVIEWS_ROOT / "stage225_tier_b_diagnostic_summary.csv"
DECISION_PATH = REVIEWS_ROOT / "stage225_decision.md"
AUDIT_CSV_PATH = REVIEWS_ROOT / "stage225_trade_audit.csv"
...
PRODUCER_PATH = Path("stage_pipelines/stage225/validation_recovery_after_lowedge_oos_gain.py")
ARTIFACT_COLUMNS = prior.ARTIFACT_COLUMNS

VARIANTS = (
    repair.RepairVariant(
        adapter_id="s225_val_lowedge_lng520",
        label="stage225_val_lowedge_lng520",
        atr_enabled=True,
        ...
        long_threshold=0.520,
        close_on_flat_signal=False,
        reverse_on_opposite_signal=True,
        close_only_on_opposite_signal=False,
        max_hold_bars=3,
        notes="Stage225 control: low-edge long guard with Stage223 long threshold.",
    ),
    repair.RepairVariant(
        adapter_id="s225_val_lowedge_lng515",
        label="stage225_val_lowedge_lng515",
        atr_enabled=True,
        model_risk_enabled=True,
        fixed_lot=0.25,
        atr_stop_multiplier=2.0325,
        ...
        same_direction_reentry_cooldown_bars=8,
        short_threshold=0.54,
        long_threshold=0.515,
        close_on_flat_signal=False,
        reverse_on_opposite_signal=True,
        close_only_on_opposite_signal=False,
        max_hold_bars=3,
        notes="Stage225 validation recovery: relax long threshold to 0.515 with low-edge long guard fixed.",
    ),
    repair.RepairVariant(
        adapter_id="s225_val_lowedge_lng510",
        label="stage225_val_lowedge_lng510",
        atr_enabled=True,
        model_risk_enabled=True,
        fixed_lot=0.25,
        atr_stop_multiplier=2.0325,
        ...
        same_direction_reentry_cooldown_bars=8,
        short_threshold=0.54,
        long_threshold=0.510,
        close_on_flat_signal=False,
        reverse_on_opposite_signal=True,
        close_only_on_opposite_signal=False,
        max_hold_bars=3,
        notes="Stage225 validation recovery: relax long threshold to 0.510 with low-edge long guard fixed.",
    ),
    repair.RepairVariant(
        adapter_id="s225_val_lowedge_lng505",
        label="stage225_val_lowedge_lng505",
        atr_enabled=True,
        model_risk_enabled=True,
        fixed_lot=0.25,
        atr_stop_multiplier=2.0325,
        ...
        close_only_on_opposite_signal=False,
        max_hold_bars=3,
        notes="Stage225 validation recovery: relax long threshold to 0.505 with low-edge long guard fixed.",
    ),
)
...
VARIANT_EXTRAS: dict[str, dict[str, Any]] = {
    variant.adapter_id: {
        **BASE_EXTRA,
        ...
        "axis": variant.adapter_id.replace("s225_val_", ""),
    }
...
def apply_stage_values(context_specs: Mapping[str, Any]) -> None:
    values = {
        ...
        "SOURCE_ADAPTER_ID": SOURCE_OOS_GAIN_ID,
    ...
    for module in (prior, prior.engine, s219, s213, s210, s192, s190, base, s188, s184, s180, s178, s176, s174, s172, s161):
    ...
    for variant_index, variant in enumerate(VARIANTS, start=1):
        ...
        for split in ("validation_is", "oos"):
            ...
            for role_index, (tier, attempt_role, prefix, attempt_token) in enumerate(
                ...
                magic = 22510000 + variant_index * 100 + (1 if split == "validation_is" else 50) + role_index
                attempts.append(
                    s161.base.attempt_payload(
                        run_root=variant_root,
                        run_id=RUN_ID,
                        stage_number=225,
                        exploration_label="stage225_BaselineAdapter__ValidationRecoveryAfterLowedgeOOSGain",
...
def configure_runner() -> None:
    ...
    for variant in VARIANTS:
        ...
        context_specs[variant.adapter_id] = {
            "gate_column": f"stage225_gate_{extra['axis']}",
            "gate_type": "encoded_stage225_validation_recovery_after_lowedge_oos_gain",
            ...
            "description": "Stage225 encoded gate: long-threshold validation recovery with low-edge OOS gain guard fixed.",
        }
...
def pass_stage225(row: Mapping[str, Any]) -> bool:
    return (
...
def decide(rows: Sequence[Mapping[str, Any]], external: str) -> str:
    if external != "completed":
        return "continue_stage225_runtime_completion_due_to_incomplete_runtime_candidate_not_final"
    if any(pass_stage225(row) for row in rows):
        return "open_stage226_stage225_validation_recovery_followup_review_candidate_not_final"
    return "open_stage226_bounded_followup_due_to_validation_recovery_tradeoff_candidate_not_final"
...
def kpi_table(rows: Sequence[Mapping[str, Any]]) -> str:
    ...
    for row in rows:
        variant = next((item for item in VARIANTS if item.adapter_id == row.get("adapter_id")), None)
        long_threshold = variant.long_threshold if variant else ""
        lines.append(
            "| {adapter} | {long_threshold} | {val:.2f} | {early:.6f} | {mid:.6f} | {dd:.4f} | {oos:.2f} | {flags} |".format(
...
def report_markdown(rows: Sequence[Mapping[str, Any]], decision: str, external: str) -> str:
    best = best_row(rows)
    return f"""# Stage225 Validation Recovery Report(225단계 검증 회복 보고서)

- stage(단계): `{STAGE_ID}`
- run(실행): `{RUN_ID}`
- source_stage(원천 단계): `{SOURCE_STAGE_ID}`
- source_run(원천 실행): `{SOURCE_RUN_ID}`
- source_stage224_evidence_commit(원천 224단계 근거 커밋): `{SOURCE_STAGE224_EVIDENCE_COMMIT}`
- source_stage224_hash_record_commit(원천 224단계 해시 기록 커밋): `{SOURCE_STAGE224_HASH_RECORD_COMMIT}`
- source_oos_gain(원천 표본외 개선): `{SOURCE_OOS_GAIN_ID}`
...
- comparison_baseline(비교 기준): Stage223(223단계) `s223_oos_lowedge_long_guard`, OOS net(표본외 순손익) `{STAGE223_OOS_GAIN["oos_net"]}`, validation net(검증 순손익) `{STAGE223_OOS_GAIN["validation_net"]}`.
...
def decision_markdown(decision: str, external: str) -> str:
    return f"""# Stage225 Decision(225단계 판정)
...
Stage225(225단계) closeout(종료)은 overall goal complete(전체 목표 완료)가 아니다.

Effect(효과): Stage226(226단계) follow-up review(후속 검토)에서 validation recovery(검증 회복)와 OOS preservation(표본외 보존) 상충을 별도로 판정한다.
"""
...
def artifact_rows(result: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    for row in rows:
        if row.get("artifact_type") != "mt5_strategy_tester_report":
            row["artifact_type"] = "stage225_evidence"
            row["notes"] = "Stage225 validation recovery after low-edge OOS gain evidence."
        else:
            row["notes"] = "Stage225 MT5 Strategy Tester report."
    return rows
        ...
        [
            {
                "run_id": RUN_ID,
                "stage_id": STAGE_ID,
                "lane": "baseline_adapter_stage225_validation_recovery",
                "status": status,
                "judgment": decision,
                "path": rel(DECISION_PATH),
                "notes": s172.ledger_pairs(
                    (
                        ("source_stage224_evidence_commit", SOURCE_STAGE224_EVIDENCE_COMMIT),
                        ("source_stage224_hash_record_commit", SOURCE_STAGE224_HASH_RECORD_COMMIT),
                        ("source_oos_gain", SOURCE_OOS_GAIN_ID),
                        ("target_surface", TARGET_SURFACE),
...
def write_packet_files(result: Mapping[str, Any], decision: str, ledger_payload: Mapping[str, Any], quality: Sequence[Mapping[str, Any]]) -> None:
    ...
    s172.write_md(
        ...
        f"""# Stage225 Closeout Packet(225단계 종료 작업 묶음)
...
Stage226(226단계)는 Stage225(225단계) validation recovery(검증 회복) 결과를 follow-up review(후속 검토)하는 bounded review(경계 검토) 단계다.

## Bounded Question(경계 질문)

Did Stage225(225단계) recover validation net(검증 순손익), early PF(초반 수익요인), and mid PF(중반 수익요인) while preserving OOS net(표본외 순손익), drawdown(낙폭), model-controlled risk%(모델 제어 위험 비율), and ATR/bracket behavior(ATR/브래킷 동작)?

Effect(효과): Stage225(225단계) 안에서 추가 수리를 하지 않고 KPI(핵심 성과 지표) 상충을 별도 검토로 닫는다.
...
- status(상태): `open_planned_from_stage225`
- source_stage(원천 단계): `{STAGE_ID}`
- source_decision(원천 판정): `{decision}`
...
- stage_status(단계 상태): `open_planned_from_stage225`
...
- >-
  Stage225(225단계) closed(종료) as `{decision}` and Stage226(226단계) `{NEXT_STAGE_ID}` is open_planned(개방 계획). Effect(효과): lowedge OOS gain(저엣지 표본외 개선)을 유지한 long-threshold validation recovery(롱 문턱값 검증 회복)의 KPI(핵심 성과 지표) 상충을 별도 review(검토)로 판정한다.
- >-
  Stage225 evidence(225단계 근거)는 `{rel(REPORT_PATH)}`, `{rel(SUMMARY_CSV_PATH)}`, `{rel(QUALITY_MATRIX_PATH)}`, `{rel(MONTHLY_KPI_PATH)}`, `{rel(CONCENTRATION_PATH)}`, `{rel(RISK_ATR_TELEMETRY_PATH)}`에 있다. Effect(효과): long threshold(롱 문턱값) 완화가 validation(검증) 회복에 도움이 됐는지 확인한다.
- >-
...
"""
    if re.search(r"(?ms)^current_focus:\r?\n.*?(?=\r?\nstage\d+_)", state):
        ...
        state = state.rstrip() + "\n" + focus
    state = re.sub(r"(?ms)^stage225_validation_recovery_after_lowedge_oos_gain:\r?\n.*?(?=^stage\d+_|\Z)", "", state)
    block = f"""
stage225_validation_recovery_after_lowedge_oos_gain:
  packet_id: {PACKET_ID}
  stage_id: {STAGE_ID}
...
- adapter_under_review(검토 중 어댑터): `stage225_validation_recovery_after_lowedge_oos_gain`
- status(상태): `stage225_{decision}`
- claim_boundary(주장 경계): `{BOUNDARY}`

Stage225(225단계)는 lowedge OOS gain(저엣지 표본외 개선) 뒤 validation recovery(검증 회복)를 MT5 Strategy Tester(MetaTrader 5 전략 테스터)로 측정했다. Effect(효과): Stage226(226단계)가 KPI(핵심 성과 지표) 상충을 별도 review(검토)로 판정한다.

## Latest Stage225 Evidence(최신 225단계 근거)
...
def append_changelog(decision: str) -> None:
    ...
    if RUN_ID in existing:
        return
    entry = (
        f"\n## {s172.utc_now()} Stage225 validation recovery closeout(225단계 검증 회복 종료)\n\n"
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    prepared = {
        ...
        "model_family": "baseline_adapter_stage225_v2_native_validation_recovery_long_threshold",
        "feature_set_id": "stage225_signal_plus_lowedge_guard_long_threshold_recovery",
