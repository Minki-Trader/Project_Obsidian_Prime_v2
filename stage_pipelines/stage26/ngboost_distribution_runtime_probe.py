...
PACKET_ID = "stage26_run20B_ngboost_distribution_runtime_probe_v1"
NEXT_ACTION_COMPLETED = "stage26_closeout_and_stage27_open_only"
NEXT_ACTION_BLOCKED = "repair_run20B_ngboost_runtime_probe_then_rerun_exact_attempts"
EXPLORATION_LABEL = "stage26_Model__NGBoostDistributionRuntimeProbe"
MODEL_FAMILY = "ngboost_distribution_score_table_runtime_probe"
MODEL_BACKEND = "ebm_table"
FEATURE_SET_ID = "feature_set_v2_ngboost_distribution_runtime_features"
...
RUNTIME_FEATURE_ORDER = (
    "ngb_direction_bias",
    "ngb_nonflat_confidence",
    "ngb_flat_probability",
    "ngb_distribution_entropy",
)
RUNTIME_FEATURE_HASH = ordered_hash(RUNTIME_FEATURE_ORDER)
BOUNDARY = "ngboost_distribution_runtime_probe_only_not_edge_not_alpha_quality_not_baseline_not_promotion_not_runtime_authority"
JUDGMENT_COMPLETED = "inconclusive_ngboost_distribution_runtime_probe_completed"
JUDGMENT_BLOCKED = "blocked_ngboost_distribution_runtime_probe_after_attempt"
...
REVIEW_PATH = STAGE_ROOT / "03_reviews/run20B_ngboost_distribution_runtime_probe_packet.md"
DECISION_PATH = ROOT / "docs/decisions/2026-05-05_stage26_run20B_ngboost_distribution_runtime_probe.md"
...
def load_source_summary() -> dict[str, Any]:
    ...
    if summary.get("selected_variant_id") != "v02_core42_distribution_surface":
        raise RuntimeError(f"Unexpected Stage26 selected variant: {summary.get('selected_variant_id')}")
    return summary
...
def load_prediction_artifacts(summary: Mapping[str, Any]) -> dict[str, pd.DataFrame]:
    ...
    if missing:
        raise FileNotFoundError(f"Missing Stage26 run20A prediction artifacts: {missing}")
    return {name: pd.read_parquet(io_path(path)) for name, path in paths.items()}


def attach_runtime_features(prob_frame: pd.DataFrame) -> pd.DataFrame:
    out = prob_frame.copy().sort_values("timestamp").reset_index(drop=True)
    probs = out.loc[:, ["p_short", "p_flat", "p_long"]].to_numpy(dtype="float64", copy=False)
    probs = np.clip(probs, 1e-12, 1.0)
    probs = probs / probs.sum(axis=1, keepdims=True)
    entropy = out.get("distribution_entropy")
    if entropy is None:
        entropy_values = -np.sum(probs * np.log(probs), axis=1) / np.log(3.0)
    else:
        entropy_values = pd.to_numeric(entropy, errors="coerce").fillna(0.0).to_numpy(dtype="float64")
    out["ngb_direction_bias"] = probs[:, 2] - probs[:, 0]
    out["ngb_nonflat_confidence"] = np.maximum(probs[:, 0], probs[:, 2])
    out["ngb_flat_probability"] = probs[:, 1]
    out["ngb_distribution_entropy"] = entropy_values
    return out
...
def runtime_logits(values: np.ndarray) -> np.ndarray:
    ...
    return np.column_stack(
        [
            (-2.0 * direction) + (0.85 * nonflat) - (0.25 * flat) - (0.35 * entropy),
            (-0.45 * nonflat) + (0.85 * flat) + (0.65 * entropy),
            (2.0 * direction) + (0.85 * nonflat) - (0.25 * flat) - (0.35 * entropy),
        ]
    )
...
def contribution(feature_name: str, value: float) -> np.ndarray:
    x = float(value)
    if feature_name == "ngb_direction_bias":
        return np.asarray([-2.0 * x, 0.0, 2.0 * x], dtype="float64")
    if feature_name == "ngb_nonflat_confidence":
        return np.asarray([0.85 * x, -0.45 * x, 0.85 * x], dtype="float64")
    if feature_name == "ngb_flat_probability":
        return np.asarray([-0.25 * x, 0.85 * x, -0.25 * x], dtype="float64")
    if feature_name == "ngb_distribution_entropy":
        return np.asarray([-0.35 * x, 0.65 * x, -0.35 * x], dtype="float64")
    raise ValueError(f"Unknown runtime feature: {feature_name}")


def export_distribution_score_table(reference_frame: pd.DataFrame, output_path: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "format": "ngboost_distribution_score_table_csv_v1",
        "feature_count": len(RUNTIME_FEATURE_ORDER),
        "feature_names": list(RUNTIME_FEATURE_ORDER),
        "runtime_policy": "direction_bias_nonflat_confidence_flat_probability_entropy_score_table_probe",
        "claim_boundary": "runtime_probe_not_native_ngboost_runtime_authority",
    }
...
def probability_frame(frame: pd.DataFrame, table_path: Path, threshold: float | None = None) -> pd.DataFrame:
    ...
    out["ngboost_runtime_threshold"] = threshold if threshold is not None else np.nan
    return out
...
def materialize_runtime_surfaces(source_summary: Mapping[str, Any]) -> tuple[dict[str, Any], list[dict[str, Any]], dict[str, Any], dict[str, pd.DataFrame]]:
    ...
    tier_a_table_path = model_root / "tier_a_ngboost_distribution_score_table.csv"
    tier_b_table_path = model_root / "tier_b_ngboost_distribution_score_table.csv"
    tier_a_table = export_distribution_score_table(tier_a_train_features, tier_a_table_path)
    tier_b_table = export_distribution_score_table(tier_b_train_features, tier_b_table_path)
    ...
    a_path = pred_root / "tier_a_ngboost_runtime_predictions.parquet"
    b_path = pred_root / "tier_b_ngboost_runtime_predictions.parquet"
    ab_path = pred_root / "tier_ab_ngboost_runtime_predictions.parquet"
    ...
    model_artifacts = {
        ...
        "source_ngboost_version": source_summary.get("ngboost_version"),
        ...
        "runtime_policy": "NGBoost probabilities are distilled to direction/confidence/flat/entropy score-table features.",
        "known_runtime_difference": "MT5 runtime_probe uses a distilled additive score table, not native NGBoost inference.",
    }
...
def export_feature_matrices(runtime_frames: Mapping[str, pd.DataFrame]) -> dict[str, Any]:
    ...
    for source_split, runtime_split in (("validation", "validation_is"), ("oos", "oos")):
        ...
        payload[f"tier_a_{runtime_split}"] = mt5.export_mt5_feature_matrix_csv(
            tier_a_frame,
            RUNTIME_FEATURE_ORDER,
            root / f"tier_a_{runtime_split}_ngboost_distribution_features.csv",
        ...
        payload[f"tier_b_fallback_{runtime_split}"] = mt5.export_mt5_feature_matrix_csv(
            tier_b_frame,
            RUNTIME_FEATURE_ORDER,
            root / f"tier_b_fallback_{runtime_split}_ngboost_distribution_features.csv",
            metadata_columns=("partial_context_subtype", "record_source"),
        )
    return payload
        ...
        attempts.append(
            attempt_payload(
                **common_kwargs,
                ...
                model_id=f"{RUN_ID}_tier_a_distribution_table",
                model_backend=MODEL_BACKEND,
        ...
        attempts.append(
            attempt_payload(
                **common_kwargs,
                ...
                model_id=f"{RUN_ID}_tier_b_distribution_table",
                model_backend=MODEL_BACKEND,
        ...
        attempts.append(
            attempt_payload(
                **common_kwargs,
                ...
                fallback_model_id=f"{RUN_ID}_tier_b_distribution_table",
...
def execute_or_block(prepared: Mapping[str, Any], args: argparse.Namespace) -> dict[str, Any]:
    ...
    for record in result.get("mt5_kpi_records", []):
        ...
        record["topic_read"] = "ngboost_distribution_distilled_score_table_runtime_handoff"
...
def build_summary(
    ...
    return {
        ...
        "topic_read": "ngboost_distribution_distilled_runtime_handoff",
...
def packet_markdown(summary: Mapping[str, Any], kpi: Mapping[str, Any]) -> str:
    ...
    return f"""# RUN20B NGBoost Distribution Runtime Probe Packet(20B 실행 NGBoost 분포 런타임 탐침 묶음)
    ...
    return [
        {
            ...
            "status": status,
            "research_path": rel(ROOT / "stage_pipelines/stage26/ngboost_probabilistic_distribution_scout.py"),
            "runtime_path": rel(ROOT / "foundation/mt5/ObsidianPrimeV2_RuntimeProbeEA.mq5"),
            "shared_contract": "4-feature NGBoost distribution score table, q0.80 non-flat thresholds, Tier A primary plus Tier B fallback routing",
...
def update_workspace_state(summary: Mapping[str, Any]) -> None:
    ...
    state = state.replace(f"      status: active_run20A_structural_scout_completed\n      current_run_id: {SOURCE_RUN_ID}", f"      status: {status}\n      current_run_id: {RUN_ID}", 1)
...
"""
    ...
    run_block = f"""stage26_ngboost_run20B_runtime_probe:
  packet_id: {PACKET_ID}
  status: {'reviewed_runtime_probe_completed' if completed else 'blocked_runtime_probe_after_attempt'}
  judgment: {summary.get('closure_judgment')}
  current_run_id: {RUN_ID}
...
def update_text_docs(summary: Mapping[str, Any], kpi: Mapping[str, Any]) -> None:
    ...
    plan = plan.replace(
        f"Current active milestone(현재 활성 마일스톤): Stage26(26단계) `{RUN_ID}` narrow MT5 runtime_probe(좁은 MT5 런타임 탐침).",
        f"Current active milestone(현재 활성 마일스톤): Stage26(26단계) `{next_action}`.",
        1,
    )
...
"""
    ...
    outcome = f"- `2026-05-05`: Stage26(26단계) `{RUN_ID}` MT5 runtime_probe(MT5 런타임 탐침)를 기록했다. judgment(판정): `{summary.get('closure_judgment')}`."
...
효과(effect, 효과): run20B(20B 실행)는 runtime_probe(런타임 탐침)일 뿐이며 baseline(기준선), promotion(승격), runtime authority(런타임 권위)를 만들지 않는다.
""",
    )
    ...
    update = f"""## Latest Stage26 RUN20B NGBoost Runtime Probe(최신 26단계 20B 실행 NGBoost 런타임 탐침)

Stage26(26단계) `{RUN_ID}`를 MT5 runtime_probe(MT5 런타임 탐침)로 실행했다.
...
"""
    ...
    write_md(
        DECISION_PATH,
        f"""# Decision(결정): Stage26 RUN20B NGBoost Runtime Probe(26단계 20B 실행 NGBoost 런타임 탐침)

Stage26(26단계) `{RUN_ID}`를 `{summary.get('closure_judgment')}`로 기록한다.

효과(effect, 효과): NGBoost(자연 그래디언트 부스팅)의 distilled distribution runtime handoff(증류된 분포 런타임 인계)는 관찰했지만, native NGBoost runtime authority(원본 NGBoost 런타임 권위)는 주장하지 않는다.
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run Stage26 NGBoost distribution MT5 runtime probe.")
