...
STAGE59_ID = "59A_adapter_repair__risk_sizing_quality_recalibration"
NEXT_REPAIR_STAGE_ID = "59B_adapter_repair__model_source_or_backup_branch"
RUN_NUMBER = "run54A"
RUN_ID = "run54A_stage59a_risk_sizing_quality_recalibration_v1"
PACKET_ID = "stage59a_risk_sizing_quality_recalibration_v1"
PARENT_RUN_ID = "run53A_stage59_post_risk_atr_repair_v1"
...
SOURCE_STAGE59_ROOT = Path("stages") / SOURCE_STAGE59_ID
SOURCE_STAGE59_DECISION = SOURCE_STAGE59_ROOT / "03_reviews/stage59_decision.md"
SOURCE_STAGE59_REPORT = SOURCE_STAGE59_ROOT / "03_reviews/adapter_repair_report.md"
SOURCE_STAGE59_SUMMARY = SOURCE_STAGE59_ROOT / "03_reviews/repaired_adapter_summary.csv"
SOURCE_STAGE59_SEGMENTS = SOURCE_STAGE59_ROOT / "03_reviews/repaired_segment_kpi_summary.csv"
SOURCE_STAGE59_RISK = SOURCE_STAGE59_ROOT / "03_reviews/repaired_risk_atr_telemetry.csv"

SIGNAL_COLUMN = "stage56_context_gap_refill_signal"
FEATURE_ORDER_HASH = ordered_hash((SIGNAL_COLUMN,))
COMMON_ROOT = f"Project_Obsidian_Prime_v2/stage59a/{RUN_NUMBER}_risk_sizing_quality_recalibration"

REPORT_PATH = REVIEWS_ROOT / "risk_sizing_quality_recalibration_report.md"
SUMMARY_JSON_PATH = REVIEWS_ROOT / "risk_sizing_quality_recalibration_summary.json"
SUMMARY_CSV_PATH = REVIEWS_ROOT / "risk_sizing_quality_recalibration_summary.csv"
SEGMENT_KPI_PATH = REVIEWS_ROOT / "risk_sizing_quality_segment_kpi_summary.csv"
EQUITY_AUDIT_PATH = REVIEWS_ROOT / "risk_sizing_quality_equity_curve_audit.md"
RISK_ATR_TELEMETRY_PATH = REVIEWS_ROOT / "risk_sizing_quality_telemetry.csv"
DECISION_PATH = REVIEWS_ROOT / "stage59a_decision.md"
AUDIT_CSV_PATH = REVIEWS_ROOT / "stage59a_trade_audit.csv"
...
STAGE59_PUSHED_COMMIT = "c6149ef513b1c6d5ad1b9f2d6287ea360a83ffda"

STAGE59_VARIANTS = (
    repair.RepairVariant(
        adapter_id="s59a_thr57_mr03_wideatr_sd5",
        label="threshold57_risk3pct_wide_atr_sd5",
        atr_enabled=True,
        ...
        long_threshold=0.57,
        notes="Stage59A bounded repair: threshold 0.57 quality filter with 3% model-controlled risk cap and wide ATR.",
    ),
    repair.RepairVariant(
        adapter_id="s59a_thr58_mr03_wideatr_sd5",
        label="threshold58_risk3pct_wide_atr_sd5",
        atr_enabled=True,
        model_risk_enabled=True,
        fixed_lot=0.25,
        atr_stop_multiplier=2.5,
        ...
        long_threshold=0.58,
        notes="Stage59A bounded repair: threshold 0.58 quality filter with 3% model-controlled risk cap and wide ATR.",
    ),
    repair.RepairVariant(
        adapter_id="s59a_thr57_mr03_atr_sd8",
        label="threshold57_risk3pct_atr_sd8",
        atr_enabled=True,
        model_risk_enabled=True,
        fixed_lot=0.25,
        ...
        short_threshold=0.57,
        long_threshold=0.57,
        notes="Stage59A bounded repair: threshold 0.57 quality filter with 3% risk cap and longer same-direction cooldown.",
    ),
)
...
def prepare_inputs(common_files_root: Path) -> dict[str, Any]:
    ...
    for split in ("validation_is", "oos"):
        token = "val" if split == "validation_is" else "oos"
        feature_local = RUN_ROOT / DEVELOPMENT_ANCHOR / "features" / f"{DEVELOPMENT_ANCHOR}_stage59a_adapter_a_{token}.csv"
    ...
    for variant_index, variant in enumerate(STAGE59_VARIANTS, start=1):
        ...
        for split in ("validation_is", "oos"):
            ...
            for role_index, (tier, attempt_role, prefix, attempt_token) in enumerate(
                ...
                magic = 5905400 + variant_index * 100 + (1 if split == "validation_is" else 50) + role_index
                attempts.append(
                    attempt_payload(
                        run_root=variant_root,
                        run_id=RUN_ID,
                        stage_number=59,
                        exploration_label="stage59A_BaselineAdapter__RiskSizingQualityRecalibration",
...
def load_existing_result() -> dict[str, Any]:
    ...
    if not path_exists(manifest) or not path_exists(kpi):
        raise FileNotFoundError("Stage59A existing run_manifest.json or kpi_record.json is missing")
            ...
            rows.append(
                {
                    ...
                    "stage_id": STAGE59_ID,
                    "run_id": RUN_ID,
                    "artifact_type": "stage59a_recalibration_evidence",
                    "path": rel(path),
                    "sha256": sha256_file_lf_normalized(path),
                    "hash_policy": "lf_normalized_text" if path.suffix.lower() in {".csv", ".json", ".md"} else "raw_file",
                    "created_at_utc": created,
                    "notes": "Stage59A bounded risk sizing quality recalibration artifact.",
                }
            )
            ...
            rows.append(
                {
                    ...
                    "created_at_utc": created,
                    "notes": "Actual Stage59A MT5 Strategy Tester HTML report.",
                }
            )
    return rows
...
def write_run_identity(result: Mapping[str, Any]) -> None:
    write_json(
        RUN_ROOT / "run_manifest.json",
        {
            ...
            "bounded_question": "Can 3% model-controlled risk calibration plus threshold quality filtering repair Stage59 cost-stressed weakness without changing the model source?",
            "source_stage59_decision": rel(SOURCE_STAGE59_DECISION),
            "source_stage59_pushed_commit": STAGE59_PUSHED_COMMIT,
            "variants": [variant.__dict__ for variant in STAGE59_VARIANTS],
...
def write_ledgers(
        ...
        [
            {
                "run_id": RUN_ID,
                "stage_id": STAGE59_ID,
                "lane": "baseline_adapter_risk_sizing_quality_recalibration",
                "status": status,
                "judgment": decision,
    ...
    ledger_rows.append(
        {
            "ledger_row_id": f"{RUN_ID}__aggregate_risk_sizing_quality_recalibration",
            "stage_id": STAGE59_ID,
            "run_id": RUN_ID,
            "subrun_id": "aggregate_risk_sizing_quality_recalibration",
            "parent_run_id": PARENT_RUN_ID,
            "record_view": "risk_sizing_quality_recalibration",
            "tier_scope": "Tier A+B",
            ...
            "external_verification_status": external,
            "notes": "Stage59A bounded risk sizing quality recalibration; not final package completion.",
        }
    )
...
def report_markdown(summary_rows: Sequence[Mapping[str, Any]], decision: str, reasons: Sequence[str], external: str) -> str:
    ...
    best = best_repaired_variant(summary_rows)
    return f"""# Stage59A Risk Sizing Quality Recalibration Report(59A단계 위험 크기 품질 재보정 보고서)

- stage(단계): `{STAGE59_ID}`
- run(실행): `{RUN_ID}`
- source_adapter(원천 어댑터): `{SOURCE_ADAPTER_ID}`
- source_stage59_commit(원천 59단계 커밋): `{STAGE59_PUSHED_COMMIT}`
...
def equity_audit_markdown(summary_rows: Sequence[Mapping[str, Any]], segment_rows: Sequence[Mapping[str, Any]]) -> str:
    ...
    return f"""# Stage59A Recalibrated Equity Curve Audit(59A단계 재보정 자금 곡선 감사)
...
def decision_markdown(decision: str, reasons: Sequence[str], best: Mapping[str, Any], external: str) -> str:
    return f"""# Stage59A Decision(59A단계 판정)
    ...
    write_json(
        SUMMARY_JSON_PATH,
        {
            ...
            "required_outputs": {
                ...
                "stage59a_decision": rel(DECISION_PATH),
            },
...
def write_packet_files(
    ...
    files = {
        ...
        "experiment_design_receipt.json": {
            "hypothesis": "3% model-controlled risk calibration plus threshold quality filtering may repair Stage59 cost-stressed weakness while preserving ATR bracket behavior",
            "control_variables": ["US100", "M5", "split_v1", "Tier B disabled", "source model unchanged", "ONNX deferred"],
            "stop_condition": "three bounded Stage59A variants only",
            "status": "completed",
        },
        ...
        "artifact_lineage_audit.json": {
            "source_inputs": [rel(SOURCE_STAGE59_DECISION), rel(SOURCE_STAGE59_REPORT), rel(SOURCE_MODEL)],
...
def write_stage_docs(decision: str) -> None:
    ...
    write_md(
        ...
        f"""# Stage59A Brief(59A단계 개요)

- stage_id(단계 ID): `{STAGE59_ID}`
- source_stage(원천 단계): `{SOURCE_STAGE59_ID}`
- source_decision(원천 판정): `continue_repair_in_new_bounded_stage`
- bounded_question(경계 질문): `risk_sizing_quality_recalibration`
- boundary(경계): `{BOUNDARY}`

Stage59A(59A단계)는 risk sizing calibration and quality filtering(위험 크기 보정과 품질 필터링)을 측정한다. Effect(효과): model source(모델 원천)를 바꾸기 전, Stage59(59단계)의 cost-stressed weakness(비용 압박 약점)를 좁힌다.
""",
    )
    write_md(
        INPUT_ROOT / "input_refs.md",
        f"""# Stage59A Input References(59A단계 입력 참조)

- stage59_decision(59단계 판정): `{rel(SOURCE_STAGE59_DECISION)}`
- stage59_report(59단계 보고서): `{rel(SOURCE_STAGE59_REPORT)}`
- stage59_summary(59단계 요약): `{rel(SOURCE_STAGE59_SUMMARY)}`
- stage59_segment_kpi(59단계 구간 KPI): `{rel(SOURCE_STAGE59_SEGMENTS)}`
- stage59_risk_telemetry(59단계 위험 텔레메트리): `{rel(SOURCE_STAGE59_RISK)}`
- stage59_pushed_commit(59단계 푸시 커밋): `{STAGE59_PUSHED_COMMIT}`
""",
    )
    write_md(
        SELECTED_ROOT / "selection_status.md",
        f"""# Stage59A Selection Status(59A단계 선택 상태)

- stage_status(단계 상태): `closed_bounded_risk_sizing_quality_recalibration`
- source_stage(원천 단계): `{SOURCE_STAGE59_ID}`
- source_decision(원천 판정): `continue_repair_in_new_bounded_stage`
- stage59a_decision(59A단계 판정): `{decision}`
- next_stage_or_branch(다음 단계/분기): `{next_stage}`
- selected_research_baseline(선택 연구 기준선): `none`
- claim_boundary(주장 경계): `{BOUNDARY}`

Effect(효과): Stage59A(59A단계)는 bounded recalibration result(경계 재보정 결과)를 보존하지만 final package(최종 패키지)나 operating claim(운영 주장)을 만들지 않는다.
""",
    )
...
Stage59B(59B단계)는 model source or backup branch(모델 원천 또는 예비 분기) 후속 수리 단계다. Effect(효과): Stage60 ONNX(60단계 ONNX)는 adapter quality(어댑터 품질)가 강해질 때까지 열지 않는다.
""",
        )
        write_md(
...
- stage59a_decision(59A단계 판정): `{rel(DECISION_PATH)}`
...
""",
        ...
        write_md(
            ...
            """# Stage59B Review Index(59B단계 검토 색인)

Stage59B(59B단계)는 아직 planned(계획) 상태다. Effect(효과): Stage59A(59A단계) 약점 기록을 다음 bounded repair(경계 수리)에 연결한다.
""",
        )
        write_md(
...
- stage_status(단계 상태): `active_planned_from_stage59a`
- source_stage(원천 단계): `{STAGE59_ID}`
- source_decision(원천 판정): `{decision}`
- selected_research_baseline(선택 연구 기준선): `none`
- claim_boundary(주장 경계): `{BOUNDARY}`

Effect(효과): Stage59B(59B단계)는 Stage59A(59A단계)의 남은 약점을 model source or backup branch(모델 원천 또는 예비 분기)로만 다룬다.
""",
        )
    write_md(
        REVIEWS_ROOT / "review_index.md",
        f"""# Stage59A Review Index(59A단계 검토 색인)
...
def update_current_truth(decision: str, summary_rows: Sequence[Mapping[str, Any]], external: str) -> None:
    best = best_repaired_variant(summary_rows)
    next_stage = next_stage_for_decision(decision)
    next_packet = "stage59b_model_source_or_backup_branch_v1" if decision == "continue_repair_in_new_bounded_stage" else "stage60_onnx_hardening_v1"
    next_run = "run55A_stage59b_model_source_or_backup_branch_v1" if decision == "continue_repair_in_new_bounded_stage" else "run55A_stage60_onnx_hardening_v1"
    write_md(
...
- status(상태): `stage59a_closed_{decision}`
...
""",
    ...
    focus = (
        "current_focus:\n"
        f"- >-\n"
        f"  Stage59A(59A단계) `{STAGE59_ID}` closed(종료) as bounded risk sizing quality recalibration(경계 위험 크기 품질 재보정); decision(판정)=`{decision}`. "
        f"Effect(효과): full adapter repair(전체 어댑터 수리)는 계속 진행 중이며 final(최종) 또는 operating(운영) 주장은 없다.\n"
        f"- >-\n"
        f"  Next stage_or_branch(다음 단계/분기) `{next_stage}` is active/planned(활성/계획). Effect(효과): Stage59A(59A단계) 결과를 다음 bounded step(경계 다음 단계)으로 넘긴다.\n"
    )
    text = re.sub(
...
stage59a_risk_sizing_quality_recalibration:
  packet_id: {PACKET_ID}
  stage_id: {STAGE59_ID}
  status: closed_bounded_risk_sizing_quality_recalibration
  ...
  boundary: {BOUNDARY}
"""
    if "stage59a_risk_sizing_quality_recalibration:" in text:
        text = re.sub(r"\nstage59a_risk_sizing_quality_recalibration:\n(?:  .*\n)*", block, text, count=1)
    else:
        text += block
...
def append_changelog(decision: str) -> None:
    entry = (
        ...
        f"- effect(효과): Stage59(59단계) cost-stressed weakness(비용 압박 약점)를 threshold quality filter(문턱값 품질 필터)와 3% risk calibration(3% 위험 보정)으로 측정하고 다음 단계/분기 조건을 남겼다.\n"
    )
...
def parse_args(argv: Sequence[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run Stage59A bounded risk sizing quality recalibration.")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.finalize_existing:
        ...
        prepared = {
            ...
            "model_family": "baseline_adapter_stage59a_risk_sizing_quality_ebm_table",
            "feature_set_id": "stage59a_context_gap_refill_signal_risk_sizing_quality",
