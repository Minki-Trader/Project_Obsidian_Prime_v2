...
from stage_pipelines.stage337 import (  # noqa: E402
    review_runtime_positive_low_pf_drawdown_side_balance_repair_training_without_db as ii,
)
...
TODAY = "2026-06-01"
STAGE_ID = ii.STAGE_ID
STAGE_DIR = ii.STAGE_DIR
RUN_NUMBER = "run337IJ"
RUN_ID = "run337IJ_materialize_runtime_positive_low_pf_drawdown_side_balance_repair_runtime_probe_package_without_db_v1"
PARENT_RUN_ID = ii.RUN_ID
NEXT_RUN_ID = "run337IK_execute_runtime_positive_low_pf_drawdown_side_balance_repair_mt5_runtime_probe_without_db_v1"
STATUS = "completed_stage337IJ_runtime_positive_repair_runtime_probe_package_materialized_no_mt5_execution"
JUDGMENT = "runtime_probe_package_ready_for_weak_proxy_positive_candidate_proxy_mt5_diff_required_no_selection"
DECISION = "stage337IJ_open_run337IK_execute_runtime_positive_repair_mt5_runtime_probe"
CLAIM_BOUNDARY = (
    "research_development_runtime_probe_package_only_no_mt5_execution_in_IJ_no_candidate_selection_"
...
REPORT_PATH = REVIEW_DIR / "run337IJ_repair_runtime_probe_package.md"
DECISION_DOC = ROOT / "docs" / "decisions" / f"{TODAY}_stage337IJ_runtime_positive_repair_runtime_probe_package.md"
...
COMMON_ROOT = f"Project_Obsidian_Prime_v2/stage337/{RUN_NUMBER}_runtime_positive_repair_probe"
...
FEATURE_MATRIX = FEATURE_DIR / "runtime_positive_repair_inner_holdout_features.csv"
FEATURE_MATRIX_MANIFEST = RUN_DIR / "runtime_feature_matrix_manifest.csv"
EXPECTED_PROBABILITY_TAPE = EXPECTED_DIR / "runtime_positive_repair_expected_probability_tape.csv"
...
EXECUTION_QUEUE = RUN_DIR / "run337IK_execution_queue.csv"
...
INPUT_FILES = (
    ...
    ii.GATE_AUDIT,
    ii.POSITIVE_MATRIX,
    ii.ih.TRAINED_MODEL_MANIFEST,
    ii.ih.FEATURE_SCHEMA,
    ii.ih.ONNX_PARITY,
    ii.ih.ig.ifr.IF_INPUT_FRAME,
)
OUTPUT_FILES = (
    FEATURE_MATRIX,
...
def load_features() -> list[str]:
    schema = read_json(ii.ih.FEATURE_SCHEMA)
...
def attempt_name(index: int, model_id: str) -> str:
    return f"ij{index:02d}_{model_id}"


def inner_holdout_unique_frame(features: Sequence[str]) -> pd.DataFrame:
    positive = read_csv(ii.POSITIVE_MATRIX)
    if positive.empty:
        raise RuntimeError("no positive proxy candidate available for IJ package")
    target = str(positive.iloc[0]["target_column"])
    valid_col = "hx_valid_fwd18" if target.endswith("fwd18") else "hx_valid_fwd6"
    frame = pd.read_parquet(io(ii.ih.ig.ifr.IF_INPUT_FRAME)).copy()
    ...
    _inner_train, inner_holdout = ii.ih.split_inner(task_frame)
    cost_order = {
...
def materialize_feature_matrix(frame: pd.DataFrame, features: Sequence[str]) -> tuple[pd.DataFrame, list[str], list[dict[str, Any]]]:
    ...
    feature_common = f"{COMMON_FEATURE_DIR}/runtime_positive_repair_inner_holdout_features.csv"
    sync_rows = [
        copy_file(
            FEATURE_MATRIX,
...
def materialize_models_and_expected(
    ...
    manifest = read_csv(ii.ih.TRAINED_MODEL_MANIFEST)
    model_lookup = {str(row["model_id"]): row for _, row in manifest.iterrows()}
    positive = read_csv(ii.POSITIVE_MATRIX).head(1)
    ...
    for index, (_, candidate) in enumerate(positive.iterrows(), start=1):
        ...
        sync_rows.append(
            copy_file(
                ...
                "joblib model(joblib 모델)을 package(패키지)에 복사해 expected tape(예상 테이프)를 재현한다.",
            )
        )
        sync_rows.append(
            copy_file(
                ...
                "ONNX model(온엑스 모델)을 package(패키지)에 복사해 산출물 hash(해시)를 고정한다.",
            )
        )
        sync_rows.append(
            copy_file(
        ...
        set_values = {
            ...
            "InpExplorationLabel": "stage337IJ_RuntimePositiveRepair__MT5RuntimeProbe",
            "InpTierLabel": "Tier A",
        ...
        attempts.append(
            {
                ...
                "known_proxy_runtime_difference": (
                    "proxy score is signal sanity; MT5 uses broker lifecycle execution"
                    "(프록시 점수는 신호 점검이고 MT5는 브로커 생명주기 실행)"
                ),
...
def contracts_and_queue() -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
        ...
        [
            {
                ...
                "effect": "IK can compare runtime parity row by row(IK가 행 단위 런타임 동등성을 비교할 수 있다).",
                "claim_boundary": CLAIM_BOUNDARY,
            }
        ]
    )
    queue = pd.DataFrame(
        [
            {
                "queue_id": "ik_execute_runtime_positive_repair_mt5_runtime_probe",
                "next_run_id": NEXT_RUN_ID,
                "priority": "P0",
                "task": "execute MT5 runtime probe for weak proxy-positive ONNX candidate(약한 프록시 양성 ONNX 후보 MT5 런타임 탐침 실행)",
...
def materialize_package() -> tuple[dict[str, Any], list[Path]]:
    ...
    feature_manifest = pd.DataFrame(
        [
            {
                "feature_matrix_id": "runtime_positive_repair_inner_holdout_features",
                "feature_set_id": ii.ih.FEATURE_SET_ID,
                "feature_count": len(features),
                "row_count": len(frame),
...
def make_gates(summary: Mapping[str, Any]) -> pd.DataFrame:
    parent_gates = read_csv(ii.GATE_AUDIT)
    ...
    gates = [
        gate_row(
            "parent_ii_gates_passed",
            "passed" if parent_passed else "failed",
            rel(ii.GATE_AUDIT),
            "II review(검토)가 통과된 후보만 package(패키지)화한다.",
        ),
        gate_row(
            "weak_positive_candidate_loaded",
            "passed" if summary["attempt_rows"] >= 1 else "failed",
            rel(ii.POSITIVE_MATRIX),
            "weak proxy-positive(약한 프록시 양성) 후보를 불러왔다.",
        ),
        gate_row(
            "feature_matrix_materialized",
            "passed" if exists(FEATURE_MATRIX) and summary["feature_matrix_rows"] > 0 else "failed",
            rel(FEATURE_MATRIX),
            "MT5(메타트레이더5)가 읽을 feature matrix(피처 행렬)를 만들었다.",
        ),
        gate_row(
            "expected_probability_tape_materialized",
            "passed" if exists(EXPECTED_PROBABILITY_TAPE) and expected_rows_ok else "failed",
            rel(EXPECTED_PROBABILITY_TAPE),
            "proxy expected value(프록시 예상값)를 MT5 diff(차이) 기준선으로 만들었다.",
        ),
        gate_row(
            "model_and_common_files_synced",
            "passed" if sync_ready else "failed",
            rel(COMMON_FILES_SYNC),
            "ONNX(온엑스)와 feature matrix(피처 행렬)를 Common Files(공용 파일)에 복사했다.",
        ),
        gate_row(
            "tester_set_ini_materialized",
            "passed"
            if summary["tester_set_rows"] == summary["attempt_rows"] and summary["tester_ini_rows"] == summary["attempt_rows"]
            else "failed",
            f"{rel(TESTER_SET_MANIFEST)};{rel(TESTER_INI_MANIFEST)}",
            "MT5 Strategy Tester(전략 테스터) 실행 파일을 만들었다.",
        ),
        gate_row(
            "runtime_attempt_package_written",
            "passed" if exists(RUNTIME_PROBE_ATTEMPT_PACKAGE) and summary["attempt_rows"] >= 1 else "failed",
            rel(RUNTIME_PROBE_ATTEMPT_PACKAGE),
            "IK 실행이 읽을 attempt package(시도 패키지)를 만들었다.",
        ),
        gate_row(
            "proxy_mt5_contracts_written",
            "passed" if exists(PROXY_MT5_COMPARISON_CONTRACT) and exists(RUNTIME_PARITY_CONTRACT) else "failed",
            f"{rel(PROXY_MT5_COMPARISON_CONTRACT)};{rel(RUNTIME_PARITY_CONTRACT)}",
            "proxy-MT5 comparison(프록시-MT5 비교) 조건을 고정했다.",
        ),
        gate_row(
            "execution_queue_opened",
            "passed" if exists(EXECUTION_QUEUE) else "failed",
            rel(EXECUTION_QUEUE),
            "다음 IK runtime probe(런타임 탐침) 실행으로 넘긴다.",
        ),
        gate_row(
            "no_mt5_execution_in_ij",
            "passed",
            rel(CLAIM_RECEIPT),
            "IJ는 package(패키지)만 만들고 MT5 실행은 하지 않는다.",
        ),
        gate_row(
        ...
        (
            ROUTING_RECEIPT,
            {
                **base,
                ...
                "effect": "runtime probe package(런타임 탐침 패키지)를 IK execution(실행)으로 넘긴다.",
            },
        ),
        (
            DATA_RECEIPT,
            {
                **base,
                ...
                "timestamp_semantics": "bar close timestamp(봉 마감 시각)",
                "effect": "MT5(메타트레이더5) 입력 시각을 고정한다.",
            },
        ),
        (
            MODEL_RECEIPT,
        ...
        (
            RUNTIME_RECEIPT,
            {
                **base,
                ...
                "runtime_probe_execution": "not_run_in_IJ",
                "portable_terminal_exists": summary["portable_terminal_exists"],
                "portable_ea_ex5_exists": summary["portable_ea_ex5_exists"],
                "effect": "runtime parity(런타임 동등성) 비교에 필요한 파일 위치를 고정한다.",
            },
        ),
        (
            FORENSICS_RECEIPT,
            {
                **base,
                ...
                "tester_identity_contract": rel(TESTER_IDENTITY_CONTRACT),
                "effect": "MT5 backtest forensics(백테스트 포렌식) 확인 기준을 만든다.",
            },
        ),
        (
            PERFORMANCE_RECEIPT,
            {
                **base,
                ...
                "allowed_use": "proxy-vs-MT5 comparison baseline(프록시-MT5 비교 기준선)",
                "forbidden_use": "MT5 KPI replacement or selection(MT5 핵심 성과 지표 대체 또는 선택)",
            },
        ),
        (
            JUDGMENT_RECEIPT,
        ...
        (
            LINEAGE_RECEIPT,
            {
                **base,
                ...
                "effect": "II review(검토)와 IK MT5 runtime probe(런타임 탐침)를 artifact lineage(산출물 계보)로 연결한다.",
            },
        ),
    ]
...
def write_final(summary: Mapping[str, Any], gates: pd.DataFrame) -> dict[str, Any]:
    final = {
        ...
        "next_action": NEXT_RUN_ID,
        "candidate_selection": "not_run",
        "mt5_runtime_probe": "not_run_in_IJ",
        "forward_passed": "not_claimed",
        "forward_failed": "not_claimed",
        "runtime_authority": "not_claimed",
        "operating_promotion": "not_claimed",
...
def write_docs(final: Mapping[str, Any]) -> list[Path]:
    report = f"""# run337IJ Runtime Probe Package(run337IJ 런타임 탐침 패키지)
...
II의 weak proxy-positive(약한 프록시 양성) ONNX(온엑스) 후보 1개를 MT5 runtime probe package(런타임 탐침 패키지)로 만들었다.
Effect(효과): feature matrix(피처 행렬), expected tape(예상 테이프), ONNX(온엑스), set/ini(설정 파일)가 같은 lineage(계보) 안에서 비교 가능해졌다.

## Boundary(경계)

IJ는 package(패키지)만 만들었다. MT5 execution(MT5 실행), candidate selection(후보 선택), runtime authority(런타임 권위), Goal Achieve(목표 달성)는 없다.

## Next(다음)

`{NEXT_RUN_ID}`에서 MT5 runtime probe(런타임 탐침)를 실행한다.
Effect(효과): proxy expected value(프록시 예상값)와 MT5 runtime output(런타임 출력)의 diff(차이)를 확인한다.
"""
    decision = f"""# {TODAY} Stage337IJ Decision(337IJ 결정)
...
Action(행동): weak proxy-positive(약한 프록시 양성) 후보를 MT5 runtime probe(런타임 탐침) 패키지로 물질화했다.
Effect(효과): 다음 실행은 model logic(모델 로직), threshold(임계값), lot(랏)을 바꾸지 않고 외부 검증만 수행한다.
...
IJ package(패키지)는 proxy(프록시)를 MT5 runtime probe(런타임 탐침)와 비교할 준비를 끝냈다.
효과는 다음 IK에서 실제 terminal/tester(터미널/테스터) 출력으로 diff(차이)를 볼 수 있게 하는 것이다.
...
- runtime_package(런타임 패키지): `materialized_for_probe_only(탐침 전용 물질화)`
...
Effect(효과): package priority(패키지 우선순위)를 selection(선택)으로 오해하지 않게 한다.
"""
...
"""
    ...
    marker = f"run337IJ {RUN_ID}"
    append_text_once(
        STAGE_BRIEF,
        marker,
        f"""## run337IJ Runtime Probe Package(런타임 탐침 패키지)

- run_id(실행 ID): `{RUN_ID}`
- judgment(판정): `{JUDGMENT}`
- next(다음): `{NEXT_RUN_ID}`
- effect(효과): ONNX(온엑스), feature matrix(피처 행렬), expected tape(예상 테이프), tester set/ini(테스터 설정)를 MT5 runtime probe(런타임 탐침)용으로 고정했다.
""",
    )
    changelog_entry = f"""## {TODAY} run337IJ Runtime Probe Package(런타임 탐침 패키지)

- action(행동): weak proxy-positive(약한 프록시 양성) 후보 1개를 MT5 runtime probe(런타임 탐침) 패키지로 만들었다.
- effect(효과): expected tape(예상 테이프) `{final['expected_probability_rows']}`행과 attempt package(시도 패키지) `{final['attempt_rows']}`행을 만들었다.
    ...
    ledger_rows = [
        {
            **base_row,
            ...
            "tier": "Tier A",
            "metric_scope": "runtime_probe_package_only",
            "candidate_model_id": final["candidate_model_ids"],
            ...
            "result_status": "runtime_probe_package_materialized_no_mt5_execution",
        },
        {
            **base_row,
