...
RUN_NUMBER = "run337GP"
RUN_ID = "run337GP_execute_runtime_positive_side_stability_gb_pf_recovery_drawdown_repair_mt5_runtime_probe_without_db_v1"
PARENT_RUN_ID = go.RUN_ID
NEXT_RUN_ID = "run337GQ_review_runtime_positive_side_stability_gb_pf_recovery_drawdown_repair_mt5_runtime_probe_or_repair_without_db_v1"
STATUS_COMPLETED = "completed_stage337GP_runtime_positive_side_stability_gb_pf_recovery_drawdown_repair_mt5_runtime_probe_executed_review_required_no_forward_decision"
STATUS_BLOCKED = "blocked_stage337GP_runtime_positive_side_stability_gb_pf_recovery_drawdown_repair_mt5_runtime_probe_attempt_missing_or_failed_outputs_no_forward_decision"
...
DECISION_COMPLETED = "stage337GP_open_run337GQ_review_runtime_positive_side_stability_gb_pf_recovery_drawdown_repair_mt5_runtime_probe"
DECISION_BLOCKED = "stage337GP_open_run337GQ_review_or_repair_mt5_runtime_probe_attempt"
CLAIM_BOUNDARY = (
    "research_development_only_stage337GP_runtime_positive_side_stability_gb_pf_recovery_drawdown_repair_mt5_runtime_probe_without_db_"
...
REPORT_PATH = REVIEWS_DIR / "run337GP_runtime_positive_side_stability_gb_pf_recovery_drawdown_repair_mt5_runtime_probe.md"
DECISION_DOC = ROOT / "docs" / "decisions" / f"{TODAY}_stage337GP_runtime_positive_side_stability_gb_pf_recovery_drawdown_repair_mt5_runtime_probe.md"
...
INPUT_FILES = (
    ...
    go.GATE_AUDIT,
    go.EXECUTION_QUEUE,
    go.RUNTIME_PROBE_ATTEMPT_PACKAGE,
    go.EXPECTED_PROBABILITY_TAPE,
    go.COMMON_FILES_SYNC,
    go.TESTER_SET_MANIFEST,
    go.TESTER_INI_MANIFEST,
    go.MODEL_HANDOFF_MANIFEST,
    go.FEATURE_MATRIX_MANIFEST,
    fa.PORTABLE_EA_EX5,
)
OUTPUT_FILES = (
    ATTEMPT_PACKAGE,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Stage337GP runtime positive side stability MT5 runtime probe.")
...
def rel(path: Path | str) -> str:
    return go.aw.rel(path)


def runtime_identity(attempt_rows: int) -> list[dict[str, Any]]:
    return [
        {
            "identity_id": "stage337GP_runtime_identity",
            ...
            "portable_ea_ex5_sha256": go.aw.sha256_file(fa.PORTABLE_EA_EX5) if path_exists(fa.PORTABLE_EA_EX5) else "",
...
def make_final(summary: Mapping[str, Any]) -> dict[str, Any]:
    go_final = fb.read_json(go.FINAL_DECISION)
    ...
    return {
        ...
        "go_next_action": go_final.get("next_action", ""),
        "go_failed_gate_rows": sum(1 for row in fb.read_csv(go.GATE_AUDIT) if row.get("status") != "passed"),
...
def build_gates(final: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    checks = [
        ("input_presence", final["missing_inputs"] == 0, str(final["missing_inputs"]), "0", rel(go.RUNTIME_PROBE_ATTEMPT_PACKAGE), "required GO package inputs exist(필수 GO 패키지 입력 존재)"),
        ("parent_go_gates_passed", final["go_failed_gate_rows"] == 0, str(final["go_failed_gate_rows"]), "0", rel(go.GATE_AUDIT), "GO gates passed(GO 게이트 통과)"),
        ("parent_next_action_matches", final["go_next_action"] == RUN_ID, str(final["go_next_action"]), RUN_ID, rel(go.FINAL_DECISION), "GP follows GO next action(GP가 GO 다음 행동을 따름)"),
...
def build_receipts(final: Mapping[str, Any], artifact_paths: Sequence[Path]) -> list[Path]:
    runtime = {
        ...
        "shared_contract": "GO feature matrix, expected tape, set/ini, ONNX handoff(GO 피처 행렬, 예상 테이프, 설정, ONNX 인계)",
    ...
    lineage = {
        ...
        "artifact_hashes": {
            rel(path): go.aw.sha256_file(path)
            for path in all_artifacts
            if path_exists(path) and go.aw.io_path(path).is_file()
        },
        "registry_links": [rel(go.RUN_REGISTRY), rel(go.ALPHA_LEDGER), rel(go.STAGE_LEDGER), rel(go.ARTIFACT_REGISTRY)],
        "lineage_judgment": "connected_GO_package_to_GP_runtime_probe(GO 패키지를 GP 런타임 탐침에 연결)",
        "claim_boundary": CLAIM_BOUNDARY,
    }
...
def write_report(final: Mapping[str, Any]) -> Path:
    text = f"""# Stage337 run337GP MT5 Runtime Probe(337단계 337GP MT5 런타임 탐침)

## Conclusion(결론)

run337GP(337GP 실행)는 run337GO(337GO 실행)의 runtime positive side stability GB/PF/recovery/drawdown repair package(런타임 긍정 방향 안정 수리 패키지)를 MT5 terminal(MT5 터미널)에 시도했다.

Action(행동): Strategy Tester(전략 테스터)를 attempt(시도)별로 실행하거나 blocker(차단 사유)를 기록했다. Effect(효과): proxy expected value(프록시 예상값)와 MT5 runtime output(MT5 런타임 출력)을 다음 GQ review(GQ 검토)에서 비교할 수 있다.
...
"""
    return go.aw.write_text_lossless(REPORT_PATH, text, True)


def write_decision(final: Mapping[str, Any]) -> Path:
    text = f"""# {TODAY} Stage337GP Decision(337GP 결정)
...
Effect(효과): 다음 GQ review(GQ 검토)가 proxy-vs-MT5 diff(프록시-MT5 차이), attribution(귀속), usability(활용 가능성)을 판정할 수 있다.

Forward/Goal(전진/목표): `not_claimed`
runtime_authority(런타임 권위): `not_claimed`
claim_boundary(주장 경계): `{CLAIM_BOUNDARY}`
"""
    return go.aw.write_text_lossless(DECISION_DOC, text, True)


def update_docs(final: Mapping[str, Any]) -> list[Path]:
    artifacts: list[Path] = []
    branch = fa.ey.current_branch()
    workspace, workspace_bom = go.aw.read_text_lossless(go.WORKSPACE_STATE)
    ...
    focus = (
        "- >-\n"
        f"  Stage337 run337GP focus complete: run337GP(337GP 실행)는 `{final['status']}`로 MT5 runtime probe(MT5 런타임 탐침)를 시도했다. "
    ...
    if "Stage337 run337GP focus complete" in workspace:
        workspace = re.sub(r"- >-\n  Stage337 run337GP focus complete:.*?(?=\n- >-|\n[a-zA-Z_]+:|$)", focus.rstrip(), workspace, count=1, flags=re.S)
    else:
        workspace = workspace.replace("current_focus:\n", "current_focus:\n" + focus, 1)
    artifacts.append(go.aw.write_text_lossless(go.WORKSPACE_STATE, workspace, workspace_bom))

    current, current_bom = go.aw.read_text_lossless(go.CURRENT_STATE)
    ...
    section = f"""## run337GP MT5 Runtime Probe(MT5 런타임 탐침)
...
- report_rows(보고서 행): `{final['report_rows']}`
- gates(게이트): `{final['passed_gates']}/{final['gate_rows']}`
- effect(효과): MT5 external check(MT5 외부 확인)를 실제로 시도하고 GQ review(GQ 검토)로 넘긴다. 운영 주장은 닫는다.
- next_action(다음 행동): `{final['next_action']}`
"""
    current = fb.upsert_section_before(current, "## run337GO Runtime Probe Package", section, "run337GP MT5 Runtime Probe")
    artifacts.append(go.aw.write_text_lossless(go.CURRENT_STATE, current, current_bom))
...
- next_action(다음 행동): `{final['next_action']}`
- effect(효과): GP(337GP 실행)는 runtime probe(런타임 탐침) 근거만 만들며 operating selection(운영 선택)은 하지 않는다.
"""
    artifacts.append(go.aw.write_text_lossless(go.SELECTED_STATUS, selection, True))

    brief, brief_bom = go.aw.read_text_lossless(go.STAGE_BRIEF)
    brief_entry = (
        f"- {TODAY}: run337GP(337GP 실행) `{final['status']}`. "
    ...
    artifacts.append(go.aw.write_text_lossless(go.STAGE_BRIEF, fb.upsert_single_line(brief, "run337GP(337GP 실행)", brief_entry), brief_bom))

    changelog, changelog_bom = go.aw.read_text_lossless(go.CHANGELOG)
    changelog_entry = (
        f"- {TODAY}: Stage337 run337GP(337GP 실행) `{final['status']}`. "
        f"Effect(효과): MT5 runtime probe(MT5 런타임 탐침)를 시도하고 `{final['next_action']}`을 열었다. Forward/Goal(전진/목표)은 주장하지 않았다."
    )
    artifacts.append(go.aw.write_text_lossless(go.CHANGELOG, fb.upsert_single_line(changelog, "Stage337 run337GP", changelog_entry), changelog_bom))
    return artifacts
...
def update_registers(final: Mapping[str, Any]) -> list[Path]:
    run_row = {
        "run_id": RUN_ID,
        "stage_id": STAGE_ID,
        "lane": "runtime_positive_side_stability_gb_pf_recovery_drawdown_repair_mt5_runtime_probe",
        "status": final["status"],
    ...
    alpha_row = {
        ...
        "parent_run_id": PARENT_RUN_ID,
        "record_view": "runtime_positive_side_stability_gb_pf_recovery_drawdown_repair_mt5_runtime_probe(런타임 긍정 방향 안정 수리 MT5 런타임 탐침)",
    ...
    stage_row = {
        ...
        "family": "runtime_positive_side_stability_gb_pf_recovery_drawdown_repair_mt5_runtime_probe",
        "question": "do GO side-stability GB/PF/recovery/drawdown ONNX runtime packages execute in MT5 and match expected probabilities",
    ...
    return [
        fb.upsert_csv_worktree(go.RUN_REGISTRY, go.aw.RUN_REGISTRY_COLUMNS, run_row, "run_id"),
        fb.upsert_csv_worktree(go.ALPHA_LEDGER, go.aw.ALPHA_LEDGER_COLUMNS, alpha_row, "ledger_row_id"),
        fb.upsert_csv_worktree(go.STAGE_LEDGER, go.aw.STAGE_LEDGER_COLUMNS, stage_row, "ledger_row_id"),
    ]
...
def configure_execution_engine() -> None:
    replacements = {
        ...
        "SELECTED_STATUS": go.SELECTED_STATUS,
        "STAGE_BRIEF": go.STAGE_BRIEF,
        "WORKSPACE_STATE": go.WORKSPACE_STATE,
        "CURRENT_STATE": go.CURRENT_STATE,
        "CHANGELOG": go.CHANGELOG,
        "RUN_REGISTRY": go.RUN_REGISTRY,
        "ALPHA_LEDGER": go.ALPHA_LEDGER,
        "ARTIFACT_REGISTRY": go.ARTIFACT_REGISTRY,
        "STAGE_LEDGER": go.STAGE_LEDGER,
        "FA_FINAL": go.FINAL_DECISION,
        "FA_GATES": go.GATE_AUDIT,
        "FA_QUEUE": go.EXECUTION_QUEUE,
        "FA_ATTEMPT_PACKAGE": go.RUNTIME_PROBE_ATTEMPT_PACKAGE,
        "FA_EXPECTED_TAPE": go.EXPECTED_PROBABILITY_TAPE,
        "FA_COMMON_SYNC": go.COMMON_FILES_SYNC,
        "FA_TESTER_SET": go.TESTER_SET_MANIFEST,
        "FA_TESTER_INI": go.TESTER_INI_MANIFEST,
        "FA_MODEL_HANDOFF": go.MODEL_HANDOFF_MANIFEST,
        "FA_FEATURE_MANIFEST": go.FEATURE_MATRIX_MANIFEST,
