...
from stage_pipelines.stage337 import materialize_mt5_negative_repair_lgbm_runtime_probe_package_without_db as gw  # noqa: E402


TODAY = "2026-05-31"
STAGE_ID = gw.STAGE_ID
RUN_NUMBER = "run337GX"
RUN_ID = "run337GX_execute_runtime_positive_side_stability_gb_pf_recovery_drawdown_mt5_negative_repair_mt5_runtime_probe_without_db_v1"
PARENT_RUN_ID = gw.RUN_ID
NEXT_RUN_ID = "run337GY_review_runtime_positive_side_stability_gb_pf_recovery_drawdown_mt5_negative_repair_mt5_runtime_probe_or_repair_without_db_v1"
STATUS_COMPLETED = "completed_stage337GX_mt5_negative_repair_lightgbm_mt5_runtime_probe_executed_review_required_no_forward_decision"
STATUS_BLOCKED = "blocked_stage337GX_mt5_negative_repair_lightgbm_mt5_runtime_probe_attempt_missing_or_failed_outputs_no_forward_decision"
...
DECISION_COMPLETED = "stage337GX_open_run337GY_review_mt5_negative_repair_lightgbm_mt5_runtime_probe"
DECISION_BLOCKED = "stage337GX_open_run337GY_review_or_repair_mt5_negative_repair_lightgbm_mt5_runtime_probe_attempt"
CLAIM_BOUNDARY = (
    "research_development_only_stage337GX_mt5_negative_repair_lightgbm_mt5_runtime_probe_without_db_"
    "no_new_training_no_threshold_tuning_no_lot_optimization_no_operating_selection_"
    "no_forward_passed_no_forward_failed_no_live_readiness_no_deployment_no_operating_promotion_"
    "no_runtime_authority_no_goal_achieve"
)

STAGE_DIR = gw.STAGE_DIR
RUN_DIR = STAGE_DIR / "02_runs" / RUN_NUMBER
MT5_DIR = RUN_DIR / "mt5"
TELEMETRY_COPY_DIR = RUN_DIR / "runtime_telemetry"
REPORT_COPY_DIR = MT5_DIR / "reports"
REVIEWS_DIR = gw.REVIEWS_DIR
REPORT_PATH = REVIEWS_DIR / "run337GX_mt5_negative_repair_lightgbm_mt5_runtime_probe.md"
DECISION_DOC = ROOT / "docs" / "decisions" / f"{TODAY}_stage337GX_mt5_negative_repair_lightgbm_mt5_runtime_probe.md"
...
INPUT_FILES = (
    ...
    gw.GATE_AUDIT,
    gw.EXECUTION_QUEUE,
    gw.RUNTIME_PROBE_ATTEMPT_PACKAGE,
    gw.EXPECTED_PROBABILITY_TAPE,
    gw.COMMON_FILES_SYNC,
    gw.TESTER_SET_MANIFEST,
    gw.TESTER_INI_MANIFEST,
    gw.MODEL_HANDOFF_MANIFEST,
    gw.FEATURE_MATRIX_MANIFEST,
    fa.PORTABLE_EA_EX5,
)
OUTPUT_FILES = (
    ATTEMPT_PACKAGE,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Stage337GX runtime positive side stability MT5 runtime probe.")
...
def rel(path: Path | str) -> str:
    return gw.aw.rel(path)


def runtime_identity(attempt_rows: int) -> list[dict[str, Any]]:
    return [
        {
            "identity_id": "stage337GX_runtime_identity",
            ...
            "portable_ea_ex5_sha256": gw.aw.sha256_file(fa.PORTABLE_EA_EX5) if path_exists(fa.PORTABLE_EA_EX5) else "",
...
def make_final(summary: Mapping[str, Any]) -> dict[str, Any]:
    gw_final = fb.read_json(gw.FINAL_DECISION)
    ...
    return {
        ...
        "gw_next_action": gw_final.get("next_action", ""),
        "gw_failed_gate_rows": sum(1 for row in fb.read_csv(gw.GATE_AUDIT) if row.get("status") != "passed"),
...
def build_gates(final: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    checks = [
        ("input_presence", final["missing_inputs"] == 0, str(final["missing_inputs"]), "0", rel(gw.RUNTIME_PROBE_ATTEMPT_PACKAGE), "required GW package inputs exist(필수 GW 패키지 입력 존재)"),
        ("parent_gw_gates_passed", final["gw_failed_gate_rows"] == 0, str(final["gw_failed_gate_rows"]), "0", rel(gw.GATE_AUDIT), "GW gates passed(GW 게이트 통과)"),
        ("parent_next_action_matches", final["gw_next_action"] == RUN_ID, str(final["gw_next_action"]), RUN_ID, rel(gw.FINAL_DECISION), "GX follows GW next action(GX가 GW 다음 행동을 따름)"),
...
def build_receipts(final: Mapping[str, Any], artifact_paths: Sequence[Path]) -> list[Path]:
    runtime = {
        ...
        "shared_contract": "GW feature matrix, expected tape, set/ini, ONNX handoff(GW 피처 행렬, 예상 테이프, 설정, ONNX 인계)",
    ...
    lineage = {
        ...
        "artifact_hashes": {
            rel(path): gw.aw.sha256_file(path)
            for path in all_artifacts
            if path_exists(path) and gw.aw.io_path(path).is_file()
        },
        "registry_links": [rel(gw.RUN_REGISTRY), rel(gw.ALPHA_LEDGER), rel(gw.STAGE_LEDGER), rel(gw.ARTIFACT_REGISTRY)],
        "lineage_judgment": "connected_GW_package_to_GX_runtime_probe(GW 패키지를 GX 런타임 탐침에 연결)",
        "claim_boundary": CLAIM_BOUNDARY,
    }
...
def write_report(final: Mapping[str, Any]) -> Path:
    text = f"""# Stage337 run337GX MT5 Negative Repair LightGBM Runtime Probe(337단계 337GX MT5 런타임 탐침)

## Conclusion(결론)

run337GX(337GX 실행)는 run337GW(337GW 실행)의 MT5 negative repair LightGBM package(런타임 긍정 방향 안정 수리 패키지)를 MT5 terminal(MT5 터미널)에 시도했다.

Action(행동): Strategy Tester(전략 테스터)를 attempt(시도)별로 실행하거나 blocker(차단 사유)를 기록했다. Effect(효과): proxy expected value(프록시 예상값)와 MT5 runtime output(MT5 런타임 출력)을 다음 GY review(GY 검토)에서 비교할 수 있다.
...
"""
    return gw.aw.write_text_lossless(REPORT_PATH, text, True)


def write_decision(final: Mapping[str, Any]) -> Path:
    text = f"""# {TODAY} Stage337GX Decision(337GX 결정)
...
claim_boundary(주장 경계): `{CLAIM_BOUNDARY}`
"""
    return gw.aw.write_text_lossless(DECISION_DOC, text, True)


def update_docs(final: Mapping[str, Any]) -> list[Path]:
    artifacts: list[Path] = []
    branch = fa.ey.current_branch()
    workspace, workspace_bom = gw.aw.read_text_lossless(gw.WORKSPACE_STATE)
    ...
    focus = (
        "- >-\n"
        f"  Stage337 run337GX focus complete: run337GX(337GX 실행)는 `{final['status']}`로 MT5 runtime probe(MT5 런타임 탐침)를 시도했다. "
    ...
    if "Stage337 run337GX focus complete" in workspace:
        workspace = re.sub(r"- >-\n  Stage337 run337GX focus complete:.*?(?=\n- >-|\n[a-zA-Z_]+:|$)", focus.rstrip(), workspace, count=1, flags=re.S)
    else:
        workspace = workspace.replace("current_focus:\n", "current_focus:\n" + focus, 1)
    artifacts.append(gw.aw.write_text_lossless(gw.WORKSPACE_STATE, workspace, workspace_bom))

    current, current_bom = gw.aw.read_text_lossless(gw.CURRENT_STATE)
    ...
    section = f"""## run337GX MT5 Runtime Probe(MT5 런타임 탐침)
...
- report_rows(보고서 행): `{final['report_rows']}`
- gates(게이트): `{final['passed_gates']}/{final['gate_rows']}`
- effect(효과): MT5 external check(MT5 외부 확인)를 실제로 시도하고 GY review(GY 검토)로 넘긴다. 운영 주장은 닫는다.
- next_action(다음 행동): `{final['next_action']}`
"""
    current = fb.upsert_section_before(current, "## run337GW Runtime Probe Package", section, "run337GX MT5 Runtime Probe")
    artifacts.append(gw.aw.write_text_lossless(gw.CURRENT_STATE, current, current_bom))
...
- next_action(다음 행동): `{final['next_action']}`
- effect(효과): GX(337GX 실행)는 runtime probe(런타임 탐침) 근거만 만들며 operating selection(운영 선택)은 하지 않는다.
"""
    artifacts.append(gw.aw.write_text_lossless(gw.SELECTED_STATUS, selection, True))

    brief, brief_bom = gw.aw.read_text_lossless(gw.STAGE_BRIEF)
    brief_entry = (
        f"- {TODAY}: run337GX(337GX 실행) `{final['status']}`. "
    ...
    artifacts.append(gw.aw.write_text_lossless(gw.STAGE_BRIEF, fb.upsert_single_line(brief, "run337GX(337GX 실행)", brief_entry), brief_bom))

    changelog, changelog_bom = gw.aw.read_text_lossless(gw.CHANGELOG)
    changelog_entry = (
        f"- {TODAY}: Stage337 run337GX(337GX 실행) `{final['status']}`. "
        f"Effect(효과): MT5 runtime probe(MT5 런타임 탐침)를 시도하고 `{final['next_action']}`을 열었다. Forward/Goal(전진/목표)은 주장하지 않았다."
    )
    artifacts.append(gw.aw.write_text_lossless(gw.CHANGELOG, fb.upsert_single_line(changelog, "Stage337 run337GX", changelog_entry), changelog_bom))
    return artifacts
...
def update_registers(final: Mapping[str, Any]) -> list[Path]:
    run_row = {
        "run_id": RUN_ID,
        "stage_id": STAGE_ID,
        "lane": "mt5_negative_repair_lightgbm_mt5_runtime_probe",
        "status": final["status"],
    ...
    alpha_row = {
        ...
        "parent_run_id": PARENT_RUN_ID,
        "record_view": "mt5_negative_repair_lightgbm_mt5_runtime_probe(런타임 긍정 방향 안정 수리 MT5 런타임 탐침)",
    ...
    stage_row = {
        ...
        "family": "mt5_negative_repair_lightgbm_mt5_runtime_probe",
        "question": "do GW MT5-negative repair LightGBM ONNX runtime packages execute in MT5 and match expected probabilities",
    ...
    return [
        fb.upsert_csv_worktree(gw.RUN_REGISTRY, gw.aw.RUN_REGISTRY_COLUMNS, run_row, "run_id"),
        fb.upsert_csv_worktree(gw.ALPHA_LEDGER, gw.aw.ALPHA_LEDGER_COLUMNS, alpha_row, "ledger_row_id"),
        fb.upsert_csv_worktree(gw.STAGE_LEDGER, gw.aw.STAGE_LEDGER_COLUMNS, stage_row, "ledger_row_id"),
    ]
...
def configure_execution_engine() -> None:
    replacements = {
        ...
        "SELECTED_STATUS": gw.SELECTED_STATUS,
        "STAGE_BRIEF": gw.STAGE_BRIEF,
        "WORKSPACE_STATE": gw.WORKSPACE_STATE,
        "CURRENT_STATE": gw.CURRENT_STATE,
        "CHANGELOG": gw.CHANGELOG,
        "RUN_REGISTRY": gw.RUN_REGISTRY,
        "ALPHA_LEDGER": gw.ALPHA_LEDGER,
        "ARTIFACT_REGISTRY": gw.ARTIFACT_REGISTRY,
        "STAGE_LEDGER": gw.STAGE_LEDGER,
        "FA_FINAL": gw.FINAL_DECISION,
        "FA_GATES": gw.GATE_AUDIT,
        "FA_QUEUE": gw.EXECUTION_QUEUE,
        "FA_ATTEMPT_PACKAGE": gw.RUNTIME_PROBE_ATTEMPT_PACKAGE,
        "FA_EXPECTED_TAPE": gw.EXPECTED_PROBABILITY_TAPE,
        "FA_COMMON_SYNC": gw.COMMON_FILES_SYNC,
        "FA_TESTER_SET": gw.TESTER_SET_MANIFEST,
        "FA_TESTER_INI": gw.TESTER_INI_MANIFEST,
        "FA_MODEL_HANDOFF": gw.MODEL_HANDOFF_MANIFEST,
        "FA_FEATURE_MANIFEST": gw.FEATURE_MATRIX_MANIFEST,
