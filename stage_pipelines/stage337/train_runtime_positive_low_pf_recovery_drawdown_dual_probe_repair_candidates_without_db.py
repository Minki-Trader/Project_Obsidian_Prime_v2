...
JM_RUN_NUMBER = "run337JM"
JL_RUN_NUMBER = "run337JL"
JM_RUN_DIR = STAGE_DIR / "02_runs" / JM_RUN_NUMBER
JL_RUN_DIR = STAGE_DIR / "02_runs" / JL_RUN_NUMBER
...
jl = SimpleNamespace(
    JL_INPUT_FRAME=JL_RUN_DIR / "jl_runtime_positive_low_pf_recovery_drawdown_repair_input_frame.parquet",
    JL_ALLOWED_FEATURES=JL_RUN_DIR / "jl_allowed_model_feature_set.csv",
    JL_TASK_SEEDS=JL_RUN_DIR / "run337JM_training_task_seed_matrix.csv",
)
jm = SimpleNamespace(
    aw=aw,
    STAGE_ID=STAGE_ID,
    ...
    FINAL_DECISION=JM_RUN_DIR / "final_decision.json",
    GATE_AUDIT=JM_RUN_DIR / "required_gate_coverage_audit.csv",
    JN_QUEUE=JM_RUN_DIR / "run337JN_training_queue.csv",
    JM_TASK_ELIGIBILITY=JM_RUN_DIR / "jm_training_task_eligibility.csv",
    JM_FEATURE_BOUNDARY_REVIEW=JM_RUN_DIR / "jm_feature_boundary_review.csv",
    JM_WEIGHT_REVIEW=JM_RUN_DIR / "jm_weight_saturation_review.csv",
    JM_RUNTIME_PARITY_REVIEW=JM_RUN_DIR / "jm_runtime_parity_review.csv",
    JM_LINEAGE_REVIEW=JM_RUN_DIR / "jm_lineage_review.csv",
    jl=jl,
)

_stub_name = "stage_pipelines.stage337.review_proxy_mt5_negative_lifecycle_cost_trade_shape_repair_inputs_without_db"
_stub = ModuleType(_stub_name)
_stub.aw = aw
_stub.STAGE_ID = STAGE_ID
_stub.STAGE_DIR = STAGE_DIR
_stub.RUN_ID = jm.RUN_ID
_stub.FINAL_DECISION = jm.FINAL_DECISION
_stub.GATE_AUDIT = jm.GATE_AUDIT
_stub.IP_QUEUE = jm.JN_QUEUE
_stub.IO_TASK_ELIGIBILITY = jm.JM_TASK_ELIGIBILITY
_stub.IO_FEATURE_BOUNDARY_REVIEW = jm.JM_FEATURE_BOUNDARY_REVIEW
_stub.inr = SimpleNamespace(
    IN_INPUT_FRAME=jl.JL_INPUT_FRAME,
    IN_ALLOWED_FEATURES=jl.JL_ALLOWED_FEATURES,
    IN_TASK_SEEDS=jl.JL_TASK_SEEDS,
)
...
RUN_NUMBER = "run337JN"
RUN_ID = "run337JN_train_runtime_positive_low_pf_recovery_drawdown_dual_probe_repair_candidates_without_db_v1"
PARENT_RUN_ID = jm.RUN_ID
NEXT_RUN_ID = "run337JO_review_runtime_positive_low_pf_recovery_drawdown_dual_probe_repair_training_without_db_v1"
STATUS = "completed_stage337JN_runtime_positive_low_pf_recovery_drawdown_dual_probe_repair_candidates_trained_onnx_materialized_review_required_no_selection_no_mt5"
JUDGMENT = "runtime_positive_low_pf_recovery_drawdown_dual_probe_repair_candidates_trained_with_onnx_parity_and_proxy_score_review_required"
DECISION = "stage337JN_open_run337JO_runtime_positive_low_pf_recovery_drawdown_dual_probe_repair_training_review"
CLAIM_BOUNDARY = (
...
REPORT_PATH = REVIEW_DIR / "run337JN_positive_low_pf_recovery_drawdown_dual_probe_repair_candidate_training.md"
DECISION_DOC = ROOT / "docs" / "decisions" / f"{TODAY}_stage337JN_positive_low_pf_recovery_drawdown_dual_probe_repair_candidate_training.md"
...
FEATURE_SCHEMA = RUN_DIR / "jn_allowed_feature_schema.json"
TRAINING_TASK_REVIEW = RUN_DIR / "jn_training_task_review.csv"
SAMPLE_WEIGHT_AUDIT = RUN_DIR / "jn_sample_weight_audit.csv"
TRAINED_MODEL_MANIFEST = RUN_DIR / "jn_trained_model_manifest.csv"
ONNX_PARITY = RUN_DIR / "jn_onnx_parity_matrix.csv"
CLASSIFICATION_SCORECARD = RUN_DIR / "jn_inner_holdout_classification_scorecard.csv"
PROXY_TRADE_SCORECARD = RUN_DIR / "jn_inner_holdout_proxy_trade_scorecard.csv"
FEATURE_IMPORTANCE = RUN_DIR / "jn_feature_importance_top20.csv"
RUNTIME_FIREWALL = RUN_DIR / "jn_runtime_firewall_review.csv"
RELEASE_DISPOSITION = RUN_DIR / "jn_training_release_disposition.csv"
JO_QUEUE = RUN_DIR / "run337JO_review_queue.csv"
...
INPUT_FILES = (
    ...
    jm.GATE_AUDIT,
    jm.JN_QUEUE,
    jm.JM_TASK_ELIGIBILITY,
    jm.JM_FEATURE_BOUNDARY_REVIEW,
    jm.JM_WEIGHT_REVIEW,
    jm.JM_RUNTIME_PARITY_REVIEW,
    jm.JM_LINEAGE_REVIEW,
    jm.jl.JL_INPUT_FRAME,
    jm.jl.JL_ALLOWED_FEATURES,
    jm.jl.JL_TASK_SEEDS,
)
OUTPUT_FILES = (
    FEATURE_SCHEMA,
...
def configure_training_engine() -> None:
    source = SimpleNamespace(
        ...
        STAGE_DIR=STAGE_DIR,
        RUN_ID=jm.RUN_ID,
        FINAL_DECISION=jm.FINAL_DECISION,
        GATE_AUDIT=jm.GATE_AUDIT,
        IP_QUEUE=jm.JN_QUEUE,
        IO_TASK_ELIGIBILITY=jm.JM_TASK_ELIGIBILITY,
        IO_FEATURE_BOUNDARY_REVIEW=jm.JM_FEATURE_BOUNDARY_REVIEW,
        inr=SimpleNamespace(
            IN_INPUT_FRAME=jm.jl.JL_INPUT_FRAME,
            IN_ALLOWED_FEATURES=jm.jl.JL_ALLOWED_FEATURES,
            IN_TASK_SEEDS=jm.jl.JL_TASK_SEEDS,
        ),
    )
...
def make_gates(summary: Mapping[str, Any]) -> pd.DataFrame:
    jm_gates = read_csv(jm.GATE_AUDIT)
    return pd.DataFrame(
        [
            gate_row(
                "parent_jm_gates_passed",
                "passed" if passed(jm_gates["status"]) else "failed",
                rel(jm.GATE_AUDIT),
                "JM review(JM 입력 검토) 통과 뒤에만 학습해 data integrity(데이터 무결성)를 보존한다.",
            ),
            gate_row(
                "feature_schema_materialized",
                "passed" if summary["feature_count"] == 58 and exists(FEATURE_SCHEMA) else "failed",
                rel(FEATURE_SCHEMA),
                "ONNX(온엑스) feature order(피처 순서)를 고정해 MT5(메타트레이더5) 입력 의미를 보존한다.",
            ),
            gate_row(
            ...
            gate_row(
                ...
                "inner train/holdout(내부 학습/보류) 분류 진단을 남겨 overfit(과적합) 위험을 볼 수 있게 한다.",
            ),
            gate_row(
                "proxy_trade_scored",
                "passed" if summary["proxy_trade_rows"] == summary["trained_model_rows"] * 2 else "failed",
                rel(PROXY_TRADE_SCORECARD),
                "proxy trade score(프록시 거래 점수)를 남기되 MT5 KPI(MT5 핵심 성과 지표)로 대체하지 않는다.",
            ),
            gate_row(
                ...
                "runtime firewall(런타임 방화벽)로 candidate training(후보 학습)과 operating claim(운영 주장)을 분리한다.",
            ),
            gate_row(
                "next_review_queue_opened",
                "passed" if exists(JO_QUEUE) else "failed",
                rel(JO_QUEUE),
                "JO review(JO 검토)를 열어 proxy usability(프록시 사용성)와 parity(동등성)를 먼저 보게 한다.",
            ),
            gate_row(
...
def generated_model_paths() -> list[Path]:
    if not exists(TRAINED_MODEL_MANIFEST):
        return []
    manifest = read_csv(TRAINED_MODEL_MANIFEST)
    paths: list[Path] = []
    for column in ("model_path", "onnx_path"):
...
def artifact_paths() -> list[Path]:
    return list(OUTPUT_FILES) + generated_model_paths()
            ...
            [
                {
                    ...
                    "queued_task": "review_runtime_positive_low_pf_recovery_drawdown_dual_probe_repair_training_before_runtime_package(런타임 양수 저PF/저회복/낙폭 이중 탐침 수리 학습 검토 후 런타임 패키지 판단)",
                    ...
                    "required_review": "proxy usability, ONNX parity, side/PF/drawdown/recovery/cost attribution(프록시 사용성, ONNX 동등성, 방향/PF/낙폭/회복/비용 귀속)",
                    "claim_boundary": CLAIM_BOUNDARY,
                }
            ]
        ),
...
def rewrite_engine_review_labels() -> None:
    if exists(RUNTIME_FIREWALL):
        ...
        if "reason" in firewall.columns:
            firewall["reason"] = "JO review required before runtime package(런타임 패키지 전 JO 검토 필요)"
...
def write_receipts(summary: Mapping[str, Any], gates: pd.DataFrame) -> None:
    ...
    base = {
        ...
        "support_skills": "obsidian-data-integrity, obsidian-artifact-lineage, obsidian-runtime-parity(데이터 무결성, 산출물 계보, 런타임 동등성)",
    }
    write_json(
        RUN_EVIDENCE_RECEIPT,
        {
            **base,
            ...
            "scoreboard": "runtime_positive_low_pf_recovery_drawdown_repair_scout(런타임 양수 저PF/저회복/낙폭 수리 탐색)",
            "parity_level": "P2_model_input_parity_closed(P2 모델 입력 동등성 닫힘)",
            "external_verification_status": "out_of_scope_by_claim(주장 범위 밖)",
            "effect": "후보는 만들었지만 MT5 KPI(MT5 핵심 성과 지표)나 운영 가능성으로 해석하지 않게 한다.",
        },
    )
    write_json(
        DATA_RECEIPT,
        {
            **base,
            "data_source": rel(jm.jl.JL_INPUT_FRAME),
            "time_axis": "UTC closed-bar/as-of inherited from JL(UTC 닫힌 봉 시점, JL 상속)",
            "sample_scope": f"Tier A rows={summary['frame_rows']}; Tier B missing_required(Tier A 행={summary['frame_rows']}, Tier B 필수 누락)",
            "feature_label_boundary": rel(jm.JM_FEATURE_BOUNDARY_REVIEW),
            ...
            "data_hash_or_identity": {rel(jm.jl.JL_INPUT_FRAME): sha(jm.jl.JL_INPUT_FRAME)},
    ...
    write_json(
        MODEL_RECEIPT,
        {
            **base,
            ...
            "target_and_label": "JL profit-quality/density labels, HX/JD runtime labels(JL 수익 품질/밀도 라벨, HX/JD 런타임 라벨)",
            "split_method": "source_row_id ordered inner holdout(source_row_id 순서 내부 보류)",
            "selection_metric": "none; review only(없음, 검토 전용)",
            "secondary_metrics": "balanced_accuracy, macro_f1, proxy net/PF/drawdown/recovery(균형 정확도, 매크로 F1, 프록시 순수익/PF/낙폭/회복)",
            "threshold_policy": "argmax only, no threshold tuning(argmax 전용, 임계값 조정 없음)",
            "overfit_risk": "multiple repair axes without WFO(여러 수리 축, WFO 없음)",
            "calibration_risk": "probabilities are uncalibrated rank signals(확률은 미보정 순위 신호)",
            "comparison_baseline": jm_final.get("positive_clue_model_id", ""),
    ...
    write_json(
        ...
        {
            **base,
            "observed_change": "runtime-positive low-PF/recovery/drawdown repair candidates trained(런타임 양수 저PF/회복/낙폭 수리 후보 학습)",
            "positive_clue_model_id": jm_final.get("positive_clue_model_id", ""),
            "negative_control_model_id": jm_final.get("negative_control_model_id", ""),
            ...
            "segment_checks": "direction, density, drawdown, recovery proxy only(방향, 밀도, 낙폭, 회복 프록시 전용)",
            "trade_shape": rel(PROXY_TRADE_SCORECARD),
            "alternative_explanations": "proxy may still fail MT5 lifecycle/cost/spread/slippage(프록시는 MT5 생명주기/비용/스프레드/슬리피지에서 실패할 수 있음)",
            "attribution_confidence": "low_until_JO_review_and_MT5_probe(JO 검토와 MT5 탐침 전까지 낮음)",
    ...
    write_json(
        JUDGMENT_RECEIPT,
        {
            **base,
            ...
            "evidence_missing": "JO review, runtime package, MT5 runtime probe(JO 검토, 런타임 패키지, MT5 런타임 탐침)",
    ...
    write_json(
        LINEAGE_RECEIPT,
        {
            **base,
            ...
            "gate_coverage": f"{int(gates['status'].astype(str).eq('passed').sum())}/{len(gates)}",
        },
    )
...
def write_docs(final: Mapping[str, Any]) -> None:
    report = f"""# run337JN Positive Low PF Recovery Drawdown Candidate Training(run337JN 양수 저PF 회복 낙폭 후보 학습)
...
- positive_inner_holdout_proxy_rows(내부 보류 프록시 양수 행): `{final['positive_inner_holdout_proxy_rows']}`
...
"""
    decision = f"""# {TODAY} Stage337JN Decision(337JN 결정)
...
JN training(JN 학습)은 ONNX(온엑스) 후보와 proxy score(프록시 점수)를 만들었다.
효과는 JO review(JO 검토)가 학습 산출물을 runtime package(런타임 패키지)로 넘길지 좁게 판단하게 하는 것이다.
...
"""
    ...
    marker = f"run337JN {RUN_ID}"
    append_text_once(
        STAGE_BRIEF,
        marker,
        f"""## run337JN Positive Low PF Recovery Drawdown Candidate Training(양수 저PF 회복 낙폭 후보 학습)
...
""",
    ...
    changelog_entry = f"""## {TODAY} run337JN Positive Low PF Recovery Drawdown Candidate Training(양수 저PF 회복 낙폭 후보 학습)
...
def update_registers(final: Mapping[str, Any]) -> None:
    base = {
        ...
        "lane": "runtime_positive_low_pf_recovery_drawdown_dual_probe_repair_candidate_training",
...
def artifact_type(path: Path) -> str:
    return "report" if path.suffix.lower() == ".md" else path.suffix.lower().lstrip(".") or "file"
...
def update_artifact_registry(paths: Sequence[Path]) -> None:
    ...
    if rows:
        ...
        registry = registry.loc[
            ~registry["path"].astype(str).isin(row_paths)
            & ~registry["artifact_id"].astype(str).isin(row_ids)
        ].copy()
...
def main() -> None:
    ...
    if not failed.empty:
        raise RuntimeError(f"JN gates failed: {failed[['gate_id', 'status']].to_dict(orient='records')}")
    print(
        json.dumps(
            {
                "run_id": RUN_ID,
