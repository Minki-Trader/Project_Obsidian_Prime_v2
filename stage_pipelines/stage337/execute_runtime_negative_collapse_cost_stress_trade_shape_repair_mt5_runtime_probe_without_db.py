...
from stage_pipelines.stage337 import (  # noqa: E402
    materialize_runtime_negative_collapse_cost_stress_trade_shape_repair_runtime_probe_package_without_db as jh,
)
...
TODAY = "2026-06-01"
STAGE_ID = jh.STAGE_ID
STAGE_DIR = jh.STAGE_DIR
RUN_NUMBER = "run337JI"
RUN_ID = "run337JI_execute_runtime_negative_collapse_cost_stress_trade_shape_repair_mt5_runtime_probe_without_db_v1"
PARENT_RUN_ID = jh.RUN_ID
NEXT_RUN_ID = "run337JJ_review_runtime_negative_collapse_cost_stress_trade_shape_repair_mt5_runtime_probe_or_repair_without_db_v1"
STATUS_COMPLETED = "completed_stage337JI_runtime_negative_collapse_repair_mt5_runtime_probe_executed_review_required_no_forward_decision"
STATUS_BLOCKED = "blocked_stage337JI_runtime_negative_collapse_repair_mt5_runtime_probe_attempt_missing_or_failed_outputs_no_forward_decision"
...
DECISION_COMPLETED = "stage337JI_open_run337JJ_review_runtime_negative_collapse_repair_mt5_runtime_probe"
DECISION_BLOCKED = "stage337JI_open_run337JJ_review_or_repair_runtime_negative_collapse_repair_mt5_runtime_probe_attempt"
CLAIM_BOUNDARY = (
...
DEFAULT_TESTER_PROFILE_ROOT = jh.DEFAULT_PORTABLE_ROOT / "MQL5" / "Profiles" / "Tester"
DEFAULT_TERMINAL_DATA_ROOT = jh.DEFAULT_PORTABLE_ROOT
...
REPORT_PATH = REVIEW_DIR / "run337JI_runtime_negative_collapse_repair_mt5_runtime_probe.md"
DECISION_DOC = ROOT / "docs" / "decisions" / f"{TODAY}_stage337JI_runtime_negative_collapse_repair_mt5_runtime_probe.md"
...
EXECUTION_SUMMARY = RUN_DIR / "runtime_negative_collapse_repair_mt5_runtime_probe_summary.csv"
...
INPUT_FILES = (
    jh.FINAL_DECISION,
    jh.GATE_AUDIT,
    jh.RUNTIME_PROBE_ATTEMPT_PACKAGE,
    jh.EXPECTED_PROBABILITY_TAPE,
    jh.COMMON_FILES_SYNC,
)
OUTPUT_FILES = (
    ATTEMPT_PACKAGE,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Stage337JI runtime-negative collapse repair MT5 runtime probe.")
    parser.add_argument("--terminal-path", default=str(jh.DEFAULT_TERMINAL))
    parser.add_argument("--common-files-root", default=str(jh.DEFAULT_COMMON_FILES))
...
def configure_execute_engine() -> None:
    parent_package = SimpleNamespace(
        ...
        STAGE_DIR=STAGE_DIR,
        RUN_ID=jh.RUN_ID,
        FINAL_DECISION=jh.FINAL_DECISION,
        GATE_AUDIT=jh.GATE_AUDIT,
        RUNTIME_PROBE_ATTEMPT_PACKAGE=jh.RUNTIME_PROBE_ATTEMPT_PACKAGE,
        EXPECTED_PROBABILITY_TAPE=jh.EXPECTED_PROBABILITY_TAPE,
        COMMON_FILES_SYNC=jh.COMMON_FILES_SYNC,
        RUNTIME_PARITY_CONTRACT=jh.RUNTIME_PARITY_CONTRACT,
        TESTER_IDENTITY_CONTRACT=jh.TESTER_IDENTITY_CONTRACT,
        DEFAULT_PORTABLE_ROOT=jh.DEFAULT_PORTABLE_ROOT,
        DEFAULT_TERMINAL=jh.DEFAULT_TERMINAL,
        DEFAULT_METAEDITOR=jh.DEFAULT_METAEDITOR,
        DEFAULT_COMMON_FILES=jh.DEFAULT_COMMON_FILES,
        PORTABLE_EA_EX5=jh.PORTABLE_EA_EX5,
        EA_BINARY=jh.EA_BINARY,
    )
...
def runtime_identity(attempt_rows: int, args: Any) -> pd.DataFrame:
    return pd.DataFrame(
        [
            {
                "identity_id": "stage337JI_runtime_identity",
                ...
                "portable_ea_ex5": jh.PORTABLE_EA_EX5.as_posix(),
                "portable_ea_ex5_exists": exists(jh.PORTABLE_EA_EX5),
                "portable_ea_ex5_sha256": sha(jh.PORTABLE_EA_EX5) if exists(jh.PORTABLE_EA_EX5) else "",
                "repo_ea_binary": jh.EA_BINARY.as_posix(),
                "repo_ea_binary_exists": exists(jh.EA_BINARY),
                "repo_ea_binary_sha256": sha(jh.EA_BINARY) if exists(jh.EA_BINARY) else "",
...
def make_gates(final: Mapping[str, Any]) -> pd.DataFrame:
    parent_gates = read_csv(jh.GATE_AUDIT)
    ...
    return pd.DataFrame(
        [
            gate_row("parent_jh_gates_passed", "passed" if passed_status(parent_gates["status"]).all() else "failed", rel(jh.GATE_AUDIT), "JH package(JH 패키지) gate(게이트)가 통과한 뒤에만 JI execution(JI 실행)을 시작한다."),
            ...
            gate_row("runtime_output_copy_recorded", "passed" if final["runtime_output_copy_rows"] >= final["attempt_rows"] * 2 else "failed", rel(RUNTIME_OUTPUT_COPY), "telemetry/summary(런타임 기록/요약) 복사 감사를 기록한다."),
...
def write_docs(final: Mapping[str, Any]) -> None:
    report = f"""# run337JI Runtime Negative Collapse Repair MT5 Runtime Probe(run337JI 런타임 음수 붕괴 수리 MT5 런타임 탐침)
...
JH package(JH 패키지)의 두 ONNX(온엑스) 후보를 MT5 runtime probe(MT5 런타임 탐침)로 실행하거나 차단 사유를 기록했다.
...
"""
    decision = f"""# {TODAY} Stage337JI Decision(337JI 결정)
...
Effect(효과): proxy(프록시)를 MT5 runtime evidence(MT5 런타임 근거) 없이 운영 주장으로 올리지 않는다.
...
JI는 MT5 runtime probe(MT5 런타임 탐침)를 시도했고, JJ review(JJ 검토)가 diff(차이)와 blocker(차단 사유)를 판정해야 한다.
...
"""
    ...
    marker = f"run337JI {RUN_ID}"
    append_text_once(
        STAGE_BRIEF,
        marker,
        f"""## run337JI MT5 Runtime Probe(MT5 런타임 탐침)
...
""",
    ...
    changelog_entry = f"""## {TODAY} run337JI MT5 Runtime Probe(MT5 런타임 탐침)
