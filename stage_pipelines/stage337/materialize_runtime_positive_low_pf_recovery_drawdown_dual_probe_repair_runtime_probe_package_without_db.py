...
RUN_NUMBER = "run337JP"
RUN_ID = "run337JP_materialize_runtime_positive_low_pf_recovery_drawdown_dual_probe_repair_runtime_probe_package_without_db_v1"
PARENT_RUN_ID = "run337JO_review_runtime_positive_low_pf_recovery_drawdown_dual_probe_repair_training_without_db_v1"
NEXT_RUN_ID = "run337JQ_execute_runtime_positive_low_pf_recovery_drawdown_dual_probe_repair_mt5_runtime_probe_without_db_v1"
STATUS = "completed_stage337JP_runtime_positive_low_pf_recovery_drawdown_dual_probe_repair_runtime_probe_package_materialized_no_mt5_execution"
JUDGMENT = "runtime_probe_package_ready_for_three_proxy_positive_repair_candidates_proxy_mt5_diff_required_no_selection"
DECISION = "stage337JP_open_run337JQ_execute_runtime_positive_low_pf_recovery_drawdown_dual_probe_repair_mt5_runtime_probe"
CLAIM_BOUNDARY = (
    "research_development_runtime_probe_package_only_no_mt5_execution_in_JP_no_candidate_selection_"
    "no_forward_no_runtime_authority_no_operating_or_goal_claim"
)

RUN_DIR = STAGE_DIR / "02_runs" / RUN_NUMBER
PARENT_RUN_DIR = STAGE_DIR / "02_runs" / "run337JO"
TRAIN_RUN_DIR = STAGE_DIR / "02_runs" / "run337JN"
INPUT_RUN_DIR = STAGE_DIR / "02_runs" / "run337JL"
MT5_DIR = RUN_DIR / "mt5"
...
REPORT_PATH = REVIEW_DIR / "run337JP_positive_low_pf_recovery_drawdown_dual_probe_repair_runtime_probe_package.md"
DECISION_DOC = ROOT / "docs" / "decisions" / f"{TODAY}_stage337JP_positive_low_pf_recovery_drawdown_dual_probe_repair_runtime_probe_package.md"
...
COMMON_ROOT = f"Project_Obsidian_Prime_v2/stage337/{RUN_NUMBER}_positive_low_pf_recovery_drawdown_probe"
...
FEATURE_COMMON_NAME = "positive_low_pf_recovery_drawdown_inner_holdout_features.csv"
...
PROBE_PRIORITY = PARENT_RUN_DIR / "jo_runtime_probe_priority_matrix.csv"
PARENT_RUNTIME_QUEUE = PARENT_RUN_DIR / "run337JP_runtime_probe_package_queue.csv"
FEATURE_SCHEMA = TRAIN_RUN_DIR / "jn_allowed_feature_schema.json"
TRAINED_MODEL_MANIFEST = TRAIN_RUN_DIR / "jn_trained_model_manifest.csv"
ONNX_PARITY = TRAIN_RUN_DIR / "jn_onnx_parity_matrix.csv"
INPUT_FRAME = INPUT_RUN_DIR / "jl_runtime_positive_low_pf_recovery_drawdown_repair_input_frame.parquet"

FEATURE_MATRIX = FEATURE_DIR / FEATURE_COMMON_NAME
FEATURE_MATRIX_MANIFEST = RUN_DIR / "runtime_feature_matrix_manifest.csv"
EXPECTED_PROBABILITY_TAPE = EXPECTED_DIR / "positive_low_pf_recovery_drawdown_expected_probability_tape.csv"
...
EXECUTION_QUEUE = RUN_DIR / "run337JQ_execution_queue.csv"
...
def format_feature_value(value: object) -> str:
    ...
    if not math.isfinite(float(number)):
        raise ValueError("non-finite feature value(유한하지 않은 피처 값)")
...
def load_features() -> list[str]:
    schema = read_json(FEATURE_SCHEMA)
...
def valid_column_for_target(target: str) -> str:
    if target == "jl_label_class_profit_quality_fwd18":
        return "jl_valid_profit_quality_fwd18"
    if target == "jl_label_class_density_throttle_fwd18":
        return "jl_valid_density_throttle_fwd18"
    if target == "jd_label_class_runtime_pnl_fwd18":
        return "jd_valid_runtime_pnl_fwd18"
    if target.endswith("fwd6"):
        return "hx_valid_fwd6"
    if target.endswith("fwd24"):
        return "hx_valid_fwd24"
    return "hx_valid_fwd18"
...
def attempt_name(index: int, model_id: str) -> str:
    return f"jp{index:02d}_{model_id}"


def inner_holdout_unique_frame(features: Sequence[str]) -> pd.DataFrame:
    priority = read_csv(PROBE_PRIORITY)
    if priority.empty:
        raise RuntimeError("no JO probe priority candidate available for JP package")
    target = str(priority.iloc[0]["target_column"])
    valid_col = valid_column_for_target(target)
    frame = pd.read_parquet(io(INPUT_FRAME)).copy()
    ...
    if missing:
        raise ValueError(f"missing runtime features(런타임 피처 누락): {missing}")
    return unique
...
def materialize_feature_matrix(frame: pd.DataFrame, features: Sequence[str]) -> tuple[pd.DataFrame, list[str], list[dict[str, Any]]]:
    ...
    feature_common = f"{COMMON_FEATURE_DIR}/{FEATURE_COMMON_NAME}"
    sync_rows = [
        copy_file(
            FEATURE_MATRIX,
            DEFAULT_COMMON_FILES / Path(feature_common),
            "common_feature_matrix",
            "Feature matrix(피처 행렬)를 Common Files(공용 파일)에 복사해 MT5 EA(MT5 전문가 자문)가 읽게 한다.",
        )
    ]
...
def materialize_models_and_expected(
    ...
    priority = read_csv(PROBE_PRIORITY).sort_values(["probe_order", "raw_proxy_rank"], ascending=[True, True]).copy()
    if priority.empty:
        raise RuntimeError("no JO probe priority rows")
    ...
    for index, (_, candidate) in enumerate(priority.iterrows(), start=1):
        model_id = str(candidate["model_id"])
        if model_id not in model_lookup:
            raise RuntimeError(f"model_id not found in manifest: {model_id}")
        ...
        sync_rows.append(copy_file(source_joblib, local_joblib, f"local_joblib::{attempt}", "joblib model(joblib 모델)을 패키지에 복사해 expected tape(예상 테이프)를 재현한다."))
        sync_rows.append(copy_file(source_onnx, local_onnx, f"local_onnx::{attempt}", "ONNX model(온엑스 모델)을 패키지에 복사해 산출물 hash(해시)를 고정한다."))
        sync_rows.append(copy_file(local_onnx, DEFAULT_COMMON_FILES / Path(common_onnx), f"common_onnx::{attempt}", "ONNX model(온엑스 모델)을 Common Files(공용 파일)에 복사해 MT5 EA(MT5 전문가 자문)가 읽게 한다."))
            ...
            expected_rows.append(
                {
                    ...
                    "model_id": model_id,
                    "task_id": task_id,
                    "probe_role": candidate.get("jo_probe_role", ""),
                    "bar_time": timestamp,
        ...
        set_values = {
            ...
            "InpExplorationLabel": "stage337JP_PositiveLowPFRecoveryDrawdownDualProbe__MT5RuntimeProbe",
            "InpTierLabel": "Tier A",
        ...
        attempts.append(
            {
                ...
                "effect": "Attempt(시도)는 모델 로직 변경 없이 실행 가능하다.",
                "claim_boundary": CLAIM_BOUNDARY,
            }
        )
...
def contracts_and_queue() -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
        ...
        [
            {
                ...
                "requirement": "compare expected probabilities, input hash, decision, KPI diff(예상 확률, 입력 해시, 결정, KPI 차이 비교)",
        ...
        [
            {
                ...
                "effect": "JQ can compare runtime parity row by row(JQ가 행 단위 런타임 동등성을 비교할 수 있다).",
                "claim_boundary": CLAIM_BOUNDARY,
            }
        ]
    )
    queue = pd.DataFrame(
        [
            {
                "queue_id": "jq_execute_positive_low_pf_recovery_drawdown_mt5_runtime_probe",
                "next_run_id": NEXT_RUN_ID,
                "priority": "P0",
                "task": "execute MT5 runtime probe for three proxy-positive ONNX candidates(프록시 양수 ONNX 후보 3개 MT5 런타임 탐침 실행)",
                ...
                "effect": "Package(패키지)를 실행으로 넘기되 threshold/lots(임계값/랏)은 바꾸지 않는다.",
                "claim_boundary": CLAIM_BOUNDARY,
            }
        ]
    )
...
def materialize_package() -> tuple[dict[str, Any], list[Path]]:
    ...
    feature_manifest = pd.DataFrame(
        [
            {
                "feature_matrix_id": "positive_low_pf_recovery_drawdown_inner_holdout_features",
                "feature_set_id": "jl_runtime_positive_low_pf_recovery_drawdown_allowed_pretrade_features_v1",
                "feature_count": len(features),
                "row_count": len(frame),
    ...
    summary = {
        ...
        "probe_roles": ";".join(attempts["probe_role"].astype(str).tolist()) if not attempts.empty else "",
        "feature_order_hash": schema.get("feature_order_hash", ""),
...
def make_gates(summary: Mapping[str, Any]) -> pd.DataFrame:
    ...
    sync_ready = summary["common_sync_rows"] == summary["common_sync_ready_rows"] and summary["common_sync_rows"] >= 1 + 3 * summary["attempt_rows"]
    expected_attempts = int(read_json(PARENT_FINAL_DECISION).get("runtime_queue_candidate_count", 0))
    return pd.DataFrame(
        [
            gate_row("parent_jo_gates_passed", "passed" if parent_passed else "failed", rel(PARENT_GATE_AUDIT), "JO review(JO 검토)가 통과한 후보만 package(패키지)화한다."),
            gate_row("probe_priority_candidates_loaded", "passed" if summary["attempt_rows"] == expected_attempts and expected_attempts >= 1 else "failed", rel(PROBE_PRIORITY), "probe priority(탐침 우선순위) 후보를 누락 없이 불러온다."),
            ...
            gate_row("model_and_common_files_synced", "passed" if sync_ready else "failed", rel(COMMON_FILES_SYNC), "ONNX(온엑스)와 feature matrix(피처 행렬)를 Common Files(공용 파일)에 복사한다."),
            ...
            gate_row("runtime_attempt_package_written", "passed" if exists(RUNTIME_PROBE_ATTEMPT_PACKAGE) and summary["attempt_rows"] >= 1 else "failed", rel(RUNTIME_PROBE_ATTEMPT_PACKAGE), "JQ 실행이 읽을 attempt package(시도 패키지)를 만든다."),
            ...
            gate_row("execution_queue_opened", "passed" if exists(EXECUTION_QUEUE) else "failed", rel(EXECUTION_QUEUE), "다음 JQ runtime probe(런타임 탐침) 실행으로 넘긴다."),
            gate_row("no_mt5_execution_in_jp", "passed", rel(CLAIM_RECEIPT), "JP는 package(패키지)만 만들고 MT5 실행은 하지 않는다."),
...
def write_receipts(summary: Mapping[str, Any], gates: pd.DataFrame) -> None:
    ...
    write_json(ROUTING_RECEIPT, {**base, "next_run_id": NEXT_RUN_ID, "attempt_rows": summary["attempt_rows"], "effect": "runtime probe package(런타임 탐침 패키지)를 JQ execution(JQ 실행)으로 넘긴다."})
    ...
    write_json(MODEL_RECEIPT, {**base, "candidate_model_ids": summary["candidate_model_ids"], "model_handoff_rows": summary["model_handoff_rows"], "feature_order_hash": summary["feature_order_hash"], "effect": "ONNX handoff(온엑스 인계) hash(해시)를 고정한다."})
    write_json(RUNTIME_RECEIPT, {**base, "research_path": rel(Path(__file__)), "runtime_path": "package_only_no_execution(패키지 전용, 실행 없음)", "shared_contract": rel(RUNTIME_PARITY_CONTRACT), "parity_check": rel(EXPECTED_PROBABILITY_TAPE), "parity_identity": f"common_sync={summary['common_sync_ready_rows']}/{summary['common_sync_rows']};attempts={summary['attempt_rows']}", "runtime_claim_boundary": "runtime_probe_package_only(런타임 탐침 패키지 전용)"})
    write_json(FORENSICS_RECEIPT, {**base, "tester_identity": rel(TESTER_IDENTITY_CONTRACT), "ea_identity": summary["ea_module_hashes"], "report_identity": "not_run_in_JP(JP에서 미실행)", "trade_evidence": "not_available_until_JQ(JQ 전에는 없음)", "cost_assumptions": "Strategy Tester output required(전략 테스터 출력 필요)", "backtest_judgment": "inconclusive_package_only(패키지만으로 불충분)"})
    write_json(PERFORMANCE_RECEIPT, {**base, "observed_change": "runtime probe package materialized(런타임 탐침 패키지 물질화)", "comparison_baseline": rel(PARENT_FINAL_DECISION), "probe_roles": summary["probe_roles"], "next_probe": NEXT_RUN_ID, "attribution_confidence": "not_applicable_until_mt5(MT5 전에는 해당 없음)"})
    ...
    write_json(CLAIM_RECEIPT, {**base, "candidate_selection": "not_run", "mt5_execution": "not_run_in_JP", "runtime_authority": "not_claimed", "operating_promotion": "not_claimed", "goal_achieve": "not_claimed"})
    write_json(
        LINEAGE_RECEIPT,
        {
            **base,
...
def write_final(summary: Mapping[str, Any], gates: pd.DataFrame) -> dict[str, Any]:
    final = {
        ...
        "mt5_runtime_probe": "not_run_in_JP",
...
def write_docs(final: Mapping[str, Any]) -> None:
    report = f"""# run337JP Positive Low PF Recovery Drawdown Runtime Probe Package(run337JP 양수 저PF 회복 낙폭 런타임 탐침 패키지)
...
- probe_roles(탐침 역할): `{final['probe_roles']}`
...
"""
    decision = f"""# {TODAY} Stage337JP Decision(337JP 결정)
...
Action(행동): positive low-PF/recovery/drawdown repair(양수 저PF/회복/낙폭 수리) 후보를 MT5 runtime probe(MT5 런타임 탐침) 입력으로 만들었다.
Effect(효과): proxy expected value(프록시 예상값)를 MT5 runtime evidence(MT5 런타임 근거)와 비교할 준비가 됐다.
...
JP package(JP 패키지)는 MT5 runtime probe(MT5 런타임 탐침) 실행에 필요한 파일을 만들었다.
효과는 JQ run(JQ 실행)이 proxy-MT5 diff(프록시-MT5 차이)를 실제 런타임 근거로 확인하게 하는 것이다.
...
- probe_priority_models(탐침 우선 모델): `{final['candidate_model_ids']}`
- MT5 execution(MT5 실행): `not_run_in_JP(JP에서 미실행)`
...
"""
    ...
    marker = f"run337JP {RUN_ID}"
    append_text_once(
        STAGE_BRIEF,
        marker,
        f"""## run337JP Positive Low PF Recovery Drawdown Runtime Probe Package(양수 저PF 회복 낙폭 런타임 탐침 패키지)
...
""",
    ...
    changelog_entry = f"""## {TODAY} run337JP Positive Low PF Recovery Drawdown Runtime Probe Package(양수 저PF 회복 낙폭 런타임 탐침 패키지)

- action(행동): JO probe priority(JO 탐침 우선순위) 3개를 MT5 runtime probe(MT5 런타임 탐침) package(패키지)로 만들었다.
- effect(효과): `{NEXT_RUN_ID}`에서 proxy-MT5 diff(프록시-MT5 차이)를 실행 근거로 볼 수 있게 됐다.
            ...
            rows.append(
                {
                    "stage_id": STAGE_ID,
                    "run_id": RUN_ID,
                    "artifact_type": "report" if path.suffix.lower() == ".md" else path.suffix.lower().lstrip(".") or "file",
                    "path": artifact_path,
                    "sha256": sha(path),
                    "created_at": TODAY,
                    "claim_boundary": CLAIM_BOUNDARY,
...
def main() -> None:
    ...
    summary, _artifacts = materialize_package()
    ...
    if not failed.empty:
        raise RuntimeError(f"JP gates failed: {failed[['gate_id', 'status']].to_dict(orient='records')}")
    print(
        json.dumps(
            {
                "run_id": RUN_ID,
