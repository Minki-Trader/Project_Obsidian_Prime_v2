...
from stage_pipelines.stage337 import (  # noqa: E402
    review_runtime_negative_collapse_cost_stress_trade_shape_repair_training_without_db as jg,
)
...
TODAY = "2026-06-01"
STAGE_ID = jg.STAGE_ID
STAGE_DIR = jg.STAGE_DIR
RUN_NUMBER = "run337JH"
RUN_ID = "run337JH_materialize_runtime_negative_collapse_cost_stress_trade_shape_repair_runtime_probe_package_without_db_v1"
PARENT_RUN_ID = jg.RUN_ID
NEXT_RUN_ID = "run337JI_execute_runtime_negative_collapse_cost_stress_trade_shape_repair_mt5_runtime_probe_without_db_v1"
STATUS = "completed_stage337JH_runtime_negative_collapse_repair_runtime_probe_package_materialized_no_mt5_execution"
JUDGMENT = "runtime_probe_package_ready_for_two_proxy_positive_repair_candidates_proxy_mt5_diff_required_no_selection"
DECISION = "stage337JH_open_run337JI_execute_runtime_negative_collapse_repair_mt5_runtime_probe"
CLAIM_BOUNDARY = (
    "research_development_runtime_probe_package_only_no_mt5_execution_in_JH_no_candidate_selection_"
...
REPORT_PATH = REVIEW_DIR / "run337JH_runtime_negative_collapse_repair_runtime_probe_package.md"
DECISION_DOC = ROOT / "docs" / "decisions" / f"{TODAY}_stage337JH_runtime_negative_collapse_repair_runtime_probe_package.md"
...
COMMON_ROOT = f"Project_Obsidian_Prime_v2/stage337/{RUN_NUMBER}_runtime_negative_collapse_repair_probe"
...
FEATURE_COMMON_NAME = "runtime_negative_collapse_repair_inner_holdout_features.csv"

FEATURE_MATRIX = FEATURE_DIR / FEATURE_COMMON_NAME
FEATURE_MATRIX_MANIFEST = RUN_DIR / "runtime_feature_matrix_manifest.csv"
EXPECTED_PROBABILITY_TAPE = EXPECTED_DIR / "runtime_negative_collapse_repair_expected_probability_tape.csv"
...
EXECUTION_QUEUE = RUN_DIR / "run337JI_execution_queue.csv"
...
INPUT_FILES = (
    ...
    jg.PROBE_PRIORITY,
    jg.RUNTIME_QUEUE,
    jg.jf.TRAINED_MODEL_MANIFEST,
    jg.jf.FEATURE_SCHEMA,
    jg.jf.ONNX_PARITY,
    jg.jf.je.jd.JD_INPUT_FRAME,
)
OUTPUT_FILES = (
    FEATURE_MATRIX,
...
def load_features() -> list[str]:
    schema = read_json(jg.jf.FEATURE_SCHEMA)
...
def attempt_name(index: int, model_id: str) -> str:
    return f"jh{index:02d}_{model_id}"


def inner_holdout_unique_frame(features: Sequence[str]) -> pd.DataFrame:
    priority = read_csv(jg.PROBE_PRIORITY)
    if priority.empty:
        raise RuntimeError("no runtime probe priority candidate available for JH package")
    target = str(priority.iloc[0]["target_column"])
    if target.endswith("fwd6"):
        valid_col = "hx_valid_fwd6"
    elif target.endswith("fwd24"):
        valid_col = "hx_valid_fwd24"
    else:
        valid_col = "hx_valid_fwd18" if target.startswith("hx_") else "jd_valid_runtime_pnl_fwd18"
    frame = pd.read_parquet(io(jg.jf.je.jd.JD_INPUT_FRAME)).copy()
...
def materialize_models_and_expected(
    ...
    manifest = read_csv(jg.jf.TRAINED_MODEL_MANIFEST)
    model_lookup = {str(row["model_id"]): row for _, row in manifest.iterrows()}
    priority = read_csv(jg.PROBE_PRIORITY).copy()
    if priority.empty:
        raise RuntimeError("no JG probe priority rows")
    ...
    for index, (_, candidate) in enumerate(priority.iterrows(), start=1):
        ...
        set_values = {
            ...
            "InpExplorationLabel": "stage337JH_RuntimeNegativeCollapseRepair__MT5RuntimeProbe",
            "InpTierLabel": "Tier A",
        ...
        model_handoff.append(
            {
                ...
                "probe_disposition": candidate.get("jg_disposition", ""),
...
def contracts_and_queue() -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
        ...
        [
            {
                ...
                "known_difference": "package only; actual broker costs are read after tester output(패키지 전용, 실제 비용은 테스터 출력 후 확인)",
                ...
                "effect": "Backtest evidence(백테스트 근거)를 실행 후 감사할 수 있게 한다.",
                "claim_boundary": CLAIM_BOUNDARY,
            }
        ]
    )
        ...
        [
            {
                ...
                "effect": "Proxy(프록시)를 comparison baseline(비교 기준선)으로만 바꾼다.",
                "claim_boundary": CLAIM_BOUNDARY,
            }
        ]
    )
        ...
        [
            {
                ...
                "effect": "JI can compare runtime parity row by row(JI가 행 단위 런타임 동등성을 비교할 수 있다).",
                "claim_boundary": CLAIM_BOUNDARY,
            }
        ]
    )
    queue = pd.DataFrame(
        [
            {
                "queue_id": "ji_execute_runtime_negative_collapse_repair_mt5_runtime_probe",
                "next_run_id": NEXT_RUN_ID,
                "priority": "P0",
                "task": "execute MT5 runtime probe for runtime-negative collapse repair candidates(런타임 음수 붕괴 수리 후보 MT5 런타임 탐침 실행)",
...
def materialize_package() -> dict[str, Any]:
    ...
    feature_manifest = pd.DataFrame(
        [
            {
                "feature_matrix_id": "runtime_negative_collapse_repair_inner_holdout_features",
                "feature_set_id": jg.jf.FEATURE_SET_ID,
                "feature_count": len(features),
                "row_count": len(frame),
...
def make_gates(summary: Mapping[str, Any]) -> pd.DataFrame:
    parent_gates = read_csv(jg.GATE_AUDIT)
    ...
    expected_attempts = int(read_json(jg.FINAL_DECISION).get("runtime_queue_candidate_count", 0))
    return pd.DataFrame(
        [
            gate_row("parent_jg_gates_passed", "passed" if parent_passed else "failed", rel(jg.GATE_AUDIT), "JG review(JG 검토)가 통과한 후보만 package(패키지)화한다."),
            gate_row("probe_priority_candidates_loaded", "passed" if summary["attempt_rows"] == expected_attempts and expected_attempts >= 1 else "failed", rel(jg.PROBE_PRIORITY), "probe priority(탐침 우선순위) 후보를 누락 없이 불러온다."),
            ...
            gate_row("runtime_attempt_package_written", "passed" if exists(RUNTIME_PROBE_ATTEMPT_PACKAGE) and summary["attempt_rows"] >= 1 else "failed", rel(RUNTIME_PROBE_ATTEMPT_PACKAGE), "JI 실행이 읽을 attempt package(시도 패키지)를 만든다."),
            ...
            gate_row("execution_queue_opened", "passed" if exists(EXECUTION_QUEUE) else "failed", rel(EXECUTION_QUEUE), "다음 JI runtime probe(런타임 탐침) 실행으로 넘긴다."),
            gate_row("no_mt5_execution_in_jh", "passed", rel(CLAIM_RECEIPT), "JH는 package(패키지)만 만들고 MT5 실행은 하지 않는다."),
...
def write_receipts(summary: Mapping[str, Any], gates: pd.DataFrame) -> None:
    ...
    write_json(ROUTING_RECEIPT, {**base, "next_run_id": NEXT_RUN_ID, "attempt_rows": summary["attempt_rows"], "effect": "runtime probe package(런타임 탐침 패키지)를 JI execution(JI 실행)으로 넘긴다."})
    ...
    write_json(FORENSICS_RECEIPT, {**base, "tester_identity": rel(TESTER_IDENTITY_CONTRACT), "ea_identity": summary["ea_module_hashes"], "report_identity": "not_run_in_JH(JH에서 미실행)", "trade_evidence": "not_available_until_JI(JI 전에는 없음)", "cost_assumptions": "Strategy Tester output required(전략 테스터 출력 필요)", "backtest_judgment": "inconclusive_package_only(패키지만으로 불충분)"})
    write_json(PERFORMANCE_RECEIPT, {**base, "observed_change": "runtime probe package materialized(런타임 탐침 패키지 물질화)", "comparison_baseline": rel(jg.FINAL_DECISION), "likely_drivers": "runtime-negative collapse repair probe priority(런타임 음수 붕괴 수리 탐침 우선순위)", "next_probe": NEXT_RUN_ID, "attribution_confidence": "not_applicable_until_mt5(MT5 전에는 해당 없음)"})
    ...
    write_json(CLAIM_RECEIPT, {**base, "candidate_selection": "not_run", "mt5_execution": "not_run_in_JH", "runtime_authority": "not_claimed", "operating_promotion": "not_claimed", "goal_achieve": "not_claimed"})
    write_json(
        LINEAGE_RECEIPT,
        {
            **base,
...
def write_final(summary: Mapping[str, Any], gates: pd.DataFrame) -> dict[str, Any]:
    final = {
        ...
        "mt5_runtime_probe": "not_run_in_JH",
...
def write_docs(final: Mapping[str, Any]) -> None:
    report = f"""# run337JH Runtime Negative Collapse Repair Runtime Probe Package(run337JH 런타임 음수 붕괴 수리 런타임 탐침 패키지)
...
JG review(JG 검토)의 probe priority(탐침 우선순위) 2개를 MT5 runtime probe(MT5 런타임 탐침) package(패키지)로 물질화했다.
Effect(효과): 다음 JI run(JI 실행)이 feature matrix(피처 행렬), ONNX(온엑스), expected tape(예상 테이프), tester set/ini(테스터 설정)를 바로 사용할 수 있다.

## Boundary(경계)

No MT5 execution in JH(JH에서 MT5 실행 없음), no candidate selection(후보 선택 없음), no runtime authority(런타임 권위 없음), no Goal Achieve(목표 달성 없음).
"""
    decision = f"""# {TODAY} Stage337JH Decision(337JH 결정)
...
Action(행동): runtime-negative collapse repair(런타임 음수 붕괴 수리) 후보를 MT5 runtime probe(MT5 런타임 탐침) 입력으로 만들었다.
...
JH package(JH 패키지)는 MT5 runtime probe(MT5 런타임 탐침) 실행에 필요한 파일을 만들었다.
효과는 JI run(JI 실행)이 proxy-MT5 diff(프록시-MT5 차이)를 실제 런타임 근거로 확인하게 하는 것이다.
...
- MT5 execution(MT5 실행): `not_run_in_JH(JH에서 미실행)`
...
"""
    ...
    marker = f"run337JH {RUN_ID}"
    append_text_once(
        STAGE_BRIEF,
        marker,
        f"""## run337JH Runtime Negative Collapse Repair Runtime Probe Package(런타임 음수 붕괴 수리 런타임 탐침 패키지)
...
""",
    ...
    changelog_entry = f"""## {TODAY} run337JH Runtime Negative Collapse Repair Runtime Probe Package(런타임 음수 붕괴 수리 런타임 탐침 패키지)
...
def main() -> None:
    ...
    if not failed.empty:
        raise RuntimeError(f"JH gates failed: {failed[['gate_id', 'status']].to_dict(orient='records')}")
    print(
        json.dumps(
            {
                "run_id": RUN_ID,
