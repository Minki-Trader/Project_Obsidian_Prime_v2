...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Materialize a model-input dataset with quarantined features removed.")
    parser.add_argument("--source-training-dataset-id", default=DEFAULT_SOURCE_TRAINING_DATASET_ID)
    parser.add_argument("--model-input-dataset-id", default=DEFAULT_MODEL_INPUT_DATASET_ID)
...
def build_model_input_dataset(training_frame: pd.DataFrame) -> tuple[pd.DataFrame, dict[str, object]]:
    ...
    missing_quarantined = [name for name in QUARANTINED_FEATURES if name not in training_frame.columns]
    if missing_quarantined:
        raise RuntimeError(f"Cannot quarantine missing features: {missing_quarantined}")

    export_columns = IDENTITY_COLUMNS + MODEL_INPUT_FEATURE_ORDER + LABEL_AND_SPLIT_COLUMNS
    model_input = training_frame.loc[:, export_columns].copy()
    _validate_feature_values(model_input, MODEL_INPUT_FEATURE_ORDER)

    for feature_name in QUARANTINED_FEATURES:
    ...
    summary = {
        ...
        "included_feature_order_hash": ordered_hash(MODEL_INPUT_FEATURE_ORDER),
        "source_feature_count": len(FEATURE_ORDER),
        "included_feature_count": len(MODEL_INPUT_FEATURE_ORDER),
        "quarantined_feature_count": len(QUARANTINED_FEATURES),
        "quarantined_features": list(QUARANTINED_FEATURES),
        "quarantine_reason": (
            "top3 basket weight source is marked placeholder_equal_weight; first model input excludes "
            "the dependent features until real monthly weights or an explicit experimental opt-in exists."
        ),
...
def write_model_input_outputs(
    *,
    ...
    feature_order_path.write_text("\n".join(MODEL_INPUT_FEATURE_ORDER), encoding="utf-8")
    ...
    feature_set_manifest = {
        ...
        "model_input_dataset_id": model_input_dataset_id,
        "source_training_dataset_id": source_training_dataset_id,
        "included_feature_order": MODEL_INPUT_FEATURE_ORDER,
    ...
    quarantine_manifest = {
        ...
        "reopen_condition": "real monthly top3 weights or explicit experimental opt-in decision",
    }
    quarantine_manifest_path.write_text(json.dumps(quarantine_manifest, indent=2), encoding="utf-8")

    debug_columns = ["timestamp", "label", "label_class", "split", *MODEL_INPUT_FEATURE_ORDER[:8]]
    ...
    run_manifest = {
        ...
        "lane": "evidence",
        "status": "reviewed",
        "command": "python foundation/pipelines/materialize_model_input_dataset.py",
        "inputs": [
        ...
        "judgment_boundary": (
            "Evidence run only. This materializes the first model-input dataset after quarantining "
            "placeholder top3 weight features; it does not train a model, prove alpha quality, close "
            "runtime authority, or claim operating promotion."
        ),
    ...
    (run_output_root / "result_summary.md").write_text(render_result_summary(run_summary), encoding="utf-8")
...
def render_result_summary(run_summary: dict[str, object]) -> str:
    lines = [
        "# Stage 04 Model-Input Feature Set v1",
        "",
        "This evidence run materialized the first model-input dataset after excluding placeholder top3 weight features.",
        "",
        "## Key Numbers",
        "",
        ...
        f"- rows: `{run_summary['rows']}`",
        f"- source_feature_count: `{run_summary['source_feature_count']}`",
        f"- included_feature_count: `{run_summary['included_feature_count']}`",
        f"- quarantined_features: `{', '.join(run_summary['quarantined_features'])}`",
        "",
        "## Boundary",
        "",
        "This is an evidence run, not model training, alpha quality, runtime authority, or operating promotion.",
    ]
    return "\n".join(lines) + "\n"
...
def materialize_model_input_dataset(
    *,
    ...
    model_input_frame, summary = build_model_input_dataset(training_frame)
...
def main() -> None:
    ...
    payload = materialize_model_input_dataset(
        source_training_dataset_id=args.source_training_dataset_id,
        model_input_dataset_id=args.model_input_dataset_id,
