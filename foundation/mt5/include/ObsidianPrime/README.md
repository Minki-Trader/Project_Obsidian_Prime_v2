...
| `RuntimeVetoTape.mqh` | timestamped veto tape(타임스탬프 차단 테이프), feature-side runtime block(피처 기반 런타임 차단) | Python proxy(Python 프록시)의 closed-bar veto(종료봉 차단)를 MT5 runtime probe(MT5 런타임 탐침)에 맞춰 재현 |
