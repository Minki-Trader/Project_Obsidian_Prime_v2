...
datetime g_target_windows[];
bool g_window_written[];
datetime g_last_seen_bar_close = 0;
int g_written_count = 0;
bool g_negative_written = false;
...
int FindPendingWindowCount()
  {
   int pending = 0;
   for(int i = 0; i < ArraySize(g_window_written); i++)
     {
      if(!g_window_written[i])
         pending++;
     }
   return pending;
  }
...
int OnInit()
  {
   ArrayResize(g_target_windows, 0);
   ArrayResize(g_window_written, 0);
   g_last_seen_bar_close = 0;
   g_written_count = 0;
   g_negative_written = false;

   if(!BuildTargetWindows(g_target_windows))
     {
      Print("No usable target windows were parsed.");
      return INIT_PARAMETERS_INCORRECT;
     }

   ArrayResize(g_window_written, ArraySize(g_target_windows));
   for(int i = 0; i < ArraySize(g_window_written); i++)
      g_window_written[i] = false;
   ...
   Print("ObsidianPrimeV2_RuntimeParityAuditEA initialized with ", ArraySize(g_target_windows), " pending window(s).");
   return INIT_SUCCEEDED;
  }

void OnTick()
  {
   datetime current_bar_open = iTime(InpMainSymbol, InpTimeframe, 0);
   if(current_bar_open <= 0)
      return;

   datetime closed_bar_close = current_bar_open;
   if(closed_bar_close <= g_last_seen_bar_close)
      return;
   g_last_seen_bar_close = closed_bar_close;

   for(int i = 0; i < ArraySize(g_target_windows); i++)
     {
      if(g_window_written[i])
         continue;
      if(g_target_windows[i] > closed_bar_close)
         continue;

      string line = "";
      if(!BuildSnapshotForWindow(g_target_windows[i], line))
        {
         Print("Failed to build snapshot for ", FormatUtc(g_target_windows[i]));
         g_window_written[i] = true;
         continue;
        }
      if(!AppendLine(line))
        {
         Print("Failed to append audit line for ", FormatUtc(g_target_windows[i]));
         return;
        }

      g_window_written[i] = true;
      g_written_count++;
      Print("Captured runtime parity window ", FormatUtc(g_target_windows[i]), " (", g_written_count, "/", ArraySize(g_target_windows), ").");
     }

   if(FindPendingWindowCount() == 0)
     {
      if(!g_negative_written)
        {
         string negative_line = "";
         if(!BuildSyntheticNegativeSnapshot(negative_line))
           {
            Print("Failed to build synthetic negative runtime parity row.");
            return;
           }
         if(!AppendLine(negative_line))
           {
            Print("Failed to append synthetic negative runtime parity row.");
            return;
           }
         g_negative_written = true;
         g_written_count++;
        }
      Print("ObsidianPrimeV2_RuntimeParityAudit wrote ", g_written_count, " line(s) to ", InpOutputPath);
      ExpertRemove();
     }
  }
