...
- latest_run_id(최신 실행 ID): `run50BV_stage56_baseline_adapter_cooldown_repair_v1`
- current run(현재 실행): `run50BV_stage56_baseline_adapter_cooldown_repair_v1`
- current_judgment(현재 판정): `adapter_repair_in_progress`
...
- repair_report(수리 보고서): `stages/56_base_engine__dense_tier_a_engine_and_tier_b_fallback_selection/03_reviews/run50BV_baseline_adapter_cooldown_repair_report.md`
- repair_summary_json(수리 요약 JSON): `stages/56_base_engine__dense_tier_a_engine_and_tier_b_fallback_selection/03_reviews/run50BV_baseline_adapter_cooldown_repair_summary.json`
- repair_summary_csv(수리 요약 CSV): `stages/56_base_engine__dense_tier_a_engine_and_tier_b_fallback_selection/03_reviews/run50BV_baseline_adapter_cooldown_repair_summary.csv`
- repair_risk_telemetry(수리 위험 텔레메트리): `stages/56_base_engine__dense_tier_a_engine_and_tier_b_fallback_selection/03_reviews/run50BV_baseline_adapter_cooldown_repair_risk_telemetry.csv`
