...
Stage15(15단계)는 아직 독립 탐색으로 다루지 않은 model learning methods(모델 학습법)를 알아가는 독립 주제였고, LDA(`Linear Discriminant Analysis`, 선형 판별 분석) covariance stability(공분산 안정성) runtime_probe(런타임 탐침)로 닫혔다.
...
효과(effect, 효과): Stage15(15단계)는 승자 선택(winner selection, 승자 선택)이 아니라, LDA(선형 판별 분석)가 이 데이터에서 무엇을 가르치는지 확인한 단계로 닫는다. QDA(`Quadratic Discriminant Analysis`, 이차 판별 분석)는 Stage16(16단계) 새 주제로 다룬다.
