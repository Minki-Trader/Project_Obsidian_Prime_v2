...
- current_run(현재 실행): `run364X_execute_density_side_balance_repair_mt5_runtime_probe_without_db_v1`
- latest_completed_run(최근 완료 실행): `run364W_package_density_side_balance_repair_runtime_probe_without_db_v1`
...
- selected_runtime_package_candidate(선택 런타임 패키지 후보): `dual_pshort_0_45__adx_block_40_0__maxhold_8`
- selected_proxy_density(선택 프록시 밀도): combined(합산) `3.2462462462`
- selected_proxy_long_short(선택 프록시 롱/숏): `952` / `129`
- package_status(패키지 상태): compile(컴파일) `completed`, portable_ea_sync(포터블 EA 동기화) `True`
