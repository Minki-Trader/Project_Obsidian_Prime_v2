...
- current_run(현재 실행): `run364BG_materialize_density_restore_forward_regime_stress_inputs_without_db_v1`
- latest_completed_run(최근 완료 실행): `run364BF_review_density_restore_stress_candidate_mt5_runtime_probe_without_db_v1`
...
- latest_mt5_net_pf_trades(최근 MT5 순수익/수익 팩터/거래수): `900.36` / `1.35` / `1016`
- latest_trade_density(최근 거래 밀도): `3.0510510511` per business day(영업일당), floor(하한) `3.0`
- latest_long_short(최근 롱/숏): `917` / `99`
- next_action(다음 행동): `run364BG_materialize_density_restore_forward_regime_stress_inputs_without_db_v1`
