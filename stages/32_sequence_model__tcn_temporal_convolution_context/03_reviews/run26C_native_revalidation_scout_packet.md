# Stage32 Native Scout(원본 탐색)

- run(실행): `run26C_torch_tcn_native_temporal_scout_v1`
- status(상태): `reviewed_native_structural_scout_completed`
- judgment(판정): `inconclusive_stage32_native_structural_scout_completed`
...
- Tier A separate(Tier A 분리): `stages/32_sequence_model__tcn_temporal_convolution_context/02_runs/run26C_torch_tcn_native_temporal_scout_v1/predictions/tier_a_stage32_native_structural_predictions.parquet`
- Tier B separate(Tier B 분리): `stages/32_sequence_model__tcn_temporal_convolution_context/02_runs/run26C_torch_tcn_native_temporal_scout_v1/predictions/tier_b_stage32_native_structural_predictions.parquet`
- Tier A+B combined(Tier A+B 합산): `stages/32_sequence_model__tcn_temporal_convolution_context/02_runs/run26C_torch_tcn_native_temporal_scout_v1/predictions/tier_ab_stage32_native_structural_predictions.parquet`
