...
- runtime_parity(런타임 동등성): out_of_scope_by_claim(주장 범위 밖), `runtime_probe_ineligible_no_scout_seed_or_runtime_candidate_after_f48_event_risk_sizing_proxy`.
- result_judgment(결과 판정): pass(통과), `negative_memory` only; weak positive PF(약한 양수 PF)는 scout threshold(탐색 임계값) 미만이면 near-miss alpha(근접 알파)가 아니라 negative_memory(부정 기억)다.
