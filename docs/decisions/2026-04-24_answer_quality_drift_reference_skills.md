...
필요한 material(재료)이 없을 때는 현재 프로젝트 경계(current project boundary, 현재 프로젝트 경계) 안에서 먼저 찾거나 재생성한다. legacy snapshot(레거시 스냅샷), 외부 폴더(external folder, 외부 폴더), 예전 프로젝트 산출물(old project output, 예전 프로젝트 산출물)은 사용자가 명시적으로 요청했을 때만 본다.

Codex가 현재 권한 안에서 material(재료), tool(도구), environment(환경)를 복구, 재생성, 설치, 설정, fetch(가져오기), 실행할 수 있으면 먼저 직접 한다. 사용자의 협조(user cooperation, 사용자 협조)가 필요한 경우에는 필요한 설치, 로그인, 계정 연결, 로컬 데이터 제공, 권한 부여를 정확히 요청하고, 해결 뒤 어떤 작업으로 복귀할지 함께 적는다.
...
- 필요한 재료가 없으면 현재 프로젝트 안에서 복구 또는 재생성할 최단 경로를 먼저 찾고, 외부/레거시는 기본 fallback(대체 경로)로 쓰지 않는다.
- Codex가 직접 풀 수 있는 blocker(차단 지점)는 직접 풀고, 사용자 협조가 필요한 blocker는 정확히 요청한 뒤 원래 product(산출물) 작업으로 돌아간다.
