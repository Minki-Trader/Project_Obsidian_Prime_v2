...
- adapter_under_review(검토 중 어댑터): `shared_weakness_breakout_second_followup_or_prune_materialization`
...
- run267DH_shared_weakness_breakout_second_followup_or_prune_materialization(267DH 공유 약점 2차 후속/가지치기 물질화): `stages/267_adapter_research__baseline_candidate_racing_protocol/03_reviews/stage267_run267DH_shared_weakness_breakout_second_followup_or_prune_materialization.md`
  Effect(효과): run267DF(267DF 실행)의 balance/time-slice/trade-quality(잔액/시간구간/거래품질) 근거를 branch decisions(분기 판단) `5`개, materialization queue(물질화 대기열) `6`개, prune matrix(가지치기 행렬) `6`개, failure memory(실패 기억) `6`개로 바꿨고, selected candidate(선택 후보), selected research baseline(선택 연구 기준선), ONNX readiness(ONNX 준비), Goal Achieve(목표 달성)는 주장하지 않는다.
