...
- active_stage: `05_feature_integrity__formula_time_alignment_leakage_audit`
- active_branch: `main`

## 쉬운 설명(Plain Read, 쉬운 설명)

프로젝트는 깨끗한 단계 구조(clean stage structure, 깨끗한 단계 구조)로 다시 시작한 뒤, shared feature frame freeze(공유 피처 프레임 동결 산출물), training label/split(학습 라벨/분할), Stage 04(4단계) model input(모델 입력) 준비까지 닫았다.
...
Stage 05(5단계)의 질문(question, 질문)은 Stage 04(4단계)에서 선택한 58 feature(58개 피처) model input(모델 입력)이 수식(formula, 수식), 시간 정렬(time alignment, 시간 정렬), 외부 정렬(external alignment, 외부 정렬), 라벨 누수(label leakage, 라벨 누수) 관점에서 첫 학습 전에 감사될 수 있는가다.

Stage 05(5단계)의 감사 입력(audit input, 감사 입력)은 다음이다.

- `docs/contracts/top3_price_proxy_weight_contract_fpmarkets_v2.md`
- `foundation/config/top3_monthly_price_proxy_weights_fpmarkets_v2.csv`
- `data/processed/datasets/dataset_fpmarkets_v2_us100_m5_20220901_20260413_cashopen_fullcash_proxyw58/dataset_summary.json`
- `data/processed/model_inputs/label_v1_fwd12_split_v1_feature_set_v2_mt5_price_proxy_58/model_input_summary.json`

효과(effect, 효과): 첫 학습 전에 “피처가 계산되고 정렬된 방식”을 따로 검토한다. 이 단계는 모델 성능(model quality, 모델 품질)을 아직 주장하지 않는다.

## Pre-Alpha Stage Queue(알파 전 단계 대기열)

model-backed alpha exploration(모델 근거 알파 탐색) 전에 남은 작업은 Stage 05~09(5~9단계)다.
...
지금 닫힌 주장은 MT5 price-proxy monthly top3 weights(MT5 가격 대리 월별 top3 가중치)와 58 feature(58개 피처) model input(모델 입력)이 물질화됐다는 evidence claim(근거 주장)뿐이다.
