...
- run325A_summary(325A 요약): ONNX export(온엑스 내보내기), Python parity(파이썬 동등성), feature order parity(피처 순서 동등성), MT5 runtime reproduction(MT5 런타임 재현)을 `completed`로 기록했다. Effect(효과): attempts(시도) `6`개와 MT5 KPI records(MT5 핵심 성과 지표 기록) `6`개를 cp322A 후보 패키지 근거로 묶고, main push(메인 푸시) 전까지 Goal Achieve(목표 달성)는 `complete_pending_main_push`로 둔다.
