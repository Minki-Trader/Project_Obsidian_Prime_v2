...
- active_branch: `codex/alpha-stage-transition-policy(알파 단계 전환 정책 브랜치)`
...
Stage 12(12단계)의 닫힌 질문(question, 질문)은 Stage 10/11(10/11단계)을 계승하지 않고 ExtraTrees(엑스트라 트리) model family(모델 계열)를 standalone(단독)으로 학습하면 어떤 구조 신호가 나오는가다.

효과(effect, 효과): Stage 12(12단계)는 Stage 10(10단계)의 threshold/session(임계값/세션), Stage 11(11단계)의 LightGBM/context surface(라이트GBM/문맥 표면)를 쓰지 않고 새 모델 자체의 학습 효과(training effect, 학습 효과)를 분리해서 봤다. 마감(closeout, 마감)은 기준선(baseline, 기준선) 선택이 아니라 topic pivot(주제 전환) 준비다.
