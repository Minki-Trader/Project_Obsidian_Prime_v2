...
- active_branch: `codex/clean-stage-restart`
...
1. 심볼(symbol, 심볼)별 원천 `M5` 바(raw M5 bars, 원천 M5 봉)를 조사한다.
2. 첫 깨끗한 피처 프레임(feature frame, 피처 프레임)에 쓸 수 있는 파일을 확인한다.
3. 실제 재고(inventory, 재고)가 나온 뒤 첫 학습 데이터셋(training dataset, 학습 데이터셋)을 정한다.
