...
- current_packet(현재 작업 묶음): `stage56_run50BD_context_timed_quality_gated_slot_v1`
...
- latest_batch(최신 묶음): `run50BD_stage56_context_timed_quality_gated_slot_v1`
...
- run50BD judgment(실행50BD 판정): selected_research_baseline(선택 연구 기준선)은 `none`이다. Effect(효과): failure_reasons(실패 사유) `oos_net_positive;oos_pf;cost_stressed_expectancy;same_move_density` 때문에 hard condition(강한 완료 조건)을 통과하지 못했다.
