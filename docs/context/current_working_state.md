...
- 현재 실행(current run, 현재 실행): `run03E_et_batch20_top_v11_mt5_probe_v1`.
...
- 최신 MT5 런타임 탐침(latest MT5 runtime probe, 최신 MT5 런타임 탐침): `run03E_et_batch20_top_v11_mt5_probe_v1`.
- 검증 대상(target, 대상): `v11_base_leaf20_q85`.
- Python 검증/표본외 적중(Python validation/OOS hit, 파이썬 검증/표본외 적중): `0.392688` / `0.457317`.
- MT5 검증 net/PF(MT5 validation net/PF, MT5 검증 순손익/수익팩터): `-205.14` / `0.88`.
- MT5 표본외 net/PF(MT5 OOS net/PF, MT5 표본외 순손익/수익팩터): `362.83` / `1.44`.
...
Stage 10(10단계) `run01Y/run01Z/run01AA/run01AB/run01AC(실행 01Y/01Z/01AA/01AB/01AC)`를 실행했다는 뜻은 200~220 closeout runtime_probe(마감 런타임 탐침)를 완료했다는 뜻이다. Stage 11(11단계) `RUN02A~RUN02S(실행 02A~02S)`는 LightGBM(라이트GBM) 학습방법, LGBM-specific threshold(LGBM 전용 임계값), 발산형/아이디어 무더기/회수 확장 runtime_probe(런타임 탐침)를 완료했다는 뜻이다. `RUN02T~RUN02V(실행 02T~02V)`와 `RUN02X~RUN02Y(실행 02X~02Y)`는 Python structural probe(파이썬 구조 탐침)이고, `RUN02W/RUN02Z/RUN02AA~RUN02AK(실행 02W/02Z/02AA~02AK)`는 MT5 runtime_probe(MT5 런타임 탐침) 또는 그 인계물이다. Stage 12(12단계) `RUN03D(실행 03D)`는 standalone batch20 Python structural scout(단독 20개 묶음 파이썬 구조 탐색)이고, `RUN03E(실행 03E)`는 그 최상위 변형의 standalone MT5 runtime_probe(단독 MT5 런타임 탐침)이다. 아직 full WFO(전체 워크포워드 최적화) 결과, alpha quality(알파 품질), operating_promotion(운영 승격)은 없다.
...
- Stage 12(12단계)는 열려 있고, `RUN03E(실행 03E)` ExtraTrees(엑스트라 트리) batch20 top MT5 runtime_probe(20개 묶음 상위 변형 MT5 런타임 탐침)가 현재 실행이다.
...
- `kpi_rebuild_inventory_v1/kpi_rebuild_mt5_recording_v1/kpi_rebuild_mt5_execution_v1/kpi_trade_attribution_v1(KPI 재구축 목록/MT5 기록/MT5 실행/거래 귀속 묶음)`은 현재 교차 단계 KPI 증거다.
