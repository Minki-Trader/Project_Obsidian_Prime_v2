...
- current_packet(현재 작업 묶음): `stage56_run50AY_agreement_firewall_density_recovery_v1`
...
- latest_batch(최신 묶음): `run50AY_stage56_agreement_firewall_density_recovery_v1`
...
- run50AY judgment(실행50AY 판정): selected_research_baseline(선택 연구 기준선)은 `none`이다. Effect(효과): failure_reasons(실패 사유) `oos_density;validation_net_positive;validation_pf;cost_stressed_expectancy;same_move_density` 때문에 hard condition(강한 조건)을 넘지 못한다.
