...
- 현재 실행(current run, 현재 실행): `run03D_et_standalone_batch20_v1`.
...
- 패키지 결과(package result, 패키지 결과): 20개 변형(variant, 변형) 완료, 최상위 `v11_base_leaf20_q85`.
- 최상위 검증/표본외 적중(best validation/OOS hit, 최상위 검증/표본외 적중): `0.392688` / `0.457317`.
- 판정(judgment, 판정): `inconclusive_standalone_batch20_structural_scout`.
...
Stage 10(10단계) `run01Y/run01Z/run01AA/run01AB/run01AC(실행 01Y/01Z/01AA/01AB/01AC)`를 실행했다는 뜻은 200~220 closeout runtime_probe(마감 런타임 탐침)를 완료했다는 뜻이다. Stage 11(11단계) `RUN02A~RUN02S(실행 02A~02S)`는 LightGBM(라이트GBM) 학습방법, LGBM-specific threshold(LGBM 전용 임계값), 발산형/아이디어 무더기/회수 확장 runtime_probe(런타임 탐침)를 완료했다는 뜻이다. `RUN02T~RUN02V(실행 02T~02V)`와 `RUN02X~RUN02Y(실행 02X~02Y)`는 Python structural probe(파이썬 구조 탐침)이고, `RUN02W/RUN02Z/RUN02AA~RUN02AK(실행 02W/02Z/02AA~02AK)`는 MT5 runtime_probe(MT5 런타임 탐침) 또는 그 인계물이다. Stage 12(12단계) `RUN03B(실행 03B)`는 ExtraTrees(엑스트라 트리) standalone Python structural scout(단독 파이썬 구조 탐침)이고, `RUN03C(실행 03C)`는 그 모델의 standalone MT5 runtime_probe(단독 MT5 런타임 탐침)이며, `RUN03D(실행 03D)`는 standalone batch20 Python structural scout(단독 20개 묶음 파이썬 구조 탐색)이다. 아직 full WFO(전체 워크포워드 최적화) 결과, alpha quality(알파 품질), operating_promotion(운영 승격)은 없다.

## 닫힌 기억(Closed Memory, 닫힌 기억)

- Stage 12(12단계)는 열려 있고, `RUN03D(실행 03D)` ExtraTrees(엑스트라 트리) batch20 Python structural scout(20개 묶음 파이썬 구조 탐색)가 현재 실행이다.
- `RUN03C(실행 03C)` ExtraTrees(엑스트라 트리)는 최신 standalone MT5 runtime_probe(단독 MT5 런타임 탐침)다.
