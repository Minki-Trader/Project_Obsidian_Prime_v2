# run23B Runtime Probe Packet(run23B 런타임 탐침 묶음)

## Judgment(판정)

- run(실행): `run23B_river_online_drift_runtime_probe_v1`
- status(상태): `reviewed_runtime_probe_completed`
- judgment(판정): `inconclusive_stage29_runtime_probe_completed`
...
효과(effect, 효과): Stage29(29단계) topic surface(주제 표면)를 MT5 score-table handoff(MT5 점수표 인계)로 관찰한다. native package runtime authority(원본 패키지 런타임 권위)는 주장하지 않는다.
...
|---|---:|---:|---:|---:|
| validation routed(검증 라우팅) | `-96.55` | `0.95` | `1081` | `334.47` |
| OOS routed(표본외 라우팅) | `319.2` | `1.22` | `916` | `154.56` |
